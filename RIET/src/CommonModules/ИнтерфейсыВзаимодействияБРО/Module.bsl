...
Функция РеглОтчетыИсполнителиТребований() Экспорт
	...
	ИсполнителиТребований.Вставить("ПлатаЗаНегативноеВоздействиеНаОкружающуюСреду", "РегламентированныйОтчетРасчетПлатыОкрСредаСвод");
	ИсполнителиТребований.Вставить("ОтходыМСП",                                     "РегламентированныйОтчетОтходыСубъектовМСП");
	ИсполнителиТребований.Вставить("2ТПОтходы",                                     "РегламентированныйОтчетФорма2ТПОтходы");
...
Функция ОписаниеФормыРеглОтчета(Требование, Организация, НачалоПериода, КонецПериода, Периодичность) Экспорт
	...
	ЭкземплярыРеглОтчета = РегламентированнаяОтчетность.ПолучитьСсылкуНаРеглОтчет(ИмяРеглОтчета, Организация, НачалоДня(КонецПериода));
...
Функция ДанныеРеглОтчетаДляУплатыНалога(Требование, Организация, ПериодДекларации) Экспорт
	...
	ЭкземплярыРеглОтчета = РегламентированнаяОтчетность.ПолучитьСсылкуНаРеглОтчет(ИмяРеглОтчета, Организация, НачалоДня(ПериодДекларации));
...
Процедура ОбновитьДанныеРеглОтчета(ИсточникОтчета, Организация, ДатаНачала, ДатаОкончания, СсылкаНаСохрРеглОтчет) Экспорт
	...
	Если ИмяФормыОтчета = Неопределено Тогда
		...
		Сообщение.Текст = ВернутьСтр("ru='Не удалось обновить данные отчета!'");
		Сообщение.Сообщить();
...
Функция СформироватьАвтоматическиРеглОтчет(ИсточникОтчета, Организация, ДатаНачала, ДатаОкончания, КорректирующийОтчет = Ложь) Экспорт
	...
	Если ИмяФормыОтчета = Неопределено Тогда
		...
		Сообщение = Новый СообщениеПользователю;
		Сообщение.Текст = ВернутьСтр("ru='Не удалось сформировать отчет!'");
		Сообщение.Сообщить();
...
Функция ЗаписатьРеглОтчетРСВ1ФормаОтчета2010Кв4(ИсточникОтчета, ИмяФормыОтчета, Организация, ДатаНачала, ДатаОкончания, РазделыОтчета, ДанныеРеглОтчета, СсылкаНаСохрРеглОтчет = Неопределено)
	...
	Исключение
		...
		Сообщение.Текст = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(ВернутьСтр("ru='%1'"), РегламентированнаяОтчетностьКлиентСервер.СформироватьТекстСообщения(ОписаниеОшибки()));
...
Функция ЗаписатьРеглОтчетРСВ1ФормаОтчета2014Кв1(ИсточникОтчета, ИмяФормыОтчета, Организация, ДатаНачала, ДатаОкончания, РазделыОтчета, ДанныеРеглОтчета, СсылкаНаСохрРеглОтчет = Неопределено)
	...
	Исключение
		...
		Сообщение.Текст = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(ВернутьСтр("ru='%1'"),
