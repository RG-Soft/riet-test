...
Процедура ПриДобавленииОбработчиковУстановкиПараметровСеанса(Обработчики) Экспорт
	...
	// RG-Soft
	// Инициализация наших параметров сеанса.
	
	Обработчики.Вставить("ТекущийПользователь", "РГСофт.УстановитьПараметрСеансаТекущийПользователь");
	Обработчики.Вставить("ГруппыТекущегоПользователя", "ПолныеПрава.ОпределитьГруппыТекущегоПользователя");
	
	Обработчики.Вставить("увГруппыТекущегоПользователя", "увМониторСопровождения.увУстановитьПараметрСеансаУвГруппыТекущегоПользователя");
	
	Обработчики.Вставить("ИспользованиеРИБ", "ПолныеПрава.ОпределитьФактИспользованияРИБ");
	
	Обработчики.Вставить("УчетнаяПолитикаПоБухгалтерскомуУчету", "РГСофт.УстановитьПараметрСеансаУчетнаяПолитикаПоБухгалтерскомуУчету");
	Обработчики.Вставить("УчетнаяПолитикаПоНалоговомуУчету", "РГСофт.УстановитьПараметрСеансаУчетнаяПолитикаПоНалоговомуУчету");
	
	Обработчики.Вставить("CCA", "CustomsСервер.УстановитьПараметрСеансаCCA");
	Обработчики.Вставить("УДАЛИТЬСахалин", "CustomsСервер.УстановитьПараметрСеансаСахалин");
	
	//добавила Наталья Петроченко 09.07.2015 для Local distribution for non-lawson
	Обработчики.Вставить("ShowNamesAndDescriptionsRUS", "LocalDistributionForNonLawsonСервер.УстановитьПараметрСеансаShowNamesAndDescriptionsRUS");
	
	//добавила Наталья Петроченко 11.12.2012 для Job Log
	Обработчики.Вставить("UserSegments", "Документы.Job.УстановитьПараметрUserSegments");
	
	//Обновление рег. Отчетности на бух. Корп 3.0.36.18
	Обработчики.Вставить("ПараметрыВнешнихРегламентированныхОтчетов", "РГСофт.УстановитьПараметрСеансаПараметрыВнешнихРегламентированныхОтчетов");	
	//<=
...
// ЗАЧЕМ ДОБАВИЛ В ТИПОВОЙ МОДУЛЬ??? ОБНОВЛЯТЬ БСП СЛОЖНЕЕ!!!
