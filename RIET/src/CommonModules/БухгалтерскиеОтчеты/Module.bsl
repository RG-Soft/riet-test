...
Процедура ПроизвестиПроверкуНаВедениеОднофирменности(Организация) Экспорт
	...
	Если НЕ ОбщегоНазначения.ЗначениеНеЗаполнено(ОргПоУмолчанию) Тогда
		Организация = ОргПоУмолчанию;
	КонецЕсли;
    
КонецПроцедуры

//Процедура назначает форме уникальный ключ идентификации
// для возможности открытия нескольких одинаковых форм
...
Процедура РедактированиеСубконто(Элемент, СписокСубконто) Экспорт
...
Процедура ОбработчикПриИзмененииСчета(ТабличноеПоле, Знач ТабПолеРазвернутогоСальдо) Экспорт
	...
	Если Не ОбщегоНазначения.ЗначениеНеЗаполнено(Счет) Тогда
...
Процедура ВывестиПредставленияСубконтоТекущейСтрокиВМакет(Знач ПрефиксНаименованияКолонки, Проводки, Знач КоличествоСубконтоСчета, 
	...
	Для Индекс = 0 По КоличествоСубконтоСчета - 1 Цикл
		...
		ОбластьМакет.Параметры.Содержание = ?(ОбщегоНазначения.ЗначениеНеЗаполнено(Проводки[ПрефиксНаименованияКолонки + Строка(Индекс + 1)])
			, "<...>"
...
Функция ВернутьСубконтоПоИмени(Знач Счет, Знач ИмяСубконто) Экспорт
...
Функция ОпределитьМожноИспользоватьСоставнойЗапросПоСубконто(ПравилаСчета) Экспорт
...
Процедура ДополнитьСтрокуОграниченийПоРеквизитам(СтрокаОграничения, Знач ИмяОграниченияПоРеквизиту, Знач ОграничениеПоРеквизиту) Экспорт
	...
	Если ОбщегоНазначения.ЗначениеНеЗаполнено(ОграничениеПоРеквизиту) Тогда
		Возврат;
	КонецЕсли;
...
//	Таблица запросов
//
Функция СформироватьТаблицуДанныхРазворотаСчетов(СтруктураПараметров, Знач ПравилаВыводаИтогов) Экспорт
...
//	Таблица запросов
//
Функция СформироватьТаблицуДанныхРазвернутогоСальдо(СтруктураПараметров, ПравилаРазвернутогоСальдо) Экспорт
...
Процедура ИнициализироватьСтруктуруХраненияИтоговОборотка(Знач МассивПоказателей, СтруктураОбщийИтог) Экспорт
...
//
Процедура ВывестиПоказателиОборотноСальдовойВедомости(ДокументРезультат, Выборка, Счет, СтрокаРазвернутогоСальдо, СтруктураОбщийИтог, СтруктураПараметров) Экспорт
...
Процедура ВывестиВложеннуюГруппировкуОборотноСальдовойВедомости(Выборка, Знач ИндексТекущейГруппировки, МассивГруппировок, Уровень, СдвигУровня, 
	СброситьСдвигУровня, ПоследнийВыведенныйСчет, СтрокаРазвернутогоСальдо, СтруктураОбщийИтог, СтруктураПараметров, ОтборСубконто) Экспорт
	...
	ВывестиГруппировкуОборотноСальдовойВедомости(Выборка.Выбрать(ОбходРезультатаЗапроса.ПоГруппировкам, МассивГруппировок[ИндексТекущейГруппировки + 1]), 
						...
						СтрокаРазвернутогоСальдо, СтруктураОбщийИтог, СтруктураПараметров, ОтборСубконто);
...
Процедура ВывестиДанныеПоВалютамОборотноСальдовойВедомости(СтруктураПараметров, ОбластьСтрокаПоказателяСчет, Выборка, 
	ДокументРезультат, СтрокаРазвернутогоСальдо, СтруктураОбщийИтог, Знач Отступ) Экспорт
...
Процедура ЗаполнитьПараметрыРасшифровкиОборотноСальдовойВедомости(Знач ДатаНач, Знач ДатаКон, Знач ИмяРегистраБухгалтерии,
	Область, Выборка, ОтборСубконто = Неопределено, ВыведенныйСчет = Неопределено, Знач ВВалютеСценария = Неопределено) Экспорт
...
Процедура ДополнитьОтборОтчетаДополнительнымиПараметрами(ОсновнойОтбор, ДопРасшифровка) Экспорт
...
Процедура ДокументРезультатОбработкаРасшифровкиОборотноСальдовойВедемости(Элемент, Расшифровка, 
...
Функция ЗначениеПоляСоответствуетОтбору(Выборка, Знач СторонаПроводки, ПостроительОтчета) Экспорт
	...
	Для Каждого ЭлементОтбора Из ПостроительОтчета.Отбор Цикл
		...
		ИначеЕсли ЭлементОтбора.ВидСравнения = ВидСравнения.ВИерархии Тогда
			...
			Результат = (ЗначениеПоля = ЭлементОтбора.Значение)
				ИЛИ (ЗначениеПоля.ПринадлежитЭлементу(ЭлементОтбора.Значение));
		...
		ИначеЕсли ЭлементОтбора.ВидСравнения = ВидСравнения.ВСпискеПоИерархии Тогда
			...
			Для Каждого Элемент Из ЭлементОтбора.Значение Цикл
				...
				Если (ЗначениеПоля = Элемент.Значение)
					ИЛИ (ЗначениеПоля.ПринадлежитЭлементу(Элемент.Значение)) Тогда
		...
		ИначеЕсли ЭлементОтбора.ВидСравнения = ВидСравнения.НеВИерархии Тогда
			...
			Результат = Не ((ЗначениеПоля = ЭлементОтбора.Значение)
				ИЛИ (ЗначениеПоля.ПринадлежитЭлементу(ЭлементОтбора.Значение)));
		...
		ИначеЕсли ЭлементОтбора.ВидСравнения = ВидСравнения.НеВСпискеПоИерархии Тогда
			...
			Для Каждого Элемент Из ЭлементОтбора.Значение Цикл
				...
				Если (ЗначениеПоля = ЭлементОтбора.Значение)
					ИЛИ (ЗначениеПоля.ПринадлежитЭлементу(ЭлементОтбора.Значение)) Тогда
					ПромРезультат = Истина;
					Прервать;
				КонецЕсли;
		...
		ИначеЕсли ЭлементОтбора.ВидСравнения = ВидСравнения.НеСодержит Тогда
			...
			ШаблонСтроки = СтрЗаменить(ШаблонСтроки, "%", "");
			ШаблонСтроки = СтрЗаменить(ШаблонСтроки, "[|||]", "%");
			Результат = СтрНайти(Строка(ЗначениеПоля), ШаблонСтроки) = 0;
...
Функция ПолучитьСтрокуФорматаПериода(Знач Период) Экспорт
	...
	ФорматПериода = "";
	Если НЕ ОбщегоНазначения.ЗначениеНеЗаполнено(Период) Тогда
...
//Процедура выводит заголовки для отчета обороты счета
Процедура ВывестиЗаголовкиОборотовОтчетаОборотыСчета(Макет, Знач ИмяОбластиВывода, Знач Выборка, 
	Знач ДокументРезультат, Знач ПоСубсчетам, Знач ИмяПоляНаличияОборота, СоотвСчетов, Знач ДополнениеКУровнюВывода = 0) Экспорт
	
	// вывод заголовков для корреспондирующих счетов
	НайденПустойЭлемент = Ложь;
	...
	Пока Выборка.Следующий() Цикл
		...
		Если Выборка.КорСчет <> Неопределено
			И Выборка.КорСчет.Пустая() Тогда
			
			Если НайденПустойЭлемент Тогда
				Продолжить;
			КонецЕсли;
			
			НайденПустойЭлемент = Истина;
		...
		Если Выборка[ИмяПоляНаличияОборота] <> 0 Тогда
			СоотвСчетов.Вставить(Выборка.КорСчет, 0);
...
Процедура ВывестиПолныйЗаголовокОтчетаОборотовСчета(Выборка = Неопределено, ШиринаТаблицы, ОбъектОтчет, Макет, ДокументРезультат,
	РезультатЗапроса, Знач ИмяСтроки, СоотвДт, СоотвКт, Знач ДополнениеКУровнюВывода = 0, Знач ВыборкаИтогов = Неопределено, 
	...
	Если ОбъектОтчет.ОборотДтКорСчета Тогда
		...
		ВывестиЗаголовкиОборотовОтчетаОборотыСчета(Макет, ИмяСтроки+"|ОборотДтКорсчет", Выборка, 
			ДокументРезультат, ОбъектОтчет.ПоСубсчетамКорСчетов, "ЕстьСуммаОборотДт", СоотвДт, ДополнениеКУровнюВывода);
	...
	Если ОбъектОтчет.ОборотКтКорСчета Тогда
		...
		ВывестиЗаголовкиОборотовОтчетаОборотыСчета(Макет, ИмяСтроки+"|ОборотКтКорсчет", Выборка, 
			ДокументРезультат, ОбъектОтчет.ПоСубсчетамКорСчетов, "ЕстьСуммаОборотКт", СоотвКт, ДополнениеКУровнюВывода);
...
Функция ПолучитьВидОтбораПоТипуЗначения(ЗначениеОтбора)
	...
	Если (ЗначениеОтбора = NULL)	
		ИЛИ ОбщегоНазначения.ЗначениеНеЗаполнено(ЗначениеОтбора) Тогда
	...
	Попытка
		...
		Если МетаданныеОбъекта<>Неопределено Тогда
			...
			Если Метаданные.Справочники.Найти(МетаданныеОбъекта.Имя) <> Неопределено 
...
Процедура СоздатьКолонкиОграниченийДляКорреспонденцииПданаСчетов(Корреспонденции, Знач ИмяПланаСчетов) Экспорт
...
Функция ОпределитьНеобходимоПропуститьСтрокуПриВыводеДанных(Выборка, Знач ПоПодстрокеСодержание, Знач ПоПодстрокеСубконто,
	...
	Если ПоПодстрокеСодержание Тогда
		...
		Если БезУчетаРегистра Тогда
			СтрокаНайдена = СтрНайти(НРег(Выборка.Содержание), ПодстрокаПоиска) > 0;
		Иначе
			СтрокаНайдена = СтрНайти(Выборка.Содержание, ПодстрокаПоиска) > 0;
		КонецЕсли;
	...
	Если ПоПодстрокеСубконто Тогда
		...
		Для Сч = 1 По МаксКоличествоСубконто Цикл
			...
			Если СтрНайти(СубконтоДт, ПодстрокаПоиска) > 0
				Или СтрНайти(СубконтоКт, ПодстрокаПоиска) > 0 Тогда
...
Процедура ДополнитьОтборОчереднойСтрокой(ОтборДанных, ТаблицаОтборов, Знач СтрокаДБКТ, Знач Счет) Экспорт
	...
	Для каждого Элемент Из ОтборДанных Цикл
		...
		КонецНомера = СтрНайти(СтрокаПоискаНомера, ".");
...
Функция ДобавитьВТаблицуОчереднойОтбор(ТаблицаОтбора, Знач Имя, Знач Значение, Знач ВидСравненияПоУмолчанию = Неопределено) Экспорт
	...
	Если ВидСравненияПоУмолчанию = Неопределено Тогда
		СтрокаДобавления.ВидСравнения = ?(ТипЗнч(Значение) = Тип("СписокЗначений") ИЛИ ТипЗнч(Значение) = Тип("Массив")
			, ВидСравнения.ВСписке, ВидСравнения.Равно);
	Иначе
...
Процедура НастроитьОтборыДанныхПопараметрам(Параметры, Корреспонденции, Валюта, ОтборПоВалюте) Экспорт
	...
	Если ТипЗнч(СтрокиОтбора) = Тип("Соответствие")
		...
		Для каждого ЭлементОтбора Из СтрокиОтбора Цикл
			...
			ИначеЕсли ВРег(ЭлементОтбора.Ключ) = "ВАЛЮТА" и ОбщегоНазначения.ЗначениеНеЗаполнено(Валюта) Тогда
				
				Валюта = ЭлементОтбора.Значение;
				ОтборПоВалюте = Не ОбщегоНазначения.ЗначениеНеЗаполнено(Валюта);
	...
	Если Не ОбщегоНазначения.ЗначениеНеЗаполнено(СчетДт) или Не ОбщегоНазначения.ЗначениеНеЗаполнено(СчетКт) Тогда
		стр = Корреспонденции.Добавить();
		
		стр.Субконто = СоздатьТаблицуДляХраненияОтбора();
		
		// заполним фильтры по дебетовой аналитике
		Если Не ОбщегоНазначения.ЗначениеНеЗаполнено(СчетДт) Тогда
			стр.СчетДт = СчетДт;
			ДополнитьОтборОчереднойСтрокой(ОтборСубконтоДт, стр.Субконто, "Дт", СчетДт);							
		КонецЕсли;
		
		// заполним фильтры по кредитовой аналитике
		Если Не ОбщегоНазначения.ЗначениеНеЗаполнено(СчетКт) Тогда
			стр.СчетКт = СчетКт;
...
Процедура УдалитьНеЗаполненныеЭлементыМассива(МассивЭлементов) Экспорт
	...
	Для н=1 По Колво Цикл
		Если ОбщегоНазначения.ЗначениеНеЗаполнено(МассивЭлементов[Колво-н]) Тогда
...
Процедура УдалитьПустыеСубконтоИзТабличнойЧасти(ТабЧастьСубконто) Экспорт
	...
	Пока СчСубконто >= 0 Цикл
		...
		Если ОбщегоНазначения.ЗначениеНеЗаполнено(ТабЧастьСубконто[СчСубконто].ВидСубконто) Тогда
