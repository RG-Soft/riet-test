...
Процедура ДобавитьУсловиеФильтра(Фильтр, Знач Свойство, Знач ВидСравненияЗначений, Знач Значение) Экспорт
	...
	Иначе
		...
		ВызватьИсключение ВернутьСтр("ru = 'Некорректно задано описание фильтра!'");
...
Функция ПроверитьУсловияФильтра(Знач ПроверяемыйОбъект, Знач Фильтр = Неопределено) Экспорт
	...
	Для Каждого Условие Из Фильтр Цикл
		...
		Иначе
			...
			ВызватьИсключение ВернутьСтр("ru = 'Некорректно задан фильтр!'");
...
Функция МенеджерАдминистрирования(Знач ПараметрыАдминистрирования)
	...
	Иначе
		...
		ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(ВернутьСтр("ru = 'Неизвестный тип подключения: %1!'"), ПараметрыАдминистрирования.ТипПодключения);
