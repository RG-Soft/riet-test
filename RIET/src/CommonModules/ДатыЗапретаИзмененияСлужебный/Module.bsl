...
// Для функции ИзменениеИлиЗагрузкаЗапрещена.
Функция ДанныеДляПроверкиИзОбъекта(Данные, ДействующиеДаты, ДатыЗапретаПоОбъектамНеУказаны) Экспорт
...
Функция ЗначениеПоля(ЗначенияПолей, Поле)
	...
	// { RGS AKozhevnikova - для РТиУ запрет по дате проведения
	Если ТипЗнч(ЗначенияПолей) = Тип("ДокументОбъект.РеализацияТоваровУслуг") И Поле.Имя = "Дата" Тогда
		Возврат ЗначенияПолей["ДатаПроведения"];
	КонецЕсли;	
	// } RGS AKozhevnikova
