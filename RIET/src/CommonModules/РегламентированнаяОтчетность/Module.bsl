...
Процедура Автозаполнение_РеестрПоНДС_Приложение5(ИДОтчета, ИДРедакцииОтчета, ПараметрыОтчета, Контейнер) Экспорт
	...
	ИсходныйМакет = Отчеты[ИДОтчета].ПолучитьМакет(ИДРедакцииОтчета + "_" + ИмяРаздела);
...
/////////////////////////////////////////////////////////////
//ВЕРСИЯ ПРОЦЕДУР И ФУНКЦИЙ МОДУЛЯ Примерно 3.0.43.50
//ОБЛАСТЬ СТАРОГО КОДА ДЛЯ ШЛЮМА "ДляСовместимостиРГСофт"
...
Функция ПриЗаписиРегламентированногоОтчетаНаСервере(Форма, КодИФНС = "не применимо", Автосохранение = Ложь, Вариант = Неопределено, ВидДокументаНомерКорректировкиИзменен, мСохраненныйДок) Экспорт
...
Процедура ПереносДанныхЭДОПриОбновлении10158() Экспорт
	...
	ЗаполнитьРегистрацииВНалоговомОргане10158();
...
Процедура ЗаполнитьРегистрацииВНалоговомОргане10158()
	
	Выборка = Справочники.РегистрацииВНалоговомОргане.Выбрать();
	
	Пока Выборка.Следующий() Цикл
		
		Объект = Выборка.ПолучитьОбъект();
		
		Объект.Доверенность = Объект.УдалитьДоверенность;
...
Процедура ЗарегистрироватьПоказательДляРасшифровки(СписокРасшифровкиПоказателей, ИмяПоказателя, ПредставлениеПоказателя,
	ЗначениеПоказателя, ФормулаПоказателя, СсылкаНаДокументРО, ИмяСтраницы, ДополнительныеВозможности = Неопределено,
	НомерСтраницыДляМногостраничныхРазделов = Неопределено, СчетчикФормулДляРасшифровок) Экспорт
...
Процедура ЗарегистрироватьФормулуДляРасшифровки(СписокРасшифровкиФормулы, ТекстовоеПредставлениеФормулы, 
	ТребуемыйНомерДляСоставныхФормул = Неопределено, СчетчикФормулДляРасшифровок) Экспорт
...
Процедура ПолучитьТаблицуПараметровИБ(ТаблицаПараметров) Экспорт
	...
	//Обновление рег. Отчетности на бух. Корп 3.0.38.43
...
Функция КодыДляИмениФайлаВФорматеВыгрузки(Параметры) Экспорт
	...
	Если ФорматВыгрузкиСтатистическойОтчетностиВЭлектронномВиде(Параметры["Документ"]["Организация"]) = 2 Тогда // "новый" формат выгрузки
		...
		Если СтрДлина(Параметры["ОтчетныйПериод"]) = 4 И Лев(Параметры["ОтчетныйПериод"], 1) = "0" Тогда
			Параметры["ОтчетныйПериод"] = Сред(Параметры["ОтчетныйПериод"], 2);
		КонецЕсли;
...
Процедура УстановитьЦветФорматДоступностьЯчеек(Форма) Экспорт 
	...
	МакетСоставаПоказателей = Отчеты[Сред(Лев(Форма.ИмяФормы, СтрНайти(Форма.ИмяФормы, ".Форма.") - 1), 7)].ПолучитьМакет(СтруктураРеквизитовФормы.мСоставПоказателей);
...
Процедура ЗаполнитьНачальныеНастройки(Форма, Таблица = "ТаблицаВариантыЗаполненияПолеТабличногоДокументаФормаОтчета") Экспорт 
	...
	МакетСоставаПоказателей = Отчеты[Сред(Лев(Форма.ИмяФормы, СтрНайти(Форма.ИмяФормы, ".Форма.") - 1), 7)].ПолучитьМакет(Форма.СтруктураРеквизитовФормы.мСоставПоказателей);
...
Процедура УдалитьНомерКорректировки(ТитульнаяСтраница) Экспорт 
	...
	КонецЕсли;
	НК = ТитульнаяСтраница.Области.Найти("ШапкаОбособлннныеПодразделения");
	Если НК <> Неопределено Тогда
		ТитульнаяСтраница.УдалитьОбласть(НК, ТипСмещенияТабличногоДокумента.ПоВертикали);
	КонецЕсли;
КонецПроцедуры
...
Функция ПолучитьШаблон(Форма, ИмяМакетаШаблона) Экспорт 
	Результат = Новый Структура("ИмяФайлаШаблона, Шаблон");
	ДД = Отчеты[Сред(Лев(Форма.ИмяФормы, СтрНайти(Форма.ИмяФормы, ".Форма.") - 1), 7)].ПолучитьМакет(ИмяМакетаШаблона);
...
Функция ПолучитьПараметрыИнициализацииУниверсальногоОтчета(Форма, ИмяМакетаАтрибутов, ИмяМакетаШаблона, ИмяСхемы) Экспорт 
	Попытка
		...
		ДанныеШаблона = ПолучитьШаблон(Форма, ИмяМакетаШаблона);
		...
		ДеревоВыгрузки = ПолучитьДеревоВыгрузки(Форма, ИмяСхемы);
...
Функция ПолучитьСведенияОШаблоне(Форма, ИмяМакетаАтрибутов) Экспорт 
	...
	ДопАтрибуты = СформироватьСтруктуруДопАтрибутов(Форма.ИмяФормы, ИмяМакетаАтрибутов);
...
Функция ПроверитьКонтрольныеСоотношенияВОтчете(ПараметрыДляПроверки) Экспорт 
	...
	Для каждого КонтрольноеСоотношение Из ПараметрыДляПроверки.ТаблицаКонтрольныхСоотношений Цикл
		...
		СтруктураОшибок.Вставить("Нарушение" + Формат(ОбщееЧислоОшибок + ОбщееЧислоБезОшибок, "ЧГ=0"), СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru='%1'"), ""));
		...
		Если ПараметрыДляПроверки.КвоОшибокКС > 0 Тогда
			СтруктураОшибок.Вставить("Комментарий" + Формат(ОбщееЧислоОшибок + ОбщееЧислоБезОшибок, "ЧГ=0"), СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru='%1'"), "Ошибка в выражении! Соотношение не используется."));
		Иначе
			СтруктураОшибок.Вставить("Комментарий" + Формат(ОбщееЧислоОшибок + ОбщееЧислоБезОшибок, "ЧГ=0"), СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru='%1'"), ?(ОбязательностьКонтроля, "", "Контролировать не обязательно")));
		КонецЕсли; 
...
Функция РезультатПроверкиКонтрольногоСоотношения(ПараметрыДляПроверки, Формула, Условие, УсловиеПериод, РазрядностьОкругления, ДопустимоеОтклонение) Экспорт 
	...
	Если РезУсловиеПериод Тогда
		...
		Если РезУсловие Тогда
			...
			Попытка
				ПараметрыДляПроверки.КвоОшибокКС = 0;
				ВектРезФормула = ВекторныйРезультатВычисленияВыраженияИзСтроки(ПараметрыДляПроверки, Формула, РазрядностьОкругления, ДопустимоеОтклонение);
				Если ВектРезФормула <> Неопределено
...
Функция ВекторныйРезультатВычисленияВыраженияИзСтроки(ПараметрыДляПроверки, ТекстовоеПредставлениеВыражения, РазрядностьОкругления = 2, Допустимоетклонение = 0, МатрицаУсловий = Неопределено) Экспорт 
	...
	Если СтрНайти(ТекстовоеВыражение, "SUM") > 0 Тогда // есть оператор суммирования
		РегламентированнаяОтчетность.ОпределитьОбластьДействияОператораСуммы(ТекстовоеВыражение, ТаблицаЭлементов, ПараметрыДляПроверки.КвоОшибокКС);
	КонецЕсли;
	...
	РегламентированнаяОтчетность.ВыполнитьЗаменуФункцийЯзыкаКС(ТекстовоеВыражение);
	...
	МассивПодвыражений = РегламентированнаяОтчетность.МассивЛогическихПодвыраженийВыделитьИНазначитьИндексВТаблице(ТекстовоеВыражение, ТаблицаЭлементов);
	...
	Для Инд = 0 По МассивПодвыражений.ВГраница() Цикл
		СтрРезультатВычисления = ?(ВекторныйРезультатВычисленияЛогическогоПодвыражения(ПараметрыДляПроверки, ИтоговыеМатрицы[Инд], МассивПодвыражений[Инд], ТаблицаЭлементов, Инд, ПараметрыДляПроверки.КвоОшибокКС), "Истина", "Ложь");
	КонецЦикла;
...
Функция ВекторныйРезультатВычисленияЛогическогоПодвыражения(ПараметрыДляПроверки, МатрицаРезультата, ШаблонВыражения, ТаблицаЭлементов, ИндексПодвыражения = 0, СчетчикОшибок = 0)
	...
	Если ИспользуетсяПредыдущийПериод Тогда
		Возврат Истина;
	КонецЕсли;
		...
		Для ИндСтроки = 1 По КоличествоСтрок Цикл
			Для ИндГрафы = 1 По КоличествоГраф Цикл
				...
				Для каждого ЭлементВыражения Из ЭлементыВыражения Цикл
					СтрЗначениеПоказателя = Формат(РегламентированнаяОтчетность.ЗначениеПоказателяИзТаблицы(ЭлементВыражения, ИндСтроки, ИндГрафы), "ЧРД=.; ЧН=; ЧГ=");
...
Процедура ВычислитьИЗаменитьУсловиеНаПериод(ПараметрыДляПроверки, ТекстовоеВыражение)
	...
	Пока Поз > 0 Цикл
		...
		Иначе
			...
			Перейти ~ПоискСледующегоУсловияНаПериод;
		КонецЕсли;
		...
		~ПоискСледующегоУсловияНаПериод:
		Поз = СтрНайти(ВремВыражение, ОпрПериода);
...
Функция ИзвлечьСтруктуруXML(ИмяФормы, ИмяМакета) Экспорт 
	...
	Если Метаданные.Отчеты.Найти(ИмяФормы) = Неопределено Тогда 
		Макет = Отчеты[Сред(Лев(ИмяФормы, СтрНайти(ИмяФормы, ".Форма.") - 1), 7)].ПолучитьМакет(ИмяМакета);
	Иначе
...
Процедура ОбработатьУсловныеЭлементы(Форма, Знач ПараметрыВыгрузки, Узел) Экспорт 
	...
	Для Инд = 1 По КоличествоСтрок Цикл
		...
		Если НЕ ПустаяСтрока(ТекСтр.Условие) Тогда
			Если НЕ РегламентированнаяОтчетность.УсловиеВыполнено(Форма, ПараметрыВыгрузки, ТекСтр.Условие, , Узел) Тогда
...
Функция СвойстваРегламентированногоОтчета(Ссылка) Экспорт
	...
	Иначе
		...
		ИЛИ СтрНайти(ИсточникОтчета, "Форма2ТПОтходы") > 0 Тогда
	...
	ИначеЕсли ВидКонтролирующегоОргана = Перечисления.ТипыКонтролирующихОрганов.ФСГС Тогда
		...
		КодКонтролирующегоОргана = КодОрганаФСГС;
	...
	ИначеЕсли ВидКонтролирующегоОргана = Перечисления.ТипыКонтролирующихОрганов.ФСС Тогда
		...
		КодКонтролирующегоОргана = ?(ПустаяСтрока(КодОрганаФСС) И ЗначениеЗаполнено(ОрганизацияОтчета),
			Лев(ОрганизацияОтчета.КодПодчиненностиФСС, 4), КодОрганаФСС);
	КонецЕсли;
...
Процедура ВставитьПоказатели_УСН(ТаблицаПоказателей, ОписаниеОтчета)
	...
	Если ОписаниеОтчета.ВыбраннаяФорма = "ФормаОтчета2014Кв1" Тогда
...
Функция РазложитьФИО(Знач ФИОСтр) Экспорт
	...
	ФИОФизическихЛиц = РегистрыСведений.ФИОФизическихЛиц.Выбрать();
...
#Область ДляСовместимостиРГСофт
...
Процедура ДобавитьНадписьВнешнийОтчет(Форма) Экспорт
	...
	РегламентированнаяОтчетностьДляСовместимости.ДобавитьНадписьВнешнийОтчет(Форма);
...
Функция ДополнитьСтроку(Знач Стр, Длина, Чем=" ", Режим = 0) Экспорт
	...
	Возврат РегламентированнаяОтчетностьКлиентСервер.ДополнитьСтроку(Стр, Длина, Чем, Режим);
	
КонецФункции 

Процедура ЗаписатьДанныеФИОВРегистр(Ссылка, Фамилия, Имя, Отчество) Экспорт
	
	РегламентированнаяОтчетностьДляСовместимости.ЗаписатьДанныеФИОВРегистр(Ссылка, Фамилия, Имя, Отчество);
...
Функция КодРегионаПоНазванию(Название) Экспорт
	...
	Возврат РегламентированнаяОтчетностьВызовСервера.КодРегионаПоНазванию(Название);
...
Функция ОтветственныеЛицаОрганизаций(Организация, ДатаСреза, Исполнитель = Неопределено) Экспорт
	...
	Если Организация <> Неопределено тогда
		...
		Если ИДКонфигурации = "ЗУП" Тогда
			...
			|		ЛЕВОЕ СОЕДИНЕНИЕ РегистрСведений.ФИОФизическихЛиц.СрезПоследних(&ДатаСреза, ФизическоеЛицо ССЫЛКА Справочник.ФизическиеЛица) КАК ФИОФизическихЛицСрезПоследних
			...
			Если Исполнитель <> Неопределено тогда
				...
				|		ЛЕВОЕ СОЕДИНЕНИЕ РегистрСведений.ФИОФизическихЛиц.СрезПоследних(&ДатаСреза, ФизическоеЛицо = &Исполнитель) КАК ФИОФизическихЛицСрезПоследних
				|		ПО РаботникиОрганизацииСрезПоследних.Сотрудник.Физлицо = ФИОФизическихЛицСрезПоследних.ФизическоеЛицо
				|ГДЕ
				|	ВЫБОР 
				|   	КОГДА РаботникиОрганизацииСрезПоследних.ПериодЗавершения <= &ДатаСреза
				|            	И РаботникиОрганизацииСрезПоследних.ПериодЗавершения <> ДАТАВРЕМЯ(1, 1, 1, 0, 0, 0)
		...
		Иначе
			...
			Если ИДКонфигурации = "УПП" ИЛИ ИДКонфигурации = "КА" Тогда
				...
				ЗапросПоЛицам.Текст = "ВЫБРАТЬ РАЗРЕШЕННЫЕ
				                      ...
				                      |	КОНЕЦ КАК ФИОПолное
				                      |ИЗ
				                      |	РегистрСведений.ФИОФизическихЛиц.СрезПоследних(&ДатаСреза, ФизическоеЛицо ССЫЛКА Справочник.ФизическиеЛица) КАК ФИОФизическихЛицСрезПоследних
			...
			Иначе
				...
				|	ОтветственныеЛицаОрганизацийСрезПоследних.ОтветственноеЛицо,
				|	ОтветственныеЛицаОрганизацийСрезПоследних.Должность.Наименование КАК Должность,
				|	ВЫБОР КОГДА (ФИОФизическихЛицСрезПоследних.ФизическоеЛицо) ЕСТЬ NULL  ТОГДА ОтветственныеЛицаОрганизацийСрезПоследних.ФизическоеЛицо.Наименование ИНАЧЕ ФИОФизическихЛицСрезПоследних.Фамилия + ВЫБОР КОГДА ПОДСТРОКА(ФИОФизическихЛицСрезПоследних.Имя, 1, 1) <> """" ТОГДА "" "" + ПОДСТРОКА(ФИОФизическихЛицСрезПоследних.Имя, 1, 1) + ""."" ИНАЧЕ """" КОНЕЦ + ВЫБОР КОГДА ПОДСТРОКА(ФИОФизическихЛицСрезПоследних.Отчество, 1, 1) <> """" ТОГДА "" "" + ПОДСТРОКА(ФИОФизическихЛицСрезПоследних.Отчество, 1, 1) + ""."" ИНАЧЕ """" КОНЕЦ КОНЕЦ КАК ФИОПолное
				|ИЗ
				|	РегистрСведений.ОтветственныеЛицаОрганизаций.СрезПоследних(&ДатаСреза, СтруктурнаяЕдиница = &Организация) КАК ОтветственныеЛицаОрганизацийСрезПоследних
				|		ЛЕВОЕ СОЕДИНЕНИЕ РегистрСведений.ФИОФизическихЛиц.СрезПоследних(&ДатаСреза, ФизическоеЛицо ССЫЛКА Справочник.ФизическиеЛица) КАК ФИОФизическихЛицСрезПоследних
				|		ПО ОтветственныеЛицаОрганизацийСрезПоследних.ФизическоеЛицо = ФИОФизическихЛицСрезПоследних.ФизическоеЛицо";
...
Функция ПолучитьПутьВыгрузки(ПутьДляВыгрузки = Неопределено) Экспорт
	...
	Возврат РегламентированнаяОтчетностьДляСовместимости.ПолучитьПутьВыгрузки(ПутьДляВыгрузки);
...
Функция ПолучитьСписокПраздниковРФ(КалендарныйГод) Экспорт
	...
	Возврат РегламентированнаяОтчетностьДляСовместимости.ПолучитьСписокПраздниковРФ(КалендарныйГод);

КонецФункции // ПолучитьСписокПраздниковРФ()

Функция ПравоДоступаКРегламентированномуОтчету(ИдентификаторОтчета) Экспорт
	
	Возврат РегламентированнаяОтчетностьВызовСервера.ПравоДоступаКРегламентированномуОтчету(ИдентификаторОтчета);
...
Функция ПредставлениеАдресаВФормате9Запятых(Знач АдресВФормате9Запятых, АнализироватьРегион = Ложь) Экспорт
	...
	Возврат РегламентированнаяОтчетностьКлиентСервер.ПредставлениеАдресаВФормате9Запятых(АдресВФормате9Запятых, АнализироватьРегион);
...
Функция ПредставлениеМестаРождения(Знач СтрокаМестоРождения) Экспорт
	...
	Возврат РегламентированнаяОтчетностьДляСовместимости.ПредставлениеМестаРождения(СтрокаМестоРождения);
...
Функция РазложитьАдрес(Знач СтрокаАдрес) Экспорт
	...
	Адрес = РегламентированнаяОтчетностьКлиентСервер.РазложитьАдрес(СтрокаАдрес);
...
Функция РазложитьМестоРождения(Знач СтрокаМестоРождения, ВерхнийРегистр = Истина) Экспорт
	...
	Возврат РегламентированнаяОтчетностьДляСовместимости.РазложитьМестоРождения(СтрокаМестоРождения, ВерхнийРегистр);	
...
Функция роПолучитьОбщуюФорму(ИмяФормы, Владелец = Неопределено, КлючУникальности = Неопределено) Экспорт
	...
	Возврат РегламентированнаяОтчетностьДляСовместимости.роПолучитьОбщуюФорму(ИмяФормы, Владелец, КлючУникальности);
...
Функция ЭтоПБОЮЛ(Организация) Экспорт
	...
	Возврат НЕ РегламентированнаяОтчетностьВызовСервера.ЭтоЮридическоеЛицо(Организация);
...
Функция ПолучитьСведенияОбОрганизации(Знач Организация, Знач ДатаЗначения = Неопределено, Знач СписокПоказателей = Неопределено) Экспорт
	...
	Возврат РегламентированнаяОтчетностьДляСовместимости.ПолучитьСведенияОбОрганизации(Организация, ДатаЗначения, СписокПоказателей);

КонецФункции

// { RGS LFedotova 30.01.2018 7:38:19 - вопрос SLI-0007473
Процедура ПолучитьЗначениеОКВЭД(ДатаПодписи, СтруктураРеквизитовФормы) Экспорт
	
	СтрокаСведений = "ОКВЭД, ОКВЭД2";
	СведенияОбОрганизации = РегламентированнаяОтчетностьВызовСервера.ПолучитьСведенияОбОрганизации(СтруктураРеквизитовФормы.Подразделение, ДатаПодписи, СтрокаСведений);
...
КонецПроцедуры
// } RGS LFedotova 30.01.2018 7:40:02 - вопрос SLI-0007473

// { RGS LFedotova 12.02.2018 22:11:11 - вопрос SLI-0007489
...
Процедура ДобавитьКнопкуПрисоединенныеФайлы(Форма) Экспорт
	...
	Если ТипЗнч(Форма) = Тип("УправляемаяФорма") Тогда
		...
		Если СтрНайти(Форма.ИмяФормы, "Отчет.РегламентированныйОтчет") > 0
		 ИЛИ СтрНайти(Форма.ИмяФормы, "Отчет.РегламентированноеУведомление") > 0 Тогда
...
// } RGS LFedotova 12.02.2018 22:11:13 - вопрос SLI-0007489

#КонецОбласти

// { RGS LFedotova 02.03.2018 22:41:31 - вопрос SLI-0007489
...
Процедура СохранитьСтатусОтправкиРеглОтчета(Форма) Экспорт
	...
	Новый Структура("Ссылка", Форма.СтруктураРеквизитовФормы.мСохраненныйДок.Ссылка));
	...
	Если ВыборкаЖОС.Следующий() Тогда
		...
		//	ВыборкаЖОС.СостояниеСдачиОтчетности) Тогда
			...
			НСтр("ru = '%1'"), Форма.Элементы.НаименованиеЭтапа.Заголовок);
...
Процедура СохранитьСтатусОтправкиУведомления(Форма) Экспорт
	...
	Новый Структура("Ссылка", Форма.Объект.Ссылка));
	...
	Если ВыборкаЖОКО.Следующий() Тогда
		...
		Если РегламентированнаяОтчетностьКлиентСервер.ОбъектНеОтправлялсяЧерез1СОтчетность(
			...
			Если СтатусыОбъектов.Найти(ТекСтатус) <> Неопределено Тогда
				...
				ЗаписьЖОКО.СтатусОтправки = ТекСтатус;
				ЗаписьЖОКО.Записать();
...
#КонецОбласти
// } RGS LFedotova 02.03.2018 22:42:14 - вопрос SLI-0007489
