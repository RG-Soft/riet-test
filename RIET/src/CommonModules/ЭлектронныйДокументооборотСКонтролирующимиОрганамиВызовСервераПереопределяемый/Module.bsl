...
Функция ПолучитьДатыПериодаДокумента(Ссылка) Экспорт
	...
	Если ТипЗнч(Ссылка) = Тип("ДокументСсылка.СправкиНДФЛДляПередачиВНалоговыйОрган") Тогда
		ГодОтчета = ОбщегоНазначения.ПолучитьЗначениеРеквизита(Ссылка, "НалоговыйПериод");
...
// Функция должна возвращать массив доступных для отправки в ФНС данному прикладному решению видов документов
//
// Результат:
//	Массив перечислений "ВидыПредставляемыхДокументов" 
//
Функция ПолучитьМассивДоступныхДляОтправкиВидовДокументов() Экспорт

	МассивВидовДокументов = Новый Массив;
	
	//Документы НДС
	МассивВидовДокументов.Добавить(Перечисления.ВидыПредставляемыхДокументов.КнигаПокупок);
	МассивВидовДокументов.Добавить(Перечисления.ВидыПредставляемыхДокументов.КнигаПродаж);
	МассивВидовДокументов.Добавить(Перечисления.ВидыПредставляемыхДокументов.ЖурналПолученныхИВыставленныхСчетовФактур);
	МассивВидовДокументов.Добавить(Перечисления.ВидыПредставляемыхДокументов.ДополнительныйЛистКнигиПокупок);
	МассивВидовДокументов.Добавить(Перечисления.ВидыПредставляемыхДокументов.ДополнительныйЛистКнигиПродаж);

	//Документы ЭД
	Если ПолучитьФункциональнуюОпцию("ИспользоватьОбменЭД") Тогда
		
		МассивВидовДокументов.Добавить(Перечисления.ВидыПредставляемыхДокументов.СчетФактура);
		МассивВидовДокументов.Добавить(Перечисления.ВидыПредставляемыхДокументов.КорректировочныйСчетФактура);
		МассивВидовДокументов.Добавить(Перечисления.ВидыПредставляемыхДокументов.АктПриемкиСдачиРабот);
		МассивВидовДокументов.Добавить(Перечисления.ВидыПредставляемыхДокументов.ТоварнаяНакладнаяТОРГ12);
...
Функция ПолучитьФайлыВыгрузкиЭД(МассивЭД, УникальныйИдентификаторФормы) Экспорт
	...
	Возврат ЭлектронныеДокументы.ПолучитьСоответствиеДокументамИБКомплектыЭлектронныхДокументов(МассивЭД, УникальныйИдентификаторФормы);
...
//  ФИО - структура с полями:
// 		Фамилия, тип Строка - фамилия сотрудника
// 		Имя, тип Строка - имя сотрудника
// 		Отчество, тип Строка - отчество сотрудника
//  Серия, тип Строка - серия документа, удостоверяющего личность сотрудника
//  Номер, тип Строка - номер документа, удостоверяющего личность сотрудника
//  ДатаВыдачи, тип Дата - дата выдачи документа, удостоверяющего личность сотрудника
//  КемВыдан, тип Строка - кем выдан документ, удостоверяющий личность сотрудника
//  ВидДокумента, тип СправочникСсылка.ВидыДокументовФизическихЛиц - вид документа, удостоверяющего личность сотрудника
// 
Функция ПолучитьДанныеИсполнителя(СсылкаФизЛицо, ОрганизацияСсылка) Экспорт
	...
	СотрудникСсылка = ПолучитьСотрудникаПоФизЛицу(СсылкаФизЛицо, ОрганизацияСсылка);
	Если НЕ СотрудникСсылка = Неопределено Тогда
		
		ДанныеСотрудников = КадровыйУчет.КадровыеДанныеСотрудников(Ложь, СотрудникСсылка, "Подразделение,Должность", НачалоДня(ТекущаяДата()) - 1);
	...
	Структура.Вставить("СНИЛС", ПолучитьСНИЛСФизЛица(СсылкаФизЛицо));
	...
	Попытка
		...
		Пока ЗапросВыборка.Следующий() Цикл
			Если ПолучитьСоответствие Тогда
				...
				СтруктураДанных.Вставить("СНИЛС", ПолучитьСНИЛСФизЛица(ЗапросВыборка.Сотрудник));
...
Функция ГлБухгалтер(Организация) Экспорт
	...
	Отбор = Новый Структура("СтруктурнаяЕдиница, ОтветственноеЛицо", Организация, ПредопределенноеЗначение("Перечисление.ОтветственныеЛицаОрганизаций.ГлавныйБухгалтер"));
	ЗаписьРегистраОтветственныеЛицаОрганизаций = РегистрыСведений.ОтветственныеЛицаОрганизаций.СрезПоследних(ТекущаяДатаСеанса(), Отбор);
	Если ЗаписьРегистраОтветственныеЛицаОрганизаций.Количество() > 0 Тогда
		Результат = ЗаписьРегистраОтветственныеЛицаОрганизаций[0].ФизическоеЛицо;
	КонецЕсли;
	Возврат Результат;
...
Функция Руководитель(Организация) Экспорт
	...
	Отбор = Новый Структура("СтруктурнаяЕдиница, ОтветственноеЛицо", Организация, ПредопределенноеЗначение("Перечисление.ОтветственныеЛицаОрганизаций.Руководитель"));
	ЗаписьРегистраОтветственныеЛицаОрганизаций = РегистрыСведений.ОтветственныеЛицаОрганизаций.СрезПоследних(ТекущаяДатаСеанса(), Отбор);
	Если ЗаписьРегистраОтветственныеЛицаОрганизаций.Количество() > 0 Тогда
		Результат = ЗаписьРегистраОтветственныеЛицаОрганизаций[0].ФизическоеЛицо;
	КонецЕсли;
...
Функция ПолучитьСсылкуНаОрганизациюИсточника(Источник) Экспорт
	...
	Организация = УчетОбособленныхПодразделений.ПолучитьСсылкуНаОрганизациюИсточника();
...
Функция ПолучитьВидОтправляемогоДокументаПоСсылке(ОбъектСсылка) Экспорт
	...
	ВидОтправляемогоДокумента = УчетОбособленныхПодразделений.ПолучитьВидОтправляемогоДокументаПоСсылке(ОбъектСсылка);
...
Функция ПолучитьСотрудникаПоФизЛицу(ФизическоеЛицо, ОрганизацияСсылка)
...
// Функция возвращает "СНИЛС" значение реквизита "СтраховойНомерПФР"
//	Параметры функции:
//		ФизЛицоСсылка - СправочникСсылка.ФизическиеЛица;
//	Возвращаемое значение:
//		"СНИЛС" - (тип "Строка"), значение реквизита "СтраховойНомерПФР".
//
Функция ПолучитьСНИЛСФизЛица(ФизЛицоСсылка)
	...
	// проверим наличие реквизита в справочнике
	Если Метаданные.Справочники.ФизическиеЛица.Реквизиты.Найти("СтраховойНомерПФР") = Неопределено Тогда
		Возврат СНИЛС;
	КонецЕсли;
	
	Возврат ФизЛицоСсылка.СтраховойНомерПФР;
