...
Процедура ВыбратьПолучателя(ОбработчикРезультата, Объект, МножественныйВыбор, ВозвращатьСоответствие) Экспорт
	...
	Если МножественныйВыбор Тогда
		ВозможныеПолучатели.Добавить(Неопределено, ВернутьСтр("ru = 'Всем получателям'"));
	КонецЕсли;
	...
	Иначе
		ВозможныеПолучатели.ПоказатьВыборЭлемента(Обработчик, ВернутьСтр("ru = 'Выбор получателя'"));
	КонецЕсли;
КонецПроцедуры
...
Процедура ПроверитьВыполнениеФоновогоЗадания(Форма) Экспорт
	...
	ИначеЕсли Результат.Статус = "Исключение" Тогда
		ПоказатьПредупреждение(, 
			ВернутьСтр("ru = 'Рассылки не выполнены из-за ошибки,
