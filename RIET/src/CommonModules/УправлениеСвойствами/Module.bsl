...
//                               Ранее реквизит называлася "Объект" и имел смысл,
		...
		Если Строка.ЗаполнятьОбязательно И НЕ Строка.Удалено Тогда
			...
			Если НЕ ЗначениеЗаполнено(Форма[Строка.ИмяРеквизитаЗначение]) Тогда
				...
				ОбщегоНазначенияКлиентСервер.ДобавитьОшибкуПользователю(Ошибки,
					...
					СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(ВернутьСтр("ru = 'Поле ""%1"" не заполнено.'"), Строка.Наименование),
					"");
			КонецЕсли;
		КонецЕсли;
	КонецЦикла;
		...
		Если НаборыСвойствОбъекта.Количество() > 1 Тогда
			...
			Иначе
				...
				Если ОписаниеНабора = Неопределено Тогда
					...
					ОписаниеНабора.Заголовок = ВернутьСтр("ru = 'Удаленные реквизиты'")
				КонецЕсли;
...
Процедура СоздатьОсновныеОбъектыФормы(Форма, Контекст, СоздатьОписаниеДополнительныхРеквизитов)
	...
	Если ОпцияИспользоватьСвойства Тогда
		...
		Если СоздатьОписаниеДополнительныхРеквизитов Тогда
			...
			//Если Пользователи.РолиДоступны("ДобавлениеИзменениеДополнительныхРеквизитовИСведений") Тогда
				...
				Команда.Заголовок = ВернутьСтр("ru = 'Изменить состав дополнительных реквизитов'");
				Команда.Действие = "Подключаемый_РедактироватьСоставСвойств";
				Команда.Подсказка = ВернутьСтр("ru = 'Изменить состав дополнительных реквизитов'");
	...
	Если ОпцияИспользоватьСвойства
		...
		Иначе
			ЗапретитьПеретаскиваниеРодительскихГрупп(ГруппаФормы);
		КонецЕсли;
...
Процедура ЗапретитьПеретаскиваниеРодительскихГрупп(ГруппаФормы)
	...
	Если ТипЗнч(Родитель) = Тип("ГруппаФормы") Тогда
		...
		Если Родитель.Вид = ВидГруппыФормы.Страница Тогда
			Возврат;
		Иначе
			ЗапретитьПеретаскиваниеРодительскихГрупп(Родитель);
		КонецЕсли;
	Иначе
		Возврат;
	КонецЕсли;
