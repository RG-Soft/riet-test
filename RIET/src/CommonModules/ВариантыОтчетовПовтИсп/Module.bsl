...
Функция ПодсистемыТекущегоПользователя() Экспорт
	...
	КорневаяСтрока.Представление = НСтр("ru = 'Все разделы'; en = 'All groups'");   //RG-Soft
...
Процедура ДобавитьПодсистемыТекущегоПользователя(РодительСтрока, РодительМетаданные = Неопределено, РазделСсылка = Неопределено)
	...
	Для Каждого Подсистема Из Подсистемы Цикл
		...
		Если ЭтоНачальнаяСтраница Тогда
			...
			СтрокаДерева.Представление = ВернутьСтр("ru = 'Начальная страница'");
		Иначе
...
// Устарела: Следует использовать ВариантыОтчетовПовтИсп.Параметры().ОтчетыСНастройками.
...
Функция ОтчетыСНастройками() Экспорт
	Возврат ВариантыОтчетовПовтИсп.Параметры().ОтчетыСНастройками;
КонецФункции
		...
		Если ХранилищеЗначения = Неопределено Тогда
			Если Не Пользователи.ЭтоПолноправныйПользователь() Тогда
				...
				Если Не ПривилегированныйРежим() Тогда
					ВызватьИсключение ВернутьСтр("ru = 'Не удалось обновить вспомогательные данные расширений. Обратитесь к администратору.'");
				КонецЕсли;
			КонецЕсли;
