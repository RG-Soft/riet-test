...
Процедура ПроверитьКонтрагентаПриИзменении(ПараметрыЗапуска) Экспорт  
	...
	Попытка
		...
		//ФоновыеЗадания.Выполнить("ПроверкаКонтрагентов.ПроверитьКонтрагентаФоновоеЗадание", 
			//Параметры, ПараметрыЗапуска.ИНН + " " + ПараметрыЗапуска.КПП, ВернутьСтр("ru = 'Проверка контрагента '"));
		ПроверкаКонтрагентов.ПроверитьКонтрагентаФоновоеЗадание(ПараметрыЗапуска);
		// } РГ-Софт Пахоменков А. 14.01.2015		
	...
	Исключение
		...
		ЗаписьЖурналаРегистрации(ВернутьСтр("ru = 'Проверка контрагентов.Проверка контрагента в фоновом задании'"),
...
Функция ОчиститьСохраненныеРезультатыПроверкиКонтрагентов() Экспорт 
	...
	Исключение
		...
		ВернутьСтр("ru = 'Проверка контрагентов.Очистка предыдущих результатов проверок'"), 
...
Процедура СохранитьРезультатыПроверкиКонтрагентовВДокументеВРегистр(СохраняемыйРезультат) Экспорт
	...
	Исключение
		...
		ЗаписьЖурналаРегистрации(
		ВернутьСтр("ru = 'Проверка контрагентов.Запись результатов проверки контрагентов в документе в регистр'"), 
		УровеньЖурналаРегистрации.Ошибка,,,
		ПодробноеПредставлениеОшибки(ИнформацияОбОшибке));
...
Функция ПредставлениеРезультатаПроверкиКонтрагента(Знач Контрагент, ИНН, КПП, АдресХранилища, Знач ИсходныйТекст = "") Экспорт
	...
	Если ИспользованиеПроверкиВозможно Тогда
		...
		Если СостояниеКонтрагента = Перечисления.СостоянияСуществованияКонтрагента.ПрекратилДеятельность
			...
			КонецЕсли;
			МассивПодстрок.Добавить(Новый ФорматированнаяСтрока(Строка(СостояниеКонтрагента),,ЦветТекста,, "ПодробнееОПроверкеКонтрагентов"));
