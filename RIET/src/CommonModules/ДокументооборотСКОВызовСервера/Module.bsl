...
КонецФункции //ИспользоватьВнешнийМодуль()
...
Функция ИнициализироватьКонтекстДокументооборотаСНалоговымиОрганами() Экспорт
	...
	Пока ИнициализироватьКонтекст Цикл
		...
		// возможно этот код целесообразнее использовать в др. ф-ции
		//Если ИмяОбработки_ДокументооборотСКО = Неопределено И ПравоДоступа("Использование", Метаданные.Обработки.ДокументооборотСКонтролирующимиОрганами) Тогда
		//	КонтекстЭДО = Обработки.ДокументооборотСКонтролирующимиОрганами.Создать()
		//Иначе 
		//	КонтекстЭДО = ВнешниеОбработки.Создать(ИмяОбработки_ДокументооборотСКО)
		//КонецЕсли;
...
Функция ЕстьПравоНаДОсКО(ПравоНаВнешнюю) Экспорт 
	...
	//	Возврат Истина; //ПравоДоступа("Использование", ВнешниеОбработки.Создать("Обработка_ДокументооборотСКО"));
	//Иначе
...
КонецФункции //ПолучитьОбработкуЭДО()
			...
			Иначе
				...
				Иначе
					Если УчетнаяЗаписьДокументооборота.СпецоператорСвязи <> Перечисления.СпецоператорыСвязи.Такском И (КонтекстЭДОСервер = Неопределено
						ИЛИ СтрНайти(";" + ВРег(СокрЛП(КонтекстЭДОСервер.ПолучитьПараметрСпецоператора(УчетнаяЗаписьДокументооборота.СпецоператорСвязи, "ОнлайнПроверкаКонтролирующиеОрганы"))) + ";", ";" + ВРег(КонтролирующийОрган) + ";") = 0) Тогда
						ПараметрыПрорисовкиКнопокОтправки.Вставить("ПроверитьВИнтернете", Ложь);
					Иначе
						ПараметрыПрорисовкиКнопокОтправки.Вставить("ПроверитьВИнтернете", Истина);
					КонецЕсли;
...
Функция УчетнаяЗаписьПредназначенаДляДокументооборотаСФНС(УчетнаяЗапись) Экспорт
	...
	Исключение
		...
		ЗаписьЖурналаРегистрации(
			ВернутьСтр("ru = 'Проверка наличия возможности отправки в ФНС для учетной записи'"), 
...
Функция УчетнаяЗаписьПредназначенаДляДокументооборотаСПФР(УчетнаяЗапись) Экспорт
	...
	Исключение
		...
		ЗаписьЖурналаРегистрации(
			ВернутьСтр("ru = 'Проверка наличия возможности отправки в ПФР для учетной записи'"), 
			УровеньЖурналаРегистрации.Ошибка,,,
			ПодробноеПредставлениеОшибки(ИнформацияОбОшибке));
...
Функция УчетнаяЗаписьПредназначенаДляДокументооборотаСФСГС(УчетнаяЗапись) Экспорт
	...
	Исключение
		...
		ЗаписьЖурналаРегистрации(
			ВернутьСтр("ru = 'Проверка наличия возможности отправки в Росстат для учетной записи'"), 
			УровеньЖурналаРегистрации.Ошибка,,,
			ПодробноеПредставлениеОшибки(ИнформацияОбОшибке));
...
Процедура ПриНажатииНаКнопкуОтправкиВКонтролирующийОрган(ОрганизацияОтчета, КонтролирующийОрган, НастроенОбменВУниверсальномФормате, УчетнаяЗаписьПредназначенаДляДокументооборотаСКО) Экспорт
	...
	СвойстваОрганизации = ОбщегоНазначения.ЗначенияРеквизитовОбъекта(ОрганизацияОтчета,
	"ВидОбменаСКонтролирующимиОрганами, УчетнаяЗаписьОбмена");
	//СвойстваУчетнойЗаписи = ОбщегоНазначения.ЗначенияРеквизитовОбъекта(СвойстваОрганизации.УчетнаяЗаписьОбмена,
...
Процедура ВыполнитьОбменСКОПоРегламентномуЗаданию() Экспорт 
	...
	//	ЗаписьЖурналаРегистрации(ВернутьСтр("ru = 'Автоматический обмен с контролирующими органами'"), 
...
Функция ПараметрыПрорисовкиСтатуса(Знач Ссылка, КонтролирующийОрган = "ФНС") Экспорт
	...
	ТекущееСостояние = КонтекстЭДОСервер.ТекущееСостояниеОтправки(Ссылка, КонтролирующийОрган, ДополнительныеПараметры); //НДС TODO
