...
Процедура УдалитьЗадание(Знач Идентификатор) Экспорт
	...
	Иначе
		Если ТипЗнч(Идентификатор) = Тип("ОбъектМетаданных") И Идентификатор.Предопределенное Тогда
			ВызватьИсключение( ВернутьСтр("ru = 'Предопределенное регламентное задание удалить невозможно.'") );
...
Функция ПолучитьРегламентноеЗадание(Знач Идентификатор) Экспорт
	...
	Если РегламентноеЗадание = Неопределено Тогда
		ВызватьИсключение( ВернутьСтр("ru = 'Регламентное задание не найдено.
...
Процедура СоздатьДокументПереоценкаРегистра() Экспорт
	Если День(ТекущаяДата()) = 1 Тогда
		ИнвЦентры = Справочники.ИнвойсинговыеЦентры.Выбрать();
		Для каждого ИнвЦентр из ИнвЦентры Цикл
			ДокументПереоценкиРегистра = Документы.ПереоценкаВалютыВРегистре.СоздатьДокумент();
			ДокументПереоценкиРегистра.Дата = ТекущаяДата();
			ДокументПереоценкиРегистра.УстановитьНовыйНомер();
			ДокументПереоценкиРегистра.Ответственный = ПараметрыСеанса.ТекущийПользователь;
			ДокументПереоценкиРегистра.ИнвойсинговыйЦентр = ИнвЦентр;
			Попытка
				ДокументПереоценкиРегистра.Записать(РежимЗаписиДокумента.Проведение);
			Исключение
				Текст = "ru = ""Невозможно провести документ."";"
				+ " en = ""Document needs reposting.""";
				#Если Клиент тогда
				Предупреждение(ВернутьСтр(Текст));
				#КонецЕсли 
			КонецПопытки;
		КонецЦикла;
	Иначе 
		...
		Запрос.Текст = 
		"ВЫБРАТЬ
		|	ДокументПереоценкаВалютыВРегистре.Ссылка
		|ИЗ
		|	Документ.ПереоценкаВалютыВРегистре КАК ДокументПереоценкаВалютыВРегистре
		|ГДЕ
		|	ДокументПереоценкаВалютыВРегистре.Дата МЕЖДУ &ДатаНачала И &ДатаКонца";
		
		Запрос.УстановитьПараметр("ДатаКонца", ТекущаяДата());
		Запрос.УстановитьПараметр("ДатаНачала", ДобавитьМесяц(НачалоМесяца(ТекущаяДата()),-3));
		
		Результат = Запрос.Выполнить();
		
		Выборка = Результат.Выбрать();
		Пока Выборка.Следующий() Цикл
			СозданныйРанееДокумент = Выборка.Ссылка.ПолучитьОбъект();
			Попытка
				СозданныйРанееДокумент.Записать(РежимЗаписиДокумента.Проведение);
			Исключение
				Текст = "ru = ""Невозможно провести документ."";"
				+ " en = ""Document needs reposting.""";
				#Если Клиент тогда
