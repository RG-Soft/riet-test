...
Функция ЗаполнитьСведенияОбОрганизацииНаКлиенте(Форма, ОргСведения, ПБОЮЛ, Знач СписокСведений, Дополнения = "", РазделДляВывода = "Титульный") Экспорт
	...
	Если (Титульный.Области.Найти("ИННУп") <> Неопределено) Или
		 (Титульный.Области.Найти("ИННУпП") <> Неопределено) Тогда
	...
	Для Каждого ПолученныйРезультат Из ОргСведения Цикл
		...
		ИначеЕсли Параметр = "ОКВЭД" Тогда
			...
			Если ОКВЭДВариант = 1 Тогда
				...
				РегламентированнаяОтчетностьКлиентСервер.ПроставитьВКвадратыЗначения(Форма, РазделДляВывода, "ОКВЭД" , 6, ОбработанныйРезультат);
...
Процедура ВывестиМашиночитаемуюФормуУведомленияОСпецрежимахВТонкомКлиентеПродолжение(Результат, ДополнительныеПараметры) Экспорт
	...
	Если ТаксДокПринт = Неопределено Тогда
		...
		ДополнительныеПараметры = Новый Структура("ВременныйКаталог, Выгрузка, ДистрибутивКомпонентыПечатиМашиночитаемыхФорм, ИмяВременногоФайла, ТаксДокПринт", ВременныйКаталог, Выгрузка, ДистрибутивКомпонентыПечатиМашиночитаемыхФорм, ИмяВременногоФайла, ТаксДокПринт);
...
Процедура ВывестиМашиночитаемуюФормуУведомленияОСпецрежимахВВебКлиенте(Форма)
	...
	ДополнительныеПараметры = Новый Структура("Форма,ВыводЧерезВебСервис,АдресПараметровПроксиВоВременномХранилище,ИмяКнопки",
												Форма, ВыводЧерезВебСервис, АдресПараметровПроксиВоВременномХранилище,"");
...
Процедура ВывестиМашиночитаемуюФормуУведомленияОСпецрежимахВВебКлиентеПослеФормированияВыгрузки(Результат, ДополнительныеПараметры) Экспорт
	...
	Если ТаксДокПринт = Неопределено Тогда
		// если не удалось создать объект, то спрашиваем у пользователя, хочет ли он установить компоненту
		ДополнительныеПараметры = Новый Структура("ВремКаталог, ВремФайл, ДистрибутивКомпонентыПечатиМашиночитаемыхФорм, ИмяКнопки, ТаксДокПринт, Форма", ВремКаталог, ВремФайл, ДистрибутивКомпонентыПечатиМашиночитаемыхФорм, ИмяКнопки, ТаксДокПринт, Форма);
...
Процедура ВывестиМашиночитаемуюФормуВВебКлиенте(Форма, ИмяКнопки) 
	...
	ДополнительныеПараметры = Новый Структура("Форма, ИмяКнопки, АдресПараметровПроксиВоВременномХранилище, ВыводЧерезВебСервис", Форма, ИмяКнопки, АдресПараметровПроксиВоВременномХранилище, ВыводЧерезВебСервис);
...
Процедура АктивизироватьОсновноеОкно()
	...
	Для Каждого Окно Из ОкнаКлиентПрил Цикл
...
Процедура СформироватьТекстВыгрузки(Форма, ВремФайл, ДляЦелейФормированияМЧБ = Ложь, ПроверкаБлокировкиФормы = Ложь, ФайлВыгрузкиВоВременномХранилище = Неопределено, ВыполняемоеОповещение = Неопределено)
...
Функция ВыгрузитьШаблоныПечатиВКаталог(Форма, ВремКаталог, Знач КНД = Неопределено, Знач ВерсияФорматаВыгрузки = Неопределено, ДистрибутивКомпонентыПечатиМашиночитаемыхФорм)
...
Функция УстановитьКомпонентуПечатиМашиночитаемыхФорм(ЖдатьЗавершения = Истина, ДистрибутивКомпонентыПечатиМашиночитаемыхФорм, ВремКаталог = "") Экспорт
...
//
// Возвращаемое значение: истина - проверка выполнена,
...
Процедура ПолучитьНачалоКонецПериода(СтрПредставлениеПериода, ДатаНачала, ДатаКонца) Экспорт
	...
	ИначеЕсли СтрНайти(Врег(СтрПредставлениеПериода), "ГОД") > 0 Тогда
		// На случай если будет реализована выдача надписи
		// годовая YYYY г. или годовой YYYY г. вместо YYYYг.
...
//  ВызывающаяФорма			- Форма, окно которой будет пропущено при просмотре окон.
...
//  Результат				- Булево, возвращается 
//							Истина если окно найдено
...
Процедура ВебКлиентНайтиАктивизироватьОкно(ИмяФормыДляПоиска, ВызывающаяФорма, Результат = Неопределено) Экспорт
	...
	//RG-Soft Обновление на релиз 3.0.43.50
	...
	//	Если Окно = Неопределено ИЛИ Окно = ВызывающаяФорма.Окно Тогда
	//		
	//		Продолжить;
	//		
	//	КонецЕсли;
...
//
// Параметры:
//  СсылкаНаСохрРеглОтчет      - ссылка на сохраненный регламентированный отчет, форму которого необходимо открыть.
//  НеОтображатьПредупреждение - признак отображения предупреждения в форме регл. отчета.
//  ОткрытьФормуОтчета         - признак, регулирующий непосредственное открытие регл. отчета (Истина), либо
//                               передачу управления открытием регл. отчета в переопределяемый метод (Ложь).
//
// Пример:
...
Процедура ОткрытьФормуНавигацииПоОшибкамВыгрузки(СообщенияОбОшибках) Экспорт
	...
	ПараметрыФормы = Новый Структура("ВладелецТС", Новый Структура("ТаблицаСообщений", СообщенияОбОшибках));
...
Процедура ОтправкаЧерезПредставителяПослеСохраненияПодтверждениеОтправкиОтчетаЗавершение(РезультатВопроса, ДополнительныеПараметры) Экспорт
	...
	Сообщить("Отправка через представителя отключена");
	//<=
...
Процедура ПриНажатииНаКнопкуОтправкиВКонтролирующийОрган(Форма, КонтролирующийОрган = "ФНС", КодРегионаВФорме = Неопределено, 
		ЭтоОтправкаИзФормыОтчетность = Ложь, СсылкаНаОтчет = Неопределено, Организация = Неопределено) Экспорт
	...
	ЭтоОтправкаИзФормыОтчетность, СсылкаНаОтчет, Организация);
	
КонецПроцедуры

Процедура ПроверитьВИнтернетеПоСсылке(Ссылка, КонтролирующийОрган = "ФНС") Экспорт
	
	ЭлектронныйДокументооборотСКонтролирующимиОрганамиКлиент.ПроверитьВИнтернетеПоСсылке(Ссылка, КонтролирующийОрган);
	
КонецПроцедуры

Процедура ПроверитьВИнтернете(Форма, КонтролирующийОрган = "ФНС") Экспорт
	
	ЭлектронныйДокументооборотСКонтролирующимиОрганамиКлиент.ПроверитьВИнтернете(Форма, КонтролирующийОрган);
...
Процедура ОткрытьИсториюОтправки(Знач Источник, Знач ТипКонтролирующихОрганов, Знач Организация = Неопределено, Знач ИмяСобытия = Неопределено) Экспорт

	ДополнительныеПараметры = Новый Структура("Источник, ТипКонтролирующихОрганов, Организация, ИмяСобытия", Источник, ТипКонтролирующихОрганов, Организация, ИмяСобытия);
...
Процедура ОбработкаВыбораПоляОбособленныхПодразделений(Форма, Область, СтандартнаяОбработка) Экспорт
	Если Область.Имя = "ИсключитьДанныеОбособленныхПодразделений" Тогда 
		...
		ОблОбособленноеПодразделение = Форма.ПолеТабличногоДокументаФормаОтчета.Области.ОбособленноеПодразделение;
...
Процедура ОткрытьФормуВыбораФИО(Форма, СтандартнаяОбработка, ИмяПоля, ИмяСтраницы) Экспорт 
	...
	ПараметрыВыборФИО = Новый Структура("Фамилия,Имя,Отчество,ИмяПоля,ИмяСтраницы",
