...
Процедура ПриВыполненииДоставкиВПапку(ПараметрыДоставки, Вложения) Экспорт
	...
	Комментарий = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
		ВернутьСтр("ru = 'Рассылка отчетов %1 от %2'"),
	...
	Пока Выборка.Следующий() Цикл
		...
		Если ЗначениеЗаполнено(Выборка.ПутьКФайлу) И ЗначениеЗаполнено(Выборка.Том) Тогда
			...
			Если Не Файл.Существует() Тогда
				...
				НесуществующийФайл.СтатусПроверки = ВернутьСтр("ru = 'Отсутствуют данные в томе на диске'");
			...
			Иначе
				...
				СуществующийФайл[0].СтатусПроверки = ВернутьСтр("ru = 'Целостные данные'");
...
Процедура ВыполнитьОчисткуДанныхОФайле(ФайлСсылка) Экспорт
	...
	Попытка
		...
		Если ФайлОбъект.ТипХраненияФайла = Перечисления.ТипыХраненияФайлов.ВТомахНаДиске Тогда
			...
			Если ФайлНаДиске.Существует() Тогда
				...
				ТекстКомментария = ВернутьСтр("ru = 'Файл удален при очистке ненужных файлов.'")
	...
	Исключение
		...
		Если Не ПустаяСтрока(ИмяФайлаСПутем) Тогда
			...
			ЗаписьЖурналаРегистрации(ВернутьСтр("ru = 'Очистка ненужных файлов'", ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка()),
...
Процедура ПриЗаполненииВозможныхПравДляНастройкиПравОбъектов(ВозможныеПрава) Экспорт
	...
	Право.Имя           = "Чтение";
	Право.Заголовок     = ВернутьСтр("ru = 'Чтение'");
	Право.Подсказка     = ВернутьСтр("ru = 'Чтение папок и файлов'");
	...
	Право.Заголовок     = ВернутьСтр("ru = 'Изменение
	                                 |папок'");
	Право.Подсказка     = ВернутьСтр("ru = 'Добавление, изменение и
	...
	Право.Заголовок     = ВернутьСтр("ru = 'Изменение
	                                 |файлов'");
	Право.Подсказка     = ВернутьСтр("ru = 'Изменение файлов в папке'");
	// Права, требуемые для этого права.
	Право.ТребуемыеПрава.Добавить("Чтение");
	// Права для стандартных шаблонов ограничений доступа.
	...
	Право.Заголовок     = ВернутьСтр("ru = 'Добавление
	                                 |файлов'");
	Право.Подсказка     = ВернутьСтр("ru = 'Добавление файлов в папку'");
	// Права, требуемые для этого права.
	...
	Право.Заголовок     = ВернутьСтр("ru = 'Пометка
	                                 |удаления'");
	Право.Подсказка     = ВернутьСтр("ru = 'Пометка удаления файлов в папке'");
	// Права, требуемые для этого права.
	Право.ТребуемыеПрава.Добавить("ИзменениеФайлов");
	...
	Право.Заголовок     = ВернутьСтр("ru = 'Управление
	                                 |правами'");
	Право.Подсказка     = ВернутьСтр("ru = 'Управление правами папки'");
	// Права, требуемые для этого права.
	Право.ТребуемыеПрава.Добавить("Чтение");
...
Процедура ПриЗаполненииСпискаТекущихДел(ТекущиеДела) Экспорт
	...
	Для Каждого Раздел Из Разделы Цикл
		...
		Дело.Представление  = ВернутьСтр("ru = 'Редактируемые файлы'");
...
Функция ПолучитьСписокКодировок() Экспорт
	...
	СписокКодировок.Добавить("ibm852",       ВернутьСтр("ru = 'IBM852 (Центральноевропейская DOS)'"));
	СписокКодировок.Добавить("ibm866",       ВернутьСтр("ru = 'IBM866 (Кириллица DOS)'"));
	СписокКодировок.Добавить("iso-8859-1",   ВернутьСтр("ru = 'ISO-8859-1 (Западноевропейская ISO)'"));
	СписокКодировок.Добавить("iso-8859-2",   ВернутьСтр("ru = 'ISO-8859-2 (Центральноевропейская ISO)'"));
	СписокКодировок.Добавить("iso-8859-3",   ВернутьСтр("ru = 'ISO-8859-3 (Латиница 3 ISO)'"));
	СписокКодировок.Добавить("iso-8859-4",   ВернутьСтр("ru = 'ISO-8859-4 (Балтийская ISO)'"));
	СписокКодировок.Добавить("iso-8859-5",   ВернутьСтр("ru = 'ISO-8859-5 (Кириллица ISO)'"));
	СписокКодировок.Добавить("iso-8859-7",   ВернутьСтр("ru = 'ISO-8859-7 (Греческая ISO)'"));
	СписокКодировок.Добавить("iso-8859-9",   ВернутьСтр("ru = 'ISO-8859-9 (Турецкая ISO)'"));
	СписокКодировок.Добавить("iso-8859-15",  ВернутьСтр("ru = 'ISO-8859-15 (Латиница 9 ISO)'"));
	СписокКодировок.Добавить("koi8-r",       ВернутьСтр("ru = 'KOI8-R (Кириллица KOI8-R)'"));
	СписокКодировок.Добавить("koi8-u",       ВернутьСтр("ru = 'KOI8-U (Кириллица KOI8-U)'"));
	СписокКодировок.Добавить("us-ascii",     ВернутьСтр("ru = 'US-ASCII (США)'"));
	СписокКодировок.Добавить("utf-8",        ВернутьСтр("ru = 'UTF-8 (Юникод UTF-8)'"));
	СписокКодировок.Добавить("windows-1250", ВернутьСтр("ru = 'Windows-1250 (Центральноевропейская Windows)'"));
	СписокКодировок.Добавить("windows-1251", ВернутьСтр("ru = 'windows-1251 (Кириллица Windows)'"));
	СписокКодировок.Добавить("windows-1252", ВернутьСтр("ru = 'Windows-1252 (Западноевропейская Windows)'"));
	СписокКодировок.Добавить("windows-1253", ВернутьСтр("ru = 'Windows-1253 (Греческая Windows)'"));
	СписокКодировок.Добавить("windows-1254", ВернутьСтр("ru = 'Windows-1254 (Турецкая Windows)'"));
	СписокКодировок.Добавить("windows-1257", ВернутьСтр("ru = 'Windows-1257 (Балтийская Windows)'"));
...
Функция СобытиеЖурналаРегистрацииДляОбмена() 
	...
	Возврат ВернутьСтр("ru = 'Файлы.Не удалось отправить файл при обмене данными'", ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка());
