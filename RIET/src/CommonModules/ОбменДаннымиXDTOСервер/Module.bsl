...
Функция ОтменитьПроведениеОбъектаВИБ(Объект, Отправитель) Экспорт
	...
	Если Тип("ДокументСсылка.РеализацияТоваровУслуг") = ТипОбъекта ИЛИ Тип("ДокументСсылка.СчетФактураВыданный") = ТипОбъекта ИЛИ Тип("ДокументСсылка.Tickets") = ТипОбъекта ИЛИ Тип("ДокументСсылка.ПлатежноеПоручениеВходящее") = ТипОбъекта Тогда
		
		Если ТипОбъекта = Тип("ДокументСсылка.РеализацияТоваровУслуг") ИЛИ ТипОбъекта = Тип("ДокументСсылка.Tickets") Тогда
			ДатаДок = Объект.ДатаПроведения;
		Иначе
			ДатаДок = Объект.Дата;
		КонецЕсли;
