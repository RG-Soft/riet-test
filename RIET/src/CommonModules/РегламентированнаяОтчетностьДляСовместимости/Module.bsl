...
Функция ЭтоПБОЮЛ(Организация) Экспорт
	...
	Если ОбщегоНазначения.ЕстьРеквизитСправочника("ИностраннаяОрганизация", Организация.Метаданные()) Тогда
		Если Организация.ИностраннаяОрганизация = Null ИЛИ Организация.ИностраннаяОрганизация Тогда  //исправил Трефиленков Дмитрий, РГ-Софт, 11.09.2011
			Возврат Ложь;
		КонецЕсли;
	...
	//Иначе
		Если ИДКонфигурации() = "БП" Тогда
			Если РедакцияКонфигурации() = "1.0" Тогда
				Возврат (СтрДлина(СокрЛП(Организация.ИНН)) = 12);
			Иначе
...
Функция РедакцияКонфигурации() Экспорт
	...
	Возврат СокрЛП(Лев(Версия, СтрНайти(Сред(Версия, ПерваяТочка + 1), ".") + ПерваяТочка - 1));
...
Функция ПолучитьСведенияОбОрганизации(Знач Организация, Знач ДатаЗначения = Неопределено, Знач СписокПоказателей = Неопределено) Экспорт
	...
	Если Организация = Неопределено ИЛИ Организация = ОбщегоНазначения.ПустоеЗначениеТипа("СправочникСсылка.Организации")
		Или Организация = ОбщегоНазначения.ПустоеЗначениеТипа("СправочникСсылка.ПодразделенияОрганизаций") Тогда  // RG-SOFT Ставров 17.01.2012 добавлено для сотсавного типа
		Возврат ОргСведения;
	КонецЕсли;
	...
	АдресЭлектроннойПочтыФизЛица = Справочники.ВидыКонтактнойИнформации.СлужебныйАдресЭлектроннойПочтыПользователя;
	...
	Для Каждого Показатель Из СписокПоказателей Цикл
		...
		ИначеЕсли ИмяПоказателя = "ТипНП" Тогда
			...
			ИначеЕсли Организация.ИностраннаяОрганизация <> Null И Организация.ИностраннаяОрганизация  Тогда //исправил Ставров Илья, РГ-Софт, 18.01.2012
				Значение = 5;
			Иначе
		...
		ИначеЕсли ИмяПоказателя = "АдрПочт" Тогда
			...
			ИмяПредопределенного = Справочники.ВидыКонтактнойИнформации.АдресОрганизацииПочтовый;
		...
		ИначеЕсли ИмяПоказателя = "КодИОСтрРег" Тогда
			// проверка на <> Null из-за составного типа
			Если ИДКонф <> "ББУ" И ИДКонф <> "БГУ" И ИДКонф <> "ЗБУ" И Организация.ИностраннаяОрганизация <> Null И Организация.ИностраннаяОрганизация Тогда //исправил Ставров Илья, РГ-Софт, 18.01.2012
			...
			Если ПБОЮЛ Тогда
				...
				ИначеЕсли ИДКонф = "УПП" ИЛИ ИДКонф = "КА" Тогда
					...
					Если Результат <> Неопределено И Результат.Количество() > 0 И Результат[0].ФизическоеЛицо <> Справочники.ФизическиеЛица.ПустаяСсылка() Тогда
						ДанныеФЛ = РегистрыСведений.ФИОФизическихЛиц.СрезПоследних(ДатаЗначения, Новый Структура("ФизическоеЛицо", Результат[0].ФизическоеЛицо));
						Если ДанныеФЛ.Количество() > 0 Тогда
							Значение = СокрЛП(СокрЛП(ДанныеФЛ[0].Фамилия) + " " + СокрЛП(ДанныеФЛ[0].Имя) + " " + СокрЛП(ДанныеФЛ[0].Отчество));
						Иначе
							Значение = "";
...
Функция КПППоКомбинацииОрганизацииИКодаИФНС(Организация, КодИФНС) Экспорт
	...
	ИдКонф = ИДКонфигурации();
	Если ИдКонф <> "ББУ" И ИдКонф <> "ЗБУ" И ИдКонф <> "БГУ" И Организация.ЮридическоеФизическоеЛицо = Перечисления.ЮридическоеФизическоеЛицо.ФизическоеЛицо Тогда
		Возврат "";
	КонецЕсли;
...
Функция ВывестиНаПечатьЛистРаздела(Форма, СтруктураПараметровЛиста)
	...
	Пока МаксимальноеКоличествоОбластейМногострочнойЧасти(МногострочныеЧасти) > 0 Цикл
		...
		Для Сч = 0 По СписокВыводимыхНаПечатьОбластей.Количество() - 1 Цикл
			...
			//Если Рез = Ложь И СтрНайти(ВРЕГ(СписокВыводимыхНаПечатьОбластей.Получить(Сч).Представление), "МНОГОСТРОЧНАЯЧАСТЬ") <> 0 Тогда
			//	Продолжить
			//КонецЕсли;
...
Процедура ЗаписатьДанныеФИОВРегистр(Ссылка, Фамилия, Имя, Отчество) Экспорт
	...
	ФИОСрез = РегистрыСведений.ФИОФизическихЛиц.ПолучитьПоследнее(,Новый Структура("ФизическоеЛицо",Ссылка));
...
Процедура роВывестиЭлементыВариантаЗаполнения(Форма, ТекущаяЯчейка) Экспорт
	...
	Если СтрНайти(ИмяПоказателя, ":") > 0 Тогда
...
Процедура роПоказатьСтатикСтраницРаздела(Форма, ИмяТекущейСтраницыПанели) Экспорт
	...
	//Добавила попытку Федотова Л., РГ-Софт, 30.05.13, вопрос SLI-0003603
	Попытка
...
Процедура роПриАктивизацииОбластиРегламентированногоОтчета(Форма, Элемент, ИмяТекущейСтраницыПанели = Неопределено) Экспорт
	...
	Если СтруктураВзаимныхПереходовПрисутствует Тогда
		Если СтрНайти(Элемент.ТекущаяОбласть.Имя, "НомСтр") > 0 Тогда
...
Функция ВыбраннаяОбластьРегламентированногоОтчетаНеВерна(Форма, Область, СтандартнаяОбработка) Экспорт
	...
	//добавила Федотова Л., РГ-Софт, 29.03.11 ->
	Если СтрНайти(Область.Имя, "ОргПодписант") >0 Тогда
		Возврат Ложь;	
	КонецЕсли;
	//<-
	Попытка
...
Функция роЗаполнитьСведенияОбОрганизации(Форма, Знач СписокСведений, Дополнения = "", РазделДляВывода = "Титульный") Экспорт
	...
	Если ТипЗнч(СписокСведений) = Тип("Строка") Тогда
		Если СтрНайти(СписокСведений, "ОКВЭДКвадратикиТочкиРазделения") > 0 Тогда
...
Процедура ВвестиКодОГРННаТитульномЛистеРегламентированногоОтчета(Форма, ОбластьИмя) Экспорт
	...
	Если СтрНайти(ОбластьИмя, "ОГРН") > 0 Тогда
		Если СтрНайти(ОбластьИмя, "ОГРНИП") > 0 Тогда
			ПБОЮЛ = Истина;
...
Функция ВернутьЛогическийЗнакВСтроке(Знач Формула) Экспорт
	...
	Пока СтрНайти(Формула, "[") > 0 Цикл
		СкобкаОтк = СтрНайти(Формула, "[");
		СкобкаЗак = СтрНайти(Формула, "]");
	...
	ИначеЕсли СтрНайти(Формула, "<=") > 0 Тогда
...
// Процедура позволяет организовать выбор из справочника РегистрацииВНалоговомОргане
	...
	Если РучнойВвод Тогда
		Если ТипЗнч(ПервоначальноеЗначение) = Тип("Строка") Тогда
			...
			Пока Вводить Цикл
				...
				Если СтрНайти(ТекстВвода, "ОКАТО") > 0 Тогда
...
// Функция возаращает данные о представителе из справочника РегистрацииВНалоговомОргане.
//
// Параметры:
...
Функция ПолучитьФИОФизЛица(ФизЛицо, НаДату = Неопределено) Экспорт
	...
	Иначе
		ДанныеФЛ = РегистрыСведений.ФИОФизическихЛиц.СрезПоследних(?(ЗначениеЗаполнено(НаДату), НаДату, РабочаяДата), Новый Структура("ФизическоеЛицо", ФизЛицо));
		Если ДанныеФЛ.Количество() > 0 Тогда
...
Процедура СформироватьСоставКнопкиОчиститьДляРегОтчетов(Форма) Экспорт
	...
	Иначе
		...
		Если СтрНайти(ЗаголовокФормы, "декларация ") > 0 Тогда
			ЧтоОчищать = "Декларацию";
		ИначеЕсли СтрНайти(ЗаголовокФормы, "расчет ") > 0 Тогда
			ЧтоОчищать = "Расчет";
		КонецЕсли;
	КонецЕсли;
	...
	ИначеЕсли ИмяСтраницы = "Титульный" Тогда
		...
		Если СтрНайти(ПредставлениеРаздела, " лист") = 0  Тогда
...
Процедура МеханизмыОчисткиРегламентированныхОтчетов(Форма, Кнопка) Экспорт
	...
	Если СтрНайти(КнопкаИмя, "ОчиститьТекущийРаздел") > 0 Тогда
	...
	ИначеЕсли СтрНайти(КнопкаИмя, "ОчиститьТекущийЭкземплярМногостраничногоРаздела") > 0 Тогда
...
Функция ПрерватьАвтоЗаполнениеПоДаннымИБ(Форма, НазванияМногостраничныхРазделовДляУдаления = "") Экспорт
	...
	Если НазванияМногостраничныхРазделовДляУдаления <> "" Тогда
		...
		Если СтрНайти(НазванияМногостраничныхРазделовДляУдаления, ",") > 0 Тогда
...
Функция ДокументыСУчетомКритериевОтбораКалендарь(ИсточникОтчета, ДатаНачалаПериодаОтчета, ДатаКонцаПериодаОтчета, ОрганизацияОтбор, КодИФНС, Периодичность) Экспорт
	...
	Если НЕ ПустаяСтрока(КодИФНС) Тогда
		Запрос.Текст = Запрос.Текст + "
	               |	И РегламентированныйОтчет.КодНалоговогоОргана = &КодИМНС";
	КонецЕсли;
	Запрос.Текст = Запрос.Текст + "
...
Функция АвтосохранениеВозможно(Форма) Экспорт
	...
	Возврат Форма.мЗаписьЗапрещена <> Истина И Форма.мСохраненныйДок <> Неопределено //И Форма.мСохраненныйДок <> Новый("ДокументОбъект.РегламентированныйОтчет")  //изменила Петроченко Наталья 03.12.2012
...
Процедура НачалоВыбораСтатусаВРегламентированномОтчете(Форма, Элемент, СтандартнаяОбработка) Экспорт
	...
	Для Инд = 1 По Метаданные.Перечисления.СтатусыРегламентированныхОтчетов.ЗначенияПеречисления.Количество() Цикл
		...
		Если СтрНайти(ТекМетаданное.Имя, "Удалить") = 0 Тогда
...
//Добавил - РГ-Софт - Иванов Антон - 2008-10-31
//Доработанная Трефиленковым процедура получения сведений об Организации/Подразделении
//Используется по крайней мере в отчете АвансыТранспорт
Функция роЗаполнитьСведенияОбОрганизации1(Форма, СписокСведений, Дополнения = "", РазделДляВывода = "Титульный") Экспорт

	Если (Форма.Подразделение  = Неопределено) Или (Форма.Подразделение = ОбщегоНазначения.ПустоеЗначениеТипа("СправочникСсылка.ПодразделенияОрганизаций")) Тогда
		Возврат Неопределено;
	КонецЕсли;
	
	//Добавил РГ-Софт - Иванов Антон - 2009-01-16
	//Заменяем название параметра "ОКВЭДКвадратикиТочкиРазделения" на "ОКВЭД" как это делается в роЗаполнитьСведенияОбОрганизации
	ОКВЭДВариант = 0; // Стандартный вариант, нигде не применяется.
	Если ТипЗнч(СписокСведений) = Тип("Строка") Тогда
		Если СтрНайти(СписокСведений, "ОКВЭДКвадратикиТочкиРазделения") > 0 Тогда
			ОКВЭДВариант = 1; // Как минимум в Авансы земля 2006 и в годовом транспортном налоге
			СписокСведений = СтрЗаменить(СписокСведений, "ОКВЭДКвадратикиТочкиРазделения", "ОКВЭД");
		КонецЕсли;
	КонецЕсли;
	
	// Теперь получаем данные из глобальной общей функции
	ОргСведения = ПолучитьСведенияОбОрганизации(Форма.Подразделение, Форма.ДатаПодписи, СписокСведений);
	Результат = "";
	ПБОЮЛ = ЭтоПБОЮЛ(Форма.Организация);
	мТитульный = Форма.ЭлементыФормы["ПолеТабличногоДокумента" + РазделДляВывода];
...
КонецФункции // ЗаполнитьСведенияОбОрганизации1

Процедура ДобавитьКнопкиПечатиМашиночитаемыхФорм(Форма, Действие) Экспорт
	
	//Если КомпонентаПечатиМашиночитаемыхФормДоступна() Тогда
		ЭлементМеню = Форма.ЭлементыФормы.КоманднаяПанельДействий.Кнопки.ПодменюПечати;
		ЭлементМеню.Кнопки.Добавить("ПечататьМашиночитаемыйБланк", ТипКнопкиКоманднойПанели.Действие,
									"Печатать сразу бланк формы с двухмерным штрихкодом PDF417", Новый Действие("КоманднаяПанельДействийПечатьМашиночитаемогоБланка"));
		ЭлементМеню.Кнопки.Добавить("ПоказатьМашиночитаемыйБланк", ТипКнопкиКоманднойПанели.Действие,
		                            "Показать бланк формы с двухмерным штрихкодом PDF417", Действие);
		ЭлементМеню.Кнопки.Добавить("РазделительПечать", ТипКнопкиКоманднойПанели.Разделитель);
	//КонецЕсли;
	
КонецПроцедуры

Процедура ОткрытьФормуОтчета(Отчет, Подразделение, ОрганизацияОтчета = Неопределено, ДатаНачалаПериодаОтчета = Неопределено, ДатаОкончанияПериодаОтчета = Неопределено, ПериодичностьОтчета = Неопределено) Экспорт
