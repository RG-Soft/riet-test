...
// Формирует типовой отчет в таблицу значений	
Функция СформироватьТиповойОтчетExcel(ОтчетОбъект, Результат = Неопределено, ДанныеРасшифровки = Неопределено) Экспорт
	...
	#Если Клиент тогда
	Состояние(ВернутьСтр("ru='Если Вы хотите прервать вывод отчета, нажмите Ctrl+Break'"));
	#КонецЕсли
...
Функция ПолучитьСтруктуруПараметровТиповогоОтчета(ОтчетОбъект) Экспорт
	...
	Если СтруктураПараметров.Свойство("Версия") тогда
		СтруктураПараметров.Версия = ТиповыеОтчеты.РабочаяВерсияНастройкиОтчетов();
	Иначе
		СтруктураПараметров.Вставить("Версия", ТиповыеОтчеты.РабочаяВерсияНастройкиОтчетов());
	КонецЕсли;
...
функция ОбработкаРасшифровкиТиповогоОтчета(Расшифровка, СтандартнаяОбработка, ОтчетОбъект, ФормаОтчета, ДополнительныеРасшифровки = Неопределено) Экспорт
	...
	ИначеЕсли Команда = "РасшифроватьПоле" Тогда
		...
		Для каждого ПолеСпискаРасшифровки из СписокПолейРасшифровки Цикл
			ДостпноеПоле = ТиповыеОтчеты.ПолучитьДоступноеПоле(Новый ПолеКомпоновкиДанных(ПолеСпискаРасшифровки.Значение), ОтчетОбъект.КомпоновщикНастроек.Настройки.ДоступныеПоляГруппировок);
...
//	   	Панель = Неопределено;
		...
		// Дополнительная кнопка выбора периода
		// АДАПТИРОВАТЬ ПОД НОВУЮ ВЕРСИЮ ШАБЛОНА!!!
		//НовыйЭлемент = ЭлементыФормы.Добавить(Тип("Кнопка"), "ДополнительнаяКнопкаВыбораПериода", Истина, ЭлементыФормы.ПанельЗакладок);
		//НовыйЭлемент.Верх = Верх;
		//НовыйЭлемент.Лево = 213;
		//НовыйЭлемент.Ширина = 19;
		//НовыйЭлемент.Высота = 19;
		//НовыйЭлемент.Заголовок = "...";
		////НовыйЭлемент.Доступность = СписокВыбора.НайтиПоЗначению(ВариантСтандартногоПериода.ПроизвольныйПериод) <> Неопределено;
		//НовыйЭлемент.Доступность = Истина;
		//Действие = ФормаОтчета.ДействияЭлементовФормы.ДействияПанелиДополнительнаяКнопкаВыбораПериодаПриИзменении;
...
Процедура НарисоватьДинамическийОтборГоризонтальнойПанели(ФормаОтчета, Индекс, Лево, ЦветФонаКнопки, СтрокаОтбора, ЭлементДинамическогоОтбора, ДоступноеПоле, ПоследнийЭлемент = ложь, Список = истина, ПараметрыИсполненияОтчета = Неопределено, ПредыдущийЭлемент, СреднийРазмерЭлементаУправления)
	...
	НовыйЭлемент.Верх = 6;
	НовыйЭлемент.Лево = Лево;
	Если СтрНайти(Строка(ДоступноеПоле.Тип),"ПланСчетов") <> 0 тогда
...
Функция УстановитьВариантПоУмолчанию(ОтчетОбъект, ФормаОтчета) Экспорт
	...
	Если Не ФормаОтчета.ЭтоОтработкаРасшифровки Тогда
...
Функция ПолучитьЗначенияНастроекПанелиПользователя(ОтчетОбъект, ФормаОтчета) Экспорт
	...
	Если ЭлементыФормы.ПанельЗакладок.Страницы.Найти("Показатели") <> Неопределено тогда
		ВидимостьСтраниц.Вставить("Показатели", ЭлементыФормы.ПанельЗакладок.Страницы.Показатели.Видимость);
       	ЗначенияНастроек.Вставить("Показатели", ПолучитьСтруктуруПоказтелей(ЭлементыФормы));
	КонецЕсли;
...
Функция ПолучитьСтруктуруПоказтелей(ЭлементыФормы)
...
Процедура ПерерисоватьПанельНастроек(ОтчетОбъект, ФормаОтчета, ЗначенияНастроек = Неопределено, ПереноситьПараметры = истина) Экспорт
	...
	Для Индекс = 1 По Количество Цикл
		...
		Если Лев(Страница.Имя, 17) = "ДинамическийОтбор" или Лев(Страница.Имя, 20) = "ДинамическийПараметр" или Лев(Страница.Имя, 23) = "ДинамическаяГруппировка" Тогда
	...
	Если Параметры.Свойство("Группировки") тогда
		...
		ВсеГруппировки = ТиповыеОтчеты.ПолучитьЭлементыСтруктуры(ОтчетОбъект.КомпоновщикНастроек);
...
Процедура ОбработкаЗакрытияНастройкиОтчета(ОтчетОбъект, ФормаОтчета, Отказ = Ложь, СтандартнаяОбработка = Истина) Экспорт
	...
	Если ОтчетОбъект.СохраненнаяНастройка.Пустая() Тогда
		// Сохраним сам отчет
		РезультатВопроса = Вопрос(ВернутьСтр("ru='Сохранить изменения в основном варианте отчета?'"), РежимДиалогаВопрос.ДаНетОтмена);
...
Процедура ВыборВариантаОтчетаНаПанелиВариантов(ОтчетОбъект, ФормаОтчета, Элемент = Неопределено, СохраненаяНастройка = Неопределено) Экспорт
	...
	Если СохраненаяНастройка = Неопределено тогда
		...
		Если Не ФормаОтчета.ЭтоОтработкаРасшифровки И Не ФормаОтчета.РежимРедактированияНастройки Тогда
			ТиповыеОтчеты.СохранитьНастройкуПользователяНастройкиОтчета(ОтчетОбъект, ФормаОтчета);
		КонецЕсли;
	Иначе
...
Процедура ОбработкаНажатияКнопкиПодбор(ОтчетОбъект, ФормаОтчета, Элемент, Владелец = Неопределено, НазваниеРеквизита = "") Экспорт
	...
	ОбъектМетаданных = Метаданные.НайтиПоТипу(ТипПоля);
	Если ОбъектМетаданных = Неопределено И Метаданные.Справочники.Найти(ОбъектМетаданных.Имя) <> Неопределено Тогда
		Возврат;
	КонецЕсли;
...
Функция ПолучитьПараметрИзФормы(ОтчетОбъект, ФормаОтчета, Элемент) Экспорт
	...
	Возврат Новый Структура("СтрокаПараметра, Индекс, Значение, Использование", СтрокаПараметра, Индекс, Значение, ВидСравнения);;
КонецФункции
...
Процедура ОбработкаВыбораФормыОтчета(ОтчетОбъект, ФормаОтчета, ЗначениеВыбора, Источник) Экспорт
	...
	ИначеЕсли СтрНайти(Источник.КлючУникальности, "ОтборПо") <> 0 тогда
...
Процедура ПриНажатииКнопкиВверхВниз(ФормаОтчета, ОтчетОбъект, Кнопка) Экспорт
	...
	Если СтрНайти(Кнопка.Имя,"Строк") <> 0 тогда
		ТабличноеПоле = ЭлементыФормы.Найти("ДинамическаяГруппировка" + Индекс + "ТабличноеПолеСтрок");
	ИначеЕсли СтрНайти(Кнопка.Имя,"Колонок") <> 0 тогда
...
Процедура СохранитьНастройкиФормыОтчета(ОтчетОбъект, ЭтаФорма) Экспорт
	...
	Если СтрНайти(НастраиваемыйОбъект, "Отчет") = 0 тогда
		Возврат;
	КонецЕсли;
...
#Если Клиент ИЛИ ВнешнееСоединение Тогда
	...
	Функция ПолучитьЗначенияНастроекПанелиПользователяПоУмолчанию(ОтчетОбъект, ФормаОтчета = Неопределено)
	...
	ВидимостьСтраниц.Вставить("Показатели", Новый Соответствие);
	ЗначенияНастроек.Вставить("ВидимостьСтраниц", ВидимостьСтраниц);
...
Процедура ВыводЗаголовкаТиповогоОтчета(ОтчетОбъект, Результат, ВыводВФормуОтчета = Истина)
	...
	//Если ПараметрыИсполненияОтчета <> Неопределено И ПараметрыИсполненияОтчета.Свойство("ПриВыводеЗаголовкаОтчета") И НЕ ПараметрыИсполненияОтчета.ПриВыводеЗаголовкаОтчета Тогда //ОтчетОбъект.РасширеннаяНастройка И 
	//	Возврат;                                           
	//КонецЕсли;
	...
	Если ЗначениеПараметраНачалоПериода <> Неопределено 
	   И ЗначениеПараметраКонецПериода <> Неопределено Тогда
		...
		Если НачалоПериода = '00010101' И КонецПериода = '00010101' Тогда
			ОписаниеПериода = ВернутьСтр("ru='Период не установлен'");
		...
		Иначе
			ОписаниеПериода = ВернутьСтр("ru='Неправильно задан период!'");
		КонецЕсли;
	ИначеЕсли ЗначениеПараметраПериод <> Неопределено Тогда
		Период = ЗначениеПараметраПериод.Значение;
		Если Период = '00010101' или Период = Неопределено Тогда
			ОписаниеПериода = ВернутьСтр("ru='на '") + Формат(ТекущаяДата(), "ДП = ...");
		Иначе
			ОписаниеПериода = ?(Период = КонецДня(Период), ВернутьСтр("ru='на конец дня '"), "") + Формат(Период, "ДФ = дд.ММ.гггг; ДП = ...");
		КонецЕсли;
	Иначе
		ОписаниеПериода = "";
	КонецЕсли;
...
Процедура ЗаполнитьПолеНабораДанныхОстаток(ПолеНабораДанных, ГруппаОстатка) Экспорт
	...
	Если СтрНайти(ПутьКДанным, "НачальныйОстаток") > 0 ИЛИ СтрНайти(ПутьКДанным, "КонечныйОстаток") > 0 Тогда
		...
		КонецЕсли;
		Если СтрНайти(ПутьКДанным, "НачальныйОстаток") > 0 Тогда
			ПолеНабораДанных.Роль.ТипОстатка = ТипОстаткаКомпоновкиДанных.НачальныйОстаток;
		ИначеЕсли СтрНайти(ПутьКДанным, "КонечныйОстаток") > 0 Тогда
...
Функция ПолучитьТаблицуДоступныхВариантов(НастраиваемыйОбъект, Пользователь = Неопределено, СПомеченнымиНаУдаление = Ложь, ТипНастройки = Неопределено) Экспорт
...
Функция ПолучитьПараметрыПанелиПользователяПоУмолчанию(ОтчетОбъект, ФормаОтчета = Неопределено)
...
Функция ПолучитьСтруктуруДинамическихПараметров(Параметры, ЭлементыФормы)
...
Функция ПолучитьСтруктуруДинамическихГруппировок(Параметры, ЭлементыФормы)
...
Процедура ЗаполнитьОтборыПоУмолчанию(ОтчетОбъект, СписокПолейПодстановки)
	...
	Для каждого СписокПолей из СписокПолейПодстановки Цикл
		...
		// Изменил РГ-Софт - Иванов Антон - 2009-08-30
		Значение        = УправлениеПользователями.ПолучитьЗначениеПоУмолчанию(ТекПользователь, СписокПолей.Значение);
		ТиповыеОтчеты.ПодставитьОтборПоПолюВПанель(ОтчетОбъект, СписокПолей.Ключ, Значение);
				...
				Для каждого Группировка из Группировки Цикл
					Если  Группировка.Значение.Имя = ЗначениДинГруппировки.Значение.ИмяГруппировки тогда
						Если ТипЗнч(Группировка) = Тип("ТаблицаКомпоновкиДанных") или ТипЗнч(Группировка) = Тип("ДиаграммаКомпоновкиДанных") тогда
							...
							КонецЕсли;
							ГруппировкиСтрок = ТиповыеОтчеты.ПолучитьГруппировки(Строки);
							ГруппировкиКолонок = ТиповыеОтчеты.ПолучитьГруппировки(Колонки);
...
Функция ЭтоСтараяВерсияОтчета(ОтчетОбъект) Экспорт
	...
	Возврат (Найти(ПолучитьИдентификаторОбъекта(ОтчетОбъект), "ОтчетОбъект") = 0 ИЛИ (ОтчетОбъект.Метаданные().Реквизиты.Найти("НастройкаПериода") <> Неопределено));
...
Функция ПолучитьНастройкуОтчета(ТекстФайлаНастройки) Экспорт
	...
	НачалоВерсии = СтрНайти(ТекстФайлаНастройки, "<ВЕРСИЯ>");
	КонецВерсии  = СтрНайти(ТекстФайлаНастройки, "</ВЕРСИЯ>");
	СтруктураНастроек.Версия = Сред(ТекстФайлаНастройки, НачалоВерсии + 8, КонецВерсии - НачалоВерсии - 8); 
	НачалоНаименования = СтрНайти(ТекстФайлаНастройки, "<НАИМЕНОВАНИЕ>");
	КонецНаименования  = СтрНайти(ТекстФайлаНастройки, "</НАИМЕНОВАНИЕ>");
	СтруктураНастроек.Наименование = Сред(ТекстФайлаНастройки, НачалоНаименования + 14, КонецНаименования - НачалоНаименования - 14); 
	НачалоОбъект = СтрНайти(ТекстФайлаНастройки, "<ОБЪЕКТ>");
	КонецОбъект  = СтрНайти(ТекстФайлаНастройки, "</ОБЪЕКТ>");
	СтруктураНастроек.НастраиваемыйОбъект = Сред(ТекстФайлаНастройки, НачалоОбъект + 8, КонецОбъект - НачалоОбъект - 8 ); 
	НачалоОписание = СтрНайти(ТекстФайлаНастройки, "<ОПИСАНИЕ>");
	КонецОписание  = СтрНайти(ТекстФайлаНастройки, "</ОПИСАНИЕ>");
	СтруктураНастроек.Описание = Сред(ТекстФайлаНастройки, НачалоОписание + 10,  КонецОписание - НачалоОписание - 10 ); 
	НачалоНастройки = СтрНайти(ТекстФайлаНастройки, "<НАСТРОЙКА>");
	КонецНастройки  = СтрНайти(ТекстФайлаНастройки, "</НАСТРОЙКА>");
...
Процедура ЗагрузитьНастройкиОтчетов(СписокНастроек, СоответсвиеИмяОтчета = Неопределено, Перезаписать = ложь) Экспорт
	...
	Для каждого ЭлементСпискаНастроек из СписокНастроек Цикл
		...
		КонецЕсли;
		ТиповыеОтчеты.ЗагрузитьНастройкуПредопределенногоЭлемента(ЭлементСпискаНастроек.Значение, ИмяОтчета,,Перезаписать );
...
Процедура ПодставитьОтборПоПолюВПанель(ОтчетОбъект, Поле, Значение) Экспорт
	...
	ЗначениеНастройкиПанели = ТиповыеОтчеты.ПолучитьЗначенияНастроекПанелиПользователяОбъекта(ОтчетОбъект);
