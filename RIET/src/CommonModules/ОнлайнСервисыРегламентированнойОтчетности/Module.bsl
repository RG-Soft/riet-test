...
Функция ПолучитьРесурсССервера(Соединение, ИмяРесурсаНаСервере, ОписаниеОшибки = Неопределено) Экспорт
	...
	ИспользуетсяТестовыйСервер = ОнлайнСервисыРегламентированнойОтчетности.ИспользоватьТестовыйСервер();
...
Процедура ВключитьМеханизмОнлайнСервисовРО() Экспорт
	...
	ОнлайнСервисыРегламентированнойОтчетности.СохранитьОбщиеНастройкиМеханизмаОнлайнСервисовРО(Истина, Ложь);
	
КонецПроцедуры

#КонецОбласти

Процедура ЗапуститьФоновоеЗаданиеАктуализироватьСлужебнуюИнформациюМеханизмаОнлайнСервисовРО(УникальныйИдентификаторФормы) Экспорт
	
	ДлительныеОперации.ЗапуститьВыполнениеВФоне(
			УникальныйИдентификаторФормы, 
			"ОнлайнСервисыРегламентированнойОтчетности.АктуализироватьСлужебнуюИнформациюМеханизмаОнлайнСервисовРО",
			Новый Структура(),
			ВернутьСтр("ru = 'Производится обновление служебной информации из Интернет...'"));
...
Функция ПроверкаФормыПоПериодуПримененияСервер(СтруктураРеквизитовФормы, ДатаКонцаПериодаОтчета, ИмяФормы, ИДОтчета) Экспорт
	...
	Попытка
		ДеревоФормИФорматовОтчета = Отчеты[ИДОтчета].ДеревоФормИФорматов();
	Исключение
	    Возврат СведенияПоБлокировке;
	КонецПопытки; 
...
Функция ДоступВИнтернетОткрыт(ИспользоватьПрокси, ИмяПользователя = "", Пароль = "") ЭКспорт
	...
	Соединение = ОнлайнСервисыРегламентированнойОтчетности.УстановитьСоединениеССерверомМеханизмаОнлайнСервисовРО();
	ОшибкаУстановкиСоединения = Ложь;
	Если Соединение = Неопределено Тогда
		ОшибкаУстановкиСоединения = Истина;
	Иначе
		КлючевойФайл = ОнлайнСервисыРегламентированнойОтчетности.ПолучитьРесурсССервера(Соединение, "infomap.dat");
...
Функция ПредставлениеФормы(ИДОтчета, ВыбраннаяФорма) Экспорт
	...
	Попытка
		ТаблицаФормОтчета = Отчеты[ИДОтчета].ТаблицаФормОтчета();
	Исключение
	КонецПопытки; 
