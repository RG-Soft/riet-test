...
Процедура УстановитьРасширенныйЗаголовокПриложения(ПриЗапуске = Ложь) Экспорт
	...
	// ИЗМЕНИЛ ИВАНОВ АНТОН. НЕ НАДО ПОДСТАВЛЯТЬ ПРЕДСТАВЛЕНИЕ КОНФИГУРАЦИИ.
	ШаблонЗаголовка = "%1 / %2";
	ЗаголовокПриложения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(ШаблонЗаголовка, 
		СокрЛП(ПараметрыКлиента.ЗаголовокПриложения), ПараметрыКлиента.ПредставлениеПользователя);
	...
	//	ШаблонЗаголовка = "%1 / %2";
	//	ЗаголовокПриложения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(ШаблонЗаголовка, 
	//		ВернутьСтр("ru = 'Не установлены разделители'"), ПараметрыКлиента.ПодробнаяИнформация);
	//КонецЕсли;
...
Процедура ПриНачалеРаботыСистемы(Знач ОповещениеЗавершения = Неопределено, НепрерывноеВыполнение = Истина) Экспорт
	...
	// { RGS AArsentev при режиме совместимости с 8.3.8 вылетает ошибка, поэтому получим все параметры при запуске.
	СтандартныеПодсистемыКлиентПовтИсп.ПараметрыРаботыКлиента();
	// } RGS AArsentev при режиме совместимости с 8.3.8 вылетает ошибка
...
Процедура ПоказатьПредупреждениеИПродолжить(Параметры, ТекстПредупреждения) Экспорт
	...
	Если Параметры.Отказ Тогда
		...
		Кнопки.Добавить("Завершить",     ВернутьСтр("ru = 'Завершить работу'"));
		Кнопки.Добавить("Перезапустить", ВернутьСтр("ru = 'Перезапустить'"));
...
Процедура ОткрытьСписокАктивныхПользователей(ПараметрыФормы = Неопределено, ВладелецФормы = Неопределено) Экспорт
	...
	Иначе
		...
		ПоказатьПредупреждение(,
			ВернутьСтр("ru = 'Для того чтобы открыть список активных пользователей, перейдите в меню
...
Процедура ВывестиПредупреждение(Форма, ПараметрыПредупреждения, ОбработчикЗавершения = Неопределено) Экспорт
	...
	Если ЗначениеЗаполнено(Контекст.Подробно) Тогда
		КнопкаПоУмолчанию = 1;
		Кнопки.Добавить(1, ВернутьСтр("ru = 'Подробнее...'"));
	КонецЕсли;
	Если ТипЗнч(Форма) = Тип("УправляемаяФорма") И ЗначениеЗаполнено(Контекст.ПутьКРеквизитуФормы) Тогда
		Кнопки.Добавить(2, ВернутьСтр("ru = 'Перейти к реквизиту'"));
	КонецЕсли;
	Кнопки.Добавить(КодВозвратаДиалога.ОК, ВернутьСтр("ru = 'Закрыть'"));
...
Процедура ПоказатьПодробнуюИнформацию(Обработчик, Текст, Заголовок = Неопределено) Экспорт
	...
	Если Не ЗначениеЗаполнено(НастройкиДиалога.Заголовок) Тогда
		НастройкиДиалога.Заголовок = ВернутьСтр("ru = 'Подробнее'");
	КонецЕсли;
	
	Кнопки = Новый СписокЗначений;
	Кнопки.Добавить(0, ВернутьСтр("ru = 'Закрыть'"));
...
Процедура ИнтерактивнаяОбработкаПроверкиВерсииПлатформыПриЗапуске(Параметры, Неопределен) Экспорт
	...
	Если ПараметрыКлиента.РаботаВПрограммеЗапрещена Тогда
		Если ПараметрыКлиента.ЕстьДоступДляОбновленияВерсииПлатформы Тогда
			ТекстСообщения = ВернутьСтр("ru = 'Вход в программу невозможен.
	...
	Иначе
		Если ПараметрыКлиента.ЕстьДоступДляОбновленияВерсииПлатформы Тогда
			ТекстСообщения = 
				ВернутьСтр("ru='Рекомендуется завершить работу программы и обновить версию платформы 1С:Предприятия.
			         |В противном случае некоторые возможности программы будут недоступны или будут работать некорректно.'");
		Иначе
			ТекстСообщения = 
				ВернутьСтр("ru='Рекомендуется завершить работу программы и обратиться к администратору для обновления версии платформы 1С:Предприятия.
			         |В противном случае некоторые возможности программы будут недоступны или будут работать некорректно.'");
		КонецЕсли;
	КонецЕсли;
...
Процедура ИдентификаторыОбъектовМетаданныхФормаСпискаСписокВыборЗначения(Форма, Элемент, Значение, СтандартнаяОбработка) Экспорт
	...
	Если НЕ Форма.ВыбиратьГруппыОбъектовМетаданных
	   ...
	   И НЕ ЗначениеЗаполнено(Элемент.ТекущиеДанные.Родитель) Тогда
		...
		Иначе
			ПоказатьПредупреждение(,
				ВернутьСтр("ru = 'Невозможно выбрать группу объектов метаданных.
...
Процедура УстановитьРазделениеСеанса()
	...
	Если ПараметрыЗапуска.Количество() < 2 Тогда
		ВызватьИсключение
			ВернутьСтр("ru = 'При указании параметра запуска ВойтиВОбластьДанных,
	...
	Исключение
		ВызватьИсключение
			ВернутьСтр("ru = 'Значением разделителя в параметре ВойтиВОбластьДанных должно быть число.'");
	КонецПопытки;
...
Процедура ВопросПользователюПередЗавершениемРаботыСистемы(Параметры, ОбработкаОтвета) Экспорт
	...
	// { начало изменения RG-Soft
	Кнопки = Новый СписокЗначений;
	Кнопки.Добавить("КодВозвратаДиалога.Да",  НСтр("ru = 'Завершить'; en = 'Quit'"));
	...
	ПараметрыВопроса.КнопкаТаймаута = КодВозвратаДиалога.Да;
	...
	ПоказатьВопросПользователю(ОбработкаОтвета, НСтр("ru = 'Завершить работу с программой?'; en = 'Do you really want to quit?'"), Кнопки, ПараметрыВопроса);
	// } конец изменения RG-Soft
...
Процедура ОткрытьПрикладнуюФормуПредупреждения(Параметры, ОбработкаОтвета, ПредупреждениеПользователю, ИмяФормы, ПараметрыФормы)
	...
	Если ДействиеГиперссылка.Свойство("ПрикладнаяФормаПредупреждения", Форма) Тогда
		...
		Если ДействиеГиперссылка.Свойство("ПараметрыПрикладнойФормыПредупреждения", ПараметрыФормы) Тогда
			...
			ПараметрыФормы.Вставить("ЗаголовокКнопкиДа",  ВернутьСтр("ru = 'Завершить'"));
			ПараметрыФормы.Вставить("ЗаголовокКнопкиНет", ВернутьСтр("ru = 'Отмена'"));
...
Процедура ОбработатьОшибкуПриЗапускеИлиЗавершении(Параметры, ИнформацияОбОшибке, Событие, ПрекратитьРаботу = Ложь)
	...
	ТекстПредупреждения = НачалоОписанияОшибки + Символы.ПС
		+ ВернутьСтр("ru = 'Техническая информация об ошибке записана в журнал регистрации.'")
...
Процедура ПоказатьПомещениеФайлаПриПодключенииРасширенияРаботыСФайлами(РасширениеПодключено, ДополнительныеПараметры) Экспорт
	...
	Если ПараметрыДиалога.Свойство("Режим") Тогда
		...
		Если Режим = РежимДиалогаВыбораФайла.ВыборКаталога Тогда
			ВызватьИсключение ВернутьСтр("ru = 'Выбор каталога не поддерживается'");
		КонецЕсли;
	Иначе
...
Процедура ОповеститьОНехваткеПамяти() Экспорт
	...
	Заголовок = ВернутьСтр("ru = 'Скорость работы снижена'");
	Текст = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(ВернутьСтр("ru = 'Рекомендуется увеличить
...
Функция Обработчик(Сценарий, КоличествоПараметров = 2)
	...
	Иначе
		ВызватьИсключение ВернутьСтр("ru = 'Обработчик оповещения для более чем 2 параметров не поддерживается'");
	КонецЕсли;
КонецФункции
