...
Процедура ОбновлениеИндексаППД() Экспорт
	...
	ОбновитьИндекс(ВернутьСтр("ru = 'Обновление индекса ППД'"), Ложь, Истина);
...
Процедура СлияниеИндексаППД() Экспорт
	...
	ОбновитьИндекс(ВернутьСтр("ru = 'Слияние индекса ППД'"), Истина);
...
Процедура ПриЗаполненииСпискаТекущихДел(ТекущиеДела) Экспорт
	...
	Дело.Представление  = ВернутьСтр("ru = 'Индекс полнотекстового поиска устарел'");
	...
	Дело.Подсказка      = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
		ВернутьСтр("ru = 'Последнее обновление %1 назад'"), Интервал);
...
Процедура ОбновитьИндекс(ПредставлениеПроцедуры, РазрешитьСлияние = Ложь, Порциями = Ложь)
	...
	// ЗАКОММЕНТИРОВАЛ ИВАНОВ АНТОН. НАМ НЕ НУЖНЫ ЛИШНИЕ СООБЩЕНИЯ В ЖУРНАЛЕ
	//ЗаписьЖурнала(Неопределено, ВернутьСтр("ru = 'Запуск процедуры ""%1"".'"), , ПредставлениеПроцедуры);
	
	Попытка
		ПолнотекстовыйПоиск.ОбновитьИндекс(РазрешитьСлияние, Порциями);
		// ЗАКОММЕНТИРОВАЛ ИВАНОВ АНТОН. НАМ НЕ НУЖНЫ ЛИШНИЕ СООБЩЕНИЯ В ЖУРНАЛЕ
		//ЗаписьЖурнала(Неопределено, ВернутьСтр("ru = 'Успешное завершение процедуры ""%1"".'"), , ПредставлениеПроцедуры);
	Исключение
		ЗаписьЖурнала(Неопределено, ВернутьСтр("ru = 'Ошибка выполнения процедуры ""%1"":'"), ИнформацияОбОшибке(), ПредставлениеПроцедуры);
	КонецПопытки;
...
Процедура ЗаписьЖурнала(УровеньЖурнала, КомментарийСПараметрами, ИнформацияОбОшибке = Неопределено, Параметр = Неопределено)
	...
	ЗаписьЖурналаРегистрации(
		ВернутьСтр("ru = 'Полнотекстовое индексирование'", ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка()), 
		УровеньЖурнала, , , 
		ТекстДляЖурнала);
