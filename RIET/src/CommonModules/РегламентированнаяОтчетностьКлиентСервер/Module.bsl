...
Процедура ПрорисоватьПодменюПечать(Форма) Экспорт
	...
	Если Форма.СтруктураРеквизитовФормы.мПечатьБезШтрихкодаРазрешена <> Неопределено
	 ИЛИ Форма.СтруктураРеквизитовФормы.ДоступенМеханизмПечатиРеглОтчетностиСДвухмернымШтрихкодомPDF417 Тогда
		...
		Форма.Элементы.ПечататьСразуСДвухмернымШтрихкодомPDF417.Видимость = НЕ РегламентированнаяОтчетностьВызовСервера.ПроверитьВозможностьВыводаМашиночитаемойФормыЧерезВебСервис(, Ложь, ЭтоВебКлиент);
		
		Если Форма.СтруктураРеквизитовФормы.мПечатьБезШтрихкодаРазрешена = Неопределено Тогда
			
			Форма.Элементы.ПечататьСразу.Видимость = Ложь;
			Форма.Элементы.ПоказатьБланк.Видимость = Ложь;
			
		Иначе
			
			Форма.Элементы.ПечататьСразу.Видимость = Форма.СтруктураРеквизитовФормы.мПечатьБезШтрихкодаРазрешена;
	...
	Иначе
		...
		Форма.Элементы.ПечататьСразуСДвухмернымШтрихкодомPDF417.Видимость = Ложь;
...
Процедура ЗаполнитьСведенияОбОрганизацииДляОтчетаСтатистики(Форма, ВПрограммеИзмененаДатаПодписи = Ложь, ИмяПоля = "ПолеТабличногоДокументаФормаОтчета") Экспорт 
	...
	Если (Форма.СтруктураРеквизитовФормы.Организация = Неопределено)
	 Или (Форма.СтруктураРеквизитовФормы.Организация = РегламентированнаяОтчетностьКлиентСервер.ПустоеЗначениеТипа("СправочникСсылка.Организации")
	 // { RGS MYurkevich 9/11/2015 2:57:39 PM - RCA-0000094
	 И Форма.СтруктураРеквизитовФормы.Подразделение = РегламентированнаяОтчетностьКлиентСервер.ПустоеЗначениеТипа("СправочникСсылка.ПодразделенияОрганизаций")
	 // } RGS MYurkevich 9/11/2015 2:57:51 PM - RCA-0000094
	 ) 
	 Тогда
		Возврат;
	КонецЕсли;
	...
	// { RGS MYurkevich 9/11/2015 2:58:45 PM - RCA-0000094
	Если Форма.СтруктураРеквизитовФормы.Подразделение = РегламентированнаяОтчетностьКлиентСервер.ПустоеЗначениеТипа("СправочникСсылка.ПодразделенияОрганизаций") Тогда
	...
	Иначе	
			ОргСведения = РегламентированнаяОтчетностьВызовСервера.ПолучитьСведенияОбОрганизации(
		Форма.СтруктураРеквизитовФормы.Подразделение, Форма.ДатаПодписи, Сведения);
    КонецЕсли;

	// } RGS MYurkevich 9/11/2015 2:58:53 PM - RCA-0000094
...
Процедура ЗаполнитьСведенияОбОрганизацииДляОтчетаСтатистикиЗП(Форма, ВПрограммеИзмененаДатаПодписи = Ложь) Экспорт
	...
	Если (НЕ ЗначениеЗаполнено(Форма.ПолеТабличногоДокументаФормаОтчета.Области.ОргКодОКПО.Значение)) ИЛИ (Форма.СтруктураРеквизитовФормы.мВПрограммеИзмененаОрганизация) Тогда
		Если ЗначениеЗаполнено(Форма.ОбособленноеПодразделение) Тогда
			Попытка
				// Устанавливаем в "ОргКодОКПО" значение "ИдентификационныйНомерДляТОГС" обособленного подразделения.
				Форма.ПолеТабличногоДокументаФормаОтчета.Области.ОргКодОКПО.Значение = СокрЛП(Форма.ОбособленноеПодразделение.ИдентификационныйНомерДляТОГС);
			Исключение
				Форма.ПолеТабличногоДокументаФормаОтчета.Области.ОргКодОКПО.Значение = "";
			КонецПопытки;
		Иначе
			// Устанавливаем в "ОргКодОКПО" значение "ОКПО" организации.
			Если НЕ РегламентированнаяОтчетностьВызовСервера.ЭтоЮридическоеЛицо(Форма.СтруктураРеквизитовФормы.Организация) Тогда
				Форма.ПолеТабличногоДокументаФормаОтчета.Области.ОргКодОКПО.Значение = "";
			Иначе
...
// { RGS LFedotova 03.03.2018 0:29:06 - вопрос SLI-0007489
...
//	
//	Возврат СостояниеСдачи.Пустая() ИЛИ (СостояниеСдачи = ПредопределенноеЗначение(
//		"Перечисление.СостояниеСдачиОтчетности.ДокументооборотНеНачат"));
//	
//КонецФункции
...
//    Вариант 2. УправляемаяФорма - экранная форма объекта регламентированной отчетности.
//
// Возвращаемое значение:
//  Массив - статусы (строковые значения).
//
Функция СтатусыОбъектовРеглОтчетностиПриРучномВводе(ОбъектРеглОтчетности = Неопределено) Экспорт
	
	Статусы = Новый Массив;
	Статусы.Добавить("В работе");
	Статусы.Добавить("Подготовлено");
	Статусы.Добавить("Сдано");
	
	//РегламентированнаяОтчетностьКлиентСерверПереопределяемый.ДополнитьСтатусыОбъектовРеглОтчетностиПриРучномВводе(
	//Статусы, ОбъектРеглОтчетности);
	
	Для Инд = 0 ПО Статусы.Количество() - 1 Цикл
		Статусы[Инд] = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru = '%1'"), Статусы[Инд]);
	КонецЦикла;
...
#КонецОбласти
// } RGS LFedotova 03.03.2018 0:29:26 - вопрос SLI-0007489 