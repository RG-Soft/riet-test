...
Функция ПолучитьСведенияОбОрганизации(Знач Организация, Знач ДатаЗначения = Неопределено, Знач СписокПоказателей = Неопределено) Экспорт
	...
	Если Организация = Неопределено ИЛИ Организация = РегламентированнаяОтчетностьКлиентСервер.ПустоеЗначениеТипа("СправочникСсылка.Организации") 
		//Обновление рег. Отчетности на бух. Корп 3.0.38.39
		Или Организация = ОбщегоНазначения.ПустоеЗначениеТипа("СправочникСсылка.ПодразделенияОрганизаций")// RG-SOFT Ставров 17.01.2012 добавлено для сотсавного типа
		//<=
		Тогда
...
Функция ПроверитьВозможностьВыводаМашиночитаемойФормыЧерезВебСервис(АдресПараметровПроксиВоВременномХранилище = "", ПолучитьОпределенияВебСервиса = Истина, ЭтоВызовИзВебКлиента = Ложь) Экспорт
	...
	ИспользоватьВебСервисДляФормированияМашиночитаемыхФорм = Ложь;
	ИспользоватьВебСервисТолькоВРежимеВебКлиента           = Ложь;
	АдресТочкиПодключенияВебСервиса                        = Неопределено;
	ИмяПользователяВебСервиса                              = Неопределено;
	ПарольПользователяВебСервиса                           = Неопределено;
	//<=
...
Функция ПроверитьКонтрольныеСоотношенияВОтчете(АдресВозвращаемыхПараметровДляПроверкиВФоне) Экспорт
	Возврат РегламентированнаяОтчетность.ПроверитьКонтрольныеСоотношенияВОтчете(ПолучитьИзВременногоХранилища(АдресВозвращаемыхПараметровДляПроверкиВФоне));
КонецФункции
