...
//получает дату запрета редактирования для объекта
//Данные - документ для которого ведется поиск

Функция ПолучитьДатуЗапретаРедактирования(Знач ОбъектДокумента,
	ОписаниеДанных = Неопределено,
	ОписаниеОшибки = Null,
	УзелПроверкиЗапретаЗагрузки = Неопределено) Экспорт
	...
	УстановитьПривилегированныйРежим(Истина);
	ДействующиеДаты = ДатыЗапретаИзмененияСлужебный.ДействующиеДатыЗапрета();
	ДатыЗапретаПоОбъектамНеУказаны = ДействующиеДаты.ДляПользователей.ДатыЗапретаПоОбъектамНеУказаны;
	ДанныеДляПроверки = ДатыЗапретаИзмененияСлужебный.ДанныеДляПроверкиИзОбъекта(ОбъектДокумента,
	...
	// Приоритеты дат запрета изменения:
	// 1. Для раздела и объекта.
	//    Для раздела и любого объекта (объект = раздел).
	//    Для любого раздела (пустой раздел) и любого объекта (объект = раздел).
	// 2. Для пользователя.
	...
	Иначе
		Разделы = ДействующиеДаты.ДляИнформационныхБаз.Разделы;
		ПустойУзелПланаОбмена = ОбщегоНазначения.МенеджерОбъектаПоСсылке(
		УзелПроверкиЗапретаЗагрузки).ПустаяСсылка();
	КонецЕсли;
	...
	МаксДата = ДатаЗапрета;
	Для каждого текСтрока Из ЗапретыИзменения Цикл
		Если МаксДата = Неопределено ИЛИ МаксДата<текСтрока.Дата Тогда
			
			МаксДата = текСтрока.Дата;
