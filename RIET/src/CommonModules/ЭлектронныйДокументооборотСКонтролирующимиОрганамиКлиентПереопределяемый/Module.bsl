...
Процедура ВыбратьНДСДокументы(ПараметрыОтбора) Экспорт 
	
	УчетНДСКлиент.ПолучитьСвойстваДокументовНДСДляПередачиФНС(ПараметрыОтбора)
	
КонецПроцедуры

//Программный интерфейс для выбора электронных документов 
//Открывает форму выбора электронных документов
//Оповещает при закрытии о выборе документов
//Имя события "ЗакрытаФормаВыбораДокументовИБДляПередачиФНС"
//
//Параметры процедуры: 
//	(необязательный) ПараметрыОтбора, тип Структура. 
//	Задает начальные значения отборов. 
...
//	(обязательный) 		Ключ: «СсылкаДокументИБ»,	Значение: ссылка на документ ИБ
//	(обязательный) 		Ключ: «ВидДокумента», 		Значение: Строка
//	(необязательный) 	Ключ: «НомерДокумента», 	Значение: Строка
//	(необязательный) 	Ключ: «ДатаДоговора», 		Значение: Дата
//
//В случае отмены выбора возвращается Неопределено.
//
//Пример (рассчитан на версию БЭД не ниже 1.2.2.1):
//ЭлектронныеДокументыКлиент.ПолучитьСвойстваДокументовИБДляПередачиФНС(ПараметрыОтбора);
Процедура ВыбратьЭлектронныеДокументы(ПараметрыОтбора) Экспорт
