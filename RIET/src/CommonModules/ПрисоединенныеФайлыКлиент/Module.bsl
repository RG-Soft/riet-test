...
//
Процедура ДобавитьФайлы(Знач ВладелецФайла, Знач ИдентификаторФормы, Знач Фильтр = "",
	ВидДокумента=Неопределено, //RG-Soft добавила Петроченко Н.Н. ВидДокумента для реализаций
	ДопПараметры=Неопределено) Экспорт // RGS AArsentev 07.09.2016 17:38:05 - S-I-0001855  
	...
	Параметры.Вставить("ВидДокумента", ВидДокумента);  //RG-Soft добавила Петроченко Н.Н. ВидДокумента для реализаций
	// { RGS AArsentev 07.09.2016 17:38:05 - S-I-0001855
	Если ДопПараметры <> Неопределено Тогда
		Параметры.Вставить("ВидДокумента", ДопПараметры.ВидДокумента);
		Параметры.Вставить("ДокументНаименование", ДопПараметры.Наименование);
		Параметры.Вставить("ДатаДокумента", ДопПараметры.Дата);
	Иначе
		Параметры.Вставить("ДокументНаименование", Неопределено);
		Параметры.Вставить("ДатаДокумента", Неопределено);
	КонецЕсли;
	// } RGS AArsentev 07.09.2016 17:38:05 - S-I-0001855	
...
Процедура ПодписатьФайл(ПрисоединенныйФайл, ИдентификаторФормы, ДополнительныеПараметры = Неопределено) Экспорт
	...
	Если Не ЗначениеЗаполнено(ПрисоединенныйФайл) Тогда
		ПоказатьПредупреждение(, ВернутьСтр("ru = 'Не выбран файл, который нужно подписать.'"));
		Возврат;
	КонецЕсли;
	
	Если Не ОбщегоНазначенияКлиент.ПодсистемаСуществует("СтандартныеПодсистемы.ЭлектроннаяПодпись") Тогда
		ПоказатьПредупреждение(, ВернутьСтр("ru = 'Добавление электронных подписей не поддерживается.'"));
		Возврат;
	КонецЕсли;
	...
	Если Не МодульЭлектроннаяПодписьКлиент.ИспользоватьЭлектронныеПодписи() Тогда
		ПоказатьПредупреждение(,
			ВернутьСтр("ru = 'Чтобы добавить электронную подпись, включите
