...
Функция ПолучитьДвоичныеДанныеФайла(Знач ПрисоединенныйФайл) Экспорт
	...
	Если ФайлОбъект.ТипХраненияФайла = Перечисления.ТипыХраненияФайлов.ВИнформационнойБазе Тогда
		...
		СообщениеОбОшибке = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			ВернутьСтр("ru = 'Двоичные данные файла отсутствуют в регистре ПрисоединенныеФайлы
			           |
			           |Ссылка на файл: ""%1"".'"),
			ПолучитьНавигационнуюСсылку(ПрисоединенныйФайл));
		ЗаписьЖурналаРегистрации(ВернутьСтр("ru = 'Файлы.Открытие файла'", ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка()),
		...
		ПредставлениеВладельцаФайла = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				ВернутьСтр("ru = 'Присоединен к %1 : %2'"),
	...
	Иначе
		...
		Исключение
			...
			ЗаписьЖурналаРегистрации(ВернутьСтр("ru = 'Файлы.Получение файла из тома'", ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка()),
				УровеньЖурналаРегистрации.Ошибка,
				Метаданные.Справочники[ПрисоединенныйФайл.Метаданные().Имя],
				ПрисоединенныйФайл,
				СообщениеОбОшибке);
...
Функция ПолучитьДанныеФайла(Знач ПрисоединенныйФайл,
	...
	ОбщегоНазначенияКлиентСервер.Проверить(ФайлОбъект <> Неопределено, 
		СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(ВернутьСтр("ru = 'Не найден присоединенный файл ""%1"" (%2)'"),
...
Функция ДобавитьПрисоединенныйФайл(ПараметрыФайла,
	...
	ЗаголовокОшибки = ВернутьСтр("ru = 'Ошибка при добавлении присоединенного файла.'");
	...
	Если НоваяСсылкаНаФайл = Неопределено Тогда
		ИмяСправочника = ПрисоединенныеФайлыСлужебный.ИмяСправочникаХраненияФайлов(ВладелецФайлов, "", ЗаголовокОшибки,
			ВернутьСтр("ru = 'В этом случае параметр ""НоваяСсылкаНаФайл"" должен быть указан.'"));
	...
	Иначе
		Если Не Справочники.ТипВсеСсылки().СодержитТип(ТипЗнч(НоваяСсылкаНаФайл))
			...
			ВызватьИсключение ВернутьСтр("ru = 'Ошибка при добавлении присоединенного файла.
	...
	Исключение
		...
		ШаблонСообщения = ВернутьСтр("ru = 'Ошибка при добавлении присоединенного файла ""%1"":
		                             |%2'");
		...
		ЗаписьЖурналаРегистрации(
			ВернутьСтр("ru = 'Файлы.Добавление присоединенного файла'",
...
Функция ДобавитьФайл(Знач ВладелецФайлов,
					 ...
					 // ИЗМЕНИЛ ИВАНОВ АНТОН
					 // ИСПОЛЬЗУЕТСЯ ДЛЯ АВТОМАТИЧЕСКОГО ПРИКРЕПЛЕНИЯ ФАЙЛА ПРИ ЗАГРУЗКЕ
					 Суффикс = "",
					 ...
					 ДатаДокумента = Неопределено) Экспорт
					 // } RGS AArsentev 07.09.2016 17:38:05 - S-I-0001855
	...
	ПрисоединенныйФайл.ПодписанЭП                   = Ложь;
	// ИЗМЕНИЛ ИВАНОВ АНТОН
	// ИСПОЛЬЗУЕТСЯ ДЛЯ АВТОМАТИЧЕСКОГО ПРИСОЕДИНЕНИЯ ФАЙЛА ПРИ ЗАГРУЗКЕ
	ПрисоединенныйФайл.Наименование	= ИмяБезРасширения + ?(Суффикс = "", "", "-" + Суффикс);
	
	//{начало добавления RG-Soft добавила Петроченко Н.Н.
	Если ТипЗнч(ВладелецФайлов) = Тип("ДокументСсылка.РеализацияТоваровУслуг") 
		// { RGS LFedotova 18.12.2017 21:48:08 - вопрос SLI-0007442
		ИЛИ ТипЗнч(ВладелецФайлов) = Тип("ДокументСсылка.ПередачаОС")
		ИЛИ ТипЗнч(ВладелецФайлов) = Тип("ДокументСсылка.ОтчетАгентаПоРасходам")
		// } RGS LFedotova 18.12.2017 21:48:28 - вопрос SLI-0007442
		Тогда
		ПрисоединенныйФайл.ВидДокумента             = ВидДокумента;
		// { RGS AArsentev 07.09.2016 17:38:05 - S-I-0001855
	ИначеЕсли ТипЗнч(ВладелецФайлов) = Тип("ДокументСсылка.TransportRequest") ИЛИ ТипЗнч(ВладелецФайлов) = Тип("ДокументСсылка.TripNonLawsonCompanies") Тогда
		ПрисоединенныйФайл.ВидДокумента             = ВидДокумента;
		ПрисоединенныйФайл.ДокументНаименование     = ДокументНаименование;
		ПрисоединенныйФайл.ДатаДокумента            = ДатаДокумента;
		// } RGS AArsentev 07.09.2016 17:38:05 - S-I-0001855
...
Функция НоваяСсылкаНаФайл(ВладелецФайлов, ИмяСправочника = Неопределено) Экспорт
	...
	ЗаголовокОшибки = ВернутьСтр("ru = 'Ошибка при получении новой ссылки на присоединенный файл.'");
...
Функция ПолучитьИмяФормыОбъектаПрисоединенныхФайловПоВладельцу(Знач ВладелецФайлов) Экспорт
	...
	ЗаголовокОшибки = ВернутьСтр("ru = 'Ошибка при получении имени формы присоединенного файла.'");
	ОкончаниеОшибки = ВернутьСтр("ru = 'В этом случае получение формы невозможно.'");
...
Процедура СконвертироватьФайлыВПрисоединенные(Знач ВладелецФайлов, ИмяСправочника = Неопределено) Экспорт
	...
	ЗаголовокОшибки = ВернутьСтр("ru = 'Ошибка при конвертации присоединенных файлов подсистемы Работа с файлами
	...
	Если Не Источник.ЭтоНовый() Тогда
		...
		Если ИзмененаПометкаУдаления И ЗначениеЗаполнено(Источник.Редактирует) Тогда
			...
			Если Источник.Редактирует = Пользователи.АвторизованныйПользователь() Тогда
				...
				ТекстОшибки = ВернутьСтр("ru = 'Действие недоступно, так как файл ""%1"" занят для редактирования.'");
...
Процедура ВыполнитьДействияПередУдалениемПрисоединенногоФайла(Источник, Отказ) Экспорт
	...
	// {RG-Soft - добавила Петроченко Н.Н. 26.08.2014
...
Процедура ПометитьНаУдалениеПрисоединенныеФайлы(Знач Источник, ИмяСправочника = Неопределено)
	...
	Исключение
		...
		ВызватьИсключение ВернутьСтр("ru = 'Ошибка при пометке на удаление присоединенных файлов.'")
			+ Символы.ПС
			+ ПредставлениеОшибки;
	КонецПопытки;
	...
	Для каждого ОписаниеИмениСправочника Из ИменаСправочников Цикл
		...
		Пока Выборка.Следующий() Цикл
			Если Источник.ПометкаУдаления И ЗначениеЗаполнено(Выборка.Редактирует) Тогда
				ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					ВернутьСтр("ru = '""%1"" не может быть удален,
