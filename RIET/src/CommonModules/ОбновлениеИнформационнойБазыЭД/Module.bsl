...
Процедура ПриДобавленииОбработчиковОбновления(Обработчики) Экспорт
	...
	Обработчик = Обработчики.Добавить();
	Обработчик.Версия = "1.2.4.4";
	Обработчик.Процедура = "ОбновлениеИнформационнойБазыЭД.ПеренестиНастройкиКонтекстаКриптографии";
	Обработчик.РежимВыполнения = "Монопольно";
...
Процедура ПервыйЗапуск() Экспорт
	...
	ЗаполнитьКонтекстРаботыСКриптографией();
...
Процедура ОбновитьНастройкиЭДО(ВидЭД) Экспорт
	...
	Пока Результат.Следующий() Цикл
		...
		Если СтрокаВидЭД = Неопределено Тогда
			...
			ИначеЕсли ВидЭД = Перечисления.ВидыЭД.АктЗаказчик
				...
				ВерсияФормата = ВернутьСтр("ru = 'ФНС 5.01'");
			КонецЕсли;
	...
	Исключение
		ВидОперации = ВернутьСтр("ru = 'Обновление информационной базы'");
		ПодробныйТекстОшибки = ПодробноеПредставлениеОшибки(ИнформацияОбОшибке());
		ЭлектронныеДокументыСлужебныйВызовСервера.ОбработатьИсключениеПоЭДНаСервере(ВидОперации, ПодробныйТекстОшибки);
		ОтменитьТранзакцию();
	КонецПопытки;
...
Процедура ЗаполнитьКонтекстРаботыСКриптографией() Экспорт
	...
	ЗаписьКонстанты = Константы.КонтекстАвторизации.СоздатьМенеджерЗначения();
	ЗаписьКонстанты.Значение = Перечисления.КонтекстыРаботыСЭД.НаКлиенте;
	ОбновлениеИнформационнойБазы.ЗаписатьДанные(ЗаписьКонстанты);
...
Процедура ЗаполнитьДанныеОПрофиляхНастроекЭДО() Экспорт
	...
	Пока Выборка.Следующий() Цикл
		...
		НастройкаЭДО.Комментарий = ВернутьСтр("ru = '##Настройка ЭДО помечена на удаление автоматически при обновлении.'");
	...
	Пока Выборка.Следующий() Цикл
		...
		Если НайденныеСтроки.Количество() = 0 Тогда
			...
			Строка.ВерсияФормата            = ВернутьСтр("ru = 'ФНС 5.01'");
	...
	Пока ВыборкаУчастников.Следующий() Цикл
		...
		Если Не ВыборкаУчастников.СоглашениеОбИспользованииЭД.ПометкаУдаления Тогда
			...
			НастройкаЭДО.УстановитьПометкуУдаления(Истина);
			НастройкаЭДО.Наименование = ВернутьСтр("ru = 'Не использовать'") + " - " + НастройкаЭДО.Наименование;
			НастройкаЭДО.Комментарий = ВернутьСтр("ru = '##Настройка ЭДО помечена на удаление автоматически при обновлении.'");
			ОбновлениеИнформационнойБазы.ЗаписатьОбъект(НастройкаЭДО);
		КонецЕсли;
...
// Переносит настройки контекста криптографии в боъект БСП СоздаватьЭлектронныеПодписиНаСервере.
//
	...
	Попытка
		Если Константы.ИспользоватьЭлектронныеПодписи.Получить() = Истина Тогда
			РаботатьСЭПНаСервере = НЕ (ОбщегоНазначения.ИнформационнаяБазаФайловая()
				...
				И (Константы.УдалитьКонтекстКриптографии.Получить() = Перечисления.КонтекстыРаботыСЭД.НаСервере);
