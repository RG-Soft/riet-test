...
Функция ПолучитьНаименованиеЗаданияВыполненияОтчета(Форма) Экспорт
	...
	НаименованиеЗадания = ВернутьСтр("ru = 'Выполнение отчета: %1'");
...
Процедура ИзменитьЗаголовокКнопкиПанельНастроек(Кнопка, ВидимостьПанелиНастроек) Экспорт
	...
	Если ВидимостьПанелиНастроек Тогда
		Кнопка.Заголовок = ВернутьСтр("ru = 'Скрыть настройки'");
	Иначе
		Кнопка.Заголовок = ВернутьСтр("ru = 'Показать настройки'");
	КонецЕсли;
...
// Функция возвращает доступность показателя 
// Ищет показатель в доступных полях компоновщика данных (ДоступныеПоля) по имени (ИмяПоказателя)
// возвращает Истина если показатель есть среди доступных, в противном случае Ложь
