...
Функция ВернутьМассивДвоичныхДанныхПакета(ПакетЭД) Экспорт
	...
	Попытка
		...
		Если Не ЗначениеЗаполнено(ПараметрыПакетаЭД.НастройкаЭДО)
			...
			СообщениеИсключения = ВернутьСтр("ru = 'По данному пакету электронных документов нет действующей настройки ЭДО.
		...
		Исключение
			...
			КонецЕсли;
			ОбработатьИсключениеПоЭДНаСервере(ВернутьСтр("ru = 'Распаковка пакета ЭД'"), ТекстОшибки, ТекстСообщения);
			ЗИПЧтение.Закрыть();
		...
		Для Каждого Файл Из ФайлыАрхива Цикл
			Если СтрНайти(Файл.Имя, "packageDescription") > 0 Тогда
				ФайлИнформации = Файл;
...
Функция ДобавитьДанныеПоПакетуЭД(ПакетЭД,
	...
	Если НЕ ЕстьКриптосредствоНаКлиенте = Истина И СтруктураДанныхПодписей.Количество() > 0
		...
		ТекстСообщения = ВернутьСтр("ru = 'Ошибка распаковки пакета:
		|%1
			...
			Иначе
				...
				Если ФайлыРасшифровки.Количество() > 0 Тогда
					...
					Исключение
						...
						ОбработатьИсключениеПоЭДНаСервере(ВернутьСтр("ru = 'создание произвольного ЭД'"),
	...
	Исключение
		...
		ТекстСообщения = КраткоеПредставлениеОшибки(ИнформацияОбОшибке())
			+ ВернутьСтр("ru = ' (подробности см. в Журнале регистрации).'");
		ОбработатьИсключениеПоЭДНаСервере(ВернутьСтр("ru = 'распаковка пакета ЭД'"),
...
Процедура ДобавитьПодпись(ПрисоединенныйФайл, ДанныеПодписи) Экспорт
	...
	Исключение
		...
		ШаблонСообщения = ВернутьСтр("ru = 'Ошибка подписи электронного документа %1'");
		ТекстСообщения = СтрЗаменить(ШаблонСообщения, "%1", ПрисоединенныйФайл);
		ВидОперации = ВернутьСтр("ru = 'установка подписи ЭП'");
...
Процедура ДействияПослеПодписанияЭДНаСервере(МассивЭДкОбновлениюСтатуса) Экспорт
	...
	Для Каждого ПрисоединенныйФайл Из МассивЭДкОбновлениюСтатуса Цикл
		...
		Исключение
			...
			ШаблонСообщения = ВернутьСтр("ru = 'Ошибка подписи электронного документа %1'");
			ТекстСообщения = СтрЗаменить(ШаблонСообщения, "%1", ПрисоединенныйФайл);
			Операция = ВернутьСтр("ru = 'установка подписи ЭП'");
...
Функция ОтправкаПакетовЭД(Знач МассивПакетовЭД, Знач СоотвСоглашенийИПараметровСертификатов, ТекстСообщения = "") Экспорт
	...
	Для Каждого ПакетЭД Из МассивПакетовЭД Цикл
		...
		Попытка
			...
			ИначеЕсли ТипОтправки =  Перечисления.СпособыОбменаЭД.ЧерезОператораЭДОТакском
				...
				Если КоличествоОтправленных <> КоличествоКОтправке Тогда
					ШаблонСообщения = ВернутьСтр("ru = 'Не удалось отправить оператору ЭДО ""%1"".'");
			...
			ИначеЕсли ТипОтправки = Перечисления.СпособыОбменаЭД.ЧерезВебРесурсБанка Тогда
				...
				Если Выборка.Количество() > 0 И Выборка.Следующий() Тогда
					...
					Если ЗначениеЗаполнено(ТекстОшибки) Тогда
						ШаблонВидаОперации = ВернутьСтр("ru = 'Отправка пакета ЭД по соглашению: %1, способ обмена %2'");
		...
		Исключение
			...
			ШаблонСообщения = ВернутьСтр("ru = 'Ошибка отправки пакета по настройке: %1, способ обмена: %2
									|%3%4'");
			ЭтоПолноправныйПользователь = Пользователи.ЭтоПолноправныйПользователь( , , Ложь);
			ТекстОЖурнале = ?(ЭтоПолноправныйПользователь, ВернутьСтр("ru =' (подробности см. в Журнале регистрации).'"), ".");
...
Процедура ОбработатьИсключениеПоЭДНаСервере(ВидОперации, ПодробныйТекстОшибки, ТекстСообщения = "", КодСобытия = 2) Экспорт
	...
	Если НЕ ПустаяСтрока(ТекстСообщения) Тогда
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(ТекстСообщения);
	КонецЕсли;
	ТекстОшибки = ВернутьСтр("ru = 'Выполнение операции:'")+ " " + ВидОперации + Символы.ПС + ПодробныйТекстОшибки;
...
Процедура ПометитьНаУдалениеЭлектронныеДокументыПоВладельцу(Ссылка) Экспорт
	...
	Пока Выборка.Следующий() Цикл
		Если НЕ Выборка.ПометкаУдаления = ПометкаУдаления Тогда
			...
			Исключение
				Шаблон = ВернутьСтр("ru = 'Не удалось заблокировать объект %1'");
				ТекстСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(Шаблон, ФайлОбъект);
				ПодробноеПредставлениеОшибки = ПодробноеПредставлениеОшибки(ИнформацияОбОшибке());
				ВидОперации = ВернутьСтр("ru = 'Пометка на удаление электронных документов'");
		...
		Если ПолучитьЗначениеФункциональнойОпции("ИспользоватьОбменЭД") Тогда
			...
			Если ОпределитьДействующуюНастройкуЭДО(СсылкаНаВладельца, ПараметрыЭД) Тогда
				...
				Если Не ЗначениеЗаполнено(ТекстСостоянияЭД) Тогда
					...
					Если ПараметрыЭД.НаправлениеЭД = Перечисления.НаправленияЭД.Исходящий Тогда
						ТекстСостоянияЭД = ВернутьСтр("ru = 'Не сформирован'");
					Иначе
						ТекстСостоянияЭД = ВернутьСтр("ru = 'Не получен'");
					КонецЕсли;
				КонецЕсли;
				...
				Если СостояниеСоглашения = Перечисления.СостоянияСоглашенийЭД.ПроверкаТехническойСовместимости Тогда
					ШаблонСостояниЭД = ВернутьСтр("ru = '%1 (проверка технической совместимости)'");
			...
			Иначе
				...
				Если ЗначениеЗаполнено(ТекстСостоянияЭД) Тогда
					ШаблонСостояниЭД = ВернутьСтр("ru = '%1 (настройка ЭДО не подключена)'");
					ТекстСостоянияЭД = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(ШаблонСостояниЭД, ТекстСостоянияЭД);
					Гиперссылка = Истина;
				Иначе
					...
					Если ПараметрыЭД.ВидЭД = Перечисления.ВидыЭД.ПлатежноеПоручение
						...
						ТекстСостоянияЭД = ВернутьСтр("ru = 'Нет действующей настройки ЭДО с банком'");
					ИначеЕсли ПараметрыЭД.ВидЭД = Перечисления.ВидыЭД.ПередачаТоваровМеждуОрганизациями
						...
						ТекстСостоянияЭД = ВернутьСтр("ru = 'Нет действующей настройки ЭДО с организацией-получателем'");
					Иначе
						ТекстСостоянияЭД = ВернутьСтр("ru = 'Настроить ЭДО с контрагентом'");
						Гиперссылка = Истина;
					КонецЕсли;
				КонецЕсли;
			КонецЕсли;
		Иначе
			ТекстСостоянияЭД = ВернутьСтр("ru = 'Обмен электронными документами отключен'");
		КонецЕсли;
	КонецЕсли;
...
Процедура ОпределитьСтатусыПодписей(ЭД, ВыводитьСообщения = Ложь) Экспорт
	...
	Для каждого СтрокаЭП из ОбъектЭД.ЭлектронныеПодписи Цикл
		...
		Исключение
			...
			ПодробноеПредставлениеОшибки = ПодробноеПредставлениеОшибки(ИнформацияОбОшибке());
			ВидОперации = ВернутьСтр("ru = 'проверка электронной подписи'");
			ОбработатьИсключениеПоЭДНаСервере(ВидОперации, ПодробноеПредставлениеОшибки, ТекстСообщения);
		КонецПопытки;
	КонецЦикла;
...
Функция ПолучитьДвоичныеДанныеФайла(ЭД, СертификатПодписи) Экспорт
	...
	Если ЗначениеЗаполнено(СертификатПодписи) Тогда
		...
		Если НЕ ЭтоВидЭДБанка
			...
			ТекстСообщения = ВернутьСтр("ru = 'Операция отменена. Необходимо заполнить поля: ""Фамилия"", ""Имя""
										|в сертификате %1.'");
		...
		ИначеЕсли ПараметрыЭД.НаправлениеЭД = Перечисления.НаправленияЭД.Исходящий
			...
			Исключение
				...
				Если Не ЭлектронныеДокументыСлужебный.ВозможноИзвлечьФайлы(ЗипЧтение, ПапкаДляРаспаковки) Тогда
					ТекстСообщения = ЭлектронныеДокументыПовтИсп.ПолучитьСообщениеОбОшибке("006");
				КонецЕсли;
				ОбработатьИсключениеПоЭДНаСервере(ВернутьСтр("ru = 'Извлечение ЭД CML из архива при подписи'"),
			...
			Исключение
				ТекстОшибки = КраткоеПредставлениеОшибки(ИнформацияОбОшибке());
				ОбработатьИсключениеПоЭДНаСервере(ВернутьСтр("ru = 'Формирование архива ЭД CML при подписи'"),
				ТекстОшибки, ТекстСообщения);
			КонецПопытки;
			УдалитьФайлы(ПапкаДляРаспаковки);
		КонецЕсли;
		...
		Если ТЗ_Сертификатов.Количество() = 0 И ЗначениеЗаполнено(ЭД) Тогда
			...
			Если ДоступныеСертификаты[0].Количество > 0 Тогда
				...
				Если Не ЭтоПрямойОбмен(СпособОбменаЭД) Тогда
					ШаблонСообщения = ВернутьСтр("ru = 'В профиле настроек ЭДО: %1
			...
			ИначеЕсли ДопПараметры.Свойство("ОшибкаНастройкиКриптографии", ОшибкаНастройкиКриптографии)
				...
				ТекстСообщения = ВернутьСтр("ru = 'Ошибка подписания электронного документа.
		...
		Если СтрНайти(ЭлементСоответствие.Ключ, ".p7s") > 0 Тогда
			...
			Исключение
				...
				ТекстСообщения = КраткоеПредставлениеОшибки(ИнформацияОбОшибке())
					+ ВернутьСтр("ru = ' (подробности см. в Журнале регистрации).'");
				ТекстОшибки = ПодробноеПредставлениеОшибки(ИнформацияОбОшибке());
				ОбработатьИсключениеПоЭДНаСервере(ВернутьСтр("ru = 'получение подтверждения ЭД'"), ТекстОшибки, ТекстСообщения);
			КонецПопытки;
...
Функция ТекстСообщенияОНеобходимостиНастройкиСистемы(ВидОперации) Экспорт
	...
	Если НЕ ЗначениеЗаполнено(ТекстСообщения) Тогда
		Если ВРег(ВидОперации) = "РАБОТАСЭД" Тогда
			ТекстСообщения = ВернутьСтр("ru = 'Для работы с электронными документами необходимо
			|в настройках системы включить использование обмена электронными документами.'");
		ИначеЕсли ВРег(ВидОперации) = "ПОДПИСАНИЕЭД" Тогда
			ТекстСообщения = ВернутьСтр("ru = 'Для возможности подписания ЭД необходимо
		...
		ИначеЕсли ВРег(ВидОперации) = "НАСТРОЙКАКРИПТОГРАФИИ" Тогда
			ТекстСообщения = ВернутьСтр("ru = 'Для возможности настройки криптографии необходимо 
			|в настройках системы включить опцию использования электронных цифровых подписей.'");
		ИначеЕсли ВРег(ВидОперации) = "РАБОТАСБАНКАМИ" Тогда
			ТекстСообщения = ВернутьСтр("ru = 'Для возможности обмена ЭД с банками необходимо 
		...
		ИначеЕсли ВРег(ВидОперации) = "ДОПОЛНИТЕЛЬНЫЕОТЧЕТЫИОБРАБОТКИ" Тогда
			ТекстСообщения = ВернутьСтр("ru = 'Для возможности прямого обмена с банком через дополнительную обработку
			|необходимо в настройках системы включить опцию использования дополнительных отчетов и обработок.'");
		Иначе
			ТекстСообщения = ВернутьСтр("ru='Операция не может быть выполнена. Не выполнены необходимые настройки системы.'");
		КонецЕсли;
	КонецЕсли;
...
Функция СоздатьНастройкуЭДОСБанком(НастройкиЗаполнения, Объект = Неопределено) Экспорт
	...
	ИначеЕсли Объект = Неопределено Тогда
		ТекстСообщения = ВернутьСтр("ru = 'В полученном файле настроек нет информации об организации.
									|Обратитесь в свой банк'");
		Возврат Неопределено;
	КонецЕсли;
	
	Если Не ЗначениеЗаполнено(СсылкаНаОрганизацию) Тогда
		ТекстСообщения = ВернутьСтр("ru = 'Не найдена организация по реквизитам: %1%2%3'");
		Если СтруктураПоискаОрганизации.Свойство("ИНН") Тогда
			ТекстИНН = ВернутьСтр("ru = '
							|ИНН: %1'");
		...
		Если СтруктураПоискаОрганизации.Свойство("Наименование") Тогда
			ТекстНаименование = ВернутьСтр("ru = '
									|Наименование: %1'");
	...
	СсылкаНаБанк = ЭлектронныеДокументыПереопределяемый.НайтиСсылкуНаОбъект("Банки", БИК, СтруктураПоискаБанка);
		
	Если НЕ ЗначениеЗаполнено(СсылкаНаБанк) Тогда
		ТекстСообщения = ВернутьСтр("ru = 'Не найден банк по реквизитам: %1%2'");
		Если СтруктураПоискаБанка.Свойство("Код") Тогда
			ТекстБИК = ВернутьСтр("ru = '
							|БИК: %1'");
	...
	Если ЭтоНастройкиСбербанка Тогда
		...
		Для Каждого Сертификат Из ЭД.SignatureParameters.Certificate Цикл
			...
			Исключение
				ТекстСообщения = ВернутьСтр("ru = 'Возникла ошибка при чтении данных сертификата из файла.'");
				ПодробноеПредставлениеОшибки = ПодробноеПредставлениеОшибки(ИнформацияОбОшибке());
				ВидОперации = ВернутьСтр("ru = 'Чтение настроек ЭДО из файла.'");
				ОбработатьИсключениеПоЭДНаСервере(ВидОперации, ПодробноеПредставлениеОшибки, ТекстСообщения, 1);
				Если Объект = Неопределено Тогда //автоматическое получение настроек
	...
	Иначе
		...
		Если Объект.ИсходящиеДокументы.Найти(Перечисления.ВидыЭД.ЗапросЗонд, "ИсходящийДокумент") = Неопределено Тогда
			НовЗапись = Объект.ИсходящиеДокументы.Добавить();
			НовЗапись.ИсходящийДокумент = Перечисления.ВидыЭД.ЗапросЗонд;
...
Функция ПолучитьМаркерБанка(АдресСервера, ИдентификаторОрганизации, ДвоичныеДанныеСертификата) Экспорт
	...
	Исключение
		ТекстСообщения = ВернутьСтр("ru = 'Ошибка аутентификации на сервере банка!
									|(подробности см. в Журнале регистрации).'");
		ВидОперации = ВернутьСтр("ru = 'аутентификация на сервере банка'");
		ПодробноеПредставлениеОшибки = ПодробноеПредставлениеОшибки(ИнформацияОбОшибке());
		ОбработатьИсключениеПоЭДНаСервере(ВидОперации, ПодробноеПредставлениеОшибки, ТекстСообщения, 1);
	...
	Если Ответ.КодСостояния <> 200 Тогда
		Шаблон = ВернутьСтр("ru = 'Ошибка работы с Интернет (%1)'");
	...
	Попытка
		...
		Если НЕ ResultBank.Error = Неопределено Тогда
			ШаблонОшибки = ВернутьСтр("ru = 'Произошла ошибка при аутентификации на сервере банка.
										|Код ошибки: %1. %2: %3'");
		...
		КонецЕсли;
	Исключение
		ТекстСообщения = ВернутьСтр("ru = 'Произошла ошибка при получении маркера банка'");
		ПодробноеПредставлениеОшибки = ПодробноеПредставлениеОшибки(ИнформацияОбОшибке());
		ВидОперации = ВернутьСтр("ru = 'Получение маркера банка'");
		ОбработатьИсключениеПоЭДНаСервере(ВидОперации, ПодробноеПредставлениеОшибки, ТекстСообщения, 1);
		Чтение.Закрыть();
		Возврат Неопределено;
	КонецПопытки;
...
Функция НастройкиОбменаСБанком(Банк) Экспорт
	...
	ИспользуетсяОбменСБанком = ПолучитьЗначениеФункциональнойОпции("ИспользоватьОбменЭДСБанками");
	Если НЕ ИспользуетсяОбменСБанком Тогда
		Константы.ИспользоватьОбменЭДСБанками.Установить(Истина);
	...
	Для Индекс = 1 По КоличествоЗаписей Цикл
		...
		Если БИК = "044525985" Тогда // ПАО БАНК "ФК ОТКРЫТИЕ"
			...
			ПоставляемаяНастройка.Вставить("Представление", ВернутьСтр("ru = 'Сигнал-КОМ CSP (ITU-T X.509 v.3)'"));
...
Функция ПолучитьНастройкиОбменаССервераБанка(Знач АдресСервера, Знач Банк, Знач НомерСчета, Знач ДанныеМаркера) Экспорт
	...
	Попытка
		...
		КонецЕсли;
	Исключение
		ТекстСообщения = ВернутьСтр("ru = 'Произошла ошибка при получении настроек из банка'");
		ПодробноеПредставлениеОшибки = ПодробноеПредставлениеОшибки(ИнформацияОбОшибке());
		ВидОперации = ВернутьСтр("ru = 'Получение настроек обмена с банком'");
		ОбработатьИсключениеПоЭДНаСервере(ВидОперации, ПодробноеПредставлениеОшибки, ТекстСообщения, 1);
		Чтение.Закрыть();
		УдалитьФайлы(ИмяФайлаРезультата);
		Возврат Неопределено
	...
	Пока ВыборкаПолученныхЭД.Следующий() Цикл
		...
		Исключение
			Если Пользователи.ЭтоПолноправныйПользователь( , , Ложь) Тогда
				ШаблонСообщения = ВернутьСтр("ru = 'Возникла ошибка при разборе пакета %1
											|(подробности см. в Журнале регистрации).'");
				КраткоеПредставлениеОшибки = КраткоеПредставлениеОшибки(ИнформацияОбОшибке());
				ТекстСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
									ШаблонСообщения, ПакетЭД, КраткоеПредставлениеОшибки);
			Иначе
				ТекстСообщения = ВернутьСтр("ru = 'Файл, полученный из банка невозможно прочитать так как он содержит ошибку'");
			КонецЕсли;
			ПодробнаяИнформация = ВернутьСтр("ru = 'Ошибка при распаковке пакета %1
			...
			ВидОперации = ВернутьСтр("ru = 'Чтение ЭД.'");
			...
			Для Каждого Document Из ResultBank.Success.GetPacketResponse.Document Цикл
				Если Document.compressed = Истина Тогда
					...
					Исключение
						...
						ОбработатьИсключениеПоЭДНаСервере(ВернутьСтр("ru = 'Распаковка пакета ЭД'"), ПодробнаяИнформация, ТекстСообщения, 1);
						ДанныеВозврата.Вставить("ЕстьОшибка", Истина);
						ДанныеВозврата.Вставить("ТекстСообщения", ТекстСообщения);
						УдалитьФайлы(ПапкаДляРаспаковки);
						Возврат;
				...
				Если Document.dockind = "02" Тогда // Извещение о состоянии электронного документа
					...
					Если ВладелецСуществует Тогда
						...
						НазваниеДокумента = ВернутьСтр("ru = 'Извещение о состоянии электронного документа'");
								...
								Исключение
									Если Пользователи.ЭтоПолноправныйПользователь( , , Ложь) Тогда
										...
										ТекстСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
											ШаблонСообщения, ПакетЭД, КраткоеПредставлениеОшибки);
									Иначе
										ТекстСообщения = ВернутьСтр("ru = 'Электронный документ, полученный из банка содержит некорретную подпись
						...
						КонецЕсли;
					Иначе
						ВидОперации = ВернутьСтр("ru = 'Чтение извещения о состоянии электронного документа'");
						ПодробнаяИнформация = ВернутьСтр("ru = 'Ошибка при распаковке пакета %1
				...
				ИначеЕсли Document.dockind = "15" Тогда // Выписка банка
					...
					Если ЗначениеЗаполнено(ЭДВладелец) Тогда
						...
						НазваниеДокумента = СтрЗаменить(НазваниеЗапроса, ВернутьСтр("ru = 'Запрос выписки'"), ВернутьСтр("ru = 'Выписка банка за период'"));
					Иначе
						НазваниеДокумента = ВернутьСтр("ru = 'Выписка банка от %1'");
					...
					СтруктураРеквизитов.Вставить("СоглашениеЭД", СоглашениеЭД);
					НаименованиеФайла = СтроковыеФункцииКлиентСервер.СтрокаЛатиницей(НазваниеДокумента);
					СтруктураРеквизитов.Вставить("НаименованиеФайла", НаименованиеФайла);
					ЭлектронныеДокументыСлужебный.ИзменитьПоСсылкеПрисоединенныйФайл(ПрисоединенныйФайл, СтруктураРеквизитов, Ложь);
...
Функция МаркерБанка(Знач АдресСервера, Знач ИдентификаторОрганизации, Знач Пользователь, Знач Пароль, ДанныеSMSАутентификации) Экспорт
	...
	Попытка
		Ответ = Соединение.ОтправитьДляОбработки(HTTPЗапрос, ИмяФайлаРезультата);
	Исключение
		ТекстСообщения = ВернутьСтр("ru = 'Ошибка отправки данных аутентификации на сервер банка
									|(подробности см. в Журнале регистрации).'");
		ОписаниеОперации = ВернутьСтр("ru = 'Аутентификация на сервере банка'");
		ПодробноеПредставлениеОшибки = ПодробноеПредставлениеОшибки(ИнформацияОбОшибке());
	...
	Попытка
		...
		Если НЕ ResultBank.Success = Неопределено Тогда
			...
			Иначе
				ТекстСообщения = ВернутьСтр("ru = 'Получен некорректный ответ банковского сервиса'");
				ТекстОшибки = ВернутьСтр("ru = 'Ответ банка содержит некорректные данные.
									|Файл ответа банка: %1'");
		...
		КонецЕсли;
	Исключение
		ТекстСообщения = ВернутьСтр("ru = 'Ошибка чтения ответа банка
									|(подробности см. в Журнале регистрации).'");
		ОписаниеОперации = ВернутьСтр("ru = 'Аутентификация на сервере банка'");
		ПодробноеПредставлениеОшибки = ПодробноеПредставлениеОшибки(ИнформацияОбОшибке());
		ОбработатьИсключениеПоЭДНаСервере(ОписаниеОперации, ПодробноеПредставлениеОшибки, ТекстСообщения, 1);
...
Процедура ПолучитьЭДИзБанкаАсинхронныйОбмен(Знач ПараметрыАвторизации, ДанныеВозврата) Экспорт
	...
	Если Параметры = Неопределено Тогда
		ДанныеВозврата.Вставить("ЕстьОшибка", Истина);
		ДанныеВозврата.Вставить("ТекстСообщения", ВернутьСтр("ru = 'Неизвестная ошибка'"));
		Возврат;
	КонецЕсли;
	...
	Иначе
		Шаблон = ВернутьСтр("ru = 'Ошибка аутентификации на сервисе по адресу (%1)'");
	...
	Исключение
		КраткоеПредставлениеОшибки = КраткоеПредставлениеОшибки(ИнформацияОбОшибке());
		ТекстСообщения = ВернутьСтр("ru = 'При получении списка новых документов из банка произошла ошибка.
									|%1'");
		ТекстСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(ТекстСообщения, КраткоеПредставлениеОшибки);
		ПодробноеПредставлениеОшибки = ПодробноеПредставлениеОшибки(ИнформацияОбОшибке());
		ВидОперации = ВернутьСтр("ru = 'Получение списка новых документов из банка'");
		ОбработатьИсключениеПоЭДНаСервере(ВидОперации, ПодробноеПредставлениеОшибки, ТекстСообщения, 1);
		ДанныеВозврата.Вставить("ЕстьОшибка", Истина);
		ДанныеВозврата.Вставить("ТекстСообщения", ТекстСообщения);
		Возврат;
	...
	Исключение
		КраткоеПредставлениеОшибки = КраткоеПредставлениеОшибки(ИнформацияОбОшибке());
		ТекстСообщения = ВернутьСтр("ru = 'При чтении документа из банка произошла ошибка.
									|%1'");
		ТекстСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(ТекстСообщения, КраткоеПредставлениеОшибки);
		ПодробноеПредставлениеОшибки = ПодробноеПредставлениеОшибки(ИнформацияОбОшибке());
		ВидОперации = ВернутьСтр("ru = 'Чтение списка новых документов'");
		ОбработатьИсключениеПоЭДНаСервере(ВидОперации, ПодробноеПредставлениеОшибки, ТекстСообщения, 1);
		ДанныеВозврата.Вставить("ЕстьОшибка", Истина);
		ДанныеВозврата.Вставить("ТекстСообщения", ТекстСообщения);
		ОбъектXML.Закрыть();
	...
	Для Каждого Идентификатор Из МассивИдентификаторов Цикл
		...
		Исключение
			КраткоеПредставлениеОшибки = КраткоеПредставлениеОшибки(ИнформацияОбОшибке());
			ТекстСообщения = ВернутьСтр("ru = 'При получении документа из банка произошла ошибка.
										|%1'");
			ТекстСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(ТекстСообщения, КраткоеПредставлениеОшибки);
			ПодробноеПредставлениеОшибки = ПодробноеПредставлениеОшибки(ИнформацияОбОшибке());
			ВидОперации = ВернутьСтр("ru = 'Получение нового пакета из банка по идентификатору'");
			ОбработатьИсключениеПоЭДНаСервере(ВидОперации, ПодробноеПредставлениеОшибки, ТекстСообщения, 1);
			ДанныеВозврата.Вставить("ЕстьОшибка", Истина);
			ДанныеВозврата.Вставить("ТекстСообщения", ТекстСообщения);
			УдалитьФайлы(ИмяФайлаРезультата);
	...
	Если ЗначениеЗаполнено(ПоследняяДатаПолученияЭД) И ПоследняяДатаПолученияЭД > НачальнаяДатаПолученияЭД Тогда // сдвигаем дату в РС
		...
		Исключение
			КраткоеПредставлениеОшибки = КраткоеПредставлениеОшибки(ИнформацияОбОшибке());
			ТекстСообщения = ВернутьСтр("ru = 'При записи даты получения документов произошла ошибка:.
										|%1'");
			ТекстСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(ТекстСообщения, КраткоеПредставлениеОшибки);
			ПодробноеПредставлениеОшибки = ПодробноеПредставлениеОшибки(ИнформацияОбОшибке());
			ВидОперации = ВернутьСтр("ru = 'Сохранение последней даты получения документов'");
			ОбработатьИсключениеПоЭДНаСервере(ВидОперации, ПодробноеПредставлениеОшибки, ТекстСообщения, 1);
			ДанныеВозврата.Вставить("ЕстьОшибка", Истина);
			ДанныеВозврата.Вставить("ТекстСообщения", ТекстСообщения);
			Возврат;
...
Функция ОтправитьЗапросСтатусаЭД(Знач ПараметрыАвторизации, ЭД) Экспорт
	...
	НазваниеЭД = ВернутьСтр("ru = 'Запрос состояния ЭД'");
...
Функция СформироватьЗапросЗонд(Знач НастройкаЭДО, Знач МассивОтпечатковСертификатов, НастройкиОбмена) Экспорт
	...
	НазваниеЭД = ВернутьСтр("ru = 'Запрос-зонд'");
	...
	СтруктураПараметров.Вставить("ДатаДокументаОтправителя", ТекущаяДатаСеанса);
	СтруктураПараметров.Вставить("НаименованиеФайла", СтроковыеФункцииКлиентСервер.СтрокаЛатиницей(НазваниеЭД));
...
Функция ПодключитьВнешнююОбработку(Знач СоглашениеЭД, Знач ТекущаяВерсия = Неопределено, НоваяВерсия = Неопределено, ИмяОбъекта = "", АдресФайла = "") Экспорт
	...
	Если Не ОбработкаПодключена Тогда
		ТекстСообщения = ВернутьСтр("ru = 'Не выбрана дополнительная обработка в соглашении об использовании прямого обмена с банком: %1.'");
...
Процедура УдалитьИдентификаторЗапроса(СоглашениеЭД, Идентификатор, ВидЭД) Экспорт
	...
	Исключение
		...
		ТекстСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
										ШаблонСообщения, КраткоеПредставлениеОшибки);
		Операция = ВернутьСтр("ru = 'Удаление идентификатора запроса банка'");
		ПодробноеПредставлениеОшибки = ПодробноеПредставлениеОшибки(ИнформацияОбОшибке());
		ОбработатьИсключениеПоЭДНаСервере(Операция, ПодробноеПредставлениеОшибки, ТекстСообщения, 1);
	КонецПопытки;
...
//
Процедура ЗаписатьСобытиеВЖурналАудита(СоглашениеЭД, ОписаниеСобытия, ТекстСообщения, ИдентификаторСобытия = Неопределено) Экспорт
...
Функция ТекстЗапросаНочнойВыписки(НастройкаЭДО, ИдентификаторЗапроса, ИдентификаторОрганизации, ЭП, СертификатПодписи) Экспорт
	...
	Попытка
		...
		Если ДанныеСертификата = Неопределено Тогда
			ВызватьИсключение ВернутьСтр("ru='Не найден сертификат установленной подписи'");
		КонецЕсли;
		...
		Если ЗначениеЗаполнено(ТекстОшибки) Тогда
			ВызватьИсключение ВернутьСтр("ru = 'Ошибка формирования запроса ночной выписки'");
		КонецЕсли;
...
//
Процедура ОбработатьОтветСбербанка(Ответ, НастройкаЭДО, ВидЭД, МассивНовыхЭД, Идентификатор = Неопределено, ИдентификаторСобытия) Экспорт
	...
	Исключение
		...
		ЧтениеXML.Закрыть();
		ТекстСообщения = ВернутьСтр("ru = 'Возникла ошибка при чтении полученных данных из банка
									|Идентификатор события: %1'");
		ТекстСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(ТекстСообщения, ИдентификаторСобытия);
		ТекстОшибки = ПодробноеПредставлениеОшибки(ИнформацияОбОшибке());
		ОбработатьИсключениеПоЭДНаСервере(ВернутьСтр("ru = 'Чтение полученных данных из банка'"), ТекстОшибки, ТекстСообщения, 1);
		Возврат
	КонецПопытки;
...
Функция ПакетXMLСбербанка(Знач ЭДСсылка, Знач НастройкаЭДО, ИдентификаторЗапроса, ИдентификаторОрганизации) Экспорт
	...
	Попытка
		...
		ИначеЕсли ЭД.Тип() = ЭлектронныеДокументыВнутренний.ПолучитьТипЗначенияCML("StmtReqType", ПространствоИмен) Тогда
			...
			Request.Проверить();
		Иначе
			ВызватьИсключение ВернутьСтр("ru='Неизвестный формат файла'");
		КонецЕсли;
		ОбъектXML.Закрыть();
		УдалитьФайлы(ВремФайл);
	...
	Исключение
		ОбъектXML.Закрыть();
		ТекстОшибки = ВернутьСтр("ru = 'Возникла ошибка при формировании пакета данных банка'");
	...
	Пока Выборка.Следующий() Цикл
		...
		Исключение
			...
			ОбработатьИсключениеПоЭДНаСервере(
							ВернутьСтр("ru = 'Формирование запроса статуса электроного документа'"),
							ПодробноеПредставлениеОшибки(ИнформацияОбОшибке()),
							ТекстСообщения,
							1);
...
Функция ТекстыЗапросаСостоянийЗапросовВыпискиБанка(НастройкаЭДО) Экспорт
	...
	Для Каждого Идентификатор Из МассивИдентификаторов Цикл
		...
		Попытка
			Request.Проверить();
		Исключение
			Операция = ВернутьСтр("ru = 'Формирование запроса состояния электронного документа'");
			ПодробноеПредставлениеОшибки = ПодробноеПредставлениеОшибки(ИнформацияОбОшибке());
			ТекстСообщения = ВернутьСтр("ru = 'При формировании запроса состояния произошла ошибка'");
			ОбработатьИсключениеПоЭДНаСервере(Операция, ПодробноеПредставлениеОшибки, ТекстСообщения, 1);
			Продолжить;
		КонецПопытки;
...
Функция ЗагрузитьСертификат(СтруктураСертификата, ОписаниеОшибки) Экспорт
	...
	ШаблонНазначения = ВернутьСтр("ru = 'Кем выдан: %1
							|Действителен до: %2
							|
							|%3'");
	НовоеНазначение = "";
...
Функция СтруктураСертификата(Знач ДвоичныеДанныеСертификата) Экспорт
	...
	Исключение
		ВидОперации = ВернутьСтр("ru = 'Загрузка сертификата с банковского ключа'");
		ПодробныйТекстОшибки = ПодробноеПредставлениеОшибки(ИнформацияОбОшибке());
		ТестСообщения = ВернутьСтр("ru = 'Не удалось прочитать файл сертификата, операция прервана.'");
		ОбработатьИсключениеПоЭДНаСервере(ВидОперации, ПодробныйТекстОшибки, ТестСообщения, 1);
		Возврат Неопределено;
	КонецПопытки;
...
Функция СоздатьНайтиСертификатЭП(ДвоичныеДанныеСертификата, Организация, НазваниеПрограммыКриптографии = Неопределено) Экспорт
	...
	Исключение
		ТекстСообщения = ВернутьСтр("ru = 'Ошибка чтения данных сертификата.
...
Функция СостояниеВерсииЭД(СсылкаНаВладельца)
	...
	Если СтруктураВерсииЭД.Свойство("СостояниеВерсииЭД") Тогда
		...
		Если ДобавитьПричинуЗакрытия Тогда
			...
			Причина = СтрЗаменить(ВернутьСтр("ru = '(причина: %1)'"), "%1", ?(ЗначениеЗаполнено(Комментарий), Комментарий, "не указана"));
...
Функция ПолучитьТекстСводногоСостоянияЭД(СсылкаНаВладельца)
	...
	Если ТаблицаДанных.Количество() > 0 Тогда
		...
		Если Строка.СостояниеВерсииЭД = Перечисления.СостоянияВерсийЭД.ЗакрытПринудительно Тогда
			ТекстСводногоСостояния = ВернутьСтр("ru = 'Закрыт принудительно.'");
		Иначе
			Если ЗначениеЗаполнено(Строка.ДействияСНашейСтороны)
				...
				ТекстСНашейСтороны = ВернутьСтр("ru = 'с нашей стороны'");
			КонецЕсли;
			Если ЗначениеЗаполнено(Строка.ДействияСоСтороныДругогоУчастника)
				...
				ТекстСоСтороныДругогоУчастника = ВернутьСтр("ru = 'со стороны других участников'");
			КонецЕсли;
			Если ЗначениеЗаполнено(ТекстСНашейСтороны) ИЛИ ЗначениеЗаполнено(ТекстСоСтороныДругогоУчастника) Тогда
				
				ТекстСводногоСостояния = ВернутьСтр("ru = 'Требуются действия'")+ " " + ТекстСНашейСтороны
			...
			ИначеЕсли ЗначениеЗаполнено(Строка.ДействияСНашейСтороны)
				...
				Если Строка.СостояниеВерсииЭД = Перечисления.СостоянияВерсийЭД.ОбменЗавершен Тогда
					ТекстСводногоСостояния = ВернутьСтр("ru = 'Обмен завершен'");
				ИначеЕсли Строка.СостояниеВерсииЭД = Перечисления.СостоянияВерсийЭД.ОбменЗавершенСИсправлением Тогда
					ТекстСводногоСостояния = ВернутьСтр("ru = 'Обмен завершен (с исправлением)'");
				КонецЕсли;
			КонецЕсли;
		КонецЕсли;
	КонецЕсли;
...
Функция ПередатьПисьмоСВложениями(ПакетЭД, СоответствиеВложений)
	...
	Исключение
		ТекстОшибки = ПодробноеПредставлениеОшибки(ИнформацияОбОшибке());
		Текст = ВернутьСтр("ru = 'Ошибка при отправке сообщения на сервер электронной почты по профилю настроек ЭДО %1.
						|%2'");
		ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
																	Текст,
																	РеквизитыПакетаЭД.ПрофильНастроекЭДО,
		...
		ШаблонСообщения = ВернутьСтр("ru = 'Ошибка при отправке эл.документов по профилю настроек ЭДО: %1, способ обмена: %2.
								|(подробности см. в Журнале регистрации).'"); 
		...
		ВидОперации = ВернутьСтр("ru = 'Отправка эл.документов'");
		ОбработатьИсключениеПоЭДНаСервере(
							ВидОперации,
							ТекстОшибки,
							ТекстСообщения);
...
Функция СформироватьТемуПисьма(Отправитель, Получатель)
	...
	Возврат ВернутьСтр("ru = 'Обмен эл.документами:'")+ " " + Отправитель + ?(ЗначениеЗаполнено(Получатель), " -> " + Получатель, "");
	...
	Если ЭлектронныеДокументыПереопределяемый.ЭлектронныйДокументГотовКУтверждению(ПрисоединенныйФайл) Тогда
		...
		Исключение
			...
			ТекстСообщения = КраткоеПредставлениеОшибки(ИнформацияОбОшибке())
				+ ВернутьСтр("ru = ' (подробности см. в Журнале регистрации).'");
			ТекстОшибки = ПодробноеПредставлениеОшибки(ИнформацияОбОшибке());
			ОбработатьИсключениеПоЭДНаСервере(ВернутьСтр("ru = 'утверждение ЭД'"), ТекстОшибки, ТекстСообщения);
		КонецПопытки;
	КонецЕсли;
...
Процедура УдалитьНедоступныеДляФормированияЭДОбъекты(МассивСсылок)
	...
	Пока Результат.Следующий() Цикл
		...
		ШаблонСообщения = ВернутьСтр("ru='Для документа %1 уже есть актуальный электронный документ.'");
...
Функция ПолучитьФайлДанныхДляОбработки(СтруктураДанных, ПапкаДляРасшифровки, ЭтоПроизвольныйЭД)
	...
	Если ЗначениеЗаполнено(ПапкаДляРасшифровки) Тогда
		...
		Если НайтиФайлы(ПапкаДляРасшифровки).Количество() > 0 Тогда
			...
			Если СтрНайти(Расширение, "zip") > 0 И ЭтоПроизвольныйЭД <> Истина Тогда
...
Процедура ОпределитьНеобрабатываемыеОбъекты(НастройкиОбъектов, МассивНеобрабатываемыхОбъектов)
	...
	Пока Результат.Следующий() Цикл
		...
		Если Результат.СпособОбменаЭД = Перечисления.СпособыОбменаЭД.ЧерезВебРесурсБанка
			...
			ТекстСообщения = ВернутьСтр("ru = 'Обработка %1.
...
Процедура СохранитьРезультатыПроверкиПодписей(ЭД, МассивРезультатов) Экспорт
	...
	Исключение
		ТекстОшибки = ВернутьСтр("ru = 'Не удалось заблокировать объект %1
						|%2'");
		
		ВидОперации = ВернутьСтр("ru = 'Сохранение результатов проверки подписей'");
		ПодробноеПредставлениеОшибки = ПодробноеПредставлениеОшибки(ИнформацияОбОшибке());
		ОбработатьИсключениеПоЭДНаСервере(ВидОперации, ПодробноеПредставлениеОшибки);
		Возврат;
	КонецПопытки;
...
Функция ПодписатьЭДОпределеннымСертификатом(ДобавленныеФайлы,
	...
	Если КриптоСертификат <> Неопределено Тогда
		...
		Исключение
			...
			ОбработатьИсключениеПоЭДНаСервере(ВернутьСтр("ru = 'проверка сертификата на корректность'"),
											  ПодробноеПредставлениеОшибки(ИнформацияОбОшибке()),
											  ТекстСообщения);
			Для Каждого НеПодписанныйЭД Из ФайлыНаПодпись Цикл
				МассивНеПодписанныхЭД.Добавить(НеПодписанныйЭД);
		...
		Для Каждого ТекДокумент Из ФайлыНаПодпись Цикл
			...
			СтруктураДанныхПодписи = Новый Структура("Сертификат, ПарольПользователя, Комментарий", КриптоСертификат, ПараметрыСертификата.ПарольПользователя,
				ВернутьСтр("ru = 'Подписание электронного документа'"));
			Попытка
			...
			Исключение
				ШаблонСообщения = ВернутьСтр("ru = 'Ошибка подписи на сервере. Необходимо провести тест сертификата криптографии для: %1.
				|%2'");
				ТекстСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(ШаблонСообщения, СертификатПодписи,
					КраткоеПредставлениеОшибки(ИнформацияОбОшибке()));
				ОбработатьИсключениеПоЭДНаСервере(
						ВернутьСтр("ru = 'установка подписи ЭП'"),
						ПодробноеПредставлениеОшибки(ИнформацияОбОшибке()),
						ТекстСообщения);
...
Функция РасшифрованныеДанные(ДвоичныеДанные, Пароль) Экспорт
	...
	Исключение
		...
		ОбработатьИсключениеПоЭДНаСервере(
				ВернутьСтр("ru = 'расшифровка пакета ЭД'"),
				ПодробноеПредставлениеОшибки(ИнформацияОбОшибке()),
				ТекстСообщения);
		Возврат Неопределено;
	КонецПопытки;
...
Функция ЗаполнитьСтруктуруСертификатаПоОтпечатку(Отпечаток) Экспорт
	...
	Если Сертификат = Неопределено Тогда
		Предупреждение = ВернутьСтр("ru = 'Сертификат не найден'");
...
Функция ПроверитьДоступностьКаталогаДляПрямогоОбмена(ПутьККаталогу) Экспорт
	...
	Если ЗначениеЗаполнено(ПутьККаталогу) Тогда
		...
		Если НЕ КаталогиДоступны Тогда
			ТекстСообщения = ВернутьСтр("ru = 'Указанный каталог %1 не может использоваться для обмена, так как он не доступен с сервера.
...
Функция ОпределитьОбъектПривязки(СтруктураПараметровДокумента)
	...
	Если СтруктураПараметровДокумента.ВидЭД = Перечисления.ВидыЭД.ТОРГ12Покупатель
		...
		Иначе
			ТекстОшибки = ВернутьСтр("ru = 'Не найден объект привязки входящего ЭД(%1) по идентификатору: %2'");
			ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
										ТекстОшибки,
										СтруктураПараметровДокумента.ВидЭД,
										СтруктураПараметровДокумента.ИдЭДВладельца);
			ВидОперации = ВернутьСтр("ru = 'Распаковка пакета'");
...
Процедура СохранитьВыпискуСбербанка(ЭД, НастройкаЭДО, Идентификатор, МассивНовыхЭД, ВладелецЭД = Неопределено)
	...
	Для Каждого ВыпискаXDTO из ЭД.Statements.Statement Цикл
		...
		ПредставлениеДокумента = ВернутьСтр("ru = 'Выписка банка за период с'") + " " + Формат(ДатаНачала, "ДЛФ=D")
			+ " " + ВернутьСтр("ru = 'по'") + " " + Формат(ДатаОкончания, "ДЛФ=D");
	...
	Для Каждого Тикет ИЗ ЭД.Tickets.Ticket Цикл
		...
		Если ВыборкаЭД.Следующий() Тогда
			...
			Если НЕ ЗначениеЗаполнено(ВыборкаЭД.ЭДОтветБанка) Тогда
				НазваниеЭД = ВернутьСтр("ru = 'Квитанция от'")+ " " + Формат(ТекДата, "ДЛФ=DDT");
				...
				СтруктураОтветаБанка.Вставить("НаименованиеФайла",           ВернутьСтр("ru = 'Квитанция от'")+ " " + Формат(ТекДата, "ДЛФ=DDT"));
			...
			Иначе
				...
				СтруктураОтветаБанка = Новый Структура;
				СтруктураОтветаБанка.Вставить("Наименование", ВернутьСтр("ru = 'Квитанция от'") + " " + Формат(ТекДата, "ДЛФ=DDT"));
...
Процедура ЗаполнитьДанныеПодписантаЭД(ИмяФайла, Организация, СертификатПодписи, ВидЭД)
	...
	Если ЭтоФизЛицо Тогда
		...
		Если Не ДанныеЮрФизЛица.Свойство("ИНН") ИЛИ СтрДлина(ДанныеЮрФизЛица.ИНН) <> 12 Тогда
			ШаблонСообщения = ВернутьСтр("ru = 'Операция отменена. Не корректно заполнено поле ""ИИН"" в организации %1.'");
...
Процедура УстановитьОтветственногоЭД(Знач СписокОбъектов, Знач НовыйОтветственный, КоличествоОбработанныхЭД, ПричинаПеренаправления = "") Экспорт
	...
	Пока Выборка.Следующий() Цикл
		...
		Исключение
			ТекстОшибки = ВернутьСтр("ru='Не удалось заблокировать электронный документ (%Объект%). %ОписаниеОшибки%'");
		...
		Исключение
			ТекстОшибки = ВернутьСтр("ru='Не удалось выполнить запись электронного документа'") + " (%Объект%). %ОписаниеОшибки%'";
			ТекстОшибки = СтрЗаменить(ТекстОшибки, "%Объект%",         Выборка.Ссылка);
			ТекстОшибки = СтрЗаменить(ТекстОшибки, "%ОписаниеОшибки%", КраткоеПредставлениеОшибки(ИнформацияОбОшибке()));
			ОбщийТекстОшибки = ОбщийТекстОшибки + Символы.ПС + ТекстОшибки;
			ОтменитьТранзакцию();
	...
	Для Каждого СсылкаНаЭД ИЗ МассивЭД Цикл
		...
		Если ЭтоИзвещение Тогда
			...
			Если НЕ (ПараметрыЭД.ТипЭлементаВерсииЭД = Перечисления.ТипыЭлементовВерсииЭД.ПДОЭСФ
				...
				ШаблонСообщения = ВернутьСтр("ru = 'Для типа ЭД %1 не предусмотрено формирование Извещения о получении.'");
				ТекстСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(ШаблонСообщения,
...
Функция ОпределитьПараметрыРаспаковки(Знач МассивСсылокНаДокументы) Экспорт
	...
	Пока ВыборкаПакеты.Следующий() Цикл
		...
		Если ВыборкаПакеты.СтатусПакета <> Перечисления.СтатусыПакетовЭД.КРаспаковке Тогда
			...
			ШаблонСообщения = ВернутьСтр("ru = 'Статус пакета %1 отличен от значения ""К распаковке"".");
		...
		Если ФайлКарточки <> Неопределено Тогда
			...
			Исключение
				...
				ТекстСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(ШаблонСообщения,
					...
					ОбработатьИсключениеПоЭДНаСервере(ВернутьСтр("ru = 'Чтение ЭД.'"), ПодробноеПредставлениеОшибки(ИнформацияОбОшибке()),
						ТекстСообщения);
			КонецПопытки;
		КонецЕсли;
...
Функция ЗапросВыпискиCMLXDTO(ДатаНачала, ДатаКонца, МассивБанковскихСчетов, Банк)
	...
	Исключение
		...
		ТекстСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
									ШаблонСообщения, КраткоеПредставлениеОшибки);
		ПодробноеПредставлениеОшибки = ПодробноеПредставлениеОшибки(ИнформацияОбОшибке());
		ВидОперациии = ВернутьСтр("ru = 'Формирование ЭД'");
...
Функция МассивЗапросовВыписки(СоглашениеЭД, Организация, Банк, ДатаНачала, ДатаОкончания, МассивБанковскихСчетов)
	...
	ШаблонСообщения = ВернутьСтр("ru='Запрос выписки с %1 по %2'");
...
Функция ДанныеВходящегоЭДБанка(НастройкаЭДО, ИмяФайла)
	...
	Попытка
		...
		ResultBank.Проверить();
	Исключение
		ШаблонСообщения = ВернутьСтр("ru = 'Получены некорректные данные из банка. Для решения проблемы обратитесь в свой банк.%1'");
		
		Если Пользователи.ЭтоПолноправныйПользователь( , , Ложь) Тогда
			ИнформацияОЖурнале = ВернутьСтр("ru = '
											|(подробности см. в Журнале регистрации).'");
		...
		ШаблонОшибкиДляЖурнала = ВернутьСтр("ru = 'Из банка получен файл с некорректным содержимым.
											|Путь к файлу: %1
											|Текст ошибки: %2.'");
		ПодробноеПредставлениеОшибки = ПодробноеПредставлениеОшибки(ИнформацияОбОшибке());
		ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
		...
		ВидОперации = ВернутьСтр("ru = 'Чтение полученного из банка электронного документа.'");
...
Функция ТекстСообщенияОбОшибкеОтветаБанка(Ошибка)
	...
	ШаблонОшибки = ВернутьСтр("ru = 'Получена ошибка из банка (%1). Код ошибки %2.
						|%3%4'");
...
Функция ДанныеДляОтправкиВБанк(Ссылки, ПрограммаБанка)
	...
	Пока ВыборкаСоглашения.Следующий() Цикл
		...
		Иначе
			ШаблонСообщения = ВернутьСтр("ru = 'Не указаны сертификаты в соглашении об использовании прямого обмена с банком %1'");
			ТекстСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
...
Функция МассивИдентификаторовВыпискиБанка(ЭДВыписка)
	...
	Если ДопИнформацияПоЭД.Свойство("СсылкаНаДвоичныеДанныеФайла")
		...
		Если ИмяФайла = Неопределено Тогда
			ТекстОшибки = ВернутьСтр("ru = 'Не удалось прочитать электронный документ. Проверьте настройку рабочего каталога'");
			ОбщегоНазначенияКлиентСервер.СообщитьПользователю(ТекстОшибки);
