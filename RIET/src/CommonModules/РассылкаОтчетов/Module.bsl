...
Функция ПодготовитьПараметрыИВыполнитьРассылку(Рассылка, ПараметрыЖурнала = Неопределено, ПредварительныеНастройки = Неопределено) Экспорт
	...
	Если Не ПараметрыЖурнала.Свойство("ИмяСобытия") Тогда
		ПараметрыЖурнала.Вставить("ИмяСобытия", ВернутьСтр("ru = 'Рассылка отчетов. Запуск по требованию'", ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка()));
	КонецЕсли;
	...
	Если Не Рассылка.Подготовлена
		...
		Если Не Рассылка.Подготовлена Тогда
			Причина = Причина + Символы.ПС + ВернутьСтр("ru = 'Рассылка не подготовлена'");
		КонецЕсли;
		Если Рассылка.ПометкаУдаления Тогда
			Причина = Причина + Символы.ПС + ВернутьСтр("ru = 'Рассылка помечена на удаление'");
		КонецЕсли;
		
		ЗаписьЖурнала(ПараметрыЖурнала, УровеньЖурналаРегистрации.Предупреждение,
			ВернутьСтр("ru = 'Завершение'"), СокрЛП(Причина));
		Возврат Ложь;
	...
	КонецЕсли;
	Если ФорматыПоУмолчанию.Количество() = 0 Тогда
		ВызватьИсключение ВернутьСтр("ru = 'Не установлены форматы по умолчанию.'");
	КонецЕсли;
	...
	Если Не ПараметрыДоставки.ИспользоватьПапку
		...
		ЗаписьЖурнала(ПараметрыЖурнала, УровеньЖурналаРегистрации.Предупреждение,
			ВернутьСтр("ru = 'Не выбран способ доставки.'"));
		Возврат Ложь;
	КонецЕсли;
	...
	Если НЕ Рассылка.Архивировать И НЕ ПустаяСтрока(Рассылка.ИмяФайлаСПараметрами) Тогда		
		ПараметрыДоставки.Вставить("ИмяФайлаСПараметрами", Рассылка.ИмяФайлаСПараметрами);	
	КонецЕсли;
	
	ПараметрыДоставки.Вставить("СмещениеНачалоПериода", Рассылка.СмещениеНачалоПериода);
	ПараметрыДоставки.Вставить("СмещениеКонецПериода", Рассылка.СмещениеКонецПериода);
	ПараметрыДоставки.Вставить("ВидСмещенияНачалоПериода", Рассылка.ВидСмещенияНачалоПериода);
	ПараметрыДоставки.Вставить("ВидСмещенияКонецПериода", Рассылка.ВидСмещенияКонецПериода);
	// } RGS ASeryakov 13.03.2018 10:00:00 PM SLI-0007499
...
Функция ВыполнитьРассылку(ТаблицаОтчетов, ПараметрыДоставки, Рассылка, ПараметрыЖурнала = Неопределено) Экспорт
	...
	ЗаписьЖурнала(ПараметрыЖурнала, ,
		ВернутьСтр("ru = 'Инициализирована рассылка %1, автор: %2'"), , 
		"'"+ Строка(Рассылка) +"'", 
	...
	Для Каждого СтрокаОтчет Из ТаблицаОтчетов Цикл
		ТекстЖурнала = ВернутьСтр("ru = 'Отчет %1 формируется'");
		Если СтрокаОтчет.Настройки = Неопределено Тогда
			ТекстЖурнала = ТекстЖурнала + Символы.ПС + ВернутьСтр("ru = '(пользовательские настройки не заданы)'");
		КонецЕсли;
		...
		Попытка
			...
			КонецЕсли;
			ЗаписьЖурнала(ПараметрыЖурнала, УровеньЖурналаРегистрации.Примечание,
				ВернутьСтр("ru = 'Отчет %1 успешно сформирован'"), ,
				"'"+ Строка(СтрокаОтчет.Отчет) +"'");
			НомерОтчета = НомерОтчета + 1;
		Исключение
			ЗаписьЖурнала(ПараметрыЖурнала, ,
				ВернутьСтр("ru = 'Отчет %1 не сформирован:'"), ИнформацияОбОшибке(),
				"'"+ Строка(СтрокаОтчет.Отчет) +"'");
		КонецПопытки;
	КонецЦикла;
	...
	Если ДеревоОтчетов.Строки.Найти(3, "Уровень", Истина) = Неопределено Тогда
		ЗаписьЖурнала(ПараметрыЖурнала, УровеньЖурналаРегистрации.Предупреждение,
			ВернутьСтр("ru = 'Не сохранено ни одного отчета'"), ,);
	...
	Для Каждого СтрокаПолучатель Из ДеревоОтчетов.Строки Цикл
		...
		Исключение
			ЗаписьЖурнала(ПараметрыЖурнала, ,
				ВернутьСтр("ru = 'Не удалось отправить отчеты получателю %1:'"), ИнформацияОбОшибке(), 
	...
	Если РассылкаВыполнена Тогда
		ЗаписьЖурнала(ПараметрыЖурнала, , ВернутьСтр("ru = 'Рассылка выполнена'"));
	Иначе
		ЗаписьЖурнала(ПараметрыЖурнала, , ВернутьСтр("ru = 'Рассылка не выполнена'"));
	КонецЕсли;
...
Процедура ВыполнитьРассылкиВФоновомЗадании(ПараметрыВыполнения, АдресРезультата) Экспорт
	...
	Если Всего = 0 Тогда
		ТекстСообщения = ВернутьСтр("ru = 'Выбранные группы не содержат рассылок отчетов.'");
	ИначеЕсли Всего <= 5 Тогда
		ТекстСообщения = "";
		Для Каждого СтрокаТаблицы Из ТаблицаРассылок Цикл
			Если Не СтрокаТаблицы.Подготовлена Тогда
				ШаблонСообщения = ВернутьСтр("ru = 'Рассылка ""%1"" не подготовлена.'");
			ИначеЕсли Не СтрокаТаблицы.Выполнена Тогда
				ШаблонСообщения = ВернутьСтр("ru = 'Рассылка ""%1"" не выполнена.'");
			ИначеЕсли СтрокаТаблицы.СОшибками Тогда
				ШаблонСообщения = ВернутьСтр("ru = 'Рассылка ""%1"" выполнена с ошибками.'");
			Иначе
				ШаблонСообщения = ВернутьСтр("ru = 'Рассылка ""%1"" выполнена.'");
			КонецЕсли;
	...
	Иначе
		ТекстСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			ВернутьСтр("ru = 'Подготовлено рассылок: %1 из %2
			|Выполнено: %3
			|С ошибками: %4
			|Не выполнено: %5'"),
...
Процедура ПриЗаполненииСпискаТекущихДел(ТекущиеДела) Экспорт
	...
	Для Каждого Раздел Из Разделы Цикл
		...
		Дело.Представление  = ВернутьСтр("ru = 'Проблемы с рассылками отчетов'");
...
Процедура ФормаОтчетаДобавитьКоманды(Форма, Отказ, СтандартнаяОбработка) Экспорт
	...
	КомандаСоздать.Заголовок = ВернутьСтр("ru = 'Создать рассылку отчетов...'");
	КомандаСоздать.Подсказка = ВернутьСтр("ru = 'Создать новую рассылку отчетов и добавить в нее отчет с текущими настройками.'");
	...
	КомандаПрисоединить.Заголовок = ВернутьСтр("ru = 'Включить в существующую рассылку отчетов...'");
	КомандаПрисоединить.Подсказка = ВернутьСтр("ru = 'Присоединить отчет с текущими настройками к существующей рассылке отчетов.'");
	...
	Подменю.Заголовок = ВернутьСтр("ru = 'Отправить рассылку'");
...
Процедура ВыполнитьРассылкуПоРасписанию(Рассылка) Экспорт
	...
	Если Не Рассылка.ВыполнятьПоРасписанию Тогда
		ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			ВернутьСтр("ru = 'У рассылки отчетов ""%1"" отключен флажок ""Выполнять по расписанию""'"),
			Строка(Рассылка));
	КонецЕсли;
	
	// Параметры записи в журнал регистрации.
	ПараметрыЖурнала = Новый Структура("ИмяСобытия, Метаданные, Данные");
	ПараметрыЖурнала.ИмяСобытия = ВернутьСтр("ru = 'Рассылка отчетов. Запуск по расписанию'", ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка());
...
Функция СформироватьСписокПолучателейРассылки(ПараметрыЖурнала, Рассылка) Экспорт
	...
	ШаблонСообщенияОбОшибке = ВернутьСтр("ru = 'При формировании списка получателей ""%1"" возникла ошибка:'");
	...
	Если СписокПолучателей.Количество() = 0 Тогда
		ТекстОшибок = ВернутьСтр("ru = 'Не удалось сформировать список получателей %1 рассылки %2 по одной из возможных причин:
...
Функция ИнициализироватьОтчет(ПараметрыЖурнала, ПараметрыОтчета, ПерсонализацияВозможна, УникальныйИдентификаторФормы = Неопределено) Экспорт
	...
	Исключение
		ПараметрыОтчета.Ошибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			ВернутьСтр("ru = 'При подключении и загрузке настроек отчета ""%1"" возникла ошибка.'"),
	...
	Если РассылкаОтчетовПовтИсп.ИсключаемыеОтчеты().Найти(Подключение.СсылкаОтчета) <> Неопределено Тогда
		ПараметрыОтчета.Ошибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			ВернутьСтр("ru = 'Отчет ""%1"" не поддерживает рассылку.
...
Функция СформироватьОтчет(ПараметрыЖурнала, ПараметрыОтчета, Получатель = Неопределено)
	...
	Если Не ПараметрыОтчета.Свойство("Инициализирован") Тогда
		ЗаписьЖурнала(ПараметрыЖурнала, ,
			ВернутьСтр("ru = 'Отчет ''%1'' не инициализирован'"), ,
			Строка(ПараметрыОтчета.Отчет));
		Возврат Результат;
	КонецЕсли;
	...
	Если Не Формирование.Успех Тогда
		ЗаписьЖурнала(ПараметрыЖурнала, УровеньЖурналаРегистрации.Ошибка,
			ВернутьСтр("ru = 'Отчет ""%1"":'"), Формирование.ТекстОшибки,
			Строка(ПараметрыОтчета.Отчет));
...
Функция ВыполнитьДоставку(ПараметрыЖурнала, ПараметрыДоставки, Вложения) Экспорт
	Результат = Ложь;
	ШаблонСообщенияОбОшибке = ВернутьСтр("ru = 'Ошибка доставки отчетов'");
	...
	Если ПараметрыДоставки.ИспользоватьЭлектроннуюПочту Тогда
		...
		Если ПараметрыДоставки.ТолькоУведомить Тогда
			ШаблонСообщенияОбОшибке = ВернутьСтр("ru = 'Невозможно отправить уведомление о рассылке по электронной почте:'");
			ВложенияПисьма = Новый Соответствие;
		Иначе
			ШаблонСообщенияОбОшибке = ВернутьСтр("ru = 'Невозможно отправить отчет по электронной почте:'");
	...
	Если Рассылка.ИспользоватьСетевойКаталог Тогда
		Если ЗначениеЗаполнено(Рассылка.СетевойКаталогWindows) Тогда
			Элемент = РаботаВБезопасномРежиме.РазрешениеНаИспользованиеКаталогаФайловойСистемы(
				...
				Истина,
				Истина,
				ВернутьСтр("ru = 'Сетевой каталог для публикации отчетов с сервера Windows.'"));
		...
		Если ЗначениеЗаполнено(Рассылка.СетевойКаталогLinux) Тогда
			Элемент = РаботаВБезопасномРежиме.РазрешениеНаИспользованиеКаталогаФайловойСистемы(
				...
				Истина,
				Истина,
				ВернутьСтр("ru = 'Сетевой каталог для публикации отчетов с сервера Linux.'"));
			Разрешения.Добавить(Элемент);
		КонецЕсли;
	КонецЕсли;
	Если Рассылка.ИспользоватьFTPРесурс Тогда
		Если ЗначениеЗаполнено(Рассылка.FTPСервер) Тогда
			Элемент = РаботаВБезопасномРежиме.РазрешениеНаИспользованиеИнтернетРесурса(
				...
				Рассылка.FTPПорт,
				ВернутьСтр("ru = 'FTP ресурс для публикации отчетов.'"));
			Разрешения.Добавить(Элемент);
		КонецЕсли;
	КонецЕсли;
	Возврат Разрешения;
...
Процедура СформироватьИСохранитьОтчет(ПараметрыЖурнала, ПараметрыОтчета, ДеревоОтчетов, ПараметрыДоставки, ПолучательСсылка)
	...
	// { RGS ASeryakov 13.03.2018 10:00:00 PM SLI-0007499
	ПараметрыДоступны = ?(ПараметрыДоставки.Свойство("СмещениеНачалоПериода"), Истина, Ложь); 
	
	СмещениеНачалоПериода = ПараметрыДоставки.СмещениеНачалоПериода;
	СмещениеКонецПериода = ПараметрыДоставки.СмещениеКонецПериода;
	ВидСмещенияНачалоПериода = ПараметрыДоставки.ВидСмещенияНачалоПериода;
	ВидСмещенияКонецПериода = ПараметрыДоставки.ВидСмещенияКонецПериода;
	
	Если ПараметрыДоступны Тогда
		
		ЭлементыНастроекКД = ПараметрыОтчета.Объект.КомпоновщикНастроек.ПользовательскиеНастройки.Элементы;
		
		Для каждого Элемент Из ЭлементыНастроекКД Цикл
			
			Если ТипЗнч(Элемент) = Тип("ЗначениеПараметраНастроекКомпоновкиДанных") 
				И Элемент.Параметр = Новый ПараметрКомпоновкиДанных("НачалоПериода") И СмещениеНачалоПериода <> 0 Тогда
				
				Если ВидСмещенияНачалоПериода Тогда					
					Элемент.Значение = ДобавитьМесяц(Элемент.Значение, СмещениеНачалоПериода); 					
				Иначе					
					Элемент.Значение = ДобавитьМесяц(Элемент.Значение, -СмещениеНачалоПериода);
				КонецЕсли;
			...
			Если ТипЗнч(Элемент) = Тип("ЗначениеПараметраНастроекКомпоновкиДанных") 
				И Элемент.Параметр = Новый ПараметрКомпоновкиДанных("КонецПериода") И СмещениеКонецПериода <> 0 Тогда
				
				Если ВидСмещенияКонецПериода Тогда					
					Элемент.Значение = КонецДня(ДобавитьМесяц(Элемент.Значение, СмещениеКонецПериода)); 					
				Иначе					
					Элемент.Значение = КонецДня(ДобавитьМесяц(Элемент.Значение, -СмещениеКонецПериода));
				КонецЕсли;
	...
	// { RGS ASeryakov 13.03.2018 10:00:00 PM SLI-0007499
	Если ПараметрыДоставки.Свойство("ИмяФайлаСПараметрами") Тогда		
		
		СтрокаОтчет.Ключ = ПривестиИмяФайла(
			РассылкаОтчетовКлиентСервер.ЗаполнитьШаблон(ПараметрыДоставки.ИмяФайлаСПараметрами,
	...
	Для Каждого Формат Из ПараметрыОтчета.Форматы Цикл
		...
		ПолноеИмяФайла = КаталогПолучателя + ПривестиИмяФайла(
			СтрокаОтчет.Ключ 
			// { RGS AArsentev 06.07.2018 - добавляет расширение в имя файла.
			//+ " ("
			//+ ПараметрыФормата.Имя 
			//+ ")" 
			// } RGS AArsentev 06.07.2018
		...
		Если СтандартнаяОбработка = Истина Тогда
			ЗаголовокОшибки = ВернутьСтр("ru = 'Ошибка записи отчета %1 в формат %2:'");
			
			Если ПараметрыФормата.ТипФайла = Неопределено Тогда
				ЗаписьЖурнала(ПараметрыЖурнала, УровеньЖурналаРегистрации.Ошибка,
					ЗаголовокОшибки, ВернутьСтр("ru = 'Формат не поддерживается'"),
					"'"+ СтрокаОтчет.Ключ +"'", 
					"'"+ ПараметрыФормата.Имя +"'");
				Продолжить;
			КонецЕсли;
		...
		Если Не ВременныйФайл.Существует() Тогда
			ЗаписьЖурнала(ПараметрыЖурнала, УровеньЖурналаРегистрации.Ошибка,
				ЗаголовокОшибки + Символы.ПС + ВернутьСтр("ru = 'Файл %3 не найден.'"), ,
				"'"+ СтрокаОтчет.Ключ +"'", 
				"'"+ ПараметрыФормата.Имя +"'",
...
Функция ЗаполнитьПараметрыРассылкиПараметрамиПоУмолчанию(ТаблицаОтчетов, ПараметрыДоставки, Рассылка, ПараметрыЖурнала)
	...
	Если ПараметрыДоставки.ИспользоватьПапку Тогда
		Если Не ЗначениеЗаполнено(ПараметрыДоставки.Папка) Тогда
			...
			ЗаписьЖурнала(ПараметрыЖурнала, УровеньЖурналаРегистрации.Предупреждение,
				ВернутьСтр("ru = 'Папка не заполнена, доставка в папку отключена'"));
		Иначе
			Если ОбщегоНазначения.ПодсистемаСуществует("СтандартныеПодсистемы.РаботаСФайлами") Тогда
				МодульРаботаСФайламиСлужебный = ОбщегоНазначения.ОбщийМодуль("РаботаСФайламиСлужебный");
			...
			Если Не ПравоДоступа Тогда
				...
				УстановитьПривилегированныйРежим(Ложь);
				ЗаписьЖурнала(ПараметрыЖурнала, УровеньЖурналаРегистрации.Ошибка,
					ВернутьСтр("ru = 'Недостаточно прав на изменение файлов папки %1, доставка в папку отключена'"), ,
					"'"+ ПредставлениеПапки +"'");
			КонецЕсли;
		КонецЕсли;
	КонецЕсли;
	...
	Если ПараметрыДоставки.ИспользоватьСетевойКаталог Тогда
		Если Не ЗначениеЗаполнено(ПараметрыДоставки.СетевойКаталогWindows) 
			...
			Если ЗначениеЗаполнено(ПараметрыДоставки.СетевойКаталогWindows) Тогда
				ЗначениеПодстановки = ВернутьСтр("ru = 'Linux'");
			ИначеЕсли ЗначениеЗаполнено(ПараметрыДоставки.СетевойКаталогLinux) Тогда
				ЗначениеПодстановки = ВернутьСтр("ru = 'Windows'");
			Иначе
				ЗначениеПодстановки = ВернутьСтр("ru = 'Windows и Linux'");
			КонецЕсли;
			
			ПараметрыДоставки.ИспользоватьСетевойКаталог = Ложь;
			ЗаписьЖурнала(ПараметрыЖурнала, УровеньЖурналаРегистрации.Ошибка,
				ВернутьСтр("ru = 'Сетевой каталог %1 не выбран, доставка в сетевой каталог отключена'"), ,
				ЗначениеПодстановки);
	...
	Если ПараметрыДоставки.ИспользоватьFTPРесурс И Не ЗначениеЗаполнено(ПараметрыДоставки.Сервер) Тогда
		...
		ЗаписьЖурнала(ПараметрыЖурнала, УровеньЖурналаРегистрации.Ошибка,
			ВернутьСтр("ru = 'FTP сервер не заполнен, доставка в папку на FTP ресурс отключена'"));
	КонецЕсли;
	
	Если ПараметрыДоставки.ИспользоватьЭлектроннуюПочту И Не ЗначениеЗаполнено(ПараметрыДоставки.УчетнаяЗапись) Тогда
		ПараметрыДоставки.ИспользоватьЭлектроннуюПочту = Ложь;
		ЗаписьЖурнала(ПараметрыЖурнала, УровеньЖурналаРегистрации.Ошибка,
			ВернутьСтр("ru = 'Учетная запись не выбрана, доставка по электронной почте отключена'"));
	КонецЕсли;
	...
	Если ПараметрыДоставки.Персонализирована Тогда
		Если Не ПараметрыДоставки.ИспользоватьЭлектроннуюПочту Тогда
			ЗаписьЖурнала(ПараметрыЖурнала, УровеньЖурналаРегистрации.Ошибка,
				ВернутьСтр("ru = 'Персонализированная рассылка может быть отправлена только по электронной почте'"));
			Возврат Ложь;
		КонецЕсли;
		ПараметрыДоставки.ИспользоватьПапку          = Ложь;
		ПараметрыДоставки.ИспользоватьСетевойКаталог = Ложь;
	...
	Если ПараметрыДоставки.ИспользоватьЭлектроннуюПочту Тогда
		...
		Если ПараметрыДоставки.ТолькоУведомить
			...
			И Не ПараметрыДоставки.ИспользоватьFTPРесурс Тогда
			ЗаписьЖурнала(ПараметрыЖурнала, УровеньЖурналаРегистрации.Предупреждение,
				ВернутьСтр("ru = 'Использование уведомлений по электронной почте возможно только совместно с другими способами доставки'"));
			Возврат Ложь;
		КонецЕсли;
		...
		Если ПараметрыПисьма.Свойство("Вложения") Тогда
			...
			Если ПараметрыДоставки.ПисьмоВФорматеHTML Тогда
				Для Каждого КлючИЗначение Из ПараметрыПисьма.ПредустановленныеВложения Цикл
					Если ТипЗнч(КлючИЗначение.Значение) = Тип("Картинка") И СтрНайти(КлючИЗначение.Ключ, ".") = 0 Тогда
	...
	Если ПараметрыДоставки.ИспользоватьЭлектроннуюПочту Тогда
		Если ПараметрыДоставки.ЗаполнитьСформированныеОтчетыВШаблонеСообщения Тогда
			ПараметрыДоставки.ПредставлениеОтчетовПолучателя = 
				...
				+ Символы.ПС 
				+ Символы.ПС
				+ ВернутьСтр("ru = 'Файлы отчетов запакованы в архив'")
				+ " ";
		КонецЕсли;
...
Функция ПравоВывода(ПараметрыЖурнала)
	...
	Если Не ПравоВывода Тогда
		ЗаписьЖурнала(ПараметрыЖурнала, УровеньЖурналаРегистрации.Ошибка,
			ВернутьСтр("ru = 'У пользователя %1 недостаточно прав на вывод информации'"), ,
...
Функция ПредставлениеСпособовДоставки(ПараметрыДоставки)
	Префикс = ВернутьСтр("ru = 'Результат'");
	ТекстПредставления = "";
	Суффикс = "";
	...
	Если Не ПараметрыДоставки.ТолькоУведомить Тогда
		...
		+ ?(ТекстПредставления = "", Префикс, " " + ВернутьСтр("ru = 'и'")) 
		+ " "
		+ ВернутьСтр("ru = 'отправлен по почте (см. вложения)'");
	...
	Если ПараметрыДоставки.ВыполненаВПапку Тогда
		...
		+ ?(ТекстПредставления = "", Префикс, " " + ВернутьСтр("ru = 'и'")) 
		+ " "
		+ ВернутьСтр("ru = 'доставлен в папку'")
		+ " ";
	...
	Если ПараметрыДоставки.ВыполненаВСетевойКаталог Тогда
		...
		+ ?(ТекстПредставления = "", Префикс, " " + ВернутьСтр("ru = 'и'")) 
		+ " "
		+ ВернутьСтр("ru = 'доставлен в сетевой каталог'")
		+ " ";
	...
	Если ПараметрыДоставки.ВыполненаНаFTP Тогда
		...
		+ ?(ТекстПредставления = "", Префикс, " " + ВернутьСтр("ru = 'и'")) 
		+ " "
		+ ВернутьСтр("ru = 'доставлен на FTP ресурс'")
		+ " ";
