...
Функция ИспользуемыеРазделы() Экспорт
	...
	Для Каждого ЭлементСписка Из СписокРазделов Цикл
		...
		Иначе
			ШаблонЗаголовка = ВернутьСтр("ru = 'Отчеты раздела ""%1""'");
		КонецЕсли;
		...
		Если РазделМетаданные = ИдентификаторНачальнойСтраницы Тогда
			...
			Строка.Представление = ВернутьСтр("ru = 'Начальная страница'");
		Иначе
...
Функция ОписаниеОтчета(Настройки, ОтчетЗначениеИлиМетаданные) Экспорт
	...
	Если СтрокаОтчет = Неопределено Тогда
		ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			ВернутьСтр("ru = 'Ошибка получения описания отчета ""%1"": он не подключен к подсистеме ""%2"".
...
Функция ОписаниеВарианта(Настройки, ОтчетСтрокаДереваИлиЗначениеИлиМетаданные, КлючВарианта) Экспорт
	...
	Если СтрокаВариант = Неопределено Тогда
		ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			ВернутьСтр("ru = 'Вариант ""%1"" не найден для отчета ""%2"".'"),
			КлючВарианта,
...
Процедура ДобавитьОбработчикиПолногоОбновления(Обработчики, Версия) Экспорт
	...
	Если РазрешеноИндексированиеНеразделенныхДанных() Тогда
		...
		Обработчик.Комментарий   = ВернутьСтр("ru = 'Полное обновление индекса поиска отчетов, предусмотренных в программе.
	...
	Обработчик.Комментарий = ВернутьСтр("ru = 'Полное обновление индекса поиска отчетов, сохраненных пользователями.
		|Поиск отчетов временно недоступен.'");
...
Процедура СброситьПользовательскиеНастройки(Ключ, ТипыНастроек = Неопределено) Экспорт
	...
	Иначе
		ВызватьИсключение ВернутьСтр("ru = 'Некорректное значение параметра ""Отчет""'");
	КонецЕсли;
	...
	Для Каждого ПолноеИмяВарианта Из КлючиВариантов Цикл
		...
		Пока Истина Цикл
			...
			Исключение
				...
				ОшибкаПоВарианту(
					Неопределено,
					ВернутьСтр("ru = 'В процессе выборки пользовательских настроек отчетов из системного хранилища возникла ошибка:'")
					+ Символы.ПС
...
Процедура ПеренестиПользовательскиеИзСтандартногоХранилища(ИменаОтчетов = "") Экспорт
	ПредставлениеПроцедуры = ВернутьСтр("ru = 'Прямая конвертация вариантов отчетов'");
	...
	Пока Истина Цикл
		...
		Исключение
			...
			ОшибкаПоВарианту(
				Неопределено,
				ВернутьСтр("ru = 'В процессе выборки вариантов отчетов из стандартного хранилища возникла ошибка:'")
				+ Символы.ПС
				+ ПодробноеПредставлениеОшибки(ИнформацияОбОшибке()));
		КонецПопытки;
...
Процедура ЗагрузитьПользовательские(ТаблицаВариантов = Неопределено) Экспорт
	...
	ПредставлениеПроцедуры = ВернутьСтр("ru = 'Завершить конвертацию вариантов отчетов'");
	ЗапускПроцедуры(ПредставлениеПроцедуры);
	...
	Для Каждого СтрокаТаблицы Из ОпределенныеВарианты Цикл
		...
		Иначе
			ОшибкаПоВарианту(
				...
				ВернутьСтр("ru = 'Вариант ""%1"" отчета ""%2"": не найден автор ""%3""'"),
...
Процедура ПриДобавленииОбработчиковОбновления(Обработчики) Экспорт
	...
	Если РазрешеноИндексированиеНеразделенныхДанных() Тогда
		...
		Обработчик.Комментарий   = ВернутьСтр("ru = 'Обновление индекса поиска отчетов, предусмотренных в программе.
			|Поиск отчетов временно недоступен.'");
	КонецЕсли;
	...
	Обработчик.Комментарий = ВернутьСтр("ru = 'Уменьшает количество быстрых настроек
...
Функция ПодключитьОтчетИЗагрузитьНастройки(Параметры) Экспорт
		...
		Если ТипЗнч(Результат.СсылкаВарианта) <> Тип("СправочникСсылка.ВариантыОтчетов")
			...
			Если Результат.СсылкаВарианта = Неопределено Тогда
				Результат.ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					ВернутьСтр("ru = 'В методе ""%1"" не указаны параметры ""%2"".'"),
	...
	Если ТипЗнч(Результат.НастройкиКД) <> Тип("НастройкиКомпоновкиДанных") Тогда
		...
		Если ВариантНастроекКД = Неопределено Тогда
			Результат.ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				ВернутьСтр("ru = 'Вариант ""%1"" не найден в схеме отчета ""%2"".'"),
	...
	Если ВариантыОтчетовПовтИсп.ОтчетыСНастройками().Найти(Результат.СсылкаОтчета) <> Неопределено Тогда
...
Функция ПодключитьОтчетОбъект(СсылкаОтчета, ПолучатьМетаданные)
	...
	Если СсылкаОтчета = Неопределено Тогда
		Результат.ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			ВернутьСтр("ru = 'В методе ""%1"" не указан параметр ""%2"".'"),
	...
	Если ТипЗнч(Результат.Ссылка) = Тип("Строка") Тогда
		Результат.ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			ВернутьСтр("ru = 'Отчет ""%1"" записан как внешний и не может быть подключен из программы'"),
			Результат.Ссылка);
		Возврат Результат;
	КонецЕсли;
	...
	Если ТипЗнч(Результат.Ссылка) = Тип("СправочникСсылка.ИдентификаторыОбъектовМетаданных")
		...
		Если ПолучатьМетаданные Тогда
			...
			Если Результат.Метаданные = Неопределено Тогда
				Результат.ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					ВернутьСтр("ru = 'Отчет ""%1"" не найден в программе'"),
					Результат.Имя);
				Возврат Результат;
			КонецЕсли;
		КонецЕсли;
...
Процедура ПриЗаписиДополнительногоОтчета(ТекущийОбъект, Отказ, ВнешнийОбъект) Экспорт
	...
	Если Не ВариантыОтчетовПовтИсп.ПравоДобавления() Тогда
		ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			ВернутьСтр("ru = 'Недостаточно прав доступа для записи вариантов дополнительного отчета ""%1"".'"),
...
Функция ПараметрыОткрытия(ВариантСсылка) Экспорт
	...
	Иначе
		...
		Если Не Выборка.Следующий() Тогда
			ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(ВернутьСтр("ru = 'Недостаточно прав для открытия варианта ""%1"".'"), Строка(ВариантСсылка));
		КонецЕсли;
...
Процедура ПриПодключенииОтчета(ПараметрыОткрытия) Экспорт
	...
	ИначеЕсли ПараметрыОткрытия.ТипОтчета = "Дополнительный" Тогда
		...
		Иначе
			СтандартныеПодсистемыКлиентСервер.ВывестиПредупреждение(ПараметрыОткрытия,
				ВернутьСтр("ru = 'Невозможно открыть вариант дополнительного отчета'"),
				ВернутьСтр("ru = 'Не внедрена обслуживающая подсистема.'"));
		КонецЕсли;
	КонецЕсли;
...
Функция СформироватьОтчет(Знач Параметры, Знач ПроверятьЗаполнение, Знач ПолучатьФлажокПустой) Экспорт
	...
	Если Не Подключение.Успех Тогда
		Результат.ТекстОшибки = ВернутьСтр("ru = 'Не удалось сформировать отчет:'") + Символы.ПС + Подключение.ТекстОшибки;
		Возврат Результат;
	КонецЕсли;
	...
	Если ПроверятьЗаполнение Тогда
		...
		Если Не ПроверкаПройдена Тогда
			Результат.ТекстОшибки = ВернутьСтр("ru = 'Отчет не прошел проверку заполнения:'");
...
Функция ДеревоПредопределенных(ФильтрПоТипуОтчетов = "Внутренний") Экспорт
	...
	Для Каждого ОтчетМетаданные Из Метаданные.Отчеты Цикл
		...
		Если СтрокаОтчет.ИспользуетСКД Тогда
			...
			Исключение
				ТекстОшибки = ВернутьСтр("ru = 'Не удалось прочитать схему отчета ""%1"":'");
			...
			Если СхемаКД <> Неопределено Тогда
				...
				Исключение
					ТекстОшибки = ВернутьСтр("ru = 'Не удалось прочитать список вариантов отчета ""%1"" из схемы:'");
					ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(ТекстОшибки, ОтчетМетаданные.Имя);
					ТекстОшибки = ТекстОшибки + Символы.ПС + ПодробноеПредставлениеОшибки(ИнформацияОбОшибке());
					ПредупреждениеПоВарианту(Неопределено, ТекстОшибки);
				КонецПопытки;
			...
			Если ВариантыНастроек = Неопределено Тогда
				...
				Исключение
					ТекстОшибки = ВернутьСтр("ru = 'Не удалось прочитать список вариантов отчета ""%1"" из модуля менеджера:'");
					ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(ТекстОшибки, ОтчетМетаданные.Имя);
					ТекстОшибки = ТекстОшибки + Символы.ПС + ПодробноеПредставлениеОшибки(ИнформацияОбОшибке());
			...
			Если ВариантыНастроек <> Неопределено Тогда
				Для Каждого ВариантНастроекКД Из ВариантыНастроек Цикл
					...
					Вариант.Наименование = СокрЛП(СтрЗаменить(СтрокаОтчет.Отчет, "(Отчет)", "")) + " [ " + ВариантНастроекКД.Представление + " ]";
					...
					Если РазрешеноИндексирование И ТипЗнч(ВариантНастроекКД) = Тип("ВариантНастроекКомпоновкиДанных") Тогда
						...
						Исключение
							ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
								ВернутьСтр("ru = 'Не удалось прочитать настройки варианта ""%1"" отчета ""%2"":'"),
		...
		Если СтрокаОтчет.ОсновнойВариант = Неопределено Тогда
			...
			Вариант.Наименование = СокрЛП(СтрЗаменить(СтрокаОтчет.Отчет, "(Отчет)", ""));
...
Процедура УстановитьРежимВыводаОтчетаВПанеляхОтчетов(ДеревоВариантов, Отчет, ВыводитьОтчетыВместоВариантов)
	...
	Иначе
		...
		Если СтрокаОтчет = Неопределено Тогда
			ПредупреждениеПоВарианту(Неопределено, ВернутьСтр("ru = 'Отчет ""%1"" не подключен к подсистеме.'"), Отчет.Имя);
			Возврат;
		КонецЕсли;
	КонецЕсли;
	СтрокаОтчет.ГруппироватьПоОтчету = ВыводитьОтчетыВместоВариантов;
...
Функция ИзмененияКлючей()
	...
	Для Каждого Изменение Из Изменения Цикл
		...
		Если Найденные.Количество() > 0 Тогда
			...
			ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				ВернутьСтр("ru = 'Ошибка регистрации изменений имени варианта отчета ""%1"":
...
Функция СформироватьИнформациюОбОтчетеПоПолномуИмени(ОтчетПолноеИмя) Экспорт
	...
	Если ВРег(Префикс) = "ОТЧЕТ" Тогда
		...
		Если Результат.ОтчетМетаданные = Неопределено Тогда
			...
			ПредупреждениеПоВарианту(
				Неопределено,
				ВернутьСтр("ru = 'Отчет ""%1"" не найден в программе, он будет значиться как внешний.'"),
				ОтчетПолноеИмя);
		ИначеЕсли Не ПравоДоступа("Просмотр", Результат.ОтчетМетаданные) Тогда
			Результат.ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				ВернутьСтр("ru = 'Недостаточно прав доступа к отчету ""%1"".'"),
				ОтчетПолноеИмя);
		КонецЕсли;
	...
	Иначе
		Результат.ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			ВернутьСтр("ru = 'Для отчета ""%1"" невозможно определить тип (не установлен префикс).'"),
			ОтчетПолноеИмя);
		Возврат Результат;
	КонецЕсли;
...
Функция ПолныеПраваНаВарианты() Экспорт
	...
	Возврат (Пользователи.РолиДоступны("ПолныеПрава")
		ИЛИ Пользователи.РолиДоступны("ImportExportAdministrator")
		ИЛИ Пользователи.РолиДоступны("LocalDistributionAdministrator")
		// { RGS ASeryakov 13.03.2018 10:00:00 PM SLI-0007499
		ИЛИ Пользователи.РолиДоступны("ImportExportReports")
		// { RGS ASeryakov 13.03.2018 10:00:00 PM SLI-0007499
		
		// { RGS ASeryakov, 07.08.2018 16:19:04 S-I-0005797
		ИЛИ Пользователи.РолиДоступны("ImportExportSpecialist")
		// } RGS ASeryakov 07.08.2018 16:19:06 S-I-0005797
		
		ИЛИ Пользователи.РолиДоступны("LocalDistributionAdministrator_ForNonLawsonCompanies"));  //}RG-Soft - конец изменения
...
Функция ГлобальныеНастройки() Экспорт
	...
	Результат.Поиск.Вставить("ПодсказкаВвода", ВернутьСтр("ru = 'Наименование, поле или автор отчета'"));
...
Процедура ЗапускПроцедуры(ПредставлениеПроцедуры)
	ИнформацияПоВарианту(Неопределено, ВернутьСтр("ru = 'Запуск процедуры ""%1"".'"), ПредставлениеПроцедуры);
КонецПроцедуры
...
Процедура ЗавершениеПроцедуры(ПредставлениеПроцедуры, Изменено = Неопределено)
	Текст = ВернутьСтр("ru = 'Завершение процедуры ""%1"".'");
	Если Изменено <> Неопределено Тогда
		Текст = Текст + " " + ВернутьСтр("ru = 'Изменено %2 объектов.'");
	КонецЕсли;
...
Процедура ПерейтиКРедакции21() Экспорт
	ПредставлениеПроцедуры = ВернутьСтр("ru = 'Перейти к редакции 2.1'");
	ЗапускПроцедуры(ПредставлениеПроцедуры);
...
Процедура ЗаполнитьСсылкиПредопределенных() Экспорт
	ПредставлениеПроцедуры = ВернутьСтр("ru = 'Заполнить ссылки предопределенных вариантов отчетов'");
	ЗапускПроцедуры(ПредставлениеПроцедуры);
...
Процедура СократитьКоличествоБыстрыхНастроек(ВходящиеПараметры = Неопределено) Экспорт
	ПредставлениеПроцедуры = ВернутьСтр("ru = 'Сокращение количества быстрых настроек в отчетах'");
	ЗапускПроцедуры(ПредставлениеПроцедуры);
	...
	Для Каждого СтрокаТаблицы Из ТаблицаОтчетов Цикл
		...
		Если ИнформацияОбОшибке <> Неопределено Тогда // Возникла проблема.
			ТекстОшибки = ВернутьСтр("ru = 'Вариант ""%1"" отчета ""%2"":'")
			+ Символы.ПС + ВернутьСтр("ru = 'При уменьшении количества быстрых настроек пользовательского возникла ошибка:'")
	...
	Если Ошибок > 0 И НомерПопытки <= 30 Тогда
		...
		ТекстОшибки = ПредставлениеПроцедуры + ":"
			+ Символы.ПС + ВернутьСтр("ru = 'Не удалось уменьшить количество быстрых настроек %1 отчетов.'");
...
Функция ОперативноеОбновлениеОбщихДанных(Режим, РазделенныеОбработчики)
	...
	Кэш.Вставить("Уточнение", ?(Кэш.Режим = "ОбщиеДанныеКонфигурации", ВернутьСтр("ru = 'метаданные конфигурации'"), ВернутьСтр("ru = 'метаданные расширений'")));
...
Функция ОтложенноеОбновлениеДанных(Режим, Полное)
	...
	Кэш.Вставить("Уточнение", НРег(ОтчетыКлиентСервер.ИмяВПредставление(Режим)));
	Кэш.Уточнение = Кэш.Уточнение + ", " + ?(Полное, ВернутьСтр("ru = 'полное'"), ВернутьСтр("ru = 'по изменениям'"));
...
Процедура ОбновитьИндексПоиска(Кэш)
	...
	ПредставлениеПроцедуры = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(ВернутьСтр("ru = 'Обновление индекса поиска (%1)'"), Кэш.Уточнение);
	ЗапускПроцедуры(ПредставлениеПроцедуры);
	...
	Выборка = Запрос.Выполнить().Выбрать();
	Пока Выборка.Следующий() Цикл
		ПримечаниеПоВарианту(Выборка.Ссылка, ВернутьСтр("ru = 'Индексация схемы'"));
		...
		Если Кэш.ОбщиеДанные Тогда
			...
			Если Найденные.Количество() = 0 Тогда
				ТекстОшибки = ВернутьСтр("ru = 'Вариант ""%1"" не найден для отчета ""%2""'");
				ОшибкаПоВарианту(ВариантОбъект.Ссылка, ТекстОшибки, ВариантОбъект.КлючВарианта, ВариантОбъект.Отчет);
...
Процедура АктуализироватьКлючиПредопределенных(Кэш)
	ПредставлениеПроцедуры = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(ВернутьСтр("ru = 'Обновление ключей вариантов отчетов (%1)'"), Кэш.Уточнение);
	ЗапускПроцедуры(ПредставлениеПроцедуры);
...
Процедура ОбновитьНастройкиПредопределенных(Кэш)
	ПредставлениеПроцедуры = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(ВернутьСтр("ru = 'Обновление настроек предопределенных (%1)'"), Кэш.Уточнение);
	ЗапускПроцедуры(ПредставлениеПроцедуры);
...
Процедура АктуализироватьСоставОбласти(Кэш)
	ПредставлениеПроцедуры = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(ВернутьСтр("ru = 'Обновление вариантов отчетов (%1)'"), Кэш.Уточнение);
	ЗапускПроцедуры(ПредставлениеПроцедуры);
...
Процедура ПометитьНаУдалениеВариантыУдаленныхОтчетов(Кэш)
	ПредставлениеПроцедуры = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(ВернутьСтр("ru = 'Удаление вариантов удаленных отчетов (%1)'"), Кэш.Уточнение);
	ЗапускПроцедуры(ПредставлениеПроцедуры);
...
Процедура ЗарегистрироватьЗамерыВариантаКОбновлению(Кэш, Знач СтарыйКлюч, Знач АктуальныйКлюч, Знач АктуальноеНаименование)
	...
	ОбновлениеЗамера.АктуальноеНаименование = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(ВернутьСтр("ru = 'Отчет ""%1"" (открытие)'"), АктуальноеНаименование);
	
	ОбновлениеЗамера = Кэш.ТаблицаЗамеров.Добавить();
	ОбновлениеЗамера.СтароеИмя     = СтарыйКлюч     + ".Формирование";
	ОбновлениеЗамера.АктуальноеИмя = АктуальныйКлюч + ".Формирование";
	ОбновлениеЗамера.АктуальноеНаименование = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(ВернутьСтр("ru = 'Отчет ""%1"" (формирование)'"), АктуальноеНаименование);
	
	ОбновлениеЗамера = Кэш.ТаблицаЗамеров.Добавить();
	ОбновлениеЗамера.СтароеИмя     = СтарыйКлюч     + ".Настройки";
	ОбновлениеЗамера.АктуальноеИмя = АктуальныйКлюч + ".Настройки";
	ОбновлениеЗамера.АктуальноеНаименование = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(ВернутьСтр("ru = 'Отчет ""%1"" (настройки)'"), АктуальноеНаименование);
КонецПроцедуры
...
Процедура ПриОбновленииКлючевыхОпераций(ТаблицаОбновленияКлючевыхОпераций)
	...
	МодульОценкаПроизводительностиВызовСервераПолныеПрава = ОбщегоНазначения.ОбщийМодуль("ОценкаПроизводительностиВызовСервераПолныеПрава");
	...
	Пока Выборка.Следующий() Цикл
		...
		Если Выборка.Приоритет = 3 Тогда
			КлючеваяОперацияСсылка = МодульОценкаПроизводительностиВызовСервераПолныеПрава.СоздатьКлючевуюОперацию(АктуальноеИмя);
		Иначе
...
Функция СписокГлобальныхКлючевыхОпераций()
	...
	СписокОпераций.Добавить("ПанельОтчетов.Открытие",       ВернутьСтр("ru = 'Панель отчетов (открытие)'"));
	СписокОпераций.Добавить("ПанельОтчетов.Поиск",          ВернутьСтр("ru = 'Панель отчетов (поиск в режиме просмотра)'"));
	СписокОпераций.Добавить("ПанельОтчетов.РежимНастройки", ВернутьСтр("ru = 'Панель отчетов (режим настройки)'"));
	СписокОпераций.Добавить("СписокОтчетов.Открытие", ВернутьСтр("ru = 'Список отчетов (открытие)'"));
	СписокОпераций.Добавить("СписокОтчетов.Поиск",    ВернутьСтр("ru = 'Список отчетов (поиск)'"));
...
Процедура ЗаписатьПараметрыВариантовОтчетов(Кэш)
	...
	КонецЕсли;
	ПредставлениеПроцедуры = ВернутьСтр("ru = 'Запись неразделенного кэша в константу'");
	ЗапускПроцедуры(ПредставлениеПроцедуры);
...
Процедура ЗафиксироватьТекущуюВерсиюРасширений(Кэш)
	...
	ПредставлениеПроцедуры = ВернутьСтр("ru = 'Запись регистра версий расширений'");
	ЗапускПроцедуры(ПредставлениеПроцедуры);
...
Функция ПроиндексироватьСодержимоеСхемы(ВариантОбъект) Экспорт
	...
	Если СхемаКД = Неопределено Тогда
		ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			ВернутьСтр("ru = 'Для варианта ""%1"" отчета ""%2"" не заполнены настройки поиска:
			|Наименования полей или Наименования параметров и отборов.'"),
			ВариантОбъект.КлючВарианта,
			ВариантОбъект.Отчет);
		Если ЭтоПредопределенный Тогда
			ТекстОшибки = ТекстОшибки + Символы.ПС
				+ ВернутьСтр("ru = 'Подробнее - см. процедуру ""НастроитьВариантыОтчетов"" модуля ""ВариантыОтчетовПереопределяемый"".'");
	...
	Если ТипЗнч(НастройкиКД) <> Тип("НастройкиКомпоновкиДанных") Тогда
		Если ТипЗнч(ВариантОбъект) = Тип("СправочникОбъект.ПредопределенныеВариантыОтчетов")
			...
			Текст = ВернутьСтр("ru = 'Не удалось прочитать настройки предопределенного варианта отчета ""%1"".'");
...
Функция УменьшитьКоличествоБыстрыхНастроек(ВариантОбъект, ОтчетОбъект)
	...
	НастройкиКД = ВариантОбъект.Настройки.Получить();
	Если ТипЗнч(НастройкиКД) <> Тип("НастройкиКомпоновкиДанных") Тогда
		ТекстОшибки = ВернутьСтр("ru = 'Обнаружены пустые настройки пользовательского варианта ""%1"" отчета ""%2"".'");
		ОшибкаПоВарианту(ВариантОбъект.Ссылка, ТекстОшибки, ВариантОбъект.КлючВарианта, ВариантОбъект.Отчет);
...
Функция НастройкиФормыОтчета(ОтчетСсылка, КлючВарианта, ОтчетОбъект) Экспорт
	...
	Если ВариантыОтчетовПовтИсп.ОтчетыСНастройками().Найти(ОтчетСсылка) <> Неопределено Тогда
		Если ОтчетОбъект = Неопределено Тогда
			Подключение = ПодключитьОтчетОбъект(ОтчетСсылка, Ложь);
			Если Подключение.Успех Тогда
				ОтчетОбъект = Подключение.Объект;
			Иначе
				Текст = ВернутьСтр("ru = 'Не удалось получить настройки отчета ""%1"":'") + Символы.ПС + Подключение.ТекстОшибки;
	...
	Если ЕстьОтборПоПодсистемам Или ЕстьСтрокаПоиска Тогда
		...
		Если Не ЕстьСтрокаПоиска И Не ПолучатьИтоговуюТаблицу Тогда
			...
			ТекстЗапроса = СтрЗаменить(
				...
				"НЕОПРЕДЕЛЕНО");
		Иначе
			Запрос.УстановитьПараметр("НаименованиеРабочегоСтола", ВернутьСтр("ru = 'Начальная страница'"));
		КонецЕсли;
