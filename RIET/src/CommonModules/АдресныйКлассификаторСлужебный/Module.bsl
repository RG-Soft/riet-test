...
// 
Процедура ЗаполнитьВерсиюПоставщикаДанныхВнутр(Результат) Экспорт
	
	Результат.Данные = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(ВернутьСтр("ru = 'БСП %1'"), СтандартныеПодсистемыСервер.ВерсияБиблиотеки());
...
// 
Функция ТаблицаДанныхДляВыбораПоПочтовомуИндексу() Экспорт
...
// 
Процедура ЗаполнитьАдресаПоПочтовомуИндексуКлассификатораВнутр(Результат, Индекс, Уровни, ДополнительныеПараметры) Экспорт
...
Функция АдресаДляИнтерактивногоВыбора(Родитель, Уровень, ДополнительныеПараметры) Экспорт
	...
	Если Уровень = 1 Тогда
		Результат.Заголовок = ВернутьСтр("ru = 'Выберите регион'");
	ИначеЕсли Уровень = 2 Тогда
		Результат.Заголовок = ВернутьСтр("ru = 'Выберите округ'");
	ИначеЕсли Уровень = 3 Тогда
		Результат.Заголовок = ВернутьСтр("ru = 'Выберите район'");
	ИначеЕсли Уровень = 4 Тогда
		Результат.Заголовок = ВернутьСтр("ru = 'Выберите город'");
	ИначеЕсли Уровень = 5 Тогда
		Результат.Заголовок = ВернутьСтр("ru = 'Выберите внутригородской район'");
	ИначеЕсли Уровень = 6 Тогда
		Результат.Заголовок = ВернутьСтр("ru = 'Выберите населенный пункт'");
	ИначеЕсли Уровень = 7 Тогда
		Результат.Заголовок = ВернутьСтр("ru = 'Выберите улицу'");
	ИначеЕсли Уровень = 90 Тогда
		Результат.Заголовок = ВернутьСтр("ru = 'Выберите дополнительный элемент'");
	ИначеЕсли Уровень = 91 Тогда
		Результат.Заголовок = ВернутьСтр("ru = 'Выберите подчиненный элемент'");
	ИначеЕсли Уровень = -1 Тогда
		Результат.Заголовок = ВернутьСтр("ru = 'Выберите ориентир'");
	КонецЕсли;
...
// 
Функция ТаблицаДанныхДляИнтерактивногоВыбора() Экспорт
...
// 
Процедура ЗаполнитьАдресаДляИнтерактивногоВыбораВнутр(Результат, Уровни, Родитель, Уровень, ДополнительныеПараметры) Экспорт
	...
	Иначе
		...
		ИначеЕсли Уровень = 91 Тогда
			...
			ТекстЗапроса = ТекстЗапроса + "
				...
				|";
		Иначе
			ВызватьИсключение ВернутьСтр("ru = 'Запрос некорректного уровня адресного объекта'");
		КонецЕсли;
...
// 
Функция ТаблицаДанныхАвтоподбора() Экспорт
...
//
Процедура ЗаполнитьСписокАвтоподбораЧастиАдресаВнутр(Результат, Текст, Родитель, УровниЗапроса, ДополнительныеПараметры) Экспорт
...
// Конструктор структуры - результата.
//
Функция СтруктураДополнительныхАдресныхСведений() Экспорт
	
	Возврат Новый Структура("OKATO, ОКТМО, КодИФНСФЛ, КодИФНСЮЛ, КодУчасткаИФНСФЛ, КодУчасткаИФНСЮЛ");
	
КонецФункции

// Заполнение дополнительных данных из загруженных данных.
//
Процедура ЗаполнитьДополнительныеАдресныеСведенияВнутр(Результат, Идентификатор) Экспорт
	...
	ТекстЗапроса = "ВЫБРАТЬ ПЕРВЫЕ 1
		...
		|ГДЕ	
		|	Идентификатор В (
		|		// Как идентификатор дополнительных сведений
		|		ВЫБРАТЬ 
		|			&Идентификатор
		|		ОБЪЕДИНИТЬ ВСЕ
		|		// Как идентификатор адресного объекта
		|		ВЫБРАТЬ ПЕРВЫЕ 1
		|			Дополнительно
		|		ИЗ
		...
		|		// Как идентификатор ориентира
		|		ОБЪЕДИНИТЬ ВСЕ
		|		ВЫБРАТЬ ПЕРВЫЕ 1
		|			Дополнительно
		|		ИЗ
		|			РегистрСведений.ОриентирыАдресныхОбъектов
		|		ГДЕ
		|			Идентификатор = &Идентификатор
		|	)";
...
//
Процедура ЗаполнитьАктуальныеАдресныеСведенияВнутр(Результат, Идентификатор) Экспорт
...
//
Процедура ЗаполнитьРезультатПроверкиАдресаПоКлассификаторуВнутр(Результат, АдресаДляПроверки) Экспорт
...
Функция АнализОдногоАдресаПоКлассификатору(Адрес, Уровни)
	...
	Если ПустаяСтрока(СубъектРФ.Наименование) Тогда
		// Нет субъекта
		ДобавитьОшибкуПроверкиАдресаПоКлассификатору(ОшибкиПроверки, "СубъектРФ", ВернутьСтр("ru = 'Не указан субъект РФ адреса'") );
		Возврат Результат;
	ИначеЕсли Уровни.Найти(1) = Неопределено Тогда
		// В уровнях должно быть указано всегда.
		ВызватьИсключение ВернутьСтр("ru = 'В уровнях для проверки адреса не указан уровень субъекта РФ.'");
	КонецЕсли;
	...
	Если РегионЗагруженВАдресныеСведения = Неопределено Тогда 
			ДобавитьОшибкуПроверкиАдресаПоКлассификатору(ОшибкиПроверки, "СубъектРФ", 
				СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(ВернутьСтр("ru = 'Регион ""%1"" не существует'"), Адрес.СубъектРФ));
		Результат.АдресПроверен = Истина;
		Возврат Результат;
	ИначеЕсли НЕ РегионЗагруженВАдресныеСведения Тогда
		ДобавитьОшибкуПроверкиАдресаПоКлассификатору(ОшибкиПроверки, "СубъектРФ", ВернутьСтр("ru = 'Отсутствуют адресные сведения по региону'") + " " + Адрес.СубъектРФ);
	...
	Если РезультатЗапроса.Пустой() Тогда
		// Нет региона
		ДобавитьОшибкуПроверкиАдресаПоКлассификатору(ОшибкиПроверки, "СубъектРФ", 
			СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(ВернутьСтр("ru = 'Субъект РФ (регион) ""%1"" не найден в адресном классификаторе'"), СубъектРФ.Наименование + " " + СубъектРФ.Сокращение));
		Возврат Результат;
	...
	Для Каждого Запись Из ЗаписиДомов Цикл;
		...
		Если ПроверятьОкруг И Не Запись.ОкругНайден Тогда
			ДобавитьОшибкуПроверкиАдресаПоКлассификатору(ОшибкиПроверки, "Округ", 
				СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(ВернутьСтр("ru = 'Округ ""%1"" отсутствует в адресном классификаторе'"), Округ.Наименование + " " + Округ.Сокращение));
		КонецЕсли;
		...
		Если ПроверятьРайон И Не Запись.РайонНайден Тогда
			ДобавитьОшибкуПроверкиАдресаПоКлассификатору(ОшибкиПроверки, "СвРайМО/Район", 
				СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(ВернутьСтр("ru = 'Район ""%1"" отсутствует в адресном классификаторе'"), Район.Наименование + " " + Район.Сокращение));
		КонецЕсли;
		
		Если ПроверятьГород И Не Запись.ГородНайден Тогда
			ДобавитьОшибкуПроверкиАдресаПоКлассификатору(ОшибкиПроверки, "Город", 
				СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(ВернутьСтр("ru = 'Город ""%1"" отсутствует в адресном классификаторе'"), Город.Наименование + " " + Город.Сокращение));
		КонецЕсли;
		...
		Если ПроверятьВнутригородскойРайон И Не Запись.ВнутригородскойРайонНайден Тогда
			ДобавитьОшибкуПроверкиАдресаПоКлассификатору(ОшибкиПроверки, "ВнутригРайон", 
				СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(ВернутьСтр("ru = 'Внутригородской район ""%1"" отсутствует в адресном классификаторе'"),
		...
		Если ПроверятьНаселенныйПункт И Не Запись.НаселенныйПунктНайден Тогда
			ДобавитьОшибкуПроверкиАдресаПоКлассификатору(ОшибкиПроверки, "НаселПункт", 
				СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(ВернутьСтр("ru = 'Населенный пункт ""%1"" отсутствует в адресном классификаторе'"),
		...
		Если ПроверятьДополнительный И Не Запись.ДополнительныйНайден Тогда
			ДобавитьОшибкуПроверкиАдресаПоКлассификатору(ОшибкиПроверки, "Дополнительный", 
				СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(ВернутьСтр("ru = 'Дополнительный элемент ""%1""отсутствует в адресном классификаторе'"),
		...
		Если ПроверятьПодчиненный И Не Запись.ПодчиненныйНайден Тогда
			ДобавитьОшибкуПроверкиАдресаПоКлассификатору(ОшибкиПроверки, "Подчиненный", 
				СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(ВернутьСтр("ru = 'Подчиненный элемент ""%1"" отсутствует в адресном классификаторе'"),
		...
		Если ИсторияОбъекта.Количество() > 0 Тогда 
			Для каждого СтрокаОбъект Из ИсторияОбъекта Цикл
				ДобавитьОшибкуПроверкиАдресаПоКлассификатору(ОшибкиПроверки, "Улица", 
					СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(ВернутьСтр("ru = 'Улица ""%1"" была переименована в ""%2""'"), СтрокаОбъект.Значение, СтрокаОбъект.Представление));
			КонецЦикла;
		Иначе
			ДобавитьОшибкуПроверкиАдресаПоКлассификатору(ОшибкиПроверки, "Улица", 
				СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(ВернутьСтр("ru = 'Улица ""%1"" отсутствует в адресном классификаторе'"), Улица.Наименование + " " + Улица.Сокращение));
		КонецЕсли;
	КонецЕсли;
	
	Если НЕ ИндексНайден Тогда
		ДобавитьОшибкуПроверкиАдресаПоКлассификатору(ОшибкиПроверки, "Индекс", 
			СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(ВернутьСтр("ru = 'Индекс ""%1"" в адресном классификаторе не соответствует введенному адресу'"), Индекс));
		Возврат Результат;
	КонецЕсли;
	...
	Если ЕстьЗаписиДомов И Не ЕстьВарианты Тогда
		...
		Если ЗначениеЗаполнено(ОписаниеЗданияПоСоответствиюФИАС) Тогда
			...
			Если НеточныеСовпадения.Количество() = 1 Тогда
				Подсказка = ", " + ВернутьСтр("ru = 'но есть'") + """" + НеточныеСовпадения[0]+ """";
			ИначеЕсли НеточныеСовпадения.Количество() > 1 Тогда
				...
				Если НеточныеСовпадения.Количество() = 2 Тогда
					Подсказка = ВернутьСтр("ru = 'но есть ""%1"" или ""%2""'");
					ВтороеЗначение =  НеточныеСовпадения[1];
				Иначе
					Подсказка = ВернутьСтр("ru = 'но есть ""%1"" и другие варианты(%2)'");
			...
			ДобавитьОшибкуПроверкиАдресаПоКлассификатору(ОшибкиПроверки, "Подчиненный", 
				СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(ВернутьСтр("ru = '""%1"" отсутствует в адресном классификаторе'"), ОписаниеЗданияПоСоответствиюФИАС(ЗданияИПомещения)) + Подсказка);
		КонецЕсли;
...
Функция СформироватьОписаниеЗдания(НазваниеВладения, НомерВладения, НомерКорпуса, НазваниеСтроения, НомерСтроения, ДополнительныеПараметры = Неопределено)
	...
	Если ЗначениеЗаполнено(НомерКорпуса) Тогда
		КорпусИСтроение = ", "+ ВернутьСтр("ru = 'корпус'") + " " + НомерКорпуса;
	КонецЕсли;
...
Функция ОписаниеЗданийФИАССодержитДанные(Описание, ЗданияИПомещения, НеточныеСовпадения = Неопределено)
	...
	Если ПустаяСтрока(НомерСтроенияЗдания) И ЗданияИПомещения["Литер"] <> Неопределено Тогда
		НомерСтроенияЗданияя = ВРег(СокрЛП(ЗданияИПомещения["Литер"].Значение));
	КонецЕсли;
...
// Заполнить коды адреса
//
// Параметры:
//  Адрес - Структура - Структура содержащая поля адреса
//
Процедура ЗаполнитьКодыАдреса(Адрес) Экспорт
	...
	Иначе
		...
		|	И АдресныйОбъектОкруг.КодСубъектаРФ = АдресныеОбъектыРайон.КодСубъектаРФ
		|	И АдресныйОбъектОкруг.Уровень = 2
		|	И АдресныйОбъектОкруг.КодРайона = 0
		|	И АдресныйОбъектОкруг.КодГорода = 0
		|	И АдресныйОбъектОкруг.КодВнутригородскогоРайона = 0
		|	И АдресныйОбъектОкруг.КодНаселенногоПункта = 0
		|	И АдресныйОбъектОкруг.КодУлицы = 0
		|	И АдресныйОбъектОкруг.КодДополнительногоЭлемента = 0
		|	И АдресныйОбъектОкруг.КодПодчиненногоЭлемента = 0";
		сОкруг = "АдресныеОбъектыОкруг.КодОкруга";
		ТекстДляВыбрать = "АдресныйОбъектОкруг.КодКЛАДР КАК КодКЛАДРОкруг, ";
	...
	Если ПустаяСтрока(Адрес.ВнутригородскойРайон) Тогда
		сВнутригородскойРайон = "0";
		ТекстДляВыбрать = ТекстДляВыбрать + "НЕОПРЕДЕЛЕНО КАК КодКЛАДРВнутригородскойРайон, ";
	Иначе
		ТекстСоединения = ТекстСоединения + Символы.ПС + " ЛЕВОЕ СОЕДИНЕНИЕ РегистрСведений.АдресныеОбъекты КАК АдресныеОбъектыВнутригородскойРайон
		|	ПО АдресныеОбъектыВнутригородскойРайон.Наименование = """ + Адрес.ВнутригородскойРайон + """
	...
	Если РезультатЗапроса.Следующий() Тогда
		Адрес.КодыКЛАДР.Регион = РезультатЗапроса.КодКЛАДРРегион;
		Адрес.КодыКЛАДР.Округ = РезультатЗапроса.КодКЛАДРОкруг;
		Адрес.КодыКЛАДР.Город = РезультатЗапроса.КодКЛАДРГород;
		Адрес.КодыКЛАДР.ВнутригородскойРайон = РезультатЗапроса.КодКЛАДРВнутригородскойРайон;
		Адрес.КодыКЛАДР.НаселенныйПункт = РезультатЗапроса.КодКЛАДРНаселенныйПункт;
		Адрес.КодыКЛАДР.Улица = РезультатЗапроса.КодКЛАДРУлица;
		Адрес.ДополнительныеКоды.ОКТМО = РезультатЗапроса.ОКТМО;
		Адрес.ДополнительныеКоды.ОКАТО = РезультатЗапроса.OKATO;
		Адрес.ДополнительныеКоды.КодИФНСФЛ = РезультатЗапроса.КодИФНСФЛ;
		Адрес.ДополнительныеКоды.КодИФНСЮЛ = РезультатЗапроса.КодИФНСЮЛ;
		Адрес.ДополнительныеКоды.КодУчасткаИФНСФЛ = РезультатЗапроса.КодУчасткаИФНСФЛ;
		Адрес.ДополнительныеКоды.КодУчасткаИФНСЮЛ = РезультатЗапроса.КодУчасткаИФНСЮЛ;
	КонецЕсли;
...
//
Процедура ПроверитьНачальноеЗаполнение() Экспорт
...
Процедура ЗагрузитьКлассификаторАдресов(КодыСубъектовРФ, ОписаниеФайлов, ОповещатьОПрогрессе = Истина) Экспорт
	...
	Для Каждого СубъектРФ Из КодыСубъектовРФ Цикл
		...
		Если ОповещатьОПрогрессе Тогда
			ДлительныеОперации.СообщитьПрогресс(, СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(ВернутьСтр("ru = 'Загрузка региона ""%1 - %2"" (осталось %3) ...'"),
...
Процедура УдалитьВременныйФайл(ПолноеИмяФайла) Экспорт
	...
	Исключение
		ЗаписьЖурналаРегистрации(СобытиеЖурналаРегистрации(), УровеньЖурналаРегистрации.Предупреждение,
			,, СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(ВернутьСтр("ru = 'Не удалось удалить временный файл
			|%1 по причине: %2'"), ПолноеИмяФайла, ПодробноеПредставлениеОшибки(ИнформацияОбОшибке())));
	КонецПопытки
...
Процедура ПриЗаполненииРазрешенийНаДоступКВнешнимРесурсам(ЗапросыРазрешений) Экспорт
	...
	Описание = ВернутьСтр("ru = 'Загрузка обновлений адресного классификатора.'");
...
Процедура ПриЗаполненииСпискаТекущихДел(ТекущиеДела) Экспорт
	...
	Для Каждого Раздел Из Разделы Цикл
			...
			Дело.Представление  = ВернутьСтр("ru = 'Адресный классификатор устарел'");
			Дело.Количество     = 0;
			Дело.Подсказка      = ВернутьСтр("ru = 'Автоподбор и проверка корректности адресов временно недоступны.'");
	...
	Подсказка = Подсказка + Символы.ПС + ВернутьСтр("ru = 'Необходимо проверить наличие обновлений.'");
	Для Каждого Раздел Из Разделы Цикл
		Дело = ТекущиеДела.Добавить();
...
Процедура ПеренестиДанныеУстаревшегоКлассификатор(КодыСубъектовРФ)
	...
	Для Каждого СубъектРФ Из КодыСубъектовРФ Цикл
		...
		НомерПоПорядку = НомерПоПорядку + 1;
		ДлительныеОперации.СообщитьПрогресс(,СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(ВернутьСтр("ru = 'Актуализация сведений о регионе ""%1 - %2"" (осталось %3) ...'"),
			СубъектРФ, АдресныйКлассификатор.НаименованиеРегионаПоКоду(СубъектРФ),
			Формат(ВсегоСубъектов - НомерПоПорядку, "ЧН=")));
...
Функция ОписаниеДиапазонаКЛАДР(Описание, ТаблицыДанных)
	...
	Если ВариантыТипов.Количество() = 0 Тогда
		ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(ВернутьСтр("ru = 'Ошибка поиска типа диапазона для %1'"), Описание);
	КонецЕсли;
...
Функция СведенияОЗагрузкеСубъектовРФ() Экспорт
	...
	Запрос = Новый Запрос("
		...
		|
		|ВЫБРАТЬ 
		|	ВсеДоступныеРегионы.Наименование + "" "" + ВсеДоступныеРегионы.Сокращение КАК Представление,
		|	ВсеДоступныеРегионы.КодСубъектаРФ                                         КАК КодСубъектаРФ,
		|	ИдентификаторыАдресныхОбъектов.Идентификатор                              КАК Идентификатор,
		|	ВЫБОР 
		|		КОГДА АдресныеОбъекты.КодСубъектаРФ ЕСТЬ NULL ТОГДА ЛОЖЬ
		|		ИНАЧЕ ИСТИНА
		|	КОНЕЦ КАК Загружено,
		...
		|) КАК АдресныеОбъекты
		|ПО
		|	АдресныеОбъекты.КодСубъектаРФ = ВсеДоступныеРегионы.КодСубъектаРФ
		|
		|ЛЕВОЕ СОЕДИНЕНИЕ
		|	РегистрСведений.АдресныеОбъекты КАК ИдентификаторыАдресныхОбъектов
		|ПО
		|	ИдентификаторыАдресныхОбъектов.КодСубъектаРФ                = ВсеДоступныеРегионы.КодСубъектаРФ
		...
		|
		|ЛЕВОЕ СОЕДИНЕНИЕ
		|	РегистрСведений.ЗагруженныеВерсииАдресныхСведений КАК Версии
		|ПО
		|	Версии.КодСубъектаРФ = ВсеДоступныеРегионы.КодСубъектаРФ
		|
		|УПОРЯДОЧИТЬ ПО
		|	ВсеДоступныеРегионы.КодСубъектаРФ,
		|	ВсеДоступныеРегионы.Наименование + "" "" + ВсеДоступныеРегионы.Сокращение
		|");
...
Функция ПредставлениеРегиона(Наименование, Сокращение)
	Если Сокращение = ВернутьСтр("ru = 'Респ'") Тогда
		Возврат ВернутьСтр("ru = 'Республика'") + " " + Наименование;
	ИначеЕсли Сокращение = ВернутьСтр("ru = 'обл'") Тогда
		Возврат Наименование + " " + ВернутьСтр("ru = 'область'");
	ИначеЕсли Сокращение = ВернутьСтр("ru = 'г'") Тогда
		Возврат ВернутьСтр("ru = 'г.'") + " " + Наименование;
	КонецЕсли;
...
Процедура ИзвлечьСписокДомов(СтрокаСоСпискомДомов, СтрокаПоиска, ВариантыДомов, ТочноеСовпадение)
	...
	Для Каждого Интервал Из Интервалы Цикл
		...
		Для НомерВладения = Интервал.Начало По Интервал.Конец Цикл
			Если ТочноеСовпадение И СтрСравнить(ОписаниеДома.Дом, СтрокаПоиска) <> 0 Тогда
				Продолжить;
			КонецЕсли;
...
Процедура ДобавитьИнформациюОДоме(ВариантыДомов, ОписаниеДома, НаименованиеЗданийИСтроений, СтрокаПоиска = "")
	...
	Если ЗначениеЗаполнено(ОписаниеДома.Корпус) Тогда
		ТекстПроДом = ТекстПроДом + ", "+ ВернутьСтр("ru = 'корпус'") + " " + ОписаниеДома.Корпус;
...
Функция НаименованиеВладенийИСтроений()
	...
	Результат.Владения.Вставить(0, ВернутьСтр("ru = 'Дом'"));
	Результат.Строения.Вставить(0, ВернутьСтр("ru = 'Строение'"));
	...
	Для Каждого КодСубъекта Из КодыСубъектовРФ Цикл
		...
		Если Не ЗагруженныйФайл.Статус Тогда
			...
			ВызватьИсключение ЗагруженныйФайл.СообщениеОбОшибке + Символы.ПС
				+ ВернутьСтр("ru = 'Возможные причины:
...
Процедура ОчиститьКлассификаторАдресов(КодыСубъектовРФ)
	...
	Для Каждого СубъектРФ Из КодыСубъектовРФ Цикл
		...
		НомерПоПорядку = НомерПоПорядку + 1;
		ДлительныеОперации.СообщитьПрогресс(, СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(ВернутьСтр("ru = 'Очистка региона ""%1 - %2"" (осталось %3) ...'"),
			СубъектРФ, АдресныйКлассификатор.НаименованиеРегионаПоКоду(СубъектРФ),
			Формат(ВсегоСубъектов - НомерПоПорядку, "ЧН=")));
...
Функция РазрешенияБезопасностиОбновления() Экспорт
	...
	Разрешения.Добавить( РаботаВБезопасномРежиме.РазрешениеНаИспользованиеИнтернетРесурса(
		ВРег(Адрес.Схема), Адрес.Хост, Адрес.Порт, ВернутьСтр("ru = 'Проверка обновления адресного классификатора.'")));
	...
	Разрешения.Добавить( РаботаВБезопасномРежиме.РазрешениеНаИспользованиеИнтернетРесурса(
		ВРег(Адрес.Схема), Адрес.Хост, Адрес.Порт, ВернутьСтр("ru = 'Загрузка данных адресного классификатора.'")));
...
Функция ОписаниеПоследнейЗагрузки()
	...
	Если ДатаПоследнейЗагрузки = '00000000' Тогда
		...
		Результат.Представление        = ВернутьСтр("ru = 'Адресный классификатор еще не загружался.'");
		Возврат Результат;
	КонецЕсли;
	...
	Если ДнейРазницы = 0 Тогда
		Представление = ВернутьСтр("ru = 'Адресный классификатор был загружен сегодня.'");
		
	ИначеЕсли ДнейРазницы = 1 Тогда
		Представление = ВернутьСтр("ru = 'Адресный классификатор был загружен вчера.'");
			
	ИначеЕсли ДнейРазницы = 2 Тогда
		Представление = ВернутьСтр("ru = 'Адресный классификатор был загружен позавчера.'");
			
	Иначе
		Представление = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(ВернутьСтр("ru = 'Адресный классификатор был загружен %1 назад.'"),
...
Функция ФайлЗагрузкиСубъектаРФ(КодСубъектаРФ, КаталогФайлов, КлючВида, ИмяОписывающегоУзла)
	...
	Если ТипXDTOЗаписи = Неопределено Тогда
		ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(ВернутьСтр("ru = 'В файле ""%1"" не найден список записей в типе  %2'"), Результат.ПолноеИмя, Результат.ТипXDTO);
	КонецЕсли;
	...
	Иначе
		ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(ВернутьСтр("ru = 'В файле ""%1"" не найден корневой узел %2'"), Результат.ПолноеИмя, ИмяОписывающегоУзла);
	КонецЕсли;
