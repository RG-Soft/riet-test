...
Функция НастройкаОтборовНаУзле(ВерсияКорреспондента, ИмяФормы, ИдентификаторНастройки) Экспорт
	...
	//СтруктураНастроек.Вставить("ВерсияФорматаОбмена",             "1.0.beta");
...
// 
Функция НастройкаОтборовНаУзлеБазыКорреспондента(ВерсияКорреспондента, ИмяФормы, ФормаНастройкиУзла, ВариантНастройкиОбмена) Экспорт
...
//
Функция КраткаяИнформацияПоОбмену(ВариантНастройкиОбмена) Экспорт
	
	ПоясняющийТекст = "Позволяет выполнять отправку данных НСИ в ERM";
	//Если ИдентификаторНастройки = "ТолькоОтправка" Тогда
	//	ПоясняющийТекст = НСтр("ru = 'Позволяет отправлять данные в любые программы.
	//	|Отправляются следующие типы данных:
	//	| - справочники Клиенты, Банковские счета и т.п'");
	//ИначеЕсли ИдентификаторНастройки = "ТолькоПолучение" Тогда
	//	ПоясняющийТекст = НСтр("ru = 'Позволяет получать данные из программы 1С:Библиотека стандартных подсистем.
	...
	//	ПоясняющийТекст = НСтр("ru = 'Позволяет синхронизировать данные между двумя однотипными программами 1С:Библиотека стандартных подсистем.
...
Функция ПодробнаяИнформацияПоОбмену(ВариантНастройкиОбмена) Экспорт
	Возврат "ПланОбмена.rgsОбменДокументамиLogelcoERM.Форма.ПодробнаяИнформация";
КонецФункции
...
Процедура НастроитьИнтерактивнуюВыгрузку(Получатель, Параметры) Экспорт
	...
	//
	//Параметры.ВариантДополнительно.ИмяФормыОтбора        = "ПланОбмена._ДемоСинхронизацияДанныхЧерезУниверсальныйФормат.Форма.ФормаНастройкиУзла";
...
Процедура ОпределитьНастройки(Настройки, ИдентификаторНастройки) Экспорт
	...
	Если ИдентификаторНастройки = "ТолькоОтправка" Тогда
		...
		Настройки.Вставить("ЗаголовокПомощникаСозданияОбмена",               НСтр("ru = 'Отправка данных (настройка)'"));
		Настройки.Вставить("ЗаголовокУзлаПланаОбмена",                       НСтр("ru = 'Отправка данных'"));
	...
	ИначеЕсли ИдентификаторНастройки = "ТолькоПолучение" Тогда
		...
		Настройки.Вставить("ЗаголовокПомощникаСозданияОбмена",               НСтр("ru = 'Получение данных (настройка)'"));
		Настройки.Вставить("ЗаголовокУзлаПланаОбмена",                       НСтр("ru = 'Получение данных'"));
	...
	ИначеЕсли ИдентификаторНастройки = "Двухсторонний" Тогда
		...
		Настройки.Вставить("ЗаголовокПомощникаСозданияОбмена",               НСтр("ru = 'Синхронизация данных (настройка)'"));
		Настройки.Вставить("ЗаголовокУзлаПланаОбмена",                       НСтр("ru = 'Синхронизация данных'"));
		
	Иначе
		Настройки.Вставить("НаименованиеКонфигурацииКорреспондента", НСтр("ru = 'Через универсальный формат обмена Logelco--ERM'"));
	КонецЕсли;
...
Функция ПолучитьУзелОбменаСERM() Экспорт
	...
	|ГДЕ
	|	rgsОбменСERM.Ссылка <> &ЭтотУзел";
	
	Запрос.УстановитьПараметр("ЭтотУзел", ПланыОбмена.rgsОбменДокументамиLogelcoERM.ЭтотУзел());
...
Процедура ПолучитьВерсииФорматаОбмена(ВерсииФормата) Экспорт
	ВерсииФормата.Вставить("1.0", rgsМенеджерОбменаДокументамиLogelcoERM);
КонецПроцедуры
...
#Область ПроцедурыДляСовместимостиСоСтарымФункционалом
...
Функция ЗаголовокКомандыДляСозданияНовогоОбменаДанными() Экспорт
	...
	Возврат НСтр("ru = 'Создать обмен с конфигурацией ""ERM""'");
