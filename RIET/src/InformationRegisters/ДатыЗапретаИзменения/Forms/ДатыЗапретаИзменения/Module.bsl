...
Процедура УстановкаДатыЗапретаПриИзменении(Элемент)
	...
	ИначеЕсли (ТекущаяУстановкаДатыЗапрета = "ПоПользователям" Или ТекущаяУстановкаДатыЗапрета = "ДляВсехПользователей")
	        И ВыбранноеЗначение = "НетЗапрета" Тогда
		
		ТекстВопроса = НСтр("ru = 'Отключить все установленные даты запрета?'")
...
Процедура ПользователиПолноеПредставлениеОбработкаВыбора(Элемент, ВыбранноеЗначение, СтандартнаяОбработка)
	...
	Иначе
		ПоказатьПредупреждение(, 
			СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru = '""%1"" уже есть в списке пользователей.'"), ВыбранноеЗначение));
	КонецЕсли;
		...
		Если ДатаЗапретаУстановлена(ТекущиеДанные, ТекущийПользователь)
		 Или ЭлементыРаздела.Количество() > 0 Тогда
			...
			ПоказатьВопрос(
				...
				ТекстВопроса, РежимДиалогаВопрос.ДаНет);
		Иначе
			ТекстСообщения = НСтр("ru = 'Для удаления даты запрета по конкретному объекту необходимо выбрать интересующий объект в одном из разделов.'");
...
Процедура ДатыЗапретаПриНачалеРедактирования(Элемент, НоваяСтрока, Копирование)
	...
	Если НоваяСтрока Тогда
		...
		КонецЕсли;
		Если ЭтоВсеПользователи(ТекущийПользователь)
...
Процедура УстановитьУсловноеОформление()
	...
	Элемент.Оформление.УстановитьЗначениеПараметра("Формат",
		СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку("ДП='%1'", НСтр("ru = 'Без запрета'")));
			...
			Для Каждого ОписаниеРаздела Из ДатыЗапрета.ПолучитьЭлементы() Цикл
				Если ДатаЗапретаУстановлена(ОписаниеРаздела, ТекущийПользователь)
				 Или ОписаниеРаздела.ПолучитьЭлементы().Количество() > 0 Тогда
					...
					ОписаниеРаздела.ОписаниеДатыЗапретаПредставление =
						ПредставлениеОписанияДатыЗапрета(ОписаниеРаздела);
				КонецЕсли;
			КонецЦикла;
		Иначе
...
Процедура ПрочитатьДанныеПользователя(Контекст, ТекущийСпособУказания = Неопределено, Данные = Неопределено)
	...
	ИначеЕсли Контекст.УстановкаДатыЗапрета = "ПоПользователям" Тогда
		...
		Если НайденныеСтроки.Количество() > 0 Тогда
			Контекст.Элементы.ТекущийПользовательПредставление.Заголовок = 
				СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru = 'Настройка для ""%1"":'"), 
	...
	Если Контекст.СпособУказанияДатыЗапрета = "ОбщаяДата" Тогда
		Контекст.Элементы.СпособыУказания.ТекущаяСтраница =	Контекст.Элементы.СпособУказанияОбщаяДата;
		// Для прижатия вверх группы "Расширенные возможности"
		Контекст.Элементы.ДатыЗапрета.РастягиватьПоВертикали = Ложь;
		...
		ДатыЗапретаИзмененияСлужебныйКлиентСервер.ОбщаяДатаЗапретаСОписаниемПриИзменении(Контекст, Ложь);
	...
	Контекст.Элементы.СпособыУказания.ТекущаяСтраница =	Контекст.Элементы.СпособУказанияПоРазделамОбъектам;
	Контекст.Элементы.ДатыЗапрета.РастягиватьПоВертикали = Истина;
	...
	Если Контекст.ПоказыватьРазделыТекущегоПользователя Тогда
		Если Контекст.ВсеРазделыБезОбъектов Тогда
			...
			Контекст.Элементы.ДатыЗапретаПолноеПредставление.Заголовок = НСтр("ru = 'Раздел'");
			Контекст.Элементы.ДатыЗапрета.Отображение = ОтображениеТаблицы.Список;
			
		Иначе
			Контекст.Элементы.ДатыЗапретаПолноеПредставление.Заголовок = НСтр("ru = 'Раздел, объект'");
...
Процедура ПрочитатьДанныеПользователяНаСервере(Знач Контекст, ДанныеПользователя, ПоказыватьРазделыТекущегоПользователя)
	...
	Иначе
		ПрочитанныеДатыЗапрета = ПрочитатьДанныеПользователяБезРазделов(
			Контекст.Пользователь, Контекст.ЕдинственныйРаздел);
	КонецЕсли;
...
Процедура УстановитьВидимость()
	...
	Если Параметры.ДатыЗапретаЗагрузкиДанных Тогда
		Если УстановкаДатыЗапрета = "НетЗапрета" Тогда
			РасширеннаяПодсказка = НСтр("ru = 'Разрешена загрузка из других программ любых данных прошлых периодов.'");
		ИначеЕсли УстановкаДатыЗапрета = "ДляВсехПользователей" Тогда
	...
	Иначе
		Если УстановкаДатыЗапрета = "НетЗапрета" Тогда
			РасширеннаяПодсказка = НСтр("ru = 'Разрешены ввод и редактирование любых данных прошлых периодов.'");
		ИначеЕсли УстановкаДатыЗапрета = "ДляВсехПользователей" Тогда
...
Процедура РедактироватьДатуЗапретаВФормеЗавершение(ВыделенныеСтроки) Экспорт
	...
	Если ВыделенныеСтроки.Количество() = 1 Тогда
		Если СпособУказанияДатыЗапрета = "ПоОбъектам" Тогда
			...
			Иначе
				ПараметрыФормы.Вставить("РазделПредставление", "");
				ПараметрыФормы.Вставить("Объект", Элементы.ДатыЗапрета.ТекущиеДанные.Объект);
			КонецЕсли;
		Иначе
			Если Элементы.ДатыЗапрета.ТекущиеДанные.ЭтоРаздел Тогда
				ПараметрыФормы.Вставить("РазделПредставление",
				...
				РазделБезОбъектов = Элементы.ДатыЗапрета.ТекущиеДанные.Раздел = РазделПустаяСсылка
					Или РазделыБезОбъектов.Найти(Элементы.ДатыЗапрета.ТекущиеДанные.Раздел) <> Неопределено;
				
			Иначе
				ПараметрыФормы.Вставить("РазделПредставление",
					Элементы.ДатыЗапрета.ТекущиеДанные.ПолучитьРодителя().Представление);
				
				ПараметрыФормы.Вставить("ОбъектПредставление", Элементы.ДатыЗапрета.ТекущиеДанные.Объект);
			КонецЕсли;
		КонецЕсли;
	Иначе
		ПараметрыФормы.Вставить("РазделПредставление", "");
	...
	КонецЕсли;
	ПараметрыФормы.Вставить("ОписаниеДатыЗапрета", Элементы.ДатыЗапрета.ТекущиеДанные.ОписаниеДатыЗапрета);
	ПараметрыФормы.Вставить("КоличествоДнейРазрешения", Элементы.ДатыЗапрета.ТекущиеДанные.КоличествоДнейРазрешения);
	ПараметрыФормы.Вставить("ДатаЗапрета", Элементы.ДатыЗапрета.ТекущиеДанные.ДатаЗапрета);
...
Процедура УстановитьКоманднуюПанельДатЗапрета(Контекст)
	...
	Если ЭтоВсеПользователи(Контекст.ТекущийПользователь) Тогда
