...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если Параметры.ТолькоОткрытие Тогда
		Заголовок = ВернутьСтр("ru = 'Открытие макета печатной формы'");
	КонецЕсли;
	...
	Если Не ОбщегоНазначенияКлиентСервер.ЭтоВебКлиент() И ТипМакета = "MXL" Тогда
		Элементы.НадписьЗавершениеИзмененияНеВебКлиент.Заголовок = ВернутьСтр(
...
Процедура УстановитьНазваниеПрограммыДляОткрытияМакета()
	...
	Если ТипФайла = "mxl" Тогда
		НазваниеПрограммыДляОткрытияМакета = ВернутьСтр("ru = '1С:Предприятие - Работа с файлами'");
		АдресПрограммыДляОткрытияМакета = "http://v8.1c.ru/metod/fileworkshop.htm";
	ИначеЕсли ТипФайла = "doc" Тогда
		НазваниеПрограммыДляОткрытияМакета = ВернутьСтр("ru = 'Microsoft Word'");
		АдресПрограммыДляОткрытияМакета = "http://office.microsoft.com/ru-ru/word";
	ИначеЕсли ТипФайла = "odt" Тогда
		НазваниеПрограммыДляОткрытияМакета = ВернутьСтр("ru = 'OpenOffice Writer'");
	...
	СведенияДляЗаполнения.Вставить("ОписаниеДействия", ?(Параметры.ТолькоОткрытие, ВернутьСтр("ru = 'открытия'"), ВернутьСтр("ru = 'внесения изменений'")));
