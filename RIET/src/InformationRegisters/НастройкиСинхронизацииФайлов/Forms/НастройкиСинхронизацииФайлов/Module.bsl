...
Процедура СинхронизацияЭлемента(Команда)
	...
	Если Не СтрокаДерева.ВозможностьДетализации Тогда
		ТекстСообщения = ВернутьСтр("ru = 'Добавление настройки возможно только для иерархических справочников'");
	...
	ПараметрыФормыВыбора.Вставить("ЗаголовокФормыПодбора", ВернутьСтр("ru = 'Подбор элементов настроек'"));
	...
	Для Каждого Тип Из ТаблицаТипов Цикл
		...
		Если ВсеСправочники.СодержитТип(Тип.ВладелецФайла) Тогда
			Если УзелСправочники = Неопределено Тогда
				...
				УзелСправочники.СинонимНаименованияОбъекта = ВернутьСтр("ru = 'Справочники'");
			КонецЕсли;
			...
			Если ДетализированныеНастройки.Количество() > 1 Тогда
				...
				Для Каждого Настройка Из ДетализированныеНастройки Цикл
					...
					ДетализированнаяНастройка.ПравилоОтбора =
						?(ЕстьПравилоОтбора, ВернутьСтр("ru = 'Выбранные файлы'"), ВернутьСтр("ru = 'Все файлы'"));
		...
		ИначеЕсли ВсеДокументы.СодержитТип(Тип.ВладелецФайла) Тогда
			Если УзелДокументы = НеОпределено Тогда
				...
				УзелДокументы.СинонимНаименованияОбъекта = ВернутьСтр("ru = 'Документы'");
			КонецЕсли;
		...
		ИначеЕсли БизнесПроцессы.ТипВсеСсылки().СодержитТип(Тип.ВладелецФайла) Тогда
			Если УзелБизнесПроцессы = Неопределено Тогда
				...
				УзелБизнесПроцессы.СинонимНаименованияОбъекта = ВернутьСтр("ru = 'Бизнес-процессы'");
			КонецЕсли;
		...
		Если Не НастройкаИмеетГлобальныеПравила Тогда
			...
			Если НайденныеНастройки.Количество() > 0 Тогда
				...
				НоваяСтрокаТаблицы.ПравилоОтбора =    ?(ЕстьПравилоОтбора, ВернутьСтр("ru = 'Выбранные файлы'"), ВернутьСтр("ru = 'Все файлы'"));
			Иначе
				НоваяСтрокаТаблицы.Синхронизировать = Перечисления.ВариантыОчисткиФайлов.НеОчищать;
				НоваяСтрокаТаблицы.ПравилоОтбора = ВернутьСтр("ru = 'Все файлы'");
			КонецЕсли;
		КонецЕсли;
	КонецЦикла;
...
Процедура СоздатьСинхронизациюФайлов(Команда)
	...
	СтрокаДерева = Элементы.ДеревоОбъектовМетаданных.ТекущиеДанные;
	Если Не СтрокаДерева.ВозможностьДетализации Тогда
		ТекстСообщения = ВернутьСтр("ru = 'Добавление настройки возможно только для справочников'");
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(ТекстСообщения);
		Возврат;
	КонецЕсли;
...
Процедура ЗапуститьРегламентноеЗадание()
	...
	Иначе
		НаименованиеФоновогоЗадания = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(ВернутьСтр("ru = 'Запуск вручную: %1'"), РегламентноеЗаданиеМетаданные.Синоним);
