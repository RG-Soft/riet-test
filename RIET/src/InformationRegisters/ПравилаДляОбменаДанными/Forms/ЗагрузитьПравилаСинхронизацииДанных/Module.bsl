...
Процедура ПриОткрытии(Отказ)
	...
	ШаблонПодсказки = ВернутьСтр("ru = 'Комплект правил можно скачать с %1
		|или найти в %2'");
	
	ШаблонКаталогОбновлений = ВернутьСтр("ru = 'каталоге поставки программы ""%1""'");
	ШаблонКаталогОбновлений = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(ШаблонКаталогОбновлений, ИмяПрограммы);
	
	ШаблонПользовательскийСайт = ВернутьСтр("ru = 'сайта поддержки пользователей системы ""1С:Предприятие 8""'");
	...
	Если Не ПустаяСтрока(РасположениеКомплектаПравил.ПутьКФайлуКомплектаПравилВКаталогеШаблонов) Тогда
		...
		ТекстПредложения = ВернутьСтр("ru = 'Для открытия каталога необходимо необходимо установить расширение работы с файлами.'");
	...
	Если РежимОтладки Тогда
		...
		Если РежимОтладкиВыгрузки Тогда
			...
			Если Не ЗначениеЗаполнено(ИмяФайла) Тогда
				...
				СтрокаСообщения = ВернутьСтр("ru = 'Не задано имя файла внешней обработки.'");
		...
		Если РежимПротоколированияОбменаДанными Тогда
			...
			Если Не ЗначениеЗаполнено(ИмяФайла) Тогда
				...
				СтрокаСообщения = ВернутьСтр("ru = 'Не задано имя файла протокола обмена.'");
...
Процедура ВнешняяОбработкаДляОтладкиВыгрузкиНачалоВыбора(Элемент, ДанныеВыбора, СтандартнаяОбработка)
	...
	НастройкиДиалога.Вставить("Фильтр", ВернутьСтр("ru = 'Внешняя обработка(*.epf)'") + "|*.epf" );
...
Процедура ФайлПротоколаОбменаНачалоВыбора(Элемент, ДанныеВыбора, СтандартнаяОбработка)
	...
	ИзмененыНастройкиРежимаОтладки = Истина;
	НастройкиДиалога = Новый Структура;
	НастройкиДиалога.Вставить("Фильтр", ВернутьСтр("ru = 'Текстовый документ(*.txt)'")+ "|*.txt" );
...
Процедура ЗагрузитьПравила(Команда)
	...
	ПараметрыДиалога.Вставить("Заголовок", ВернутьСтр("ru = 'Укажите архив с правилами обмена'"));
	ПараметрыДиалога.Вставить("Фильтр", ВернутьСтр("ru = 'Архивы ZIP (*.zip)'") + "|*.zip");
...
Процедура ВыгрузитьПравила(Команда)
	...
	Если ПустаяСтрока(ЧастиИмени.ИмяБезРасширения) Тогда
		ПолноеИмяФайла = ВернутьСтр("ru = 'Правила конвертации'");
	Иначе
	...
	ПараметрыДиалога.Вставить("Заголовок", ВернутьСтр("ru = 'Укажите в какой файл выгрузить правила'") );
...
Процедура ЗаписатьИЗакрыть(Команда)
	...
	Иначе
		Если ИсточникПравилКонвертации = ПредопределенноеЗначение("Перечисление.ИсточникиПравилДляОбменаДанными.МакетКонфигурации") Тогда
			...
			ОписаниеОшибки = ВернутьСтр("ru = 'Правила из файла не загружены. Закрытие приведет к использованию типовых правил конвертации.
			|Использовать типовые правила конвертации?'");
			
			Оповещение = Новый ОписаниеОповещения("ЗакрытьФормуЗагрузкиПравил", ЭтотОбъект);
			
			Кнопки = Новый СписокЗначений;
			Кнопки.Добавить("Использовать", ВернутьСтр("ru = 'Использовать'"));
...
Процедура ЗагрузитьПравилаЗавершение(Знач РезультатПомещенияФайлов, Знач ДополнительныеПараметры) Экспорт
	...
	Если ПустаяСтрока(ТекстОшибки) И ПустаяСтрока(АдресПомещенногоФайла) Тогда
		ТекстОшибки = ВернутьСтр("ru = 'Ошибка передачи файла настроек синхронизации данных на сервер'");
	КонецЕсли;
	...
	Если НРег(ЧастиИмени.Расширение) <> ".zip" Тогда
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(ВернутьСтр("ru = 'Некорректный формат файла комплекта правил. Ожидается zip архив, содержащий три файла:
...
Процедура ВыполнитьЗагрузкуПравил(Знач АдресПомещенногоФайла, Знач ИмяФайла, ОписаниеОшибки = Неопределено)
	...
	Состояние(ВернутьСтр("ru = 'Выполняется загрузка правил в информационную базу...'"));
	...
	Если ТипЗнч(ОписаниеОшибки) <> Тип("Булево") И ОписаниеОшибки <> Неопределено Тогда
		...
		Если ОписаниеОшибки.ВидОшибки = "НекорректнаяКонфигурация" Тогда
			Кнопки.Добавить("Отмена", ВернутьСтр("ru = 'Закрыть'"));
		Иначе
		...
		ПараметрыФормы.ПредлагатьБольшеНеЗадаватьЭтотВопрос = Ложь;
		Если ОписаниеОшибки.ВидОшибки = "НекорректнаяКонфигурация" Тогда
			ПараметрыФормы.Заголовок = ВернутьСтр("ru = 'Правила не могут быть загружены'");
		Иначе
			ПараметрыФормы.Заголовок = ВернутьСтр("ru = 'Синхронизация данных может работать некорректно'");
		КонецЕсли;
	...
	ИначеЕсли Отказ Тогда
		ТекстОшибки = ВернутьСтр("ru = 'В процессе загрузки правил были обнаружены ошибки.
	...
	Иначе
		ПоказатьОповещениеПользователя(,, ВернутьСтр("ru = 'Правила успешно загружены в информационную базу.'"));
		Закрыть();
	КонецЕсли;
...
Функция ПолучитьАдресВременногоХранилищаАрхиваПравилНаСервере()
	...
	Если Результат.Пустой() Тогда
		...
		НСтрока = ВернутьСтр("ru = 'Не удалось получить правила обмена.'");
...
Процедура ОбновитьИнформациюОПравилах()
	...
	Если ИсточникПравил = "ЗагруженныеИзФайла" Тогда
		ИнформацияОбИспользовании = ВернутьСтр("ru = 'Используются правила загруженные из файла.'");
	Иначе
		ИнформацияОбИспользовании = ВернутьСтр("ru = 'Используются типовые правила из состава конфигурации.'");
	КонецЕсли;
