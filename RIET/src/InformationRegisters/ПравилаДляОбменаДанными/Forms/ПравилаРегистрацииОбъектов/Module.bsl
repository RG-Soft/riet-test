...
Процедура ЗагрузитьПравила(Команда)
	...
	ПараметрыДиалога.Вставить("Заголовок", ВернутьСтр("ru = 'Укажите, из какого файла загрузить правила'"));
	ПараметрыДиалога.Вставить("Фильтр",
		  ВернутьСтр("ru = 'Файлы правил регистрации (*.xml)'") + "|*.xml|"
		+ ВернутьСтр("ru = 'Архивы ZIP (*.zip)'")   + "|*.zip");
...
Процедура ВыгрузитьПравила(Команда)
	...
	ФильтрИмени = ВернутьСтр("ru = 'Файлы правил (*.xml)'") + "|*.xml";
	...
	Если ПустаяСтрока(ЧастиИмени.ИмяБезРасширения) Тогда
		ПолноеИмяФайла = ВернутьСтр("ru = 'Правила регистрации'");
	Иначе
...
Процедура ЗагрузитьПравилаЗавершение(Знач РезультатПомещенияФайлов, Знач ДополнительныеПараметры) Экспорт
	...
	Если ПустаяСтрока(ТекстОшибки) И ПустаяСтрока(АдресПомещенногоФайла) Тогда
		ТекстОшибки = ВернутьСтр("ru = 'Ошибка передачи файла на сервер'");
	КонецЕсли;
