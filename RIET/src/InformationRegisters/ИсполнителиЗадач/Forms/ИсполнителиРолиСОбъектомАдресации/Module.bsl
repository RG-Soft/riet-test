...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Заголовок = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(ВернутьСтр("ru = 'Исполнители роли ""%1""'"), Роль);
...
Процедура Подобрать(Команда)
	...
	Если РольДоступнаДляВнешнихПользователей И РольДоступнаДляПользователей Тогда
		...
		Выбор.Добавить("ВнешнийПользователь", ВернутьСтр("ru = 'Внешний пользователь'"));
		Выбор.Добавить("Пользователь", ВернутьСтр("ru = 'Пользователь'"));
		ОписаниеОповещения = Новый ОписаниеОповещения("ПослеВыбораТипаПользователя", ЭтотОбъект);
		Выбор.ПоказатьВыборЭлемента(ОписаниеОповещения, ВернутьСтр("ru = 'Выберите тип пользователя'"));
