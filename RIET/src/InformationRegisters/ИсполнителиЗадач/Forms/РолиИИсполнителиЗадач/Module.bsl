...
Процедура НазначитьИсполнителей(Команда)
	...
	Если Назначение = Неопределено Тогда
		ПоказатьПредупреждение(,ВернутьСтр("ru = 'Необходимо выбрать роль в списке.'"));
		Возврат;
	КонецЕсли;
...
Процедура ОбновитьДанныеЭлементов()
	...
	Для каждого СтрокаСписка Из СписокОбъект Цикл
		Если СтрокаСписка.Исполнители = 0 Тогда
			СтрокаСписка.ИсполнителиСтрока = ?(СтрокаСписка.ВнешняяРоль, ВернутьСтр("ru = 'заданы в другой базе'"), ВернутьСтр("ru = 'не заданы'"));
		...
		Иначе
			СтрокаСписка.ИсполнителиСтрока = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(ВернутьСтр("ru = '%1 чел'"), Строка(СтрокаСписка.Исполнители) );
			СтрокаСписка.Картинка = -1;
		КонецЕсли;
	КонецЦикла;
