...
Процедура ПередЗаписью(Отказ, Замещение)
	...
	Если ОбменДанными.Загрузка Тогда
		...
		Если ОбщегоНазначения.ЭтоПодчиненныйУзелРИБ() Тогда 
			...
			Для Каждого СтрТабл Из ЭтотОбъект Цикл
				...
				НаборДляРегистрацииОтветаВГлавныйУзел.Отбор.Узел             = СтрТабл.Узел;
				НаборДляРегистрацииОтветаВГлавныйУзел.Отбор.ОбъектМетаданных = СтрТабл.ОбъектМетаданных;
				НаборДляРегистрацииОтветаВГлавныйУзел.Отбор.Данные           = СтрТабл.Данные;
				НаборДляРегистрацииОтветаВГлавныйУзел.Отбор.Очередь          = СтрТабл.Очередь;
				НаборДляРегистрацииОтветаВГлавныйУзел.Отбор.КлючУникальности = СтрТабл.КлючУникальности;
	...
	Иначе
		Если Количество() > 0
			...
			Отказ = Истина;
			ТекстИсключения = ВернутьСтр("ru = 'Запись в РегистрСведений.ДанныеОбработанныеВЦентральномУзлеРИБ возможна только при отметке выполнения отложенного обработчика обновления информационной базы, который выполняется только в центральном узле.'");
