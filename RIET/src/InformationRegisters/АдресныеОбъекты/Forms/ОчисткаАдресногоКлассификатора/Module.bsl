...
Процедура ПередЗакрытием(Отказ, СтандартнаяОбработка)
	...
	Текст = ВернутьСтр("ru = 'Прервать очистку адресного классификатора?'");
...
Процедура ОбновитьИнтерфейсПоКоличествуОчищаемых()
	...
	// Страница загрузки
	ТекстОписанияОчистки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(ВернутьСтр("ru = 'Очищаются данные выбранных регионов (%1)'"), ВыбраноРегионовДляОчистки);
...
Процедура ОчиститьКлассификатор()
	...
	ТекстСостоянияОчистки = ВернутьСтр("ru = 'Очистка адресного классификатора ...'");
...
Процедура ЗапуститьФоновуюОчисткуНаСервере(Знач КодыРегионов)
	...
	Попытка
		РезультатЗапуска = ДлительныеОперации.ЗапуститьВыполнениеВФоне(
			...
			ПараметрыМетода,
			ВернутьСтр("ru = 'Очистка адресного классификатора'"));
	Исключение
...
Процедура Подключаемый_ОжиданиеДлительнойОперации()
	...
	ИначеЕсли Состояние.Завершено Тогда
		...
		ТекстОписанияОчистки = ВернутьСтр("ru = 'Адресный классификатор успешно очищен.'");
