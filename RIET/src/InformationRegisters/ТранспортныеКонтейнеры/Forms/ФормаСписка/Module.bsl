...
Процедура РегистрСведенийСписокПередНачаломДобавления(Элемент, Отказ, Копирование, Родитель, Группа)
	...
	Иначе
		ЭлементыСписка = Список.Отбор.Элементы;
	КонецЕсли;
	...
	Если ТранспортноеСообщение = Неопределено ИЛИ НЕ ЗначениеЗаполнено(ТранспортноеСообщение) Тогда
		ПоказатьПредупреждение(, ВернутьСтр("ru = 'Не установлен отбор по транспортным сообщениям!'"));
		Возврат;
	КонецЕсли;
	
	// подключаем расширение, если не подключено
	...
	Иначе
		ДобавитьТранспортныйКонтейнерПослеУстановкиРасширенияРаботыСФайлами(ТранспортноеСообщение)
	КонецЕсли;
	
КонецПроцедуры

&НаКлиенте
Процедура РегистрСведенийСписокПередНачаломДобавленияЗавершение(ДополнительныеПараметры) Экспорт
...
Процедура ДобавитьТранспортныйКонтейнерПослеУстановкиРасширенияРаботыСФайламиЗавершение(Результат, АдресКонтейнераВоВременномХранилище, ПолноеИмяФайлаФайла, ДополнительныеПараметры) Экспорт
	...
	ОбъектФайл = Новый Файл(ПолноеИмяФайлаФайла);
	...
	Если КоличествоТранспортныхКонтейнеров > 0 Тогда
		...
		Если КонтейнерСТемЖеИменемСуществует Тогда
			ДополнительныеПараметры = Новый Структура("КоличествоТранспортныхКонтейнеров, КонтейнерСТемЖеИменемСуществует, ТранспортноеСообщение, ОбъектФайл, АдресКонтейнераВоВременномХранилище", КоличествоТранспортныхКонтейнеров, КонтейнерСТемЖеИменемСуществует, ТранспортноеСообщение, ОбъектФайл, АдресКонтейнераВоВременномХранилище);
			...
			ПоказатьВопрос(ОписаниеОповещения, ВернутьСтр("ru = 'Загружаемый контейнер заменит существующий с тем же именем. Продолжить?'"), РежимДиалогаВопрос.ДаНет);
		Иначе
	...
	Иначе
		...
		КонтекстЭДОКлиент.ДобавитьТранспортныйКонтейнер(ТранспортноеСообщение, АдресКонтейнераВоВременномХранилище, ОбъектФайл.Имя, ОбъектФайл.Размер());
...
Процедура УдалитьПрочиеКонтейнерыТранспортногоСообщения(КоличествоТранспортныхКонтейнеров, КонтейнерСТемЖеИменемСуществует, ТранспортноеСообщение, ОбъектФайл, АдресКонтейнераВоВременномХранилище)
	...
	Если КоличествоТранспортныхКонтейнеров - ?(КонтейнерСТемЖеИменемСуществует, 1, 0) > 0 Тогда
		ДополнительныеПараметры = Новый Структура("ТранспортноеСообщение, АдресКонтейнераВоВременномХранилище, ОбъектФайл", ТранспортноеСообщение, АдресКонтейнераВоВременномХранилище, ОбъектФайл);
		...
		ПоказатьВопрос(ОписаниеОповещения, ВернутьСтр("ru = 'Удалить прочие контейнеры транспортного сообщения?'"), РежимДиалогаВопрос.ДаНетОтмена);
	Иначе
		// добавляем новый контейнер
		КонтекстЭДОКлиент.ДобавитьТранспортныйКонтейнер(ТранспортноеСообщение, АдресКонтейнераВоВременномХранилище, ОбъектФайл.Имя, ОбъектФайл.Размер());
