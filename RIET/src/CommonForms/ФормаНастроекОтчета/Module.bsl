...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если Не Параметры.Свойство("НастройкиОтчета", НастройкиОтчета) Тогда
		ВызватьИсключение ВернутьСтр("ru = 'Не передан служебный параметр ""НастройкиОтчета"".'");
	КонецЕсли;
	Если Не Параметры.Свойство("ВариантНаименование", ВариантНаименование) Тогда
		ВызватьИсключение ВернутьСтр("ru = 'Не передан служебный параметр ""ВариантНаименование"".'");
	КонецЕсли;
	...
	Если ИдентификаторТекущегоУзлаКД <> Неопределено Тогда
		...
		Если Не Параметры.Свойство("Заголовок", Заголовок) Тогда
			ВызватьИсключение ВернутьСтр("ru = 'Не передан служебный параметр ""Заголовок"".'");
		КонецЕсли;
		Если Не Параметры.Свойство("ТипТекущегоУзлаКД", ТипТекущегоУзлаКД) Тогда
			ВызватьИсключение ВернутьСтр("ru = 'Не передан служебный параметр ""ТипТекущегоУзлаКД"".'");
		КонецЕсли;
	Иначе
		Если Не ЗначениеЗаполнено(ВариантНаименование) Тогда
			ВариантНаименование = НастройкиОтчета.Наименование;
		КонецЕсли;
		Заголовок = ВернутьСтр("ru = 'Настройки отчета'") + " """ + ВариантНаименование + """";
	КонецЕсли;
	...
	Иначе
		Для Каждого ЭлементСпискаДубльВФорме Из СписокЗначенийВФорме Цикл
			Если ЭлементСпискаДубльВФорме.Значение = Значение И ЭлементСпискаДубльВФорме <> ЭлементСпискаВФорме Тогда
				Состояние(ВернутьСтр("ru = 'Обнаружены дублирующиеся записи. Редактирование отменено.'"));
...
Процедура Подключаемый_СписокСПодбором_ОбработкаВыбора(Элемент, РезультатВыбора, СтандартнаяОбработка)
	...
	Если Дополнение.Всего > 0 Тогда
		Если Дополнение.Всего = 1 Тогда
			ЗаголовокОповещения = ВернутьСтр("ru = 'Элемент добавлен в список'");
		Иначе
			ЗаголовокОповещения = ВернутьСтр("ru = 'Элементы добавлены в список'");
		КонецЕсли;
...
Процедура СтруктураВариантаДобавитьГруппировкуПослеВыбораПоля(ДоступноеПолеКД, ПараметрыВыполнения) Экспорт
	...
	Если ДоступноеПолеКД = "<>" Тогда
		...
		Представление = ВернутьСтр("ru = '<Детальные записи>'");
	Иначе
...
Процедура ДобавитьТаблицуИлиДиаграмму(ТипЭлемента)
	...
	Если СтрокаТаблицы.Тип = "Диаграмма" Тогда
		...
		СтрокаТаблицы.Представление = ВернутьСтр("ru = 'Диаграмма'");
		...
		ВложеннаяСтрока.Представление = ВернутьСтр("ru = 'Точки'");
		...
		ВложеннаяСтрока.ИдентификаторКД = УзелКД.ПолучитьИдентификаторПоОбъекту(ВложенныйЭлементКД);
		ВложеннаяСтрока.ИндексКартинки = -1;
		ВложеннаяСтрока.Представление = ВернутьСтр("ru = 'Серии'");
		
	ИначеЕсли СтрокаТаблицы.Тип = "Таблица" Тогда
		
		СтрокаТаблицы.Представление = ВернутьСтр("ru = 'Таблица'");
		...
		ВложеннаяСтрока.ИдентификаторКД = УзелКД.ПолучитьИдентификаторПоОбъекту(ВложенныйЭлементКД);
		ВложеннаяСтрока.ИндексКартинки = -1;
		ВложеннаяСтрока.Представление = ВернутьСтр("ru = 'Строки'");
		...
		ВложеннаяСтрока.ИдентификаторКД = УзелКД.ПолучитьИдентификаторПоОбъекту(ВложенныйЭлементКД);
		ВложеннаяСтрока.ИндексКартинки = -1;
		ВложеннаяСтрока.Представление = ВернутьСтр("ru = 'Колонки'");
...
Процедура СтруктураВарианта_СохранитьВФайл(Команда)
	...
	ПолучитьФайл(Адрес, ВернутьСтр("ru = 'Настройки.xml'"), Истина);
КонецПроцедуры
...
Функция ВыполнитьШаг(ВыборПользователя, Сценарий)
	...
	Иначе
		Сценарий.ПричинаОтказа = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(ВернутьСтр("ru = 'Ошибка в сценарии: Неизвестный шаг ""%1"".'"), Сценарий.ТекущийШаг);
		Возврат Ложь;
...
Функция ШагОпределитьВыделенныеСтроки(ВыборПользователя, Сценарий)
	...
	Если Сценарий.СтрокиДерева.Количество() = 0 Тогда
		Сценарий.ПричинаОтказа = ВернутьСтр("ru = 'Выберите элементы.'");
		Возврат Ложь; // Завершить сценарий.
	КонецЕсли;
	Если Сценарий.ТекущаяСтрока = Неопределено Тогда
		Если Сценарий.Действие = "ИзменитьГруппу" Тогда
			Сценарий.ПричинаОтказа = ВернутьСтр("ru = 'Выберите группу.'");
			Возврат Ложь; // Завершить сценарий.
		КонецЕсли;
	КонецЕсли;
...
Функция ЗарегистрироватьВыделеннуюСтроку(Сценарий, СтрокаДерева, Особенности)
	...
	Если Особенности.МогутБытьПараметры И СтрокаДерева.ЭтоПараметр Тогда
		Если Сценарий.Действие = "Переместить" Тогда
			Сценарий.ПричинаОтказа = ВернутьСтр("ru = 'Параметры не могут быть перемещены.'");
		ИначеЕсли Сценарий.Действие = "Сгруппировать" Тогда
			Сценарий.ПричинаОтказа = ВернутьСтр("ru = 'Параметры не могут быть участниками групп.'");
		ИначеЕсли Сценарий.Действие = "Удалить" Тогда
			Сценарий.ПричинаОтказа = ВернутьСтр("ru = 'Параметры не могут быть удалены.'");
		КонецЕсли;
		Возврат Ложь; // Завершить сценарий.
	КонецЕсли;
	Если Особенности.ТребоватьОдногоРодителя Тогда
		...
		ИначеЕсли Сценарий.ТекущийРодитель <> Родитель Тогда
			Если Сценарий.Действие = "Переместить" Тогда
				Сценарий.ПричинаОтказа = ВернутьСтр("ru = 'Выбранные элементы не могут быть перемещены, поскольку они принадлежат разным родителям.'");
			ИначеЕсли Сценарий.Действие = "Сгруппировать" Тогда
				Сценарий.ПричинаОтказа = ВернутьСтр("ru = 'Выбранные элементы не могут быть сгруппированы, поскольку они принадлежат разным родителям.'");
			КонецЕсли;
			Возврат Ложь; // Завершить сценарий.
		КонецЕсли;
	КонецЕсли;
...
Процедура ОтборыВыбратьВидСравнения(СтрокаТаблицы)
	...
	Если ИнформацияОТипах.ОграниченнойДлины
		...
		Список.Добавить(ВидСравненияКомпоновкиДанных.ВСпискеПоИерархии); // ВернутьСтр("ru = 'В списке включая подчиненные'")
		Список.Добавить(ВидСравненияКомпоновкиДанных.НеВСпискеПоИерархии); // ВернутьСтр("ru = 'Не в списке включая подчиненные'").
		
		Список.Добавить(ВидСравненияКомпоновкиДанных.ВИерархии); // ВернутьСтр("ru = 'В группе'")
		Список.Добавить(ВидСравненияКомпоновкиДанных.НеВИерархии); // ВернутьСтр("ru = 'Не в группе'")
...
Процедура СтруктураВариантаИзменитьУзел(ТаблицаИмя, СтрокаИдентификатор, СтрокаТаблицы, ИмяСтраницы)
	...
	ШаблонЗаголовка = ВернутьСтр("ru = 'Настройка %1 отчета ""%2""'");
	Если СтрокаТаблицы.Тип = "Диаграмма" Тогда
		ПредставлениеУзла = ВернутьСтр("ru = 'диаграммы'");
	Иначе
		ПредставлениеУзла = ВернутьСтр("ru = 'группировки'");
	КонецЕсли;
...
Процедура ТаблицыПолейИзменитьУровеньДоступа(ТаблицаИмя, СтрокаИдентификатор, ПоказыватьРежимыВвода, ПоказыватьРежимыФлажков)
	...
	Если ПоказыватьРежимыВвода Тогда
		Список.Добавить(2, ВернутьСтр("ru = 'В шапке отчета'"), , БиблиотекаКартинок.БыстрыйДоступ);
	КонецЕсли;
	Если ПоказыватьРежимыФлажков Тогда
		Список.Добавить(1, ВернутьСтр("ru = 'Только флажок в шапке отчета'"), , БиблиотекаКартинок.БыстрыйДоступСФлажком);
	КонецЕсли;
	Если ПоказыватьРежимыВвода Тогда
		Список.Добавить(4, ВернутьСтр("ru = 'В настройках отчета'"), , БиблиотекаКартинок.Реквизит);
	КонецЕсли;
	Если ПоказыватьРежимыФлажков Тогда
		Список.Добавить(3, ВернутьСтр("ru = 'Только флажок в настройках отчета'"), , БиблиотекаКартинок.ОбычныйДоступСФлажком);
	КонецЕсли;
	Список.Добавить(5, ВернутьСтр("ru = 'Не показывать'"), , БиблиотекаКартинок.СкрытаяНастройкаОтчета);
...
Процедура ОформлениеИзменитьЭлемент(ТаблицаИмя, СтрокаИдентификатор, СтрокаТаблицы)
	...
	КонецЕсли;
	ПараметрыФормы.Вставить("Заголовок", СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(ВернутьСтр("ru = 'Элемент условного оформления отчета ""%1""'"), ВариантНаименование));
...
Процедура ТаблицыПолейРазгруппировать(ТаблицаИмя)
	...
	Если ВыделенныеСтроки.Количество() <> 1 Тогда
		ПоказатьПредупреждение(, ВернутьСтр("ru = 'Выберите одну группу.'"));
		Возврат;
	КонецЕсли;
	...
	Если ГруппаДерева = Неопределено Или Не ГруппаДерева.ЭтоГруппа Тогда
		ПоказатьПредупреждение(, ВернутьСтр("ru = 'Выберите группу.'"));
		Возврат;
	КонецЕсли;
...
Функция ТаблицыПолейНайтиУзел(ЭтотОбъект, ТаблицаИмя, СтрокаТаблицы = Неопределено, ИдентификаторУзлаКД = Неопределено)
	Если ЭтотОбъект.РасширенныйРежим = 1 Тогда
		...
		Иначе
			ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(ВернутьСтр("ru = 'Изменение узлов таблицы ""%1"" не поддерживается.'"), ТаблицаИмя);
		КонецЕсли;
	Иначе
...
Процедура ВидимостьДоступностьКорректность(ИмяСобытия = "")
	Если ИмяСобытия = "ПриСозданииНаСервере" Тогда
		Если РежимИзмененияУзлаВарианта Тогда
			...
			Элементы.ЗакрытьИСформировать.Заголовок = ВернутьСтр("ru = 'Завершить редактирование'");
	...
	Если ОбновитьНастройкиВарианта Тогда
		...
		Если ИнформацияОбОшибке <> Неопределено Тогда
			СтандартныеПодсистемыКлиентСервер.ВывестиИнформациюОбОшибке(
				...
				ВернутьСтр("ru = 'Не удалось получить настройки отчета:'"),
				ИнформацияОбОшибке);
...
Процедура БыстрыеНастройкиСоздатьЭлементыУправленияИЗагрузитьЗначения(ПараметрыЗаполнения, Информация)
	...
	ГруппаВывода = ГруппыВывода.ОбычныеОтборы;
	КоличествоКолонок = ?(ГруппаВывода.ЕстьДлинныеПоля И ГруппаВывода.Размер < 10, 1, 2);
...
Процедура РасширенныеНастройкиЗагрузитьЗначения(ПараметрыЗаполнения, Информация)
	...
	Если ВидимостьВыбранныхПолей И ВидимостьСортировок Тогда
		Элементы.СтраницаВыбранныеПоляИСортировки.Заголовок = ВернутьСтр("ru = 'Поля и сортировки'");
	ИначеЕсли ВидимостьВыбранныхПолей Тогда
		Элементы.СтраницаВыбранныеПоляИСортировки.Заголовок = ВернутьСтр("ru = 'Поля'");
	ИначеЕсли ВидимостьСортировок Тогда
		Элементы.СтраницаВыбранныеПоляИСортировки.Заголовок = ВернутьСтр("ru = 'Сортировки'");
	КонецЕсли;
...
Функция ЗарегистрироватьЭлементСтруктурыОтчета(СтрокаДерева, СтрокиРодителя)
	...
	Если СтрокаФормы.Тип = "Настройки" Тогда
		...
		СтрокаФормы.Представление = ВернутьСтр("ru = 'Отчет'");
...
Функция ЗарегистрироватьЭлементыСортировки()
	...
	СтрокаРаздел.Представление = ВернутьСтр("ru = 'Сортировки'");
	...
	Для Каждого ЭлементКД Из НаборСтрокКД Цикл
		...
		Если ТипНастройки = "АвтоЭлементПорядка" Тогда
			СтрокаТаблицы.Представление = ВернутьСтр("ru = 'Авто (сортировки родителя)'");
...
Процедура ЗарегистрироватьЭлементыСоставаГруппировки(УзелКД = Неопределено, НаборСтрокКД = Неопределено, НаборСтрок = Неопределено)
	...
	Для Каждого ЭлементКД Из НаборСтрокКД Цикл
		...
		Если ТипНастройки = "АвтоПолеГруппировки" Тогда
			СтрокаТаблицы.Представление  = ВернутьСтр("ru = 'Авто (по всем полям)'");
...
Функция ЗарегистрироватьЭлементыВыбранныхПолей(УзелКД = Неопределено, НаборСтрокКД = Неопределено, НаборСтрок = Неопределено, ДоступныеПоля = Неопределено)
	...
	Если НаборСтрок = Неопределено Тогда
		...
		СтрокаРаздел.ЭтоРаздел = Истина;
		СтрокаРаздел.Представление = ВернутьСтр("ru = 'Поля'");
	...
	Для Каждого ЭлементКД Из НаборСтрокКД Цикл
		...
		Если ТипНастройки = "АвтоВыбранноеПоле" Тогда
			Представление = ВернутьСтр("ru = 'Авто (поля родителя)'");
			ИндексКартинки = 6;
		Иначе
...
Процедура ЗарегистрироватьПараметры(Информация)
	...
	СтрокаРаздел.Представление = ВернутьСтр("ru = 'Параметры'");
...
Процедура ЗарегистрироватьЭлементыОтборов(Информация, НаборСтрокВарианта = Неопределено, НаборСтрокФормы = Неопределено)
	...
	Если НаборСтрокФормы = Неопределено Тогда
		...
		СтрокаРаздел.ЭтоРаздел = Истина;
		СтрокаРаздел.Представление = ВернутьСтр("ru = 'Отборы'");
