	...
	Если Не ПустаяСтрока(Параметры.ПереданнаяСтрокаПоиска) Тогда
		...
		Если РезультатыПоиска.Количество() <> 0 Тогда
			...
			ПоказаныРезультатыСПо = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				ВернутьСтр("ru = 'Показаны %1 - %2 из %3'"),
		...
		Иначе
			ПоказаныРезультатыСПо = НСтр("ru = 'Не найдено'; en = 'No data found'");
			Элементы.Далее.Доступность = Ложь;
			Элементы.Назад.Доступность = Ложь;
		КонецЕсли;
	Иначе
...
Процедура Искать(Направление, Отказ = Неопределено)
	...
	Если ПустаяСтрока(СтрокаПоиска) Тогда
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(НСтр("ru = 'Введите, что нужно найти.'; en = 'Please, fill in text.'"), , "СтрокаПоиска");
		Отказ = Истина;
		Возврат;
	КонецЕсли;
	...
	Состояние(СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(ВернутьСтр("ru = 'Выполняется поиск ""%1""...'"), СтрокаПоиска));
	...
	Если РезультатыПоиска.Количество() > 0 Тогда
		...
		Если Направление = 0 И Результат.ТекущаяПозиция = 0 И Результат.СлишкомМногоРезультатов Тогда
			ПоказатьПредупреждение(, ВернутьСтр("ru = 'Слишком много результатов, уточните запрос.'"));
		КонецЕсли;
	
	Иначе
		
		ПоказаныРезультатыСПо = ВернутьСтр("ru = 'Не найдено'");
		
		Элементы.Далее.Доступность = Ложь;
		Элементы.Назад.Доступность = Ложь;
		
		ТекстПоиска = ВернутьСтр("ru = 'Искомая комбинация слов нигде не встречается.<br><br>
			|<b>Рекомендации:</b>
...
Процедура СформироватьПредставлениеОбластиПоиска()
	...
	Если Не ИскатьВРазделах Или ОбластиПоиска.Количество() = 0 Тогда
		Элементы.ОбластиПоискаПредставление.Заголовок = НСтр("ru = 'Везде'; en = 'universally'");
		Возврат;
	КонецЕсли;
	...
	Иначе
		Элементы.ОбластиПоискаПредставление.Заголовок = НСтр("ru = 'В выбранных разделах'; en = 'In selected modules'");
	КонецЕсли;
