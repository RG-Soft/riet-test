...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
		...
		Исключение
			...
			Если Не ПриВходеВПрограмму Тогда
				ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					ВернутьСтр("ru = 'Не удалось открыть форму смены пароля по причине:
					           |
					           |%1'"),
	...
	Если ПриВходеВПрограмму Тогда
		...
		Иначе
			...
			Элементы.ПояснениеПриВходе.Заголовок =
				ВернутьСтр("ru = 'Для входа в программу нужно установить пароль.'")
		КонецЕсли;
	Иначе
		...
		Элементы.ФормаЗакрытьФорму.Заголовок = ВернутьСтр("ru = 'Отмена'");
	КонецЕсли;
	...
	Если Не ДополнительныеПараметры.ЭтоТекущийПользовательИБ
	 Или Не ДополнительныеПараметры.ПарольУстановлен Тогда
		...
		АвтоЗаголовок = Ложь;
		Заголовок = ВернутьСтр("ru = 'Установка пароля'");
...
Процедура УстановитьПароль(Команда)
	...
	Если Не ПоказыватьНовыйПароль И НовыйПароль <> Подтверждение Тогда
		...
		ПоказатьПредупреждение(, ВернутьСтр("ru = 'Подтверждение пароля указано некорректно.'"));
		Возврат;
	КонецЕсли;
