...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если НЕ ЗначениеЗаполнено(ЭлектронныйДокумент) Тогда
		ТекстСообщения = ВернутьСтр("ru = 'Не выбран электронный документ'");
...
Процедура ПрочитатьНоменклатуруКонтрагента(Команда)
	...
	Если Модифицированность Тогда
		...
		ТекстВопроса = ВернутьСтр("ru = 'Несохраненные изменения будут утеряны.
			|Продолжить?'");
			
		ОбработчикОповещения = Новый ОписаниеОповещения("ПрочитатьНоменклатуруОповещение", ЭтотОбъект);
		ПоказатьВопрос(ОбработчикОповещения, ТекстВопроса, РежимДиалогаВопрос.ДаНет,,, "Заполнение списка номенклатуры");
...
Процедура СохранитьДанныеКаталога(СсылкаНаЭД)
	...
	ИначеЕсли ТипЗнч(СсылкаНаЭД) = Тип("Структура") Тогда // однократная сделка
		...
		ДвоичныеДанныеФайла = Новый ДвоичныеДанные(СсылкаНаЭД.ПолноеИмяФайла);
		ДопИнформацияПоЭД.Вставить("СсылкаНаДвоичныеДанныеФайла", ПоместитьВоВременноеХранилище(ДвоичныеДанныеФайла));
		Файл = Новый Файл(СсылкаНаЭД.ПолноеИмяФайла);
		ДопИнформацияПоЭД.Вставить("Расширение", Файл.Расширение);
	...
	Если ДопИнформацияПоЭД.Свойство("СсылкаНаДвоичныеДанныеФайла")
		...
		Если СтрНайти(ДопИнформацияПоЭД.Расширение, "zip") > 0 Тогда
			...
			Исключение
				...
				КонецЕсли;
				Операция = ВернутьСтр("ru = 'Распаковка ЭД'");
		...
		СтруктураРазбора = ЭлектронныеДокументыВнутренний.СформироватьДеревоРазбора(ИмяФайлаДанных,
...
Процедура ПрочитатьНоменклатуруКонтрагентаСервер()
	...
	Если НЕ ЗначениеЗаполнено(Запрос.Текст) Тогда
		ТекстСообщения = ВернутьСтр("ru = 'Не определено сопоставление номенклатуры и номенклатуры поставщиков.
