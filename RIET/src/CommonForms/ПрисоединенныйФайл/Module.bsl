...
Процедура СтандартныеУстановитьПометкуУдаления(Команда)
	...
	Иначе
		Если ЭтотОбъект.Объект.ПометкаУдаления Тогда
			ТекстВопроса = ВернутьСтр("ru = 'Снять пометку на удаление с файла
			                          |""%1""?'");
		Иначе
			ТекстВопроса = ВернутьСтр("ru = 'Пометить на удаление файл
			                          |""%1""?'");
		КонецЕсли;
	КонецЕсли;
...
Процедура СтандартнаяПеречитать(Команда)
	...
	ТекстВопроса = ВернутьСтр("ru = 'Данные изменены. Перечитать данные?'");
...
Процедура УдалитьЭП(Команда)
	...
	ПоказатьВопрос(ОписаниеОповещения, ВернутьСтр("ru = 'Удалить выделенные подписи?'"), РежимДиалогаВопрос.ДаНет);
...
Процедура ОбновитьЗаголовок()
	...
	Если ЗначениеЗаполнено(ЭтотОбъект.Объект.Ссылка) Тогда
		Заголовок = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			ВернутьСтр("ru = '%1 (Присоединенный файл)'"), Строка(ЭтотОбъект.Объект.Ссылка));
	Иначе
		Заголовок = ВернутьСтр("ru = 'Присоединенный файл (Создание)'")
	КонецЕсли;
...
Процедура ЗаполнитьСписокШифрования()
	...
	ТекстЗаголовка = ВернутьСтр("ru = 'Разрешено расшифровывать'");
...
Процедура ЗаполнитьСписокПодписей()
	...
	ТекстЗаголовка = ВернутьСтр("ru = 'Электронные подписи'");
...
Функция ОбработатьКомандуЗаписиФайла()
	...
	Если ПустаяСтрока(ЭтотОбъект.Объект.Наименование) Тогда
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(
			ВернутьСтр("ru = 'Для продолжения укажите имя файла.'"), , "Наименование", "Объект");
		Возврат Ложь;
	КонецЕсли;
...
Функция ЗаписатьФайл(Знач ПараметрОбъект = Неопределено)
	...
	Если ТипЗнч(ЗаписываемыйОбъект.ВладелецФайла) = Тип("ДокументСсылка.РеализацияТоваровУслуг") Тогда
		Если Не ЗначениеЗаполнено(ЭтотОбъект.ВидДокумента) Тогда 
			ОбщегоНазначенияКлиентСервер.СообщитьПользователю("Укажите вид документа!", , "ВидДокумента", "Объект");
			Возврат Ложь;
		иначе
			ЗаписываемыйОбъект.ВидДокумента = ЭтотОбъект.ВидДокумента;
		КонецЕсли;
		// { RGS AArsentev 07.09.2016 17:38:05 - S-I-0001855
	ИначеЕсли ТипЗнч(ЗаписываемыйОбъект.ВладелецФайла) = Тип("ДокументСсылка.TransportRequest") ИЛИ ТипЗнч(ЗаписываемыйОбъект.ВладелецФайла) = Тип("ДокументСсылка.TripNonLawsonCompanies") Тогда	
		Если Не ЗначениеЗаполнено(ЭтотОбъект.ВидДокумента) Тогда 
			ОбщегоНазначенияКлиентСервер.СообщитьПользователю("Укажите вид документа!", , "ВидДокумента", "Объект");
			Возврат Ложь;
		иначе
			ЗаписываемыйОбъект.ВидДокумента = ЭтотОбъект.ВидДокумента;
		КонецЕсли;
		Если Не ЗначениеЗаполнено(ЭтотОбъект.ДокументНаименование) Тогда 
			ОбщегоНазначенияКлиентСервер.СообщитьПользователю("Введите наименование документа!", , "ДокументНаименование", "Объект");
			Возврат Ложь;
		иначе
			ЗаписываемыйОбъект.ДокументНаименование = ЭтотОбъект.ДокументНаименование;
		КонецЕсли;
		Если Не ЗначениеЗаполнено(ЭтотОбъект.ДатаДокумента) Тогда 
			ОбщегоНазначенияКлиентСервер.СообщитьПользователю("Укажите дату документа!", , "ДатаДокумента", "Объект");
			Возврат Ложь;
		иначе
			ЗаписываемыйОбъект.ДатаДокумента = ЭтотОбъект.ДатаДокумента;
		КонецЕсли;
		// } RGS AArsentev 07.09.2016 17:38:05 - S-I-0001855
	КонецЕсли; 
	//}конец добавления RG-Soft добавила Петроченко Н.Н.
	...
	Исключение
		Если ТранзакцияАктивна Тогда
			ОтменитьТранзакцию();
			ЗаписьЖурналаРегистрации(ВернутьСтр("ru = 'Файлы.Ошибка записи присоединенного файла'", ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка()),
...
Процедура НастроитьОбъектФормы(Знач НовыйОбъект)
	...
	//{начало добавления RG-Soft добавила Петроченко Н.Н.
	Если ИмяСправочника = "РеализацияТоваровУслугПрисоединенныеФайлы" Тогда
		
		НовыйЭлемент = Элементы.Добавить("ВидДокумента", Тип("ПолеФормы"),Элементы.Найти("ГруппаОбщаяИнформация"));
		                         
		МассивНовыхРеквизитов = Новый Массив;
		МассивНовыхРеквизитов.Добавить(Новый РеквизитФормы("ВидДокумента", Новый ОписаниеТипов("ПеречислениеСсылка.ВидыПрисоединенныхДокументовРеализацийТоваровУслуг"), , "Вид Документа"));
		ЭтотОбъект.ИзменитьРеквизиты(МассивНовыхРеквизитов);
		НовыйЭлемент.ПутьКДанным = "ВидДокумента";
		НовыйЭлемент.Вид = ВидПоляФормы.ПолеВвода;
		НовыйЭлемент.АвтоОтметкаНезаполненного = Истина;
		
		ЭтотОбъект.ВидДокумента = НовыйОбъект.ВидДокумента;
	// { RGS AArsentev 07.09.2016 17:38:05 - S-I-0001855	
	ИначеЕсли ИмяСправочника = "TransportRequestПрисоединенныеФайлы" ИЛИ ИмяСправочника = "TripNonLawsonCompaniesПрисоединенныеФайлы" Тогда
		
		НовыйЭлемент = Элементы.Добавить("ВидДокумента", Тип("ПолеФормы"),Элементы.Найти("ГруппаОбщаяИнформация"));	
		МассивНовыхРеквизитов = Новый Массив;
		МассивНовыхРеквизитов.Добавить(Новый РеквизитФормы("ВидДокумента", Новый ОписаниеТипов("ПеречислениеСсылка.ВидыПрисоединенныхДокументовTransportRequest"), , "Вид Документа"));
		ЭтотОбъект.ИзменитьРеквизиты(МассивНовыхРеквизитов);
		НовыйЭлемент.ПутьКДанным = "ВидДокумента";
		НовыйЭлемент.Вид = ВидПоляФормы.ПолеВвода;
		НовыйЭлемент.АвтоОтметкаНезаполненного = Истина;
		...
		НовыйЭлемент = Элементы.Добавить("ДокументНаименование", Тип("ПолеФормы"),Элементы.Найти("ГруппаОбщаяИнформация"));
		МассивНовыхРеквизитов = Новый Массив;
		МассивНовыхРеквизитов.Добавить(Новый РеквизитФормы("ДокументНаименование", Новый ОписаниеТипов("Строка"), , "Документ наименование"));
		ЭтотОбъект.ИзменитьРеквизиты(МассивНовыхРеквизитов);
		НовыйЭлемент.ПутьКДанным = "ДокументНаименование";
		НовыйЭлемент.Вид = ВидПоляФормы.ПолеВвода;
		НовыйЭлемент.АвтоОтметкаНезаполненного = Истина;
		ЭтотОбъект.ДокументНаименование = НовыйОбъект.ДокументНаименование;
		
		НовыйЭлемент = Элементы.Добавить("ДатаДокумента", Тип("ПолеФормы"),Элементы.Найти("ГруппаОбщаяИнформация"));
		МассивНовыхРеквизитов = Новый Массив;
		МассивНовыхРеквизитов.Добавить(Новый РеквизитФормы("ДатаДокумента", Новый ОписаниеТипов("Дата"), , "Дата Документа"));
		ЭтотОбъект.ИзменитьРеквизиты(МассивНовыхРеквизитов);
		НовыйЭлемент.ПутьКДанным = "ДатаДокумента";
		НовыйЭлемент.Вид = ВидПоляФормы.ПолеВвода;
		НовыйЭлемент.АвтоОтметкаНезаполненного = Истина;
		ЭтотОбъект.ДатаДокумента = НовыйОбъект.ДатаДокумента;
		// } RGS AArsentev 07.09.2016 17:38:05 - S-I-0001855
