...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	ТекстСообщения = ВернутьСтр("ru = 'Пожалуйста, подождите...'");
...
Процедура Подключаемый_ПроверитьВыполнениеЗадания()
	...
	ИначеЕсли Статус = "Ошибка" Тогда
		...
		ПоказатьПредупреждение(, ?(ПустаяСтрока(Результат.КраткоеПредставлениеОшибки), 
			ВернутьСтр("ru = 'Не удалось выполнить запрошенное действие.'"), Результат.КраткоеПредставлениеОшибки));
		Закрыть(РезультатВыполнения(Результат));
		Возврат;
...
Процедура ПоказатьОповещение()
	...
	ПоказатьОповещениеПользователя(?(Оповещение.Текст <> Неопределено, Оповещение.Текст, ВернутьСтр("ru = 'Действие выполнено'")), 
