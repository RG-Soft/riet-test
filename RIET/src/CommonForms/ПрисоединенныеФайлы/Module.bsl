...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если Параметры.ВладелецФайла = Неопределено Тогда
		ВызватьИсключение ВернутьСтр("ru = 'Список присоединенных файлов можно посмотреть
	...
	Если Параметры.РежимВыбора Тогда
		...
		Заголовок = ВернутьСтр("ru = 'Выбор присоединенного файла'");
...
Процедура СписокПередНачаломДобавления(Элемент, Отказ, Копирование, Родитель, Группа)
	...
	Иначе
		...
		//{начало изменения RG-Soft добавила Петроченко Н.Н.
		Если ТипЗнч(Параметры.ВладелецФайла) = Тип("ДокументСсылка.РеализацияТоваровУслуг") 
			// { RGS LFedotova 18.12.2017 21:19:34 - вопрос SLI-0007442
			ИЛИ ТипЗнч(Параметры.ВладелецФайла) = Тип("ДокументСсылка.ПередачаОС")
			ИЛИ ТипЗнч(Параметры.ВладелецФайла) = Тип("ДокументСсылка.ОтчетАгентаПоРасходам")
			// } RGS LFedotova 18.12.2017 21:20:20 - вопрос SLI-0007442
			Тогда
			
			Оповещение = Новый ОписаниеОповещения("ПослеВыбораВидаДокумента", ЭтотОбъект, Параметры);
			ПоказатьВводЗначения(Оповещение, ПредопределенноеЗначение("Перечисление.ВидыПрисоединенныхДокументовРеализацийТоваровУслуг.СчетФактура"), "Выберите вид документа", Новый ОписаниеТипов("ПеречислениеСсылка.ВидыПрисоединенныхДокументовРеализацийТоваровУслуг"));
			
			// { RGS AArsentev 07.09.2016 17:38:05 - S-I-0001855
		ИначеЕсли ТипЗнч(Параметры.ВладелецФайла) = Тип("ДокументСсылка.TransportRequest") или ТипЗнч(Параметры.ВладелецФайла) = Тип("ДокументСсылка.TripNonLawsonCompanies") Тогда			
			Оповещение = Новый ОписаниеОповещения("ОткрытьПараметрыЗавершение", ЭтотОбъект); 
			ОткрытьФорму("ОбщаяФорма.ПрисоединенныеФайлыДопИнформация", , , , , , Оповещение);			
			// } RGS AArsentev 07.09.2016 17:38:05 - S-I-0001855		
		иначе
			ПрисоединенныеФайлыКлиент.ДобавитьФайлы(Параметры.ВладелецФайла, УникальныйИдентификатор);
		КонецЕсли;  //}конец изменения RG-Soft добавила Петроченко Н.Н.
...
&НаКлиенте
Процедура ПослеВыбораВидаДокумента(ВыбЗнач, Параметры) Экспорт  //RG-Soft добавила Петроченко Н.Н.
	...
	ПрисоединенныеФайлыКлиент.ДобавитьФайлы(Параметры.ВладелецФайла, УникальныйИдентификатор, ,ВыбЗнач, );
	
КонецПроцедуры

&НаКлиенте 
Процедура ОткрытьПараметрыЗавершение(Результат, ДополнительныеПараметры) Экспорт  // { RGS AArsentev 07.09.2016 17:38:05 - S-I-0001855
	
	ЕстьОшибки = Ложь;
	Если Результат <> Неопределено Тогда
		Если Не ЗначениеЗаполнено(Результат.Наименование) Тогда
			Сообщить("Не введено наименование документа!");
			ЕстьОшибки = Истина;
		КонецЕсли;
		Если Не ЗначениеЗаполнено(Результат.ВидДокумента) Тогда
			Сообщить("Не выбран вид документа!");
			ЕстьОшибки = Истина;
		КонецЕсли;
	...
	ПрисоединенныеФайлыКлиент.ДобавитьФайлы(Параметры.ВладелецФайла, УникальныйИдентификатор, , ,Результат);

КонецПроцедуры // } RGS AArsentev 07.09.2016 17:38:05 - S-I-0001855   
...
Процедура Печать(Команда)
	...
	Если СистемнаяИнфо.ТипПлатформы <> ТипПлатформы.Windows_x86 
		И СистемнаяИнфо.ТипПлатформы <> ТипПлатформы.Windows_x86_64 Тогда
			ПоказатьПредупреждение(, ВернутьСтр("ru = 'Печать файлов возможна только в Windows.'"));
			Возврат;
	КонецЕсли;
...
Процедура НастроитьДинамическийСписок(ИмяСправочникаХранилищаФайлов)
	...
	Если Параметры.Свойство("ДополнительныеРеквизиты") Тогда
		...
		МассивИменКолонок = СтроковыеФункцииКлиентСервер.РазложитьСтрокуВМассивПодстрок(Параметры.ДополнительныеРеквизиты);
		Для Каждого ИмяКолонки Из МассивИменКолонок Цикл
			ТекстЗапроса = ТекстЗапроса + "
			|	Файлы." + СокрЛП(ИмяКолонки) + " КАК " + СокрЛП(ИмяКолонки) + ",";	
		КонецЦикла;
	...
	ЗаголовокОшибки = ВернутьСтр("ru = 'Ошибка при настройке динамического списка присоединенных файлов.'");
	ОкончаниеОшибки = ВернутьСтр("ru = 'В этом случае настройка динамического списка невозможна.'");
	...
	Если Параметры.Свойство("ДополнительныеРеквизиты") Тогда
		...
		Для Каждого ИмяКолонки Из МассивИменКолонок Цикл
			...
			НовыйЭлемент = Элементы.Добавить(ИмяКолонки, Тип("ПолеФормы"),Элементы.Найти("Список"));
			НовыйЭлемент.ПутьКДанным = "Список."+ИмяКолонки;
	...
	// { RGS AArsentev 07.09.2016 17:38:05 - S-I-0001855	видимость всех документов для связанных TR/Trip как в аттачах трипа, так и в аттачах TR 
	Если (ТипЗнч(Параметры.ВладелецФайла) = Тип("ДокументСсылка.TripNonLawsonCompanies") ИЛИ ТипЗнч(Параметры.ВладелецФайла) = Тип("ДокументСсылка.TransportRequest")) И Не Параметры.Свойство("ЭтоФотоГруза") Тогда
		
		Список.ТекстЗапроса = ПолучитьТекстЗапросаTR();		
		Владельцы = Новый Массив;
		
		ЗапросTR = Новый Запрос;
		ЗапросTR.УстановитьПараметр("Ссылка",Параметры.ВладелецФайла); 
		Если  ТипЗнч(Параметры.ВладелецФайла) = Тип("ДокументСсылка.TripNonLawsonCompanies") Тогда 	
			ЗапросTR.Текст = "ВЫБРАТЬ РАЗЛИЧНЫЕ
			...
			РезультатTR = ЗапросTR.Выполнить().Выгрузить();
			Владельцы = РезультатTR.ВыгрузитьКолонку("Ссылка");
			Владельцы.Добавить(Параметры.ВладелецФайла);
		ИначеЕсли ТипЗнч(Параметры.ВладелецФайла) = Тип("ДокументСсылка.TransportRequest") Тогда
			ЗапросTR.Текст = "ВЫБРАТЬ РАЗЛИЧНЫЕ
			|	TripNonLawsonCompaniesParcels.Ссылка КАК Ссылка
			|ИЗ
			|	Документ.TripNonLawsonCompanies.Parcels КАК TripNonLawsonCompaniesParcels
			|ГДЕ
			|	TripNonLawsonCompaniesParcels.Parcel.TransportRequest = &Ссылка";
			РезультатTrip = ЗапросTR.Выполнить().Выгрузить();
			Владельцы = РезультатTrip.ВыгрузитьКолонку("Ссылка");
			Владельцы.Добавить(Параметры.ВладелецФайла);
		КонецЕсли;
		Список.Параметры.УстановитьЗначениеПараметра("ВладельцыФайлов", Владельцы);
	// { RGS AGazialiev 20.03.2017 - S-I-0001855	открытие сномков груза
	ИначеЕсли ТипЗнч(Параметры.ВладелецФайла) = Тип("ДокументСсылка.TripNonLawsonCompanies") И Параметры.Свойство("ЭтоФотоГруза") Тогда
		Список.ТекстЗапроса = ПолучитьТекстЗапросаTrip();		
		Список.Параметры.УстановитьЗначениеПараметра("ВладелецФайла", Параметры.ВладелецФайла);
		ИмяСправочникаХранилищаФайлов = "TripNonLawsonCompaniesПрисоединенныеФайлы";
		ПолноеИмяСправочника = "Справочник." + ИмяСправочникаХранилищаФайлов;
	// } RGS AGazialiev 20.03.2017 - S-I-0001855
	Иначе
...
Процедура ОбновитьПояснениеОблачногоСервиса()
	...
	Если ПолучитьФункциональнуюОпцию("ИспользоватьСинхронизациюФайлов") Тогда
		...
		Пока ВыборкаДетальныеЗаписи.Следующий() Цикл
			...
			ЧастиСтроки.Добавить(ВернутьСтр("ru = 'Работа с файлами ведется в облачном сервисе'"));
...
&НаСервере
Функция ПолучитьТекстЗапросаTR() // { RGS AArsentev 07.09.2016 17:38:05 - S-I-0001855
	...
	ЗапросТекст = "ВЫБРАТЬ
	              ...
	              |	Файлы.ДокументНаименование КАК ДокументНаименование,
	              |	Файлы.ДатаДокумента КАК ДатаДокумента,
	              |	Файлы.Расширение КАК Расширение
	              |ИЗ
	              |	Справочник.TransportRequestПрисоединенныеФайлы КАК Файлы
	              |		ЛЕВОЕ СОЕДИНЕНИЕ РегистрСведений.СтатусыСинхронизацииФайловСОблачнымСервисом КАК СтатусыСинхронизацииФайловСОблачнымСервисом
	              |		ПО Файлы.Ссылка = СтатусыСинхронизацииФайловСОблачнымСервисом.Файл
	              |ГДЕ
...
КонецФункции // } RGS AArsentev 07.09.2016 17:38:05 - S-I-0001855

&НаСервере
Функция ПолучитьТекстЗапросаTrip() // { RGS AGazialiev 20.03.2017
	...
	|	Файлы.Type
	|ИЗ
	|	Справочник.TripNonLawsonCompaniesПрисоединенныеФайлы КАК Файлы
	|		ЛЕВОЕ СОЕДИНЕНИЕ РегистрСведений.СтатусыСинхронизацииФайловСОблачнымСервисом КАК СтатусыСинхронизацииФайловСОблачнымСервисом
	|		ПО Файлы.Ссылка = СтатусыСинхронизацииФайловСОблачнымСервисом.Файл
	|ГДЕ
...
КонецФункции // } RGS AGazialiev 20.03.2017
