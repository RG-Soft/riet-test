...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Иначе
		СопровождающийТекст = ВернутьСтр("ru = 'Команда выполняется...'");
	КонецЕсли;
	...
	Иначе
		Заголовок = ВернутьСтр("ru = 'Пожалуйста, подождите'");
	КонецЕсли;
	...
	Попытка
		...
		ЗаданиеРезультат = ДлительныеОперации.ЗапуститьВыполнениеВФоне(
			...
			СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				ВернутьСтр("ru = 'Выполнение дополнительного отчета или обработки ""%1"", имя команды ""%2""'"),
...
Процедура Отменить(Команда)
	ТекстВопроса = ВернутьСтр("ru = 'Длительная операция еще выполняется.'");
