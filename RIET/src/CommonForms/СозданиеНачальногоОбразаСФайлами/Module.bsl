...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если Параметры.Узел = Менеджер.ЭтотУзел() Тогда
		ВызватьИсключение
			ВернутьСтр("ru = 'Создание начального образа для данного узла невозможно.'");
	Иначе
	...
	Если Не СтандартныеПодсистемыВызовСервера.ПараметрыРаботыКлиента().ИнформационнаяБазаФайловая Тогда
		Элементы.ПутьКАрхивуСФайламиТомов.ПодсказкаВвода = ВернутьСтр("ru = '\\имя сервера\resource\files.zip'");
...
Процедура СоздатьНачальныйОбраз(Команда)
	...
	Если ВидБазы = 0 И НЕ МожноСоздатьФайловуюБазу Тогда
		...
		ВызватьИсключение
			ВернутьСтр("ru = 'Создание начального образа файловой информационной базы
...
Процедура ОбработчикСохраненияФайлаПослеПодключенияРасширенияРаботыСФайлами(Подключено, ДополнительныеПараметры) Экспорт
	...
	Диалог.Заголовок                = ВернутьСтр("ru = 'Выберите файл для сохранения'");
...
Процедура ЗапуститьСозданиеНачальногоОбраза()
	ФоновоеЗаданиеЗапуститьНаКлиенте(ВидБазы);
КонецПроцедуры

&НаКлиенте
Процедура ФоновоеЗаданиеЗапуститьНаКлиенте(Действие)
	
	Результат = ФоновоеЗаданиеЗапустить(Действие);
...
&НаСервере
Функция ФоновоеЗаданиеЗапустить(Знач Действие)
	...
	Если Действие = 0 Тогда
		...
		НаименованиеЗадания = ВернутьСтр("ru = 'Создание файлового начального образа'");
		НаименованиеПроцедуры = "СоздатьФайловыйНачальныйОбразНаСервере";
		
		// Функция обработки, проверки и подготовки параметров.
		МожноЗапускатьЗадание = ФайловыеФункцииСлужебный.ПодготовитьДанныеДляСозданияФайловогоНачальногоОбраза(ПараметрыЗадания);
	Иначе
		...
		НаименованиеЗадания = ВернутьСтр("ru = 'Создание серверного начального образа'");
		НаименованиеПроцедуры = "СоздатьСерверныйНачальныйОбразНаСервере";
		
		// Функция обработки, проверки и подготовки параметров.
		МожноЗапускатьЗадание = ФайловыеФункцииСлужебный.ПодготовитьДанныеДляСозданияСерверногоНачальногоОбраза(ПараметрыЗадания);
	КонецЕсли;
	Если МожноЗапускатьЗадание = Ложь Тогда
		// Ошибка при инициализации параметров.
	...
	РезультатФоновогоЗадания = ДлительныеОперации.ЗапуститьВыполнениеВФоне(
		...
		"ФайловыеФункцииСлужебный."+НаименованиеПроцедуры,
		ПараметрыЗадания,
		НаименованиеЗадания);
...
Функция СостояниеЗадания(Знач ИдентификаторЗадания)
	...
	Исключение
		...
		ЗаписьЖурналаРегистрации(ВернутьСтр("ru = 'Обмен данными'", ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка()),
