...
Процедура ПередЗакрытием(Отказ, СтандартнаяОбработка)
	...
	ТекстВопроса = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(ВернутьСтр("ru = 'Сохранить изменения в %1?'"), ИмяДокумента);
...
Процедура НачатьДиалогСохраненияФайла(Знач ОбработчикЗавершения)
	...
	ДиалогСохраненияФайла.Фильтр = ВернутьСтр("ru = 'Табличный документ'") + " (*.mxl)|*.mxl";
...
Процедура УстановитьЗаголовок()
	...
	Если ЭтоНовый() Тогда
		Заголовок = Заголовок + " (" + ВернутьСтр("ru = 'создание'") + ")";
	ИначеЕсли РедактированиеЗапрещено Тогда
		Заголовок = Заголовок + " (" + ВернутьСтр("ru = 'только просмотр'") + ")";
	КонецЕсли;
