...
Процедура ПерейтиКСписку()
	...
	Для Каждого ОкноКлиентскогоПриложения Из ПолучитьОкна() Цикл
		Если ОкноКлиентскогоПриложения.ПолучитьНавигационнуюСсылку() = НавигационнаяСсылка Тогда
			...
			Кнопки.Добавить("Переоткрыть", ВернутьСтр("ru = 'Переоткрыть'"));
			Кнопки.Добавить("Отмена", ВернутьСтр("ru = 'Не переоткрывать'"));
			ТекстВопроса = 
				ВернутьСтр("ru = 'Список уже открыт. Переоткрыть список, 
