...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если ВозможныеПрава.ПоТипамСсылок[ТипЗнч(СсылкаНаОбъект)] = Неопределено Тогда
		ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			ВернутьСтр("ru = 'Права доступа не настраиваются
			           |для объектов типа ""%1"".'"),
...
Процедура Перечитать(Команда)
	...
	Иначе
		ПоказатьВопрос(
			...
			ВернутьСтр("ru = 'Данные изменены. Прочитать без сохранения?'"),
...
Процедура ЗаписатьНачало(Закрыть = Ложь)
	...
	Если ПодтвердитьОтказОтУправленияПравами = Истина Тогда
		...
		Кнопки.Добавить("ЗаписатьИЗакрыть", ВернутьСтр("ru = 'Записать и закрыть'"));
		Кнопки.Добавить("Отмена", ВернутьСтр("ru = 'Отмена'"));
		ПоказатьВопрос(
			Новый ОписаниеОповещения("ЗаписатьПослеПодтверждения", ЭтотОбъект),
			ВернутьСтр("ru = 'После записи настройка прав станет недоступной.'"),
			Кнопки,, "Отмена");
	Иначе
		Если Закрыть Тогда
			Закрыть();
...
Процедура ДобавитьРеквизитыИлиЭлементыФормы(НовыеРеквизиты = Неопределено)
	...
	Если НовыеРеквизиты = Неопределено И Параметры.СсылкаНаОбъект.Метаданные().Иерархический Тогда
		...
		Элемент.Заголовок = ВернутьСтр("ru = 'Для
		                               |подпапок'");
		Элемент.Подсказка = ВернутьСтр("ru = 'Права не только для текущей папки,
		...
		Элемент.Заголовок = ВернутьСтр("ru = 'Наследуется от'");
		Элемент.Подсказка = ВернутьСтр("ru = 'Папка, от которой наследуются настройка прав'");
	...
	Пока НЕ Отказ И НомерСтроки >= 0 Цикл
		...
		Если НетЗаполненногоПрава Тогда
			ОбщегоНазначенияКлиентСервер.СообщитьПользователю(
				ВернутьСтр("ru = 'Не заполнено ни одно право доступа.'"),
				,
		...
		Если НЕ ЗначениеЗаполнено(ТекущаяСтрока["Пользователь"]) Тогда
			ОбщегоНазначенияКлиентСервер.СообщитьПользователю(
				ВернутьСтр("ru = 'Не заполнен пользователь или группа.'"),
				,
		...
		Если ГруппыПрав.НайтиСтроки(Отбор).Количество() > 1 Тогда
			Если ТипЗнч(Отбор.Пользователь) = Тип("СправочникСсылка.Пользователи") Тогда
				ТекстСообщения = ВернутьСтр("ru = 'Настройка для пользователя ""%1"" уже есть.'");
			Иначе
				ТекстСообщения = ВернутьСтр("ru = 'Настройка для группы пользователей ""%1"" уже есть.'");
			КонецЕсли;
			ОбщегоНазначенияКлиентСервер.СообщитьПользователю(
	...
	Если ЗначениеЗаполнено(ТекущийВладелецНастройки)
	   И ТекущийВладелецНастройки <> Параметры.СсылкаНаОбъект Тогда
		...
		ТекстСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			ВернутьСтр("ru = 'Эти права унаследованы, их можно изменить в форме настройки прав
			           |вышестоящей папки ""%1"".'"),
		...
		Если ПроверкаУдаления Тогда
			ТекстСообщения = ТекстСообщения + Символы.ПС + Символы.ПС + СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				ВернутьСтр("ru = 'Для удаления всех унаследованных прав следует
				           |снять флажок ""%1"".'"),
...
Процедура ПроверитьРазрешениеНаУправлениеПравами()
	...
	ВызватьИсключение ВернутьСтр("ru = 'Настройка прав недоступна.'");
