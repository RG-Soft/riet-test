...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если Не ЗначениеЗаполнено(Параметры.ИсточникДанных) Тогда 
		ОбщегоНазначенияКлиентСервер.Проверить(ТипЗнч(Параметры.ПараметрКоманды) = Тип("Массив") Или ОбщегоНазначения.ЗначениеСсылочногоТипа(Параметры.ПараметрКоманды),
			СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(ВернутьСтр("ru = 'Недопустимое значение параметра ПараметрКоманды при вызове метода УправлениеПечатьюКлиент.ВыполнитьКомандуПечати.
	...
	Если ЭтоПечатьКомплекта() Тогда
		Элементы.Копий.Заголовок = ВернутьСтр("ru = 'Копий комплекта'");
	КонецЕсли;
...
Процедура ПриОткрытии(Отказ)
	...
	Если Этаформа.Параметры.ИмяМенеджераПечати = "Документ.ПередачаОС" Тогда
		НастройкиПечатныхФорм.Сортировать("Название");
	КонецЕсли;	
	// } RGS AFokin 06.12.2016 12:48:25 - SLI-0007020 
КонецПроцедуры
	...
	Если ВРег(ИсточникВыбора.ИмяФормы) = ВРег("ОбщаяФорма.СохранениеПечатнойФормы") Тогда
		...
		Если ВыбранноеЗначение <> Неопределено И ВыбранноеЗначение <> КодВозвратаДиалога.Отмена Тогда
			...
			Иначе
				...
				КонецЦикла;
				Состояние(ВернутьСтр("ru = 'Сохранение успешно завершено.'"));
			КонецЕсли;
		КонецЕсли;
...
Процедура Сохранить(Команда)
	...
	// { RGS AFokin 28.11.2016 17:38:05 - SLI-0006956
	Если ОбъектыПечати.Количество() > 0 И ЭтотОбъект.Параметры.ИмяМенеджераПечати = "Документ.СчетФактураВыданный" Тогда
...
Процедура ПерейтиКДокументу(Команда)
	...
	СписокВыбора.ПоказатьВыборЭлемента(ОписаниеОповещения, ВернутьСтр("ru = 'Перейти к печатной форме'"));
...
Процедура СоздатьРеквизитыИЭлементыФормыДляПечатныхФорм(КоллекцияПечатныхФорм)
	...
	Для Каждого РеквизитФормы Из НовыеРеквизитыФормы Цикл
		...
		РанееДобавленнаяНастройкаПечатнойФормы = ДобавленныеНастройкиПечатныхФорм[ОписаниеПечатнойФормы.СинонимМакета];
		Если РанееДобавленнаяНастройкаПечатнойФормы = Неопределено Тогда
			...
			ДобавленныеНастройкиПечатныхФорм.Вставить(НоваяНастройкаПечатнойФормы.Название, НоваяНастройкаПечатнойФормы);
		Иначе
...
Процедура НастроитьВидимостьЭлементовФормы(Знач ЕстьРазрешенныйВывод)
	...
	ДоступноИзменениеМакетов = Пользователи.РолиДоступны("БазовыеПрава") И ЕстьРедактируемыеМакеты(); // изменено RG-Soft
	...
	Для Каждого НастройкаПечатнойФормы Из НастройкиПечатныхФорм Цикл
		Если НастройкаПечатнойФормы.Количество <> 1 Тогда
			НастройкаПечатнойФормы.Представление = НастройкаПечатнойФормы.Представление 
				+ " (" + НастройкаПечатнойФормы.Количество + " " + ВернутьСтр("ru = 'экз.'") + ")";
		КонецЕсли;
	КонецЦикла;
КонецПроцедуры
	...
	Если ЕстьРазрешенныйВывод Тогда
		Для Каждого НастройкаПечатнойФормы Из НастройкиПечатныхФорм Цикл
			...
			Если ПолеТабличногоДокумента.Вывод = ИспользованиеВывода.Запретить Тогда
				НастройкаПечатнойФормы.Представление = НастройкаПечатнойФормы.Представление + " (" + ВернутьСтр("ru = 'вывод не доступен'") + ")";
			ИначеЕсли ПолеТабличногоДокумента.Защита Тогда
				НастройкаПечатнойФормы.Представление = НастройкаПечатнойФормы.Представление + " (" + ВернутьСтр("ru = 'только печать'") + ")";
			КонецЕсли;
		КонецЦикла;
	КонецЕсли;	
КонецПроцедуры
...
Процедура УстановитьИмяПринтераВПодсказкеКнопкиПечать()
	Если НастройкиПечатныхФорм.Количество() > 0 Тогда
		...
		Если Не ПустаяСтрока(ИмяПринтера) Тогда
			ЭтотОбъект.Команды["Печать"].Подсказка = ВернутьСтр("ru = 'Напечатать на принтере'") + " (" + ИмяПринтера + ")";
		КонецЕсли;
	КонецЕсли;
КонецПроцедуры
...
Процедура УстановитьЗаголовокФормы()
	...
	Иначе
		Если ЭтоПечатьКомплекта() Тогда
			Заголовок = НСтр("ru = 'Печать комплекта'; en = 'Printing'");
		ИначеЕсли ТипЗнч(Параметры.ПараметрКоманды) <> Тип("Массив") Или Параметры.ПараметрКоманды.Количество() > 1 Тогда
			Заголовок = НСтр("ru = 'Печать документов'; en = 'Printing'");
		Иначе
			Заголовок = НСтр("ru = 'Печать документа'; en = 'Printing'");
		КонецЕсли;
	КонецЕсли;
КонецПроцедуры
			...
			Для Каждого ТипФайла Из ВыбранныеФорматыСохранения Цикл
				...
				Если СоответствиеОбъектаПечатнойФорме.Ключ <> "ОбъектыПечатиНеЗаданы" Тогда
					...
					Если ИмяФайла = Неопределено Тогда
						// { RGS LFedotova 23.12.2017 22:42:26 - вопрос SLI-0007442
						НастройкаПечатнойФормы.Название = ?(Лев(НастройкаПечатнойФормы.Название,26) = "СчетФактураПоПостановлению", "Счет-фактура", НастройкаПечатнойФормы.Название);  
						// } RGS LFedotova 23.12.2017 22:42:45 - вопрос SLI-0007442
...
Функция ИмяФайлаПечатнойФормыПоУмолчанию(ОбъектПечати, НазваниеПечатнойФормы)
	...
	Если ОбщегоНазначения.ЭтоДокумент(Метаданные.НайтиПоТипу(ТипЗнч(ОбъектПечати))) Тогда
		...
		// { RGS LFedotova 23.12.2017 22:22:04 - вопрос SLI-0007442
		ПараметрыДляВставки.Номер = СокрЛП(ПараметрыДляВставки.Номер);
		// } RGS LFedotova 23.12.2017 22:22:16 - вопрос SLI-0007442
		...
		Шаблон = ВернутьСтр("ru = '[НазваниеПечатнойФормы] № [Номер] от [Дата]'");
	Иначе
		...
		Шаблон = ВернутьСтр("ru = '[НазваниеПечатнойФормы] - [ПредставлениеОбъекта] - [ТекущаяДата]'");
	КонецЕсли;
...
Функция ПолучитьИмяФайлаДляАрхива()
	...
	Для Каждого НастройкаПечатнойФормы Из НастройкиПечатныхФорм Цикл
		...
		Иначе
			Результат = ВернутьСтр("ru = 'Документы'");
			Прервать;
		КонецЕсли;
	КонецЦикла;
...
Процедура СохранитьПечатныеФормыВПапку(СписокФайловВоВременномХранилище, Знач Папка = "")
	...
	Состояние(ВернутьСтр("ru = 'Сохранение успешно завершено'"), , ВернутьСтр("ru = 'в папку:'") + " " + Папка);
	...
	Если ОбщегоНазначения.ПодсистемаСуществует("СтандартныеПодсистемы.ПрисоединенныеФайлы") Тогда
		...
		Для Каждого Файл Из ФайлыВоВременномХранилище Цикл
			Результат.Добавить(МодульПрисоединенныеФайлы.ДобавитьФайл(ОбъектДляПрисоединения, 
				Файл.Представление, , , , Файл.АдресВоВременномХранилище, , ВернутьСтр("ru = 'Печатная форма'")));
		КонецЦикла;
	КонецЕсли;
	Возврат Результат;
КонецФункции
...
Процедура ОткрытьМакетДляРедактирования()
	...
	ОтобразитьСостояниеТекущейПечатнойФормы(ВернутьСтр("ru = 'Макет редактируется'"));
...
Процедура ПереформироватьПечатнуюФорму(ИмяМакета, ИмяРеквизита)
	...
	Если Отказ Тогда
		ВызватьИсключение ВернутьСтр("ru = 'Печатная форма не была переформирована.'");
	КонецЕсли;
