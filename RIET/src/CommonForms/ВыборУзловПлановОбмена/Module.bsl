...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если РежимПодбора Тогда
		Заголовок = ВернутьСтр("ru = 'Подбор узлов планов обмена'");
	КонецЕсли;
...
Процедура Выбрать(Команда)
	...
	Иначе
		...
		Если ТекущиеДанные = Неопределено Тогда
			ПоказатьПредупреждение(, ВернутьСтр("ru = 'Узел не выбран.'"));
		Иначе
...
Процедура ДобавитьИспользуемыйПланОбмена(ОбъектМетаданных)
	...
	Если Параметры.ВыбиратьВсеУзлы Тогда
		...
		НоваяСтрока.УзелПредставление       = ВернутьСтр("ru = '<Все информационные базы>'");
	КонецЕсли;
