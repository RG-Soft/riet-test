...
Функция РезультатПроверкиПараметровДоступа()
	...
	Если НЕ ПроверкаКонтрагентов.ЕстьДоступКВебСервисуФНС() Тогда
		ТекстПредупреждения = ВернутьСтр("ru = 'Доступ к веб-сервису отсутствует'");
		Возврат ТекстПредупреждения;
	КонецЕсли;
	
	ТекстПредупреждения = ВернутьСтр("ru = 'Проверка доступа к веб-сервису успешно пройдена!'");
	Возврат ТекстПредупреждения;
	
КонецФункции

&НаКлиенте
Процедура ИспользоватьСервисРасширеннаяПодсказкаОбработкаНавигационнойСсылки(Элемент, НавигационнаяСсылкаФорматированнойСтроки, СтандартнаяОбработка)
