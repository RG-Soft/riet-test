...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	// { RGS LFedotova 04.03.2018 23:41:46 - вопрос SLI-0007489
	СтруктураРеквизитовФормы.Организация             = Параметры.Организация;
	// } RGS LFedotova 04.03.2018 23:42:32 - вопрос SLI-0007489 
...
Процедура СформироватьСтруктуруРеквизитовФормы()
	...
	//  RGS EKoshkina - Обновление на 3.0.43.50 - перенос из предыдущей версии вопрос SLI-0007489
...
Функция Сохранить(Автосохранение = Ложь, КодИФНС, КПП, Вариант, ВидДокументаНомерКорректировкиИзменен = Неопределено, НаимТекРаздела) Экспорт
	...
	// { RGS LFedotova 03.03.2018 14:25:37 - вопрос SLI-0007489
	СтруктураРеквизитовФормы.мСохраненныйДок.Подразделение            = СтруктураРеквизитовФормы.Подразделение;
	// } RGS LFedotova 03.03.2018 14:26:18 - вопрос SLI-0007489
...
Процедура ВосстановитьСохраненныеДанные()
	...
	// { RGS LFedotova 03.03.2018 14:30:30 - вопрос SLI-0007489
	Если НЕ ЗначениеЗаполнено(СтруктураРеквизитовФормы.Подразделение) Тогда
		СтруктураРеквизитовФормы.Подразделение = СтруктураРеквизитовФормы.мСохраненныйДок.Подразделение;
	КонецЕсли;
	Подразделение = СтруктураРеквизитовФормы.мСохраненныйДок.Подразделение;
	// } RGS LFedotova 03.03.2018 14:31:04 - вопрос SLI-0007489 
...
Функция ЗаполнитьАвтоНаСервере()
	...
	// { RGS LFedotova 03.03.2018 16:05:58 - вопрос SLI-0007489
	ПараметрыОтчета.Вставить("Подразделение", 			     СтруктураРеквизитовФормы.Подразделение);
	// } RGS LFedotova 03.03.2018 16:06:03 - вопрос SLI-0007489
...
Процедура Расшифровать(Команда)
	...
	// { RGS LFedotova 12.02.2018 17:20:12 - вопрос SLI-0007489
	...
	// } RGS LFedotova 12.02.2018 17:24:00 - вопрос SLI-0007489
КонецПроцедуры
...
Процедура ПроверитьКС_1_19(СтруктураРезультата)
	...
	ОписаниеКС     = НСтр("ru='Р.2 ст. 141 = сумма р.2.1. ст. 050 всех блоков строк 010 - 050'");
	...
	СобратьМассивПоказателей(МассивКБК, СтраницыРаздела2, Р2Ст11Гр00.Имя);
	МассивОКТМО_КБК = Новый Массив;
	СобратьМассивПарныхПоказателей(МассивОКТМО_КБК, СтраницыРаздела2, Р2Ст10Гр01.Имя, Р2Ст11Гр00.Имя);
	Для Каждого ОКТМО Из МассивОКТМО Цикл
		Для Каждого КБК Из МассивКБК Цикл
			Для Каждого СтраницаР2 Из СтраницыРаздела2 Цикл
				...
				КонецЕсли;
				ВставитьЗначениеПоказателя(ДанныеСтраницыР2, Р2Ст11Гр00);
				Если Р2Ст11Гр00.Значение <> КБК Тогда
					Продолжить;
				КонецЕсли;
				СчетчикФормулДляРасшифровок = СчетчикФормулДляРасшифровок + 1;
				ЗарегистрироватьФормулуДляРасшифровки(ОписаниеКС);
				ВнестиПоказательВРасшифровку(Р2Ст10Гр01, "Раздел2", НомерСтраницыР2Стр);
				ВнестиПоказательВРасшифровку(Р2Ст11Гр00, "Раздел2", НомерСтраницыР2Стр);
				ВставитьЗначениеПоказателя(ДанныеСтраницыР2, Р2Ст141Гр03);
				ВнестиПоказательВРасшифровку(Р2Ст141Гр03, "Раздел2", НомерСтраницыР2Стр);
				Погрешность = 0;
				СуммаР21Ст50Гр03 = 0;
				СуммаР21Ст50Гр03Стр = "";
				Для Каждого СтраницаР21 Из СтраницыРаздела21 Цикл
					...
					Если Р21Ст10Гр00.Значение <> ОКТМО Тогда
						Продолжить;
					КонецЕсли;
					ВставитьЗначениеПоказателя(ДанныеСтраницыР21, Р21Ст11Гр00);
					Если Р21Ст11Гр00.Значение <> КБК Тогда
						Продолжить;
					КонецЕсли;
				...
				КС = ПоказателиКС(Р2Ст141Гр03.Значение, СуммаР21Ст50Гр03);
				Расшифровка = КС.ЛеваяЧастьСтр + " = " + РасшифровкаВСтр(СуммаР21Ст50Гр03Стр) + " Итог [" + КС.ПраваяЧастьСтр
					+ "] (±" + ПогрешностьСтр + ")";
...
Процедура ПроверитьКС_1_20(СтруктураРезультата)
	...
	Для Каждого СтраницаР3 Из СтраницыРаздела3 Цикл
		...
		Если УсловиеВыполнено Тогда
			Продолжить;
		КонецЕсли;
		УсловиеВыполнено = (Р3Ст50ч.Значение = 0 ИЛИ Р3Ст50з.Значение = 0);
		Если УсловиеВыполнено Тогда
			Продолжить;
		КонецЕсли;
		СчетчикФормулДляРасшифровок = СчетчикФормулДляРасшифровок + 1;
...
Процедура ПроверитьКС_1_21(СтруктураРезультата)
	...
	Для Каждого СтраницаР3 Из СтраницыРаздела3 Цикл
		...
		Если УсловиеВыполнено Тогда
			Продолжить;
		КонецЕсли;
		УсловиеВыполнено = (ЗначениеЗаполнено(Р3Ст50ч.Значение) И ЗначениеЗаполнено(Р3Ст50з.Значение));
...
Функция ПоказательПоСтроке(УсловныйКодПоказателя)
	...
	ИначеЕсли УсловныйКодПоказателя = "Р21Ст10Гр00" Тогда
		...
		Результат.Вставить("Представление", НСтр("ru='Код по ОКТМО (%1 раздела 21)'"));
		
	ИначеЕсли УсловныйКодПоказателя = "Р21Ст11Гр00" Тогда
		Результат.Вставить("Имя", "П000210001100");
		Результат.Вставить("Представление", НСтр("ru='Код бюджетной классификации (%1 раздела 21)'"));
		
	ИначеЕсли УсловныйКодПоказателя = "Р21Ст50Гр03" Тогда
		Результат.Вставить("Имя", "П000210005003");
		Результат.Вставить("Представление", НСтр("ru='Остаточная стоимость основных средств на 31.12 (строка 050%1 раздела 21)'"));
		Результат.Вставить("ИзменяемаяРазмерность", Истина);
...
&НаСервере
Процедура СобратьМассивПарныхПоказателей(МассивПоказателей, Лист, Показатель1, Показатель2)
	ТаблицаПоказателей = Новый ТаблицаЗначений;
	ТаблицаПоказателей.Колонки.Добавить("ОКТМО");
	ТаблицаПоказателей.Колонки.Добавить("КБК");
	Для Каждого Страница Из Лист Цикл
		ДанныеСтраницы = Страница.Данные;
		ЗначениеПоказателя1 = ДанныеСтраницы[СтрЗаменить(Показатель1, "_%1", "")];
		ЗначениеПоказателя2 = ДанныеСтраницы[СтрЗаменить(Показатель2, "_%1", "")];
		Если ЗначениеПоказателя1 <> "" И ЗначениеПоказателя2 <> "" Тогда
			НоваяСтрока = ТаблицаПоказателей.Добавить();
			НоваяСтрока.ОКТМО = ЗначениеПоказателя1;
			НоваяСтрока.КБК = ЗначениеПоказателя2;
		КонецЕсли;
	КонецЦикла;
	ТаблицаПоказателей.Свернуть("ОКТМО,КБК");
	
	ТаблицаПоказателей2 = Новый ТаблицаЗначений;
	ТаблицаПоказателей2.Колонки.Добавить("ОКТМО");
	ТаблицаПоказателей2.Колонки.Добавить("КБК");
	Для Каждого Страница Из Лист Цикл
		ДанныеСтраницы = Страница.Данные;
		ЗначениеПоказателя1 = ДанныеСтраницы[СтрЗаменить(Показатель1, "_%1", "")];
		ЗначениеПоказателя2 = ДанныеСтраницы[СтрЗаменить(Показатель2, "_%1", "")];
		Если ЗначениеПоказателя1 <> "" И ЗначениеПоказателя2 <> "" Тогда
			СтруктураПоиска = Новый Структура("ОКТМО,КБК", ЗначениеПоказателя1, ЗначениеПоказателя2);
			Если ТаблицаПоказателей2.НайтиСтроки(СтруктураПоиска).Количество() = 0 Тогда
				НоваяСтрока = ТаблицаПоказателей2.Добавить();
				НоваяСтрока.ОКТМО = ЗначениеПоказателя1;
				НоваяСтрока.КБК = ЗначениеПоказателя2;
			КонецЕсли;
		КонецЕсли;
...
Процедура ЗаполнитьСведенияОбОрганизацииНаСервере(ВПрограммеИзмененаДатаПодписи = Ложь)
	...
	Если НЕ ВПрограммеИзмененаДатаПодписи Тогда
		...
		// { RGS MYurkevich 29.01.2015 10:26:19 - Обновление на 3.0.37.38 - аналогично предыдущей версии вопрос SLI-0007489
		...
		// { RGS LFedotova 04.03.2018 22:30:38 - вопрос SLI-0007489
		Если СведенияОбОрганизации.Количество() = 0 Тогда
			Возврат;
		КонецЕсли;  
		// } RGS LFedotova 04.03.2018 22:30:59 - вопрос SLI-0007489
...
Процедура ПередЗакрытием(Отказ, ЗавершениеРаботы, ТекстПредупреждения, СтандартнаяОбработка)
	...
	// { RGS LFedotova 13.02.2018 22:49:40 - вопрос SLI-0007489
	...
	// } RGS LFedotova 13.02.2018 22:50:49 - вопрос SLI-0007489 
