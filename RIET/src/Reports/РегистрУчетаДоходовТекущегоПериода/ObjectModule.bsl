...
Функция ВидДоходаПоОперации(Счет, Субконто1, Субконто2, Субконто3, ВидДоходов, Контрагент, Договор)
	...
	Если      Счет = Счет90_01_1 ИЛИ Счет.ПринадлежитЭлементу(Счет90_01_1) Тогда
		ВидДохода   = "Выручка от реализации товаров (работ, услуг) собственного производства";
		ОбъектУчета = Строка(Субконто2);

	ИначеЕсли Счет = Счет90_01_2 ИЛИ Счет.ПринадлежитЭлементу(Счет90_01_2) Тогда
		ВидДохода   = "Выручка от реализации покупных товаров";
		ОбъектУчета = Строка(Субконто2);

	ИначеЕсли Счет = Счет91_01_1 ИЛИ Счет.ПринадлежитЭлементу(Счет91_01_1) Тогда
		ВидДохода   = "Выручка от реализации ОС";
		ОбъектУчета = Строка(Субконто2);

	ИначеЕсли Счет = Счет91_01_2 ИЛИ Счет.ПринадлежитЭлементу(Счет91_01_2) Тогда
		ВидДохода   = "Выручка от реализации НМА";
		ОбъектУчета = Строка(Субконто2);

	ИначеЕсли Счет = Счет91_01_3 ИЛИ Счет.ПринадлежитЭлементу(Счет91_01_3) Тогда
		ВидДохода   = "Выручка от реализации прочего имущества";
		ОбъектУчета = Строка(Субконто2);

	ИначеЕсли Счет = Счет91_01_4 ИЛИ Счет.ПринадлежитЭлементу(Счет91_01_4) Тогда
		...
		КонецЕсли;
		Если НЕ ОбщегоНазначения.ЗначениеНеЗаполнено(Контрагент) тогда
			ОбъектУчета = Контрагент + " (" + Договор + ")";
		КонецЕсли;

	ИначеЕсли Счет = Счет91_01_5 ИЛИ Счет.ПринадлежитЭлементу(Счет91_01_5) Тогда
		ВидДохода   = "Выручка от реализации ЦБ";
		ОбъектУчета = Строка(Субконто2);

	ИначеЕсли Счет = Счет91_01_6 ИЛИ Счет.ПринадлежитЭлементу(Счет91_01_6) Тогда
		...
		ИначеЕсли ВидДоходов <> Перечисления.ВидыПрочихДоходовИРасходов.ДоходыРасходыНеУчитываемыеВЦеляхНалогообложения Тогда
