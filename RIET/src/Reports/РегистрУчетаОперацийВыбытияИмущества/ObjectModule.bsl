...
Функция ВидДоходаПоОперации(Счет, Субконто1, Субконто2, Субконто3, Выборка)
	...
	ИначеЕсли Счет.ПринадлежитЭлементу(Счет91_01_5) ИЛИ Счет = Счет91_01_5 Тогда
		...
		Если Выборка.СубконтоКт1.ВидРасходовНУ = Перечисления.ВидыПрочихДоходовИРасходов.ДоходыРасходыСвязанныеСРеализациейПраваТребованияКакОказанияФинансовыхУслуг Тогда
			ВидДохода = "Выручка от реализации прав требования";

		ИначеЕсли Выборка.СубконтоКт1.ВидРасходовНУ = Перечисления.ВидыПрочихДоходовИРасходов.ДоходыРасходыСвязанныеСРеализациейПраваТребованияДоНаступленияСрокаПлатежа Тогда
			ВидДохода = "Выручка от уступки права до наступления срока платежа";

		ИначеЕсли Выборка.СубконтоКт1.ВидРасходовНУ = Перечисления.ВидыПрочихДоходовИРасходов.ДоходыРасходыСвязанныеСРеализациейПраваТребованияПослеНаступленияСрокаПлатежа Тогда
...
Процедура СформироватьОтчет(ДокументРезультат, ПоказыватьЗаголовок, ВысотаЗаголовка, ТолькоЗаголовок = Ложь) Экспорт
	...
	Пока Выборка.Следующий() Цикл
		...
		Если ВидРасходаСоответствуетКритериюОтбора(СтруктураДохода.ВидДохода) Тогда
			...
			//Если  ТипЗнч(Выборка.Регистратор) = Тип("ДокументСсылка.ВозвратТоваровОтПокупателя") ИЛИ СтруктураДохода.ВидДохода = "Доход не возникает" Тогда
			Если  СтруктураДохода.ВидДохода = "Доход не возникает" Тогда
