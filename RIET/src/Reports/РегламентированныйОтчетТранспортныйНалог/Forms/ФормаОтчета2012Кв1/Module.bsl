...
Процедура УстановитьДоступностьОбластей(Форма, ИмяСтраницыПанели = "")
	...
	ИначеЕсли ИмяСтраницыПанели = "Раздел2" Тогда
		...
		ОбрабатываемаяОбласть = Форма.ТабличныйДокумент.Области.П000020021003_Основание;
		ОбрабатываемаяОбласть2 = Форма.ТабличныйДокумент.Области.П000020022003;
		Если ПустаяСтрока(ОпределяющаяОбласть.Значение) Тогда
			ЗапретитьВводДанныхВОбласть(Форма, ОбрабатываемаяОбласть);
			ЗапретитьВводДанныхВОбласть(Форма, ОбрабатываемаяОбласть2);
		Иначе
...
Процедура ЗаполнитьСтруктуруЗависимыхЯчеекДляАвторасчета()
	...
	СтруктураЗависимыхЯчеекДляАвторасчета.Вставить("П000010003003_3", "П000010002103_3;П000010002303_3;П000010002503_3;П000010002703_3");
	...
	СтруктураЗависимыхЯчеекДляАвторасчета.Вставить("П000020023003", "П000020007003;П000020013003;П000020011003;П000020017003_КодЛьготы;П000020007003;П000020013003;П000020016003;П000020015003;П000020020003;П000020022003");
	...
	Для Каждого СтрПоказатели Из Форма.мТаблицаСоставПоказателей Цикл
		...
		Если (ТипДанныхПоказателя = "Е") И (ПустаяСтрока(КодПоказателяПоСтруктуре)) Тогда
			...
			Если ПризнМногострочность <> "/" Тогда
				...
				ФорматируемаяОбласть.Формат = ?(ОтображатьНольКакЧисло(ИмяПоказателя), СтрокаФорматаНольЦифрой, Форма.СтруктураРеквизитовФормы.мСтрокаФормата);
			КонецЕсли;
		КонецЕсли;
	КонецЦикла;			
КонецПроцедуры
...
Функция ОтображатьНольКакЧисло(ИмяПоказателя)
	...
	Результат = СтрНайти(ИмяПоказателя, "П000010002303") = 1
	        ИЛИ СтрНайти(ИмяПоказателя, "П000010002503") = 1
	        ИЛИ СтрНайти(ИмяПоказателя, "П000010002703") = 1;
...
Процедура СформироватьРаздел1()	
	...
	Для Инд = 0 По КоличествоСтраниц - 1 Цикл
		...
		ОКАТО = ТаблицаСтраницРаздела2[Инд].Данные[0].Значение["П000020002003"];
