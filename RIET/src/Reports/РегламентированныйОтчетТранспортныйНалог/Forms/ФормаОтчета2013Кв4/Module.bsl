...
Процедура СформироватьСпискиВыбора()
	...
	МакетСоставаПоказателей = Отчеты[Сред(Лев(ЭтаФорма.ИмяФормы, СтрНайти(ЭтаФорма.ИмяФормы, ".Форма.") - 1), 7)].ПолучитьМакет("СпискиВыбора2013Кв4");
...
Процедура СобратьДанные(Команда)
	...
	СохранитьДанныеРаздела(Элементы.РазделыОтчета.ТекущиеДанные.КолонкаРазделыОтчетаСокрНаим, , Ложь);
...
Процедура ВывестиНаПечатьЛист(Парам1, ОбщиеРеквизитыРазделов, СтрПоля = Неопределено)
	...
	ПрефиксИмениМакетов = "ПечатныйБланк2013Кв4_";
...
Функция ПроверитьВозможностьВыгрузки(ПараметрыВыгрузки)
	...
	//   - нет ни одного раздела с заполненным кодом ОКТМО;
	//   - код ОКТМО задан неверно;
	//   - код ОКТМО дублируетя;
	//   - не задан код ОКТМО при заполненных суммах налогов и платежей.
	Раздел1 = мДанныеОтчета.Раздел1;
	...
	ОКТМОРаздела1 = Новый ТаблицаЗначений;
	ОКТМОРаздела1.Колонки.Добавить("КодОКТМО");
	...
	Для Каждого Лист Из Раздел1 Цикл
		...
		Для Инд = 1 По КоличествоСходныхБлоковРаздела1 Цикл
			...
			ИмяОбластиОКТМО = "П000010002003" + ПостфиксБлока;
			...
			ЕстьЗаполненныйБлокРаздела1 = (ЕстьЗаполненныйБлокРаздела1 ИЛИ ЗаданоОКТМО ИЛИ ЗаданыСуммыНалогаИПлатежей);
			
			Если ЗаданоОКТМО Тогда
				НоваяСтрокаОКТМО = ОКТМОРаздела1.Добавить();
				НоваяСтрокаОКТМО.КодОКТМО = ОКТМО;
				
				ДлинаОКТМО = СтрДлина(ОКТМО);
				Если ДлинаОКТМО <> 11 И ДлинаОКТМО <> 8 Тогда
					РегламентированнаяОтчетность.СообщитьВТаблицуСообщений(ЭтаФорма, "В разделе 1 на странице " + НомерСтраницы + " неверно задан код по ОКТМО!", "Раздел1", ИмяОбластиОКТМО, НомерСтраницы);
					СтатусПроверки = Ложь;
				КонецЕсли;
			Иначе
				Если ЗаданыСуммыНалогаИПлатежей Тогда
					РегламентированнаяОтчетность.СообщитьВТаблицуСообщений(ЭтаФорма, "В разделе 1 на странице " + НомерСтраницы + " не задан код по ОКТМО!", "Раздел1", ИмяОбластиОКТМО, НомерСтраницы);
					СтатусПроверки = Ложь;
				КонецЕсли;
			КонецЕсли;
	...
	ТаблицаПроверкиУникальностиОКТМО = ОКТМОРаздела1.Скопировать();
	ТаблицаПроверкиУникальностиОКТМО.Колонки.Добавить("Количество", Новый ОписаниеТипов("Число", , , Новый КвалификаторыЧисла(1)));
	ТаблицаПроверкиУникальностиОКТМО.ЗаполнитьЗначения(1, "Количество");
	ТаблицаПроверкиУникальностиОКТМО.Свернуть("КодОКТМО", "Количество");
	
	Для Каждого ПроверяемыйОКТМО Из ТаблицаПроверкиУникальностиОКТМО Цикл
		Если ПроверяемыйОКТМО.Количество > 1 Тогда
			РегламентированнаяОтчетность.СообщитьВТаблицуСообщений(ЭтаФорма, "В разделе 1 обнаружен повторяющийся код ОКТМО " + ПроверяемыйОКТМО.КодОКТМО + "!", "Раздел1", , 1);
			СтатусПроверки = Ложь;
		КонецЕсли;
	КонецЦикла;
	...
	// Возможные проблемы:
	//   - не задан код ОКТМО при заполненных полях раздела;
	//   - код ОКТМО задан неверно;
	//   - код ОКТМО раздела 2 не соответствует ни одному коду ОКТМО из раздела 1;
	...
	Для Каждого Лист Из Раздел2 Цикл
		...
		Если ЗаданоОКТМО Тогда
			ДлинаОКТМО = СтрДлина(ОКТМО);
			Если ДлинаОКТМО <> 11 И ДлинаОКТМО <> 8 Тогда
				РегламентированнаяОтчетность.СообщитьВТаблицуСообщений(ЭтаФорма, "В разделе 2 на странице " + НомерСтраницы + " неверно задан код по ОКТМО!", "Раздел2", ИмяОбластиОКТМО, НомерСтраницы);
				СтатусПроверки = Ложь;
			КонецЕсли;
			
			ОКТМОНайденВРазделе1 = (ОКТМОРаздела1.Найти(ОКТМО, "КодОКТМО") <> Неопределено);
			
			Если НЕ ОКТМОНайденВРазделе1 Тогда
				РегламентированнаяОтчетность.СообщитьВТаблицуСообщений(ЭтаФорма, "Для значения кода по ОКТМО на странице " + НомерСтраницы + " раздела 2 не найдено соответствующее значение кода по ОКТМО в разделе 1!", "Раздел2", ИмяОбластиОКТМО, НомерСтраницы);
				СтатусПроверки = Ложь;
			КонецЕсли;
			
		Иначе
			Если ЛистЗаполнен Тогда
				РегламентированнаяОтчетность.СообщитьВТаблицуСообщений(ЭтаФорма, "В разделе 2 на странице " + НомерСтраницы + " не задан код по ОКТМО!", "Раздел2", ИмяОбластиОКТМО, НомерСтраницы);
				СтатусПроверки = Ложь;
			КонецЕсли;
		КонецЕсли;
...
Процедура ПроверитьКС_1_1(СтруктураРезультата)
	...
	ОписаниеКС     = НСтр("ru='Исчисленная сумма налога, подлежащая уплате в бюджет за налоговый период (строка 021 Раздел 1) должна быть равна сумме всех значений по строкам 230 Раздела 2 по соответствующим кодам ОКТМО'");
	...
	Для Каждого СтраницаР1 Из Раздел1 Цикл
		...
		Для Инд = 1 По 3 Цикл
			...
			Для Каждого СтраницаР2 Из Раздел2 Цикл
				...
				ВставитьЗначениеМногострочногоПоказателя(ДанныеСтраницыР2, Р2Стр230, Инд);
				...
				КонтрольноеЗначение = КонтрольноеЗначение + Р2Стр230.Значение;
...
Процедура ПроверитьКС_1_3(СтруктураРезультата)
	...
	Для Каждого Страница Из Раздел1 Цикл
		...
		Для Инд = 1 По 3 Цикл
			...
			Расшифровка = Р1Стр040.ЗначениеСтр + " = ("
			            + Р1Стр023.ЗначениеСтр + " + " + Р1Стр025.ЗначениеСтр + " + " + Р1Стр027.ЗначениеСтр
...
Процедура ПроверитьКС_1_4(СтруктураРезультата)
	...
	ОписаниеКС     = НСтр("ru='Код налоговой льготы в виде освобождения от налогообложения (строка 170 Раздела 2) должен быть заполнен, если значение строки 180 Раздела 2 больше нуля'");
	...
	Для Каждого Страница Из Раздел2 Цикл
		...
		ПроверятьСоотношение = (Р2Стр180.Значение > 0);
		Если НЕ ПроверятьСоотношение Тогда
			Продолжить;
		КонецЕсли;
		...
		Если НЕ ЗначениеЗаполнено(Р2Стр170.Значение) Тогда
			ЭтоОшибка = Истина;
			ОбщееЧислоОшибок = ОбщееЧислоОшибок + 1;
		Иначе
			ЭтоОшибка = Ложь;
		...
		Расшифровка = "Код льготы заполнен ("""+ Р2Стр170.ЗначениеСтр + """)"
		            + " при условии, что " + Р2Стр180.ЗначениеСтр + " > 0";
...
Процедура ПроверитьКС_1_5(СтруктураРезультата)
	...
	ОписаниеКС     = НСтр("ru='Сумма налоговой льготы (строка 180 Раздела 2) должна быть больше нуля, если значение строки 170 Раздела 2 заполнено'");
	...
	Для Каждого Страница Из Раздел2 Цикл
		...
		Если НЕ Р2Стр180.Значение > 0 Тогда
			ЭтоОшибка = Истина;
			ОбщееЧислоОшибок = ОбщееЧислоОшибок + 1;
		Иначе
			ЭтоОшибка = Ложь;
		...
		Расшифровка = Р2Стр180.ЗначениеСтр + " > 0"
		            + " при условии, что код льготы заполнен ("""+ Р2Стр170.ЗначениеСтр + """)";
...
Процедура ПроверитьКС_1_6(СтруктураРезультата)
	...
	ОписаниеКС     = НСтр("ru='Код налоговой льготы в виде уменьшения суммы налога (строка 190 Раздела 2) должен быть заполнен, если значение строки 200 Раздела 2 больше нуля'");
...
Процедура ПроверитьКС_1_8(СтруктураРезультата)
	...
	ОписаниеКС     = НСтр("ru='Код налоговой льготы в виде снижения налоговой ставки (строка 210 Раздела 2) должен быть заполнен, если значение строки 220 Раздела 2 больше нуля'");
...
Процедура ПроверитьКС_1_10(СтруктураРезультата)
	...
	ОписаниеКС     = НСтр("ru='Количество полных месяцев использования льготы (строка 150 Раздела 2) должно быть заполнено одновременно с показателями кода налоговой льготы ( строка 170 или строка 190 или строка 210 Раздела 2 ) и суммы налоговой льготы (строка 180 или строка 200 или строка 220 Раздела 2)'");
	...
	Для Каждого Страница Из Раздел2 Цикл
		...
		НомерСтраницыСтр = ЧислоСтрЧГ0(Раздел2.Индекс(Страница) + 1);
			...
			ПроверятьСоотношение = ЗначениеЗаполнено(Р2Стр150.Значение)
			                   ИЛИ ЗначениеЗаполнено(Р2Стр170.Значение)
			                   ИЛИ ЗначениеЗаполнено(Р2Стр180.Значение)
		...
		СоотношениеВыполняется = ЗначениеЗаполнено(Р2Стр150.Значение)
		                       И ((ЗначениеЗаполнено(Р2Стр170.Значение) И ЗначениеЗаполнено(Р2Стр180.Значение))
		                       ИЛИ (ЗначениеЗаполнено(Р2Стр190.Значение) И ЗначениеЗаполнено(Р2Стр200.Значение))
		...
		КонецЕсли;
			...
			Расшифровка = Р2Стр150.ЗначениеСтр + " <> 0 и "
			            + " (код льготы в строке 170 заполнен (""" + Р2Стр170.ЗначениеСтр
			            + """) или код льготы в строке 190 заполнен (""" + Р2Стр190.ЗначениеСтр
			            + """) или код льготы в строке 210 заполнен (""" + Р2Стр210.ЗначениеСтр + """)"
			            + " и (" + Р2Стр180.ЗначениеСтр + " <> 0 или " + Р2Стр200.ЗначениеСтр + " <> 0 или " + Р2Стр220.ЗначениеСтр + " <> 0)";
...
Процедура ПроверитьКС_1_12(СтруктураРезультата)
	...
	ОписаниеКС     = НСтр("ru='Сумма налоговой льготы (строка 180 Раздела 2) должна быть равна строка 070 Раздела 2 * строка 130 Раздела 2 * строка 160 Раздела 2, если заполнен код льготы (строка 170 Раздела 2)'");
	...
	Для Каждого Страница Из Раздел2 Цикл
		...
		КонтрольноеЗначение = Р2Стр070.Значение * Р2Стр130.Значение * Р2Стр160.Значение;
		...
		Расшифровка = Р2Стр180.ЗначениеСтр + " = " + Р2Стр070.ЗначениеСтр + " * " + Р2Стр130.ЗначениеСтр + " * " + Р2Стр160.Значение
		            + " Итог [" + КонтрольноеЗначение + "]"
		            + " при условии, что код льготы заполнен ("""+ Р2Стр170.ЗначениеСтр + """)";
...
Процедура ПроверитьКС_1_13(СтруктураРезультата)
	...
	ОписаниеКС     = НСтр("ru='Коэффициент Кв (строка 120 Раздела 2) должен быть равен 1, если строка 110 Раздела 2 = 12 (транспортное средство использовалось в течение всего налогового периода)'");
	...
	Для Каждого Страница Из Раздел2 Цикл
		...
		Расшифровка = Р2Стр120.ЗначениеСтр + " = " + КонтрольноеЗначение
...
Процедура ПроверитьКС_1_14(СтруктураРезультата)
	...
	ОписаниеКС     = НСтр("ru='Сумма исчисленного налога (строка 140 Раздела 2) должна быть равна произведению строк 070, 130, 120 Раздела 2'");
	...
	Для Каждого Страница Из Раздел2 Цикл
		...
		ЛистЗаполнен
			...
			ИЛИ ЗначениеСуммыЗаполнено(ДанныеСтраницы.П000020014003)
			ИЛИ ЗначениеЗаполнено(ДанныеСтраницы.П000020015003)
			ИЛИ ЗначениеЗаполнено(ДанныеСтраницы.П000020016003)
			ИЛИ ЗначениеЗаполнено(ДанныеСтраницы.П000020017003_КодЛьготы)
			ИЛИ ЗначениеСуммыЗаполнено(ДанныеСтраницы.П000020018003)
		...
		КонтрольноеЗначение = Окр(Р2Стр070.Значение * Р2Стр130.Значение * Р2Стр120.Значение, 2);
		...
		Расшифровка = Р2Стр140.ЗначениеСтр + " = " + Р2Стр070.ЗначениеСтр + " * " + Р2Стр130.ЗначениеСтр + " * " + Р2Стр120.ЗначениеСтр
		            + " Итог [" + КонтрольноеЗначение + "]";
...
Процедура ПроверитьКС_1_18(СтруктураРезультата)
	...
	ОписаниеКС     = НСтр("ru='Сумма налоговой льготы (строка 200 Раздела 2) должна быть равна произведению значений строк 070, 130, 160, 190а  Раздела 2 / 100%, если заполнен код льготы (строка 190 Раздела 2)'");
	...
	Р2Стр160  = ПоказательПоСтроке("Р2Стр160");
	Р2Стр190  = ПоказательПоСтроке("Р2Стр190");
	Р2Стр190а = ПоказательПоСтроке("Р2Стр190а");
	Р2Стр200  = ПоказательПоСтроке("Р2Стр200");
	...
	Для Каждого Страница Из Раздел2 Цикл
		...
		ВставитьЗначениеПоказателя(ДанныеСтраницы, Р2Стр160);
		ВставитьЗначениеПоказателя(ДанныеСтраницы, Р2Стр190);
		ВставитьЗначениеПоказателя(ДанныеСтраницы, Р2Стр190а);
		ВставитьЗначениеПоказателя(ДанныеСтраницы, Р2Стр200);
		...
		ВнестиПоказательВРасшифровку(Р2Стр130, "Раздел2", НомерСтраницыСтр);
		ВнестиПоказательВРасшифровку(Р2Стр160, "Раздел2", НомерСтраницыСтр);
		ВнестиПоказательВРасшифровку(Р2Стр190а, "Раздел2", НомерСтраницыСтр);
		ВнестиПоказательВРасшифровку(Р2Стр190, "Раздел2", НомерСтраницыСтр);
		
		КонтрольноеЗначение = Р2Стр070.Значение * Р2Стр130.Значение * Р2Стр160.Значение * Р2Стр190а.Значение / 100;
		КонтрольноеЗначение = РегламентированнаяОтчетностьКлиентСервер.ОкруглитьЧислоПоФорматуДелителя(ЭтаФорма, Окр(КонтрольноеЗначение, 2));
		...
		Расшифровка = Р2Стр200.ЗначениеСтр + " = " + Р2Стр070.ЗначениеСтр + " * " + Р2Стр130.ЗначениеСтр
		            + " * " + Р2Стр160.ЗначениеСтр + " * " + Р2Стр190а.ЗначениеСтр + " / 100"
		            + " Итог [" + КонтрольноеЗначение + "]"
		            + " при условии, что код льготы заполнен ("""+ Р2Стр190.ЗначениеСтр + """)";
...
Процедура ПроверитьКС_1_19(СтруктураРезультата)
	...
	ОписаниеКС     = НСтр("ru='Сумма налоговой льготы в виде снижения ставки налога (строка 220 Раздела 2) должна быть равна (строка 070 Раздела 2 х строка 130 Раздела 2)  - (строка 070 Раздела 2 х строка 210а Раздела 2) x значение коэффициента Кл в строке 160, если заполнен код льготы (строка 210 Раздела 2)'");
	...
	Для Каждого Страница Из Раздел2 Цикл
		...
		КонтрольноеЗначение = Окр(Р2Стр070.Значение * (Р2Стр130.Значение - Р2Стр210а.Значение) * Р2Стр160.Значение, 2);
		КонтрольноеЗначение = РегламентированнаяОтчетностьКлиентСервер.ОкруглитьЧислоПоФорматуДелителя(ЭтаФорма, КонтрольноеЗначение);
		...
		Расшифровка = Р2Стр220.ЗначениеСтр + " = " + Р2Стр070.ЗначениеСтр + " * (" + Р2Стр130.ЗначениеСтр + " - " + Р2Стр210а.ЗначениеСтр + ")"
		            + " * " + Р2Стр160.ЗначениеСтр
		            + " Итог [" + КонтрольноеЗначение + "]"
		            + " при условии, что код льготы заполнен ("""+ Р2Стр210.ЗначениеСтр + """)";
...
Процедура ПроверитьКС_1_20(СтруктураРезультата)
	...
	ОписаниеКС     = НСтр("ru='Исчисленная сумма налога, подлежащая уплате в бюджет за налоговый период (строка 230 Раздела 2) должна быть равна разнице строк 140 и 180 Раздела 2, если строка 180 Раздела 2 (сумма налоговой льготы) больше нуля'");
	...
	Для Каждого Страница Из Раздел2 Цикл
		...
		КонтрольноеЗначение = Р2Стр140.Значение - Р2Стр180.Значение;
		...
		Расшифровка = Р2Стр230.ЗначениеСтр + " = " + Р2Стр140.ЗначениеСтр + " - " + Р2Стр180.ЗначениеСтр
		            + " Итог [" + КонтрольноеЗначение + "]"
		            + " при условии, что "+ Р2Стр180.ЗначениеСтр + " > 0";
...
Процедура ПроверитьКС_1_21(СтруктураРезультата)
	...
	ТекстФормулы   = НСтр("ru='Проверка раздела 2 (КС 1.21)'");
	ОписаниеКС     = НСтр("ru='Исчисленная сумма налога, подлежащая уплате в бюджет за налоговый период (строка 230 Раздела 2) должна быть равна разнице строк 140 и 200 Раздела 2, если строка 200 Раздела 2 (сумма налоговой льготы) больше нуля и при этом заполнен код льготы по строке 190 Раздела 2'");
	...
	Для Каждого Страница Из Раздел2 Цикл
		...
		ПроверятьСоотношение = (Р2Стр200.Значение > 0)
		                     И ЗначениеЗаполнено(Р2Стр190.Значение);
		Если НЕ ПроверятьСоотношение Тогда
			Продолжить;
		КонецЕсли;
		...
		КонтрольноеЗначение = Р2Стр140.Значение - Р2Стр200.Значение;
		...
		Расшифровка = Р2Стр230.ЗначениеСтр + " = " + Р2Стр140.ЗначениеСтр + " - " + Р2Стр200.ЗначениеСтр
		            + " Итог [" + КонтрольноеЗначение + "]"
		            + " при условии, что "+ Р2Стр200.ЗначениеСтр + " > 0"
		            + " и код льготы заполнен ("""+ Р2Стр190.ЗначениеСтр + """)";
...
Процедура ПроверитьКС_1_22(СтруктураРезультата)
	...
	ТекстФормулы   = НСтр("ru='Проверка раздела 2 (КС 1.22)'");
	ОписаниеКС     = НСтр("ru='Исчисленная сумма налога, подлежащая уплате в бюджет за налоговый период (строка 230 Раздела 2) должна быть равна разнице строк 140 и 220 Раздела 2, если строка 220 Раздела 2 (сумма налоговой льготы) больше нуля и при этом заполнен код льготы по строке 210 Раздела 2'");
	...
	Для Каждого Страница Из Раздел2 Цикл
		...
		ПроверятьСоотношение = (Р2Стр220.Значение > 0)
		                     И ЗначениеЗаполнено(Р2Стр210.Значение);
		Если НЕ ПроверятьСоотношение Тогда
			Продолжить;
		КонецЕсли;
		...
		КонтрольноеЗначение = Р2Стр140.Значение - Р2Стр220.Значение;
		...
		Расшифровка = Р2Стр230.ЗначениеСтр + " = " + Р2Стр140.ЗначениеСтр + " - " + Р2Стр220.ЗначениеСтр
		            + " Итог [" + КонтрольноеЗначение + "]"
		            + " при условии, что "+ Р2Стр220.ЗначениеСтр + " > 0"
		            + " и код льготы заполнен ("""+ Р2Стр210.ЗначениеСтр + """)";
...
Функция ПоказательПоСтроке(УсловныйКодПоказателя)
	...
	ИначеЕсли УсловныйКодПоказателя = "Р2Стр120" Тогда
		...
		Результат.Вставить("Представление", НСтр("ru='Коэффициент Кв (строка 120%1 раздела 2)'"));
		
	ИначеЕсли УсловныйКодПоказателя = "Р2Стр130" Тогда
		Результат.Вставить("Имя", "П000020013003");
		Результат.Вставить("Представление", НСтр("ru='Налоговая ставка (строка 130%1 раздела 2)'"));
		
	ИначеЕсли УсловныйКодПоказателя = "Р2Стр140" Тогда
		Результат.Вставить("Имя", "П000020014003");
		Результат.Вставить("Представление", НСтр("ru='Сумма исчисленного налога (руб.) (строка 140%1 раздела 2)'"));
		Результат.Вставить("ИзменяемаяРазмерность", Истина);
		
	ИначеЕсли УсловныйКодПоказателя = "Р2Стр150" Тогда
		Результат.Вставить("Имя", "П000020015003");
		Результат.Вставить("Представление", НСтр("ru='Количество полных месяцев использования льготы (строка 150%1 раздела 2)'"));
		
	ИначеЕсли УсловныйКодПоказателя = "Р2Стр160" Тогда
		Результат.Вставить("Имя", "П000020016003");
		Результат.Вставить("Представление", НСтр("ru='Коэффициент Кл (строка 160%1 раздела 2)'"));
		
	ИначеЕсли УсловныйКодПоказателя = "Р2Стр170" Тогда
		Результат.Вставить("Имя", "П000020017003_КодЛьготы");
		Результат.Вставить("Представление", НСтр("ru='Код налоговой льготы в виде освобождения от налогообложения (строка 170%1 раздела 2)'"));
		
	ИначеЕсли УсловныйКодПоказателя = "Р2Стр180" Тогда
		Результат.Вставить("Имя", "П000020018003");
		Результат.Вставить("Представление", НСтр("ru='Сумма налоговой льготы (руб.) (строка 180%1 раздела 2)'"));
		Результат.Вставить("ИзменяемаяРазмерность", Истина);
		
	ИначеЕсли УсловныйКодПоказателя = "Р2Стр190" Тогда
		Результат.Вставить("Имя", "П000020019003_КодЛьготы");
		Результат.Вставить("Представление", НСтр("ru='Код налоговой льготы в виде уменьшения суммы налога (строка 190%1 раздела 2)'"));
		
	ИначеЕсли УсловныйКодПоказателя = "Р2Стр190а" Тогда
		Результат.Вставить("Имя", "П000020019003_Процент");
		Результат.Вставить("Представление", НСтр("ru='Процент, уменьшающий исчисленную сумму налога (строка 190а%1 раздела 2)'"));
	...
	ИначеЕсли УсловныйКодПоказателя = "Р2Стр210" Тогда
		...
		Результат.Вставить("Представление", НСтр("ru='Код налоговой льготы в виде снижения налоговой ставки (строка 210%1 раздела 2)'"));
		
	ИначеЕсли УсловныйКодПоказателя = "Р2Стр210а" Тогда
		Результат.Вставить("Имя", "П000020021003_Ставка");
		Результат.Вставить("Представление", НСтр("ru='Пониженная налоговая ставка (строка 210а%1 раздела 2)'"));
	...
	ИначеЕсли УсловныйКодПоказателя = "Р2Стр230" Тогда
		...
		Результат.Вставить("Представление", НСтр("ru='Исчисленная сумма налога, подлежащая уплате в бюджет за налоговый период (руб.) (строка 230%1 раздела 2)'"));
		Результат.Вставить("ИзменяемаяРазмерность", Истина);
