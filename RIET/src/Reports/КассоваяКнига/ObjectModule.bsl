...
Процедура СформироватьОтчет(ДокументРезультат) Экспорт
	...
	|	КассовыеДокументы.Документ.Основание КАК Основание,
	|	ВЫБОР
	...
	СведенияОбОрганизации = КонтактнаяИнформация.СведенияОЮрФизЛице(Организация, КонецДня(ДатаКон));
	...
	Пока ПоПроводкам Или ПоДокументам  Цикл
		...
		Если ДатаЛиста >= ДатаНач Тогда
			...
			//добавил Трефиленков Дмитрий, РГ-Софт, 20 мая 2008 года
			Запрос = Новый Запрос("ВЫБРАТЬ
			                      ...
			                      |			И Субконто1.ВидДвиженияДенежныхСредств = &ОплатаТруда) КАК ХозрасчетныйОстатки");
			Запрос.УстановитьПараметр("Счет50", ПланыСчетов.Хозрасчетный.НайтиПоКоду("50"));
			Запрос.УстановитьПараметр("Организация", Организация);
			Запрос.УстановитьПараметр("Граница", Новый Граница(НачалоДня(ДатаЛиста),ВидГраницы.Исключая));
			Запрос.УстановитьПараметр("ОплатаТруда", Перечисления.ВидыДвиженийДенежныхСредств.ОплатаТруда);
			выборка = Запрос.Выполнить().Выбрать();
			Если Не Выборка.Следующий() ИЛИ выборка.суммаостаток = 0 Тогда
				ОбластьОстатокНаНДОтчет.Параметры.ЗП = "";
			Иначе
				ОбластьОстатокНаНДОтчет.Параметры.ЗП = Формат(выборка.суммаостаток,"ЧЦ=15; ЧДЦ=2; ЧРД==");		
			КонецЕсли;
			//конец добавления
...
//		ЛистовЗаГод = 1 стр
		...
		Если ПоДокументам И ВыборкаДокументовПоДням.День = ДатаЛиста Тогда
			...
			Пока ВыборкаДокументов.Следующий() Цикл
				...
				Если Не ВыборкаДокументов.Валюта = мВалютаРегламентированногоУчета Тогда
					Валюты.Добавить(ВыборкаДокументов.Валюта);
					НоваяСтрока.Валютный = Истина;
				Иначе
		...
		Для Каждого СтрокаТаблица ИЗ РабочаяТаблица Цикл
			...
			СтруктураКурсов = ОбщегоНазначения.ПолучитьКурсВалюты(СтрокаТаблица.Валюта,ДатаЛиста);
		...
		Для Каждого Валюта ИЗ Валюты Цикл
			...
			Если РабочаяТаблица.Найти(Валюта, "Валюта") = Неопределено Тогда
		...
		Для Каждого Документ Из ТаблицаДокументы Цикл
			...
			Если ЕстьВалюта Тогда
				...
				СтруктураКурсов = ОбщегоНазначения.ПолучитьКурсВалюты(Валюта,Документ.ДатаДок);
			...
			НомерДокПечатнойФормы = ОбщегоНазначения.ПолучитьНомерНаПечатьБезПрефикса(СтруктураНомерДок);
		...
		Для Каждого Строка ИЗ РабочаяТаблица Цикл
			...
			СтруктураКурсов = ОбщегоНазначения.ПолучитьКурсВалюты(Строка.Валюта, ДатаЛиста);
		...
		Если ДатаЛиста >= ДатаНач Тогда
			...
			Запрос.УстановитьПараметр("Счет50", ПланыСчетов.Хозрасчетный.НайтиПоКоду("50"));
			Запрос.УстановитьПараметр("Организация", Организация);
			Запрос.УстановитьПараметр("Граница", Новый Граница(КонецДня(ДатаЛиста)));
			Запрос.УстановитьПараметр("ОплатаТруда", Перечисления.ВидыДвиженийДенежныхСредств.ОплатаТруда);
			выборка = Запрос.Выполнить().Выбрать();
			Если Не Выборка.Следующий() ИЛИ выборка.суммаостаток = 0 Тогда
				ОбластьПодвалОтчет.Параметры.ЗП = "";
			Иначе
				ОбластьПодвалОтчет.Параметры.ЗП = Формат(выборка.суммаостаток,"ЧЦ=15; ЧДЦ=2; ЧРД==");		
			КонецЕсли;
			//конец добавления
