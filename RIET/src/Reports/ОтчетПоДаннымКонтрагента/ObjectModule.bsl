...
Процедура ВывестиДанные(Таб, ПорядковыйНомер, Имя)
	...
	Если Имя = "ОбщиеДанные" Тогда
		...
		Если НЕ ОбщегоНазначения.ЗначениеНеЗаполнено(КонтрагентОтчета.ЮридическоеФизическоеЛицо) Тогда

			Если КонтрагентОтчета.ЮридическоеФизическоеЛицо = Перечисления.ЮридическоеФизическоеЛицо.ЮридическоеЛицо Тогда
		...
		Если НЕ ОбщегоНазначения.ЗначениеНеЗаполнено(КонтрагентОтчета.ГоловнойКонтрагент) Тогда

			СтрокаГоловногоКонтрагента = Строка(КонтрагентОтчета.ГоловнойКонтрагент);

			Если НЕ ОбщегоНазначения.ЗначениеНеЗаполнено(КонтрагентОтчета.ГоловнойКонтрагент.ЮридическоеФизическоеЛицо) Тогда
				Если КонтрагентОтчета.ЮридическоеФизическоеЛицо = Перечисления.ЮридическоеФизическоеЛицо.ЮридическоеЛицо Тогда
		...
		Если КонтрагентОтчета.ЮридическоеФизическоеЛицо = Перечисления.ЮридическоеФизическоеЛицо.ЮридическоеЛицо Тогда //Юридическое лицо
		...
		Если НЕ ОбщегоНазначения.ЗначениеНеЗаполнено(КонтрагентОтчета.ОсновнойБанковскийСчет) тогда
			...
			Если Не ОбщегоНазначения.ЗначениеНеЗаполнено(КонтрагентОтчета.ОсновнойБанковскийСчет.Банк) Тогда
	...
	ИначеЕсли Имя = "КонтЛица" Тогда
		...
		Для каждого СтрокаКЛ Из ТаблицаКЛ Цикл
			...
			Если НЕ ОбщегоНазначения.ЗначениеНеЗаполнено(СтрокаКЛ.Роль) Тогда
...
Процедура СформироватьОтчет(Таб) Экспорт
	...
	Если ОбщегоНазначения.ЗначениеНеЗаполнено(КонтрагентОтчета) Тогда
	...
	Для Индекс = 0 По СписокНастроек.Количество() - 1 Цикл
		...
		ВывестиДанные(Таб, ПрядковыйНомер, СписокНастроек[Индекс].Значение);
