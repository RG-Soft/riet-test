...
Функция ЗаголовокОтчета() Экспорт
	Возврат "Отчет по проводкам (налоговый)";
...
Функция ПолучитьОписаниеНастроек()
	...
	Если Не ОбщегоНазначения.ЗначениеНеЗаполнено(ВидУчета) Тогда
		СтрокаОписания = СтрокаОписания+?(ПустаяСтрока(СтрокаОписания),"","; ")+"Вид учета = "+Строка(ВидУчета);
	КонецЕсли;
...
Функция ПолучитьТекстЗапроса()
	...
	Если Не ОбщегоНазначения.ЗначениеНеЗаполнено(ВидУчета) Тогда
		СтрокаОграниченийПоРеквизитам = БухгалтерскиеОтчеты.ОбъединитьОграничения(СтрокаОграниченийПоРеквизитам, "(ВидУчетаДт = &ВидУчета ИЛИ ВидУчетаКт = &ВидУчета)");
	КонецЕсли;
	...
	Если Корреспонденции.Количество()>0 Тогда
		Для каждого стр Из Корреспонденции Цикл
			...
			ТекстОтбораСчетов = "";
			Если Не ОбщегоНазначения.ЗначениеНеЗаполнено(стр.СчетДт) Тогда
				ТекстОтбораСчетов = ТекстОтбораСчетов+" И СчетДт В ИЕРАРХИИ(&СчетДт"+НомерСтроки+")";
			КонецЕсли;
			Если Не ОбщегоНазначения.ЗначениеНеЗаполнено(стр.СчетКт) Тогда
...
Процедура СформироватьОтчет(ДокументРезультат, ПоказыватьЗаголовок = Истина, ВысотаЗаголовка = 0) Экспорт
	...
	Пока Выборка.Следующий() Цикл
		...
		КолвоСтрокПрочих = 1 + ?(ЕстьКолво,1,0);
...
Процедура Настроить(СтруктураПараметров) Экспорт
	...
	Если ОбщегоНазначения.ЗначениеНеЗаполнено(Регистратор) Тогда
...
БухгалтерскиеОтчеты.СоздатьКолонкиОграниченийДляКорреспонденцииПданаСчетов(Корреспонденции, ИмяПланаСчетов);
