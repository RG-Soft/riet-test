...
Процедура СформироватьОбложкуИнвентарнойКнигиОС(ДокументРезультат) Экспорт
	...
	ДатаОтбора = ?(НЕ ОбщегоНазначения.ЗначениеНеЗаполнено(КонецПериода), КонецПериода, Неопределено);
	
	Сведения             = КонтактнаяИнформация.СведенияОЮрФизЛице(Организация, ДатаОтбора);
	...
	Макет.Параметры.Период                           = " с " + ?(ОбщегоНазначения.ЗначениеНеЗаполнено(НачалоПериода), """____"" ___________ ______ г.", Формат(НачалоПериода, "ДЛФ=DD")) + " по " + ?(ОбщегоНазначения.ЗначениеНеЗаполнено(КонецПериода), """____"" ___________ ______ г.", Формат(КонецПериода, "ДЛФ=DD"));
	Макет.Параметры.НаименованиеПодразделения        = ?(НЕ ОбщегоНазначения.ЗначениеНеЗаполнено(Подразделение), Подразделение.Наименование, "");  
	Макет.Параметры.КодПодразделения                 = ?(НЕ ОбщегоНазначения.ЗначениеНеЗаполнено(Подразделение), Подразделение.Код, "");
...
Процедура СформироватьСтраницыИнвентарнойКнигиОС(ДокументРезультат) Экспорт
	...
	Запрос.УстановитьПараметр("НачалоПериода", ?(ОбщегоНазначения.ЗначениеНеЗаполнено(НачалоПериода), НачалоПериода, НачалоДня(НачалоПериода)));
	Запрос.УстановитьПараметр("КонецПериода",  ?(ОбщегоНазначения.ЗначениеНеЗаполнено(КонецПериода), КонецПериода, КонецДня(КонецПериода)));
	...
	|				
	|				ГДЕ
	|" + ?(ОбщегоНазначения.ЗначениеНеЗаполнено(НачалоПериода) И ОбщегоНазначения.ЗначениеНеЗаполнено(КонецПериода), "", "СобытияОСОрганизаций.Период МЕЖДУ &НачалоПериода И &КонецПериода И") + "				   
	...
	|
	|" + ?((НЕ ОбщегоНазначения.ЗначениеНеЗаполнено(Подразделение)) ИЛИ (НЕ ОбщегоНазначения.ЗначениеНеЗаполнено(МОЛ)), "ГДЕ " ,"") + ?(ОбщегоНазначения.ЗначениеНеЗаполнено(Подразделение), "", "СрезПоследнихПоРегистрамСведенийИБухгалтерии.ПоследнееМестонахождение = &Подразделение") + 
				   ?(ОбщегоНазначения.ЗначениеНеЗаполнено(МОЛ), "", ?(ОбщегоНазначения.ЗначениеНеЗаполнено(Подразделение), "", " И ") + "СрезПоследнихПоРегистрамСведенийИБухгалтерии.ПоследнееМОЛ = &МОЛ") + "
	|
	|УПОРЯДОЧИТЬ ПО
	|	ДатаДокумента
	|";
	...
	Пока Выборка.Следующий() Цикл
		...
		СтрокаИнвентарнойКниги.ОсновноеСредство                                = ?(ОбщегоНазначения.ЗначениеНеЗаполнено(Выборка.ОсновноеСредствоНаименованиеПолное), Выборка.ОсновноеСредствоНаименование, Выборка.ОсновноеСредствоНаименованиеПолное);
		...
		Если Выборка.ТипДокумента = "ПринятиеКУчетуОС" Тогда
			...
			СтрокаИнвентарнойКниги.ПоступилоМОЛ                                = ПроцедурыУправленияПерсоналом.ФамилияИнициалыФизЛица(Выборка.ПоследнееМОЛПредставление);
		...
		ИначеЕсли Выборка.ТипДокумента = "ПеремещениеОС" И Выборка.ПоследнееМестонахождение = Подразделение Тогда
			...
			СтрокаИнвентарнойКниги.ПеремещениеИлиВыбытиеМОЛ                    = ПроцедурыУправленияПерсоналом.ФамилияИнициалыФизЛица(Выборка.ПредпоследнееМОЛПредставление);
			СтрокаИнвентарнойКниги.ПеремещениеИлиВыбытиеПричинаВыбытия         = "";
		...
		Иначе                                                         
			...
			СтрокаИнвентарнойКниги.ПеремещениеИлиВыбытиеМОЛ                    = ПроцедурыУправленияПерсоналом.ФамилияИнициалыФизЛица(Выборка.ПоследнееМОЛПредставление);
