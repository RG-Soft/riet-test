...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	ТабличныйДокумент.Вывести(
		Отчеты[Сред(Лев(ЭтаФорма.ИмяФормы, СтрНайти(ЭтаФорма.ИмяФормы, ".Форма.") - 1), 7)].ПолучитьМакет(
	...
	//РГ-Софт Загрузка из эксель, разрез подразделений
	СтруктураРеквизитовФормы.Вставить("Подразделение", Справочники.ПодразделенияОрганизаций.ПустаяСсылка());
	Если НЕ ЗначениеЗаполнено(Подразделение) И ЗначениеЗаполнено(Параметры.мСохраненныйДок) Тогда
		СтруктураРеквизитовФормы.Вставить("Подразделение", Параметры.мСохраненныйДок.Подразделение);
		Подразделение = СтруктураРеквизитовФормы.Подразделение;
	КонецЕсли;
	//<=
...
Процедура ВыводДанныхОФизическомЛице_(Форма, Знач СвязьУстановлена, Знач Данные, ТипДанных, ОграниченияВыводаУчесть = Ложь, ТаблицаСтраницРазделаМ, ИндексВТабл);
	...
	Если (НЕ СвязьУстановлена) Тогда
		...
		// { EKoshkina - исправлена ошибка при создании отчета
...
Функция Сохранить(Автосохранение = Ложь, КодИФНС, КПП, Вариант, ВидДокументаНомерКорректировкиИзменен = Неопределено, НаимТекРаздела) Экспорт	
	...
	//РГ-Софт Загрузка из эксель, разрез подразделений
	СтруктураРеквизитовФормы.мСохраненныйДок.Подразделение            = Подразделение;
	//<=
...
Процедура ВывестиПоказатель(ТекстВыгрузки, ЗначениеЯчейки, ИмяЯчейки, ПризнакФормата, ФорматВыгрузки)
	...
	Если НЕ ЗначениеЗаполнено(ЗначениеЯчейки) И ПризнакФормата <> "Д" Тогда
		// { RGS EKoshkina 26.11.2015 13:40:24 - SLI-0005921
		//ТекстВыгрузки.ДобавитьСтроку(ИмяЯчейки + ":")
		ТекстВыгрузки.ДобавитьСтроку(ИмяЯчейки + ":" + ЗначениеЯчейки)
		// } RGS EKoshkina 26.11.2015 13:40:24 - SLI-0005921
	Иначе
	...
	Для Сч = 1 По КолСтрок Цикл
		...
		Если Тек13Симв = "П000020012003" Тогда
			...
			ИначеЕсли НЕ ПустаяСтрока(КодВидаДеятельности) Тогда
				//РГ-Софт 25.03.2015 это должно действовать для редких случаев!
				//ТекстВыгрузки.ЗаменитьСтроку(ТекИнд, ТекСтрока + "," + КодВидаДеятельности);
				ТекстВыгрузки.ЗаменитьСтроку(ТекИнд, ТекСтрока + ",");
				//<=
			КонецЕсли;
		КонецЕсли;
...
#Область РГСофт_ЗаполнениеИзExcel 

////////////////////////////////////////////////////////////////////////////////////////////////
//////////////////////////////////Заполнение декларации по файлу Excel
...
Процедура ЗаполнитьИзExcel(Команда)
	...
	ДиалогОткрытияФайла.Фильтр 						= "Files Excel (*.xlsb, *.xlsx, *.xls)|*.xlsb; *.xlsx; *.xls";
...
&НаКлиенте
Процедура ПослеВыбораФайлаЗагрузки(Файлы, Параметры) Экспорт
	...
	ПроверитьФайл(ПутьКФайлу, Отказ);
	
	Если Отказ Тогда
		Возврат;
	КонецЕсли;
	
	ЗаполнитьДанныеИзФайлаExcel(ПутьКФайлу); 
...
Функция ПолучитьОписаниеФайлаЗагрузки()     	

	СтруктураОписания = Новый Структура;
	
	СтруктураОписания.Вставить("НомерСтолбцаПоказателя", 	3);
	СтруктураОписания.Вставить("НомерСтолбцаРесурса", 		7);
	
	МассивПоказателей = Новый Массив;
	
	//Раздел 5
	ДобавитьСтруктуруПоказателя(МассивПоказателей, 5, "TOTAL REVENUES", 						"010");
	ДобавитьСтруктуруПоказателя(МассивПоказателей, 5, "OPERATIONAL REVENUES", 					"020");
	ДобавитьСтруктуруПоказателя(МассивПоказателей, 5, "Operational revenue", 					"030");
	ДобавитьСтруктуруПоказателя(МассивПоказателей, 5, "Revenue from FA&other property sale",	"040"); 	
	ДобавитьСтруктуруПоказателя(МассивПоказателей, 5, "NONOPERATIONAL REVENUES", 				"060"); 
	ДобавитьСтруктуруПоказателя(МассивПоказателей, 5, "Positive sum differences", 				"140"); 
	ДобавитьСтруктуруПоказателя(МассивПоказателей, 5, "Exchange differences", 					"150"); 
	ДобавитьСтруктуруПоказателя(МассивПоказателей, 5, "Bank interest", 							"150"); 
	ДобавитьСтруктуруПоказателя(МассивПоказателей, 5, "Other nonoperational revenues", 			"150"); 
	ДобавитьСтруктуруПоказателя(МассивПоказателей, 5, "TOTAL EXPENSES", 						"160"); 
	ДобавитьСтруктуруПоказателя(МассивПоказателей, 5, "TOTAL OPERATIONAL EXPENSES", 			"170"); 
	ДобавитьСтруктуруПоказателя(МассивПоказателей, 5, "NONOPERATION EXPENSES", 					"190"); 
	ДобавитьСтруктуруПоказателя(МассивПоказателей, 5, "PROFIT/LOSS", 							"250"); 
	ДобавитьСтруктуруПоказателя(МассивПоказателей, 5, "Loss c/f", 								"300"); 
	
	//Раздел 6 		
	ДобавитьСтруктуруПоказателя(МассивПоказателей, 6, "Salary",									"010"); 
	ДобавитьСтруктуруПоказателя(МассивПоказателей, 6, "Materials & Supplies",					"020"); 
	ДобавитьСтруктуруПоказателя(МассивПоказателей, 6, "Services",								"030"); 
	ДобавитьСтруктуруПоказателя(МассивПоказателей, 6, "Services - sub-contractors",				"040"); 
	ДобавитьСтруктуруПоказателя(МассивПоказателей, 6, "Rent",									"050"); 
	ДобавитьСтруктуруПоказателя(МассивПоказателей, 6, "Leasing",								"060"); 
	ДобавитьСтруктуруПоказателя(МассивПоказателей, 6, "Royalty to Russian companies",			"070"); 
	ДобавитьСтруктуруПоказателя(МассивПоказателей, 6, "Royalty to foreign companies",			"080"); 
	ДобавитьСтруктуруПоказателя(МассивПоказателей, 6, "Business travel",						"090"); 
	ДобавитьСтруктуруПоказателя(МассивПоказателей, 6, "Entertainment",							"100"); 
	ДобавитьСтруктуруПоказателя(МассивПоказателей, 6, "Training",								"110"); 
	ДобавитьСтруктуруПоказателя(МассивПоказателей, 6, "Advertising",							"120"); 
	ДобавитьСтруктуруПоказателя(МассивПоказателей, 6, "Depreciation",							"130"); 
	ДобавитьСтруктуруПоказателя(МассивПоказателей, 6, "Unified Social Tax",						"140"); 
	ДобавитьСтруктуруПоказателя(МассивПоказателей, 6, "Other expenses",							"150"); 
	ДобавитьСтруктуруПоказателя(МассивПоказателей, 6, "TOTAL OPERATIONAL EXPENSES",				"160"); 	
	
	СтруктураОписания.Вставить("МассивПоказателей", МассивПоказателей);
...
&НаСервереБезКонтекста
Функция ПолучитьСоответствиеСтрокЯчейкам(Раздел)
	
	Структура = Новый Структура;
	
	Если Раздел = 5 Тогда
	
		Структура.Вставить("Строка010", "П000050001004");
		Структура.Вставить("Строка020", "П000050002004");
		Структура.Вставить("Строка030", "П000050003004");
		Структура.Вставить("Строка040", "П000050004004");
		Структура.Вставить("Строка050", "П000050005004");
		Структура.Вставить("Строка060", "П000050006004");
		Структура.Вставить("Строка070", "П000050007004");
		Структура.Вставить("Строка080", "П000050008004");
		Структура.Вставить("Строка090", "П000050009004");
		Структура.Вставить("Строка100", "П000050010004");
		Структура.Вставить("Строка110", "П000050011004");
		Структура.Вставить("Строка120", "П000050012004");
		Структура.Вставить("Строка130", "П000050013004");
		Структура.Вставить("Строка140", "П000050014004");
		Структура.Вставить("Строка150", "П000050015004");
		Структура.Вставить("Строка160", "П000050016004");
		Структура.Вставить("Строка170", "П000050017004");
		Структура.Вставить("Строка180", "П000050018004");		
		Структура.Вставить("Строка190", "П000050019004");
		Структура.Вставить("Строка200", "П000050020004");
		Структура.Вставить("Строка210", "П000050021004");
		Структура.Вставить("Строка220", "П000050022004");
		Структура.Вставить("Строка230", "П000050023004");
		Структура.Вставить("Строка240", "П000050024004");
		Структура.Вставить("Строка250", "П000050025004");
		Структура.Вставить("Строка260", "П000050026004");
		Структура.Вставить("Строка270", "П000050027004");
		Структура.Вставить("Строка280", "П000050028004");
		Структура.Вставить("Строка290", "П000050029004");
		Структура.Вставить("Строка291", "П000050029104");
		Структура.Вставить("Строка300", "П000050030004");
		Структура.Вставить("Строка310", "П000050031004");
		Структура.Вставить("Строка320", "П000050032004");
		Структура.Вставить("Строка330", "П000050033004");
		Структура.Вставить("Строка340", "П000050034004");
		Структура.Вставить("Строка350", "П000050035004"); 
		
	ИначеЕсли Раздел = 6 Тогда
		
		Структура.Вставить("Строка010", "П000060001004");
		Структура.Вставить("Строка020", "П000060002004");
		Структура.Вставить("Строка030", "П000060003004");
		Структура.Вставить("Строка040", "П000060004004");
		Структура.Вставить("Строка050", "П000060005004");
		Структура.Вставить("Строка060", "П000060006004");
		Структура.Вставить("Строка070", "П000060007004");
		Структура.Вставить("Строка080", "П000060008004");
		Структура.Вставить("Строка090", "П000060009004");
		Структура.Вставить("Строка100", "П000060010004");
		Структура.Вставить("Строка110", "П000060011004");
		Структура.Вставить("Строка120", "П000060012004");
		Структура.Вставить("Строка130", "П000060013004");
		Структура.Вставить("Строка140", "П000060014004");
		Структура.Вставить("Строка150", "П000060015004");
		Структура.Вставить("Строка160", "П000060016004");
		
	ИначеЕсли Раздел = 7 Тогда
		
		Структура.Вставить("Строка020", "П000070002004");
		Структура.Вставить("Строка030", "П000070003004");
		Структура.Вставить("Строка040", "П000070004004");
		Структура.Вставить("Строка050", "П000070005004");
		Структура.Вставить("Строка060", "П000070006004");
		Структура.Вставить("Строка070", "П000070007004");
		Структура.Вставить("Строка080", "П000070008004");
		Структура.Вставить("Строка090", "П000070009004");
		Структура.Вставить("Строка100", "П000070010004");
		Структура.Вставить("Строка110", "П000070011004");
		Структура.Вставить("Строка120", "П000070012004");
		Структура.Вставить("Строка130", "П000070013004");
		Структура.Вставить("Строка140", "П000070014004");
		Структура.Вставить("Строка150", "П000070015004");
		Структура.Вставить("Строка160", "П000070016004");
		Структура.Вставить("Строка170", "П000070017004");
		Структура.Вставить("Строка180", "П000070018004");		
		Структура.Вставить("Строка190", "П000070019004");
		Структура.Вставить("Строка200", "П000070020004");
		Структура.Вставить("Строка210", "П000070021004");
		Структура.Вставить("Строка220", "П000070022004");
		Структура.Вставить("Строка230", "П000070023004");
		Структура.Вставить("Строка240", "П000070024004");
		Структура.Вставить("Строка250", "П000070025004");
		Структура.Вставить("Строка260", "П000070026004");
		Структура.Вставить("Строка270", "П000070027004");  		
...
&НаКлиенте
Процедура УстановитьЗначениеВКэшЯчеек(НомерРаздела, КэшЗначенийЯчеек, СоответствиеСтрокЯчейкам, КодСтроки, Значение, УстановитьЗаново = Ложь)
	
	КодСтрокиСПрефиксом = "Строка" + КодСтроки;
	
	Если СоответствиеСтрокЯчейкам.Свойство(КодСтрокиСПрефиксом) Тогда
		ИмяЯчейки = СоответствиеСтрокЯчейкам[КодСтрокиСПрефиксом];
	Иначе
		Возврат;
	КонецЕсли;  
	...
	ИдентификаторСтруктурыРаздела = Строка("Раздел" + НомерРаздела);
	
	Если НЕ КэшЗначенийЯчеек.Свойство(ИдентификаторСтруктурыРаздела) Тогда
		КэшЗначенийЯчеек.Вставить(ИдентификаторСтруктурыРаздела, Новый Структура);	
	КонецЕсли;     				
		
	Если НЕ УстановитьЗаново И КэшЗначенийЯчеек[ИдентификаторСтруктурыРаздела].Свойство(ИмяЯчейки) Тогда
		КэшЗначенийЯчеек[ИдентификаторСтруктурыРаздела].Вставить(ИмяЯчейки, КэшЗначенийЯчеек[ИдентификаторСтруктурыРаздела][ИмяЯчейки] + Значение);	
	Иначе
		КэшЗначенийЯчеек[ИдентификаторСтруктурыРаздела].Вставить(ИмяЯчейки, Значение);	
	КонецЕсли; 
	
КонецПроцедуры

&НаКлиенте
Функция ЗначениеВКэшеЯчеек(НомерРаздела, КэшЗначенийЯчеек, СоответствиеСтрокЯчейкам, ИмяСтроки)
	
	КодСтрокиСПрефиксом = "Строка" + ИмяСтроки;
	...
	Возврат КэшЗначенийЯчеек[Строка("Раздел" + НомерРаздела)][ИмяЯчейки];
		
КонецФункции

&НаСервере 
Функция ПреобразоватьМассивСтруктурВТаблицуЗначений(МассивСтруктур)
	
	ТаблицаДанныеЗагрузки = Новый ТаблицаЗначений;	
	Для каждого Элемент из МассивСтруктур[0] Цикл
		ТаблицаДанныеЗагрузки.Колонки.Добавить(Элемент.Ключ, ОбщегоНазначения.ОписаниеТипаСтрока(100));			
	КонецЦикла;	
	Для каждого Элемент из МассивСтруктур Цикл
		Строка = ТаблицаДанныеЗагрузки.Добавить();
		ЗаполнитьЗначенияСвойств(Строка, Элемент);
	КонецЦикла;
	
	Возврат ТаблицаДанныеЗагрузки;
	
КонецФункции

&НаСервереБезКонтекста
Процедура ДобавитьСтруктуруПоказателя(Массив, НомерРаздела, Представление, КодСтроки = Неопределено)
	
	Структура = Новый Структура;
	
	Структура.Вставить("Представление", Представление);
	Структура.Вставить("КодСтроки", 	КодСтроки);
	Структура.Вставить("НомерРаздела", 	НомерРаздела);
...
&НаКлиенте
Функция НайтиСтрокуДереваЗначений(МассивИменСтраниц)
	
	ТекущийЭлементДерева = РазделыОтчета;
	СтрокаНужногоРаздела = Неопределено;
	
	Для каждого ЭлементМассива из МассивИменСтраниц Цикл
				
		ТекущийЭлементНайден = Ложь;
		
		Для каждого Элемент из ТекущийЭлементДерева.ПолучитьЭлементы() Цикл
			Если Элемент.КолонкаРазделыОтчета = ЭлементМассива Тогда
				ТекущийЭлементДерева = Элемент; 	
				ТекущийЭлементНайден = Истина;
			КонецЕсли;
		КонецЦикла;
		
		Если НЕ ТекущийЭлементНайден Тогда
			Прервать;
		Иначе
			Если ЭлементМассива = МассивИменСтраниц[МассивИменСтраниц.Количество()-1] Тогда
				СтрокаНужногоРаздела = ТекущийЭлементДерева; 	
			КонецЕсли;
		КонецЕсли;	

	КонецЦикла;
	
	Возврат СтрокаНужногоРаздела;
...
&НаКлиенте
Процедура ПроверитьФайл(ПутьКФайлу, Отказ)
	
	Состояние("Выполняется первоначальная проверка");
	
	ПолноеИмя = СокрЛП(ПутьКФайлу);
	
	Если НЕ ЗначениеЗаполнено(ПолноеИмя) Тогда 		
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю("Не выбран файл!",,"Объект","ПутьКФайлу");
		Отказ = Истина;
		Возврат;
	КонецЕсли;   
	...
	Если НЕ Файл.ЭтоФайл() Тогда
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю("Файл """ + ПолноеИмя + """ не является файлом!");
		Отказ = Истина;
	КонецЕсли; 	
	
	Если Отказ Тогда
		Сообщить("Ошибки, файл не загружен");
	КонецЕсли; 	
	
КонецПроцедуры

&НаКлиенте 
Процедура ЗаполнитьДанныеИзФайлаExcel(Путь)
	
	Connection = Новый COMОбъект("ADODB.Connection");
	СтрокаПодключения = "Provider=Microsoft.ACE.OLEDB.12.0;Data Source=" + Путь + ";Extended Properties=""Excel 12.0;HDR=No;IMEX=1""";	
	Попытка 
		Connection.Open(СтрокаПодключения);	
	Исключение
		Попытка
			СтрокаПодключения = "Provider=Microsoft.Jet.OLEDB.4.0;Data Source=" + Путь + ";Extended Properties=""Excel 8.0;HDR=No;IMEX=1""";
			Connection.Open(СтрокаПодключения);
		Исключение
	...
	ИменаЛистовСписок 			= ПолучитьИменаЛистовФайла(Connection);
	
	Если НЕ ИменаЛистовСписок.Количество() Тогда
		Сообщить("В файле отсутствуют листы данных!");
		Возврат;
	КонецЕсли;
	
	ПараметрыПослеВыбораЛиста 	= Новый Структура("Connection", Connection);
	ОповещениеВыборЛиста 		= Новый ОписаниеОповещения("ВыборЛистаФайлаЗавершение", ЭтаФорма, ПараметрыПослеВыбораЛиста);
	
	ИменаЛистовСписок.ПоказатьВыборЭлемента(ОповещениеВыборЛиста, "Выбор листа данных");  
	
КонецПроцедуры

&НаКлиенте
Процедура ВыборЛистаФайлаЗавершение(Результат, Параметры) Экспорт
	
	Если Результат = Неопределено Тогда
		Сообщить("Не был выбран лист");
		Возврат;
	КонецЕсли;
	
	Connection = Параметры.Connection;
	...
	SqlString = "select * from [" + ИмяТаблицы + "]"; 
	RecordSet.Close();
	RecordSet.Open(SqlString);
	
	//СЧИТЫВАЕМ ДАННЫЕ
	
	ОписаниеФайла = ПолучитьОписаниеФайлаЗагрузки();
	
	МассивДанных 			= Новый Массив;	
	ПоказателиДляЗагрузки 	= ОписаниеФайла.МассивПоказателей;	
	НомерСтолбцаПоказателя  = ОписаниеФайла["НомерСтолбцаПоказателя"];
	НомерСтолбцаРесурса     = ОписаниеФайла["НомерСтолбцаРесурса"];
	
	МассивСчитанныхПоказателей 		= Новый Массив;	
	МассивНеобходимыхПоказателей 	= Новый Массив;	
	Для каждого ОписаниеПоказателя из ПоказателиДляЗагрузки Цикл
		МассивНеобходимыхПоказателей.Добавить(ОписаниеПоказателя.Представление);
	КонецЦикла;
	...
	Пока НЕ RecordSet.EOF Цикл
		...
		ТекущийПоказательВФайле = СокрЛП(RecordSet.Fields(НомерСтолбцаПоказателя - 1).Value);
		
		Для каждого ОписаниеПоказателя из ПоказателиДляЗагрузки Цикл
			
			ПредставлениеПоказателя = ОписаниеПоказателя.Представление;
			
			Если ТекущийПоказательВФайле = ПредставлениеПоказателя Тогда
				
				ЗначениеПоказателя = RecordSet.Fields(НомерСтолбцаРесурса - 1).Value;							
				ЗначениеПоказателя =?(ЗначениеПоказателя = NULL, 0, ЗначениеПоказателя); 
				
				Попытка
					ЗначениеПоказателяПреобразованное 	= Число(ЗначениеПоказателя);
				Исключение
					Отказ = Истина;
					Сообщить("Для показателя """ + ТекущийПоказательВФайле + """ указана сумма в неправильном формате!");
				КонецПопытки;
				
				СтруктураПоказателя = Новый Структура;
				СтруктураПоказателя.Вставить("Показатель", 		ТекущийПоказательВФайле); 
				СтруктураПоказателя.Вставить("Значение",   		ЗначениеПоказателяПреобразованное); 
				СтруктураПоказателя.Вставить("КодСтроки",  		ОписаниеПоказателя.КодСтроки); 
				СтруктураПоказателя.Вставить("НомерРаздела",	ОписаниеПоказателя.НомерРаздела); 
				
				МассивДанных.Добавить(СтруктураПоказателя);  
				
				МассивСчитанныхПоказателей.Добавить(ТекущийПоказательВФайле);
	...
	Если Отказ Тогда
		Сообщить("Не удалось преобразовать суммы в числовые значения. Суммы должны быть указаны в файле в числовом формате, без разделителей порядков!");
		Возврат;
	КонецЕсли;

	Для каждого Показатель из МассивНеобходимыхПоказателей Цикл
		Если МассивСчитанныхПоказателей.Найти(Показатель) = Неопределено Тогда
			Отказ = Истина;
			Сообщить("В файле не найдены данные для """ + Показатель + """!");
		КонецЕсли;
	КонецЦикла;
	...
	ОповещениеСформированыДанные = Новый ОписаниеОповещения("ПослеСчитыванияДанныхИзФайла", ЭтаФорма);
	ВыполнитьОбработкуОповещения(ОповещениеСформированыДанные, МассивДанных);
	
КонецПроцедуры

&НаКлиенте
Функция ПолучитьИменаЛистовФайла(Connection)	
	
	axCatalog = Новый COMОбъект("ADOX.Catalog");                                                
	axCatalog.ActiveConnection = Connection;
	
	ИменаЛистов = Новый СписокЗначений;

	Для каждого Лист ИЗ axCatalog.Tables Цикл
		
		ИмяЛиста 				= Лист.Name;		
		
		ПозицияСлужебныхСимволов = СтрНайти(ИмяЛиста, "$'");
		Если ПозицияСлужебныхСимволов = 0 Тогда
			ПозицияСлужебныхСимволов = СтрНайти(ИмяЛиста, "$");			
			СчетчикСлужебныхСимволов = 1;
		Иначе
			СчетчикСлужебныхСимволов = 2;
		КонецЕсли;			
		                                          
		Если ПозицияСлужебныхСимволов = 0 ИЛИ НЕ ПустаяСтрока(Сред(ИмяЛиста, ПозицияСлужебныхСимволов + СчетчикСлужебныхСимволов)) Тогда
			Продолжить; // исключаем без разделителя, на который завязываемся, а так же всякие полу-страницы, невидимые пользователю
		КонецЕсли;
		
		Представление 	= Сред(ИмяЛиста, СчетчикСлужебныхСимволов, ПозицияСлужебныхСимволов - СчетчикСлужебныхСимволов);		
        ИменаЛистов.Добавить(ИмяЛиста,  Представление);
		
	КонецЦикла;
	
	ИменаЛистов.СортироватьПоПредставлению();
...
&НаКлиенте
Процедура ПослеСчитыванияДанныхИзФайла(Результат, Параметры) Экспорт
	
	МассивСтруктурЗагрузки = Результат;
	
	КэшЗначенийЯчеек = Новый Структура;
	
	ЗаполнитьРаздел5(МассивСтруктурЗагрузки, КэшЗначенийЯчеек);
	
	ЗаполнитьРаздел6(МассивСтруктурЗагрузки, КэшЗначенийЯчеек);
	
	ЗаполнитьРаздел7(КэшЗначенийЯчеек);	
	
	ЗаполнитьРаздел2();
	
	ЗаполнитьРаздел1_1();
	
	ЗаполнитьТитульныйЛист();
	
КонецПроцедуры

&НаКлиенте
Процедура ЗаполнитьРаздел5(МассивСтруктурЗагрузки, КэшЗначенийЯчеек)
	
	МассивПоиска = Новый Массив;
	МассивПоиска.Добавить("Раздел 5");
	МассивПоиска.Добавить("Стр. 1");
	СтрокаНужногоРаздела = НайтиСтрокуДереваЗначений(МассивПоиска);
	
	Если СтрокаНужногоРаздела =  Неопределено Тогда
		Сообщить("Невозможно заполнить 5-й раздел ""Стр. 1"", таблицы не найдены в дереве разделов");
		Возврат;
	КонецЕсли;      
	
	Элементы.РазделыОтчета.ТекущаяСтрока = СтрокаНужногоРаздела.ПолучитьИдентификатор();  	
	
	СоответствиеСтрокЯчейкам 	= ПолучитьСоответствиеСтрокЯчейкам(5);  
	
	//Установка значений ячеек из строк файла (по заданию)
	Для каждого СтруктураОписанияПоказателя из МассивСтруктурЗагрузки Цикл
		
		НомерРаздела = СтруктураОписанияПоказателя.НомерРаздела;
		
		Если НомерРаздела <> 5 Тогда
			Продолжить;
		КонецЕсли;	
		
		КодСтроки 	 		= СтруктураОписанияПоказателя.КодСтроки;    
		ЗначениеПоказателя 	= СтруктураОписанияПоказателя.Значение;
		
		УстановитьЗначениеВКэшЯчеек(5, КэшЗначенийЯчеек, СоответствиеСтрокЯчейкам, КодСтроки, ЗначениеПоказателя, Ложь); 
  		                                                                                                                    		
	КонецЦикла;
	
	//Дополнительные установки (расчетные)    
	УстановитьЗначениеВКэшЯчеек(5, КэшЗначенийЯчеек, СоответствиеСтрокЯчейкам, "050", 0);
	УстановитьЗначениеВКэшЯчеек(5, КэшЗначенийЯчеек, СоответствиеСтрокЯчейкам, "070", 0);
	УстановитьЗначениеВКэшЯчеек(5, КэшЗначенийЯчеек, СоответствиеСтрокЯчейкам, "080", 0);
	УстановитьЗначениеВКэшЯчеек(5, КэшЗначенийЯчеек, СоответствиеСтрокЯчейкам, "090", 0);
	УстановитьЗначениеВКэшЯчеек(5, КэшЗначенийЯчеек, СоответствиеСтрокЯчейкам, "100", 0);
	УстановитьЗначениеВКэшЯчеек(5, КэшЗначенийЯчеек, СоответствиеСтрокЯчейкам, "110", 0);
	УстановитьЗначениеВКэшЯчеек(5, КэшЗначенийЯчеек, СоответствиеСтрокЯчейкам, "120", 0);
	УстановитьЗначениеВКэшЯчеек(5, КэшЗначенийЯчеек, СоответствиеСтрокЯчейкам, "130", 0);   	
	УстановитьЗначениеВКэшЯчеек(5, КэшЗначенийЯчеек, СоответствиеСтрокЯчейкам, "180", ЗначениеВКэшеЯчеек(5, КэшЗначенийЯчеек, СоответствиеСтрокЯчейкам,"170"));  	
	УстановитьЗначениеВКэшЯчеек(5, КэшЗначенийЯчеек, СоответствиеСтрокЯчейкам, "200", 0);
	УстановитьЗначениеВКэшЯчеек(5, КэшЗначенийЯчеек, СоответствиеСтрокЯчейкам, "210", 0);
	УстановитьЗначениеВКэшЯчеек(5, КэшЗначенийЯчеек, СоответствиеСтрокЯчейкам, "220", 0);
	УстановитьЗначениеВКэшЯчеек(5, КэшЗначенийЯчеек, СоответствиеСтрокЯчейкам, "230", 0);  	
	УстановитьЗначениеВКэшЯчеек(5, КэшЗначенийЯчеек, СоответствиеСтрокЯчейкам, "240", ЗначениеВКэшеЯчеек(5, КэшЗначенийЯчеек, СоответствиеСтрокЯчейкам,"190"));	
	УстановитьЗначениеВКэшЯчеек(5, КэшЗначенийЯчеек, СоответствиеСтрокЯчейкам, "260", 0);
	УстановитьЗначениеВКэшЯчеек(5, КэшЗначенийЯчеек, СоответствиеСтрокЯчейкам, "270", 0);
	УстановитьЗначениеВКэшЯчеек(5, КэшЗначенийЯчеек, СоответствиеСтрокЯчейкам, "280", 0);  	
	УстановитьЗначениеВКэшЯчеек(5, КэшЗначенийЯчеек, СоответствиеСтрокЯчейкам, "290", ЗначениеВКэшеЯчеек(5, КэшЗначенийЯчеек, СоответствиеСтрокЯчейкам,"250")); 	
	УстановитьЗначениеВКэшЯчеек(5, КэшЗначенийЯчеек, СоответствиеСтрокЯчейкам, "291", 0);  	
	УстановитьЗначениеВКэшЯчеек(5, КэшЗначенийЯчеек, СоответствиеСтрокЯчейкам, "310", ЗначениеВКэшеЯчеек(5, КэшЗначенийЯчеек, СоответствиеСтрокЯчейкам,"290") - ЗначениеВКэшеЯчеек(5, КэшЗначенийЯчеек, СоответствиеСтрокЯчейкам,"300"));	
	УстановитьЗначениеВКэшЯчеек(5, КэшЗначенийЯчеек, СоответствиеСтрокЯчейкам, "320", 0);
	УстановитьЗначениеВКэшЯчеек(5, КэшЗначенийЯчеек, СоответствиеСтрокЯчейкам, "330", 0);	
	УстановитьЗначениеВКэшЯчеек(5, КэшЗначенийЯчеек, СоответствиеСтрокЯчейкам, "340", ЗначениеВКэшеЯчеек(5, КэшЗначенийЯчеек, СоответствиеСтрокЯчейкам,"310"));
	УстановитьЗначениеВКэшЯчеек(5, КэшЗначенийЯчеек, СоответствиеСтрокЯчейкам, "350", ЗначениеВКэшеЯчеек(5, КэшЗначенийЯчеек, СоответствиеСтрокЯчейкам,"310"));	
	                                                                                
	Если ЗначениеВКэшеЯчеек(5, КэшЗначенийЯчеек, СоответствиеСтрокЯчейкам,"250") < 0 Тогда
		УстановитьЗначениеВКэшЯчеек(5, КэшЗначенийЯчеек, СоответствиеСтрокЯчейкам, "290", 0, Истина); 		
		УстановитьЗначениеВКэшЯчеек(5, КэшЗначенийЯчеек, СоответствиеСтрокЯчейкам, "300", 0, Истина); 		
		УстановитьЗначениеВКэшЯчеек(5, КэшЗначенийЯчеек, СоответствиеСтрокЯчейкам, "310", 0, Истина); 		
		УстановитьЗначениеВКэшЯчеек(5, КэшЗначенийЯчеек, СоответствиеСтрокЯчейкам, "340", 0, Истина); 		
		УстановитьЗначениеВКэшЯчеек(5, КэшЗначенийЯчеек, СоответствиеСтрокЯчейкам, "350", 0, Истина); 				
	КонецЕсли;    
	
	ВывестиДанныеВТабличныйДокумент(КэшЗначенийЯчеек["Раздел5"]);
	
	Сообщить("Раздел 5, стр. 1 заполнен");	
	
КонецПроцедуры

&НаКлиенте
Процедура ЗаполнитьРаздел6(МассивСтруктурЗагрузки, КэшЗначенийЯчеек)
	
	МассивПоиска = Новый Массив;
	МассивПоиска.Добавить("Раздел 6");
	МассивПоиска.Добавить("Стр. 1");
	СтрокаНужногоРаздела = НайтиСтрокуДереваЗначений(МассивПоиска);
	
	Если СтрокаНужногоРаздела =  Неопределено Тогда
		Сообщить("Невозможно заполнить 6-й раздел ""Стр. 1"", таблицы не найдены в дереве разделов");
		Возврат;
	КонецЕсли;      
	
	Элементы.РазделыОтчета.ТекущаяСтрока = СтрокаНужногоРаздела.ПолучитьИдентификатор();
	
	СоответствиеСтрокЯчейкам 	= ПолучитьСоответствиеСтрокЯчейкам(6);
	...
	Для каждого СтруктураОписанияПоказателя из МассивСтруктурЗагрузки Цикл
		...
		УстановитьЗначениеВКэшЯчеек(6, КэшЗначенийЯчеек, СоответствиеСтрокЯчейкам, КодСтроки, ЗначениеПоказателя, Ложь); 
  		                                                                                                                    		
	КонецЦикла;
	
	//Дополнительные установки (расчетные)
	УстановитьЗначениеВКэшЯчеек(6, КэшЗначенийЯчеек, СоответствиеСтрокЯчейкам, "170", 0);	
	
	ВывестиДанныеВТабличныйДокумент(КэшЗначенийЯчеек["Раздел6"]);
	
	Сообщить("Раздел 6, стр. 1 заполнен");	
	
КонецПроцедуры

&НаКлиенте
Процедура ЗаполнитьРаздел7(КэшЗначенийЯчеек)
	
	МассивПоиска = Новый Массив;
	МассивПоиска.Добавить("Раздел 7");
	МассивПоиска.Добавить("Стр. 1");
	СтрокаНужногоРаздела = НайтиСтрокуДереваЗначений(МассивПоиска);
	
	Если СтрокаНужногоРаздела =  Неопределено Тогда
		Сообщить("Невозможно заполнить 7-й раздел ""Стр. 1"", таблицы не найдены в дереве разделов");
		Возврат;
	КонецЕсли;      
	
	Элементы.РазделыОтчета.ТекущаяСтрока = СтрокаНужногоРаздела.ПолучитьИдентификатор();
	
	СоответствиеСтрокЯчейкамРаздел7		= ПолучитьСоответствиеСтрокЯчейкам(7);                        
	СоответствиеСтрокЯчейкамРаздел5 	= ПолучитьСоответствиеСтрокЯчейкам(5);
	                                                   
	//Дополнительные установки (расчетные)
	УстановитьЗначениеВКэшЯчеек(7, КэшЗначенийЯчеек, СоответствиеСтрокЯчейкамРаздел7, "020", ЗначениеВКэшеЯчеек(5, КэшЗначенийЯчеек, СоответствиеСтрокЯчейкамРаздел5, "340"));
	УстановитьЗначениеВКэшЯчеек(7, КэшЗначенийЯчеек, СоответствиеСтрокЯчейкамРаздел7, "030", ЗначениеВКэшеЯчеек(5, КэшЗначенийЯчеек, СоответствиеСтрокЯчейкамРаздел5, "350"));
	УстановитьЗначениеВКэшЯчеек(7, КэшЗначенийЯчеек, СоответствиеСтрокЯчейкамРаздел7, "040", 0);
	УстановитьЗначениеВКэшЯчеек(7, КэшЗначенийЯчеек, СоответствиеСтрокЯчейкамРаздел7, "050", 0);
	УстановитьЗначениеВКэшЯчеек(7, КэшЗначенийЯчеек, СоответствиеСтрокЯчейкамРаздел7, "060", 0);
	УстановитьЗначениеВКэшЯчеек(7, КэшЗначенийЯчеек, СоответствиеСтрокЯчейкамРаздел7, "070", 0);
	УстановитьЗначениеВКэшЯчеек(7, КэшЗначенийЯчеек, СоответствиеСтрокЯчейкамРаздел7, "080", 0);
	УстановитьЗначениеВКэшЯчеек(7, КэшЗначенийЯчеек, СоответствиеСтрокЯчейкамРаздел7, "090", 0);
	УстановитьЗначениеВКэшЯчеек(7, КэшЗначенийЯчеек, СоответствиеСтрокЯчейкамРаздел7, "100", ЗначениеВКэшеЯчеек(7, КэшЗначенийЯчеек, СоответствиеСтрокЯчейкамРаздел7, "020"));
	УстановитьЗначениеВКэшЯчеек(7, КэшЗначенийЯчеек, СоответствиеСтрокЯчейкамРаздел7, "110", ЗначениеВКэшеЯчеек(7, КэшЗначенийЯчеек, СоответствиеСтрокЯчейкамРаздел7, "030"));
	УстановитьЗначениеВКэшЯчеек(7, КэшЗначенийЯчеек, СоответствиеСтрокЯчейкамРаздел7, "120", 0);
	УстановитьЗначениеВКэшЯчеек(7, КэшЗначенийЯчеек, СоответствиеСтрокЯчейкамРаздел7, "130", 2);
	УстановитьЗначениеВКэшЯчеек(7, КэшЗначенийЯчеек, СоответствиеСтрокЯчейкамРаздел7, "140", 18);
	УстановитьЗначениеВКэшЯчеек(7, КэшЗначенийЯчеек, СоответствиеСтрокЯчейкамРаздел7, "150", 0);
	
	УстановитьЗначениеВКэшЯчеек(7, КэшЗначенийЯчеек, СоответствиеСтрокЯчейкамРаздел7, "170", ЗначениеВКэшеЯчеек(7, КэшЗначенийЯчеек, СоответствиеСтрокЯчейкамРаздел7, "100") * 2 / 100);
	УстановитьЗначениеВКэшЯчеек(7, КэшЗначенийЯчеек, СоответствиеСтрокЯчейкамРаздел7, "180", ЗначениеВКэшеЯчеек(7, КэшЗначенийЯчеек, СоответствиеСтрокЯчейкамРаздел7, "110") * 18 / 100);
	УстановитьЗначениеВКэшЯчеек(7, КэшЗначенийЯчеек, СоответствиеСтрокЯчейкамРаздел7, "160", ЗначениеВКэшеЯчеек(7, КэшЗначенийЯчеек, СоответствиеСтрокЯчейкамРаздел7, "170")
																					 	   + ЗначениеВКэшеЯчеек(7, КэшЗначенийЯчеек, СоответствиеСтрокЯчейкамРаздел7, "180"));	
																						   
	УстановитьЗначениеВКэшЯчеек(7, КэшЗначенийЯчеек, СоответствиеСтрокЯчейкамРаздел7, "190", 0);
	УстановитьЗначениеВКэшЯчеек(7, КэшЗначенийЯчеек, СоответствиеСтрокЯчейкамРаздел7, "200", 0);
	
	//Эти данные устанавливаются на основе предыдущего отчета. Сначала проставим нулевые значения ->
	УстановитьЗначениеВКэшЯчеек(7, КэшЗначенийЯчеек, СоответствиеСтрокЯчейкамРаздел7, "210", 0);
	УстановитьЗначениеВКэшЯчеек(7, КэшЗначенийЯчеек, СоответствиеСтрокЯчейкамРаздел7, "220", 0); 
	УстановитьЗначениеВКэшЯчеек(7, КэшЗначенийЯчеек, СоответствиеСтрокЯчейкамРаздел7, "230", 0);
	
	ДанныеПредыдущегоОтчета = ПолучитьДанныеПредыдущегоОтчета(СтруктураРеквизитовФормы.мДатаКонцаПериодаОтчета, Подразделение);
	
	Если ДанныеПредыдущегоОтчета = Неопределено Тогда
		
		Сообщить("Предыдущий отчет не найден, по строкам 210, 220, 230 будут проставлены нулевые значения!");
		
	Иначе                                             
		
		Если ДанныеПредыдущегоОтчета.ОшибкаПриПолученииДанных Тогда
			
			Сообщить("Ошибка! Данные предыдущего отчета не получены, по строкам 210, 220, 230 будут проставлены нулевые значения");
			
		Иначе              
			
			ИмяЯчейкиПоСтроке160 	= СоответствиеСтрокЯчейкамРаздел7["Строка" + "160"];
			ПредДанныеПоСтроке160 	= ДанныеПредыдущегоОтчета.ДанныеЯчеек[ИмяЯчейкиПоСтроке160];
			ИмяЯчейкиПоСтроке170 	= СоответствиеСтрокЯчейкамРаздел7["Строка" + "170"];
			ПредДанныеПоСтроке170 	= ДанныеПредыдущегоОтчета.ДанныеЯчеек[ИмяЯчейкиПоСтроке170];
			ИмяЯчейкиПоСтроке180 	= СоответствиеСтрокЯчейкамРаздел7["Строка" + "180"];
			ПредДанныеПоСтроке180 	= ДанныеПредыдущегоОтчета.ДанныеЯчеек[ИмяЯчейкиПоСтроке180];  			                                  
			
			УстановитьЗначениеВКэшЯчеек(7, КэшЗначенийЯчеек, СоответствиеСтрокЯчейкамРаздел7, "210", ПредДанныеПоСтроке160);
			УстановитьЗначениеВКэшЯчеек(7, КэшЗначенийЯчеек, СоответствиеСтрокЯчейкамРаздел7, "220", ПредДанныеПоСтроке170); 
			УстановитьЗначениеВКэшЯчеек(7, КэшЗначенийЯчеек, СоответствиеСтрокЯчейкамРаздел7, "230", ПредДанныеПоСтроке180);
			
			Сообщить("В строки 210, 220, 230 раздела 7 установлены значения строк 160, 170, 180 декларации " + ДанныеПредыдущегоОтчета.Документ);
	...
	УстановитьЗначениеВКэшЯчеек(7, КэшЗначенийЯчеек, СоответствиеСтрокЯчейкамРаздел7, "240", 0);
	
	Значение160Минус210 = ЗначениеВКэшеЯчеек(7, КэшЗначенийЯчеек, СоответствиеСтрокЯчейкамРаздел7, "160") - ЗначениеВКэшеЯчеек(7, КэшЗначенийЯчеек, СоответствиеСтрокЯчейкамРаздел7, "210");
	
	Если Значение160Минус210 > 0 Тогда
		УстановитьЗначениеВКэшЯчеек(7, КэшЗначенийЯчеек, СоответствиеСтрокЯчейкамРаздел7, "250", Значение160Минус210);
		УстановитьЗначениеВКэшЯчеек(7, КэшЗначенийЯчеек, СоответствиеСтрокЯчейкамРаздел7, "260", 0);
	ИначеЕсли Значение160Минус210 < 0 Тогда
		УстановитьЗначениеВКэшЯчеек(7, КэшЗначенийЯчеек, СоответствиеСтрокЯчейкамРаздел7, "250", 0);
		Значение210Минус160 = ЗначениеВКэшеЯчеек(7, КэшЗначенийЯчеек, СоответствиеСтрокЯчейкамРаздел7, "210") - ЗначениеВКэшеЯчеек(7, КэшЗначенийЯчеек, СоответствиеСтрокЯчейкамРаздел7, "160");
		УстановитьЗначениеВКэшЯчеек(7, КэшЗначенийЯчеек, СоответствиеСтрокЯчейкамРаздел7, "260", Значение210Минус160);
	КонецЕсли;

	УстановитьЗначениеВКэшЯчеек(7, КэшЗначенийЯчеек, СоответствиеСтрокЯчейкамРаздел7, "270", 0);
	УстановитьЗначениеВКэшЯчеек(7, КэшЗначенийЯчеек, СоответствиеСтрокЯчейкамРаздел7, "280", 0);
	УстановитьЗначениеВКэшЯчеек(7, КэшЗначенийЯчеек, СоответствиеСтрокЯчейкамРаздел7, "290", 0);
	УстановитьЗначениеВКэшЯчеек(7, КэшЗначенийЯчеек, СоответствиеСтрокЯчейкамРаздел7, "300", 0);
	УстановитьЗначениеВКэшЯчеек(7, КэшЗначенийЯчеек, СоответствиеСтрокЯчейкамРаздел7, "310", 0);
	УстановитьЗначениеВКэшЯчеек(7, КэшЗначенийЯчеек, СоответствиеСтрокЯчейкамРаздел7, "320", 0);
	
	Если ЗначениеВКэшеЯчеек(5, КэшЗначенийЯчеек, СоответствиеСтрокЯчейкамРаздел5, "250") < 0 Тогда
		УстановитьЗначениеВКэшЯчеек(7, КэшЗначенийЯчеек, СоответствиеСтрокЯчейкамРаздел7, "020", 0, Истина);
		УстановитьЗначениеВКэшЯчеек(7, КэшЗначенийЯчеек, СоответствиеСтрокЯчейкамРаздел7, "030", 0, Истина);
		УстановитьЗначениеВКэшЯчеек(7, КэшЗначенийЯчеек, СоответствиеСтрокЯчейкамРаздел7, "100", 0, Истина);
		УстановитьЗначениеВКэшЯчеек(7, КэшЗначенийЯчеек, СоответствиеСтрокЯчейкамРаздел7, "110", 0, Истина);
		УстановитьЗначениеВКэшЯчеек(7, КэшЗначенийЯчеек, СоответствиеСтрокЯчейкамРаздел7, "210", 0, Истина); 	
		УстановитьЗначениеВКэшЯчеек(7, КэшЗначенийЯчеек, СоответствиеСтрокЯчейкамРаздел7, "220", 0, Истина); 	
		УстановитьЗначениеВКэшЯчеек(7, КэшЗначенийЯчеек, СоответствиеСтрокЯчейкамРаздел7, "230", 0, Истина); 	
	КонецЕсли;
	            
	ВывестиДанныеВТабличныйДокумент(КэшЗначенийЯчеек["Раздел7"]);
	
	Сообщить("Раздел 7, стр. 1 заполнен");	
...
Процедура ЗаполнитьРаздел2()
	...
	МассивПоиска = Новый Массив;
	МассивПоиска.Добавить("Раздел 2");
	СтрокаНужногоРаздела = НайтиСтрокуДереваЗначений(МассивПоиска);
	
	Если СтрокаНужногоРаздела =  Неопределено Тогда
		Сообщить("Невозможно заполнить 2-й раздел, таблицы не найдены в дереве разделов");
		Возврат;
	КонецЕсли;      
	
	Элементы.РазделыОтчета.ТекущаяСтрока = СтрокаНужногоРаздела.ПолучитьИдентификатор();
	
	СохраненныеДанные = ПолучитьРеквизитыПодразделенийДляРаздела2();
	
	Если СохраненныеДанные = Неопределено Тогда
		Сообщить("Для заполнения 2-го раздела отсутствуют данные по подразделению " + Подразделение + ". Заполните данные в карточке подразделения, вкладка ""дополнительно""");
		Возврат;
	КонецЕсли;
	
	Для каждого ЭлементДанных из СохраненныеДанные Цикл
		
		ИмяЯчейки 		= ЭлементДанных.Ключ;
		
		Если ИмяЯчейки <> "ТаблицаG" Тогда
			
			ЗначениеЯчейки 	= ЭлементДанных.Значение;
			
			Если ТипЗнч(ЗначениеЯчейки) = Тип("Булево") Тогда
				ЗначениеЯчейки = ?(ЗначениеЯчейки = Истина, "Х", "");
			КонецЕсли;
			
			ТабличныйДокумент.Области[ИмяЯчейки].Значение = ЗначениеЯчейки;
			ОбработкаИзмененияСодержимогоВПолеТабличногоДокумента(Элементы.ТабличныйДокумент, ТабличныйДокумент.Области[ИмяЯчейки]);  	
			
		Иначе
			
			СчетчикСтрок = 1;			                   
			
			Для каждого СтруктураСтроки из ЭлементДанных.Значение Цикл
				
				Если СчетчикСтрок > 1 Тогда
					#Если ВебКлиент Тогда		
						ДобавитьСтрокуНаСервере("Раздел2");		
					#Иначе    		
						ДобавитьСтрокуНаКлиенте("Раздел2");		
					#КонецЕсли  
				КонецЕсли;
				
				Для каждого ЭлементСтруктурыСтроки из СтруктураСтроки Цикл
					ИмяЯчейки 		= ЭлементСтруктурыСтроки.Ключ + "_" + СчетчикСтрок;		
					ЗначениеЯчейки 	= ЭлементСтруктурыСтроки.Значение;
					Если ТипЗнч(ЗначениеЯчейки) = Тип("Булево") Тогда
						ЗначениеЯчейки = ?(ЗначениеЯчейки = Истина, "Х", "");
					КонецЕсли;
					ТабличныйДокумент.Области[ИмяЯчейки].Значение = ЗначениеЯчейки;
	...
	Сообщить("Раздел 2 заполнен");	
	                 
КонецПроцедуры

&НаКлиенте
Процедура ЗаполнитьРаздел1_1()
	
	МассивПоиска = Новый Массив;
	МассивПоиска.Добавить("Раздел 1.1");
	СтрокаНужногоРаздела = НайтиСтрокуДереваЗначений(МассивПоиска);
	
	Если СтрокаНужногоРаздела =  Неопределено Тогда
		Сообщить("Невозможно заполнить 1.1 -й раздел, таблицы не найдены в дереве разделов");
		Возврат;
	КонецЕсли;      
	...
	ИмяЯчейки 		= "КодОКАТОР";
	ЗначениеЯчейки 	= Подразделение.КодПоОКТМО;	
	ТабличныйДокумент.Области[ИмяЯчейки].Значение = ЗначениеЯчейки;
	ОбработкаИзмененияСодержимогоВПолеТабличногоДокумента(Элементы.ТабличныйДокумент, ТабличныйДокумент.Области[ИмяЯчейки]);
	
	Сообщить("Раздел 1.1 заполнен");
...
Процедура ЗаполнитьТитульныйЛист()
	...
	МассивПоиска = Новый Массив;
	МассивПоиска.Добавить("Титульный лист");
	СтрокаНужногоРаздела = НайтиСтрокуДереваЗначений(МассивПоиска);
	
	Если СтрокаНужногоРаздела =  Неопределено Тогда
		Сообщить("Невозможно заполнить титульный лист, таблицы не найдены в дереве разделов");
		Возврат;
	КонецЕсли;      
	...
	ИмяЯчейки 		= "КодИМНС";
	ЗначениеЯчейки 	= Подразделение.РегистрацияВНалоговомОргане.Код;
	ТабличныйДокумент.Области[ИмяЯчейки].Значение = ЗначениеЯчейки;
	ОбработкаИзмененияСодержимогоВПолеТабличногоДокумента(Элементы.ТабличныйДокумент, ТабличныйДокумент.Области[ИмяЯчейки]);  	
	
	ИмяЯчейки 		= "КПП1";
	ЗначениеЯчейки 	= Подразделение.РегистрацияВНалоговомОргане.КПП;	
	ТабличныйДокумент.Области[ИмяЯчейки].Значение = ЗначениеЯчейки;
	ОбработкаИзмененияСодержимогоВПолеТабличногоДокумента(Элементы.ТабличныйДокумент, ТабличныйДокумент.Области[ИмяЯчейки]);  	
	
	ИмяЯчейки 		= "ОргИМНС";
	ЗначениеЯчейки 	= Подразделение.РегистрацияВНалоговомОргане.НаименованиеИФНС;	
	ТабличныйДокумент.Области[ИмяЯчейки].Значение = ЗначениеЯчейки;
	ОбработкаИзмененияСодержимогоВПолеТабличногоДокумента(Элементы.ТабличныйДокумент, ТабличныйДокумент.Области[ИмяЯчейки]);  		
	
	Сообщить("Титульный лист заполнен");	
	                 
КонецПроцедуры

&НаСервере
Функция ПолучитьРеквизитыПодразделенийДляРаздела2()
...
&НаКлиенте
Процедура ВывестиДанныеВТабличныйДокумент(СтруктураЯчеекРаздела)  	
		
	Для каждого Ячейка из СтруктураЯчеекРаздела Цикл
		ИмяЯчейки 		= Ячейка.Ключ;
...
&НаСервереБезКонтекста
Функция ПолучитьДанныеПредыдущегоОтчета(ПериодОтчета,Подразделение)     	
	...
	Запрос.Текст = 
		...
		|ГДЕ
		|	РегламентированныйОтчет.ИсточникОтчета = ""РегламентированныйОтчетПрибыльИностраннойОрганизации""
		|	И НЕ РегламентированныйОтчет.ПометкаУдаления
		|	И РАЗНОСТЬДАТ(РегламентированныйОтчет.Дата, &ПериодОтчета, КВАРТАЛ) = 1
		|	И РегламентированныйОтчет.Подразделение = &Подразделение
		|
		|УПОРЯДОЧИТЬ ПО
		|	РегламентированныйОтчет.Дата УБЫВ";
	...
	Если Результат.Количество() Тогда
		...
		Попытка
			ДанныеЯчеек = Результат[0].ДанныеОтчета.Получить().ДанныеМногостраничныхРазделов.Раздел7[0].Данные;	
			ОшибкаПриПолученииДанных = Ложь;
		Исключение
			ОшибкаПриПолученииДанных = Истина;
		КонецПопытки;                         		
		
		ДанныеПоПредыдущейДекларации = Новый Структура;
		ДанныеПоПредыдущейДекларации.Вставить("ДанныеЯчеек", 				ДанныеЯчеек);
		ДанныеПоПредыдущейДекларации.Вставить("ОшибкаПриПолученииДанных", 	ОшибкаПриПолученииДанных);
		ДанныеПоПредыдущейДекларации.Вставить("Документ", 					Результат[0].Ссылка);       		

		Возврат ДанныеПоПредыдущейДекларации;
...
&НаКлиенте 
Функция ПрочитатьФайлExcel(Путь, Отказ)
	...
	RecordSet.ActiveConnection = Connection;
	RecordSet = Connection.OpenSchema(20);
	SqlString = "select * from [" + RecordSet.Fields("Table_Name").Value+ "]";
	RecordSet.Close();
	RecordSet.Open(SqlString);
