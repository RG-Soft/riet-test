...
Процедура ПолеВыбораПериодичностиПоказаПериодаПриИзменении(Элемент)
	...
	Если Найти(ВРег(СтрВыбораПериодичностиПоказаПериода),"КВАРТАЛ") > 1 Тогда
...
Процедура ОткрытьФормуОтчета(Команда)
	...
	ПараметрыФормы.Вставить("мВыбраннаяФорма",          "ФормаОтчета2015Кв1");
	Сообщить("Для проверки новой формы используется 2015 форма! Отключение в основной форме отчета ""ОткрытьФормуОтчета"" ");
	//<=	
	...
	ОткрытьФорму(СтрЗаменить(ЭтаФорма.ИмяФормы, "ОсновнаяФорма", "") + "ФормаОтчета2015Кв1", ПараметрыФормы);	    
	//<=
