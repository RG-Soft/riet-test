...
Процедура СформироватьОтчет(ПараметрыОтчета, АдресХранилища) Экспорт
	...
	// { RGS LFedotova 20.01.2018 21:19:53 - вопрос SLI-0007469
	Если ВерсияПостановленияНДС1137 = 4 Тогда
		Макет = ПолучитьОбщийМакет("ЖурналУчетаСчетовФактур981");
	// } RGS LFedotova 20.01.2018 21:20:24 - вопрос SLI-0007469
	...
	СписокДоступныхОрганизаций.Добавить(ПараметрыОтчета.Организация);
	//<=
...
#КонецОбласти
	...
	ОтветственныеЛица = ОбщегоНазначения.ОтветственныеЛица(СтруктураПараметров.Организация, СтруктураПараметров.НалоговыйПериод);
	//<=
