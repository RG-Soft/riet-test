...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	ПолеТабличногоДокументаФормаОтчета.Вывести(
		Отчеты[Сред(Лев(ЭтаФорма.ИмяФормы, Найти(ЭтаФорма.ИмяФормы, ".Форма.") - 1), 7)].ПолучитьМакет("ФормаОтчета2010Кв1_ФормаОтчета"));
...
Процедура Расчет(Форма, ИмяТекущейСтраницыПанели = "", ОбластьИмя = "")
	...
	Для НомСтр = 1 По Форма.П0002021.Количество() Цикл
		Сумма1 = Сумма1 + РегламентированнаяОтчетностьКлиентСервер.ОкруглитьЧислоПоФормату(Форма, Форма.ПолеТабличногоДокументаФормаОтчета.Области["П000202104_" + НомСтр].Значение);
		Сумма2 = Сумма2 + РегламентированнаяОтчетностьКлиентСервер.ОкруглитьЧислоПоФормату(Форма, Форма.ПолеТабличногоДокументаФормаОтчета.Области["П000202105_" + НомСтр].Значение);
		Сумма3 = Сумма3 + РегламентированнаяОтчетностьКлиентСервер.ОкруглитьЧислоПоФормату(Форма, Форма.ПолеТабличногоДокументаФормаОтчета.Области["П000202106_" + НомСтр].Значение);
	КонецЦикла;
	...
	Если ТаблицаВариантовЗаполнения.Количество() > 0 И ТаблицаВариантовЗаполнения[0].ВариантЗаполнения = 4 Тогда
		Форма.ПолеТабличногоДокументаФормаОтчета.Области["П000100103"].Значение = Сумма1;
	КонецЕсли;
	
	ВариантыЗаполненияПоказателя = ТаблицаВариантовЗаполнения.НайтиСтроки(Новый Структура("КодПоказателя", "П000100104"));
	Если ТаблицаВариантовЗаполнения.Количество() > 0 И ТаблицаВариантовЗаполнения[0].ВариантЗаполнения = 4 Тогда
		Форма.ПолеТабличногоДокументаФормаОтчета.Области["П000100104"].Значение = Сумма2;
	КонецЕсли;
	
	ВариантыЗаполненияПоказателя = ТаблицаВариантовЗаполнения.НайтиСтроки(Новый Структура("КодПоказателя", "П000100105"));
	Если ТаблицаВариантовЗаполнения.Количество() > 0 И ТаблицаВариантовЗаполнения[0].ВариантЗаполнения = 4 Тогда
		Форма.ПолеТабличногоДокументаФормаОтчета.Области["П000100105"].Значение = Сумма3;
	КонецЕсли;
...
Процедура ОчиститьТабличноеПолеНаСервере()
	...
	ВыбТабличноеПоле.Вывести(Отчеты[Сред(Лев(ЭтаФорма.ИмяФормы, Найти(ЭтаФорма.ИмяФормы, ".Форма.") - 1), 7)].ПолучитьМакет("ФормаОтчета2010Кв1_ФормаОтчета"));
...
Процедура СформироватьСпискиВыбора()
	...
	Если СтруктураРеквизитовФормы.мДатаКонцаПериодаОтчета < '20120101' Тогда
		МакетСоставаПоказателей = Отчеты[Сред(Лев(ЭтаФорма.ИмяФормы, Найти(ЭтаФорма.ИмяФормы, ".Форма.") - 1), 7)].ПолучитьМакет("Списки2010Кв1");
	Иначе	
		МакетСоставаПоказателей = Отчеты[Сред(Лев(ЭтаФорма.ИмяФормы, Найти(ЭтаФорма.ИмяФормы, ".Форма.") - 1), 7)].ПолучитьМакет("Списки2012Кв1");
	КонецЕсли;
...
Функция ПроверитьПоказатели() Экспорт
	...
	Если СтруктураРеквизитовФормы.мДатаКонцаПериодаОтчета < '20120101' Тогда
		ИмяМакета = "СхемаВыгрузкиXML2010Кв1"
	Иначе
		ИмяМакета = "СхемаВыгрузкиXML2012Кв1"
	КонецЕсли;
	ДеревоВыгрузки = РегламентированнаяОтчетность.ПолучитьДеревоВыгрузки(ЭтаФорма, ИмяМакета);
...
Функция ПолучитьДокументИлиПараметрыОтбораП1Пр3(ПараметрыОтбораОтчета)
	...
	Запрос.УстановитьПараметр("НаименованиеОтчета", "Статистика: Форма П-1 Приложение 3"); 
	...
	|	И РегламентированныйОтчет.НаименованиеОтчета = &НаименованиеОтчета
...
Функция СформироватьСтруктуруДопАтрибутов() 
	...
	Если СтруктураРеквизитовФормы.мДатаКонцаПериодаОтчета < '20120101' Тогда
		Макет = Отчеты[Сред(Лев(ЭтаФорма.ИмяФормы, Найти(ЭтаФорма.ИмяФормы, ".Форма.") - 1), 7)].ПолучитьМакет("АтрибВыгрузкиXML2010Кв1");
	Иначе
		Макет = Отчеты[Сред(Лев(ЭтаФорма.ИмяФормы, Найти(ЭтаФорма.ИмяФормы, ".Форма.") - 1), 7)].ПолучитьМакет("АтрибВыгрузкиXML2012Кв1");
	КонецЕсли;
...
Функция ПроверитьКонтрольныеСоотношенияВОтчете() Экспорт
	...
	Нарушение = "Приказ Росстата от 14.10.2009 № 226";
