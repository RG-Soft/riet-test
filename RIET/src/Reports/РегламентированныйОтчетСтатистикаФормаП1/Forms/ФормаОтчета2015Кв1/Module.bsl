...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	СтруктураРеквизитовФормы.Вставить("ВыбранныйОтчетПриложение3", Неопределено);
	...
	//
	// Версия формы
	СтруктураРеквизитовФормы.мВерсияФормы = "12/12/2015";
	...
	ПолеТабличногоДокументаФормаОтчета.Вывести(
		Отчеты[Сред(Лев(ЭтаФорма.ИмяФормы, Найти(ЭтаФорма.ИмяФормы, ".Форма.") - 1), 7)].ПолучитьМакет("ФормаОтчета2015Кв1_ФормаОтчета"));
	...
	КонецЕсли;
	// { RGS MYurkevich 9/11/2015 2:45:42 PM - RCA-0000094 
	...
	КонецЕсли;   
	// } RGS MYurkevich 9/11/2015 2:45:43 PM - RCA-0000094 
...
Процедура СформироватьСтруктуруРеквизитовФормы()
	...
	// { RGS MYurkevich 9/11/2015 3:24:35 PM - RCA-0000094
	СтруктураРеквизитовФормы.ОтображатьКнопкуЗаполнить = Истина;
	// } RGS MYurkevich 9/11/2015 3:24:36 PM - RCA-0000094
КонецПроцедуры
...
Процедура Инициализация(БезОткрытияФормы = Ложь) Экспорт
	...
	Иначе
		...
		Если ЗначениеЗаполнено(СтруктураРеквизитовФормы.мСкопированаФорма) Тогда
			...
			СтруктураРеквизитовФормы.ВыбранныйОтчетПриложение3 = Неопределено;
		КонецЕсли;
	...
	Если ПустаяСтрока(ПолеТабличногоДокументаФормаОтчета.Области.П000101903.Значение) И ПустаяСтрока(ПолеТабличногоДокументаФормаОтчета.Области.П000102003.Значение) Тогда
		ПолеТабличногоДокументаФормаОтчета.Области.П000102003.Значение = "V";
	КонецЕсли; 
	...
	Если СтруктураРеквизитовФормы.ОтображатьКнопкуЗаполнить Тогда
		// { RGS MYurkevich 9/11/2015 3:39:36 PM - 		
	//	Если НЕ РегламентированнаяОтчетностьПереопределяемый.РеализованаНастройкаАвтозаполняемыхПоказателейРеглОтчета(Сред(Лев(ЭтаФорма.ИмяФормы, Найти(ЭтаФорма.ИмяФормы, ".Форма.") - 1), 7), Сред(ЭтаФорма.ИмяФормы, Найти(ЭтаФорма.ИмяФормы, ".Форма.") + 7)) Тогда
	//		
	...
	//		
	//	КонецЕсли;
		// } RGS MYurkevich 9/11/2015 3:39:37 PM - 	
	Иначе
...
Процедура ВосстановитьСохраненныеДанные()
	...
	// восстановим данные о выбранном приложении №3
	СписокСохранения.Свойство("ВыбранныйОтчетПриложение3", СтруктураРеквизитовФормы.ВыбранныйОтчетПриложение3);
...
Функция Сохранить(Автосохранение = Ложь, КодИФНС = "не применимо", Вариант, ВидДокументаНомерКорректировкиИзменен = Неопределено, ПоказателиОтчета) Экспорт
	...
	// { RGS MYurkevich 9/11/2015 3:03:04 PM - RCA-0000094
	СтруктураРеквизитовФормы.мСохраненныйДок.Подразделение              = СтруктураРеквизитовФормы.Подразделение;
	// } RGS MYurkevich 9/11/2015 3:03:05 PM - RCA-0000094
	...
	// вставляем выбранный отчет Приложения №3
	СписокСохранения.Вставить("ВыбранныйОтчетПриложение3", СтруктураРеквизитовФормы.ВыбранныйОтчетПриложение3);	
...
Процедура ОтфильтроватьЕдиницыИзмеренияПоКодуОКПД(КодОКП, ИмяСписка)
	...
	|ГДЕ
	|	ТЗКодыЕдиницИзмерения.КодОКП = &КодОКП Или &КодОКППуст
	|;
	|
	|ВЫБРАТЬ РАЗЛИЧНЫЕ
	|	ТЗКодыЕдиницИзмерения.Код,
...
Процедура ПолеТабличногоДокументаФормаОтчетаВыбор(Элемент, Область, СтандартнаяОбработка)
	...
	ИначеЕсли Найти("П000101503,П000101603,П000101703,П000101803,П000101903,П000102003", Область.Имя) > 0 Тогда
		...
		ИначеЕсли Область.Имя = "П000101903" Тогда
			ПолеТабличногоДокументаФормаОтчета.Области.П000102003.Значение = "";
		ИначеЕсли Область.Имя = "П000102003" Тогда
			ПолеТабличногоДокументаФормаОтчета.Области.П000101903.Значение = "";
		КонецЕсли;
		
		Модифицированность = Истина;
	ИначеЕсли Не СтруктураРеквизитовФормы.мАвтоВыборКодов Тогда 
		Если Найти(Область.Имя, "П000500103_") = 1 Или Найти(Область.Имя, "П000500105_") = 1 Тогда
			...
			КонецЕсли;
		ИначеЕсли Найти(Область.Имя, "П000500101_") = 1 Или Найти(Область.Имя, "П000500104_") = 1 Тогда
			НомСтр = ПолучитьНомерВСтроке(Область.Имя);
			Если НомСтр = "50" Тогда 
		...
		ИначеЕсли Найти(Область.Имя, "П000500102_") = 1 Тогда
			...
			Если ПараметрВводаПоля <> Неопределено И ПараметрВводаПоля.ТаблицаЗначений <> Неопределено Тогда
				...
				ВвестиПоказатель(Элемент, ОбластьДляВвода, СмежнаяОбласть, КодПоказателя, Ложь, ПараметрВводаПоля.ТекстПриВыборе, ТЗВыбора, СтандартнаяОбработка, , МассивОчистки);
			КонецЕсли;
		ИначеЕсли Найти(Область.Имя, "П000202101_") = 1 Или Найти(Область.Имя, "П000202103_") = 1 Тогда
...
Процедура Расчет(Форма, ИмяТекущейСтраницыПанели = "", ОбластьИмя = "")
	...
	Для НомСтр = 1 По Форма.П0002021.Количество() Цикл
		...
		Сумма3 = Сумма3 + РегламентированнаяОтчетностьКлиентСервер.ОкруглитьЧислоПоФормату(Форма, Области["П000202106_" + НомСтр].Значение);
	КонецЦикла;
	...
	ВариантыЗаполненияПоказателя = ТаблицаВариантовЗаполнения.НайтиСтроки(Новый Структура("КодПоказателя", "П000100105"));
	Если ТаблицаВариантовЗаполнения.Количество() > 0 И ТаблицаВариантовЗаполнения[0].ВариантЗаполнения = 4 Тогда
		Области["П000100105"].Значение = Сумма3;
	КонецЕсли;
...
Процедура ОчиститьТабличноеПолеНаСервере()
	...
	ВыбТабличноеПоле.Вывести(Отчеты[Сред(Лев(ЭтаФорма.ИмяФормы, Найти(ЭтаФорма.ИмяФормы, ".Форма.") - 1), 7)].ПолучитьМакет("ФормаОтчета2015Кв1_ФормаОтчета"));
...
&НаКлиенте
Процедура ВыгрузитьОтчетПродолжение(Результат, ДополнительныеПараметры) Экспорт
		
    // Получаем приложение №3 для данного отчета
	ПараметрыОтбораОтчета = Новый Структура;
	ПараметрыОтбораОтчета.Вставить("Док",           СтруктураРеквизитовФормы.мСохраненныйДок);
	ПараметрыОтбораОтчета.Вставить("ДатаНачала",    СтруктураРеквизитовФормы.мДатаНачалаПериодаОтчета);
	ПараметрыОтбораОтчета.Вставить("ДатаОкончания", СтруктураРеквизитовФормы.мДатаКонцаПериодаОтчета);
	
	ТекДокП1Пр3 = Неопределено;
	
	ОписаниеОповещения = Новый ОписаниеОповещения("ВыгрузитьОтчетЗавершение", ЭтотОбъект);
	ПолучитьФормуП1Пр3(ПараметрыОтбораОтчета, ТекДокП1Пр3, ОписаниеОповещения);
		
КонецПроцедуры

&НаКлиенте
Процедура ВыгрузитьОтчетЗавершение(ТекФормаП1Пр3, ДополнительныеПараметры) Экспорт
	
	Если ТекФормаП1Пр3 <> Неопределено Тогда
		ТекДокП1Пр3 = ТекФормаП1Пр3.СтруктураРеквизитовФормы.мСохраненныйДок;
	Иначе
		ТекДокП1Пр3   = Неопределено;
	...
	СтруктураДополнительныхПараметров.Вставить("ТекДокП1Пр3",   ТекДокП1Пр3);
	СтруктураДополнительныхПараметров.Вставить("ТекФормаП1Пр3", ТекФормаП1Пр3);
		
	ФормаВыгрузкиРеглОтчета.СформироватьИЗаписать(СпДокОсн,,,,,,,,,,,,, СтруктураДополнительныхПараметров);
...
&НаКлиенте
Процедура ПолучитьФормуП1Пр3(ПараметрыОтбораОтчета, ДокументОтчета = Неопределено, ВыполняемоеОповещение, ВызовИзФормыОтчета = Истина) Экспорт
	
	Если ТипЗнч(ПараметрыОтбораОтчета) <> Тип("Структура") Тогда
		ВыполнитьОбработкуОповещения(ВыполняемоеОповещение, Неопределено);
		Возврат;
	КонецЕсли;
	
	ДокументОтчета = ПолучитьДокументИлиПараметрыОтбораП1Пр3(ПараметрыОтбораОтчета);
	
	Если ДокументОтчета = Неопределено Тогда
		
		Если ПараметрыОтбораОтчета.Свойство("ИсточникОтчета") Тогда
			
			ФормаВыбораОтчета = РегламентированнаяОтчетностьКлиент.ПолучитьОбщуюФормуПоИмени("ФормаВыбораОтчета", ПараметрыОтбораОтчета, ЭтаФорма);
			ФормаВыбораОтчета.Элементы.ФормаСоздатьОтчет.Доступность = Ложь;
			
			ДополнительныеПараметры = Новый Структура("ПараметрыОтбораОтчета, ДокументОтчета, ВыполняемоеОповещение", ПараметрыОтбораОтчета, ДокументОтчета, ВыполняемоеОповещение);
			ОписаниеОповещения = Новый ОписаниеОповещения("ПолучитьФормуП1Пр3Завершение", ЭтотОбъект, ДополнительныеПараметры);
		...
		Иначе
			...
			// Запомним выбранный отчет Приложение№3 для выгрузки из журнала
			СтруктураРеквизитовФормы.ВыбранныйОтчетПриложение3 = "Выгрузка без приложения";
	...
	Иначе
		...
		// Запомним выбранный отчет Приложение№3 для выгрузки из журнала
		СтруктураРеквизитовФормы.ВыбранныйОтчетПриложение3 = ДокументОтчета;
		
	КонецЕсли; 
	
	Если НЕ ПараметрыОтбораОтчета.Свойство("ИсточникОтчета") Тогда
		ВыполнитьОбработкуОповещения(ВыполняемоеОповещение, Неопределено);
		Возврат;
	КонецЕсли;

	ИмяТекФормы = ПолучитьФормуДляПериода(ПараметрыОтбораОтчета.ИсточникОтчета, ПараметрыОтбораОтчета.ДатаКонцаПериодаОтчета);
	
	ПараметрыФормы = Новый Структура;
	ПараметрыФормы.Вставить("мСохраненныйДок",          ДокументОтчета);
	ПараметрыФормы.Вставить("мДатаНачалаПериодаОтчета", ПараметрыОтбораОтчета.ДатаНачалаПериодаОтчета);
	ПараметрыФормы.Вставить("мДатаКонцаПериодаОтчета",  ПараметрыОтбораОтчета.ДатаКонцаПериодаОтчета);
	ПараметрыФормы.Вставить("мВыбраннаяФорма",          ИмяТекФормы);
	
	ФормаОтчетаП1Пр3 = ПолучитьФорму("Отчет." + ПараметрыОтбораОтчета.ИсточникОтчета + ".Форма." + ИмяТекФормы, ПараметрыФормы, , Новый УникальныйИдентификатор);

	ВыполнитьОбработкуОповещения(ВыполняемоеОповещение, ФормаОтчетаП1Пр3);
	
КонецПроцедуры

&НаКлиенте
Процедура ПолучитьФормуП1Пр3Завершение(РезультатВыбора, ДополнительныеПараметры) Экспорт
	
	ПараметрыОтбораОтчета = ДополнительныеПараметры.ПараметрыОтбораОтчета;
	ДокументОтчета = ДополнительныеПараметры.ДокументОтчета;
	ВыполняемоеОповещение = ДополнительныеПараметры.ВыполняемоеОповещение;
	
	Если РезультатВыбора <> Неопределено Тогда
		ДокументОтчета = РезультатВыбора;
		// Запомним выбранный отчет Приложение№3 для выгрузки из журнала
		СтруктураРеквизитовФормы.ВыбранныйОтчетПриложение3 = ДокументОтчета;
	Иначе
		Возврат;
...
&НаСервере
Функция ПолучитьФормуДляПериода(ИсточникОтчета, НаДату)
	
	ТаблицаФормОтчета = Отчеты[ИсточникОтчета].ТаблицаФормОтчета();
	...
	Для Каждого Строка Из ТаблицаФормОтчета Цикл
		...
		Возврат Строка.ФормаОтчета;
		
	КонецЦикла;

	Возврат	ТаблицаФормОтчета[0].ФормаОтчета;

КонецФункции

&НаСервере
Функция ПолучитьДокументИлиПараметрыОтбораП1Пр3(ПараметрыОтбораОтчета)
	
	Док = Неопределено;
	ПараметрыОтбораОтчета.Свойство("Док", Док);
	...
	Запрос.УстановитьПараметр("Периодичность", Док.Периодичность); 
	Запрос.УстановитьПараметр("Период", Док.Период);
	...
	ИначеЕсли ТаблицаЗапроса.Количество() = 1 Тогда
		ТекСтрока = ТаблицаЗапроса[0];
	КонецЕсли;
		
	СписокОрганизаций = Новый СписокЗначений;
	СписокОрганизаций.Добавить(Док.Организация);
	
	ПараметрыОтбора = Новый Структура;
	ПараметрыОтбора.Вставить("ИсточникОтчета", ТаблицаЗапроса[0].Док.ИсточникОтчета);
	ПараметрыОтбора.Вставить("Периодичность", "Произвольный");
	ПараметрыОтбора.Вставить("Организация", СписокОрганизаций);
	ПараметрыОтбора.Вставить("ОтборКодИФНС", Ложь);
	ПараметрыОтбора.Вставить("КодИФНС", "");
	ПараметрыОтбора.Вставить("ОтборПериод", Истина);
	ПараметрыОтбора.Вставить("ДатаНачалаПериодаОтчета", ПараметрыОтбораОтчета.ДатаНачала);
	ПараметрыОтбора.Вставить("ДатаКонцаПериодаОтчета", ПараметрыОтбораОтчета.ДатаОкончания);
	
	ПараметрыОтбораОтчета = ПараметрыОтбора;
	
	Если ТекСтрока <> Неопределено Тогда
		
		Возврат ТекСтрока.Док;
...
&НаСервере
Функция ОбъединитьТекстыXML_П1(ТекстXMLОсн, ТекстXMLДоп) Экспорт
	...
	НайденаНужнаяСекция = Ложь;
	ЧтениеXMLДоп = Новый ЧтениеXML;
	ЧтениеXMLДоп.УстановитьСтроку(ТекстXMLДоп);
	Пока ЧтениеXMLДоп.Прочитать() Цикл
		Если (ЧтениеXMLДоп.ТипУзла = ТипУзлаXML.НачалоЭлемента) и ЧтениеXMLДоп.Имя = "section" Тогда	
			Если ЧтениеXMLДоп.КоличествоАтрибутов() > 0 Тогда
				Если ЧтениеXMLДоп.ПолучитьАтрибут("code") = "6" Тогда
					НайденаНужнаяСекция = Истина;
					Прервать;
				КонецЕсли;	
			КонецЕсли;
		КонецЕсли;	
	...
	ЧтениеXMLОсн = Новый ЧтениеXML;
	ЧтениеXMLОсн.УстановитьСтроку(ТекстXMLОсн);
	СтрокаДС = ДеревоСтруктуры;
	Пока ЧтениеXMLОсн.Прочитать() Цикл
		Если ЧтениеXMLОсн.ТипУзла = ТипУзлаXML.НачалоЭлемента Тогда
			Если ЧтениеXMLОсн.Имя = "section" И НайденаНужнаяСекция Тогда
				Если ЧтениеXMLОсн.КоличествоАтрибутов() > 0 Тогда
					Если ЧтениеXMLОсн.ПолучитьАтрибут("code") = "6" Тогда
						ЧтениеXMLОсн = ЧтениеXMLДоп;
					КонецЕсли;	
				КонецЕсли;
			КонецЕсли;	
			СтрокаДС = СтрокаДС.Строки.Добавить();
			СтрокаДС.Тип = "С";
			СтрокаДС.Код = ЧтениеXMLОсн.Имя;
			Если ЧтениеXMLОсн.КоличествоАтрибутов() > 0 Тогда
				Строки = СтрокаДС.Строки;
				Пока ЧтениеXMLОсн.ПрочитатьАтрибут() Цикл
					Атрибут = Строки.Добавить();
					Атрибут.Код = ЧтениеXMLОсн.Имя;
					Атрибут.Значение = ЧтениеXMLОсн.Значение;
					Атрибут.Тип = "А";
				КонецЦикла;
			КонецЕсли;
		ИначеЕсли ЧтениеXMLОсн.ТипУзла = ТипУзлаXML.КонецЭлемента Тогда
			СтрокаДС = СтрокаДС.Родитель;
		Иначе
			СтрокаДС.Значение = ЧтениеXMLОсн.Значение;
		Конецесли;
	КонецЦикла;
	ЧтениеXMLОсн.Закрыть();
	
	Парам = Новый Структура("ИмяФайлаСхемы", "");
	Возврат РегламентированнаяОтчетность.ВыгрузитьДеревоВXML(ДеревоСтруктуры, Парам);
...
Функция ПроверитьВозможностьВыгрузки(ПараметрыВыгрузки, ДеревоВыгрузки)
	...
	Если ПустаяСтрока(ПараметрыВыгрузки.ОргНазв) Тогда
		РегламентированнаяОтчетность.СообщитьВТаблицуСообщений(ЭтаФорма, "Не задано наименование оганизации!", ИмяПоляТаблДокумента, "ОргНазв");
		СтатусПроверки = Ложь;
	КонецЕсли;
	...
	Если НЕ РегламентированнаяОтчетность.ПроверитьРазделыНаНаличиеИнформации(ЭтаФорма, ДеревоВыгрузки, ИмяПоляТаблДокумента) Тогда
		...
		РегламентированнаяОтчетность.СообщитьВТаблицуСообщений(ЭтаФорма, "Не заполнены разделы 1, 2, 3, 4, 5!", ИмяПоляТаблДокумента, "");
		//СтатусПроверки = Ложь;
...
Функция СформироватьСтруктуруПараметров()
	...
	ПараметрыВыгрузки.Вставить("Зн_П000101903", ?(ПустаяСтрока(ТабДокумент.П000101903), 20, 19));
...
Процедура ЗаполнитьАвто(Команда)
	...
	Отказ = Ложь;
	// { RGS MYurkevich 9/14/2015 11:04:55 AM - 
	СтруктураРеквизитовФормы.мВПрограммеИзмененаОрганизация = Истина;
	ЗаполнитьСведенияОбОрганизацииНаКлиенте();
	СтруктураРеквизитовФормы.мВПрограммеИзмененаОрганизация = Ложь;	
	// } RGS MYurkevich 9/14/2015 11:04:56 AM - 
	ДополнительныеПараметры = Новый Структура();
	ДополнительныеПараметры.Вставить("Команда", Команда);
...
Процедура ЗаполнитьАвтоЗавершение(Отказ, ДополнительныеПараметры) Экспорт
	...
	ИДОтчета = Сред(Лев(ЭтаФорма.ИмяФормы, Найти(ЭтаФорма.ИмяФормы, ".Форма.") - 1), 7);
	ИДРедакцииОтчета = Сред(ЭтаФорма.ИмяФормы, Найти(ЭтаФорма.ИмяФормы, ".Форма.") + 7);
...
&НаСервере
Процедура ЗагрузитьПодготовленныеДанные(Контейнер=Неопределено)
...
Функция ЗаполнитьАвтоНаСервере()
	...
	// { RGS MYurkevich 9/11/2015 3:04:04 PM - RCA-0000094
	ПараметрыОтчета.Вставить("Подразделение", 			     СтруктураРеквизитовФормы.Подразделение);
	// } RGS MYurkevich 9/11/2015 3:04:05 PM - RCA-0000094
	Для Каждого ЭлементСтруктуры Из мСтруктураМногострочныхРазделов Цикл
		ОбновитьСтруктуруМногострочногоРаздела(ЭтаФорма, ЭлементСтруктуры.Значение);
	КонецЦикла;
	...
	//		АдресХранилища);
	СтруктураВыручки = ПолучитьВыручку(Подразделение, СтруктураРеквизитовФормы.мДатаНачалаПериодаОтчета, СтруктураРеквизитовФормы.мДатаКонцаПериодаОтчета);
	Контейнер.ПолеТабличногоДокументаФормаОтчета.П000100103 = СтруктураВыручки.ОборотЗаМесяц;
	Контейнер.ПолеТабличногоДокументаФормаОтчета.П000100104 = СтруктураВыручки.ОборотЗаПредыдущийМесяц;
	Контейнер.ПолеТабличногоДокументаФормаОтчета.П000100105 = СтруктураВыручки.ОборотВПрошломГоду;
	Контейнер.ПолеТабличногоДокументаФормаОтчета.П000101703 = "V";
	Контейнер.ПолеТабличногоДокументаФормаОтчета.П000101803 = "";
	
	СтрокаНоменклатуры = Контейнер.ПолеТабличногоДокументаФормаОтчета_ДанныеДопСтрок1[0];
	СтрокаНоменклатуры.П000202101 = "Предоставление услуг по бурению, связанному с добычей нефти, газа и газового конденсата";
	СтрокаНоменклатуры.П000202103 = "11.20.1";
	СтрокаНоменклатуры.П000202104 = СтруктураВыручки.ОборотЗаМесяц;
	СтрокаНоменклатуры.П000202105 = СтруктураВыручки.ОборотЗаПредыдущийМесяц;
	СтрокаНоменклатуры.П000202106 = СтруктураВыручки.ОборотВПрошломГоду;
	// } RGS MYurkevich 9/11/2015 3:04:05 PM - RCA-0000094
...
Процедура Расшифровать(Команда)
	...
	РегламентированнаяОтчетностьКлиент.ОткрытьРасшифровкуОтчета(
		Сред(Лев(ЭтаФорма.ИмяФормы, Найти(ЭтаФорма.ИмяФормы, ".Форма.") - 1), 7), Сред(ЭтаФорма.ИмяФормы, Найти(ЭтаФорма.ИмяФормы, ".Форма.") + 7), ИДИменПоказателей, ПараметрыОтчета)
...
Функция ПроверитьКонтрольныеСоотношенияВОтчете() Экспорт
	...
	Нарушение = "Приказ Росстата от 04.09.2014 № 547";
	...
	Для НомерКолонки = 1 По 3 Цикл
		...
		ТекстФормулы = "стр. 11 гр. " + НомерКолонки + " " + Знак + " стр. 01 гр. " + НомерКолонки;
		Описание     = "стр. 11 гр. " + НомерКолонки + " " + Знак + " стр. 01 гр. " + НомерКолонки;
		
		ЛеваяЧасть  = РегламентированнаяОтчетностьКлиентСервер.ОкруглитьЧислоПоФормату(ЭтаФорма, Показатели["П00010110" + (2 + НомерКолонки)]);
		ПраваяЧасть = РегламентированнаяОтчетностьКлиентСервер.ОкруглитьЧислоПоФормату(ЭтаФорма, Показатели["П00010010" + (2 + НомерКолонки)]);
	...
	Для НомерКолонки = 1 По 3 Цикл
		...
		ТекстФормулы = "стр. 12 гр. " + НомерКолонки + " " + Знак + " стр. 01 гр. " + НомерКолонки;
		Описание     = "стр. 12 гр. " + НомерКолонки + " " + Знак + " стр. 01 гр. " + НомерКолонки;
		
		ЛеваяЧасть  = РегламентированнаяОтчетностьКлиентСервер.ОкруглитьЧислоПоФормату(ЭтаФорма, Показатели["П00010120" + (2 + НомерКолонки)]);
		ПраваяЧасть = РегламентированнаяОтчетностьКлиентСервер.ОкруглитьЧислоПоФормату(ЭтаФорма, Показатели["П00010010" + (2 + НомерКолонки)]);
	...
	ТекстФормулы = "стр. 14 гр. 1 " + Знак + " стр. 13 гр. 1";
	Описание     = "стр. 14 гр. 1 " + Знак + " стр. 13 гр. 1";
	
	ЛеваяЧасть  = РегламентированнаяОтчетностьКлиентСервер.ОкруглитьЧислоПоФормату(ЭтаФорма, Показатели["П000101403"]);
	ПраваяЧасть = РегламентированнаяОтчетностьКлиентСервер.ОкруглитьЧислоПоФормату(ЭтаФорма, Показатели["П000101303"]);
	...
	Для НомерКолонки = 1 По 3 Цикл
		...
		Если РегламентированнаяОтчетностьКлиентСервер.ОкруглитьЧислоПоФормату(ЭтаФорма, Показатели["П00010020" + (2 + НомерКолонки)]) <> 0 Тогда
			
			ТекстФормулы = "если стр. 02 гр. " + НомерКолонки + " " + Знак + " 0, то стр. 22 гр. " + НомерКолонки
				+ " + стр. 26 гр. " + НомерКолонки+ " + стр. 27 гр. " + НомерКолонки + " " + Знак + " 0";
			Описание     = "если стр. 02 гр. " + НомерКолонки + " " + Знак + " 0, то стр. 22 гр. " + НомерКолонки
				+ " и/или стр. 26 гр. " + НомерКолонки+ " и/или стр. 27 гр. " + НомерКолонки + " " + Знак + " 0";
			
			ЛеваяЧасть_1  = РегламентированнаяОтчетностьКлиентСервер.ОкруглитьЧислоПоФормату(ЭтаФорма, Показатели["П00030010" + (2 + НомерКолонки)]);
			ЛеваяЧасть_2  = РегламентированнаяОтчетностьКлиентСервер.ОкруглитьЧислоПоФормату(ЭтаФорма, Показатели["П00030050" + (2 + НомерКолонки)]);
			ЛеваяЧасть_3  = РегламентированнаяОтчетностьКлиентСервер.ОкруглитьЧислоПоФормату(ЭтаФорма, Показатели["П00030060" + (2 + НомерКолонки)]);
			
			ПраваяЧасть = 0;
			
			Расшифровка = Строка(ЛеваяЧасть_1) + " и/или " + Строка(ЛеваяЧасть_2)
				+ " и/или " + Строка(ЛеваяЧасть_3) + " " + Знак + " " + Строка(ПраваяЧасть);
	...
	Для НомерКолонки = 1 По 3 Цикл
		...
		ТекстФормулы = "сумма стр. 21 по гр. " + НомерКолонки + " " + Знак + " стр. 01 гр. " + НомерКолонки;
		Описание     = "сумма стр. 21 по гр. " + НомерКолонки + " " + Знак + " стр. 01 гр. " + НомерКолонки;
		
		Сумма = 0;
		Для НомСтр = 1 По П0002021.Количество() Цикл
			Сумма = Сумма + РегламентированнаяОтчетностьКлиентСервер.ОкруглитьЧислоПоФормату(ЭтаФорма, Показатели["П00020210" + (3 + НомерКолонки) + "_" + НомСтр]);
		КонецЦикла;
	...
	Для НомерКолонки = 1 По 3 Цикл
		...
		ТекстФормулы = "стр. 23 гр. " + НомерКолонки + " " + Знак + " стр. 22 гр. " + НомерКолонки;
		Описание     = "стр. 23 гр. " + НомерКолонки + " " + Знак + " стр. 22 гр. " + НомерКолонки;
		
		ЛеваяЧасть  = РегламентированнаяОтчетностьКлиентСервер.ОкруглитьЧислоПоФормату(ЭтаФорма, Показатели["П00030020" + (2 + НомерКолонки)]);
		ПраваяЧасть = РегламентированнаяОтчетностьКлиентСервер.ОкруглитьЧислоПоФормату(ЭтаФорма, Показатели["П00030010" + (2 + НомерКолонки)]);
	...
	Для НомерКолонки = 1 По 3 Цикл
		...
		ТекстФормулы = "стр. 24 гр. " + НомерКолонки + " " + Знак + " стр. 22 гр. " + НомерКолонки;
		Описание     = "стр. 24 гр. " + НомерКолонки + " " + Знак + " стр. 22 гр. " + НомерКолонки;
		
		ЛеваяЧасть  = РегламентированнаяОтчетностьКлиентСервер.ОкруглитьЧислоПоФормату(ЭтаФорма, Показатели["П00030030" + (2 + НомерКолонки)]);
		ПраваяЧасть = РегламентированнаяОтчетностьКлиентСервер.ОкруглитьЧислоПоФормату(ЭтаФорма, Показатели["П00030010" + (2 + НомерКолонки)]);
	...
	Для НомерКолонки = 1 По 3 Цикл
		...
		ТекстФормулы = "стр. 25 гр. " + НомерКолонки + " " + Знак + " стр. 24 гр. " + НомерКолонки;
		Описание     = "стр. 25 гр. " + НомерКолонки + " " + Знак + " стр. 24 гр. " + НомерКолонки;
		
		ЛеваяЧасть  = РегламентированнаяОтчетностьКлиентСервер.ОкруглитьЧислоПоФормату(ЭтаФорма, Показатели["П00030040" + (2 + НомерКолонки)]);
		ПраваяЧасть = РегламентированнаяОтчетностьКлиентСервер.ОкруглитьЧислоПоФормату(ЭтаФорма, Показатели["П00030030" + (2 + НомерКолонки)]);
	...
	Для НомерКолонки = 1 По 3 Цикл
		...
		ТекстФормулы = "стр. 30 гр. " + НомерКолонки + " " + Знак + " стр. 29 гр. " + НомерКолонки;
		Описание     = "стр. 30 гр. " + НомерКолонки + " " + Знак + " стр. 29 гр. " + НомерКолонки;
		
		ЛеваяЧасть  = РегламентированнаяОтчетностьКлиентСервер.ОкруглитьЧислоПоФормату(ЭтаФорма, Показатели["П00040020" + (3 + НомерКолонки)]);
		ПраваяЧасть = РегламентированнаяОтчетностьКлиентСервер.ОкруглитьЧислоПоФормату(ЭтаФорма, Показатели["П00040010" + (3 + НомерКолонки)]);
	...
	Для НомерКолонки = 1 По 3 Цикл
		...
		ТекстФормулы = "стр. 32 гр. " + НомерКолонки + " " + Знак + " стр. 31 гр. " + НомерКолонки;
		Описание     = "стр. 32 гр. " + НомерКолонки + " " + Знак + " стр. 31 гр. " + НомерКолонки;
		
		ЛеваяЧасть  = РегламентированнаяОтчетностьКлиентСервер.ОкруглитьЧислоПоФормату(ЭтаФорма, Показатели["П00040040" + (3 + НомерКолонки)]);
		ПраваяЧасть = РегламентированнаяОтчетностьКлиентСервер.ОкруглитьЧислоПоФормату(ЭтаФорма, Показатели["П00040030" + (3 + НомерКолонки)]);
	...
	Для НомерКолонки = 1 По 3 Цикл
		...
		Если РегламентированнаяОтчетностьКлиентСервер.ОкруглитьЧислоПоФормату(ЭтаФорма, Показатели["П00040010" + (3 + НомерКолонки)]) <> 0 Тогда
			
			ТекстФормулы = "если стр. 29 гр. " + НомерКолонки + " " + Знак + " 0, то стр. 31 гр. " + НомерКолонки + " " + Знак + " 0";
			Описание     = "если стр. 29 гр. " + НомерКолонки + " " + Знак + " 0, то стр. 31 гр. " + НомерКолонки + " " + Знак + " 0";
			
			ЛеваяЧасть  = РегламентированнаяОтчетностьКлиентСервер.ОкруглитьЧислоПоФормату(ЭтаФорма, Показатели["П00040030" + (3 + НомерКолонки)]);
	...
	Для НомерКолонки = 1 По 3 Цикл
		...
		Если РегламентированнаяОтчетностьКлиентСервер.ОкруглитьЧислоПоФормату(ЭтаФорма, Показатели["П00040020" + (3 + НомерКолонки)]) <> 0 Тогда
			
			ТекстФормулы = "если стр. 30 гр. " + НомерКолонки + " " + Знак + " 0, то стр. 32 гр. " + НомерКолонки + " " + Знак + " 0";
			Описание     = "если стр. 30 гр. " + НомерКолонки + " " + Знак + " 0, то стр. 32 гр. " + НомерКолонки + " " + Знак + " 0";
	...
	Для НомерКолонки = 1 По 3 Цикл
		...
		ТекстФормулы = "если стр. с кодом 70 по гр. " + НомерКолонки + " " + Знак + " 0, то стр. 22 гр. " + НомерКолонки + " " + Знак + " 0";
		Описание     = "если стр. с кодом 70 по гр. " + НомерКолонки + " " + Знак + " 0, то стр. 22 гр. " + НомерКолонки + " " + Знак + " 0";
		
		Сумма = 0;
		Для НомСтр = 1 По П0005001.Количество() Цикл
			Если СокрЛП(Показатели["П000500102_" + НомСтр]) = "70" Тогда
				Сумма = Сумма + РегламентированнаяОтчетностьКлиентСервер.ОкруглитьЧислоПоФормату(ЭтаФорма, Показатели["П00050010" + (5 + НомерКолонки) + "_" + НомСтр]);
			КонецЕсли;
		КонецЦикла;
		
		Если Сумма > 0 Тогда
			
			ЛеваяЧасть  = РегламентированнаяОтчетностьКлиентСервер.ОкруглитьЧислоПоФормату(ЭтаФорма, Показатели["П00030010" + (2 + НомерКолонки)]);
	...
	Для НомерКолонки = 1 По 3 Цикл
		...
		ТекстФормулы = "если стр. с кодом 80 по гр. " + НомерКолонки + " " + Знак + " 0, то стр. 26 гр. " + НомерКолонки + " " + Знак + " 0";
		Описание     = "если стр. с кодом 80 по гр. " + НомерКолонки + " " + Знак + " 0, то стр. 26 гр. " + НомерКолонки + " " + Знак + " 0";
		
		Сумма = 0;
		Для НомСтр = 1 По П0005001.Количество() Цикл
			Если СокрЛП(Показатели["П000500102_" + НомСтр]) = "80" Тогда
				Сумма = Сумма + РегламентированнаяОтчетностьКлиентСервер.ОкруглитьЧислоПоФормату(ЭтаФорма, Показатели["П00050010" + (5 + НомерКолонки) + "_" + НомСтр]);
			КонецЕсли;
		КонецЦикла;
		...
		Если Сумма > 0 Тогда
			...
			ЛеваяЧасть  = РегламентированнаяОтчетностьКлиентСервер.ОкруглитьЧислоПоФормату(ЭтаФорма, Показатели["П00030050" + (2 + НомерКолонки)]);
	...
	ТекстФормулы = "если стр. с кодом 50 по гр. 4 " + Знак + " 0, то стр. 01 гр. 1 " + Знак + " 0";
	Описание     = "если стр. с кодом 50 по гр. 4 " + Знак + " 0, то стр. 01 гр. 1 " + Знак + " 0";
	
	Сумма = 0;
	Для НомСтр = 1 По П0005001.Количество() Цикл
		Если СокрЛП(Показатели["П000500102_" + НомСтр]) = "50" Тогда
			Сумма = Сумма + РегламентированнаяОтчетностьКлиентСервер.ОкруглитьЧислоПоФормату(ЭтаФорма, Показатели["П000500109_" + НомСтр]);
		КонецЕсли;
	КонецЦикла;
	...
	ТекстФормулы = "если стр. с кодом 50 по гр. 4 " + Знак + " гр. 5";
	Описание     = "если стр. с кодом 50 по гр. 4 " + Знак + " гр. 5";
	
	Для НомСтр = 1 По П0005001.Количество() Цикл
		
		Если СокрЛП(Показатели["П000500102_" + НомСтр]) = "50" Тогда
			
			ЛеваяЧасть  = РегламентированнаяОтчетностьКлиентСервер.ОкруглитьЧислоПоФормату(ЭтаФорма, Показатели["П000500109_" + НомСтр]);
			
			ПраваяЧасть = РегламентированнаяОтчетностьКлиентСервер.ОкруглитьЧислоПоФормату(ЭтаФорма, Показатели["П000500110_" + НомСтр]);
			...
			ДопОписание = Описание + " (в свободной строке " + НомСтр + ")";
			
			РегламентированнаяОтчетность.ДобавитьЭлементВСтруктуруОшибок(СтруктураОшибок, ТекстФормулы, "", ОбщееЧислоОшибок, ОбщееЧислоБезОшибок, Расшифровка, Нарушение, ДопОписание, ЭтоОшибка);
...
// { RGS MYurkevich 9/11/2015 2:40:36 PM - RCA-0000094
&НаКлиенте
...
&НаСервереБезКонтекста
Функция ПолучитьВыручку(Подразделение, НачалоПериода, КонецПериода)
	
	Запрос = Новый Запрос;
	Запрос.Текст = 
		"ВЫБРАТЬ
		|	ВложенныйЗапрос.НалоговаяРегистрация,
		|	СУММА(ВложенныйЗапрос.ОборотЗаМесяц) КАК ОборотЗаМесяц,
		|	СУММА(ВложенныйЗапрос.ОборотЗаПредыдущийМесяц) КАК ОборотЗаПредыдущийМесяц,
		|	СУММА(ВложенныйЗапрос.ОборотВПрошломГоду) КАК ОборотВПрошломГоду
		|ИЗ
		|	(ВЫБРАТЬ
		|		ПродажиВыручка_SBОбороты.ПодразделениеОрганизации КАК НалоговаяРегистрация,
		|		ПродажиВыручка_SBОбороты.СуммаБезНДСRURОборот КАК ОборотЗаМесяц,
		|		0 КАК ОборотЗаПредыдущийМесяц,
		|		0 КАК ОборотВПрошломГоду
		|	ИЗ
		|		РегистрНакопления.ПродажиВыручка_SB.Обороты(
		|				&НачалоПериода,
		|				&КонецПериода,
		|				Период,
		|				ВЫБОР
		|					КОГДА &ОтборПоПодразделению
		|						ТОГДА ПодразделениеОрганизации = &Подразделение
		|					ИНАЧЕ ИСТИНА
		|				КОНЕЦ) КАК ПродажиВыручка_SBОбороты
		|	ГДЕ
		|		ПродажиВыручка_SBОбороты.Тип <> ""DIF""
		|	
		|	ОБЪЕДИНИТЬ ВСЕ
		|	
		|	ВЫБРАТЬ
		|		ПродажиВыручка_SBОбороты.ПодразделениеОрганизации,
		|		0,
		|		ПродажиВыручка_SBОбороты.СуммаБезНДСRURОборот,
		|		0
		|	ИЗ
		|		РегистрНакопления.ПродажиВыручка_SB.Обороты(
		...
		|	ИЗ
		|		РегистрНакопления.ПродажиВыручка_SB.Обороты(
		|				ДОБАВИТЬКДАТЕ(&НачалоПериода, ГОД, -1),
		|				ДОБАВИТЬКДАТЕ(&КонецПериода, ГОД, -1),
		|				Период,
		|				ВЫБОР
		|					КОГДА &ОтборПоПодразделению
		|						ТОГДА ПодразделениеОрганизации = &Подразделение
		...
		|				КОНЕЦ) КАК ПродажиВыручка_SBОбороты
		|	ГДЕ
		|		ПродажиВыручка_SBОбороты.Тип <> ""DIF"") КАК ВложенныйЗапрос
		|
		|СГРУППИРОВАТЬ ПО
		|	ВложенныйЗапрос.НалоговаяРегистрация";
	...
	СтруктураВозврата.Вставить("ОборотЗаМесяц", Выборка.ОборотЗаМесяц);
	СтруктураВозврата.Вставить("ОборотЗаПредыдущийМесяц", Выборка.ОборотЗаПредыдущийМесяц);
	СтруктураВозврата.Вставить("ОборотВПрошломГоду", Выборка.ОборотВПрошломГоду);

	Возврат СтруктураВозврата;
КонецФункции	
// } RGS MYurkevich 9/11/2015 2:43:10 PM - RCA-0000094
