...
Процедура НастроитьВариантыОтчета(Настройки, НастройкиОтчета) Экспорт
	...
	НастройкиВарианта.Описание = 
		ВернутьСтр("ru = 'Позволяет выполнять мониторинг активности пользователей
	...
	НастройкиВарианта.Описание = 
		ВернутьСтр("ru = 'Подробная информация о том,
	...
	НастройкиВарианта.Описание = ВернутьСтр("ru = 'Список критичных записей журнала регистрации.'");
	...
	НастройкиВарианта.Описание = ВернутьСтр("ru = 'Выводит график выполнения регламентных заданий в программе.'");
	...
	// { RGS LGoncharova 02.11.2018 S-I-0006261
	НастройкиВарианта = МодульВариантыОтчетов.ОписаниеВарианта(Настройки, НастройкиОтчета, "МаксКоличествоПользователей");
	НастройкиВарианта.Описание = ВернутьСтр("ru = 'Выводит информацию о максимальном количестве одновременно открытых сеансов'");
	// } RGS LGoncharova 02.11.2018 S-I-0006261
КонецПроцедуры
...
Функция ДанныеИзЖурналаРегистрации(ПараметрыОтчета) Экспорт
	...
	////АдресФайлаЖурнала = ПараметрыОтчета.АдресФайлаЖурнала;
	// } RGS EKoshkina 06.11.2015 10:48:12 - S-I-0001345
	...
	Если ПользовательФильтр.Количество() = 0 И НЕ ВариантОтчета = "МаксКоличествоПользователей" Тогда
	// } RGS LGoncharova 02.11.2018 S-I-0006261		
	...
	//Если АдресФайлаЖурнала = "" Тогда
	//	
		ВыгрузитьЖурналРегистрации(ДанныеЖурналаРегистрации, ОтборЖурналаРегистрации);
	//	
	//Иначе
	//	
	//	ВыгрузитьЖурналРегистрации(ДанныеЖурналаРегистрации, ОтборЖурналаРегистрации, , АдресФайлаЖурнала);
	//	
	//КонецЕсли; 
	// } RGS EKoshkina 06.11.2015 10:48:12 - S-I-0001345
	...
	соотвПользователиИБ = Новый Соответствие;
	// } RGS LGoncharova 07.11.2018 S-I-0006261	
	
	// { RGS LGoncharova 02.11.2018 S-I-0006261	
	//Если ВариантОтчета = "АнализАктивностиПользователей" Тогда
	Если ВариантОтчета = "АнализАктивностиПользователей" Или ВариантОтчета = "МаксКоличествоПользователей" Тогда
	// } RGS LGoncharova 02.11.2018 S-I-0006261	
...
Функция ПользователиДляАнализа(ПользовательФильтр, Элемент)
	...
	ИначеЕсли Элемент = Неопределено Тогда
		...
		Для каждого Пользователь Из МассивПользователей Цикл
			...
			ПользовательФильтр.Добавить(Пользователь.Имя);
...
Функция СтрокаНеделяГода(ДатаГода)
	Возврат СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(ВернутьСтр("ru='Неделя %1'"), НеделяГода(ДатаГода));
КонецФункции
	...
	Для Каждого Элемент Из МаксМассивПользователей Цикл
		...
		Для Каждого ИмяПользователя Из ВременныйМассив Цикл
			Если ИмяПользователя = Элемент Тогда
				...
				ПользовательИНомер = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(ВернутьСтр("ru = '%1 (%2)'"),
					Элемент,
...
Функция СформироватьОтчетПоПродолжительностиРаботыРегламентныхЗаданий(ПараметрыЗаполнения) Экспорт
	...
	Если ВыводитьОтбор.Значение = ТипВыводаТекстаКомпоновкиДанных.Выводить
		...
		Если МинимальнаяПродолжительностьСеансовРегламентныхЗаданий > 0 Тогда
			РежимОтображенияИнтервалов = ВернутьСтр("ru = 'Отключено'");
		Иначе
			РежимОтображенияИнтервалов = ВернутьСтр("ru = 'Включено'");
		КонецЕсли;
		...
		Для Каждого СтрокаОдновременноСессий Из ОдновременноСессий Цикл
			...
			Для Каждого Элемент Из МассивРегламентныхЗаданий Цикл
				Если Не ТипЗнч(Элемент) = Тип("Число")
					...
					Область.Параметры.СписокРегламентныхЗаданий = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(ВернутьСтр("ru = '%1 (сеанс %2)'"),
						Элемент,
		...
		Если ДлительностьИнтервалаРегламентногоЗадания >= МинимальнаяПродолжительностьСеансовРегламентныхЗаданий Тогда
			Если ТекущееСобытие <> СтрокаРегламентныеЗадания.НазваниеСобытия Тогда
				Если ТекущееСобытие <> Неопределено
					...
					ТочкаНазвание = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(ВернутьСтр("ru = '%1 (%2 из %3)'"),
						Точка.Значение,
				...
				ИначеЕсли Не ЗначениеЗаполнено(СтрокаРегламентныеЗадания.СобытиеМетаданные) Тогда
					ТочкаНазвание = ВернутьСтр("ru = 'Фоновые задания'");
					Точка = ДиаграммаГанта.УстановитьТочку(ТочкаНазвание);
					ОбщаяПродолжительностьРЗ = 0;
				КонецЕсли;
			КонецЕсли;
			...
			Интервал.Текст = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(ВернутьСтр("ru = '%1 - %2'"),
	...
	Для Каждого СтрокаДанныхЖурналаРегистрации Из ДанныеЖурналаРегистрации Цикл 
		...
		Если ТекущаяДата <> ДатаОдновременноРегламентныхЗаданий Тогда
			Если СтрокаТаблицы <> Неопределено Тогда
				...
				СтрокаТаблицы.ДатаОдновременноРегламентныхЗаданий = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(ВернутьСтр("ru = '%1 - %2'"),
...
Функция РасшифровкаРегламентногоЗадания(Расшифровка) Экспорт
	...
	Область = Макет.ПолучитьОбласть("Таблица");
	Область.Параметры.ТипЗадания = ВернутьСтр("ru = 'Регламентное'");
...
Функция ПродолжительностьРегламентногоЗадания(ПродолжительностьРЗ)
	...
	ИначеЕсли ПродолжительностьРЗ <= 60 Тогда
		ОбщаяПродолжительностьРЗ = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(ВернутьСтр("ru = '%1 сек'"), ПродолжительностьРЗ);
	ИначеЕсли 60 < ПродолжительностьРЗ <= 3600 Тогда
		...
		ОбщаяПродолжительностьРЗ = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(ВернутьСтр("ru = '%1 мин %2 сек'"), ПродолжительностьМинуты, ПродолжительностьСекунды);
	ИначеЕсли ПродолжительностьРЗ > 3600 Тогда
		...
		ОбщаяПродолжительностьРЗ = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(ВернутьСтр("ru = '%1 ч %2 мин'"), ПродолжительностьЧасы, ПродолжительностьМинуты);
	КонецЕсли;
