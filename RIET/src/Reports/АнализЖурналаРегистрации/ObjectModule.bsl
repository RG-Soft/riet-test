...
Процедура ОбработкаПроверкиЗаполнения(Отказ, ПроверяемыеРеквизиты)
	...
	Если ВариантОтчета = "ДиаграммаГанта" Тогда
		...
		Если Не ЗначениеЗаполнено(ПериодДень.Дата) Тогда
			ОбщегоНазначенияКлиентСервер.СообщитьПользователю(
				ВернутьСтр("ru = 'Не заполнено значение поля День!'"), , );
			Отказ = Истина;
			Возврат;
		КонецЕсли;
		...
		И КонецВыборки.Использование Тогда
			ОбщегоНазначенияКлиентСервер.СообщитьПользователю(
				ВернутьСтр("ru = 'Значение начала периода не может быть больше значения конца!'"), , );
			Отказ = Истина;
			Возврат;
		КонецЕсли;
	...
	ИначеЕсли ВариантОтчета = "АктивностьПользователя" Тогда
		...
		Если Не ЗначениеЗаполнено(Пользователь) Тогда
			ОбщегоНазначенияКлиентСервер.СообщитьПользователю(
				ВернутьСтр("ru = 'Не заполнено значение поля Пользователь!'"), , );
			Отказ = Истина;
			Возврат;
		КонецЕсли;
		...
		Если Отчеты.АнализЖурналаРегистрации.ИмяПользователяИБ(Пользователь) = Неопределено Тогда
			ОбщегоНазначенияКлиентСервер.СообщитьПользователю(
				ВернутьСтр("ru = 'Формирование отчета возможно только для пользователя, которому указано имя для входа в программу.'"), , );
			Отказ = Истина;
			Возврат;
		КонецЕсли;
	...
	ИначеЕсли ВариантОтчета = "АнализАктивностиПользователей" Тогда
		...
		// { RGS EKoshkina 06.11.2015 10:38:12 - S-I-0001345
		//Если Не ЗначениеЗаполнено(ПользователиИГруппы) Тогда
		//	ОбщегоНазначенияКлиентСервер.СообщитьПользователю(
		//		ВернутьСтр("ru = 'Не заполнено значение поля Пользователи!'"), , );
		//	Отказ = Истина;
		//	Возврат;
		//КонецЕсли;
		// } RGS EKoshkina 06.11.2015 10:38:12 - S-I-0001345
...
Функция ПараметрыОтчетаАктивностьПользователя(НастройкиОтчета)
	...
	////АдресФайлаЖурнала = НастройкиОтчета.ПараметрыДанных.Элементы.Найти("АдресФайлаЖурнала");
	////ПараметрыОтчета.Вставить("АдресФайлаЖурнала", АдресФайлаЖурнала.Значение);
	// } RGS EKoshkina 06.11.2015 10:48:12 - S-I-0001345
