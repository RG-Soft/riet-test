...
Процедура СформироватьОтчет(ДокументРезультат, ПоказыватьЗаголовок, ВысотаЗаголовка, ТолькоЗаголовок = Ложь) Экспорт
	...
	Запрос.УстановитьПараметр("ОплатаТрудаРБП",Справочники.РасходыБудущихПериодов.РБПНаОплатуТруда);
		...
		СчетаЗатрат.Добавить(ПланыСчетов.Налоговый.ОбщепроизводственныеРасходы);
		СчетаЗатрат.Добавить(ПланыСчетов.Налоговый.ОбщехозяйственныеРасходы);
		СчетаЗатрат.Добавить(ПланыСчетов.Налоговый.РасходыНаПродажу);
	Запрос.УстановитьПараметр("Счет",    СчетаЗатрат);
	Запрос.УстановитьПараметр("КорСчет",  ПланыСчетов.Налоговый.РасходыБудущихПериодовНаДобровольноеСтрахованиеРаботников);
	
	Запрос.Текст = "ВЫБРАТЬ
	               |	МЕСЯЦ(НалоговыйОбороты.Период) КАК Месяц,
	               |	СУММА(0) КАК ОплатаТруда,
	               |	СУММА(ВЫБОР
	               |			КОГДА НалоговыйОбороты.Субконто1.ВидРасходовНУ = &Страхование1
	               |				ТОГДА НалоговыйОбороты.СуммаОборотДт
	               |			ИНАЧЕ 0
	               |		КОНЕЦ) КАК Страхование1Дт,
	               |	СУММА(0) КАК Страхование1Кт,
	               |	СУММА(ВЫБОР
	               |			КОГДА НалоговыйОбороты.Субконто1.ВидРасходовНУ = &Страхование2
	               |				ТОГДА НалоговыйОбороты.СуммаОборотДт
	               |			ИНАЧЕ 0
	               |		КОНЕЦ) КАК Страхование2Дт,
	               |	СУММА(0) КАК Страхование2Кт,
	               |	СУММА(ВЫБОР
	               |			КОГДА НалоговыйОбороты.Субконто1.ВидРасходовНУ = &Страхование3
	               |				ТОГДА НалоговыйОбороты.СуммаОборотДт
	               |			ИНАЧЕ 0
	...
	Для НМесяца = Месяц(НачалоПериода) по Месяц(КонецПериода) Цикл
		...
		СтрокаМесяца.Норма2     = Окр(СтрокаМесяца.ОплатаТруда*0.03,2,1);
		...
		Если (СтрокаМесяца.Страхование1Дт > 0) или
            ...
            ИтогоПоСтрахованию = ИтогоПоСтрахованию + СтрокаМесяца.Страхование1Кт + СтрокаМесяца.Страхование2Кт + СтрокаМесяца.Страхование3Кт;
