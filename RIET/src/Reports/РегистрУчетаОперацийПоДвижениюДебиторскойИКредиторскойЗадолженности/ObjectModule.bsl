...
Функция НаименованиеОперации(Выборка)
	...
	Если Выборка.СуммаОборотДт <> 0 Тогда
		...
		ИначеЕсли СчетПринадлежит(ПланыСчетов.Хозрасчетный.Продажи, Выборка.КорСчет) Тогда
			Если ОбщегоНазначения.ЗначениеНеЗаполнено(Выборка.КорСубконто1) Тогда
...
Процедура СформироватьОтчет(ДокументРезультат, ПоказыватьЗаголовок, ВысотаЗаголовка, ТолькоЗаголовок = Ложь) Экспорт
	...
	Пока ВыборкаДоговоров.Следующий() Цикл
		...
		", " + УправлениеВзаиморасчетами.ОпределениеВидаРасчетовПоПараметрамДоговора(ВыборкаДоговоров.Субконто2, мВалютаРегламентированногоУчета);
		...
		Пока ВыборкаЗаписей.Следующий() Цикл
			...
			Если Найти(ВыборкаЗаписей.Содержание,"Курсов") > 0 Тогда
			...
			ИначеЕсли Найти(ВыборкаЗаписей.Содержание,"Суммов") > 0  Тогда
