...
//Оповещение ЭтотОбъект - ЭтаФорма 
//<=
...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	//Обновление рег. Отчетности на бух. Корп 3.0.36.18 // добавлено не через макет для простоты обновления
	...
	Иначе
		...
		Если ЗначениеЗаполнено(СтруктураРеквизитовФормы.мСкопированаФорма) Тогда
			...
			Если ЗначениеЗаполнено(ИсходныйПериод) Тогда
				...
				ЛиквидацияОрганизации = (Лев(ИсходныйПериод, 1) = "5");
...
Процедура СформироватьСпискиВыбора()
	...
	// Раздел 2.
	РегламентированнаяОтчетность.ДобавитьСтрокуОписанияВвода(СвойстваПоказателей, "П000020000101", 1, , "Выбор кода вида имущества", КоллекцияСписковВыбора["ВидыИмущества"]);
...
Процедура Расчет(Форма, ИмяСтраницыПанели = "", ОбластьИмя = "")
	...
	Если ИмяСтраницыПанели = "Раздел2" Тогда
		...
		Если Форма.СтруктураРеквизитовФормы.НаимТекущегоРаздела = "Раздел2" Тогда
			...
			Если КодВидаИмущества = "1" ИЛИ КодВидаИмущества = "3"
			 ИЛИ КодВидаИмущества = "5" ИЛИ КодВидаИмущества = "6" Тогда
...
Процедура СформироватьРаздел1()
	...
	Для Каждого Стр из ТаблДанных Цикл
		...
		Если НЕ ПустаяСтрока(ОКТМО) И НЕ ПустаяСтрока(ВидИмущества) И СтрНайти("12356", ВидИмущества) <> 0 Тогда	
...
Процедура ПроверитьЗаполнение_Раздел2(СочетаниеРазделов, СтатусПроверки)
	...
	Для Каждого Лист Из ДанныеРаздела Цикл
		...
		Если НЕ ЗначениеЗаполнено(КодВидаИмущества) ИЛИ СтрНайти("123456", КодВидаИмущества) = 0 Тогда
			РегламентированнаяОтчетность.СообщитьВТаблицуСообщений(ЭтаФорма, "На странице " + НомерЛиста + " раздела 2 код вида имущества должен быть установлен в значение ""1"", ""2"", ""3"", ""4"", ""5"" или ""6""", "Раздел2", "П000020000101", НомерЛиста);
			СтатусПроверки = Ложь;
		КонецЕсли;
...
Процедура ПроверитьЗаполнение_ТитульныйЛист(ПараметрыВыгрузки, СтатусПроверки)
	...
	Если ПустаяСтрока(ПараметрыВыгрузки.Период) Тогда
		РегламентированнаяОтчетность.СообщитьВТаблицуСообщений(ЭтаФорма, "На титульном листе не задан код налогового период", "Титульный", "Период1");
		СтатусПроверки = Ложь;
...
Процедура ПроверитьКС_1_9(СтруктураРезультата)
	...
	ОписаниеКС     = НСтр("ru='Сумма авансового платежа (строка 180 Раздела 2) должна быть равна 1/4 (строка 120 Раздела 2 - строка 140 Раздела 2) * строка 170 Раздела 2 / 100, если код вида имущества (строка 001 Раздела 2) = ""1"" или ""3"" или ""5"" или ""6""'");
	...
	Для Каждого Страница Из Раздел2 Цикл
		...
		Расшифровка = Р2Стр180.ЗначениеСтр + " = 1 / 4 * (" + Р2Стр120.ЗначениеСтр
		            ...
		            + " Итог [" + КонтрольноеЗначение + "]"
		            + " при условии, что """ + Р2Стр001.ЗначениеСтр + """ = [""1"", ""3"", ""5"", ""6""]";
...
Процедура ПроверитьКС_1_17(СтруктураРезультата)
	...
	Для Каждого Страница Из Раздел3 Цикл
		...
		КонтрольноеЗначение = 1 / 4 * (Р3Стр020.Значение - Р3Стр030.Значение) * Р3Стр050ч.Значение / Р3Стр050з.Значение
		                    * Р3Стр080ч.Значение / Р3Стр080з.Значение * Р3Стр070.Значение / 100;
		КонтрольноеЗначение = РегламентированнаяОтчетностьКлиентСервер.ОкруглитьЧислоПоФормату(ЭтаФорма, Окр(КонтрольноеЗначение, 2));
...
Функция ПоказательПоСтроке(УсловныйКодПоказателя)
	...
	ИначеЕсли УсловныйКодПоказателя = "Р3Стр070" Тогда
		...
		Результат.Вставить("Представление", НСтр("ru='Налоговая ставка (процент) (строка 070%1 раздела 3)'"));
		//<=		
