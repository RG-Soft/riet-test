...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если НЕ ЗначениеЗаполнено(Параметры.Пользователь) Тогда
		Если УправлениеДоступомСлужебный.УпрощенныйИнтерфейсНастройкиПравДоступа() Тогда
			ВызватьИсключение
				ВернутьСтр("ru = 'Чтобы открыть отчет откройте карточку пользователя,
				           |перейдите по ссылке ""Права доступа"", нажмите на кнопку ""Отчет по правам доступа"".'");
		Иначе
			ВызватьИсключение
				ВернутьСтр("ru = 'Чтобы открыть отчет откройте карточку пользователя или группы пользователей,
				           |перейдите по ссылке ""Права доступа"", нажмите на кнопку ""Отчет по правам доступа"".'");
		КонецЕсли;
	КонецЕсли;
	...
	Если Параметры.Пользователь <> Пользователи.АвторизованныйПользователь()
	   И НЕ Пользователи.ЭтоПолноправныйПользователь() Тогда
		...
		ВызватьИсключение ВернутьСтр("ru = 'Недостаточно прав для просмотра отчета.'");
	КонецЕсли;
...
Процедура ВывестиОтчет(Ссылка)
	...
	Если ТипЗнч(Ссылка) = Тип("СправочникСсылка.Пользователи") Тогда
		Свойства.Вставить("ЗаголовокОтчета",             ВернутьСтр("ru = 'Отчет по правам пользователя'"));
		Свойства.Вставить("РолиПоПрофилямГруппировка",   ВернутьСтр("ru = 'Роли пользователя по профилям'"));
		Свойства.Вставить("ПредставлениеОбъекта",        ВернутьСтр("ru = 'Пользователь: %1'"));
		
	ИначеЕсли ТипЗнч(Ссылка) = Тип("СправочникСсылка.ВнешниеПользователи") Тогда
		Свойства.Вставить("ЗаголовокОтчета",             ВернутьСтр("ru = 'Отчет по правам внешнего пользователя'"));
		Свойства.Вставить("РолиПоПрофилямГруппировка",   ВернутьСтр("ru = 'Роли внешнего пользователя по профилям'"));
		Свойства.Вставить("ПредставлениеОбъекта",        ВернутьСтр("ru = 'Внешний пользователь: %1'"));
		
	ИначеЕсли ТипЗнч(Ссылка) = Тип("СправочникСсылка.ГруппыПользователей") Тогда
		Свойства.Вставить("ЗаголовокОтчета",             ВернутьСтр("ru = 'Отчет по правам группы пользователей'"));
		Свойства.Вставить("РолиПоПрофилямГруппировка",   ВернутьСтр("ru = 'Роли группы пользователей по профилям'"));
		Свойства.Вставить("ПредставлениеОбъекта",        ВернутьСтр("ru = 'Группа пользователей: %1'"));
	Иначе
		Свойства.Вставить("ЗаголовокОтчета",             ВернутьСтр("ru = 'Отчет по правам группы внешних пользователей'"));
		Свойства.Вставить("РолиПоПрофилямГруппировка",   ВернутьСтр("ru = 'Роли группы внешних пользователей по профилям'"));
		Свойства.Вставить("ПредставлениеОбъекта",        ВернутьСтр("ru = 'Группа внешних пользователей: %1'"));
	КонецЕсли;
	...
	Если НЕ ВыводПравГруппы Тогда
		...
		Если ПользовательИБПрочитан Тогда
			...
			Если НЕ ЗначениеЗаполнено(СвойстваПользователяИБ.ПользовательОС) Тогда
				Область.Параметры.ПользовательОС = ВернутьСтр("ru = 'Не указан'");
			КонецЕсли;
			Документ.Вывести(Область, 2);
		Иначе
	...
	Если ПодробныеСведенияОПравахДоступа Тогда
		...
		Если ОднаПерсональнаяГруппа Тогда
			Если ТипЗнч(Ссылка) = Тип("СправочникСсылка.Пользователи") Тогда
				ПредставлениеДоступа = ВернутьСтр("ru = 'Ограничения доступа пользователя'");
				
			ИначеЕсли ТипЗнч(Ссылка) = Тип("СправочникСсылка.ВнешниеПользователи") Тогда
				ПредставлениеДоступа = ВернутьСтр("ru = 'Ограничения доступа внешнего пользователя'");
				
			ИначеЕсли ТипЗнч(Ссылка) = Тип("СправочникСсылка.ГруппыПользователей") Тогда
				ПредставлениеДоступа = ВернутьСтр("ru = 'Ограничения доступа группы пользователей'");
			Иначе
				ПредставлениеДоступа = ВернутьСтр("ru = 'Ограничения доступа группы внешних пользователей'");
			КонецЕсли;
		Иначе
			Если ТипЗнч(Ссылка) = Тип("СправочникСсылка.Пользователи") Тогда
				ПредставлениеДоступа = ВернутьСтр("ru = 'Группы доступа пользователя'");
				
			ИначеЕсли ТипЗнч(Ссылка) = Тип("СправочникСсылка.ВнешниеПользователи") Тогда
				ПредставлениеДоступа = ВернутьСтр("ru = 'Группы доступа внешнего пользователя'");
				
			ИначеЕсли ТипЗнч(Ссылка) = Тип("СправочникСсылка.ГруппыПользователей") Тогда
				ПредставлениеДоступа = ВернутьСтр("ru = 'Группы доступа группы пользователей'");
			Иначе
				ПредставлениеДоступа = ВернутьСтр("ru = 'Группы доступа группы внешних пользователей'");
			КонецЕсли;
		КонецЕсли;
	...
	Область.Параметры.ПредставлениеГруппировкиПравОбъектов = ВернутьСтр("ru = 'Просмотр объектов'");
	Документ.Вывести(Область, 1);
	...
	Для каждого ОписаниеВидаОбъектов Из ПраваОбъектов.Строки Цикл
		...
		Если УпрощенныйИнтерфейс Тогда
			Область.Параметры.ПредставлениеПрофилиИлиГруппыДоступа = ВернутьСтр("ru = 'Профили'");
		Иначе
			Область.Параметры.ПредставлениеПрофилиИлиГруппыДоступа = ВернутьСтр("ru = 'Группы доступа'");
		КонецЕсли;
		...
		Если ПодробныеСведенияОПравахДоступа Тогда
			Область.Параметры.ПредставлениеПрофилиИлиГруппыДоступа = ВернутьСтр("ru = '(профиль, роли)'");
		Иначе
		...
		Для каждого ОписаниеОбъекта Из ОписаниеВидаОбъектов.Строки Цикл
			...
			Если ОписаниеОбъекта.ЧтениеБезОграничения Тогда
				Если ОписаниеОбъекта.Просмотр Тогда
					УточнениеПредставленияОбъекта = ВернутьСтр("ru = '(просмотр, не ограничен)'");
				Иначе
					УточнениеПредставленияОбъекта = ВернутьСтр("ru = '(просмотр*, не ограничен)'");
				КонецЕсли;
			Иначе
				Если ОписаниеОбъекта.Просмотр Тогда
					УточнениеПредставленияОбъекта = ВернутьСтр("ru = '(просмотр, ограничен)'");
				Иначе
					УточнениеПредставленияОбъекта = ВернутьСтр("ru = '(просмотр*, ограничен)'");
				КонецЕсли;
			КонецЕсли;
								...
								Если ПодробныеСведенияОПравахДоступа Тогда
									...
									Если КоличествоРолей > 1 Тогда
										УточнениеПредставленияГруппыДоступа =
											ВернутьСтр("ru = '(профиль: %1, роли:
											           |%2)'")
									Иначе
										УточнениеПредставленияГруппыДоступа =
											ВернутьСтр("ru = '(профиль: %1, роль:
											           |%2)'")
									КонецЕсли;
	...
	Область = Макет.ПолучитьОбласть("ПраваОбъектовГруппировка");
	Область.Параметры.ПредставлениеГруппировкиПравОбъектов = ВернутьСтр("ru = 'Редактирование объектов'");
	Документ.Вывести(Область, 1);
		...
		Для каждого ОписаниеОбъекта Из ОписаниеВидаОбъектов.Строки Цикл
				...
				Если ОписаниеОбъекта.Добавление И ОписаниеОбъекта.Изменение Тогда
					Если ОписаниеОбъекта.ДобавлениеБезОграничения И ОписаниеОбъекта.ИзменениеБезОграничения Тогда
						Если ОписаниеОбъекта.ИнтерактивноеДобавление И ОписаниеОбъекта.Редактирование Тогда
							УточнениеПредставленияОбъекта = ВернутьСтр("ru = '(добавление, не ограничено
					...
					ИначеЕсли НЕ ОписаниеОбъекта.ДобавлениеБезОграничения И ОписаниеОбъекта.ИзменениеБезОграничения Тогда
						Если ОписаниеОбъекта.ИнтерактивноеДобавление И ОписаниеОбъекта.Редактирование Тогда
							УточнениеПредставленияОбъекта = ВернутьСтр("ru = '(добавление, ограничено
							                                           |изменение, не ограничено)'");
						ИначеЕсли НЕ ОписаниеОбъекта.ИнтерактивноеДобавление И ОписаниеОбъекта.Редактирование Тогда
							УточнениеПредставленияОбъекта = ВернутьСтр("ru = '(добавление*, ограничено
							                                           |изменение, не ограничено)'");
				...
				ИначеЕсли НЕ ОписаниеОбъекта.Добавление И ОписаниеОбъекта.Изменение Тогда
					...
					Если ОписаниеОбъекта.ИзменениеБезОграничения Тогда
						Если ОписаниеОбъекта.Редактирование Тогда
							УточнениеПредставленияОбъекта = ВернутьСтр("ru = '(добавление не доступно
							                                           |изменение, не ограничено)'");
			...
			Иначе
				Если ОписаниеОбъекта.Изменение Тогда
					Если ОписаниеОбъекта.ИзменениеБезОграничения Тогда
						Если ОписаниеОбъекта.Редактирование Тогда
							УточнениеПредставленияОбъекта = ВернутьСтр("ru = '(изменение, не ограничено)'");
						Иначе // НЕ ОписаниеОбъекта.Редактирование
							УточнениеПредставленияОбъекта = ВернутьСтр("ru = '(изменение*, не ограничено)'");
						КонецЕсли;
					Иначе
						Если ОписаниеОбъекта.Редактирование Тогда
							УточнениеПредставленияОбъекта = ВернутьСтр("ru = '(изменение, ограничено)'");
						Иначе // НЕ ОписаниеОбъекта.Редактирование
							УточнениеПредставленияОбъекта = ВернутьСтр("ru = '(изменение*, ограничено)'");
						КонецЕсли;
					КонецЕсли;
				Иначе // НЕ ОписаниеОбъекта.Изменение
					УточнениеПредставленияОбъекта = ВернутьСтр("ru = '(изменение не доступно)'");
				КонецЕсли;
			КонецЕсли;
	...
	Для каждого ОписаниеТипаОбъектов Из НастройкиПрав.Строки Цикл
		...
		ЗаголовокДляПодпапок =
			ВернутьСтр("ru = 'Для
			           |подпапок'");
		ПодсказкаДляПодпапок = ВернутьСтр("ru = 'Права не только для текущей папки, но и для ее нижестоящих папок'");
		...
		ЗаголовокНастройкаПолученаОтГруппы = ВернутьСтр("ru = 'Настройка прав получена от группы'");
		
		Область = Макет.ПолучитьОбласть("НастройкиПравЛегендаСтрокаНаследование");
		Область.Параметры.Подсказка = ВернутьСтр("ru = 'Наследование прав от вышестоящих папок'");
		Документ.Вывести(Область, 2);
		...
		ТекстДа  = ВернутьСтр("ru = 'Да'");
		ТекстНет = ВернутьСтр("ru = 'Нет'");
	...
	Если ОписаниеВидаДоступа.Строки.Количество() = 0 Тогда
		...
		ИначеЕсли ОписаниеВидаДоступа.ВсеРазрешены Тогда
			Если ОписаниеВидаДоступа.ВидДоступа = Справочники.Пользователи.ПустаяСсылка() Тогда
				ШаблонПредставленияВидаДоступа =
					ВернутьСтр("ru = '%1 (без запрещенных, текущий пользователь всегда разрешен)'");
				
			ИначеЕсли ОписаниеВидаДоступа.ВидДоступа = Справочники.ВнешниеПользователи.ПустаяСсылка() Тогда
				ШаблонПредставленияВидаДоступа =
					ВернутьСтр("ru = '%1 (без запрещенных, текущий внешний пользователь всегда разрешен)'");
			Иначе
				ШаблонПредставленияВидаДоступа = ВернутьСтр("ru = '%1 (без запрещенных)'");
			КонецЕсли;
		Иначе
			Если ОписаниеВидаДоступа.ВидДоступа = Справочники.Пользователи.ПустаяСсылка() Тогда
				ШаблонПредставленияВидаДоступа =
					ВернутьСтр("ru = '%1 (без разрешенных, текущий пользователь всегда разрешен)'");
				
			ИначеЕсли ОписаниеВидаДоступа.ВидДоступа = Справочники.ВнешниеПользователи.ПустаяСсылка() Тогда
				ШаблонПредставленияВидаДоступа =
					ВернутьСтр("ru = '%1 (без разрешенных, текущий внешний пользователь всегда разрешен)'");
			Иначе
				ШаблонПредставленияВидаДоступа = ВернутьСтр("ru = '%1 (без разрешенных)'");
			КонецЕсли;
		КонецЕсли;
	Иначе
		Если ОписаниеВидаДоступа.ВсеРазрешены Тогда
			Если ОписаниеВидаДоступа.ВидДоступа = Справочники.Пользователи.ПустаяСсылка() Тогда
				ШаблонПредставленияВидаДоступа =
					ВернутьСтр("ru = '%1 (запрещенные, текущий пользователь всегда разрешен):'");
				
			ИначеЕсли ОписаниеВидаДоступа.ВидДоступа = Справочники.ВнешниеПользователи.ПустаяСсылка() Тогда
				ШаблонПредставленияВидаДоступа =
					ВернутьСтр("ru = '%1 (запрещенные, текущий внешний пользователь всегда разрешен):'");
			Иначе
				ШаблонПредставленияВидаДоступа = ВернутьСтр("ru = '%1 (запрещенные):'");
			КонецЕсли;
		Иначе
			Если ОписаниеВидаДоступа.ВидДоступа = Справочники.Пользователи.ПустаяСсылка() Тогда
				ШаблонПредставленияВидаДоступа =
					ВернутьСтр("ru = '%1 (разрешенные, текущий пользователь всегда разрешен):'");
				
			ИначеЕсли ОписаниеВидаДоступа.ВидДоступа = Справочники.ВнешниеПользователи.ПустаяСсылка() Тогда
				ШаблонПредставленияВидаДоступа =
					ВернутьСтр("ru = '%1 (разрешенные, текущий внешний пользователь всегда разрешен):'");
			Иначе
				ШаблонПредставленияВидаДоступа = ВернутьСтр("ru = '%1 (разрешенные):'");
			КонецЕсли;
		КонецЕсли;
	КонецЕсли;
...
Функция ПредставлениеРежимаЗапуска(РежимЗапуска)
	...
	Если РежимЗапуска = "Авто" Тогда
		ПредставлениеРежимаЗапуска = ВернутьСтр("ru = 'Авто'");
		
	ИначеЕсли РежимЗапуска = "ОбычноеПриложение" Тогда
		ПредставлениеРежимаЗапуска = ВернутьСтр("ru = 'Обычное приложение'");
		
	ИначеЕсли РежимЗапуска = "УправляемоеПриложение" Тогда
		ПредставлениеРежимаЗапуска = ВернутьСтр("ru = 'Управляемое приложение'");
	Иначе
	...
	Пока Индекс >= 0 Цикл
		...
		Если СвойстваВидаДоступа = Неопределено Тогда
			...
			Если МетаданныеВладельцаНастроекПрав = Неопределено Тогда
				Строка.Представление = ВернутьСтр("ru = 'Неизвестный вид доступа'");
			Иначе
				Строка.Представление = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(ВернутьСтр("ru = 'Настройки прав на %1'"),
