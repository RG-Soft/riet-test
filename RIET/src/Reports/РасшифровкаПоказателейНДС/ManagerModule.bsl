...
Процедура ПолучитьНаборДанныхНачислениеРеализация(Запрос, СтруктураВнешнихНаборов) 
	...
	|	ВТСоставКнигиПродаж.ДокументОснование
	...
	|		ПО (ВТПроведеноРанжированиеОшибок.ДокументОснование = СчетФактураВыданныйДокументыОснования.ДокументОснование
	|				ИЛИ ВТПроведеноРанжированиеОшибок.ДокументОснование = СчетФактураВыданныйДокументыОснования.Ссылка
...
Процедура ПолучитьНаборДанныхВычетПоПриобретеннымЦенностям(Запрос, СтруктураВнешнихНаборов) 
	...
	|					ТОГДА ВЫБОР
	|							КОГДА НДСПредъявленныйОстатки.ДатаОплаты = ДАТАВРЕМЯ(1, 1, 1)
	...
	|					ТОГДА ВЫБОР
	|							КОГДА НДСПредъявленныйОстатки.ДатаОплаты = ДАТАВРЕМЯ(1, 1, 1)
	|								ТОГДА ""Не подтверждена оплата в бюджет""
	|							ИНАЧЕ &ТекстВычетНеОтраженВКнигеПокупок
	|						КОНЕЦ
	|				КОГДА ВЫБОР
	|							КОГДА НДСПредъявленныйОстатки.ДокументОснование ССЫЛКА Документ.СчетФактураВыданный
...
Процедура ПолучитьНаборДанныхНачислениеАвансыПолученныеУпрощенныйУчетНДС(Запрос, СтруктураВнешнихНаборов) 
	...
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ
	|	ВТАвансыВыданныеВременнаяТаблица.СчетФактура,
	...
	|	ЕСТЬNULL(СуммыПлатежей.СуммаБезНДС, ВТАвансыВыданныеВременнаяТаблица.СуммаБезНДС) КАК СуммаБезНДС,
	|	ЕСТЬNULL(СуммыПлатежей.СуммаНДС, ВТАвансыВыданныеВременнаяТаблица.СуммаНДС) КАК СуммаНДС
	|ПОМЕСТИТЬ ВТАвансыВыданные
	|ИЗ
	|	ВТХозрасчетныйВсеОстаткиАвансыПолученные КАК ВТАвансыВыданныеВременнаяТаблица
	|		ЛЕВОЕ СОЕДИНЕНИЕ СуммыПлатежей КАК СуммыПлатежей
	|		ПО ВТАвансыВыданныеВременнаяТаблица.СчетФактура = СуммыПлатежей.Ссылка
	...
	|	АвансыВыданные.ДоговорКонтрагента,
	|	АвансыВыданные.СтавкаНДС,
	|	АвансыВыданные.СуммаБезНДС - ЕСТЬNULL(ВТСоставКнигиПродаж.СуммаБезНДСОборот, 0) КАК СуммаБезНДС,
	|	АвансыВыданные.СуммаНДС - ЕСТЬNULL(ВТСоставКнигиПродаж.НДСОборот, 0) КАК СуммаНДС,
	|	ВТСоставКнигиПродаж.ВидЦенности,
	|	ВТСоставКнигиПродаж.Событие
	...
	|		ПО АвансыВыданные.СчетФактура = ВТСоставКнигиПродаж.СчетФактура
	|			И АвансыВыданные.ДоговорКонтрагента = ВТСоставКнигиПродаж.ДоговорКонтрагента
	|			И АвансыВыданные.СтавкаНДС = ВТСоставКнигиПродаж.СтавкаНДС
	|ГДЕ
	|	АвансыВыданные.СуммаНДС - ЕСТЬNULL(ВТСоставКнигиПродаж.НДСОборот, 0) > 0
	|
	|ИНДЕКСИРОВАТЬ ПО
	|	АвансыВыданные.СчетФактура,
	|	АвансыВыданные.ДоговорКонтрагента
