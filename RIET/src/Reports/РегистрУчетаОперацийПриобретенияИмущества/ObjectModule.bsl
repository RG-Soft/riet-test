...
Функция ВидРасходаПоОперации(Счет, Субконто1, Субконто2, Субконто3, Выборка)
	...
	Если Счет.ПринадлежитЭлементу(Счет07) ИЛИ Счет = Счет07 Тогда
		...
		Если  ОбщегоНазначения.ЗначениеНеЗаполнено(ОбъектУчета) Тогда
			 ОбъектУчета = СокрЛП(Выборка.СубконтоДт2);
		ИначеЕсли  ОбщегоНазначения.ЗначениеНеЗаполнено(ОбъектУчета) Тогда
	...
	ИначеЕсли Счет.ПринадлежитЭлементу(Счет08) ИЛИ Счет = Счет08 Тогда
		...
		Если ОбщегоНазначения.ЗначениеНеЗаполнено(Выборка.НоменклатураНаименованиеПолное) Тогда
			ОбъектУчета = Строка(Субконто1);
		Иначе
			ОбъектУчета = СокрЛП(Выборка.НоменклатураНаименованиеПолное);
		КонецЕсли;
	...
	|		    И Активность = Истина
		|			И СчетКт = &Счет
		|			" + ?(?(НЕ ОбщегоНазначения.ЗначениеНеЗаполнено(Субконто1), НЕ (Счет.ВидыСубконто[0].ТолькоОбороты = Истина), Ложь), "И СубконтоКт1 = &Субконто1", "") + "
		|			" + ?(?(НЕ ОбщегоНазначения.ЗначениеНеЗаполнено(Субконто2), НЕ (Счет.ВидыСубконто[1].ТолькоОбороты = Истина), Ложь), "И СубконтоКт2 = &Субконто2", "") + "
		|			" + ?(?(НЕ ОбщегоНазначения.ЗначениеНеЗаполнено(Субконто3), НЕ (Счет.ВидыСубконто[2].ТолькоОбороты = Истина), Ложь), "И СубконтоКт3 = &Субконто3", "") + "
		...
		Пока ВыборкаОбъектовУчета.Следующий() Цикл
			ДокументПринятия = ДокументПринятия + ?(ОбщегоНазначения.ЗначениеНеЗаполнено(ДокументПринятия), "", ", ") + ВыборкаОбъектовУчета.ДокументПринятия;
			ДатаПринятия     = ДатаПринятия + ?(ОбщегоНазначения.ЗначениеНеЗаполнено(ДатаПринятия), "", ", ") + Формат(ВыборкаОбъектовУчета.ДатаПринятия, "ДФ=dd.MM.yyyy");
	...
	ИначеЕсли Счет.ПринадлежитЭлементу(Счет91_02_6)  ИЛИ Счет = Счет91_02_6 Тогда
	...
	ИначеЕсли (Счет.ПринадлежитЭлементу(Счет44) ИЛИ Счет = Счет44 ИЛИ
			   ...
			   Счет.ПринадлежитЭлементу(Счет29) ИЛИ Счет = Счет29 ИЛИ
			   Счет.ПринадлежитЭлементу(Счет91_02) ИЛИ Счет = Счет91_02
			   )
			   И 
			   (ВидЗатратНалоговогоУчета = Перечисления.ВидыРасходовНУ.НеУчитываемыеВЦеляхНалогообложения) Тогда
...
Процедура СформироватьОтчет(ДокументРезультат, ПоказыватьЗаголовок, ВысотаЗаголовка, ТолькоЗаголовок = Ложь) Экспорт
	...
	Пока Выборка.Следующий() Цикл
		...
		Если ВидРасходаСоответствуетКритериюОтбора(ВидРасхода.ВидРасхода) Тогда
			...
			ОбластьСтрока.Параметры.НаименованиеОперации = ?(ОбщегоНазначения.ЗначениеНеЗаполнено(НаименованиеОперации),Выборка.Регистратор.Метаданные().Синоним,НаименованиеОперации);
			ОбластьСтрока.Параметры.УсловиеПоступления = УсловияПолучения.НайтиПоЗначению(Выборка.УсловиеПоступления).Представление;
			ОбластьСтрока.Параметры.Основание          = СокрЛП(Выборка.КонтрагентНаименованиеПолное) + " (" + СокрЛП(Выборка.Договор) + ")";
