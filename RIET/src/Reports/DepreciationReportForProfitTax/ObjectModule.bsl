...
Функция ОпределитьКопиюТаблицыЗапросаДляВывода(тзРезультат)	
	...
	Если тзРезультат.Количество() Тогда
		...
		СтрокаТЗ.АмортизацияНаНачалоПериодаПоОСВЦелом 	= ИтоговаяСтрока.АмортизацияНаНачалоПериодаПоОСВЦелом;
		...
		СтрокаТЗ.СуммаКВ								= ИтоговаяСтрока.СуммаКВ;
		...
		СтрокаТЗ.ОстаточнаяСтоимостьПоОСВЦелом  		= ИтоговаяСтрока.ОстаточнаяСтоимостьПоОСВЦелом;
...
Процедура ПолучитьНачальныеНастройкиЗапроса(сукНачальныеНастройкиЗапроса)
	...
	Если ВидСравненияПодразделения = ВидСравненияКомпоновкиДанных.Равно Тогда
		 сукНачальныеНастройкиЗапроса.УсловиеПодразделение = "Подразделение = &Подразделение И";
	ИначеЕсли ВидСравненияПодразделения = ВидСравненияКомпоновкиДанных.НеРавно Тогда
		 сукНачальныеНастройкиЗапроса.УсловиеПодразделение = "НЕ Подразделение = &Подразделение И";
	ИначеЕсли ВидСравненияПодразделения = ВидСравненияКомпоновкиДанных.ВСписке Тогда
		 сукНачальныеНастройкиЗапроса.УсловиеПодразделение = "Подразделение В (&Подразделение) И";
	ИначеЕсли ВидСравненияПодразделения = ВидСравненияКомпоновкиДанных.НеВСписке Тогда
		 сукНачальныеНастройкиЗапроса.УсловиеПодразделение = "НЕ Подразделение В (&Подразделение) И";
	ИначеЕсли ВидСравненияПодразделения = ВидСравненияКомпоновкиДанных.ВИерархии ИЛИ ВидСравненияПодразделения = ВидСравненияКомпоновкиДанных.ВСпискеПоИерархии Тогда
		 сукНачальныеНастройкиЗапроса.УсловиеПодразделение = "Подразделение В Иерархии (&Подразделение) И";
	ИначеЕсли ВидСравненияПодразделения = ВидСравненияКомпоновкиДанных.НеВИерархии ИЛИ ВидСравненияПодразделения = ВидСравненияКомпоновкиДанных.НеВСпискеПоИерархии Тогда
		 сукНачальныеНастройкиЗапроса.УсловиеПодразделение = "НЕ Подразделение В Иерархии (&Подразделение) И";		 
	 КонецЕсли;
...
Функция СформироватьОтчет(Результат = Неопределено, ДанныеРасшифровки = Неопределено, ВыводВФормуОтчета = Истина) Экспорт
	...
	|	СписокОС.ОС КАК ОсновноеСредство,
	...
	|	МестонахождениеОСБухгалтерскийУчетСрезПоследних1.Местонахождение КАК МестонахождениеКон
	|ПОМЕСТИТЬ ОС
	|ИЗ
	|	(ВЫБРАТЬ РАЗЛИЧНЫЕ
	|		СтоимостьАмортизацияОСОбороты.ОсновноеСредство КАК ОС
	|	ИЗ
	...
	|					И ОсновноеСредство.ПометкаУдаления = ЛОЖЬ) КАК СтоимостьАмортизацияОСОбороты
	|	ГДЕ
	|		(СтоимостьАмортизацияОСОбороты.АмортизацияНУПриход <> 0
	|				ИЛИ СтоимостьАмортизацияОСОбороты.АмортизацияНУРасход <> 0
	|				ИЛИ СтоимостьАмортизацияОСОбороты.СтоимостьНУПриход <> 0
	|				ИЛИ СтоимостьАмортизацияОСОбороты.СтоимостьНУРасход <> 0)) КАК СписокОС
	|		ЛЕВОЕ СОЕДИНЕНИЕ РегистрСведений.МестонахождениеОСБухгалтерскийУчет.СрезПоследних(&Кон, Местонахождение В ИЕРАРХИИ (&Подразделение)) КАК МестонахождениеОСБухгалтерскийУчетСрезПоследних
	|		ПО СписокОС.ОС = МестонахождениеОСБухгалтерскийУчетСрезПоследних.ОсновноеСредство
	|		ЛЕВОЕ СОЕДИНЕНИЕ РегистрСведений.МестонахождениеОСБухгалтерскийУчет.СрезПоследних(&Кон, ) КАК МестонахождениеОСБухгалтерскийУчетСрезПоследних1
	|		ПО СписокОС.ОС = МестонахождениеОСБухгалтерскийУчетСрезПоследних1.ОсновноеСредство
	|
	|ИНДЕКСИРОВАТЬ ПО
	|	ОсновноеСредство
	|;
	...
	|	ОС.ОсновноеСредство.АмортизационнаяГруппа КАК АмортизационнаяГруппа,
	...
	|	НачислениеАмортизации.СпециальныйКоэффициент КАК СпецКоэффициент,
	|	ПринятоКУчету.ДатаСостояния КАК ДатаПринятияКУчету,
	|	СтоимостьАмортизацияОС.СтоимостьНУНачальныйОстаток КАК СтоимостьНаНачалоПериода,
	|	СтоимостьАмортизацияОС.АмортизацияНУНачальныйОстаток КАК АмортизацияНаНачалоПериода,
	|	СтоимостьАмортизацияОСБезПодразделений.АмортизацияНУНачальныйОстаток КАК АмортизацияНаНачалоПериодаПоОСВЦелом,
	|	СтоимостьАмортизацияОС.СтоимостьНУОборот КАК ИзменениеСтоимостиЗаПериод,
	|	СтоимостьАмортизацияОС_КВ_АмНУОборот.АмортизацияНУОборот КАК АмортизацияЗаПериод,
	|	СтоимостьАмортизацияОС_КВ_АмНУОборот.СуммаКВОборот КАК СуммаКВ,
	|	СтоимостьАмортизацияОС.СтоимостьНУКонечныйОстаток КАК СтоимостьНаКонецПериода,
	|	СтоимостьАмортизацияОСБезПодразделений.АмортизацияНУКонечныйОстаток КАК АмортизацияНаКонецПериода,
	|	ВЫБОР
	|		КОГДА СтоимостьАмортизацияОСБезПодразделений.СтоимостьНУКонечныйОстаток = 0
	|			ТОГДА 0
	|		ИНАЧЕ СтоимостьАмортизацияОСБезПодразделений.СтоимостьНУКонечныйОстаток - ЕСТЬNULL(СтоимостьАмортизацияОСБезПодразделений.АмортизацияНУКонечныйОстаток, 0)
	|	КОНЕЦ КАК ОстаточнаяСтоимостьПоОСВЦелом,
	...
	|	ОС.Местонахождение КАК НалоговаяРегистрацияКон,
	|	ОС.МестонахождениеКон
	...
	|			СУММА(СтоимостьАмортизацияОСОстаткиИОбороты.СтоимостьНУНачальныйОстаток) КАК СтоимостьНУНачальныйОстаток,
	|			СУММА(СтоимостьАмортизацияОСОстаткиИОбороты.СтоимостьНУОборот) КАК СтоимостьНУОборот,
	|			СУММА(СтоимостьАмортизацияОСОстаткиИОбороты.СтоимостьНУКонечныйОстаток) КАК СтоимостьНУКонечныйОстаток,
	|			СУММА(СтоимостьАмортизацияОСОстаткиИОбороты.АмортизацияНУКонечныйОстаток) КАК АмортизацияНУКонечныйОстаток,
	|			СУММА(СтоимостьАмортизацияОСОстаткиИОбороты.АмортизацияНУНачальныйОстаток) КАК АмортизацияНУНачальныйОстаток,
	|			СУММА(СтоимостьАмортизацияОСОстаткиИОбороты.АмортизацияНУПриход) КАК АмортизацияНУОборот,
	|			СУММА(ВЫБОР
	|					КОГДА СтоимостьАмортизацияОСОстаткиИОбороты.СтоимостьНУОборот > 0
	|						ТОГДА 0
	|					ИНАЧЕ СтоимостьАмортизацияОСОстаткиИОбороты.СуммаКВРасход
	|				КОНЕЦ) КАК СуммаКВОборот
	|		ИЗ
	...
	|			СУММА(СтоимостьАмортизацияОСОстаткиИОбороты.АмортизацияНУНачальныйОстаток) КАК АмортизацияНУНачальныйОстаток,
	|			СУММА(СтоимостьАмортизацияОСОстаткиИОбороты.АмортизацияНУПриход) КАК АмортизацияНУОборот,
	|			СУММА(СтоимостьАмортизацияОСОстаткиИОбороты.СуммаКВРасход) КАК СуммаКВОборот
	|		ИЗ
	|			РегистрНакопления.СтоимостьАмортизацияОС.ОстаткиИОбороты(
	|					&Нач,
	|					&Кон,
	...
	|		
	|		СГРУППИРОВАТЬ ПО
	|			СтоимостьАмортизацияОСОстаткиИОбороты.ОсновноеСредство) КАК СтоимостьАмортизацияОСБезПодразделений
	|		ПО ОС.ОсновноеСредство = СтоимостьАмортизацияОСБезПодразделений.ОсновноеСредство
	|		ЛЕВОЕ СОЕДИНЕНИЕ (ВЫБРАТЬ
	|			СтоимостьАмортизацияОСОстаткиИОбороты.ОсновноеСредство КАК ОсновноеСредство,
	|			СУММА(ВЫБОР
	|					КОГДА СтоимостьАмортизацияОСОстаткиИОбороты.Регистратор ССЫЛКА Документ.НачислениеАмортизации
	|						ТОГДА СтоимостьАмортизацияОСОстаткиИОбороты.СуммаКВРасход
	|					ИНАЧЕ 0
	|				КОНЕЦ) КАК СуммаКВОборот,
	|			СУММА(ВЫБОР
	|					КОГДА СтоимостьАмортизацияОСОстаткиИОбороты.Регистратор ССЫЛКА Документ.НачислениеАмортизации
	|						ТОГДА СтоимостьАмортизацияОСОстаткиИОбороты.АмортизацияНУОборот
	|					ИНАЧЕ 0
	...
	|								ОС.ОсновноеСредство
	|							ИЗ
	|								ОС)) КАК СтоимостьАмортизацияОСОстаткиИОбороты
	|		
	|		СГРУППИРОВАТЬ ПО
	|			СтоимостьАмортизацияОСОстаткиИОбороты.ОсновноеСредство) КАК СтоимостьАмортизацияОС_КВ_АмНУОборот
	|		ПО ОС.ОсновноеСредство = СтоимостьАмортизацияОС_КВ_АмНУОборот.ОсновноеСредство
	|;
	|
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ
	...
	|	ИтоговаяТаблица.АмортизационнаяГруппа,
	|	ИтоговаяТаблица.КодОКОФ,
	|	ИтоговаяТаблица.ГруппаОС,
	|	ИтоговаяТаблица.НомерЗаказа,
	|	ИтоговаяТаблица.НомерИнвойса,
	|	ИтоговаяТаблица.СрокПолезногоИспользования КАК СрокПолезногоИспользования,
	|	ИтоговаяТаблица.СпецКоэффициент КАК СпецКоэффициент,
	|	ИтоговаяТаблица.ДатаПринятияКУчету,
	|	ИтоговаяТаблица.СтоимостьНаНачалоПериода КАК СтоимостьНаНачалоПериода,
	|	ИтоговаяТаблица.АмортизацияНаНачалоПериода КАК АмортизацияНаНачалоПериода,
	|	ИтоговаяТаблица.АмортизацияНаНачалоПериодаПоОСВЦелом КАК АмортизацияНаНачалоПериодаПоОСВЦелом,
	|	ИтоговаяТаблица.ИзменениеСтоимостиЗаПериод КАК ИзменениеСтоимостиЗаПериод,
	|	ИтоговаяТаблица.АмортизацияЗаПериод КАК АмортизацияЗаПериод,
	|	ИтоговаяТаблица.СуммаКВ КАК СуммаКВ,
	|	ИтоговаяТаблица.СтоимостьНаКонецПериода КАК СтоимостьНаКонецПериода,
	|	ИтоговаяТаблица.АмортизацияНаКонецПериода КАК АмортизацияНаКонецПериода,
	|	ВЫБОР
	|		КОГДА НЕ ИтоговаяТаблица.МестонахождениеКон В (&Подразделение)
	|				ИЛИ ИтоговаяТаблица.ОстаточнаяСтоимостьПоОСВЦелом < 0
	|			ТОГДА 0
	|		ИНАЧЕ ИтоговаяТаблица.ОстаточнаяСтоимостьПоОСВЦелом
	|	КОНЕЦ КАК ОстаточнаяСтоимостьПоОСВЦелом,
