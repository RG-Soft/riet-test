...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если Параметры.Пользователь <> Неопределено Тогда
		...
		Заголовок = ВернутьСтр("ru = 'Настройки пользователя'");
...
Процедура ПользовательСсылкаПриИзменении(Элемент)
	...
	Если РезультатПолученияНастроек() = "НетПользователяИБ" Тогда
		ТекстСообщения = ВернутьСтр("ru = 'У выбранного пользователя ""%1"" не указан
...
Процедура ПользовательСсылкаНачалоВыбора(Элемент, ДанныеВыбора, СтандартнаяОбработка)
	...
	Если ИспользоватьВнешнихПользователей Тогда
		...
		ВыборТипаПользователей.Добавить("ВнешниеПользователи", ВернутьСтр("ru = 'Внешние пользователи'"));
		ВыборТипаПользователей.Добавить("Пользователи", ВернутьСтр("ru = 'Пользователи'"));
...
Процедура НастройкиПередУдалением(Элемент, Отказ)
	...
	Отказ = Истина;
	ТекстВопроса = ВернутьСтр("ru = 'Очистить выбранные настройки?'");
...
Процедура СкопироватьВсеНастройки(Команда)
	...
	КопируемыеНастройки.Добавить("НастройкиОтчетов", ВернутьСтр("ru='Настройки отчетов'"));
	КопируемыеНастройки.Добавить("НастройкиВнешнегоВида", ВернутьСтр("ru='Настройки внешнего вида'"));
	КопируемыеНастройки.Добавить("ДанныеФорм", ВернутьСтр("ru='Данные форм'"));
	КопируемыеНастройки.Добавить("ПерсональныеНастройки", ВернутьСтр("ru='Персональные настройки'"));
	КопируемыеНастройки.Добавить("Избранное", ВернутьСтр("ru='Избранное'"));
	КопируемыеНастройки.Добавить("НастройкиПечати", ВернутьСтр("ru='Настройки печати'"));
	КопируемыеНастройки.Добавить(
		"ПрочиеПользовательскиеНастройки", ВернутьСтр("ru='Настройки дополнительных отчетов и обработок'"));
...
Процедура Очистить(Команда)
	...
	Если ВыделенныеСтроки.Количество() = 0 Тогда
		...
		ПоказатьПредупреждение(,ВернутьСтр("ru = 'Необходимо выбрать настройки, которые требуется удалить.'"));
		Возврат;
		
	КонецЕсли;
	
	Оповещение = Новый ОписаниеОповещения("ОчиститьЗавершение", ЭтотОбъект, ДеревоНастроек);
	ТекстВопроса = ВернутьСтр("ru = 'Очистить выделенные настройки?'");
	ПоказатьВопрос(Оповещение, ТекстВопроса, РежимДиалогаВопрос.ДаНет,, КодВозвратаДиалога.Да);
...
Процедура ОчиститьВсеНастройки(Команда)
	...
	ТекстВопроса = ВернутьСтр("ru = 'Очистить все настройки у пользователя ""%1""?'");
	ТекстВопроса = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(ТекстВопроса, Строка(ПользовательСсылка));
	КнопкиВопроса = Новый СписокЗначений;
	КнопкиВопроса.Добавить("Очистить", ВернутьСтр("ru='Очистить'"));
	КнопкиВопроса.Добавить("Отмена", ВернутьСтр("ru='Отмена'"));
...
Процедура ОчиститьНастройкиОтчетовИВнешнегоВида(Команда)
	...
	ТекстВопроса = ВернутьСтр("ru = 'Очистить все настройки отчетов и внешнего вида у пользователя ""%1""?'");
	ТекстВопроса = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(ТекстВопроса, Строка(ПользовательСсылка));
	КнопкиВопроса = Новый СписокЗначений;
	КнопкиВопроса.Добавить("Очистить", ВернутьСтр("ru='Очистить'"));
	КнопкиВопроса.Добавить("Отмена", ВернутьСтр("ru='Отмена'"));
...
Процедура ОчиститьНастройкиВсехПользователей(Команда)
	...
	ТекстВопроса = ВернутьСтр("ru = 'Сейчас будут очищены все настройки всех пользователей.
		                      |Продолжить?'");
	КнопкиВопроса = Новый СписокЗначений;
	КнопкиВопроса.Добавить("ОчиститьВсе", ВернутьСтр("ru='Очистить все'"));
	КнопкиВопроса.Добавить("Отмена", ВернутьСтр("ru='Отмена'"));
...
Процедура РассчитатьКоличествоНастроек()
	...
	Если КоличествоНастроек <> 0 Тогда
		Элементы.НастройкиОтчетовСтраница.Заголовок = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(ВернутьСтр("ru='Настройки отчетов (%1)'"), КоличествоНастроек);
	Иначе
		Элементы.НастройкиОтчетовСтраница.Заголовок = ВернутьСтр("ru='Настройки отчетов'");
	КонецЕсли;
	...
	Если КоличествоНастроек <> 0 Тогда
		Элементы.ВнешнийВидСтраница.Заголовок = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(ВернутьСтр("ru='Внешний вид (%1)'"), КоличествоНастроек);
	Иначе
		Элементы.ВнешнийВидСтраница.Заголовок = ВернутьСтр("ru='Внешний вид'");
	КонецЕсли;
	...
	Если КоличествоНастроек <> 0 Тогда
		Элементы.ПрочиеНастройкиСтраница.Заголовок = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(ВернутьСтр("ru='Прочие настройки (%1)'"), КоличествоНастроек);
	Иначе
		Элементы.ПрочиеНастройкиСтраница.Заголовок = ВернутьСтр("ru='Прочие настройки'");
	КонецЕсли;
	...
	Если ВРег(ИмяСобытия) = ВРег("ВыборПользователя") Тогда
		...
		Если Параметр.КопироватьВсе Тогда
			...
			Если МассивНастроек.Количество() = 7 Тогда
				ТекстПояснения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(ВернутьСтр("ru = 'Скопированы все настройки %1'"), ПояснениеКомуСкопированыНастройки);
			Иначе
				ТекстПояснения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(ВернутьСтр("ru = '%1 скопированы %2'"), ИменаНастроек, ПояснениеКомуСкопированыНастройки);
			КонецЕсли;
				
			Состояние(ВернутьСтр("ru = 'Выполняется копирование настроек'"));
			
			Отчет = Неопределено;
			СкопироватьВсеНастройкиНаСервере(ПользовательИнформационнойБазы, ПользователиПриемник, МассивНастроек, Отчет);
			
			Если Отчет <> Неопределено Тогда
				ТекстВопроса = ВернутьСтр("ru = 'Не все варианты отчетов и настройки были скопированы.'");
				КнопкиВопроса = Новый СписокЗначений;
				КнопкиВопроса.Добавить("Ок", ВернутьСтр("ru='ОК'"));
				КнопкиВопроса.Добавить("ПоказатьОтчет", ВернутьСтр("ru='Показать отчет'"));
			...
			ПоказатьОповещениеПользователя(
				ВернутьСтр("ru = 'Копирование настроек'"), , ТекстПояснения, БиблиотекаКартинок.Информация32);
...
Процедура ОчиститьНастройкиВсехПользователейЗавершение(Ответ, ДополнительныеПараметры) Экспорт
	...
	ПоказатьОповещениеПользователя(ВернутьСтр("ru = 'Очистка настроек'"), ,
		ВернутьСтр("ru='Очищены все настройки всех пользователя'"), БиблиотекаКартинок.Информация32);
...
Процедура ОчиститьВсеНастройкиЗавершение(Ответ, ДополнительныеПараметры) Экспорт
	...
	ТекстПояснения = ВернутьСтр("ru = 'Очищены все настройки пользователя ""%1""'");
	ТекстПояснения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(ТекстПояснения, ПользовательСсылка);
	ПоказатьОповещениеПользователя(ВернутьСтр("ru = 'Очистка настроек'"), , ТекстПояснения, БиблиотекаКартинок.Информация32);
...
Процедура ОчиститьНастройкиОтчетовИВнешнегоВидаЗавершение(Ответ, ДополнительныеПараметры) Экспорт
	...
	ТекстПояснения = ВернутьСтр("ru = 'Очищены все настройки отчетов и внешнего вида у пользователя ""%1""'");
...
Процедура ОповеститьОбУдалении(КоличествоНастроек, НазваниеНастройки = Неопределено, КоличествоПользователей = Неопределено)
	...
	ПрописьПредмета = Формат(КоличествоНастроек, "ЧДЦ=0") + " "
		+ ПользователиСлужебныйКлиентСервер.ПредметЦелогоЧисла(КоличествоНастроек,
			"Л = ru_RU", ВернутьСтр("ru = 'настройка,настройки,настроек,,,,,,0'"));
	
	Если КоличествоНастроек = 1
		И КоличествоПользователей = Неопределено Тогда
		ТекстПояснения = ВернутьСтр("ru = '""%1"" очищена пользователю ""%2""'");
	...
	ИначеЕсли КоличествоПользователей = Неопределено Тогда
		ТекстПояснения = ВернутьСтр("ru = 'Очищено %1 пользователю ""%2""'");
	...
	Если КоличествоПользователей <> Неопределено Тогда
		...
		Если КоличествоНастроек = 1 Тогда
			ТекстПояснения = ВернутьСтр("ru = '""%1"" очищена %2'");
		...
		Иначе
			ТекстПояснения = ВернутьСтр("ru = 'Очищено %1 %2'");
	...
	ПоказатьОповещениеПользователя(
		ВернутьСтр("ru = 'Очистка настроек'"), , ТекстПояснения, БиблиотекаКартинок.Информация32);
...
Процедура СкопироватьНастройки()
	...
	Если ДеревоНастроек.ВыделенныеСтроки.Количество() = 0 Тогда
		ПоказатьПредупреждение(,ВернутьСтр("ru = 'Необходимо выбрать настройки, которые требуется скопировать.'"));
		Возврат;
	ИначеЕсли ДеревоНастроек.ВыделенныеСтроки.Количество() = 1 Тогда
		
		Если ДеревоНастроек.ТекущиеДанные.Тип = "ВариантЛичный" Тогда
			ПоказатьПредупреждение(,ВернутьСтр("ru = 'Невозможно скопировать личный вариант отчета.
		...
		ИначеЕсли ДеревоНастроек.ТекущиеДанные.Тип = "НастройкаЛичная" Тогда
			ПоказатьПредупреждение(,ВернутьСтр("ru = 'Невозможно скопировать настройку личного варианта отчета.
