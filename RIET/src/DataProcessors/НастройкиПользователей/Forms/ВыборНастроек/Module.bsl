...
Процедура ОбновитьЗаголовокСтраницы(КоличествоНастроек)
	...
	Если ВыбраннаяСтраницаНастроек = "НастройкиОтчетовСтраница" Тогда
		...
		Если КоличествоНастроекОтчетов = 0 Тогда
			ТекстЗаголовка = ВернутьСтр("ru='Настройки отчетов'");
		Иначе
			ТекстЗаголовка = ВернутьСтр("ru='Настройки отчетов (%1)'");
	...
	ИначеЕсли ВыбраннаяСтраницаНастроек = "ВнешнийВидСтраница" Тогда
		...
		Если КоличествоНастроекВнешнегоВида = 0 Тогда
			ТекстЗаголовка = ВернутьСтр("ru='Внешний вид'");
		Иначе
			ТекстЗаголовка = ВернутьСтр("ru='Внешний вид (%1)'");
	...
	ИначеЕсли ВыбраннаяСтраницаНастроек = "ПрочиеНастройкиСтраница" Тогда
		...
		Если КоличествоПрочихНастроек = 0 Тогда
			ТекстЗаголовка = ВернутьСтр("ru='Прочие настройки'");
		Иначе
			ТекстЗаголовка = ВернутьСтр("ru='Прочие настройки (%1)'");
...
Процедура ЗагрузитьЗначенияПометок(ДеревоЗначений, ПомеченныеНастройки, ВидНастроек)
	...
	Если КоличествоПомеченных > 0 Тогда
		...
		Если ВидНастроек = "НастройкиОтчетов" Тогда
			...
			Элементы.НастройкиОтчетовСтраница.Заголовок = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(ВернутьСтр("ru='Настройки отчетов (%1)'"), КоличествоПомеченных);
		ИначеЕсли ВидНастроек = "ВнешнийВид" Тогда
			КоличествоНастроекВнешнегоВида = КоличествоПомеченных;
			Элементы.ВнешнийВидСтраница.Заголовок = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(ВернутьСтр("ru='Внешний вид (%1)'"), КоличествоПомеченных);
		ИначеЕсли ВидНастроек = "ПрочиеНастройки" Тогда
			КоличествоПрочихНастроек = КоличествоПомеченных;
			Элементы.ПрочиеНастройкиСтраница.Заголовок = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(ВернутьСтр("ru='Прочие настройки (%1)'"), КоличествоПомеченных);
		КонецЕсли;
