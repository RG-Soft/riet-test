...
Процедура Печать(ПолеОтчета, Источник, ИмяКолонкиПометки) Экспорт
	...
	Для Каждого СтрокаИсточника Из Источник Цикл
		Если СтрокаИсточника[ИмяКолонкиПометки] Тогда
			...
			Если ОбщегоНазначения.ЗначениеНеЗаполнено(Док) Тогда
				Продолжить;
			КонецЕсли;
...
Функция СоздатьНужныйДоговор(Владелец, ТекОрганизация,Плательщик) Экспорт
	...
	Иначе
		...
		Если ОбщегоНазначения.ЗначениеНеЗаполнено(НайденныйДоговорОбъект.ВалютаВзаиморасчетов) Тогда
		...
		Если ОбщегоНазначения.ЗначениеНеЗаполнено(НайденныйДоговорОбъект.Организация) Тогда
...
Процедура РассчитатьСуммуВзаиморасчетов(СтрокаПлатеж, ОбъектДокумента)
	...
	СтрокаПлатеж.СуммаВзаиморасчетов = ОбщегоНазначения.ПересчитатьИзВалютыВВалюту(СтрокаПлатеж.СуммаПлатежа, ОбъектДокумента.ВалютаДокумента,
...
Процедура РассчитатьКурсИСуммуВзаиморасчетов(СтрокаПлатеж, мТекущаяВалютаВзаиморасчетов, ДатаКурса, ОбъектДокумента)
	...
	СтруктураКурсаВзаиморасчетов = ОбщегоНазначения.ПолучитьКурсВалюты(СтрокаПлатеж.ДоговорКонтрагента.ВалютаВзаиморасчетов, ДатаКурса);
...
Функция ПолучитьНомерВФорматеОбмена(НомерДокумента, Знач ПрефиксОрганизации)
	...
	Если СтрНайти(Номер, ПрефиксОрганизации)=1 Тогда 
...
Процедура СписокНеНайденных(Реквизиты = Неопределено, Документ = Неопределено, НовыйКонтрагент=Истина) Экспорт
	...
	Для Каждого Список Из КопияСписокКонтрагентов Цикл
		...
		Если НЕ Список.Значение.Свойство("ТИПКОНТРАГЕНТА", ТипКонтрагента) Тогда
			...
			Если Список.Значение.Свойство("ДАТАПОСТУПИЛО") И ( НЕ ОбщегоНазначения.ЗначениеНеЗаполнено(Список.Значение.ДАТАПОСТУПИЛО)) Тогда
...
Функция ОрганизацияПлательщик(ВидДокумента)
	...
	Если (ВидДокумента = "ПлатежноеПоручениеВходящее")
		...
		ИЛИ (ВидДокумента = "ИнкассовоеПоручениеПереданное") Тогда
		Возврат Ложь;
...
Процедура ЗаполнитьЗначениеЭкспорта(СтрокаЭкспорта, ВыборкаДляЭкспорта, ИмяСвойства, Плательщик, Получатель)
	...
	ИначеЕсли ИмяСвойства = "Плательщик" Тогда // Плательщик1 Плательщик2 Плательщик3 Плательщик4
		...
		Значение = ВыборкаДляЭкспорта[Плательщик];
			...
			Если ВыборкаДляЭкспорта.ВидОперации = Перечисления.ВидыОперацийППИсходящее.ПеречислениеНалога Тогда
				Если Плательщик = "Организация" Тогда
					ТекстПлательщика = ВыборкаДляЭкспорта.ОрганизацияПеречислениеНалога;
			...
			Если ПустаяСтрока(ТекстПлательщика) Тогда
				ТекстПлательщика = ВыборкаДляЭкспорта[Плательщик];
			КонецЕсли;
	...
	ИначеЕсли ИмяСвойства = "ПлательщикРасчСчет" Тогда // ПлательщикБанк1 ПлательщикБанк2 ПлательщикБИК ПлательщикКорсчет
		...
		НепрямыеРасчетыУПлательщика = НЕ ОбщегоНазначения.ЗначениеНеЗаполнено(ВыборкаДляЭкспорта[Плательщик + "БанкДляРасчетов"]);
	...
	ИначеЕсли ИмяСвойства = "ПолучательРасчСчет" Тогда //
		...
		НепрямыеРасчетыУПолучателя = НЕ ОбщегоНазначения.ЗначениеНеЗаполнено(ВыборкаДляЭкспорта[Получатель + "БанкДляРасчетов"]);
	...
	ИначеЕсли ИмяСвойства = "ПлательщикКПП" Тогда
		Если ОбщегоНазначения.ЗначениеНеЗаполнено(СтрокаЭкспорта.ПлательщикКПП) Тогда
			СтрокаЭкспорта.ПлательщикКПП = ВыборкаДляЭкспорта.КПППлательщика;
		КонецЕсли;
		
	ИначеЕсли ИмяСвойства = "ПолучательКПП" Тогда
		Если ОбщегоНазначения.ЗначениеНеЗаполнено(СтрокаЭкспорта.ПолучательКПП) Тогда
	...
	ИначеЕсли ИмяСвойства = "СтатусСоставителя" Тогда // ПлательщикКПП ПолучательКПП ПоказательКБК ОКАТО ПоказательОснования ПоказательПериода ПоказательНомера ПоказательДаты ПоказательТипа
		...
		Если ВыборкаДляЭкспорта.ВидОперации = Перечисления.ВидыОперацийППИсходящее.ПеречислениеНалога Тогда
			...
			Если ОбщегоНазначения.ЗначениеНеЗаполнено(ВыборкаДляЭкспорта.ПоказательДаты) Тогда
...
Процедура ПроверитьНаПустоеЗначениеЭкспорта(СтрокаЭкспорта, ИмяСвойства, ПредставлениеСвойства)
	...
	Если Экспорт_НеПустые[СтрокаЭкспорта.ТипДокумента][ИмяСвойства] = Истина Тогда
		Если ОбщегоНазначения.ЗначениеНеЗаполнено(СтрокаЭкспорта[ИмяСвойства]) Тогда
...
Процедура ПроверитьНаКорректностьЗначениеЭкспорта(СтрокаЭкспорта, ИмяСвойства)
	...
	ИначеЕсли ИмяСвойства = "СтатусСоставителя" Тогда
		Если НЕ ПустаяСтрока(Значение) Тогда
			Если СтрНайти("<0><01><02><03><04><05><06><07><08><09><10><11><12><13><14><15>", "<" + Значение + ">") = 0 Тогда
...
Процедура РаспознатьДанныеВСтрокеДокумента(СтрокаДокумента, РеквизитИмпорта)
	...
	Если РеквизитИмпорта = "Документ" Тогда
		...
		Попытка
			...
			СтрокаДокумента.НомерДок = Формат(НомерДок, "ЧГ=0");
		...
		Если ОбщегоНазначения.ЗначениеНеЗаполнено(СтрокаДокумента.Документ) Тогда
		...
		Если ИспользоватьГраницуОбработки Тогда
			Если НЕ ОбщегоНазначения.ЗначениеНеЗаполнено(ДатаГраницыОбработки) Тогда
	...
	ИначеЕсли РеквизитИмпорта = "ТипДокумента" Тогда
		...
		Если ВидДокумента = "ПлатежноеПоручениеИсходящее" Тогда
			...
			Если ИспользоватьГраницуОбработки Тогда
				Если НЕ ОбщегоНазначения.ЗначениеНеЗаполнено(ДатаГраницыОбработки) Тогда
					Если НЕ ОбщегоНазначения.ЗначениеНеЗаполнено(СтрокаДокумента.Списано) Тогда
		...
		ИначеЕсли ВидДокумента = "ПлатежноеПоручениеВходящее" Тогда
			...
			Если ИспользоватьГраницуОбработки Тогда
				Если НЕ ОбщегоНазначения.ЗначениеНеЗаполнено(ДатаГраницыОбработки) Тогда
					Если НЕ ОбщегоНазначения.ЗначениеНеЗаполнено(СтрокаДокумента.Поступило) Тогда
	...
	ИначеЕсли РеквизитИмпорта = "Контрагент" Тогда
		...
		Если ОрганизацияПлательщик(СтрокаДокумента.ВидДокумента) Тогда
		...
		Если СокрЛП(Организация.ИНН) <> СокрЛП(ИННОрганизации) Тогда
			ДобавитьЗамечание(СтрокаДокумента, 2, "В документе указан неверный ИНН организации: " + ИННОрганизации + " !");
		КонецЕсли;
	...
	ИначеЕсли РеквизитИмпорта = "СчетКонтрагента" Тогда
		...
		Пока ВыборкаПоиска.Следующий() Цикл
			...
			Если СтепеньКандидата = 0 Тогда
				...
				Если ОбщегоНазначения.ЗначениеНеЗаполнено(СтрокаДокумента.Контрагент) Тогда
					Прервать;
...
Процедура ПроверитьНаПустоеЗначениеИмпорта(СтрокаИмпорта, ИмяСвойства, ПредставлениеСвойства)
	...
	Если Импорт_НеПустые[ВариантНеПустых][ИмяСвойства] = Истина Тогда
		Если ОбщегоНазначения.ЗначениеНеЗаполнено(СтрокаИмпорта[ИмяСвойства]) Тогда
...
Процедура ЗаписатьОбъект(ОбъектДляЗаписи)
	...
	Если ОбщегоНазначения.ЗначениеНеЗаполнено(ОбъектДляЗаписи.Дата) Тогда
	...
	ТипДокумента = ОбъектДляЗаписи.Метаданные().Имя;
		...
		Если ТипДокумента = "ПлатежноеПоручениеИсходящее" Тогда
			...
			НастройкаОтражатьНУ = "ППИсх_ОтражатьВНалоговомУчете";
		...
		ИначеЕсли ТипДокумента = "ПлатежноеПоручениеВходящее" Тогда
			...
			НастройкаОтражатьНУ = "ППВх_ОтражатьВНалоговомУчете";
		...
		ИначеЕсли ТипДокумента = "ПлатежныйОрдерСписаниеДенежныхСредств" Тогда
			...
			НастройкаОтражатьНУ = "ПОСпис_ОтражатьВНалоговомУчете";
		...
		ИначеЕсли ТипДокумента = "ПлатежныйОрдерПоступлениеДенежныхСредств" Тогда
			...
			НастройкаОтражатьНУ = "ПОПост_ОтражатьВНалоговомУчете";
		...
		Если (ЗаполнятьАвтоматически = Истина) Тогда
			УстановитьСвойство(ОбъектДляЗаписи, "ОтражатьВНалоговомУчете", (ПолучитьНастройкуЗаполнения(НастройкаОтражатьНУ) = Истина));
...
Функция ПолучитьВидДокумента(ПлатежнаяОперация, Списано, Поступило)
	...
	Списали = НЕ ОбщегоНазначения.ЗначениеНеЗаполнено(Списано);
	Зачислили = НЕ ОбщегоНазначения.ЗначениеНеЗаполнено(Поступило);
...
Функция СоздатьНовыйПлатежныйДокумент(ВидДокумента, Номер, Дата)
	...
	Если ВидДокумента = "ПлатежноеПорученияИсходящее" Тогда
...
Процедура ЗаполнитьРеквизитПлатежногоПорученияИсходящее(ОбъектДокумента, ИсходныеДанные, ИмяРеквизита)
	...
	Если ИмяРеквизита = "Организация" Тогда
		...
		Если ОбъектДокумента.ЭтоНовый() Тогда
			...
			Если ОбъектДокумента.Метаданные().Имя <> "ПлатежноеПорученияВходящее" Тогда
	...
	ИначеЕсли ИмяРеквизита = "Контрагент" Тогда
		...
		Если ОбщегоНазначения.ЗначениеНеЗаполнено(ОбъектДокумента.ДоговорКонтрагента) Тогда
			Если Не ОбщегоНазначения.ЗначениеНеЗаполнено(ОбъектДокумента.Контрагент) Тогда
				ОбъектДокумента.ДоговорКонтрагента =СоздатьНужныйДоговор(ОбъектДокумента.Контрагент, Организация, Ложь);
		...
		Если НЕ (ОбщегоНазначения.ЗначениеНеЗаполнено(ОбъектДокумента.Контрагент) ИЛИ ОбщегоНазначения.ЗначениеНеЗаполнено(ОбъектДокумента.ДоговорКонтрагента)) Тогда
			СчетаУчета = УправлениеВзаиморасчетами.ПолучитьСчетаРасчетовСКонтрагентом(Организация, ОбъектДокумента.Контрагент, ОбъектДокумента.ДоговорКонтрагента);
			СчетУчетаРегистр = СчетаУчета.СчетРасчетовПокупателя;
			
		КонецЕсли;
		
		Если ОбщегоНазначения.ЗначениеНеЗаполнено(СчетУчетаРегистр) Тогда
			
			Если ОбъектДокумента.ВалютаДокумента.Код = "643" Тогда // рубль
				Если ОбщегоНазначения.ЗначениеНеЗаполнено(ИсходныеДанные.Поступило) Тогда // платим мы в рублях
					СчетУчета = ПланыСчетов.Хозрасчетный.РасчетыСПоставщиками;
				КонецЕСли;
				
			Иначе // валюта
				Если ОбщегоНазначения.ЗначениеНеЗаполнено(ИсходныеДанные.Поступило) Тогда //платим мы в валюте
		...
		УстановитьСвойство(СтрокаРасшифровки, "СчетУчетаРасчетовСКонтрагентом", СчетУчета);
	...
	ИначеЕсли ИмяРеквизита = "СчетОрганизации" Тогда //заполняет при заполнении Контаргент
		...
		Если ОбщегоНазначения.ЗначениеНеЗаполнено(ОбъектДокумента.СчетБанк) Тогда
			// счет банк
	...
	ИначеЕсли ИмяРеквизита = "ВидОперации" Тогда 
		...
		Если НЕ ОбщегоНазначения.ЗначениеНеЗаполнено(ИсходныеДанные.СтатусСоставителя) Тогда
			ВидОперации = Перечисления.ВидыОперацийППИсходящее.ПеречислениеНалога;
		Иначе
			Если ОбщегоНазначения.ЗначениеНеЗаполнено(ИсходныеДанные.Поступило) Тогда
	...
	ИначеЕсли ИмяРеквизита = "СуммаДокумента" Тогда
		Если ОбъектДокумента.ЭтоНовый() Тогда
			...
			Если НЕ ОбщегоНазначения.ЗначениеНеЗаполнено(ИсходныеДанные.Списано) Тогда
		...
		Иначе
			...
			Если НЕ ОбщегоНазначения.ЗначениеНеЗаполнено(ИсходныеДанные.Списано) Тогда
				...
				Если ОбщегоНазначения.ЗначениеНеЗаполнено(ИсходныеДанные.ПлатежныйОрдер) Тогда
...
Процедура ЗаполнитьРеквизитПлатежногоПорученияВходящее(ОбъектДокумента, ИсходныеДанные, ИмяРеквизита)
	...
	Если ИмяРеквизита = "Организация" Тогда
		...
		Если НЕ ОбщегоНазначения.ЗначениеНеЗаполнено(ИсходныеДанные.Поступило) Тогда
			УстановитьСвойство(ОбъектДокумента, "Оплачено", Истина);
			
		КонецЕсли;

		Если (ОбъектДокумента.Организация <> Организация)
		 ИЛИ (ОбъектДокумента.Дата <> ИсходныеДанные.Поступило)
		 ИЛИ (ОбъектДокумента.ЭтоНовый()) Тогда

			Если ОбщегоНазначения.ЗначениеНеЗаполнено(ОбъектДокумента.Дата) Тогда
	...
	ИначеЕсли ИмяРеквизита = "Контрагент" Тогда
		...
		Если ОбщегоНазначения.ЗначениеНеЗаполнено(ОбъектДокумента.ДоговорКонтрагента) Тогда
			Если Не ОбщегоНазначения.ЗначениеНеЗаполнено(ОбъектДокумента.Контрагент) Тогда
				ОбъектДокумента.ДоговорКонтрагента =СоздатьНужныйДоговор(ОбъектДокумента.Контрагент, ОбъектДокумента.Организация, Истина);
				СтрокаРасшифровки.ДоговорКонтрагента = ОбъектДокумента.ДоговорКонтрагента;
			КонецЕсли;
		КонецЕсли;
		...
		Если (ЗаполнятьАвтоматически <> Истина) Тогда
			...
			Если ОбщегоНазначения.ЗначениеНеЗаполнено(СчетУчетаРегистр) Тогда
				...
				Если ОбъектДокумента.ВалютаДокумента.Код = "643" Тогда // рубль
					Если ОбщегоНазначения.ЗначениеНеЗаполнено(ИсходныеДанные.Списано) Тогда // платят нам в рублях - ДЛЯ ВХОДЯЩЕГО
						СчетУчета = ПланыСчетов.Хозрасчетный.РасчетыСПокупателями;
					КонецЕСли;
					
				Иначе // валюта
					Если ОбщегоНазначения.ЗначениеНеЗаполнено(ИсходныеДанные.Списано) Тогда //платят нам в валюте - ДЛЯ ВХОДЯЩЕГО
...
Функция СоздатьПлатежныйОрдер(ОбъектДокумента, СуммаДокумента)
	...
	НовыйОрдер.Дата = ОбъектДокумента.Дата;
	
	НовыйОрдер.УстановитьНовыйНомер(НовыйОрдер.Организация.Префикс);
	...
	Для каждого СтрокаСекции из ДокументыКИмпорту Цикл
		Если СтрокаСекции.Загрузить Тогда
			...
			Если ИспользоватьГраницуОбработки Тогда
				Если НЕ ОбщегоНазначения.ЗначениеНеЗаполнено(ДатаГраницыОбработки) Тогда
					Если НачалоДня(СтрокаСекции.ДатаДок) <= НачалоДня(ДатаГраницыОбработки) Тогда
						Ответ = Вопрос("Платежный документ """ + СтрокаСекции.Операция + """ №" + СтрокаСекции.Номер + " от " + СтрокаСекции.Дата + """ имеет дату равной или меньшей границы обработки ! Загрузить?", РежимДиалогаВопрос.ДаНет);
						Если Ответ = КодВозвратаДиалога.Нет Тогда
							Продолжить;
						КонецЕсли;
						
					ИначеЕсли НЕ ОбщегоНазначения.ЗначениеНеЗаполнено(СтрокаСекции.Поступило) Тогда
						Если НачалоДня(СтрокаСекции.ДатаДок) <= НачалоДня(ДатаГраницыОбработки) Тогда
							Ответ = Вопрос("Платежный документ """ + СтрокаСекции.Операция + """ №" + СтрокаСекции.Номер + " от " + СтрокаСекции.Дата + """ имеет дату поступления средств равной или меньшей границы обработки ! Загрузить?", РежимДиалогаВопрос.ДаНет);
							Если Ответ = КодВозвратаДиалога.Нет Тогда
								Продолжить;
							КонецЕсли;
						КонецЕсли;
						
					ИначеЕсли НЕ ОбщегоНазначения.ЗначениеНеЗаполнено(СтрокаСекции.Списано) Тогда
						Если НачалоДня(СтрокаСекции.ДатаДок) <= НачалоДня(ДатаГраницыОбработки) Тогда
							Ответ = Вопрос("Платежный документ """ + СтрокаСекции.Операция + """ №" + СтрокаСекции.Номер + " от " + СтрокаСекции.Дата + """ имеет дату списания средств равной или меньшей границы обработки ! Загрузить?", РежимДиалогаВопрос.ДаНет);
							Если Ответ = КодВозвратаДиалога.Нет Тогда
								Продолжить;
							КонецЕсли;
						КонецЕсли;
			...
			Если ПустаяСтрока(РезультатПроверки) Тогда
				...
				Если ОбщегоНазначения.ЗначениеНеЗаполнено(СтрокаСекции.Документ) Тогда
				...
				Если ОбщегоНазначения.ЗначениеНеЗаполнено(ОбъектДокумента.Контрагент) ИЛИ
					ОбщегоНазначения.ЗначениеНеЗаполнено(ОбъектДокумента.ДоговорКонтрагента) ИЛИ
					ОбщегоНазначения.ЗначениеНеЗаполнено(СтрокаСекции.СчетКонтрагента) Тогда
				...
				Если НЕ (ОбщегоНазначения.ЗначениеНеЗаполнено(СтрокаСекции.Списано) И ОбщегоНазначения.ЗначениеНеЗаполнено(СтрокаСекции.Поступило)) Тогда
