...
Функция СоздатьСосединениеЧерезПрокси()
	...
	Если ЗапомнитьИмяИПароль ИЛИ НЕ ОбщегоНазначения.ЗначениеНеЗаполнено(ИмяПользователя) Тогда
	...
	Если ТипЗнч(HTTP) <> Тип("HTTPСоединение") Тогда
		...
		Если ЗапомнитьИмяИПароль = Истина ИЛИ НЕ ОбщегоНазначения.ЗначениеНеЗаполнено(ИмяПользователя) Тогда
...
Функция ЗапроситьФайлыССервера(СерверИсточникПараметр, СтрокаПараметраПолученияПараметр, ИмяВходящегоФайлаПараметр, HTTP = Неопределено) Экспорт
	...
	Попытка
		...
		Возврат Истина;
	Исключение
		Если СтрНайти(ОписаниеОшибки(), "407") <> 0 И СтрНайти(НРег(ОписаниеОшибки()), "authentication") <> 0 Тогда
