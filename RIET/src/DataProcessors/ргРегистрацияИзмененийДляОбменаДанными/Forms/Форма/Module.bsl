...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	ИначеЕсли Параметры.ИдентификаторКоманды="ОткрытьФормуРедактированияРегистрации" Тогда
		...
		Иначе
			ВызватьИсключение СтрЗаменить(
				НСтр("ru='Неверные параметры команды открытия ""%1""'"),
	...
	Иначе
		ВызватьИсключение СтрЗаменить(
			НСтр("ru='Нераспознанная команда ""%1""'"),
			"%1", Параметры.ИдентификаторКоманды);
	КонецЕсли;
	...
	Параметры.Свойство("ЗапрещеноВыбиратьУзелОбмена", ЗапрещеноВыбиратьУзелОбмена);
	Если Не ИнициализироватьНастройки() И ОткрытиеСУзломПараметром Тогда
		ВызватьИсключение СтрЗаменить(
...
Процедура ПриОткрытии(Отказ)
	...
	Если ПараметрОбъектРегистрации<>Неопределено Тогда
		...
		ОткрытьФорму(
			...
			Новый Структура("ОбъектРегистрации, ТаблицаРегистрации", ПараметрОбъектРегистрации, ПараметрТаблицаРегистрации));
	КонецЕсли;
...
Процедура ОбработкаВыбора(ВыбранноеЗначение, ИсточникВыбора)
	...
	Если ТипЗнч(ВыбранноеЗначение)<>Тип("Структура") 
		...
		Или ТипЗнч(ВыбранноеЗначение.ДанныеВыбора)<>Тип("Строка")
	Тогда
...
Процедура ПриЗагрузкеДанныхИзНастроекНаСервере(Настройки)
	...
	Иначе
		...
		// Если восстановленый узел обмена был удален, то очищаем его
		Если УзелОбменаСсылка<>Неопределено 
		    И ПланыОбмена.ТипВсеСсылки().СодержитТип(ТипЗнч(УзелОбменаСсылка))
		    И ПустаяСтрока(УзелОбменаСсылка.ВерсияДанных) 
		Тогда
	...
	Если Данные<>Неопределено Тогда
		...
		ОткрытьФорму(ПолучитьИмяФормы() + "Форма.УзлыРегистрацииОбъекта",
			Новый Структура("ОбъектРегистрации, ТаблицаРегистрации, ОповещатьОбИзменениях", 
				Данные, ТаблицаРегистрации, Истина
			),
			ЭтаФорма);
	КонецЕсли;
...
Процедура РедактироватьНомерСообщенияКонстанты()
	...
	Если ВвестиЧисло(НомерСообщения, Подсказка) Тогда
		...
		СообщитьОРезультатахРегистрации(НомерСообщения, 
			ИзменитьНомерСообщенияНаСервере(УзелОбменаСсылка, НомерСообщения, ТекДанные.МетаПолноеИмя));
...
Процедура РедактироватьНомерСообщенияСсылки()
	...
	Если ВвестиЧисло(НомерСообщения, Подсказка) Тогда
		...
		СообщитьОРезультатахРегистрации(НомерСообщения, 
			ИзменитьНомерСообщенияНаСервере(УзелОбменаСсылка, НомерСообщения, ТекДанные.Ссылка));
...
Процедура РедактироватьНомерСообщенияСписокНаборов()
	...
	Если ВвестиЧисло(НомерСообщения, Подсказка) Тогда
		...
		СообщитьОРезультатахРегистрации(НомерСообщения, 
			ИзменитьНомерСообщенияНаСервере(УзелОбменаСсылка, НомерСообщения, ДанныеСтроки, СписокНаборовЗаписейИмяТаблицы));
...
Процедура СообщитьОРезультатахРегистрации(Команда, Результаты)
	...
	Если Предупреждением Тогда
		Предупреждение(ТекстПредупреждения,,ЗаголовокПредупреждения);
	Иначе
...
Процедура ДобавитьРегистрациюКонстантыВСписке()
	...
	ТекПараметры = Новый Структура("УзелОбмена, МассивИменМетаданных, МассивПредставлений, МассивАвторегистрации", 
		УзелОбменаСсылка,
		СтруктураИменМетаданных.Константы,
		СтруктураПредставленийМетаданных.Константы,
...
Процедура УдалитьРегистрациюКонстантыВСписке()
	...
	Если Ответ=КодВозвратаДиалога.Да Тогда
		СообщитьОРезультатахРегистрации(Ложь, 
			УдалитьРегистрациюНаСервере(Истина, УзелОбменаСсылка, СписокИмен));
		Элементы.СписокКонстант.Обновить();
		ЗаполнитьКоличествоРегистрацийВДереве();
	КонецЕсли;
КонецПроцедуры
...
Процедура ДобавитьРегистрациюВСписокСсылок(ЭтоПодбор=Ложь)
	...
	ТекПараметры = Новый Структура("РежимВыбора, МножественныйВыбор, ЗакрыватьПриВыборе, ВыборГруппИЭлементов", 
		Истина, Истина, ЭтоПодбор, ИспользованиеГруппИЭлементов.ГруппыИЭлементы);
...
Процедура УдалитьРегистрациюИзСпискаСсылок()
	...
	Если Ответ=КодВозвратаДиалога.Да Тогда
		СообщитьОРезультатахРегистрации(Ложь,
			УдалитьРегистрациюНаСервере(Истина, УзелОбменаСсылка, СписокУдаления));
		Элементы.СписокСсылок.Обновить();
		ЗаполнитьКоличествоРегистрацийВДереве();
	КонецЕсли;
КонецПроцедуры
...
Процедура УдалитьРегистрациюВНабореЗаписей()
	...
	Выбор = Новый Структура("ИмяТаблицы, ДанныеВыбора, ДействиеВыбора, СтруктураПолей",
		СписокНаборовЗаписейИмяТаблицы,
		Данные,
		Ложь,
		СтруктураДанных);
...
Процедура ДобавитьРегистрациюВыделенныхОбъектов(БезУчетаАвторегистрации=Истина)
	...
	Результат = ДобавитьРегистрациюНаСервере(БезУчетаАвторегистрации, УзелОбменаСсылка, Данные.МетаИмена);
	ЗаполнитьКоличествоРегистрацийВДереве();
	ОбновитьСодержимоеСтраницы();
	СообщитьОРезультатахРегистрации(Истина, Результат);
...
Процедура УдалитьРегистрациюВыделенныхОбъектов(БезУчетаАвторегистрации=Истина)
	...
	СообщитьОРезультатахРегистрации(Ложь,
		УдалитьРегистрациюНаСервере(БезУчетаАвторегистрации, УзелОбменаСсылка, Данные.МетаИмена));
	ЗаполнитьКоличествоРегистрацийВДереве();
	ОбновитьСодержимоеСтраницы();
КонецПроцедуры
...
Процедура ОбработкаВыбораДанных(ТаблицаФормы, ВыбранноеЗначение)
	...
	Иначе
		...
		Ответ = Вопрос(Текст, РежимДиалогаВопрос.ДаНет,,,"Подтверждение");
		Если Ответ<>КодВозвратаДиалога.Да Тогда
			Возврат;
		КонецЕсли;
		Результат = УдалитьРегистрациюНаСервере(Истина, УзелОбменаСсылка, Данные, ИмяТаблицы);
	КонецЕсли;
...
Процедура НастроитьРедактированиеИзмененийСервер(ТекущаяСтрока)
	...
	ИначеЕсли Метаданные.Справочники.Содержит(Мета) 
		...
		Или Метаданные.Задачи.Содержит(Мета)
	Тогда
		// Ссылочный тип
