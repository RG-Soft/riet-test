...
Процедура ПроверитьСервереВеб(ТекстДляПроверки, Отказ)
	...
	Попытка
		...
		Пока ТекстДляПроверки <> "" И ТекстДляПроверки <> Символы.ПС Цикл
			...
			Если SpellResult.Свойства().Количество()>0 Тогда
				Если ТипЗнч(SpellResult.error) = Тип("ОбъектXDTO") Тогда // И SpellResult.Получить("error")  SpellResult.Свойства().Количество()=1 И
					...
					ПозицияНачалоОшибки = СтрНайти(ТекущийТекстПроверкиТМП, Строка.word);
					...
					Если Строка.Свойства().Получить("s") <> Неопределено Тогда
						...
						КонецЕсли; 
					Иначе
						НоваяСтрока.ВариантЗамены = "вариантов замены не обнаружено";
					КонецЕсли; 
					
					СлужебноеСообщение = СлужебноеСообщение + Символы.ПС + "<SPAN style=""COLOR: #ff0000"">Вариант замены:</SPAN> " + ВариантЗаменыСтрока;
		...
		Если Не ФлагОшибок  Тогда 
			...
			Сообщение.Текст = "Орфографических ошибок не обнаружено";
...
Функция ПолучитьТекущийТекстПроверки(ТекстДляПроверки, ДлинаБлокаПроверки)
	...
	Если СтрНайти(СимволыРазделители, Лев(ТекстВозврата,1))=0 Тогда 
		Сч = 0;
		ТекСимвол = "";
		Пока СтрНайти(СимволыРазделители, ТекСимвол)=0 И Сч>0 Цикл 
	...
	Если  СтрНайти(СимволыРазделители, Прав(ТекстВозврата,1))=0 Тогда 
...
Процедура ПриОткрытии(Отказ)
	...
	Если Отказ Тогда
		Предупреждение("Превышена максимальная длина сообщения для проверки!");
	КонецЕсли;
...
Процедура ТаблицаОшибокВыбор(Элемент, ВыбраннаяСтрока, Поле, СтандартнаяОбработка)
	...
	Если Элемент.ТекущиеДанные.ВариантЗамены = "вариантов замены не обнаружено" Тогда
		Возврат;
	КонецЕсли; 
...
Процедура ЗаменитьВсе(Команда)
	...
	Если Вопрос("Заменить все ошибочные слова по выбранным в списке?", РежимДиалогаВопрос.ДаНет)=КодВозвратаДиалога.Нет Тогда
		Возврат;
	КонецЕсли; 
	
	Для Каждого СтрокаТЗ Из ТаблицаОшибок Цикл
		Если СтрокаТЗ.Активность И СтрокаТЗ.ВариантЗамены <> "вариантов замены не обнаружено" Тогда
...
Процедура ДобавитьВИсключение(Команда)
	...
	КонецЕсли; 
	Попытка
		ОшибочноеСлово = Элементы.ТаблицаОшибок.ТекущиеДанные.ОшибочноеСлово;
		ЗаписатьСловоВРегистр(ОшибочноеСлово);
		Массив = ТаблицаОшибок.НайтиСтроки(Новый Структура("ОшибочноеСлово", ОшибочноеСлово));
		Для Каждого ЭлМассива Из Массив Цикл
			ТаблицаОшибок.Удалить(ЭлМассива);
		КонецЦикла; 
	Исключение
	КонецПопытки;
...
&НаСервереБезКонтекста
Процедура ЗаписатьСловоВРегистр(ОшибочноеСлово)
	
	НаборЗаписей = РегистрыСведений.ИсключенияОрфографии.СоздатьНаборЗаписей();
	НаборЗаписей.Отбор.СловоИсключение.Установить(ОшибочноеСлово);
	Если НаборЗаписей.Количество() = 0 Тогда
		НоваяЗапись = НаборЗаписей.Добавить();
		НоваяЗапись.СловоИсключение = ОшибочноеСлово;
	КонецЕсли; 
