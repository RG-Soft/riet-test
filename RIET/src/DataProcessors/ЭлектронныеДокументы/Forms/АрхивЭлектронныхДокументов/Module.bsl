	...
	Пока Выборка.Следующий() Цикл
		...
		Попытка
			...
			Если ВидПараметра = "Ответственный" Тогда
				...
				ЭлектронныеДокументыСлужебныйВызовСервера.ИзменитьПоСсылкеПрисоединенныйФайл(Выборка.Ссылка, СтруктураПараметров, Ложь);
			КонецЕсли;
...
Процедура СравнитьДанныеЭД(ТекущийСписок)
	...
	#Если НЕ ТолстыйКлиентУправляемоеПриложение И НЕ ТолстыйКлиентОбычноеПриложение Тогда
		Сообщить(ВернутьСтр("ru='Сравнение электронных документов можно сделать только в режиме толстого клиента.'"));
		Возврат;
	#Иначе
	...
	Для Каждого СтрокаТаблицы Из Элементы[ТаблицаПакетов].ВыделенныеСтроки Цикл
		...
		Исключение
			...
			ЭлектронныеДокументыСлужебныйВызовСервера.ОбработатьИсключениеПоЭДНаСервере(
				ВернутьСтр("ru = 'изменение статуса пакетов ЭД'"), ТекстОшибки, ТекстСообщения);
		КонецПопытки;
	КонецЦикла;
...
Процедура ВыполнитьСравнениеЭД(СтруктураПараметров)
	...
	#Если НЕ ТолстыйКлиентУправляемоеПриложение И НЕ ТолстыйКлиентОбычноеПриложение Тогда
		ТекстСообщения = ВернутьСтр("ru='Сравнение электронных документов можно сделать только в режиме толстого клиента.'");
	...
	#Иначе
		...
		Если НЕ (ЗначениеЗаполнено(ЭДПервый) И ЗначениеЗаполнено(ЭДВторой)) Тогда
			ТекстСообщения = ВернутьСтр("ru='Не указан один из сравниваемых электронных документов.'");
			ОбщегоНазначенияКлиентСервер.СообщитьПользователю(ТекстСообщения);
			Возврат;
		КонецЕсли;
		...
		Если ПереченьВременныхФайлов = Неопределено Тогда
			ТекстСообщения = ВернутьСтр("ru='Ошибка при разборе электронного документа.'");
			ОбщегоНазначенияКлиентСервер.СообщитьПользователю(ТекстСообщения);
			Возврат;
		КонецЕсли;
...
Процедура Отправить(Команда)
	...
	ЭлектронныеДокументыСлужебныйКлиент.ОтправитьМассивПакетовЭД(МассивЭД, ОбработкаОповещения);
...
Процедура КомандаОтправитьОповещение(КолОтправленныхПакетов, ДополнительныеПараметры) Экспорт
	...
	ТекстОповещения     = ВернутьСтр("ru = 'Отправленных пакетов нет'");
	
	Если ЗначениеЗаполнено(КолОтправленныхПакетов) Тогда
	
		ТекстОповещения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			ВернутьСтр("ru = 'Отправлено пакетов: (%1)'"), КолОтправленныхПакетов);
	КонецЕсли;
...
Процедура УстановитьСтатусКРаспаковке(Команда)
	...
	ТекстОповещения = ВернутьСтр("ru = 'Изменен статус пакетов на ""К распаковке""'") + ": (%1)";
...
Процедура УстановитьСтатусПодготовленКОтправке(Команда)
	...
	ТекстОповещения = ВернутьСтр("ru = 'Изменен статус пакетов на ""Подготовлен к отправке""'" + ": (%1)");
	ТекстОповещения = СтрЗаменить(ТекстОповещения, "%1", Количество);
	ПоказатьОповещениеПользователя(ВернутьСтр("ru = 'Обмен электронными документами'"), , ТекстОповещения);
	Элементы[ТаблицаПакетов].Обновить();
...
Процедура СохранитьПакетыЭДНаДиск(Команда)
	...
	Если МассивФайлов.Количество() Тогда
		ПустойОбработчик = Новый ОписаниеОповещения("ПустойОбработчик", ЭлектронныеДокументыСлужебныйКлиент);
...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если НЕ ИспользуетсяОбменСБанком Тогда
		МассивИсключенияВходящихЭД = ЭлектронныеДокументыСлужебный.БанковскиеВидыЭД();
	КонецЕсли;
	ВидыЭДВходящие = ОбщегоНазначенияКлиентСервер.СократитьМассив(АктуальныеВидыЭД, МассивИсключенияВходящихЭД);
	Если ИспользуетсяОбменСБанком Тогда
		ВидыЭДВходящие.Добавить(Перечисления.ВидыЭД.ВыпискаБанка);
		ВидыЭДВходящие.Добавить(Перечисления.ВидыЭД.ИзвещениеОСостоянииЭД);
		ВидыЭДВходящие.Добавить(Перечисления.ВидыЭД.Квитанция);
	КонецЕсли;
	...
	Если НЕ ИспользуетсяОбменСБанком Тогда
		...
		ОбщегоНазначенияКлиентСервер.ДополнитьМассив(МассивИсключенияИсходящихЭД, БанковскиеВидыЭД, Истина);
	КонецЕсли;
	ВидыЭДИсходящие = ОбщегоНазначенияКлиентСервер.СократитьМассив(АктуальныеВидыЭД, МассивИсключенияИсходящихЭД);
	Если ИспользуетсяОбменСБанком Тогда
		ВидыЭДИсходящие.Добавить(Перечисления.ВидыЭД.ЗапросВыписки);
		ВидыЭДИсходящие.Добавить(Перечисления.ВидыЭД.ЗапросЗонд);
		ВидыЭДИсходящие.Добавить(Перечисления.ВидыЭД.ЗапросНаОтзывЭД);
		ВидыЭДИсходящие.Добавить(Перечисления.ВидыЭД.ЗапросОСостоянииЭД);
		ВидыЭДИсходящие.Добавить(Перечисления.ВидыЭД.ПлатежноеПоручение);
		ВидыЭДИсходящие.Добавить(Перечисления.ВидыЭД.ПлатежноеТребование);
	КонецЕсли;
