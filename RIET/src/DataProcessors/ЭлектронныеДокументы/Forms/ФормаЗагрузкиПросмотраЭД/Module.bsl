...
Процедура ИзменитьВидимостьДоступностьПриСозданииНаСервере()
	...
	Если ЗагрузкаЭД Тогда
		...
		Если ВРег(ВидЭД) = ВРег("РеквизитыОрганизации") Тогда
			...
			Заголовок = ВернутьСтр("ru = 'Загрузка данных из файла'");
			
			Элементы.ГруппаНастроекСправочники.Видимость = Истина;
			Элементы.ГруппаНастроекДокументы.Видимость = Ложь;
		Иначе
			
			Заголовок = ВернутьСтр("ru = 'Загрузка документа из файла'");
	...
	Иначе
		Текст = ВернутьСтр("ru = 'Электронный документ'");
		Заголовок = Текст;
		Элементы.ГруппаНастроекСправочники.Видимость = Ложь;
		Элементы.ГруппаНастроекДокументы.Видимость = Ложь;
	...
	Если ВидЭД = Перечисления.ВидыЭД.КаталогТоваров Тогда
		...
		Элементы.ТипОбъекта.Заголовок = ВернутьСтр("ru = 'Загрузить'");
...
Функция СформироватьДокументИБ(ДанныеФормы, ТекстСообщения, Записывать = Ложь, ОбновитьСтруктуруРазбора = Ложь)
	...
	Если СтруктураРазбора <> Неопределено И СтруктураРазбора.Свойство("ДеревоРазбора", ДеревоРазбора)
		...
		Попытка
			...
			Иначе
				КонтрагентОбъект = КонтрагентСсылка;
			КонецЕсли;
				...
				Если ВыборкаЭДДопДанных.Следующий() Тогда
					...
					Если ДопДанныеЭД.Свойство("СсылкаНаДвоичныеДанныеФайла", СсылкаНаДДДопДанныхЭД)
						...
						Если ИмяФайлаДопДанных = Неопределено Тогда
							ТекстОшибки = ВернутьСтр("ru = 'Не удалось получить доп. данные электронного документа.
...
Процедура СформироватьТекстСлужебногоСообщения()
	...
	Если ЗначениеЗаполнено(ИмяФайлаХМЛ) Тогда
		...
		ЗаголовокЭлемента = ВернутьСтр("ru = 'Не удалось прочитать файл """+ ИмяФайлаХМЛ+".""'");
		
	Иначе
		
		ЗаголовокЭлемента = ВернутьСтр("ru = 'Не найден файл электронного документа ""* .xml.""'");
...
Процедура ЗагрузитьРеквизитыОрганизации()
	...
	Если СпособЗагрузкиДокумента = 1 И Не ЗначениеЗаполнено(ДокументИБ) Тогда
		ТекстСообщения = ВернутьСтр("ru = 'Не указан элемент справочника для перезаполнения.'");
...
Процедура ЗагрузитьДокументЭДО(ЗакрытьФорму)
	...
	Если НЕ МожноЗагрузитьЭДВида(ВидЭД) Тогда
		ТекстСообщения = ВернутьСтр("ru = 'Не поддерживается загрузка электронных документов вида ""%1"".'");
	...
	Если НЕ ЗначениеЗаполнено(Контрагент) Тогда
		ТекстСообщения = ВернутьСтр("ru = 'Не указан контрагент.'");
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(ТекстСообщения);
		Отказ = Истина;
	КонецЕсли;
	...
	Если СпособЗагрузкиДокумента = 1 Тогда
		...
		Если Не ЗначениеЗаполнено(ДокументИБ) Тогда
			ТекстСообщения = ВернутьСтр("ru = 'Не указан документ для перезаполнения.'");
			ОбщегоНазначенияКлиентСервер.СообщитьПользователю(ТекстСообщения);
			Отказ = Истина;
		КонецЕсли;
...
&НаКлиенте
Процедура СопоставитьПослеЗаполненияОповещение(Результат, ДополнительныеПараметры) Экспорт
