...
Процедура ВыгрузитьРеквизитыОрганизации(АдресХранилища, УникальныйИдентификатор)
	...
	Попытка
		...
		Иначе
			...
			ЭлектронныеДокументыСлужебныйВызовСервера.ОбработатьИсключениеПоЭДНаСервере(ВернутьСтр("ru = 'Формирование ЭД'"),
																						ТекстОшибки,
																						ТекстОшибки);
		КонецЕсли;
...
Процедура ОбновитьФорму()
	...
	Если ЮрФизЛицо = ИндивидуальныйПредприниматель Тогда
		...
		НадписьСвидетельство = ВернутьСтр("ru ='Свидетельство №'") + СвидетельствоНомер + ВернутьСтр("ru =' от '") + Формат(СвидетельствоДата, "ДЛФ=D");
	Иначе
...
Процедура ВыгрузитьРеквизиты(Команда)
	...
	Если СпособВыгрузки = ПредопределенноеЗначение("Перечисление.СпособыОбменаЭД.ЧерезЭлектроннуюПочту") Тогда
		Если НЕ ЗначениеЗаполнено(АдресВыгрузки) Тогда
			ТекстСообщения = ВернутьСтр("ru = 'Не выбрана учетная запись электронной почты.'");
