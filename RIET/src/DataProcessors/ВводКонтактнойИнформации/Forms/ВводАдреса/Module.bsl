...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если ВидКонтактнойИнформации.ПроверятьПоФИАС Тогда
		Команды.ПроверитьЗаполнениеАдреса.Подсказка = ВернутьСтр("ru = 'Проверить заполнение адреса в формате ФИАС'");
	Иначе
		Команды.ПроверитьЗаполнениеАдреса.Подсказка  = ВернутьСтр("ru = 'Проверить заполнение адреса в формате КЛАДР'");
	КонецЕсли;
	...
	Если АдресТолькоРоссийский  Тогда
		...
		Если Страна <> СтранаРоссия Тогда
			...
			КонецЕсли;
			ТекстПредупрежденияПриОткрытии = ВернутьСтр("ru = 'Адрес введен некорректно: допускается ввод только российских адресов. Значение поля ""Страна"" было изменено на ""Россия"", необходимо проверить остальные поля.'"); 
				...
				Если ОбщегоНазначения.ПодсистемаСуществует("СтандартныеПодсистемы.АдресныйКлассификатор") Тогда
					...
					Если СведенияОРегионе.Загружен = Ложь Тогда
						...
						ДоступностьКлассификатора.КраткоеПредставлениеОшибки = 
							СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(ВернутьСтр("ru = 'Необходимо загрузить адресные сведения по региону ""%1"" (в меню ""%2"").'"),
							XDTOКонтактная.Состав.Состав.СубъектРФ,
							?(ЭтоТакси, ВернутьСтр("ru = 'Еще'"), ВернутьСтр("ru = 'Все действия'")));
	...
	ИначеЕсли ВыбранноеЗначение.Отказ Тогда
		...
		Иначе
			Если НЕ ПодборПоИндексуДоступен Тогда
				ТекстСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(ВернутьСтр("ru = 'Индекс %1 не найден в адресном классификаторе'"),
	...
	Если ТипЗначения = Тип("Структура") Тогда
		...
		Если МожноЗагружатьКлассификатор И РегионНеЗагружен Тогда
			...
			УправлениеКонтактнойИнформациейКлиент.ПредложениеЗагрузкиКлассификатора(
				СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(ВернутьСтр("ru='Данные для ""%1"" не загружены.'"), ВыбранноеЗначение.Представление),
...
Процедура УлицаНачалоВыбора(Элемент, ДанныеВыбора, СтандартнаяОбработка)
	...
	Иначе
		...
		ПараметрыФормы.Вставить("Улица", Улица);
		ПараметрыФормы.Вставить("Заголовок", НаселенныйПункт + " " + ВернутьСтр("ru = '(не найден)'"));
	КонецЕсли;
...
Процедура ПроверитьЗаполнениеАдреса(Команда)
	...
	Если РазрешитьВводАдресаВСвободнойФорме Тогда
		ПоказатьПредупреждение(, ВернутьСтр("ru='Адрес не может быть проверен, так как он введен в свободной форме.'"));
		Возврат;
	КонецЕсли;
...
Процедура ВвестиАдресВСвободнойФорме(Команда)
	...
	Если РазрешитьВводАдресаВСвободнойФорме Тогда
		ТекстВопроса = ВернутьСтр("ru='Изменения, введенные вручную, будут потеряны.
								|Исходный адрес будет перемещен в комментарий.
		                        |Продолжить?'");
	Иначе
		ТекстВопроса = ВернутьСтр("ru='Ввести адрес в свободной форме?
	...
	ПоказатьВопрос(Оповещение, ТекстВопроса, РежимДиалогаВопрос.ДаНет, , , ВернутьСтр("ru='Подтверждение'"));
КонецПроцедуры
...
Процедура ВвестиАдресВСвободнойФормеЗавершение(Знач РезультатВопроса, Знач ДополнительныеПараметры) Экспорт
	...
	Если РазрешитьВводАдресаВСвободнойФорме Тогда
		Комментарий = ?(НЕ ПустаяСтрока(Комментарий), Комментарий + Символы.ПС, "")
			+ ВернутьСтр("ru = 'Исходный адрес:'") + " " + ПредставлениеАдреса;
		ПодключитьОбработчикОжидания("УстановитьПиктограммуКомментария", 0.1, Истина);
	КонецЕсли;
...
Функция СписокОшибокЗаполнения(Контекст, ПредупреждатьОбОтсутствии)
	...
	Если Результат.Количество() = 0 // Нет ошибок
		...
		Результат.Добавить("/", ВернутьСтр("ru = 'Адрес пуст.'"));
	КонецЕсли;
...
Процедура ОтобразитьИнформациюОДатахДействияАдреса(ДействуетС, ДействуетДо)
	...
	Если ЗначениеЗаполнено(ДействуетДо) И ЗначениеЗаполнено(ДействуетС)Тогда
		Если НачалоДня(ДействуетДо -10) = НачалоДня(ДействуетС) Тогда
			ТекстИсторическийАдрес = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(ВернутьСтр("ru = 'Адрес действовал только %1'"), Формат(ДействуетС, "ДЛФ=DD"));
		Иначе
			ТекстИсторическийАдрес = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(ВернутьСтр("ru = 'Адрес действовал с %1 по %2 (включительно)'"), Формат(ДействуетС, "ДЛФ=DD"), Формат(ДействуетДо - 10, "ДЛФ=DD"));
		КонецЕсли;
	ИначеЕсли НЕ ЗначениеЗаполнено(ДействуетС) И ЗначениеЗаполнено(ДействуетДо) Тогда
		ТекстИсторическийАдрес = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(ВернутьСтр("ru = 'Адрес действовал до %1 (включительно)'"), Формат(ДействуетДо - 10, "ДЛФ=DD"));
	Иначе
		ТекстИсторическийАдрес = ВернутьСтр("ru = 'Адрес действует в настоящие время'");
	КонецЕсли;
...
Процедура СообщитьОбОшибкахЗаполнения(СписокОшибок, ПредупреждатьОбОтсутствии)
	...
	Если КоличествоОшибок = 0 И ПредупреждатьОбОтсутствии Тогда
		// Нет ошибок
		ПоказатьПредупреждение(, ВернутьСтр("ru='Адрес введен корректно.'"));
		Возврат;
	КонецЕсли;	
...
Процедура СостояниеВводаПредставленияАдреса(РедактироватьПредставлениеАдреса, ФормироватьПредставление = Истина)
	...
	Если РедактироватьПредставлениеАдреса Тогда
		...
		Элементы.ПредставлениеАдреса.Заголовок          = ВернутьСтр("ru='Адрес в свободной форме'");
...
Процедура ОбновитьДоступностьКлассификатора(Контекст)
	...
	Если ДоступностьКлассификатора.Отказ Тогда
		Контекст.ТекстСообщенияСервиса = ВернутьСтр("ru = 'Автоподбор и проверка адреса недоступны:'") + Символы.ПС 
			+ ДоступностьКлассификатора.КраткоеПредставлениеОшибки;
	КонецЕсли;
