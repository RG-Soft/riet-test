...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если ТипЗнч(Параметры.СписокКонтактнойИнформации) = Тип("Массив") Тогда
		...
		КонецЦикла;
		ЭтотОбъект.Заголовок = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(ВернутьСтр("ru = 'История изменений (%1)'"), ПредставлениеКонтактнойИнформации);
...
Процедура ИсторияПередУдалением(Элемент, Отказ)
	...
	Если НЕ Отказ Тогда
		...
		ПоказатьВопрос(Оповещение, ВернутьСтр("ru = 'Вы хотите удалить адрес действующий с'") + " " + Формат(ДействуетС, "ДЛФ=DD")+ "?", РежимДиалогаВопрос.ДаНет);
	КонецЕсли;
	Отказ = Истина;
КонецПроцедуры
...
Процедура УстановитьУсловноеОформление()
	...
	Элемент.Оформление.УстановитьЗначениеПараметра("Текст", ВернутьСтр("ru = 'Первоначальное значение'"));
...
Функция КонтактнаяИнформацияНеКорректная()
	...
	Для Индекс = 0 По История.Количество() - 1 Цикл
		...
		Если ПустаяСтрока(СтрокаИстории.ЗначенияПолей) ИЛИ ПустаяСтрока(СтрокаИстории.Представление)Тогда
			ОбщегоНазначенияКлиентСервер.СообщитьПользователю(СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(ВернутьСтр("ru = 'Поле ""%1"" не может быть пустым.'"), ПредставлениеКонтактнойИнформации),, "История[" + Строка(Индекс) + "].Представление");
