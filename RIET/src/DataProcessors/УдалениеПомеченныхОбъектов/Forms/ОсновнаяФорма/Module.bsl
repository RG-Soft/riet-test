...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если НЕ Пользователи.ЭтоПолноправныйПользователь() Тогда
		ТекстОшибки = ВернутьСтр("ru = 'Недостаточно прав для выполнения операции.'");
		Возврат; // Отказ устанавливается в ПриОткрытии.
	КонецЕсли;
	
	Если ОбщегоНазначенияПовтИсп.РазделениеВключено()
		И Не ОбщегоНазначения.ИспользованиеРазделителяСеанса() Тогда
		ТекстОшибки = ВернутьСтр("ru = 'Для удаления помеченных необходимо войти в область данных.'");
		Возврат; // Отказ устанавливается в ПриОткрытии.
	КонецЕсли;
...
Процедура ПередЗакрытием(Отказ, СтандартнаяОбработка)
	Если Элементы.СтраницыФормы.ТекущаяСтраница = Элементы.СтраницаДлительнаяОперация Тогда
		...
		ТекстВопроса = ВернутьСтр("ru = 'Удаление помеченных еще выполняется.
		|Прервать?'");
		Кнопки = Новый СписокЗначений;
		Кнопки.Добавить(КодВозвратаДиалога.Прервать);
		Кнопки.Добавить(КодВозвратаДиалога.Пропустить, ВернутьСтр("ru = 'Не прерывать'"));
...
Процедура НеУдаленныеЗаменитьНа(Команда)
	...
	Если МассивСсылок.Количество() = 0 Тогда
		ПоказатьПредупреждение(, ВернутьСтр("ru = 'Выберите объекты'"));
		Возврат;
	КонецЕсли;
...
Процедура ФоновоеЗаданиеЗапуститьНаКлиенте(Действие)
	...
	Текст = ВернутьСтр("ru = 'Удаляются объекты, помеченные на удаление...'");
	Если ПараметрыЗадания.ИскатьПомеченные И ПараметрыЗадания.УдалятьПомеченные Тогда
		Состояние(ВернутьСтр("ru = 'Поиск и удаление помеченных объектов...'"));
	ИначеЕсли ПараметрыЗадания.ИскатьПомеченные Тогда
		Текст = ВернутьСтр("ru = 'Поиск помеченных на удаление объектов...'");
		Состояние(Текст);
	Иначе
		Состояние(ВернутьСтр("ru = 'Удаление выбранных объектов...'"));
	КонецЕсли;
	...
	ИначеЕсли Результат.ОшибкаПриУстановкеМонопольногоРежима Тогда
		...
		Иначе
			СтандартныеПодсистемыКлиентСервер.ВывестиПредупреждение(
				Результат,
				ВернутьСтр("ru = 'Не удалось запустить удаление помеченных объектов'"),
...
Процедура ПоказатьСвязиНеУдаленныхНаКлиенте()
	...
	Если СтрокаДерева = Неопределено Или СтрокаДерева.НомерКартинки < 1 Тогда
		...
		НеУдаленныеПодсказка = ВернутьСтр("ru = 'Выберите объект, чтобы узнать причину,
	...
	Иначе
		...
		Шаблон = ВернутьСтр("ru = 'Места использования объекта ""%1"" (%2):'");
...
Процедура ПометитьВыделенныеОбъектыТаблицыНаУдаление(ТаблицаЭлемент)
	...
	Если КоличествоМожноУдалить = 0 Тогда
		ТекстОшибки = ВернутьСтр("ru = 'Выберите объект.'");
		Если КоличествоВыделено = 1 Тогда
			Если ЕстьЗаписиРегистров Тогда
				ТекстОшибки = ВернутьСтр("ru = 'Удаление записи регистра выполняется из ее карточки.'");
			ИначеЕсли ЕстьКонстанты Тогда
				ТекстОшибки = ВернутьСтр("ru = 'Очистка значения константы выполняется из ее карточки.'");
			КонецЕсли;
		Иначе
			Если ЕстьЗаписиРегистров Или ЕстьКонстанты Тогда
				ТекстОшибки = ВернутьСтр("ru = 'Удаление записей регистров или очистка значений констант выполняется из их карточек.'");
			КонецЕсли;
		КонецЕсли;
		ПоказатьПредупреждение(, ТекстОшибки);
		Возврат;
	...
	Если КоличествоМожноУдалить = 1 Тогда
		Если УстанавливатьПометку Тогда
			ТекстВопроса = ВернутьСтр("ru = 'Пометить ""%1"" на удаление?'");
		Иначе
			ТекстВопроса = ВернутьСтр("ru = 'Снять с ""%1"" пометку на удаление?'");
		КонецЕсли;
		ТекстВопроса = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(ТекстВопроса, МассивСтрокТаблицы[0].Представление);
	Иначе
		Если УстанавливатьПометку Тогда
			ТекстВопроса = ВернутьСтр("ru = 'Пометить выделенные объекты (%1) на удаление?'");
		Иначе
			ТекстВопроса = ВернутьСтр("ru = 'Снять с выделенных объектов (%1) пометку на удаление?'");
		КонецЕсли;
...
Процедура ВидимостьДоступность(Форма)
	...
	Элементы.КнопкаДалее.Заголовок = ВернутьСтр("ru = 'Удалить'");
	Элементы.КнопкаЗакрыть.Заголовок = ВернутьСтр("ru = 'Закрыть'");
	Элементы.КнопкаНазад.Заголовок = ВернутьСтр("ru = '< В начало'");
	
	Если ТекущаяСтраница = Элементы.СтраницаВыборРежимаУдаления Тогда
		Элементы.КнопкаНазад.Видимость = Ложь;
		Элементы.КнопкаДалее.Видимость = Истина;
		Если Форма.РежимУдаления <> "Полный" Тогда
			Элементы.КнопкаДалее.Заголовок = ВернутьСтр("ru = 'Далее >'");
		КонецЕсли;
		Элементы.КнопкаДалее.КнопкаПоУмолчанию = Истина;
		Элементы.КнопкаЗакрыть.Заголовок = ВернутьСтр("ru = 'Отмена'");
	ИначеЕсли ТекущаяСтраница = Элементы.СтраницаПомеченныеНаУдаление Тогда
		Элементы.КнопкаНазад.Видимость = Истина;
		Элементы.КнопкаНазад.Заголовок = ВернутьСтр("ru = '< Назад'");
		Элементы.КнопкаДалее.Видимость = Истина;
		Элементы.КнопкаДалее.Заголовок = ВернутьСтр("ru = 'Удалить'");
		Элементы.КнопкаДалее.КнопкаПоУмолчанию = Истина;
	ИначеЕсли ТекущаяСтраница = Элементы.СтраницаДлительнаяОперация Тогда
		...
		Элементы.КнопкаЗакрыть.Заголовок = ВернутьСтр("ru = 'Прервать и закрыть'");
	ИначеЕсли ТекущаяСтраница = Элементы.СтраницаПричиныНевозможностиУдаления Тогда
		Элементы.КнопкаНазад.Видимость = Истина;
		Элементы.КнопкаДалее.Видимость = Истина;
		Элементы.КнопкаДалее.Заголовок = ВернутьСтр("ru = 'Повторить удаление'");
		Элементы.КнопкаДалее.КнопкаПоУмолчанию = Истина;
...
Функция ФоновоеЗаданиеЗапустить(Знач ПараметрыЗадания)
	...
	РезультатФоновогоЗадания = ДлительныеОперации.ЗапуститьВыполнениеВФоне(
		...
		ПараметрыЗадания,
		ВернутьСтр("ru = 'Удаление помеченных объектов (интерактивное)'"));
...
Функция ИзменитьПометкуУдаленияОбъектов(МассивСсылок, ПометкаУдаления)
	...
	Если КоличествоОбъектов = 0 Тогда
		ОповещениеЗаголовок = ВернутьСтр("ru = 'Объект не найден'");
	Иначе
		Если ПометкаУдаления Тогда
			ОповещениеЗаголовок = ВернутьСтр("ru = 'Пометка удаления установлена'");
		Иначе
			ОповещениеЗаголовок = ВернутьСтр("ru = 'Пометка удаления снята'");
		КонецЕсли;
...
Процедура ФоновоеЗаданиеПоказатьСтраницуДлительнойОперации(ПараметрыЗадания)
	...
	Если ПоказыватьБублик Тогда
		Элементы.НадписьДлительнаяОперация.Заголовок = ВернутьСтр("ru = 'Пожалуйста, подождите...'");
	...
	ИначеЕсли ПараметрыЗадания.УдалятьПомеченные Тогда
		Элементы.НадписьДлительнаяОперация.Заголовок = ВернутьСтр("ru = 'Удаляются объекты, помеченные на удаление...'");
		Элементы.ФоновоеЗаданиеСостояние.ГоризонтальноеПоложение = ГоризонтальноеПоложениеЭлемента.Центр;
	Иначе
		Элементы.НадписьДлительнаяОперация.Заголовок = ВернутьСтр("ru = 'Поиск объектов, помеченных на удаление...'");
		Элементы.ФоновоеЗаданиеСостояние.ГоризонтальноеПоложение = ГоризонтальноеПоложениеЭлемента.Центр;
	КонецЕсли;
КонецПроцедуры
	...
	Если РезультатВыполненияВФоне.УдалятьПомеченные Тогда
		...
		ИначеЕсли НеУдаленныеКоличество = 0 Тогда
			...
			ОповещениеТекст = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				ВернутьСтр("ru = 'Удаление помеченных объектов успешно завершено.
					|Удалено объектов: %1.'"),
		...
		Иначе
			...
			ОповещениеТекст = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				ВернутьСтр("ru = 'Удаление помеченных объектов завершено.
					|Удалено объектов: %1,
					|Не удалено: %2.'"),
				Формат(УдаленныеКоличество, "ЧН=0; ЧГ="),
			...
			Если УдаленныеКоличество = 0 Тогда
				Элементы.НадписьИтогиЧастичногоУдаления.Заголовок = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					ВернутьСтр("ru = 'Не получилось удалить объекты, помеченные на удаление (%1):'"),
					Формат(НеУдаленныеКоличество, "ЧН=0; ЧГ="));
			Иначе
				Элементы.НадписьИтогиЧастичногоУдаления.Заголовок = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					ВернутьСтр("ru = 'Успешно удалено: %1 из %2, остальные объекты не удалены (%3):'"),
					Формат(УдаленныеКоличество, "ЧН=0; ЧГ="),
		...
		Если ОповещениеТекст <> Неопределено Тогда
			СтандартныеПодсистемыКлиентСервер.ВывестиОповещение(
				Результат,
				ВернутьСтр("ru = 'Удаление помеченных'"),
				ОповещениеТекст,
				ОповещениеКартинка,
				НавигационнаяСсылка);
		КонецЕсли;
	...
	Для Каждого Причина Из ПрепятствующиеУдалению Цикл
		...
		Иначе
			...
			Если Причина.ОбнаруженныйСсылка = Неопределено Тогда // Константа
				...
				СтрокаПрепятствующегоУдалению.Представление = ОбнаруженныйИнформация.ПредставлениеЭлемента + " (" + ВернутьСтр("ru = 'Константа'") + ")";
			Иначе
	...
	Если ОбнаруженыРегистры И Не ОбнаруженыСсылочныеНеОтмеченные Тогда
		Если Не ПодписьДобавлена Тогда
			Элементы.НадписьВПодвале.Заголовок = ЗаголовокПодвала + " "
				+ ВернутьСтр("ru = 'Для устранения сложных зависимостей рекомендуется повторить удаление с включенным флажком ""Заблокировать всю работу в программе и ускорить удаление"" (нажмите кнопку ""< В начало"").'");
		КонецЕсли;
	Иначе
