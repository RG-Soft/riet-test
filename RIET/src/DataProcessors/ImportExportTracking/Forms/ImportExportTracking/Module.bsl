...
Процедура PoLineRestock(Команда)
	...
	Для Каждого Строка Из ApprovalPOs Цикл
		...
		Если Строка.Выбран Тогда
			Если Не ОбщегоНазначения.ПолучитьЗначениеРеквизита(Строка.PoLine, "Restock") Тогда
				ЕстьОшибка = ЕстьОшибка ИЛИ НЕ ИзменитьРесток(Строка.PoLine, Истина);
				ОповеститьОбИзменении(Строка.POLine);
			КонецЕсли;			
		КонецЕсли;
	...
	Если НЕ ЕстьОшибка Тогда
		ПоказатьПредупреждение(, "PO Lines were successfully updated");
	Иначе
		ПоказатьПредупреждение(, "There were errors during update. Check messages for details");
	КонецЕсли;
	
КонецПроцедуры

&НаСервереБезКонтекста
Функция ИзменитьРесток(PoLine, Restock)
	НачатьТранзакцию();
