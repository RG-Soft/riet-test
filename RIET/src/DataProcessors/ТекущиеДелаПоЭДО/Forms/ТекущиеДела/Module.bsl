...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если НЕ ЭлектронныеДокументыСлужебный.НемедленнаяОтправкаЭД() Тогда
		Элементы.КомандаПодписать.Заголовок = ВернутьСтр("ru = 'Подписать'");
		Элементы.СформироватьПодписатьИОтправить.Заголовок = ВернутьСтр("ru = 'Утвердить и подписать'");
		Команды.УтвердитьПодписатьИОтправить.Подсказка = ВернутьСтр("ru = 'Утвердить, подписать и подготовить к отправке
																	|выделенные электронные документы'");
		Если НЕ ИспользоватьЭП Тогда
			Элементы.СформироватьПодписатьИОтправить.Заголовок = ВернутьСтр("ru = 'Утвердить'");
			Команды.УтвердитьПодписатьИОтправить.Подсказка = ВернутьСтр("ru = 'Утвердить и подготовить к отправке
																		|выделенные электронные документы'");
		КонецЕсли;
	ИначеЕсли НЕ ИспользоватьЭП Тогда
		Элементы.СформироватьПодписатьИОтправить.Заголовок = ВернутьСтр("ru = 'Утвердить и отправить'");
		Команды.УтвердитьПодписатьИОтправить.Подсказка = ВернутьСтр("ru = 'Утвердить и отправить
																	|выделенные электронные документы'");
	КонецЕсли;
	...
	ИмяСчетаФактурыПолученногоАванс = ЭлектронныеДокументыПовтИсп.ИмяНаличиеОбъектаРеквизитаВПрикладномРешении(
		"СчетФактураПолученныйАвансВМетаданных");
	РегистрацияЦенНоменклатурыПоставщика = ЭлектронныеДокументыПовтИсп.ИмяНаличиеОбъектаРеквизитаВПрикладномРешении(
	...
	ИмяСчетаФактурыВыданногоАванс = ЭлектронныеДокументыПовтИсп.ИмяНаличиеОбъектаРеквизитаВПрикладномРешении(
		"СчетФактураВыданныйАвансВМетаданных");
	ИмяКоммерческогоПредложения = ЭлектронныеДокументыПовтИсп.ИмяНаличиеОбъектаРеквизитаВПрикладномРешении(
...
Процедура УтвердитьЭД(Команда)
	...
	ЭлектронныеДокументыСлужебныйКлиент.ОбработатьЭД(Неопределено, "УтвердитьОтправить", , МассивСсылок);
...
Процедура ОтправитьПакеты(Команда)
	...
	ЭлектронныеДокументыСлужебныйКлиент.ПолучитьCоответствиеСоглашенийИПараметровСертификатов(ОбработчикОповещения);
...
Процедура ОтправитьПриглашения(Команда)
	...
	ЭлектронныеДокументыСлужебныйКлиент.ПолучитьCоответствиеСоглашенийИПараметровСертификатов(ОбработкаОповещения, МассивПрофилейНастроекЭДО);
...
Функция ДанныеДереваДействий(Знач ДеревоДействий, Запросы)
	...
	ПредставлениеСтроки = ВернутьСтр("ru = 'Сформировать'");
	УстановитьПредставлениеСтрокДереваРекурсивно(Дерево, "Сформировать", ПредставлениеСтроки, ДанныеОКоличествах[0]);
	
	УстановитьПредставлениеСтрокДереваРекурсивно(Дерево, "Обработать", ВернутьСтр("ru = 'Обработать'"), ДанныеОКоличествах[1]);
	
	УстановитьПредставлениеСтрокДереваРекурсивно(Дерево, "Утвердить", ВернутьСтр("ru = 'Утвердить'"), ДанныеОКоличествах[2]);

	УстановитьПредставлениеСтрокДереваРекурсивно(Дерево, "Подписать", ВернутьСтр("ru = 'Подписать'"), ДанныеОКоличествах[3]);
	
	УстановитьПредставлениеСтрокДереваРекурсивно(Дерево, "Исправить", ВернутьСтр("ru = 'Исправить'"), ДанныеОКоличествах[4]);
	
	УстановитьПредставлениеСтрокДереваРекурсивно(Дерево, "Аннулировать", ВернутьСтр("ru = 'Аннулировать'"), ДанныеОКоличествах[5]);

	УстановитьПредставлениеСтрокДереваРекурсивно(Дерево, "НаКонтроле", ВернутьСтр("ru = 'На контроле'"), ДанныеОКоличествах[6]);
	
	УстановитьПредставлениеСтрокДереваРекурсивно(Дерево, "Отправить", ВернутьСтр("ru = 'Отправить'"), ДанныеОКоличествах[7]);

	ПредставлениеСтроки = ВернутьСтр("ru = 'Распаковать'");
	УстановитьПредставлениеСтрокДереваРекурсивно(Дерево, "Распаковать", ПредставлениеСтроки, ДанныеОКоличествах[8]);

	УстановитьПредставлениеСтрокДереваРекурсивно(Дерево, "Разобрать", ВернутьСтр("ru = 'Разобрать'"), ДанныеОКоличествах[9]);
	
	УстановитьПредставлениеСтрокДереваРекурсивно(Дерево, "Приглашения", ВернутьСтр("ru = 'Приглашения'"), ДанныеОКоличествах[10]);
	УстановитьПредставлениеСтрокДереваРекурсивно(Дерево, "ТребуетсяПригласить", ВернутьСтр("ru = 'Требуется пригласить'"), ДанныеОКоличествах[11]);
	УстановитьПредставлениеСтрокДереваРекурсивно(Дерево, "ЖдемСогласия", ВернутьСтр("ru = 'Ждем согласия'"), ДанныеОКоличествах[12]);
	УстановитьПредставлениеСтрокДереваРекурсивно(Дерево, "ТребуетсяСогласие", ВернутьСтр("ru = 'Требуется согласие'"), ДанныеОКоличествах[13]);
...
Процедура УстановитьСтатусПакетов(Знач МассивЭД, Знач СтатусПакета, КоличествоИзмененных)
	...
	Для Каждого СтрокаТаблицы Из МассивЭД Цикл
		...
		Исключение
			...
			ТекстОперации  = ВернутьСтр("ru = 'изменение статуса пакетов ЭД'");
...
Процедура СформироватьТаблицуБыстрогоОтбора()
	...
	НоваяСтрока.ПредставлениеПараметра = ВернутьСтр("ru = 'Организация:'");
	...
	НоваяСтрока.ПредставлениеПараметра = ВернутьСтр("ru = 'Контрагент:'");
	...
	НоваяСтрока.ПредставлениеПараметра = ВернутьСтр("ru = 'Дата с:'");
	НоваяСтрока.Тип = "Дата";
	НоваяСтрока.Значение = Дата(1,1,1);
	
	НоваяСтрока = БыстрыеОтборы.Добавить();
	НоваяСтрока.Параметр = "Дата_По";
	НоваяСтрока.ПредставлениеПараметра = ВернутьСтр("ru = 'Дата по:'");
	НоваяСтрока.Тип = "Дата";
	НоваяСтрока.Значение = Дата(1,1,1);
	
	НоваяСтрока = БыстрыеОтборы.Добавить();
	НоваяСтрока.Параметр = "Сумма_С";
	НоваяСтрока.ПредставлениеПараметра = ВернутьСтр("ru = 'Сумма с:'");
	НоваяСтрока.Тип = "Число";
	НоваяСтрока.Значение = Неопределено;
	
	НоваяСтрока = БыстрыеОтборы.Добавить();
	НоваяСтрока.Параметр = "Сумма_По";
	НоваяСтрока.ПредставлениеПараметра = ВернутьСтр("ru = 'Сумма по:'");
	НоваяСтрока.Тип = "Число";
	НоваяСтрока.Значение = Неопределено;
	
	НоваяСтрока = БыстрыеОтборы.Добавить();
	НоваяСтрока.Параметр = "ВидЭД";
	НоваяСтрока.ПредставлениеПараметра = ВернутьСтр("ru = 'Вид документа:'");
	...
	НоваяСтрока.ПредставлениеПараметра = ВернутьСтр("ru = 'Направление:'");
	...
	НоваяСтрока.ПредставлениеПараметра = ВернутьСтр("ru = 'Доп. информация:'");
	НоваяСтрока.Тип = "Строка";
	НоваяСтрока.Значение = "";
	
	НоваяСтрока = БыстрыеОтборы.Добавить();
	НоваяСтрока.Параметр = "Ответственный";
	НоваяСтрока.ПредставлениеПараметра = ВернутьСтр("ru = 'Ответственный:'");
...
Процедура УстановитьСтатусОтменен(МассивЭД)
	...
	УстановитьСтатусПакетов(МассивЭД,
							...
							Количество);
	ТекстОповещения = ВернутьСтр("ru = 'Изменен статус пакетов на ""Отменен"": (%1)'");
	ТекстОповещения = СтрЗаменить(ТекстОповещения, "%1", Количество);
	ПоказатьОповещениеПользователя(ВернутьСтр("ru = 'Обмен электронными документами'"), , ТекстОповещения);
	ОбновитьДанныеДереваНаКлиенте();
	ОбновитьДинамическиеСписки();
...
Процедура УстановитьОтветственного(МассивЭД)
	...
	ЭлектронныеДокументыСлужебныйКлиент.ИзменитьОтветственного(МассивЭД, ОбработкаОповещения);
...
Функция СписокАктуальныхВидовЭД()
	...
	МассивАктуальныхЭД = ОбщегоНазначенияКлиентСервер.СократитьМассив(МассивАктуальныхЭД, БанковскиеВидыЭД);
...
Процедура ОповеститьОРезультатах(КоличествоПодписанных, КолПодготовленных, КолОтправленных)
	...
	ТекстСостояния = ВернутьСтр("ru = 'Произвольные ЭД:
		| подписано: (%1)'");
	Количество = 0;
	...
	//ОповеститьОРезультатах(КоличествоПодписанных, КолПодготовленных, КолОтправленных);
...
Процедура ОтправитьПриглашенияСервер(ОтправленоПриглашений, СоответствиеПрофилейНастроекЭДОМаркерам)
	...
	Для каждого НастройкаЭДО Из Элементы.ТребуетсяПригласить.ВыделенныеСтроки Цикл
		...
		Если Не ЗначениеЗаполнено(СтруктураПараметровНастройкиЭДО.ЭлектроннаяПочтаДляПриглашения) Тогда
			ШаблонСообщения = ВернутьСтр("ru = 'Для отправки приглашения к обмену ЭД для получателя %1
...
Процедура ОтклонитьПриглашенияОповещение(Результат, ДополнительныеПараметры) Экспорт
	...
	Если Результат.Свойство("СоответствиеПрофилейИПараметровСертификатов", СоответствиеПрофилейИПараметровСертификатов)
		...
		ТекстЗаголовка = ВернутьСтр("ru = 'Отклоняются приглашения'");
		...
		ШаблонСообщения = ВернутьСтр("ru = 'Отклонено приглашений: %1'");
		...
		ОбновитьДинамическиеСписки();
	Иначе
		ТекстОшибки = ВернутьСтр("ru = 'При отклонении приглашений возникли ошибки.
...
Процедура ПринятьПриглашенияОповещение(Результат, ДополнительныеПараметры) Экспорт
	...
	Если Результат.Свойство("СоответствиеПрофилейИПараметровСертификатов", СоответствиеПрофилейИПараметровСертификатов)
		...
		ТекстЗаголовка = ВернутьСтр("ru = 'Принимаются приглашения'");
		...
		ШаблонСообщения = ВернутьСтр("ru = 'Принято приглашений: %1'");
		...
		ОбновитьДинамическиеСписки();
	Иначе
		ТекстОшибки = ВернутьСтр("ru = 'При принятии приглашений возникли ошибки.
			|Необходимо выполнить тест настроек ЭДО с указанными контрагентами.'");
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(ТекстОшибки);
	КонецЕсли;
...
Процедура ОтправитьПриглашениеОповещение(Результат, ДополнительныеПараметры) Экспорт
	...
	Если Результат.Свойство("СоответствиеПрофилейИПараметровСертификатов", СоответствиеПрофилейИПараметровСертификатов)
		...
		ТекстЗаголовка = ВернутьСтр("ru = 'Отправка приглашений получателям'");
		
		ОтправленоПриглашений = 0;
		ОтправитьПриглашенияСервер(ОтправленоПриглашений, СоответствиеПрофилейИПараметровСертификатов);
		
		ШаблонСообщения = ВернутьСтр("ru = 'Отправлено приглашений: %1'");
		...
		ОбновитьДинамическиеСписки();
	Иначе
		ТекстОшибки = ВернутьСтр("ru = 'При отправке приглашений возникли ошибки.
			|Необходимо выполнить тест настроек ЭДО с указанными контрагентами.'");
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(ТекстОшибки);
	КонецЕсли;
...
Процедура ОтправитьПакетыОповещение(Результат, ДополнительныеПараметры) Экспорт
	...
	ШаблонОповещения = ВернутьСтр("ru = 'Отправлено пакетов: (%1).'");
	ТекстОповещения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(ШаблонОповещения, КоличествоОтправленных);
	
	Оповестить("ОбновитьСостояниеЭД");
		
	ЗаголовокОповещения = ВернутьСтр("ru = 'Отправка электронных документов'");
