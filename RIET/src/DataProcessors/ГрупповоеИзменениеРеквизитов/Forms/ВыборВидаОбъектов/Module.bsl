...
Процедура ЗаполнитьСписокТиповОбъектов()
		...
		Для Каждого ОбъектМетаданных Из КоллекцияОбъектовМетаданных Цикл
			...
			Если ПравоДоступа("Изменение", ОбъектМетаданных) Тогда
				Если СтрНачинаетсяС(НРег(ОбъектМетаданных.Имя),ПрефиксУдаляемыхОбъектов) Тогда
					УдаляемыеОбъекты.Добавить(ОбъектМетаданных.ПолноеИмя(), ОбъектМетаданных.Синоним);
				Иначе 
	...
	Для Каждого Элемент Из УдаляемыеОбъекты Цикл
		ДоступныеОбъектыДляИзменения.Добавить(Элемент.Значение, Элемент.Представление);
	КонецЦикла;
...
Процедура ОбновитьКоличествоВыбранных()
	ТекстКнопкиВыбора = ВернутьСтр("ru = 'Выбрать'");
