...
Процедура ПередЗакрытием(Отказ, СтандартнаяОбработка)
	...
	Если Модифицированность И ЗначениеЗаполнено(ИсходнаяФормула) И ИсходнаяФормула <> Формула Тогда
		...
		ПоказатьВопрос(Новый ОписаниеОповещения("ПередЗакрытиемЗавершение", ЭтотОбъект), ВернутьСтр("ru='Данные были изменены. Сохранить изменения?'"), РежимДиалогаВопрос.ДаНетОтмена);
	КонецЕсли;
...
Процедура ОперандыВыбор(Элемент, ВыбраннаяСтрока, Поле, СтандартнаяОбработка)
	...
	Если Элемент.ТекущиеДанные.ПометкаУдаления Тогда
		...
		ПоказатьВопрос(
			...
			ВернутьСтр("ru = 'Выбранный элемент помечен на удаление. 
				|Продолжить?'"), 
...
Процедура ОперандыОкончаниеПеретаскивания(Элемент, ПараметрыПеретаскивания, СтандартнаяОбработка)
	...
	Если Элемент.ТекущиеДанные.ПометкаУдаления Тогда
		Ответ = Неопределено;
		ПоказатьВопрос(Новый ОписаниеОповещения("ОперандыОкончаниеПеретаскиванияЗавершение", ЭтотОбъект), ВернутьСтр("ru = 'Выбранный элемент помечен на удаление'") + Символы.ПС + ВернутьСтр("ru = 'Продолжить?'"), РежимДиалогаВопрос.ДаНет);
	КонецЕсли;
...
Функция ПолучитьСтандартноеДеревоОператоров()
	...
	ГруппаОператоров = ДобавитьГруппуОператоров(Дерево, ВернутьСтр("ru='Разделители'"));
	...
	ДобавитьОператор(Дерево, ГруппаОператоров, ",", " + "", "" + ");
	ДобавитьОператор(Дерево, ГруппаОператоров, ".", " + "". "" + ");
	ДобавитьОператор(Дерево, ГруппаОператоров, ВернутьСтр("ru='Пробел'"), " + "" "" + ");
	ДобавитьОператор(Дерево, ГруппаОператоров, "(", " + "" ("" + ");
	ДобавитьОператор(Дерево, ГруппаОператоров, ")", " + "") "" + ");
	ДобавитьОператор(Дерево, ГруппаОператоров, """", " + """""""" + ");
	
	ГруппаОператоров = ДобавитьГруппуОператоров(Дерево, ВернутьСтр("ru='Операторы'"));
	...
	ГруппаОператоров = ДобавитьГруппуОператоров(Дерево, ВернутьСтр("ru='Логические операторы и константы'"));
	ДобавитьОператор(Дерево, ГруппаОператоров, "<", " < ");
	ДобавитьОператор(Дерево, ГруппаОператоров, ">", " > ");
	ДобавитьОператор(Дерево, ГруппаОператоров, "<=", " <= ");
	ДобавитьОператор(Дерево, ГруппаОператоров, ">=", " >= ");
	ДобавитьОператор(Дерево, ГруппаОператоров, "=", " = ");
	ДобавитьОператор(Дерево, ГруппаОператоров, "<>", " <> ");
	ДобавитьОператор(Дерево, ГруппаОператоров, ВернутьСтр("ru='И'"),      " " + ВернутьСтр("ru='И'")      + " ");
	ДобавитьОператор(Дерево, ГруппаОператоров, ВернутьСтр("ru='Или'"),    " " + ВернутьСтр("ru='Или'")    + " ");
	ДобавитьОператор(Дерево, ГруппаОператоров, ВернутьСтр("ru='Не'"),     " " + ВернутьСтр("ru='Не'")     + " ");
	ДобавитьОператор(Дерево, ГруппаОператоров, ВернутьСтр("ru='ИСТИНА'"), " " + ВернутьСтр("ru='ИСТИНА'") + " ");
	ДобавитьОператор(Дерево, ГруппаОператоров, ВернутьСтр("ru='ЛОЖЬ'"),   " " + ВернутьСтр("ru='ЛОЖЬ'")   + " ");
	
	ГруппаОператоров = ДобавитьГруппуОператоров(Дерево, ВернутьСтр("ru='Числовые функции'"));
	
	ДобавитьОператор(Дерево, ГруппаОператоров, ВернутьСтр("ru='Максимум'"),    ВернутьСтр("ru='Макс(,)'"), 2);
	ДобавитьОператор(Дерево, ГруппаОператоров, ВернутьСтр("ru='Минимум'"),     ВернутьСтр("ru='Мин(,)'"),  2);
	ДобавитьОператор(Дерево, ГруппаОператоров, ВернутьСтр("ru='Округление'"),  ВернутьСтр("ru='Окр(,)'"),  2);
	ДобавитьОператор(Дерево, ГруппаОператоров, ВернутьСтр("ru='Целая часть'"), ВернутьСтр("ru='Цел()'"),   1);
	
	ГруппаОператоров = ДобавитьГруппуОператоров(Дерево, ВернутьСтр("ru='Строковые функции'"));
	
	ДобавитьОператор(Дерево, ГруппаОператоров, ВернутьСтр("ru='Строка'"), ВернутьСтр("ru='Строка()'"));
	ДобавитьОператор(Дерево, ГруппаОператоров, ВернутьСтр("ru='ВРег'"), ВернутьСтр("ru='ВРег()'"));
	ДобавитьОператор(Дерево, ГруппаОператоров, ВернутьСтр("ru='Лев'"), ВернутьСтр("ru='Лев()'"));
	ДобавитьОператор(Дерево, ГруппаОператоров, ВернутьСтр("ru='НРег'"), ВернутьСтр("ru='НРег()'"));
	ДобавитьОператор(Дерево, ГруппаОператоров, ВернутьСтр("ru='Прав'"), ВернутьСтр("ru='Прав()'"));
	ДобавитьОператор(Дерево, ГруппаОператоров, ВернутьСтр("ru='СокрЛ'"), ВернутьСтр("ru='СокрЛ()'"));
	ДобавитьОператор(Дерево, ГруппаОператоров, ВернутьСтр("ru='СокрЛП'"), ВернутьСтр("ru='СокрЛП()'"));
	ДобавитьОператор(Дерево, ГруппаОператоров, ВернутьСтр("ru='СокрП'"), ВернутьСтр("ru='СокрП()'"));
	ДобавитьОператор(Дерево, ГруппаОператоров, ВернутьСтр("ru='ТРег'"), ВернутьСтр("ru='ТРег()'"));
	ДобавитьОператор(Дерево, ГруппаОператоров, ВернутьСтр("ru='СтрЗаменить'"), ВернутьСтр("ru='СтрЗаменить(,,)'"));
	ДобавитьОператор(Дерево, ГруппаОператоров, ВернутьСтр("ru='СтрДлина'"), ВернутьСтр("ru='СтрДлина()'"));
	
	ГруппаОператоров = ДобавитьГруппуОператоров(Дерево, ВернутьСтр("ru='Прочие функции'"));
	
	ДобавитьОператор(Дерево, ГруппаОператоров, ВернутьСтр("ru='Условие'"), "?(,,)", 3);
	ДобавитьОператор(Дерево, ГруппаОператоров, ВернутьСтр("ru='Предопределенное значение'"), ВернутьСтр("ru='ПредопределенноеЗначение()'"));
	ДобавитьОператор(Дерево, ГруппаОператоров, ВернутьСтр("ru='Значение заполнено'"), ВернутьСтр("ru='ЗначениеЗаполнено()'"));
	ДобавитьОператор(Дерево, ГруппаОператоров, ВернутьСтр("ru='Формат'"), ВернутьСтр("ru='Формат(,)'"));
...
Функция ПроверитьФормулу(Формула, Операнды, Знач Поле = "", СтроковаяФормула = Ложь)
	...
	Если ЗначениеЗаполнено(Формула) Тогда
			...
			Если СтроковаяФормула Тогда
				...
				Если ОтсутствиеРазделителей > 0 Тогда
					СообщитьПользователю(
						ВернутьСтр("ru='В формуле обнаружены ошибки. Между операндами должен присутствовать оператор или разделитель'"),
						Поле,
						ПутьКДанным);
					Результат = Ложь;
				КонецЕсли;
		...
		Исключение
			Результат = Ложь;
			СообщитьПользователю(
				ВернутьСтр("ru='В формуле обнаружены ошибки. Проверьте формулу. Формулы должны составляться по правилам написания выражений на встроенном языке 1С:Предприятия.'"),
				Поле,
				ПутьКДанным);
		КонецПопытки;
	...
	Если ЗначениеЗаполнено(Формула) Тогда
		Если ПроверитьФормулу(Формула, Операнды, Поле, СтроковаяФормула) Тогда
			ПоказатьОповещениеПользователя(
				ВернутьСтр("ru = 'В формуле ошибок не обнаружено'"),
				,
				,
