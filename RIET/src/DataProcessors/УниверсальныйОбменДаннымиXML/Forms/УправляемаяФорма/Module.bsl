...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если Не ПравоДоступа("Администрирование", Метаданные) Тогда
		ВызватьИсключение ВернутьСтр("ru = 'Использование обработки в интерактивном режиме доступно только администратору.'");
	КонецЕсли;
	...
	ЗаголовокФормы = ВернутьСтр("ru = 'Универсальный обмен данными в формате XML (%ВерсияОбработки%)'");
...
Процедура ПриОткрытии(Отказ)
	...
	Если СохраненныйРежимЗагрузки
		...
		Если Объект.НастройкаАвтоматическойЗагрузкиДанных = 1 Тогда
			...
			ПоказатьВопрос(ОписаниеОповещения, ВернутьСтр("ru = 'Выполнить загрузку данных из файла обмена?'"), РежимДиалогаВопрос.ДаНет, , КодВозвратаДиалога.Да);
...
Процедура КаталогИнформационнойБазыДляПодключенияНачалоВыбора(Элемент, ДанныеВыбора, СтандартнаяОбработка)
	...
	ДиалогВыбораФайла.Заголовок = ВернутьСтр("ru = 'Выберите каталог информационной базы'");
...
Процедура ИмяФайлаПравилПриИзменении(Элемент)
	...
	Если ПустаяСтрока(ИмяФайлаПравил) Или Не Файл.Существует() Тогда
		СообщитьПользователю(ВернутьСтр("ru = 'Не найден файл правил обмена'"), "ИмяФайлаПравил");
	...
	ПоказатьВопрос(ОписаниеОповещения, ВернутьСтр("ru = 'Загрузить правила обмена данными?'"), РежимДиалогаВопрос.ДаНет, , КодВозвратаДиалога.Да);
...
Процедура ПолучитьИнформациюОФайлеОбмена(Команда)
	...
	Если ЭтоКлиент Тогда
		...
		НачатьПомещениеФайла(ОписаниеОповещения, АдресФайла,ВернутьСтр("ru = 'Файл обмена'"),, УникальныйИдентификатор);
...
Процедура ПолучитьИнформациюОФайлеОбменаЗавершение(Результат, Адрес, ВыбранноеИмяФайла, ДополнительныеПараметры) Экспорт
	...
	Если Результат Тогда
		...
		Исключение
			...
			СообщитьПользователю(ВернутьСтр("ru = 'Не удалось прочитать файл обмена.'"));
			ОчиститьДанныеОФайлеДляЗагрузкиДанных();
...
Процедура УдалениеУдалить(Команда)
	...
	ПоказатьВопрос(ОписаниеОповещения, ВернутьСтр("ru = 'Удалить выбранные данные в информационной базе?'"), РежимДиалогаВопрос.ДаНет, , КодВозвратаДиалога.Нет);
...
Процедура УдалениеУдалитьЗавершение(Результат, ДополнительныеПараметры) Экспорт
	...
	Если Результат = КодВозвратаДиалога.Да Тогда
		...
		Состояние(ВернутьСтр("ru = 'Выполняется удаление данных. Пожалуйста, подождите...'"));
		УдалитьНаСервере();
...
Процедура НастройкаОтладкиЗагрузки(Команда)
	...
	Если ЭтоКлиент Тогда
		...
		НачатьПомещениеФайла(ОписаниеОповещения, АдресФайлаОбменаВХранилище,ВернутьСтр("ru = 'Файл обмена'"),, УникальныйИдентификатор);
...
Процедура ПрочитатьПравилаОбмена(Команда)
	...
	Если ЭтоLinuxКлиент() И ПрямаяВыгрузка = 1 Тогда
		ПоказатьПредупреждение(,ВернутьСтр("ru = 'Прямое подключение к информационной базе не поддерживается в клиенте под управлением ОС Linux.'"));
		Возврат;
	КонецЕсли;
	...
	Если ЭтоКлиент Тогда
		...
		НачатьПомещениеФайла(ОписаниеОповещения, АдресФайлаПравилВХранилище,ВернутьСтр("ru = 'Файл правил обмена'"),, УникальныйИдентификатор);
...
Процедура ПрочитатьПравилаОбменаЗавершение(Результат, Адрес, ВыбранноеИмяФайла, ДополнительныеПараметры) Экспорт
	...
	Если Результат Тогда
		...
		Состояние(ВернутьСтр("ru = 'Выполняется чтение правил обмена. Пожалуйста, подождите...'"));
...
Функция ИменаФайловПравилИОбменаСовпадают()
	...
	Если ВРег(СокрЛП(ИмяФайлаПравил)) = ВРег(СокрЛП(ИмяФайлаДанных)) Тогда
		...
		СообщитьПользователю(ВернутьСтр("ru = 'Файл правил обмена не может совпадать с файлом данных.
...
Процедура ЗаполнитьСписокТиповДоступныхДляУдаления()
	...
	СтрокаДерева.Представление = ВернутьСтр("ru = 'Справочники'");
	...
	СтрокаДерева = ДеревоДанных.Строки.Добавить();
	СтрокаДерева.Представление = ВернутьСтр("ru = 'Планы видов характеристик'");
	...
	СтрокаДерева = ДеревоДанных.Строки.Добавить();
	СтрокаДерева.Представление = ВернутьСтр("ru = 'Документы'");
...
Процедура ВыборФайла(Элемент, ИмяСвойства, ПроверятьСуществование, Знач РасширениеПоУмолчанию = "xml",
	...
	ДиалогВыбораФайла.Заголовок = ВернутьСтр("ru = 'Выберите файл'");
...
Функция ВыполнитьПодключениеКИБПриемникуНаСервере()
	...
	Если РезультатПодключения <> Неопределено Тогда
		...
		СообщитьПользователю(ВернутьСтр("ru = 'Подключение успешно установлено.'"));
...
Процедура ОткрытьФайлЗагрузкиНаСервере(АдресФайла)
	...
	Иначе
		...
		Если Не ФайлНаСервере.Существует() Тогда
			...
			СообщитьПользователю(ВернутьСтр("ru = 'Не найден файл обмена на сервере.'"), "ИмяФайлаОбмена");
			Возврат;
...
Процедура ВыполнитьЗагрузкуИзФормыЗавершение(Результат, Адрес, ВыбранноеИмяФайла, ДополнительныеПараметры) Экспорт
	...
	Если Результат Тогда
		...
		Состояние(ВернутьСтр("ru = 'Выполняется загрузка данных. Пожалуйста, подождите...'"));
...
Процедура ВыполнитьЗагрузкуНаСервере(АдресФайла, ИмяФайлаДляРасширения)
	...
	Исключение
		ЗаписьЖурналаРегистрации(ВернутьСтр("ru = 'Универсальный обмен данными в формате XML'", ОбъектДляСервера.КодОсновногоЯзыка()),
...
Функция ИмяФайлаНаСервереИлиКлиенте(ИмяРеквизита ,Знач АдресФайла, Знач ИмяФайлаДляРасширения = ".xml",
	...
	Иначе
		...
		Если Не ФайлНаСервере.Существует() И ПроверятьСуществование Тогда
			...
			СообщитьПользователю(ВернутьСтр("ru = 'Указанный файл не существует.'"));
...
Процедура ВыполнитьВыгрузкуИзФормы()
	...
	Состояние(ВернутьСтр("ru = 'Выполняется выгрузка данных. Пожалуйста, подождите...'"));
	...
	Если ЭтоКлиент И Не ПрямаяВыгрузка И Не Объект.ФлагОшибки Тогда
		...
		ИмяСохраняемогоФайла = ?(Объект.АрхивироватьФайл, ВернутьСтр("ru = 'Файл выгрузки.zip'"),ВернутьСтр("ru = 'Файл выгрузки.xml'"));
...
Функция ВыполнитьВыгрузкуНаСервере()
	...
	Если Не ПрямаяВыгрузка Тогда
		...
		Если ИмяВременногоФайлаДанных = Неопределено Тогда
			...
			СообщитьПользователю(ВернутьСтр("ru = 'Не определен файл данных'"));
	...
	Если ОбъектДляСервера.ФлагРежимОтладкиОбработчиков Тогда
		...
		Если Не Файл.Существует() Тогда
			...
			СообщитьПользователю(ВернутьСтр("ru = 'Файл внешней обработки отладчиков событий не существует на сервере'"));
			Возврат Неопределено;
		...
		Если Отказ Тогда
			...
			СообщитьПользователю(ВернутьСтр("ru = 'Не удалось выгрузить обработчики событий'"));
			Возврат "";
...
Процедура ИзменитьРежимОбработки(РежимРаботы)
	...
	?(РежимРаботы, ВернутьСтр("ru = 'Режим работы (на клиенте)'"), ВернутьСтр("ru = 'Режим работы (на сервере)'"));
...
Функция ПустоеЗначениеРеквизита(Реквизит, ПутьКДанным, Заголовок)
	...
	Если ПустаяСтрока(Реквизит) Тогда
		...
		ТекстСообщения = ВернутьСтр("ru = 'Поле ""%1"" не заполнено'");
