...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	КонецЕсли;	  
	Заголовок = ВернутьСтр("ru = 'Выбрать запрос'");	
КонецПроцедуры
...
Процедура СравнитьРезультатыЗапросов(Команда)
	...
	Если ВыделенныеЗапросы.Количество() <> 2 Тогда
		Предупреждение(ВернутьСтр("ru = 'Для сравнения необходимо выбрать только 2 запроса", "Объект'"));
		Возврат;
	Иначе
	...
	КонецЕсли;	
	#Иначе
	 	Предупреждение(ВернутьСтр("ru = 'Сравнивать результаты можно только в режиме толстого клиента.", "Объект'"));
	#КонецЕсли
КонецПроцедуры
...
//
&НаСервере
Процедура СохранитьЗапросы(ИмяФайла, знач Объект)
...
&НаКлиенте
Процедура ОбработкаСохраненияФайла(ИмяФайла)
	...
	Если РасширениеПодключено Тогда 
		...
		Если Не ПустаяСтрока(ИмяФайла) Тогда 
			СохранитьЗапросы(ИмяФайла, Объект);
			ВладелецФормы.Модифицированность = Ложь;
		КонецЕсли;
	Иначе 
	    ТекстСообщения = "ru = 'В данном браузере невозможно работать с файлами.'";
...
Процедура ЗагрузитьЗапросыИзФайла(ИмяФайла)
	ОбъектОбработки = ОбъектОбработки().ПрочитатьЗапросыИзФайлаXML(ИмяФайла);
	ЗаполнитьЗапросыИПармаетрыИзОбъектаВнешнейОбработки(ОбъектОбработки);
КонецПроцедуры	
...
//
&НаСервере
Процедура ЗаполнитьЗапросыИПармаетрыИзОбъектаВнешнейОбработки(ОбъектОбработки)
