...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	ИмяЗапросаПоУмолчанию 			= ВернутьСтр("ru = 'Новый запрос'");
	...
	ЗаголовокФормы 		= ВернутьСтр("ru = 'Консоль запросов (%ИмяЗапросаПоУмолчанию%)'");
...
Процедура ПередЗакрытием(Отказ, СтандартнаяОбработка)
	...
	Текст = ВернутьСтр("ru = 'Данные изменены. Сохранить изменения?'");
...
Процедура ТипВФормеНачалоВыбора(Элемент, ДанныеВыбора, СтандартнаяОбработка)
	...
	ЗаголовокЭлемента = ВернутьСтр("ru = 'Выбрать тип'");
...
Процедура ПараметрыПередНачаломДобавления(Элемент, Отказ, Копирование, Родитель, Группа)
	...
	Если Не Копирование Тогда 
		...
		Если ИндексТекущегоЗапроса = Неопределено тогда
			ТекстСообщения = ВернутьСтр("ru = 'Выберите запрос.'");
...
Процедура ОткрытьКонструкторЗапроса(Команда)
	#Если ТолстыйКлиентУправляемоеПриложение ИЛИ ТолстыйКлиентОбычноеПриложение Тогда
		...
		КонецЕсли;
	#Иначе
		ТекстСообщения = ВернутьСтр("ru = 'Конструктор запроса доступен только в режиме Толстого клиента.'");
...
Процедура СкопироватьЗапрос(Команда)
	ИндексТекущегоЗапроса = ИндексТекущегоВопроса();
	Если ИндексТекущегоЗапроса = Неопределено тогда
		ПоказатьСообщениеПользователю(ВернутьСтр("ru = 'Выберите запрос.'"), "Объект");
		Возврат;
	КонецЕсли;
...
Процедура ВывестиРезультатЗапроса()
	...
	Для каждого СтрЗапросы из Объект.Запросы Цикл 
		Если СтрЗапросы.Идентификатор = ИдентификаторТекущегоЗапроса Тогда 
			...
			РезультСтрока	= ВернутьСтр("ru = 'Результат запроса (количество строк = %КолСтрок%, время выполнения = %ВремяВыполнения% с)'");
...
Процедура ОбновитьФормуКлиент() 
	...
	Для каждого СтрЗапросы из Объект.Запросы Цикл 
		Если СтрЗапросы.Идентификатор = ИдентификаторТекущегоЗапроса тогда
			...
			ЗаголовокФормы = ВернутьСтр("ru = 'Консоль запросов (%ИмяЗапроса%)'");
	...
	КоличествоЗапросов = Объект.Запросы.Количество();
	ЗаголовокКнопкиВыбораЗапроса = ВернутьСтр("ru = 'Запросы'");
...
Функция ОтветПользователяНаУдалениеПараметровЗапроса()
	...
	Текст = ВернутьСтр("ru = 'Таблица параметров не пуста. Очистить таблицу?'");
	Ответ = Вопрос(Текст, Режим, 0);
	Если Ответ = КодВозвратаДиалога.Да Тогда
		Возврат Истина;
	Иначе
...
Процедура СохранитьЗапросы()
	...
	Если Не ПустаяСтрока(ИмяФайла) Тогда 
		СохранитьЗапросыСервер(ИмяФайла);
		ПоказатьОповещениеПользователя(ВернутьСтр("ru = 'Автосохранение прошло успешно.'"), ИмяФайла);
		Модифицированность = Ложь;
	КонецЕсли;	
КонецПроцедуры	
...
//
&НаСервере
Процедура СохранитьЗапросыСервер(ИмяФайла)
	ОбъектОбработки().ЗаписатьЗапросыВФайлXML(ИмяФайла, Объект);	
КонецПроцедуры
...
Процедура ЗапуститьВыполнениеЗапроса(ВыводитьВременныеТаблицы)
	...
	Если ПустаяСтрока(Элементы.ТекстЗапроса) Тогда 
		ТекстПредупреждения = ВернутьСтр("ru = 'Введите текст запроса.'");
		Предупреждение(ТекстСообщения);
		Возврат;
	КонецЕсли;
...
//
&НаСервере
Функция ВыполнитьЗапросСервер(ИндексТекущегоЗапроса, ТабличныйДокументРезультата, ВыводитьВременныеТаблицы, ВыводитьИдентификатор, Текст, ТекстСообщения)
	...
	// Выполнение запроса.
	ОбъектОбработки().ВыполнитьЗапрос(Текст, МассивПараметров, ВыводитьВременныеТаблицы, ВыводитьИдентификатор, ПорядокОбхода, ТабличныйДокументРезультата, ВремяВыполнения, КоличествоСтрок, ТекстСообщения, ИспользованиеЧередования);
	...
	РезультСтрока	= ВернутьСтр("ru = 'Результат запроса (количество строк = %КоличествоСтрок%, время выполнения = %ВремяВыполнения% с)'");
...
Функция ТипСтрока(Значение)
	...
	Если СписокДобавленныхТипов.НайтиПоЗначению(СтроковоеПредставлениеТипа) <> Неопределено Тогда 
...
Функция ДиалогСохраненияФайла(ИмяФайла)
	...
	Если РасширениеПодключено Тогда 
		Если ПустаяСтрока(ИмяФайла) Тогда 
			...
			Диалог.Заголовок					= ВернутьСтр("ru = 'Выберите файл запросов'");
			Диалог.ПредварительныйПросмотр  	= Ложь;
			Диалог.Фильтр   					= ВернутьСтр("ru = 'Файл запросов (*.q1c)|*.q1c'");
		...
		КонецЕсли;	
	Иначе 
	    ТекстСообщения = ВернутьСтр("ru = 'В данном браузере невозможно работать с файлами.'");
		ПоказатьСообщениеПользователю(ТекстСообщения, "Объект");
	КонецЕсли;	
КонецФункции
...
Процедура ОбработкаЧтенияФайла(Удалять)
	...
	Если Не ПустаяСтрока(ИмяФайла) Тогда 
		...
		КонецЕсли;	
		ЗагрузитьЗапросыИзФайла(ИмяФайла);
		Объект.ИмяФайла = ИмяФайла;	
	КонецЕсли;
КонецПроцедуры	
...
Процедура ЗагрузитьЗапросыИзФайла(ИмяФайла)
	ОбъектВнешнейОбработки = ОбъектОбработки().ПрочитатьЗапросыИзФайлаXML(ИмяФайла);
...
Процедура ЗаполнитьПараметрыКлиент()
	...
	Если Не ПустаяСтрока(ТекущийЗапрос.Текст) тогда
		...
		ОбновитьФормуКлиент();
	Иначе
		Предупреждение(ВернутьСтр("ru = 'Текст запроса пустой.'"));
	КонецЕсли;
КонецПроцедуры	
	...
	Для каждого СчитанныйПараметр из ВозврСтруктура Цикл
		...
		Если не НайденПараметр Тогда 
			Если Не ФлагВыводаСообщения Тогда
				Сообщение 				= Новый СообщениеПользователю(); 
			    Сообщение.Текст 		= ВернутьСтр("ru = 'Найденные параметры были добавлены автоматически.'");
...
Функция СформироватьИмяКопииЗапроса(Имя)
	...
	Пока Флаг Цикл 
		ФормируемоеИмяЗапроса = ВернутьСтр("ru = '%ИмяЗапроса% - Копия %НомерКопии%'");
