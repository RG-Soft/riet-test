...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если Не ПроконтролироватьНастройки() И ОткрытиеСУзломПараметром Тогда
		...
		ТекстСообщения = СтрЗаменить(
			НСтр("ru = 'Для ""%1"" редактирование регистрации объектов недоступно.'; en = 'Editing objects isn't available to ""%1"".'"),
	...
	// { RGS VShamin 04.04.2016 18:13:59 - S-I-0001590
	rgsУстановитьДоступностьЭлементов();
	// } RGS VShamin 04.04.2016 18:15:23 - S-I-0001590
...
Процедура ОбработкаВыбора(ВыбранноеЗначение, ИсточникВыбора)
	...
	Если ВыбранноеЗначение.ДействиеВыбора Тогда
		Текст = НСтр("ru = 'Зарегистрировать результат запроса на узле ""%1""?'"); 
	Иначе
		Текст = НСтр("ru = 'Отменить регистрацию результата запроса на узле ""%1""?'");
	КонецЕсли;
...
Процедура ПоказатьРезультатВыгрузки(Команда)
	...
	Если Сериализация.Количество() > 0 Тогда
		...
		ЗаголовокТекста = НСтр("ru = 'Результат стандартной выгрузки (РИБ)'; en = 'Result of standard unloading'");
...
Процедура ЗарегистрироватьИОМИПредопределенные(Команда)
	...
	ТекстВопроса     = СтрЗаменить( 
		НСтр("ru = 'Зарегистрировать данные для восстановления подчиненного узла РИБ на узле ""%1""?'"),
		"%1", УзелОбменаСсылка);
...
Процедура УстановитьУсловноеОформление()
	...
	Элемент.Оформление.УстановитьЗначениеПараметра("Текст", НСтр("ru = 'Не выгружалось'; en = 'It wasn't unloaded'"));
	...
	Элемент.Оформление.УстановитьЗначениеПараметра("Текст", НСтр("ru = 'Нет изменений'; en = 'It wasn't unloaded'"));
...
Процедура РедактироватьНомерСообщенияКонстанты()
	...
	Подсказка = НСтр("ru = 'Номер отправленного'; en = 'Number of sent'"); 
...
Процедура УстановитьЗаголовокНомеровСообщений()
	...
	Текст = НСтр("ru = '№ отправленного %1, № принятого %2'; en = '# of sent %1, # of accepted $2'");
...
Процедура СообщитьОРезультатахРегистрации(Команда, Результаты)
	...
	Если ТипЗнч(Команда) = Тип("Булево") Тогда
		Если Команда Тогда
			ЗаголовокПредупреждения = НСтр("ru = 'Регистрация изменений:'; en = 'Registration of changes:'");
			ТекстПредупреждения = НСтр("ru = 'Зарегистрировано %1 изменений из %2 на узле ""%0""'; en = 'It is registered %1 of changes from %2 on node ""%0""'");
		Иначе
			ЗаголовокПредупреждения = НСтр("ru = 'Отмена регистрации:'; en = 'Registration canceling:'");
			ТекстПредупреждения = НСтр("ru = 'Отменена регистрация %1 изменений на узле ""%0"".'; en = 'Registration canceling %1 of changes on node ""%0"".'");
		КонецЕсли;
	Иначе
		ЗаголовокПредупреждения = НСтр("ru = 'Изменение номера сообщения:'; en = 'Change of number of messages:'");
		ТекстПредупреждения = НСтр("ru = 'Номер сообщения изменен на %3 у %1 объекта(ов)'; en = 'The message number if changed on %3 at %1 object(s)'");
	КонецЕсли;
...
Процедура УдалитьРегистрациюКонстантыВСписке()
	...
	ИначеЕсли Количество = 1 Тогда
		Текст = НСтр("ru = 'Отменить регистрацию ""%2"" на узле ""%1""?'; en = 'To cancel registration ""%2"" on node ""%1""?'"); 
	Иначе
		Текст = НСтр("ru = 'Отменить регистрацию выбранных констант на узле ""%1""?'; en = 'To cancel registration of the selected constants on a node ""%1""?'"); 
	КонецЕсли;
	Текст = СтрЗаменить(Текст, "%1", УзелОбменаСсылка);
	Текст = СтрЗаменить(Текст, "%2", СписокПредставлений[0]);
	
	ЗаголовокВопроса = НСтр("ru = 'Подтверждение'; en = 'Confirmation'");
...
Процедура ДобавитьРегистрациюВСписокСсылок(ЭтоПодбор = Ложь)
	...
	// { RGS VShamin 06.04.2016 14:19:49 - S-I-0001590
	//ТекПараметры.Вставить("ЗакрыватьПриВыборе", ЭтоПодбор);
	ТекПараметры.Вставить("ЗакрыватьПриВыборе", Истина);
	// } RGS VShamin 06.04.2016 14:19:57 - S-I-0001590
...
Процедура УдалитьРегистрациюИзСпискаСсылок()
	...
	ИначеЕсли Количество = 1 Тогда
		Текст = НСтр("ru = 'Отменить регистрацию ""%2"" на узле ""%1""?'; en = 'To cancel registration ""%2"" on node ""%1""?'"); 
	Иначе
		Текст = НСтр("ru = 'Отменить регистрацию выбранных объектов на узле ""%1""?'; en = 'To cancel registration of the selected objects on a node ""%1""?'"); 
	КонецЕсли;
	Текст = СтрЗаменить(Текст, "%1", УзелОбменаСсылка);
...
Процедура ДобавитьРегистрациюВыделенныхОбъектов(БезУчетаАвторегистрации = Истина)
	...
	Текст = НСтр("ru = 'Зарегистрировать %1 для выгрузки на узле ""%2""?
	                 |
	                 |Изменение регистрации большого количества объектов может занять продолжительное время!'; en = 'To register %1 for uploading on a node ""%2""?'");
...
Процедура УдалитьРегистрациюВыделенныхОбъектов(БезУчетаАвторегистрации = Истина)
	...
	Текст = НСтр("ru = 'Отменить регистрацию %1 для выгрузки на узле ""%2""?
	                 |
	                 |Изменение регистрации большого количества объектов может занять продолжительное время!'; en = 'To cancel registration %1 for uploading on a node ""%2""?'");
...
Процедура ОбработкаВыбораДанных(ТаблицаФормы, ВыбранноеЗначение)
	...
	Если Действие Тогда
		...
		// { RGS VShamin 06.04.2016 18:10:37 - S-I-0001590
		Если Данные <> Неопределено И Данные.Количество() > 0 И ТипЗнч(Данные[0]) = Тип("ДокументСсылка.КонсолидированныйПакетЗаявокНаПеревозку") Тогда
		
			Оповещение = Новый ОписаниеОповещения("rgsАвтоматическаяРегистрацияПарселей", ЭтотОбъект, Новый Структура);
			...
			Оповещение.ДополнительныеПараметры.Вставить("УзелОбменаСсылка",       УзелОбменаСсылка);
			ПоказатьВопрос(Оповещение, "Зарегистрировать парсели?", РежимДиалогаВопрос.ДаНет);
		
		КонецЕсли;
		// } RGS VShamin 06.04.2016 18:10:43 - S-I-0001590
	...
	Если Ссылка = Неопределено Тогда
		Текст = НСтр("ru = 'Отменить регистрацию выбранных объектов
		                 |на узле ""%1""?'; en = 'To cancel registration of the selected objects on a node ""%1""?'"); 
	Иначе
		Текст = НСтр("ru = 'Отменить регистрацию ""%2""
		                 |на узле ""%1""?'; en = 'To cancel registration ""%2"" on a node ""%1""?'"); 
	КонецЕсли;
...
Процедура НастроитьПустуюСтраницу(Наименование, ИмяТаблицы = Неопределено)
	...
	Иначе
		...
		Если Строка <> Неопределено Тогда
			ТекстКоличеств = НСтр("ru = 'Зарегистрировано объектов: %1
			                          ...
			                          |'; en = 'It is registered objects: %1
			                          |It is unloaded objects: %2
			                          |Objects aren't unloaded: %3'");
	...
	Текст = НСтр("ru = '%1.
	                 ...
	                 |
	                 |%2
	                 |For registration or canceling of registration of a data interchange on a node
	                 |""%3""
	                 |select an object type at the left from a tree of meta data and use
	                 |commands ""To registered"" или ""To cancel registered""'");
...
Процедура НастроитьВидимостьКомандОбщегоМеню()
	...
	// { RGS VShamin 06.04.2016 17:42:13 - S-I-0001590
	rgsУстановитьДоступностьЭлементов();
	// } RGS VShamin 06.04.2016 17:42:23 - S-I-0001590
...
Процедура ОбработатьЗапретИзмененияУзла()
	...
	Если ОперацииРазрешены Тогда
		...
		Заголовок = НСтр("ru = 'Регистрация изменений для обмена данными'; en = 'Registration of changes for data exchange'");
	Иначе
		Элементы.УзелОбменаСсылка.Видимость = Ложь;
		Заголовок = СтрЗаменить(НСтр("ru = 'Регистрация изменений для обмена с  ""%1""'; en = 'Registration of changes for data exchange with ""%1""'"), "%1", Строка(УзелОбменаСсылка));
	КонецЕсли;
...
// { RGS VShamin 04.04.2016 18:13:59 - ограничение для пользователей
Процедура rgsУстановитьДоступностьЭлементов()
	...
	Элементы.ФормаДобавитьРегистрациюОтбор.Доступность = Ложь;
	Элементы.ДеревоМетаданныхДобавитьРегистрациюРезультатаЗапроса.Доступность = Ложь;
	
	Элементы.ФормаУдалитьРегистрациюОтбор.Доступность = Ложь;
	Элементы.ДеревоМетаданныхУдалитьРегистрациюРезультатаЗапроса.Доступность = Ложь;
	
	Элементы.ДеревоМетаданныхДобавитьРегистрациюВсехОбъектов.Доступность = Ложь;
	Элементы.ДеревоМетаданныхДобавитьРегистрациюАвтоОбъектов.Доступность = Ложь;
	
	Элементы.ДеревоМетаданныхУдалитьРегистрациюВсехОбъектов.Доступность = Ложь;
	Элементы.ДеревоМетаданныхУдалитьРегистрациюАвтоОбъектов.Доступность = Ложь;
	
	Элементы.ДеревоМетаданныхКонтекстноеМенюДобавитьРегистрациюВсехОбъектов.Доступность = Ложь;
	Элементы.ДеревоМетаданныхКонтекстноеМенюДобавитьРегистрациюАвтоОбъектов.Доступность = Ложь;
	Элементы.ДеревоМетаданныхКонтекстноеМенюДобавитьРегистрациюОтбор.Доступность = Ложь;
	
	Элементы.ДеревоМетаданныхКонтекстноеМенюУдалитьРегистрациюВсехОбъектов.Доступность = Ложь;
	Элементы.ДеревоМетаданныхКонтекстноеМенюУдалитьРегистрациюАвтоОбъектов.Доступность = Ложь;
	Элементы.ДеревоМетаданныхКонтекстноеМенюУдалитьРегистрациюОтбор.Доступность = Ложь;
	
	Элементы.СписокСсылокКонтекстноеМенюДобавитьРегистрациюСсылкиОтбор.Доступность = Ложь;
	Элементы.СписокСсылокКонтекстноеМенюУдалитьРегистрациюСсылкиОтбор.Доступность = Ложь;
	
КонецПроцедуры // } RGS VShamin 04.04.2016 18:15:23 - ограничение для пользователей

// { RGS VShamin 06.04.2016 18:12:29 - S-I-0001590
&НаКлиенте
Процедура rgsАвтоматическаяРегистрацияПарселей(РезультатВопроса, ДополнительныеПараметры) Экспорт

	Если РезультатВопроса = КодВозвратаДиалога.Да Тогда
		
		МассивПарселей = rgsАвтоматическаяРегистрацияПарселейНаСервере(ДополнительныеПараметры.Данные);
		Результат = ДобавитьРегистрациюНаСервере(Истина, ДополнительныеПараметры.УзелОбменаСсылка, МассивПарселей, ДополнительныеПараметры.ИмяТаблицы);
...
КонецПроцедуры // } RGS VShamin 06.04.2016 18:12:29 - S-I-0001590

// { RGS VShamin 06.04.2016 18:12:29 - S-I-0001590
&НаСервере
Функция rgsАвтоматическаяРегистрацияПарселейНаСервере(Данные)

	МассивПарселей = Новый Массив;
	Для каждого ТекДокумент Из Данные Цикл
		
		Для каждого ТекПарсель Из ТекДокумент.Parcels.ВыгрузитьКолонку("Parcel") Цикл
			МассивПарселей.Добавить(ТекПарсель);
		КонецЦикла;
		
	КонецЦикла;
	Возврат МассивПарселей;

КонецФункции // } RGS VShamin 06.04.2016 18:12:29 - S-I-0001590
