...
//Отредактировано создание описания оповещения: ЭтотОбъект -> ЭтаФорма
//<=
...
Функция НачатьЗамерВремени(Отчет)
	...
	Если ОбщегоНазначенияКлиент.ПодсистемаСуществует("СтандартныеПодсистемы.ОценкаПроизводительности") Тогда
		...
		Если КлючеваяОперация <> Неопределено Тогда
			ОбщегоНазначенияКлиент.ОбщийМодуль("ОценкаПроизводительностиКлиентСервер").НачатьЗамерВремени(КлючеваяОперация);
		КонецЕсли;
...
&НаСервере
Функция ОткрытьФормуОтчетаНаСервере(Знач Отчет, ОрганизацияОтчета = Неопределено, 
									ДатаНачалаПериодаОтчета = Неопределено, ДатаОкончанияПериодаОтчета = Неопределено, 
									ПериодичностьОтчета = Неопределено, НаименованиеОтчета = Неопределено)
...
Процедура ПоказатьБланк(Команда)
	...
	Если НЕ Элементы.ЖурналОтчетов.ТекущиеДанные = Неопределено
	   И Элементы.ЖурналОтчетов.ТекущиеДанные.НаименованиеОтчета = "РСВ-1 ПФР" Тогда
	...
	Если СтандартнаяОбработка Тогда
		...
		Если ОтчетыДляПакетнойОбработки.Количество() <> 0 Тогда
			...
			Если ОтчетыДляПакетнойОбработки.Количество() = 1 Тогда
				ТекстВопроса = "Напечатать выбранный отчет?";
			Иначе
				ТекстВопроса = "Выбрано " + ОтчетыДляПакетнойОбработки.Количество()
				+ " отчетов." + Символы.ПС + "Напечатать выбранные отчеты?";
			КонецЕсли;
...
Процедура Выгрузить(Команда)
	...
	Если Элементы.ЖурналОтчетов.ТекущиеДанные.НаименованиеОтчета = "РВ-3 ПФР" Тогда
		ОписаниеРВ_3 = ОписаниеРегОтчета(Элементы.ЖурналОтчетов.ТекущаяСтрока);
	...
	Если Элементы.ЖурналОтчетов.ТекущиеДанные.НаименованиеОтчета = "РСВ-2 ПФР" Тогда
		ОписаниеРСВ_2 = ОписаниеРегОтчета(Элементы.ЖурналОтчетов.ТекущаяСтрока);
...
Процедура Сохранить(Команда)
	...
	Если СуществуютТекстыДляСохраненияВФайл(Элементы.ЖурналВыгрузки.ТекущаяСтрока) Тогда
		...
		ПолучитьФорму("Документ.ВыгрузкаРегламентированныхОтчетов.Форма.ФормаДокумента", ПараметрыФормы, ЭтаФорма).СохранитьТексты(Истина);
...
Процедура ОбработатьАктивизациюСтрокиОсновнойТаблицы()
	...
	Если ТекДанные = Неопределено
		ИЛИ 
		( ИсточникиОтчетовПФР.Найти(ТекДанные_ИсточникОтчета) = Неопределено
...
Процедура ЦиклыОбменаОбменятьсяЗавершение(Результат, ДополнительныеПараметры) Экспорт
	...
	КонтекстЭДОКлиент.ОсуществитьОбменПоУчетнойЗаписи(ВыбраннаяОрганизация, , СписокПолученныхТС, ОписаниеОповещения);
...
Процедура ПечататьСразуСДвухмернымШтрихкодомPDF417(Команда)
	...
	Если ФормаОтчета.СтруктураРеквизитовФормы.Свойство("ДоступенМеханизмПечатиРеглОтчетностиСДвухмернымШтрихкодомPDF417") Тогда
		...
		РегламентированнаяОтчетностьКлиент.ВывестиМашиночитаемуюФорму(ФормаОтчета, "ПечататьСразуСДвухмернымШтрихкодомPDF417");
