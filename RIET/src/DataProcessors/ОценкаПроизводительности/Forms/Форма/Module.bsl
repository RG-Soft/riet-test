...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если ОбщаяПроизводительностьСистемы.Пустая() Тогда
		Объект.ОбщаяПроизводительностьСистемы = ВернутьСтр("ru = 'Общая производительность системы'");
	Иначе
	...
	Исключение
		ТекстСообщения = ВернутьСтр("ru = 'Не удалось загрузить настройки.'");
...
Процедура УказатьAPDEX(Команда)
	...
	Если СтрокаТЧ[Элемент.Имя] = 0 Тогда
		ПоказатьПредупреждение(,ВернутьСтр("ru = 'Отсутствуют замеры производительности.
			|Рассчитать целевое время невозможно.'"));
		Возврат;
	КонецЕсли;
	
	Оповещение = Новый ОписаниеОповещения("УказатьAPDEXЗавершение", ЭтотОбъект);
	Подсказка = ВернутьСтр("ru = 'Введите желаемое значение APDEX'"); 
	ЗначениеAPDEX = 0;
...
Процедура УказатьAPDEXЗавершение(Знач ЗначениеAPDEX, Знач ДополнительныеПараметры) Экспорт
	...
	Если 0 > ЗначениеAPDEX Или ЗначениеAPDEX > 1 Тогда
		ПоказатьПредупреждение(,ВернутьСтр("ru = 'Введен неправильный показатель APDEX.
...
Процедура ОбновитьПоказатели(ЗначенияОтбора = Неопределено)
	...
	Если ТаблицаКлючевыхОпераций = Неопределено Тогда
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(ВернутьСтр("ru = 'Период установлен не верно.'"));
		Возврат;
	КонецЕсли;
...
Процедура ОбновитьДиаграмму(ТаблицаКлючевыхОпераций)
	...
	ТекстЗаголовка = ВернутьСтр("ru = 'Диаграмма производительности с %1 по %2 - шаг: %3'");
...
Процедура ОбменятьПриоритеты(КлючеваяОперация1, Приоритет1, КлючеваяОперация2, Приоритет2)
	...
	Исключение
		ОтменитьТранзакцию();
		ЗаписьЖурналаРегистрации(ВернутьСтр("ru = 'Выполнение операции'", ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка()), УровеньЖурналаРегистрации.Ошибка,,, ПодробноеПредставлениеОшибки(ИнформацияОбОшибке()));
 		ВызватьИсключение;
	КонецПопытки;
КонецПроцедуры
...
Процедура УдалитьФайлыНаСервере(Каталог)
	...
	Исключение
		Событие = ВернутьСтр("ru = 'Удалить файлы на сервере'", ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка());
...
Функция ВычислитьДатыВременногоОтрезка(ДатаНачала, ДатаОкончания, ИндексПериода)
	...
	Если КоличествоШагов <= ИндексПериода Тогда
		ВызватьИсключение ВернутьСтр("ru = 'Количество шагов не может быть меньше индекса.'");
	КонецЕсли;
	...
	Для Каждого СтрокаТЧ Из Объект.Производительность Цикл
		...
		Если СтрокаТЧ.ЦелевоеВремя = 0 
			...
			ОбщегоНазначенияКлиентСервер.СообщитьПользователю(
				ВернутьСтр("ru = 'Целевое время обязательно должно быть заполнено.'"),
				,
...
Процедура ВыбратьФайлЭкспортаПредложено(РасширениеРаботыСФайламиПодключено, ДополнительныеПараметры) Экспорт
	...
	Состояние(ВернутьСтр("ru = 'Экспорт данных...'"));
