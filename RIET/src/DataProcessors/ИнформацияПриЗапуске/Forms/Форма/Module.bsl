...
Функция ПодготовитьДанныеФормы()
	...
	Для Каждого ПакетСтраниц Из ПодготовленныеПакеты Цикл
		ПакетСтраниц.ЗаголовокФормы = ВернутьСтр("ru = 'Информация'");
		...
		Если СтрНачинаетсяС(ПакетСтраниц.Раздел, "_") Тогда
			...
			Иначе
				...
				Если ТекущееПодменю = Неопределено Тогда
					ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(ВернутьСтр("ru = 'Не найдена группа ""%1""'"), ИмяПодменю);
				КонецЕсли;
		...
		ИначеЕсли НаименованиеТекущегоРаздела <> ПакетСтраниц.Раздел Тогда
			...
			ЭтоГлавная = (ПакетСтраниц.Раздел = ВернутьСтр("ru = 'Главная'"));
