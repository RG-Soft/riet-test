...
Процедура ЗаполнитьСписокПользователейБД(ОтборСтрок) Экспорт
	...
	Для Каждого Пользователь Из МассивПользователейИБ Цикл
...
// в случае, если "ТекущиеДанные" содержат любую из доступных ролей текущего отброа
//
...
//процедура инициализации начальных параметорв
Процедура ИнициализироватьРабоуСПользователямиБД() Экспорт
...
//процедура инициализации начальных параметорв
Процедура СохранитьПараметрыРабоыСПользователямиБД() Экспорт
...
//функция возвращает соответсвие имян и пользователей БД
...
Процедура УстановитьОтбораДанныхПользовательБД(ОтборСтрок, ЭтаФорма, КнопкаИстирияОтборов, КнопкаОтменыОтбора) Экспорт
	...
	ДобавитьВИсториюОтбора(ОтборСтрок, КнопкаИстирияОтборов);
	ОбновитьКнопки(ОтборСтрок, КнопкаОтменыОтбора);
