...
Процедура ГиперссылкаОтложенноеОбновлениеНажатие(Элемент)
	...
	Иначе
		ТекстПредупреждения = ВернутьСтр("ru = 'Обработка данных еще не выполнялась.'");
...
Процедура Приостановить(Команда)
	...
	ТекстВопроса = ВернутьСтр("ru = 'Остановка дополнительных процедур обработки данных
...
Процедура СформироватьТаблицуОтложенныхОбработчиков(ВыполненыВсеОбработчики = Истина, НачальноеЗаполнение = Ложь)
	...
	Если ОбработчикиНеВыполнялись Тогда
		Элементы.ТекстПояснения.Заголовок = ВернутьСтр("ru = 'Рекомендуется запустить невыполненные процедуры обработки данных.'");
	Иначе
		Элементы.ТекстПояснения.Заголовок = ВернутьСтр("ru = 'Невыполненные процедуры рекомендуется запустить повторно.'");
	КонецЕсли;
...
Процедура ДобавитьОтложенныйОбработчик(СтрокаОбработчика, ОбработчикиНеВыполнялись, ВыполненыВсеОбработчики, НачальноеЗаполнение, ВыполняетсяИзменениеПриоритета)
	...
	ШаблонИнформацияОПроцедуреОбновления = ВернутьСтр("ru = '%1%2Процедура ""%3"" обработки данных %4.'");
	...
	Если СтрокаОбработчика.Статус = "Выполнено" Тогда
		...
		ПредставлениеСтатусаВыполнения = ВернутьСтр("ru = 'завершилась успешно'");
		СтрокаСписка.СтатусПредставление = ВернутьСтр("ru = 'Выполнено'");
	...
	ИначеЕсли СтрокаОбработчика.Статус = "Выполняется" Тогда
		...
		ПредставлениеСтатусаВыполнения = ВернутьСтр("ru = 'в данный момент выполняется'");
		Если Прогресс <> Неопределено Тогда
			ШаблонСтатус = ВернутьСтр("ru = 'Выполняется (%1%)'");
			СтрокаСписка.СтатусПредставление = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(ШаблонСтатус, Прогресс)
		Иначе
			СтрокаСписка.СтатусПредставление = ВернутьСтр("ru = 'Выполняется'");
		КонецЕсли;
	ИначеЕсли СтрокаОбработчика.Статус = "Ошибка" Тогда
		
		ОбработчикиНеВыполнялись = Ложь;
		ВыполненыВсеОбработчики        = Ложь;
		ПредставлениеСтатусаВыполнения = ВернутьСтр("ru = 'Процедура ""%1"" обработки данных завершилась с ошибкой:'") + Символы.ПС + Символы.ПС;
		...
		СтрокаСписка.СтатусПредставление = ВернутьСтр("ru = 'Ошибка'");
	...
	ИначеЕсли СтрокаОбработчика.Статус = "Приостановлен" Тогда
		...
		ОбработчикиНеВыполнялись = Ложь;
		ВыполненыВсеОбработчики        = Ложь;
		ПредставлениеСтатусаВыполнения = ВернутьСтр("ru = 'остановлена администратором'");
		СтрокаСписка.СтатусПредставление = ВернутьСтр("ru = 'Остановлено'");
		СтрокаСписка.СтатусКартинка    = БиблиотекаКартинок.ЗнакСтоп;
	Иначе
		
		ВыполненыВсеОбработчики        = Ложь;
		ПредставлениеСтатусаВыполнения = ВернутьСтр("ru = 'еще не выполнялась'");
		СтрокаСписка.СтатусПредставление = ВернутьСтр("ru = 'Не выполнялась'");
	КонецЕсли;
...
Функция ДлительностьВыполненияПроцедурыОбновления(ДлительностьВыполнения)
	...
	ШаблонСекунды = ВернутьСтр("ru = '%1 сек.'");
	ШаблонМинуты = ВернутьСтр("ru = '%1 мин. %2 сек.'");
	ШаблонЧасы = ВернутьСтр("ru = '%1 ч. %2 мин.'");
	...
	Если ДлительностьВСекундах < 1 Тогда
		Возврат ВернутьСтр("ru = 'менее секунды'")
