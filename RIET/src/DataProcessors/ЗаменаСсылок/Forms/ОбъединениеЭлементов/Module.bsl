...
Процедура ПередЗакрытием(Отказ, СтандартнаяОбработка)
	...
	Если ПодтверждениеЗакрытияФормы<>Истина Тогда
		...
		Текст = ВернутьСтр("ru = 'Закрыть объединение элементов?'");
...
Процедура МестаИспользованияПередУдалением(Элемент, Отказ)
	...
	ТекстВопроса = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
		ВернутьСтр("ru = 'Удалить из списка для объединения элемент ""%1""?'"),
...
Функция ПроверитьОбъединяемыеСсылки(Знач НаборСсылок)
	...
	Если КоличествоСсылок < 2 Тогда
		Результат.Ошибка = ВернутьСтр("ru = 'Для объединения необходимо указать несколько элементов.'");
		Возврат Результат;
	КонецЕсли;
	...
	Если Контроль.ЕстьГруппы Тогда
		Результат.Ошибка = ВернутьСтр("ru = 'Один из элементов для объединения является группой.
		                              |Группы не могут быть объединены.'");
	ИначеЕсли Контроль.КоличествоВладельцев > 1 Тогда 
		Результат.Ошибка = ВернутьСтр("ru = 'У элементов для объединения различные владельцы.
		                              |Такие элементы не могут быть объединены.'");
	ИначеЕсли Контроль.КоличествоСсылок <> КоличествоСсылок Тогда
		Результат.Ошибка = ВернутьСтр("ru = 'Все элементы для объединения  должны быть одного типа.'");
	Иначе 
		// Все нормально
...
Процедура СформироватьПодсказкуОбъединения()
	...
	Элементы.ПодсказкаВыбораОсновногоЭлемента.Заголовок = ФорматированнаяСтрока(
		ВернутьСтр("ru = 'Объединить элементы (%1) в ""%2"".
...
Функция СформироватьНадписьЗавершения()
	...
	ТекстРезультата = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
		ВернутьСтр("ru = 'Элементы (%1) объединены в ""%2""'"),
		МестаИспользования.Количество(),
		ОсновнойЭлемент
	);
...
Процедура СформироватьНадписьНеудачныеЗамены()
	...
	Элементы.РезультатНеудачныеЗамены.Заголовок = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
		ВернутьСтр("ru = 'Не удалось объединить элементы (%1 из %2). В некоторых местах использования не может быть произведена
...
Процедура ЗаполнитьМестаИспользования(Знач ТаблицаИспользования)
	...
	Для Каждого Строка Из ТаблицаИспользования Цикл
		...
		СтрокаИспользования.НеИспользуется = ?(Мест = 0, ВернутьСтр("ru = 'Не используется'"), "");
	КонецЦикла;
	...
	ТекстЗаголовка = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
		ВернутьСтр("ru = 'Объединение элементов %1 в один'"),
		Представление
	);
КонецПроцедуры
...
Процедура ДобавитьСтрокиМестИспользования(Знач МассивСсылок)
	...
	Для Каждого Ссылка Из МассивСсылок Цикл
		...
		Если СуществующиеСтроки.Количество() = 0 Тогда
			...
			Строка.НеИспользуется    = ВернутьСтр("ru = 'Не рассчитано'");
		Иначе
...
Процедура ИнициализироватьСценарийМастера()
	...
	ДобавитьШагМастера(Элементы.ШагПоискМестИспользования, 
			...
			КнопкаШагаМастера("Отмена", ВернутьСтр("ru = 'Прервать'"), ВернутьСтр("ru = 'Отказаться от объединения элементов'"),
		)))
	);
	...
	ДобавитьШагМастера(Элементы.ШагВыборОсновногоЭлемента, 
			...
			КнопкаШагаМастера("Далее", ВернутьСтр("ru = 'Объединить'") + " >", ВернутьСтр("ru = 'Начать объединение элементов'"), Истина, Истина, Истина,
			КнопкаШагаМастера("Назад", "", "", Ложь, Ложь, Ложь, 
			КнопкаШагаМастера("Отмена", ВернутьСтр("ru = 'Отмена'"),  ВернутьСтр("ru = 'Отказаться от объединения элементов'"),
		)))
	);
	...
	ДобавитьШагМастера(Элементы.ШагОбъединение, 
			...
			КнопкаШагаМастера("Далее", "", "", Ложь, Ложь, Ложь,
			КнопкаШагаМастера("Назад", "", "", Ложь, Ложь, Ложь,
			КнопкаШагаМастера("Отмена", ВернутьСтр("ru = 'Прервать'"), ВернутьСтр("ru = 'Прервать объединение элементов'"),
		)))
	);
	...
	ДобавитьШагМастера(Элементы.ШагУспешногоЗавершения, 
			...
			КнопкаШагаМастера("Далее", "", "", Ложь, Ложь, Ложь, 
			КнопкаШагаМастера("Назад", "", "", Ложь, Ложь, Ложь, 
			КнопкаШагаМастера("Отмена", ВернутьСтр("ru = 'Закрыть'"), ВернутьСтр("ru = 'Закрыть результаты объединения'"), Истина,  Истина,  Истина,
		)))
	);
	...
	ДобавитьШагМастера(Элементы.ШагПовторОбъединения,
			...
			КнопкаШагаМастера("Далее", ВернутьСтр("ru = 'Повторить объединение'") + " >", ВернутьСтр("ru = 'Повторить объединение'"),  Истина, Истина, Истина, 
			КнопкаШагаМастера("Назад", "< " + ВернутьСтр("ru = 'Назад'"), ВернутьСтр("ru = 'Вернутся к выбору основного элемента'"), Истина, Истина, Ложь,
			КнопкаШагаМастера("Отмена", ВернутьСтр("ru = 'Закрыть'"), ВернутьСтр("ru = 'Закрыть результаты объединения'"),
		)))
	);
...
Процедура ШагПоискМестИспользованияПередДействиемОтмена(Знач ПараметрыШага, Знач ДополнительныеПараметры) Экспорт
	...
	ТекстВопроса = ВернутьСтр("ru = 'Прервать поиск мест использования и закрыть?'");
...
Процедура ШагВыборОсновногоЭлементаПередДействиемДалее(Знач ПараметрыШага, Знач ДополнительныеПараметры) Экспорт
	...
	ПараметрыПредупреждения.Вставить("Заголовок", ВернутьСтр("ru = 'Невозможно объединить элементы'"));
...
Процедура ШагВыборОсновногоЭлементаПередДействиемОтмена(Знач ПараметрыШага, Знач ДополнительныеПараметры) Экспорт
	// Запрос подтверждения на отмену
	ТекстВопроса = ВернутьСтр("ru = 'Отменить объединение элементов и закрыть?'");
	Описание = Новый ОписаниеОповещения("ЗавершениеОтменыИЗакрытиеФормы", ЭтотОбъект, Новый Структура);
	Описание.ДополнительныеПараметры.Вставить("ПараметрыШага", ПараметрыШага);
...
Процедура ШагОбъединениеПередДействиемОтмена(Знач ПараметрыШага, Знач ДополнительныеПараметры) Экспорт
	// Запрос подтверждения на отмену
	ТекстВопроса = ВернутьСтр("ru = 'Прервать процесс объединения и закрыть?'");
	Описание = Новый ОписаниеОповещения("ЗавершениеОтменыИЗакрытиеФормы", ЭтотОбъект, Новый Структура);
	Описание.ДополнительныеПараметры.Вставить("ПараметрыШага", ПараметрыШага);
