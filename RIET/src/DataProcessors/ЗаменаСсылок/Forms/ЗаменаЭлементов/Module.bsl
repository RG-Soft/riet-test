...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Элементы["СписокСсылкаНовый"].Заголовок = ВернутьСтр("ru = 'Наименование'");
	...
	Элементы.НадписьТипВыбираемого.Заголовок = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
		ВернутьСтр("ru = 'Выберите элемент ""%1"", на который будет произведена замена.'"),
...
Процедура ПередЗакрытием(Отказ, СтандартнаяОбработка)
	...
	Если ПодтверждениеЗакрытияФормы<>Истина Тогда
		...
		Текст = ВернутьСтр("ru = 'Закрыть замену элементов?'");
...
Процедура СформироватьЦелевойЭлементИПодсказку(Контекст)
	...
	Если Количество = 1 Тогда
		...
		Если Контекст.ЕстьПравоБезвозвратногоУдаления Тогда
			Если Контекст.ТекущийВариантУдаления = "Пометка" Тогда
				ТекстСсылки = ВернутьСтр("ru = 'помечен на удаление'");
			Иначе
				ТекстСсылки = ВернутьСтр("ru = 'удален безвозвратно'");
			КонецЕсли;
			
			Гиперссылка = ГиперссылкаФорматированнойСтроки(ТекстСсылки, "ПереключениеРежимаУдаления");
		Иначе
			Гиперссылка = ВернутьСтр("ru = 'помечен на удаление'");
		КонецЕсли;
		
		ТекстПодсказки = ВернутьСтр("ru = 'Заменить ""%1"" на ""%2"".
		                            |Элемент ""%1"" будет %3'");
	...
	Если Контекст.ЕстьПравоБезвозвратногоУдаления Тогда
		Если Контекст.ТекущийВариантУдаления = "Пометка" Тогда
			ТекстСсылки = ВернутьСтр("ru = 'помечены на удаление'");
		Иначе
			ТекстСсылки = ВернутьСтр("ru = 'удалены безвозвратно'");
		КонецЕсли;
		
		Гиперссылка = ГиперссылкаФорматированнойСтроки(ТекстСсылки, "ПереключениеРежимаУдаления");
	Иначе
		Гиперссылка = ВернутьСтр("ru = 'помечены на удаление'");
	КонецЕсли;
	
	ТекстПодсказки = ВернутьСтр("ru = 'Заменить элементы (%1) на ""%2"".
...
Функция СформироватьНадписьЗавершения()
	...
	Если Количество = 1 Тогда
		ТекстРезультата = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			ВернутьСтр("ru = 'Элемент ""%1"" заменен на ""%2""'"),
			ЗаменяемыеСсылки[0].Ссылка,
			ЦелевойЭлементРезультат
		);
	Иначе
		ТекстРезультата = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			ВернутьСтр("ru = 'Элементы (%1) заменены на ""%2""'"),
			Количество,
			ЦелевойЭлементРезультат
		);
	КонецЕсли;
...
Процедура СформироватьНадписьНеудачныеЗамены()
	...
	Элементы.РезультатНеудачныеЗамены.Заголовок = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
		ВернутьСтр("ru = 'Не удалось заменить элементы (%1 из %2). В некоторых местах использования не может быть произведена
...
Функция ЗаполнитьНеудачныеЗамены(Знач РезультатыЗамены)
	...
	Для Каждого СтрокаРезультата Из РезультатыЗамены Цикл
		...
		Если ТипОшибки = "НеизвестныеДанные" Тогда
			СтрокаОшибки.Причина = ВернутьСтр("ru = 'Обнаружена данные, обработка которых не планировалась.'");
			
		ИначеЕсли ТипОшибки = "ОшибкаБлокировки" Тогда
			СтрокаОшибки.Причина = ВернутьСтр("ru = 'Данные заблокированы другим пользователем.'");
			
		ИначеЕсли ТипОшибки = "ДанныеИзменены" Тогда
			СтрокаОшибки.Причина = ВернутьСтр("ru = 'Данные изменены другим пользователем.'");
		...
		ИначеЕсли ТипОшибки = "ОшибкаУдаления" Тогда
			СтрокаОшибки.Причина = ВернутьСтр("ru = 'Невозможно удалить данные.'");
			
		Иначе
			СтрокаОшибки.Причина = ВернутьСтр("ru = 'Неизвестная ошибка.'");
...
Процедура ИнициализироватьЗаменяемыеСсылки(Знач МассивСсылок)
	...
	Если КоличествоСсылок = 0 Тогда
		ТекстОшибкиПараметров = ВернутьСтр("ru = 'Не указано ни одного элемента для замены.'");
		Возврат;
	КонецЕсли;
	...
	Если Условия.ЕстьГруппы Тогда
		ТекстОшибкиПараметров = ВернутьСтр("ru = 'Один из элементов для замены является группой.
		                                   |Группы не могут быть заменены.'");
	ИначеЕсли Условия.КоличествоВладельцев > 1 Тогда 
		ТекстОшибкиПараметров = ВернутьСтр("ru = 'У элементов для замены разные владельцы.
		                                   |Такие элементы не могут быть заменены.'");
	ИначеЕсли Условия.КоличествоСсылок <> КоличествоСсылок Тогда
		ТекстОшибкиПараметров = ВернутьСтр("ru = 'Все элементы для замены должны быть одного типа.'");
	Иначе 
...
Процедура ИнициализироватьСценарийМастера()
	...
	ДобавитьШагМастера(Элементы.ШагВыборЦелевогоЭлемента, 
			...
			КнопкаШагаМастера("Далее", ВернутьСтр("ru = 'Заменить'") + " >", ВернутьСтр("ru = 'Начать замену элементов'"), Истина, Истина, Истина,
			КнопкаШагаМастера("Назад", "", "", Ложь, Ложь, Ложь, 
			КнопкаШагаМастера("Отмена", ВернутьСтр("ru = 'Отмена'"), ВернутьСтр("ru = 'Отказаться от замены элементов'"),
		)))
	);
	...
	ДобавитьШагМастера(Элементы.ШагЗамена, 
			...
			КнопкаШагаМастера("Назад", "", "", Ложь, Ложь, Ложь, 
			КнопкаШагаМастера("Отмена", ВернутьСтр("ru = 'Прервать'"), ВернутьСтр("ru = 'Прервать замену элементов'"),
		)))
	);
	...
	ДобавитьШагМастера(Элементы.ШагУспешногоЗавершения, 
			...
			КнопкаШагаМастера("Далее", "", "", Ложь, Ложь, Ложь, 
			КнопкаШагаМастера("Назад", "", "", Ложь, Ложь, Ложь, 
			КнопкаШагаМастера("Отмена", ВернутьСтр("ru = 'Закрыть'"), ВернутьСтр("ru = 'Закрыть результаты замены элементов'"), Истина,  Истина,  Истина,
		)))
	);
	...
	ДобавитьШагМастера(Элементы.ШагПовторЗамены,
			...
			КнопкаШагаМастера("Далее", ВернутьСтр("ru = 'Повторить замену'") + " >", ВернутьСтр("ru = 'Повторить замену элементов'"), Истина, Истина, Истина, 
			КнопкаШагаМастера("Назад", "< " + ВернутьСтр("ru = 'Назад'"), ВернутьСтр("ru = 'Вернутся к выбору целевого элемента'"), Истина, Истина, Ложь,
			КнопкаШагаМастера("Отмена", ВернутьСтр("ru = 'Закрыть'"), ВернутьСтр("ru = 'Закрыть результаты замены элементов'"),
		)))
	);
...
Процедура ШагВыборЦелевогоЭлементаПередДействиемДалее(Знач ПараметрыШага, Знач ДополнительныеПараметры) Экспорт
	...
	ИначеЕсли ЗаменяемыеСсылки.Количество() = 1 И ТекущиеДанные.Ссылка = ЗаменяемыеСсылки.Получить(0).Ссылка Тогда
		ПоказатьПредупреждение( , ВернутьСтр("ru = 'Нельзя заменять элемент сам на себя.'"));
		Возврат;
		
	ИначеЕсли ЗначениеРеквизита(ТекущиеДанные, "ЭтоГруппа", Ложь) Тогда
		ПоказатьПредупреждение( , ВернутьСтр("ru = 'Нельзя заменять элемент на группу.'"));
		Возврат;
	КонецЕсли;
	...
	Если ТекущийВладелец <> ОбщийВладелецЗаменяемыхСсылок Тогда
		ПоказатьПредупреждение( , СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			ВернутьСтр("ru = 'Нельзя заменять на элемент, подчиненный другому владельцу.
	...
	Текст = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
		ВернутьСтр("ru = 'Элемент %1 помечен на удаление. Продолжить?'"),
...
Процедура ШагВыборЦелевогоЭлементаПередДействиемОтмена(Знач ПараметрыШага, Знач ДополнительныеПараметры) Экспорт
	...
	ТекстВопроса = ВернутьСтр("ru = 'Отменить замену элементов и закрыть?'");
...
Процедура ШагЗаменаПередДействиемОтмена(Знач ПараметрыШага, Знач ДополнительныеПараметры) Экспорт
	// Запрос подтверждения на отмену
	ТекстВопроса = ВернутьСтр("ru = 'Прервать процесс замены и закрыть?'");
	Описание = Новый ОписаниеОповещения("ЗавершениеОтменыИЗакрытиеФормы", ЭтотОбъект, Новый Структура);
	Описание.ДополнительныеПараметры.Вставить("ПараметрыШага", ПараметрыШага);
...
Процедура ПроверкаДопустимостиЗаменыПрикладнойОбласти(Знач ПараметрыШага)
	...
	ПараметрыПредупреждения.Вставить("Заголовок", ВернутьСтр("ru = 'Невозможно заменить элементы'"));
...
//                      ДействиеШагаМастера("ПередДействиемОтмена", "ШагВыборЦелевогоЭлементаПередДействиемОтмена",
//                  ))), 
//                      КнопкаШагаМастера("Далее", ВернутьСтр("ru = 'Заменить'") + " >", Истина, Истина, Истина,
//                      КнопкаШагаМастера("Назад", "", Ложь, Ложь, Ложь, 
//                      КнопкаШагаМастера("Отмена", ВернутьСтр("ru = 'Отмена'")
//                  )))
//              );
//
...
Процедура ДобавитьШагМастера(Знач Страница, Знач Действия, Знач Кнопки)
	...
	ОписаниеШага.КнопкаДалее.Вставить("Заголовок", ВернутьСтр("ru='Далее'") + " >" );
	
	ОписаниеШага.Вставить("КнопкаНазад", Новый Структура(ПоляКнопки, Истина, Истина, Ложь));
	ОписаниеШага.КнопкаНазад.Вставить("Заголовок", "< " + ВернутьСтр("ru='Назад'"));
	
	ОписаниеШага.Вставить("КнопкаОтмена",Новый Структура(ПоляКнопки, Истина, Истина, Ложь));
	ОписаниеШага.КнопкаОтмена.Вставить("Заголовок", ВернутьСтр("ru='Отмена'"));
		...
		Для Каждого ОписаниеШага Из НастройкиМастера.Шаги Цикл
			Для Каждого КлючЗначение Из ВозможныеДействия Цикл
				...
				Если Не ПустаяСтрока(ИмяОбработчика) Тогда
					...
					Исключение
						Текст = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
							ВернутьСтр("ru = 'Ошибка создания обработчика события %1 для страницы %2, не определена процедура %3'"),
							ИмяДействия, 
...
Процедура ПерейтиНаШагМастера(Знач ИдентификаторШага, Знач ИнициироватьСобытия = Ложь)
	...
	Если СледующийШаг = Неопределено Тогда
		Ошибка = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			ВернутьСтр("ru = 'Не найден шаг помощника %1'"),
			ИдентификаторШага
		);
