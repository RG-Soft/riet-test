...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	//Элементы.ГруппаПроверкаКонтрагентовРасписание.Видимость = Не РежимРаботы.МодельСервиса И РежимРаботы.ЭтоАдминистраторСистемы;
	...
	//		
	//	ПроверкаКонтрагентов.УстановитьВидимостьИЗаголовокПредупрежденияПроМедленнуюРаботу(
	//		Элементы.ПредупреждениеПроМедленноеОткрытие);
...
Процедура ЦентрМониторингаАдресСервисаПриИзменении(Элемент)
	...
	Исключение
		...
		ОписаниеОшибки = ВернутьСтр("ru = 'Адрес сервиса'") + " "
			+ ЦентрМониторингаАдресСервиса + " "
			+ ВернутьСтр("ru = 'не является допустимым адресом веб-сервиса для отправки отчетов об использовании программы.'"); 
...
Процедура УстановитьДоступность(РеквизитПутьКДанным = "")
	...
	Если Элементы.УдалениеПомеченныхНастроитьРасписание.Видимость
		...
		Иначе
			Представление = ВернутьСтр("ru = '<Отключено>'");
		КонецЕсли;
	...
	// СтандартныеПодсистемы.РаботаСКонтрагентами
	Если Элементы.ГруппаПроверкаКонтрагентовРасписание.Видимость
		И (РеквизитПутьКДанным = "АвтоматическиПроверятьКонтрагентовРасписание"
		...
		Элементы.ГруппаПроверкаКонтрагентовРасписание.Доступность = ПроверкаКонтрагентов.ПроверкаКонтрагентовВключена();
		
		РегламентноеЗадание = РегламентныеЗаданияНайтиПредопределенное("ПроверкаКонтрагентов");
		Если РегламентноеЗадание = Неопределено Тогда
			АвтоматическиПроверятьКонтрагентовИспользование = Ложь;
		Иначе
		...
		КонецЕсли;
		Если АвтоматическиПроверятьКонтрагентовИспользование Тогда
			РасписаниеПредставление = Строка(АвтоматическиПроверятьКонтрагентовРасписание);
			Представление = ВРег(Лев(РасписаниеПредставление, 1)) + Сред(РасписаниеПредставление, 2);
			Элементы.НастроитьРасписаниеПроверкиКонтрагентов.Доступность   = Истина;
			Элементы.АвтоматическиПроверятьКонтрагентовПояснение.Видимость = Истина;
			Элементы.АвтоматическиПроверятьКонтрагентовПояснение.Заголовок = Представление;
		Иначе
			Элементы.НастроитьРасписаниеПроверкиКонтрагентов.Доступность   = Ложь;
