...
Процедура ПередЗаписью(Отказ, ПараметрыЗаписи)
	...
	Если НЕ ПараметрыЗаписи.Свойство("КогдаНаименованиеУжеИспользуется") Тогда
		...
		Если ЗначениеЗаполнено(ТекстВопроса) Тогда
			...
			Кнопки.Добавить("ПродолжитьЗапись",            ВернутьСтр("ru = 'Продолжить запись'"));
			Кнопки.Добавить("ВернутьсяКВводуНаименования", ВернутьСтр("ru = 'Вернуться к вводу наименования'"));
...
Процедура ДополнительныеЗначенияСВесомПриИзменении(Элемент)
	...
	Если ЗначениеЗаполнено(Объект.Ссылка)
	   И НЕ Объект.ДополнительныеЗначенияСВесом Тогда
		...
		ТекстВопроса =
			ВернутьСтр("ru = 'Очистить введенные весовые коэффициенты?
			           |
			           |Данные будут записаны.'");
		
		Кнопки = Новый СписокЗначений;
		Кнопки.Добавить("ОчиститьИЗаписать", ВернутьСтр("ru = 'Очистить и записать'"));
		Кнопки.Добавить("Отмена", ВернутьСтр("ru = 'Отмена'"));
...
Процедура ДоступенПриИзменении(Элемент)
	Элементы.ЗадатьУсловиеДоступности.Доступность = Объект.Доступен;
КонецПроцедуры
...
&НаКлиенте
Процедура ВиденПриИзменении(Элемент)
	Элементы.ЗадатьУсловиеВидимости.Доступность = Объект.Виден;
КонецПроцедуры
...
Процедура КомандаДалее(Команда)
	...
	Если ИндексСтраницы = 0
		...
		ТекстПредупреждения = ВернутьСтр("ru = 'Элемент не выбран!'");
		ПоказатьПредупреждение(, ТекстПредупреждения);
		Возврат;
	КонецЕсли;
...
Процедура ЗаписатьОбъект(ВариантТекстаВопроса, ИмяПроцедурыПродолжения, ДополнительныеПараметры = Неопределено, ПараметрыЗаписи = Неопределено)
	...
	Если ВариантТекстаВопроса = "ПереходКСпискуЗначений" Тогда
		ТекстВопроса =
			ВернутьСтр("ru = 'Переход к работе со списком значений
			           |возможен только после записи данных.
			           |
			           |Данные будут записаны.'");
	Иначе
		ТекстВопроса =
			ВернутьСтр("ru = 'Данные будут записаны.'")
	КонецЕсли;
	
	Кнопки = Новый СписокЗначений;
	Кнопки.Добавить("Записать", ВернутьСтр("ru = 'Записать'"));
	Кнопки.Добавить("Отмена", ВернутьСтр("ru = 'Отмена'"));
...
Процедура РедактироватьФорматЗначенияЗавершение(Текст, Неопределен) Экспорт
	...
	Если Текст <> Неопределено Тогда
		...
		Для каждого Подстрока Из Массив Цикл
			Если СтрНайти(Подстрока, "ДП=") > 0 ИЛИ СтрНайти(Подстрока, "DE=") > 0 Тогда
				ТекстПредупреждения = ТекстПредупреждения + Символы.ПС
					+ ВернутьСтр("ru = 'Представление пустой даты не поддерживается в полях ввода.'");
				Продолжить;
			КонецЕсли;
			Если СтрНайти(Подстрока, "ЧН=") > 0 ИЛИ СтрНайти(Подстрока, "NZ=") > 0 Тогда
				ТекстПредупреждения = ТекстПредупреждения + Символы.ПС
					+ ВернутьСтр("ru = 'Представление пустого числа не поддерживается в полях ввода.'");
				Продолжить;
			КонецЕсли;
			Если СтрНайти(Подстрока, "ДФ=") > 0 ИЛИ СтрНайти(Подстрока, "DF=") > 0 Тогда
				Если СтрНайти(Подстрока, "ддд") > 0 ИЛИ СтрНайти(Подстрока, "ddd") > 0 Тогда
					ТекстПредупреждения = ТекстПредупреждения + Символы.ПС
						+ ВернутьСтр("ru = 'Кратное название дня недели не поддерживается в полях ввода.'");
				КонецЕсли;
				Если СтрНайти(Подстрока, "дддд") > 0 ИЛИ СтрНайти(Подстрока, "dddd") > 0 Тогда
					ТекстПредупреждения = ТекстПредупреждения + Символы.ПС
						+ ВернутьСтр("ru = 'Полное название дня недели не поддерживается в полях ввода.'");
				КонецЕсли;
				Если СтрНайти(Подстрока, "МММ") > 0 ИЛИ СтрНайти(Подстрока, "MMM") > 0 Тогда
					ТекстПредупреждения = ТекстПредупреждения + Символы.ПС
						+ ВернутьСтр("ru = 'Кратное название месяца не поддерживается в полях ввода.'");
				КонецЕсли;
				Если СтрНайти(Подстрока, "ММММ") > 0 ИЛИ СтрНайти(Подстрока, "MMMM") > 0 Тогда
					ТекстПредупреждения = ТекстПредупреждения + Символы.ПС
						+ ВернутьСтр("ru = 'Полное название месяца не поддерживается в полях ввода.'");
				КонецЕсли;
			КонецЕсли;
			Если СтрНайти(Подстрока, "ДЛФ=") > 0 ИЛИ СтрНайти(Подстрока, "DLF=") > 0 Тогда
				Если СтрНайти(Подстрока, "ДД") > 0 ИЛИ СтрНайти(Подстрока, "DD") > 0 Тогда
					ТекстПредупреждения = ТекстПредупреждения + Символы.ПС
						+ ВернутьСтр("ru = 'Длинная дата (месяц прописью) не поддерживается в полях ввода.'");
				КонецЕсли;
			КонецЕсли;
		КонецЦикла;
		...
		Если ЗначениеЗаполнено(ТекстПредупреждения) Тогда
			ТекстПредупреждения = ТекстПредупреждения + Символы.ПС
				+ ВернутьСтр("ru = 'В местах использования полей надписей ограничений нет.'");
			ПоказатьПредупреждение(, ТекстПредупреждения);
		КонецЕсли;
	КонецЕсли;
...
Процедура УстановитьНастройкиПомощника(ТекущаяСтраница = Неопределено)
	...
	ШаблонЗаголовкаСписка        = ВернутьСтр("ru = 'Выберите %1 для включения в набор ""%2""'");
	ШаблонЗаголовкаПереключателя = ВернутьСтр("ru = 'Выберите вариант добавления дополнительного %1 ""%2"" в набор ""%3""'");
	
	Если ТекущаяСтраница = Элементы.ВыборРеквизита Тогда
		Если ПереданныеПараметрыФормы.ЭтоДополнительноеСведение Тогда
			Заголовок = ВернутьСтр("ru = 'Добавление дополнительного сведения'");
		Иначе
			Заголовок = ВернутьСтр("ru = 'Добавление дополнительного реквизита'");
		КонецЕсли;
		
		Элементы.КоманднаяПанельЛево.Доступность = Ложь;
		Элементы.КомандаДалее.Заголовок = ВернутьСтр("ru = 'Далее >'");
		
		Элементы.ДекорацияЗаголовок.Заголовок = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			ШаблонЗаголовкаСписка,
			?(ПереданныеПараметрыФормы.ЭтоДополнительноеСведение, ВернутьСтр("ru = 'дополнительное сведение'"), ВернутьСтр("ru = 'дополнительный реквизит'")),
	...
	ИначеЕсли ТекущаяСтраница = Элементы.ВыборДействия Тогда
		...
		Элементы.РежимДобавленияРеквизита.Заголовок = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			...
			?(ПереданныеПараметрыФормы.ЭтоДополнительноеСведение, ВернутьСтр("ru = 'сведения'"), ВернутьСтр("ru = 'реквизита'")),
	...
	Иначе
		Элементы.КомандаДалее.Заголовок = ВернутьСтр("ru = 'Готово'");
		Элементы.КоманднаяПанельЛево.Доступность = Истина;
	КонецЕсли;
...
Процедура ОбновитьСоставЭлементовФормы(ТекстПредупреждения = "")
				...
				Если СтарыйТипЗначения.СодержитТип(Тип("СправочникСсылка.ЗначенияСвойствОбъектовИерархия"))
				   И НЕ Объект.ТипЗначения.СодержитТип(Тип("СправочникСсылка.ЗначенияСвойствОбъектовИерархия")) Тогда
					...
					ТекстПредупреждения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
						ВернутьСтр("ru = 'Недопустимо удалять тип ""%1"",
		...
		Если Объект.ТипЗначения.СодержитТип(Тип("СправочникСсылка.ЗначенияСвойствОбъектовИерархия"))
		   И Объект.ТипЗначения.СодержитТип(Тип("СправочникСсылка.ЗначенияСвойствОбъектов")) Тогда
			...
			Если НЕ СтарыйТипЗначения.СодержитТип(Тип("СправочникСсылка.ЗначенияСвойствОбъектовИерархия")) Тогда
				...
				ТекстПредупреждения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					ВернутьСтр("ru = 'Недопустимо одновременно использовать типы значения
					           |""%1"" и
					           |""%2"".
					           |
					           |Второй тип удален.'"),
	...
	Иначе
		...
		Если СвойстваВладельца.ЭтоДополнительноеСведение <> Истина Тогда
			ШаблонУточнения = ВернутьСтр("ru = 'Список значений общий с реквизитом ""%1"" набора ""%2""'");
		Иначе
			ШаблонУточнения = ВернутьСтр("ru = 'Список значений общий со сведением ""%1"" набора ""%2""'");
		КонецЕсли;
	...
	Иначе
		...
		Если СписокНаборов.Количество() > 0 Тогда
			...
			Если ЗначениеЗаполнено(Объект.НаборСвойств)
			   И СписокНаборов.Количество() < 2 Тогда
				...
				Если Объект.ЭтоДополнительноеСведение Тогда
					ШаблонУточнения = ВернутьСтр("ru = 'Сведение входит в набор: %1'");
				Иначе
					ШаблонУточнения = ВернутьСтр("ru = 'Реквизит входит в набор: %1'");
				КонецЕсли;
			...
			Иначе
				Если СписокНаборов.Количество() > 1 Тогда
					Если Объект.ЭтоДополнительноеСведение Тогда
						ШаблонУточнения = ВернутьСтр("ru = 'Общее сведение входит в %1 %2'");
					Иначе
						ШаблонУточнения = ВернутьСтр("ru = 'Общий реквизит входит в %1 %2'");
					КонецЕсли;
					
					СтрокаНаборы = ПользователиСлужебныйКлиентСервер.ПредметЦелогоЧисла(СписокНаборов.Количество(),
						"Л = ru_RU", ВернутьСтр("ru = 'набор,набора,наборов,,,,,,0'"));
				...
				Иначе
					Если Объект.ЭтоДополнительноеСведение Тогда
						ШаблонУточнения = ВернутьСтр("ru = 'Общее сведение входит в набор: %1'");
					Иначе
						ШаблонУточнения = ВернутьСтр("ru = 'Общий реквизит входит в набор: %1'");
					КонецЕсли;
		...
		Иначе
			...
			Если ЗначениеЗаполнено(Объект.НаборСвойств) Тогда
				Если Объект.ЭтоДополнительноеСведение Тогда
					ТекстКомментария = ВернутьСтр("ru = 'Сведение не входит в набор'");
				Иначе
					ТекстКомментария = ВернутьСтр("ru = 'Реквизит не входит в набор'");
				КонецЕсли;
			Иначе
				Если Объект.ЭтоДополнительноеСведение Тогда
					ТекстКомментария = ВернутьСтр("ru = 'Общее сведение не входит в наборы'");
				Иначе
					ТекстКомментария = ВернутьСтр("ru = 'Общий реквизит не входит в наборы'");
				КонецЕсли;
			КонецЕсли;
		КонецЕсли;
		...
		Если Элементы.УточнениеНаборовКомментарий.Гиперссылка Тогда
			Элементы.УточнениеНаборовКомментарий.Подсказка = ВернутьСтр("ru = 'Переход к набору'");
		Иначе
...
Процедура ЗаполнитьСписокДействийПриДобавленииРеквизита()
	...
	Если ПереданныеПараметрыФормы.ЭтоДополнительноеСведение Тогда
		ДобавитьОбщий = ВернутьСтр("ru = 'Добавить общее сведение в набор (рекомендуется)
			|
		...
		СделатьОбщим = ВернутьСтр("ru = 'Сделать дополнительное сведение общим и добавить в набор
			|
			|Этот вариант подходит для тех случаев, когда сведение должно быть одинаково для обоих наборов.
			|В этом случае будет возможно отбирать по нему данные разных типов в списках и отчетах.'");
		СделатьПоОбразцу = ВернутьСтр("ru = 'Сделать копию сведения по образцу (с общим списком значений)
			|
		...
		СоздатьКопированием = ВернутьСтр("ru = 'Сделать копию сведения
			|
			|Будет создана копия сведения%1'");
	Иначе
		ДобавитьОбщий = ВернутьСтр("ru = 'Добавить общий реквизит в набор (рекомендуется)
			|
			|Выбранный реквизит уже входит в несколько наборов, поэтому рекомендуется также включить его в этот набор ""как есть"".
			|В этом случае будет возможно отбирать по нему данные разных типов в списках и отчетах.'");
		СделатьОбщим = ВернутьСтр("ru = 'Сделать реквизит общим и добавить в набор
			|
			|Этот вариант подходит для тех случаев, когда реквизит должен быть одинаков для обоих наборов.
			|В этом случае будет возможно отбирать по нему данные разных типов в списках и отчетах.'");
		СделатьПоОбразцу = ВернутьСтр("ru = 'Сделать копию реквизита по образцу (с общим списком значений)
			|
		...
		СоздатьКопированием = ВернутьСтр("ru = 'Сделать копию реквизита
			|
	...
	Если РеквизитСоСпискомДополнительныхЗначений Тогда
		ШаблонВставки = " " + ВернутьСтр("ru = 'и всех его значений.'");
	Иначе
		ШаблонВставки = ".";
	КонецЕсли;
...
Процедура УстановитьЗаголовкиГиперссылок()
	...
	ШаблонЗависимостьЗадана = ВернутьСтр("ru = 'при заданном условии'");
	ШаблонЗависимостьНеЗадана = ВернутьСтр("ru = 'всегда'");
...
Процедура УстановитьЗаголовокФормы()
	...
	Если ЗначениеЗаполнено(Объект.Ссылка) Тогда
		...
		Если ЗначениеЗаполнено(Объект.НаборСвойств) Тогда
			Если Объект.ЭтоДополнительноеСведение Тогда
				Заголовок = Строка(Объект.Заголовок) + " " + ВернутьСтр("ru = '(Дополнительное сведение)'");
			Иначе
				Заголовок = Строка(Объект.Заголовок) + " " + ВернутьСтр("ru = '(Дополнительный реквизит)'");
			КонецЕсли;
		Иначе
			Если Объект.ЭтоДополнительноеСведение Тогда
				Заголовок = Строка(Объект.Заголовок) + " " + ВернутьСтр("ru = '(Общее дополнительное сведение)'");
			Иначе
				Заголовок = Строка(Объект.Заголовок) + " " + ВернутьСтр("ru = '(Общий дополнительный реквизит)'");
			КонецЕсли;
		КонецЕсли;
	Иначе
		Если ЗначениеЗаполнено(Объект.НаборСвойств) Тогда
			Если Объект.ЭтоДополнительноеСведение Тогда
				Заголовок = ВернутьСтр("ru = 'Дополнительное сведение (создание)'");
			Иначе
				Заголовок = ВернутьСтр("ru = 'Дополнительный реквизит (создание)'");
			КонецЕсли;
		Иначе
			Если Объект.ЭтоДополнительноеСведение Тогда
				Заголовок = ВернутьСтр("ru = 'Общее дополнительное сведение (создание)'");
			Иначе
				Заголовок = ВернутьСтр("ru = 'Общий дополнительный реквизит (создание)'");
			КонецЕсли;
		КонецЕсли;
	КонецЕсли;
		   ...
		   И УправлениеСвойствамиСлужебный.ТипЗначенияСодержитЗначенияСвойств(Выборка.ТипЗначения) Тогда
			...
			Если ПервыеЗначения.Количество() = 0 Тогда
				Если РезультатыЗапроса[1].Пустой() Тогда
					ПредставлениеЗначений = ВернутьСтр("ru = 'Значения еще не введены'");
				Иначе
					ПредставлениеЗначений = ВернутьСтр("ru = 'Значения помечены на удаление'");
				КонецЕсли;
			Иначе
...
Функция НаименованиеУжеИспользуется(Знач Заголовок, Знач ТекущееСвойство, Знач НаборСвойств, НовоеНаименование)
	...
	Если ЗначениеЗаполнено(Выборка.НаборСвойств) Тогда
		Если Выборка.ЭтоДополнительноеСведение Тогда
			ТекстВопроса = ВернутьСтр("ru = 'Существует дополнительное сведение с наименованием
			                          |""%1"".'");
		Иначе
			ТекстВопроса = ВернутьСтр("ru = 'Существует дополнительный реквизит с наименованием
			                          |""%1"".'");
		КонецЕсли;
	Иначе
		Если Выборка.ЭтоДополнительноеСведение Тогда
			ТекстВопроса = ВернутьСтр("ru = 'Существует общее дополнительное сведение с наименованием
			                          |""%1"".'");
		Иначе
			ТекстВопроса = ВернутьСтр("ru = 'Существует общий дополнительный реквизит с наименованием
			                          |""%1"".'");
		КонецЕсли;
	КонецЕсли;
	...
	ТекстВопроса = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
		ТекстВопроса + Символы.ПС + Символы.ПС
		                         + ВернутьСтр("ru ='Рекомендуется использовать другое наименование,
...
Процедура УстановитьЗаголовокКнопкиФормата(Форма)
	...
	Если ПустаяСтрока(Форма.Объект.ФорматСвойства) Тогда
		ТекстЗаголовка = ВернутьСтр("ru = 'Формат по умолчанию'");
	Иначе
		ТекстЗаголовка = ВернутьСтр("ru = 'Формат установлен'");
	КонецЕсли;
