...
Процедура НастроитьАвтоматическиСейчасПослеСохранения()
	...
	Организация = КонтекстЭДОКлиент.ОпределитьОрганизациюПоУчетнойЗаписи(Объект.Ссылка);
...
Процедура НастроитьАвтоматическиСейчасПослеПолученияПараметровОрганизации(ПараметрыОрганизации)
	...
	КонтекстЭДОКлиент.АвтоматическаяНастройкаУчетнойЗаписи(Объект.Ссылка, ПараметрыОрганизации, ОписаниеОповещения, Истина, , Истина, , Истина);
...
Процедура ОбработкаОповещения(ИмяСобытия, Параметр, Источник)
	...
	Если ИмяСобытия = "Получение контекста ЭДО" И КонтекстЭДОКлиент <> Неопределено И ТипЗнч(Параметр) = Тип("Структура") Тогда
...
Процедура ОбновитьФорму()
	...
	Если КонтекстЭДОКлиент <> Неопределено Тогда
		КонтекстЭДОКлиент.ОтобразитьСертификатВПолеВвода(Объект.ЭлектроннаяПодписьВМоделиСервиса, Элементы.СертификатРуководителяПредставление, Объект.СертификатРуководителя, СертификатРуководителяПредставление);
		КонтекстЭДОКлиент.ОтобразитьСертификатВПолеВвода(Объект.ЭлектроннаяПодписьВМоделиСервиса, Элементы.СертификатГлавногоБухгалтераПредставление, Объект.СертификатГлавногоБухгалтера, СертификатГлавногоБухгалтераПредставление);
		КонтекстЭДОКлиент.ОтобразитьСертификатВПолеВвода(Объект.ЭлектроннаяПодписьВМоделиСервиса, Элементы.СертификатШифрованиеПредставление, Объект.СертификатДляШифрования, СертификатШифрованиеПредставление);
	КонецЕсли;
	...
	Если Объект.ПредназначенаДляДокументооборотаСФСГС Тогда
		...
		//Если Объект.ОбменНапрямую Тогда
			//Объект.ОбменНапрямуюСФСГС = Истина;
		//КонецЕсли;
	Иначе
		//Объект.ОбменНапрямуюСФСГС = Ложь;
	КонецЕсли;
...
Процедура УстановитьВидимостьКнопкиЛицензия()
	...
	СпецоператорПоддерживаетВторичныеЗаявления = КонтекстЭДОКлиент.ПолучитьПараметрСпецоператора(Объект.СпецоператорСвязи, "ПоддерживаетсяОтправкаВторичногоЗаявления") = "Истина";
...
Процедура ПриОткрытии(Отказ)
	...
	Если Не Объект.ЭлектроннаяПодписьВМоделиСервиса Тогда
		...
		Если ДокументооборотСКОКлиент.Инициализация_ПодключитьВК(ПутьВК) Тогда
		...
		Иначе
			...
			Если ВебБраузер = "FIREFOX" Тогда
				// В файрфоксе форму не открываем, только предлагаем утсановить компоненту
				Отказ = Истина;
				ДокументооборотСКОКлиент.ИнициализацияВК();
			Иначе
				// В других браузерах предлагаем установку компоненты в форме
				ПодключитьОбработчикОжидания("Подключаемый_ИнициализироватьУстановкуВнешнейКомпоненты", 1, Истина);
			КонецЕсли;
...
&НаКлиенте
Процедура Подключаемый_ИнициализироватьУстановкуВнешнейКомпоненты()
	
	ОписаниеОповещения = Новый ОписаниеОповещения("ИнициализироватьУстановкуВнешнейКомпонентыЗавершение", ЭтотОбъект);
	ДокументооборотСКОКлиент.ИнициализацияВК(ОписаниеОповещения);
	
КонецПроцедуры

&НаКлиенте
Процедура ИнициализироватьУстановкуВнешнейКомпонентыЗавершение(Результат, ДополнительныеПараметры) Экспорт
...
Процедура СертификатРуководителяПредставлениеОчистка(Элемент, СтандартнаяОбработка)
	...
	//ЗаполнитьСпискиВыбора(ЭтаФорма);
...
Процедура СертификатРуководителяПредставлениеОткрытие(Элемент, СтандартнаяОбработка)
	...
	Если ЗначениеЗаполнено(Объект.СертификатРуководителя) Тогда
		КонтекстЭДОКлиент.ПоказатьСертификат(Объект.ЭлектроннаяПодписьВМоделиСервиса, Объект.СертификатРуководителя, "MY");
	КонецЕсли;
...
Процедура СертификатГлавногоБухгалтераПредставлениеОткрытие(Элемент, СтандартнаяОбработка)
	...
	Если ЗначениеЗаполнено(Объект.СертификатГлавногоБухгалтера) Тогда
		КонтекстЭДОКлиент.ПоказатьСертификат(Объект.ЭлектроннаяПодписьВМоделиСервиса, Объект.СертификатГлавногоБухгалтера, "MY");
	КонецЕсли;
...
Процедура СертификатШифрованиеПредставлениеОткрытие(Элемент, СтандартнаяОбработка)
	...
	Если ЗначениеЗаполнено(Объект.СертификатДляШифрования) Тогда
		КонтекстЭДОКлиент.ПоказатьСертификат(Объект.ЭлектроннаяПодписьВМоделиСервиса, Объект.СертификатДляШифрования, "MY");
	КонецЕсли;
...
Процедура СертификатРуководителяПредставлениеНачалоВыбора(Элемент, ДанныеВыбора, СтандартнаяОбработка)
	...
	ОписаниеОповещения = Новый ОписаниеОповещения("СертификатРуководителяПредставлениеНачалоВыбораЗавершение", ЭтотОбъект, ДополнительныеПараметры);	
	КонтекстЭДОКлиент.ВыбратьСертификат(Объект.ЭлектроннаяПодписьВМоделиСервиса, Объект.СертификатРуководителя,,,, ОписаниеОповещения);
	
КонецПроцедуры

&НаКлиенте
Процедура СертификатРуководителяПредставлениеНачалоВыбораЗавершение(РезультатВыбора, ДополнительныеПараметры) Экспорт
	
	Элемент = ДополнительныеПараметры.Элемент;
	
	Если РезультатВыбора <> Неопределено Тогда
		Объект.СертификатРуководителя = РезультатВыбора.Отпечаток;
		КонтекстЭДОКлиент.ОтобразитьСертификатВПолеВвода(Объект.ЭлектроннаяПодписьВМоделиСервиса, Элементы.СертификатРуководителяПредставление, Объект.СертификатРуководителя, СертификатРуководителяПредставление);
		Модифицированность = Истина;
	КонецЕсли;
...
Процедура СертификатГлавногоБухгалтераПредставлениеНачалоВыбора(Элемент, ДанныеВыбора, СтандартнаяОбработка)
	...
	ДополнительныеПараметры = Новый Структура("Элемент", Элемент);
	ОписаниеОповещения = Новый ОписаниеОповещения("СертификатГлавногоБухгалтераПредставлениеНачалоВыбораЗавершение", ЭтотОбъект, ДополнительныеПараметры);	
	КонтекстЭДОКлиент.ВыбратьСертификат(Объект.ЭлектроннаяПодписьВМоделиСервиса, Объект.СертификатГлавногоБухгалтера,,,, ОписаниеОповещения);
	
КонецПроцедуры

&НаКлиенте
Процедура СертификатГлавногоБухгалтераПредставлениеНачалоВыбораЗавершение(РезультатВыбора, ДополнительныеПараметры) Экспорт
	
	Элемент = ДополнительныеПараметры.Элемент;
	
	Если РезультатВыбора <> Неопределено Тогда
		Объект.СертификатГлавногоБухгалтера = РезультатВыбора.Отпечаток;
		КонтекстЭДОКлиент.ОтобразитьСертификатВПолеВвода(Объект.ЭлектроннаяПодписьВМоделиСервиса, Элементы.СертификатГлавногоБухгалтераПредставление, Объект.СертификатГлавногоБухгалтера, СертификатГлавногоБухгалтераПредставление);
		Модифицированность = Истина;
	КонецЕсли;
...
Процедура СертификатШифрованиеПредставлениеНачалоВыбора(Элемент, ДанныеВыбора, СтандартнаяОбработка)
	...
	ДополнительныеПараметры = Новый Структура("Элемент", Элемент);
	ОписаниеОповещения = Новый ОписаниеОповещения("СертификатШифрованиеПредставлениеНачалоВыбораЗавершение", ЭтотОбъект, ДополнительныеПараметры);	
	КонтекстЭДОКлиент.ВыбратьСертификат(Объект.ЭлектроннаяПодписьВМоделиСервиса, Объект.СертификатДляШифрования,,,, ОписаниеОповещения);
	
КонецПроцедуры

&НаКлиенте
Процедура СертификатШифрованиеПредставлениеНачалоВыбораЗавершение(РезультатВыбора, ДополнительныеПараметры) Экспорт
	
	Элемент = ДополнительныеПараметры.Элемент;
	
	Если РезультатВыбора <> Неопределено Тогда
		Объект.СертификатДляШифрования = РезультатВыбора.Отпечаток;
		КонтекстЭДОКлиент.ОтобразитьСертификатВПолеВвода(Объект.ЭлектроннаяПодписьВМоделиСервиса, Элементы.СертификатШифрованиеПредставление, Объект.СертификатДляШифрования, СертификатШифрованиеПредставление);
		Модифицированность = Истина;
	КонецЕсли;
...
Функция ПараметрыДоступаВерны()
	...
	Исключение
		...
		ЗаписьЖурналаРегистрации(
			ВернутьСтр("ru = 'Электронный документооборот с КО.Проверка параметров подключения'"),
			УровеньЖурналаРегистрации.Ошибка,,,
			ПодробноеПредставлениеОшибки(Инф));
			
		Сообщить("При проверке настроек учетной записи произошли ошибки:" + Символы.ПС + КраткоеПредставлениеОшибки(Инф), СтатусСообщения.Важное);
		Возврат Ложь;
	КонецПопытки;
...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если ЭтоНовый Тогда
		...
		ИначеЕсли РежимАвтонастройки = Перечисления.РежимыАвтонастройкиУчетнойЗаписиНалогоплательщика.Включена
		ИЛИ РежимАвтонастройки = Перечисления.РежимыАвтонастройкиУчетнойЗаписиНалогоплательщика.ВключенаВТихомРежиме Тогда
			//РезультатНастройки = КонтекстЭДО.АвтонастройкаНовойУчетнойЗаписи(Объект);
...
Процедура ОтметитьПользователейЗавершение(Список, ДополнительныеПараметры) Экспорт
	...
	Если НЕ ТекущийПользовательВСписке.Пометка Тогда 
		ТекстВопроса = ВернутьСтр("ru = 'Текущий пользователь был удален из списка пользователей учетных записей.
...
//Процедура ЗаполнитьСпискиВыбора(Форма)
//	
//	СписокВыбора = Форма.Элементы.СертификатШифрованиеПредставление.СписокВыбора;
//	СписокВыбора.Очистить();
//	Если НЕ ПустаяСтрока(Форма.Объект.СертификатРуководителя) Тогда
//		СписокВыбора.Добавить("Для целей шифрования использовать сертификат руководителя");
//	КонецЕсли;
//	Если НЕ ПустаяСтрока(Форма.Объект.СертификатГлавногоБухгалтера) Тогда
//		СписокВыбора.Добавить("Для целей шифрования использовать сертификат главного бухгалтера");
//	КонецЕсли;
//	
//КонецПроцедуры

//&НаКлиенте
//Процедура СертификатШифрованиеПредставлениеПриИзменении(Элемент)
//	
//	ПриЛюбомИзмененииСертификатШифрованиеПредставление();
//		
//КонецПроцедуры

//&НаКлиенте
//Процедура ПриЛюбомИзмененииСертификатШифрованиеПредставление()
//	
//	ТекстВыбора = Элементы.СертификатШифрованиеПредставление.ТекстРедактирования;
//		
//	Если ТекстВыбора = "Для целей шифрования использовать сертификат руководителя" Тогда
//		Объект.СертификатДляШифрования = Объект.СертификатРуководителя;
//		КонтекстЭДОКлиент.ОтобразитьСертификатВПолеВвода(Элементы.СертификатШифрованиеПредставление, Объект.СертификатДляШифрования, СертификатШифрованиеПредставление);
//	ИначеЕсли ТекстВыбора = "Для целей шифрования использовать сертификат главного бухгалтера" Тогда
//		Объект.СертификатДляШифрования = Объект.СертификатГлавногоБухгалтера;
//		КонтекстЭДОКлиент.ОтобразитьСертификатВПолеВвода(Элементы.СертификатШифрованиеПредставление, Объект.СертификатДляШифрования, СертификатШифрованиеПредставление);	
//	КонецЕсли;
//		
//КонецПроцедуры