...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	// { RGS ASeryakov 14.04.2018 S-I-0005278
	УстановитьВидимостьГруппаПарольДляСуществующего();
	// { RGS ASeryakov 14.04.2018 S-I-0005278
...
Процедура ПередЗаписьюНаСервере(Отказ, ТекущийОбъект, ПараметрыЗаписи)
	...
	МассивBorgsТЧ = ТекущийОбъект.Borgs.Выгрузить().ВыгрузитьКолонку("Borg");
	МассивBorgsСписок = Borgs.ВыгрузитьЗначения();
	Если НЕ РГСофтКлиентСервер.МассивыИдентичны(МассивBorgsТЧ,МассивBorgsСписок) Тогда
		ТекущийОбъект.Borgs.Очистить();
		Для Каждого Строка Из МассивBorgsСписок Цикл
			СтрокаБорг = ТекущийОбъект.Borgs.Добавить();
			СтрокаБорг.Borg = Строка;
		КонецЦикла;
	КонецЕсли;
	// } RGS AArsentev 21.08.2017 S-I-0003524
...
Процедура ПослеЗаписиНаСервере(ТекущийОбъект, ПараметрыЗаписи)
	...
	Если ТребуетсяЗаписьПользователяИБ(ЭтотОбъект) Тогда
		ПараметрыЗаписи.Вставить(
			ТекущийОбъект.ДополнительныеСвойства.ОписаниеПользователяИБ.Действие);
	КонецЕсли;
	...
	Если ПользовательИБ <> Неопределено Тогда
		...
		Если ЗначениеЗаполнено(Объект.CCA) Тогда
			
			ПользовательИБ.АутентификацияСтандартная = Истина;
			ПользовательИБ.ПоказыватьВСпискеВыбора = Ложь;
			
			ПользовательИБ.Роли.Добавить(Метаданные.Роли.ImportExportBroker);
		...
		//Устранаем различия в свойствах пользователя и пользователя ИБ
	...
	Иначе
		...
		Сообщение.Текст = "На найден пользователь базы данных для создаваемого пользователя!";
		Сообщение.Сообщить();
...
Процедура УстановитьУсловноеОформление()
	...
	// { RGS AArsentev 21.08.2017 S-I-0003524
	МассивBorgs = Объект.Borgs.Выгрузить().ВыгрузитьКолонку("Borg");
	Borgs.ЗагрузитьЗначения(МассивBorgs);
	// } RGS AArsentev 21.08.2017 S-I-0003524
...
Процедура ОбщаяНастройкаФормы(ПриСозданииНаСервере = Ложь, ПараметрыЗаписи = Неопределено)
	...
	Элементы.Alias.ТолькоПросмотр =
		Не (ДействияВФорме.СвойстваЭлемента = "Редактирование" И УровеньДоступа.УправлениеСписком);
	
	Элементы.Недействителен.ТолькоПросмотр = Элементы.Alias.ТолькоПросмотр;
	//Элементы.ФизическоеЛицо.ТолькоПросмотр = Элементы.Наименование.ТолькоПросмотр; //закомментировано rg-soft
	//Элементы.Подразделение.ТолькоПросмотр  = Элементы.Наименование.ТолькоПросмотр; //закомментировано rg-soft
...
Функция ОписаниеПользователяИБ(ДляПроверкиПервогоАдминистратора = Ложь)
	...
	Если УровеньДоступа.УправлениеСписком
	   И ДействияВФорме.СвойстваЭлемента = "Редактирование" Тогда
		...
		ПользовательИБПолноеИмя = Объект.Код;
	КонецЕсли;
...
Процедура УстановитьДоступностьСвойств(Форма)
	...
	Элементы.ГруппаИзменитьОграничение.Доступность = Не Объект.Недействителен
	                                               И Не Элементы.Alias.ТолькоПросмотр;
...
&НаКлиенте
Процедура КартинкаПодписиНачалоВыбора(Элемент, ДанныеВыбора, СтандартнаяОбработка)
	
	СтандартнаяОбработка = Ложь;
	
	ПараметрыФормы = Новый Структура("ВладелецФайла, ТолькоПросмотр, РежимВыбора", Объект.Ссылка, Ложь, Истина);
	
	Объект.КартинкаПодписи = ОткрытьФормуМодально("ОбщаяФорма.ПрисоединенныеФайлы", ПараметрыФормы, ЭтаФорма);
	
КонецПроцедуры

// { RGS ASeryakov 14.04.2018 S-I-0005278
&НаСервере
Процедура УстановитьВидимостьГруппаПарольДляСуществующего()
	...
	Если НЕ Объект.Ссылка.Пустая() Тогда
		...
		Если НЕ ПользовательИБ = Неопределено Тогда
			...
			Если ЗначениеЗаполнено(Объект.CCA) И ПользовательИБ.Роли.Содержит(Метаданные.Роли.ImportExportBroker) Тогда
				
				УстановитьВидимостьГруппаПароль(Истина);
			Иначе;
				УстановитьВидимостьГруппаПароль(Ложь);
			КонецЕсли;
...
&НаСервере
Процедура УстановитьВидимостьГруппаПароль(УстановитьПароль)
	...
	Если УстановитьПароль Тогда
		...
		Элементы.СвойстваПользователяИБ.Видимость = Истина;
		
		Для каждого Страница Из Элементы.СвойстваПользователяИБ.ПодчиненныеЭлементы Цикл
			Страница.Видимость = Ложь;
		КонецЦикла;
		
		Элементы.ОсновныеСвойства.Видимость = Истина;
		
		Для каждого Элемент Из Элементы.ОсновныеСвойства.ПодчиненныеЭлементы Цикл
			Элемент.Видимость = Ложь;
		КонецЦикла;
		
		Элементы.СвойстваАутентификации1СПредприятия.Видимость = Истина;
		
		Для каждого Элемент Из Элементы.СвойстваАутентификации1СПредприятия.ПодчиненныеЭлементы Цикл
			Элемент.Видимость = Ложь;
		КонецЦикла;
		
		Элементы.ГруппаОтступ.Видимость = Истина;
		
		Для каждого Элемент Из Элементы.ГруппаОтступ.ПодчиненныеЭлементы Цикл
			Элемент.Видимость = Ложь;
		КонецЦикла;
		
		Элементы.ПараметрыАутентификации1СПредприятия.Видимость = Истина;
		Элементы.ПараметрыАутентификации1СПредприятия.Доступность = Истина;
		
		Для каждого Элемент Из Элементы.ПараметрыАутентификации1СПредприятия.ПодчиненныеЭлементы Цикл
			Элемент.Видимость = Ложь;
		КонецЦикла;
...
КонецПроцедуры // УстановитьВидимостьГруппаПароль()

&НаКлиенте
Процедура CCAПриИзменении(Элемент)
	
	УстановитьВидимостьГруппаПароль(ЗначениеЗаполнено(Объект.CCA));
