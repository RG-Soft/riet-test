...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если НЕ Пользователи.ЭтоПолноправныйПользователь(, Не РазделениеВключено) Тогда
		ВызватьИсключение ВернутьСтр("ru = 'Недостаточно прав для открытия списка пользователей информационной базы.'");
	КонецЕсли;
...
Процедура ОтменитьСопоставление(Команда)
	...
	Кнопки.Добавить("ОтменитьСопоставление", ВернутьСтр("ru = 'Отменить сопоставление'"));
	Кнопки.Добавить("ОставитьСопоставление", ВернутьСтр("ru = 'Оставить сопоставление'"));
	
	ПоказатьВопрос(
		Новый ОписаниеОповещения("ОтменитьСопоставлениеПродолжение", ЭтотОбъект),
		ВернутьСтр("ru = 'Отмена сопоставления пользователя информационной базы с пользователем в справочнике.
		           |
...
Процедура УстановитьУсловноеОформление()
	...
	Элемент.Оформление.УстановитьЗначениеПараметра("Текст", ВернутьСтр("ru = '<Нет данных>'"));
...
Процедура УдалитьТекущегоПользователяИБ(УдалитьСтроку = Ложь)
	...
	ПоказатьВопрос(
		...
		ВернутьСтр("ru = 'Удалить пользователя информационной базы?'"),
