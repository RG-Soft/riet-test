...
Процедура ПередЗаписью(Отказ)
	...
	Если Ссылка = Справочники.ГруппыДоступа.Администраторы Тогда
		...
		Если НЕ ПривилегированныйРежим()
		   И НЕ УправлениеДоступом.ЕстьРоль("ПолныеПрава") Тогда
			...
			ВызватьИсключение
				ВернутьСтр("ru = 'Предопределенную группу доступа Администраторы
		...
		Для каждого ТекущаяСтрока Из ЭтотОбъект.Пользователи Цикл
			Если ТипЗнч(ТекущаяСтрока.Пользователь) <> Тип("СправочникСсылка.Пользователи") Тогда
				ВызватьИсключение
					ВернутьСтр("ru = 'Предопределенная группа доступа Администраторы
	...
	ИначеЕсли Профиль = Справочники.ПрофилиГруппДоступа.Администратор Тогда
		ВызватьИсключение
			ВернутьСтр("ru = 'Предопределенный профиль Администратор может быть только
...
Процедура ПриЗаписи(Отказ)
	...
	Для Каждого СтрокаПользователя из Пользователи Цикл 
		...
		Если ТипЗнч(Пользователь) = Тип("СправочникСсылка.Пользователи")
			И УправлениеДоступом.ЕстьРоль("ImportExportBroker", , Пользователь) 
			И Не УправлениеДоступом.ЕстьРоль("ПолныеПрава", , Пользователь) 
			И Не ЗначениеЗаполнено(ОбщегоНазначения.ЗначениеРеквизитаОбъекта(Пользователь, "CCA")) Тогда 
			ОбщегоНазначенияКлиентСервер.СообщитьПользователю(
			"For User " + СокрЛП(Пользователь) + " CCA is empty!",
			ЭтотОбъект, , , Отказ);
		КонецЕсли;  
	КонецЦикла; // } RG-Soft - конец добавления
