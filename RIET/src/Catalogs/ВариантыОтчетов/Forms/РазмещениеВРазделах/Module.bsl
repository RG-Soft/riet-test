...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	СмешаннаяВажность = ВернутьСтр("ru = 'Различная'");
...
Процедура Разместить(Команда)
	Если ПроверитьКоличествоВариантов() Тогда
		ЗаписатьНаСервере();
		ТекстОповещения = ВернутьСтр("ru = 'Изменены настройки вариантов отчетов (%1 шт.).'");
...
Функция ПроверитьКоличествоВариантов()
	...
	Если КоличествоВариантов = 0 Тогда
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(
			ВернутьСтр("ru = 'Необходимо заполнить список ""Варианты отчетов""'"),
			,
			"ИзменяемыеВарианты");
		Возврат Ложь;
	КонецЕсли;
...
Процедура ПерезаполнитьДерево(ТолькоСнятьФлажки)
	...
	Если КоличествоВариантов = 0 Тогда
		ТекстСообщения = ВернутьСтр("ru = 'Необходимо выбрать варианты отчетов'");
	...
	Если КоличествоОшибок > 0 Тогда
		...
		Для Каждого СтрокаТаблицы Из ОтфильтрованныеВарианты Цикл
			Если ТекущаяПричина <> СтрокаТаблицы.Причина Тогда
				...
				Если ТекущаяПричина = 1 Тогда
					РезультатВыполнения.Подробно = РезультатВыполнения.Подробно + ВернутьСтр("ru = 'Помеченные на удаление:'");
				ИначеЕсли ТекущаяПричина = 2 Тогда
					РезультатВыполнения.Подробно = РезультатВыполнения.Подробно + ВернутьСтр("ru = 'Недостаточно прав для изменения:'");
				ИначеЕсли ТекущаяПричина = 3 Тогда
					РезультатВыполнения.Подробно = РезультатВыполнения.Подробно + ВернутьСтр("ru = 'Отчет отключен или недоступен по правам:'");
				ИначеЕсли ТекущаяПричина = 4 Тогда
					РезультатВыполнения.Подробно = РезультатВыполнения.Подробно + ВернутьСтр("ru = 'Вариант отчета отключен по функциональной опции:'");
				КонецЕсли;
			КонецЕсли;
		...
		Если КоличествоВариантов = 0 Тогда
			РезультатВыполнения.Текст = ВернутьСтр("ru = 'Недостаточно прав для размещения в разделах выбранных вариантов отчетов.'");
		...
		Иначе
			РезультатВыполнения.Текст = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				ВернутьСтр("ru = 'Недостаточно прав для размещения в разделах некоторых вариантов отчетов (%1).'"),
