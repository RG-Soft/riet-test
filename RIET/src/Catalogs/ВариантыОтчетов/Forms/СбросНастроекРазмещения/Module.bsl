...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если Не Параметры.Свойство("МассивВариантов") Или ТипЗнч(Параметры.МассивВариантов) <> Тип("Массив") Тогда
		ТекстОшибки = ВернутьСтр("ru = 'Не указаны варианты отчетов.'");
		Возврат;
	КонецЕсли;
...
Процедура КомандаСбросить(Команда)
	...
	Если КоличествоВыбранныхВариантов = 0 Тогда
		ПоказатьПредупреждение(, ВернутьСтр("ru = 'Не указаны варианты отчетов.'"));
		Возврат;
	КонецЕсли;
	...
	Если КоличествоВариантов = 1 И КоличествоВыбранныхВариантов = 1 Тогда
		...
		ОповещениеЗаголовок = ВернутьСтр("ru = 'Сброшены настройки размещения варианта отчета'");
	...
	Иначе
		ОповещениеТекст = ВернутьСтр("ru = 'Сброшены настройки размещения
...
Процедура Отфильтровать()
	...
	Если КоличествоДоФильтрации <> КоличествоПослеФильтрации Тогда
		Если КоличествоПослеФильтрации = 0 Тогда
			ТекстОшибки = ВернутьСтр("ru = 'Сброс настроек размещения выбранных вариантов отчетов не требуется по одной или нескольким причинам:
