...
Процедура ОформитьНаборыИСвойства()
	...
	ЭлементЦветаОформления.Значение = ВернутьСтр("ru = 'Наборы'");
...
Процедура НастроитьОтображениеНаборов()
	...
	Если ЭтоНаборыДополнительныхСведений Тогда
		Заголовок = ВернутьСтр("ru = 'Дополнительные сведения'");
		
		КомандаСоздать.Подсказка          = ВернутьСтр("ru = 'Создать уникальное сведение'");
		КомандаСоздать.Заголовок          = ВернутьСтр("ru = 'Новое'");
		КомандаСоздать.Подсказка          = ВернутьСтр("ru = 'Создать уникальное сведение'");
		
		КомандаСкопировать.Подсказка        = ВернутьСтр("ru = 'Создать новое сведение копированием текущего'");
		КомандаИзменить.Подсказка           = ВернутьСтр("ru = 'Изменить (или открыть) текущее сведение'");
		КомандаПометитьНаУдаление.Подсказка = ВернутьСтр("ru = 'Пометить текущее сведение на удаление (Del)'");
		КомандаПереместитьВверх.Подсказка   = ВернутьСтр("ru = 'Переместить текущее сведение вверх'");
		КомандаПереместитьВниз.Подсказка    = ВернутьСтр("ru = 'Переместить текущее сведение вниз'");
		...
		Элементы.СвойстваТипЗначения.Подсказка =
			ВернутьСтр("ru = 'Типы значения, которое можно ввести при заполнении сведения.'");
		
		Элементы.СвойстваОбщиеЗначения.Подсказка =
			ВернутьСтр("ru = 'Сведение использует список значений сведения-образца.'");
		
		Элементы.СвойстваОбщее.Заголовок = ВернутьСтр("ru = 'Общее'");
		Элементы.СвойстваОбщее.Подсказка = ВернутьСтр("ru = 'Общее дополнительное сведение, которое используется в
		                                              |нескольких наборах дополнительных сведений.'");
	Иначе
		Заголовок = ВернутьСтр("ru = 'Дополнительные реквизиты'");
		КомандаСоздать.Заголовок          = ВернутьСтр("ru = 'Новый'");
		КомандаСоздать.Подсказка          = ВернутьСтр("ru = 'Создать уникальный реквизит'");
		
		КомандаСкопировать.Подсказка        = ВернутьСтр("ru = 'Создать новый реквизит копированием текущего'");
		КомандаИзменить.Подсказка           = ВернутьСтр("ru = 'Изменить (или открыть) текущий реквизит'");
		КомандаПометитьНаУдаление.Подсказка = ВернутьСтр("ru = 'Пометить текущий реквизит на удаление (Del)'");
		КомандаПереместитьВверх.Подсказка   = ВернутьСтр("ru = 'Переместить текущий реквизит вверх'");
		КомандаПереместитьВниз.Подсказка    = ВернутьСтр("ru = 'Переместить текущий реквизит вниз'");
		...
		Элементы.СвойстваТипЗначения.Подсказка =
			ВернутьСтр("ru = 'Типы значения, которое можно ввести при заполнении реквизита.'");
		
		Элементы.СвойстваОбщиеЗначения.Подсказка =
			ВернутьСтр("ru = 'Реквизит использует список значений реквизита-образца.'");
		
		Элементы.СвойстваОбщее.Заголовок = ВернутьСтр("ru = 'Общий'");
		Элементы.СвойстваОбщее.Подсказка = ВернутьСтр("ru = 'Общий дополнительный реквизит, который используется в
	...
	Если Элементы.Свойства.ТекущиеДанные <> Неопределено Тогда
		...
		Если ЭтоНаборыДополнительныхСведений Тогда
			Если Элементы.Свойства.ТекущиеДанные.Общее Тогда
				ТекстВопроса = ВернутьСтр("ru ='Исключить текущее общее сведения из набора?'");
				
			ИначеЕсли Элементы.Свойства.ТекущиеДанные.ПометкаУдаления Тогда
				ТекстВопроса = ВернутьСтр("ru ='Снять с текущего сведения пометку на удаление?'");
			Иначе
				ТекстВопроса = ВернутьСтр("ru ='Пометить текущее сведение на удаление?'");
			КонецЕсли;
		Иначе
			Если Элементы.Свойства.ТекущиеДанные.Общее Тогда
				ТекстВопроса = ВернутьСтр("ru ='Исключить текущий общий реквизит из набора?'");
				
			ИначеЕсли Элементы.Свойства.ТекущиеДанные.ПометкаУдаления Тогда
				ТекстВопроса = ВернутьСтр("ru ='Снять с текущего реквизита пометку на удаление?'");
			Иначе
				ТекстВопроса = ВернутьСтр("ru ='Пометить текущий реквизит на удаление?'");
			КонецЕсли;
		КонецЕсли;
	...
	Попытка
		...
		Попытка
			...
			Если ТекущийНаборОбъект.ВерсияДанных <> ТекущийНаборВерсияДанных Тогда
				...
				Если ЭтоНаборыДополнительныхСведений Тогда
					ВызватьИсключение
						ВернутьСтр("ru = 'Действие не выполнено, так как состав дополнительных сведений
				...
				Иначе
					ВызватьИсключение
						ВернутьСтр("ru = 'Действие не выполнено, так как состав дополнительных реквизитов
						           |был изменен другим пользователем.
