...
Процедура ОбновитьСтраныМираПоКлассификатору(Знач Добавлять = Ложь) Экспорт
	...
	Для Каждого СтрокаКлассификатора Из ТаблицаКлассификатора() Цикл
		...
		Выборка = Справочники.КлассификаторСтранМира.Выбрать(,,Фильтр);
		...
		Исключение
			...
			ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				ВернутьСтр("ru='Ошибка записи страны мира %1 (код %2) при обновлении классификатора, %3'"),
...
Функция СсылкаПоДаннымКлассификатора(Знач Отбор, Знач ДополнительныеДанные = Неопределено) Экспорт
	...
	Если ДанныеПоиска=Неопределено Тогда
		ВызватьИсключение ВернутьСтр("ru='Некорректный код страны мира для поиска в классификаторе'");
	КонецЕсли;
...
Функция ПоляПоиска() Экспорт
	...
	СписокПолей = Справочники.КлассификаторСтранМира.ПустаяСсылка().Метаданные().ВводПоСтроке;
...
Процедура ОбработкаПолученияДанныхВыбора(ДанныеВыбора, Параметры, СтандартнаяОбработка)
	...
	Иначе
		...
		ШаблонЗапроса = "
			...
			|	ЕСТЬNULL(КлассификаторСтранМира.Код, Классификатор.Код)                   КАК Код,
			|	ЕСТЬNULL(КлассификаторСтранМира.Наименование, Классификатор.Наименование) КАК Наименование,
			|	ЕСТЬNULL(КлассификаторСтранМира.ПометкаУдаления, ЛОЖЬ)                    КАК ПометкаУдаления,
			|
			|	ВЫБОР КОГДА КлассификаторСтранМира.Ссылка ЕСТЬ NULL ТОГДА 
			|		%2 
			|	ИНАЧЕ 
			|		%3
			...
			|	Классификатор
			|ПО
			|	Классификатор.Код = КлассификаторСтранМира.Код
			|	И Классификатор.Наименование = КлассификаторСтранМира.Наименование
			|ГДЕ 
			|	(КлассификаторСтранМира.%1 ПОДОБНО &СтрокаПоиска ИЛИ Классификатор.%1 ПОДОБНО &СтрокаПоиска)
			|	И (" + СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(ШаблонОтбора, "Классификатор") + ")
			|	И (" + СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(ШаблонОтбора, "КлассификаторСтранМира") + ")
			|
			|УПОРЯДОЧИТЬ ПО
			|	ЕСТЬNULL(КлассификаторСтранМира.%1, Классификатор.%1)
			|";
	КонецЕсли;
	...
	Для Каждого ДанныеПоля Из ИменаПолей.СписокПолей Цикл
		Запрос.Текст = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(ШаблонЗапроса, 
			...
			СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(ДанныеПоля.ШаблонПредставления, "КлассификаторСтранМира"),
		);
