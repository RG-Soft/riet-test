...
Процедура СписокПередНачаломДобавления(Элемент, Отказ, Копирование, Родитель, Группа)
	...
	Если Копирование И Группа Тогда
		...
		ПоказатьПредупреждение(, ВернутьСтр("ru='Добавление новых групп в справочнике запрещено.'"));
	КонецЕсли;
...
&НаКлиенте
Процедура ПереместитьЭлементВверх() //rg-soft - подсистема не внедрена
	
	//Если Не ВозможноПеремещениеЭлемента(Элементы.Список.ТекущиеДанные.Ссылка, "Вверх") Тогда
	//	ОбщегоНазначенияКлиентСервер.СообщитьПользователю(ВернутьСтр("ru = 'Перемещение данного вида контактной информации не предусмотрено'"));
	//	Возврат;
	//КонецЕсли;
	//
...
&НаКлиенте
Процедура ПереместитьЭлементВниз() //rg-soft - подсистема не внедрена
...
&НаСервере
Функция ВозможноПеремещениеЭлемента(ТекущийЭлемент, Направление)  //rg-soft - подсистема не внедрена
