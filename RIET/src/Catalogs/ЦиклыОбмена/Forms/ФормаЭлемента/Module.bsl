...
Процедура ИнициализироватьСоответствия()
	...
	//СоответствиеКартинкиЦветуНадписи = СоответствиеКЦН;
...
Процедура ЗаполнитьДанными()
	...
	Если ЗначениеЗаполнено(Объект.Идентификатор) Тогда
		Элементы.Идентификатор.Заголовок = Объект.Идентификатор;
		Элементы.Идентификатор.ЦветТекста = Новый Цвет;
	Иначе
		Элементы.Идентификатор.Заголовок = "<отсутствует>";
		Элементы.Идентификатор.ЦветТекста = Новый Цвет(128, 128, 128);
	КонецЕсли;
			...
			Для ИндСтр = 1 По ТаблицаСообщений.Количество() Цикл
				...
				Если СтрСообщение.Тип = Перечисления.ТипыТранспортныхСообщений.РезультатПриемаДекларацияНО Тогда
					Если СтрСообщение.ПротоколСОшибкой Тогда
						...
						//Элементы.НадписьРезультатПриемаДекларацияНО.Картинка = БиблиотекаКартинок.КритическаяОшибка;
	...
	ИначеЕсли Объект.Тип = Перечисления.ТипыЦикловОбмена.ОтчетностьПФР Тогда
		...
		Для ИндСтр = 1 По ТаблицаСообщений.Количество() Цикл
			...
			Если СтрСообщение.Тип = Перечисления.ТипыТранспортныхСообщений.ПротоколПФР Тогда
				//Элементы.ПанельПротоколПФР.Видимость = Истина;
				Элементы.ГруппаПротоколПФР.Видимость = Истина;
				Элементы.ГруппаПротоколКвитанцияПФР.Видимость = Истина;					
				Если СтрСообщение.ПротоколСОшибкой Тогда
...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	// НЕ работает с типом ДанныеФормыСтруктура
	//Если ЭтоНовый() Тогда
	Если Параметры.Ключ.Пустая() Тогда
		
		ТекстСообщения = ВернутьСтр("ru='Справочник не предназначен для ручного заполнения!'");
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(ТекстСообщения);
		Отказ = Истина;
		Возврат;
...
Функция НадписьНажатиеОбработатьНаСервере(ТипСообщения)
	...
	ЗапросНаВвод = КонтекстЭДОСервер.ЗапросНаВводПодтверждения(Объект.Ссылка, Вычислить(ТипСообщения));
...
Функция ВычислитьНаСервере(ТипСообщения)
	
	Возврат Вычислить(ТипСообщения);
...
Процедура НадписьНажатиеОбработатьНаКлиентеЗавершение(РезультатВопроса, ДополнительныеПараметры) Экспорт
	...
	Если РезультатВопроса = КодВозвратаДиалога.Да Тогда
		...
		Если КонтекстЭДОКлиент = Неопределено Тогда 
			ОписаниеОповещения = Новый ОписаниеОповещения("НадписьНажатиеОбработатьНаКлиентеПослеПолученияКонтекстаЗавершение", ЭтотОбъект, ДополнительныеПараметры);
			ДокументооборотСКОКлиент.ПолучитьКонтекстЭДО(ОписаниеОповещения);
		Иначе
			Подтверждение = КонтекстЭДОКлиент.ВвестиПодтверждение(Объект.Ссылка, ВычислитьНаСервере(ТипСообщения));
			Если Подтверждение <> Неопределено Тогда 
				ПоказатьЗначение(, Подтверждение);
				ЗаполнитьДанными();
			КонецЕсли;
		КонецЕсли;
	КонецЕсли;
...
Процедура НадписьСформироватьПротоколКвитанцияПФРНажатие(Элемент)
	...
	НадписьНажатиеОбработатьНаКлиенте("Перечисления.ТипыТранспортныхСообщений.ПротоколПФР");
...
Процедура НадписьСформироватьПодтверждениеполученияНеформализованногоСообщенияПФРНажатие(Элемент)
	...
	НадписьНажатиеОбработатьНаКлиенте("Перечисления.ТипыТранспортныхСообщений.ПисьмоВходящееПФР");
...
Процедура НадписьСформироватьИзвещениеПодтверждениеНПНажатие(Элемент)
	...
	НадписьНажатиеОбработатьНаКлиенте("Перечисления.ТипыТранспортныхСообщений.ПодтверждениеДекларацияНО");
...
Процедура НадписьСформироватьИзвещениеРезультатПриемаНПНажатие(Элемент)
	...
	НадписьНажатиеОбработатьНаКлиенте("Перечисления.ТипыТранспортныхСообщений.РезультатПриемаДекларацияНО");
...
Процедура НадписьСформироватьИзвещениеРезультатОбработкиНПНажатие(Элемент)
	...
	НадписьНажатиеОбработатьНаКлиенте("Перечисления.ТипыТранспортныхСообщений.РезультатОбработкиДекларацияНО");
...
Процедура НадписьСформироватьИзвещениеПодтверждениеНП2НДФЛНажатие(Элемент)
	...
	НадписьНажатиеОбработатьНаКлиенте("Перечисления.ТипыТранспортныхСообщений.ПодтверждениеФорма2НДФЛНО");
...
Процедура НадписьСформироватьИзвещениеРезультатПриемаНП2НДФЛНажатие(Элемент)
	...
	НадписьНажатиеОбработатьНаКлиенте("Перечисления.ТипыТранспортныхСообщений.РезультатПриемаФорма2НДФЛНО");
...
Процедура НадписьСформироватьИзвещениеПодтверждениеНПОбращениеНажатие(Элемент)
	...
	НадписьНажатиеОбработатьНаКлиенте("Перечисления.ТипыТранспортныхСообщений.ИзвещениеОбращениеНО");
...
Процедура НадписьСформироватьИзвещениеРезультатПриемаНПОбращениеНажатие(Элемент)
	...
	НадписьНажатиеОбработатьНаКлиенте("Перечисления.ТипыТранспортныхСообщений.РезультатПриемаОбращениеНО");
...
Процедура НадписьСформироватьИзвещениеПисьмоНПНажатие(Элемент)
	...
	НадписьНажатиеОбработатьНаКлиенте("Перечисления.ТипыТранспортныхСообщений.ПисьмоНО");
...
Процедура НадписьСформироватьИзвещениеРассылкаНПНажатие(Элемент)
	...
	НадписьНажатиеОбработатьНаКлиенте("Перечисления.ТипыТранспортныхСообщений.РассылкаНО");
...
Процедура НадписьСформироватьИзвещениеПодтверждениеНПЗапросНажатие(Элемент)
	...
	НадписьНажатиеОбработатьНаКлиенте("Перечисления.ТипыТранспортныхСообщений.ПодтверждениеЗапросНО");
...
Процедура НадписьСформироватьИзвещениеРезультатПриемаНПЗапросНажатие(Элемент)
	...
	НадписьНажатиеОбработатьНаКлиенте("Перечисления.ТипыТранспортныхСообщений.РезультатПриемаЗапросНО");
...
Процедура НадписьСформироватьИзвещениеРезультатОбработкиНПЗапросНажатие(Элемент)
	...
	НадписьНажатиеОбработатьНаКлиенте("Перечисления.ТипыТранспортныхСообщений.РезультатОбработкиЗапросНО");
...
Процедура НадписьСформироватьИзвещениеДокументНПНажатие(Элемент)
	...
	НадписьНажатиеОбработатьНаКлиенте("Перечисления.ТипыТранспортныхСообщений.ДокументНО");
...
Процедура НадписьСформироватьИзвещениеПодтверждениеЗаявлениеНПНажатие(Элемент)
	...
	НадписьНажатиеОбработатьНаКлиенте("Перечисления.ТипыТранспортныхСообщений.ПодтверждениеЗаявлениеНО");
...
Процедура НадписьСформироватьИзвещениеРезультатПриемаЗаявлениеНПНажатие(Элемент)
	...
	НадписьНажатиеОбработатьНаКлиенте("Перечисления.ТипыТранспортныхСообщений.РезультатПриемаЗаявлениеНО");
...
Процедура НадписьСформироватьИзвещениеРезультатОбработкиРФНПНажатие(Элемент)
	...
	НадписьНажатиеОбработатьНаКлиенте("Перечисления.ТипыТранспортныхСообщений.РезультатОбработкиЗаявлениеРФНО");
...
Процедура НадписьСформироватьИзвещениеОбОтзывеЗаявлениеРФНПНажатие(Элемент)
	...
	НадписьНажатиеОбработатьНаКлиенте("Перечисления.ТипыТранспортныхСообщений.СообщениеОбОтзывеЗаявлениеРФНО");
...
Процедура НадписьСформироватьИзвещениеРезультатОбработкиТСНПНажатие(Элемент)
	...
	НадписьНажатиеОбработатьНаКлиенте("Перечисления.ТипыТранспортныхСообщений.РезультатОбработкиЗаявлениеТСНО");
...
Процедура НадписьСформироватьРезультатПриемаДокументНПНажатиеЗавершение(РезультатВопроса, ДополнительныеПараметры) Экспорт
	...
	Если РезультатВопроса = КодВозвратаДиалога.Да Тогда
		РезультатПриема = КонтекстЭДОКлиент.СоздатьРезультатПриемаНаКлиентеСУчетомВопроса(СообщениеОснование, Истина, Истина);
	ИначеЕсли РезультатВопроса = КодВозвратаДиалога.Нет Тогда
		РезультатПриема = КонтекстЭДОКлиент.СоздатьРезультатПриемаНаКлиентеСУчетомВопроса(СообщениеОснование, Ложь, Истина);
	Иначе
		Возврат;
	КонецЕсли;
...
Процедура НадписьСформироватьИзвещениеПодтверждениеДатыОтправкиНажатие(Элемент)
	...
	НадписьНажатиеОбработатьНаКлиенте("Перечисления.ТипыТранспортныхСообщений.ПодтверждениеДатыОтправкиФСГС");
...
Процедура НадписьСформироватьИзвещениеПротоколФСГСНажатие(Элемент)
	...
	НадписьНажатиеОбработатьНаКлиенте("Перечисления.ТипыТранспортныхСообщений.ПротоколВходногоКонтроляОтчетностиФСГС");
...
Процедура НадписьСформироватьИзвещениеПисьмоВходящееФСГСНажатие(Элемент)
	...
	НадписьНажатиеОбработатьНаКлиенте("Перечисления.ТипыТранспортныхСообщений.ПисьмоВходящееФСГС");
...
Процедура НадписьСформироватьИзвещениеПодтверждениеНППредставлениеНажатие(Элемент)
	...
	НадписьНажатиеОбработатьНаКлиенте("Перечисления.ТипыТранспортныхСообщений.ИзвещениеПредставлениеНО");
...
Процедура НадписьСформироватьИзвещениеРезультатПриемаНППредставлениеНажатие(Элемент)
	...
	НадписьНажатиеОбработатьНаКлиенте("Перечисления.ТипыТранспортныхСообщений.РезультатПриемаПредставлениеНО");
...
Процедура НадписьСформироватьОтветНаЗапросКвитанцияПФРНажатие(Элемент)
	...
	НадписьНажатиеОбработатьНаКлиенте("Перечисления.ТипыТранспортныхСообщений.ОтветНаЗапросПФР");
	
КонецПроцедуры

&НаКлиенте
Процедура НадписьСформироватьИзвещениеРезультатОбработкиЗаявлениеНПНажатие(Элемент)
	НадписьНажатиеОбработатьНаКлиенте("Перечисления.ТипыТранспортныхСообщений.РезультатОбработкиЗаявлениеНО");
КонецПроцедуры
