...
Процедура ПутьКДискуИТСНачалоВыбора(Элемент, ДанныеВыбора, СтандартнаяОбработка)
	...
	ДиалогВыбораКаталога.Заголовок = ВернутьСтр("ru = 'Укажите путь к диску ИТС'");
	...
	Если НЕ ФайлДанных.Существует() Тогда
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(
			ВернутьСтр("ru ='В указанном каталоге не обнаружены данные классификатора. Необходимо указать путь к диску 1С:ИТС, на котором содержится база ""Гарант. Налоги, бухучет, предпринимательство.""'"),
			,
			"ПутьКДискуИТС");
	КонецЕсли;
	...
	Иначе
		...
		Если ВариантЗагрузки = "ИТС" Тогда
			Если НЕ ЗначениеЗаполнено(ПутьКДискуИТС) И ОбщегоНазначенияКлиентСервер.ЭтоLinuxКлиент() Тогда
				...
				ОбщегоНазначенияКлиентСервер.СообщитьПользователю(
					ВернутьСтр("ru = 'При работе в ОС Linux необходимо явно указать путь к диску'"),
					,
					"ПутьКДискуИТС");
				Возврат;
			КонецЕсли;
...
Процедура ЗагрузитьРезультат()
	...
	Если ВариантЗагрузки = "ИТС" Тогда
		Источник = ВернутьСтр("ru ='Диск ИТС'");
	Иначе
		Источник = ВернутьСтр("ru ='Сайт РБК'");
	КонецЕсли;
	
	ПараметрыЗагрузкиКлассификатора = ПолучитьИзВременногоХранилища(АдресХранилища);
	
	ИмяСобытия = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(ВернутьСтр("ru ='Загрузка классификатора банков. %1.'"), Источник);
...
Процедура Подключаемый_ПроверитьВыполнениеЗадания()
	...
	Исключение
		ЖурналРегистрацииКлиент.ДобавитьСообщениеДляЖурналаРегистрации(ВернутьСтр("ru = 'Загрузка классификатора банков'", ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка()),
		...
		Элементы.ПоясняющийТекст.Заголовок = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			ВернутьСтр("ru = 'Загрузка классификатора банков прервана по причине:
				|%1
...
// Параметры:
	...
	Если ФайлНайден Тогда
		...
		ФайлДанных = Неопределено;
	Иначе
		ТекстСообщения = ВернутьСтр("ru ='На диске 1С:ИТС не обнаружены данные классификатора БИК РФ. 
...
// Параметры:
	...
	Иначе
		ПараметрыЗагрузкиФайловИТС.Вставить("ТекстСообщения", ВернутьСтр("ru ='На диске ИТС не обнаружен файл обработки подготовки данных классификатора БИК РФ.'"));
	КонецЕсли;
...
Процедура УстановитьИзмененияВИнтерфейсе()
	...
	Если Элементы.СтраницыФормы.ТекущаяСтраница = Элементы.СтраницаВыборИсточника
		...
		Элементы.ФормаКнопкаДалее.Заголовок = ВернутьСтр("ru ='Загрузить'");
	...
	Иначе
		...
		Элементы.ФормаКнопкаДалее.Заголовок = ВернутьСтр("ru ='Закрыть'");
...
Функция ПолучитьДанныеРБКНаСервере(ПараметрыЗагрузкиФайловРБК)
	...
	НаименованиеЗадания = ВернутьСтр("ru = 'Загрузка классификатора банков РФ'");
...
Функция ТекстЗагрузкаЗапрещена()
	Возврат ВернутьСтр("ru = 'Загрузка классификатора банков в разделенном режиме запрещена.'");
КонецФункции
