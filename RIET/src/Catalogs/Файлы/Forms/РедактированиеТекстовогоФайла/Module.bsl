...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если Не РежимРедактирования Тогда
		ЗаголовокСтрока = ЗаголовокСтрока + " " + ВернутьСтр("ru='(только просмотр)'");
	КонецЕсли;
	...
	Если ЗначениеЗаполнено(ДанныеФайла.Версия) Тогда
		...
		Если ЗначениеЗаполнено(КодировкаТекстаФайла) Тогда
			...
			КонецЕсли;
		Иначе
			КодировкаПредставление = ВернутьСтр("ru='По умолчанию'");
		КонецЕсли;
...
Процедура ПередЗакрытием(Отказ, СтандартнаяОбработка)
	...
	Если Модифицированность Тогда
		...
		ТекстВопроса = СтрЗаменить(ВернутьСтр("ru ='Файл ""%1"" был изменен.'"), "%1", ИмяИРасширение);
...
Процедура СохранитьКак(Команда)
	...
	Если ВыборФайла.Выбрать() Тогда
		...
		Состояние(ВернутьСтр("ru = 'Файл успешно сохранен'"), , ВыбранноеПолноеИмяФайла);
...
Процедура ПоказатьОтличия(Команда)
	#Если ВебКлиент Тогда
		ПоказатьПредупреждение(, ВернутьСтр("ru = 'Сравнение версий в веб-клиенте не поддерживается.'"));
	#Иначе
