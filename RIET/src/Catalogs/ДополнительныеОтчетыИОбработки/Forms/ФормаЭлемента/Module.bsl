...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если Не ПравоДобавления Тогда
		Если ЭтоНовый Тогда
			ВызватьИсключение ВернутьСтр("ru = 'Недостаточно прав доступа для добавления дополнительных отчетов или обработок.'");
		Иначе
	...
	Если ПоказатьДиалогЗагрузкиИзФайлаПриОткрытии И Не Элементы.ЗагрузитьИзФайла.Видимость Тогда
		ВызватьИсключение ВернутьСтр("ru = 'Недостаточно прав для загрузки дополнительных отчетов и обработок'");
	КонецЕсли;
...
Процедура ВариантыДополнительногоОтчетаПередУдалением(Элемент, Отказ)
	...
	Если НЕ Вариант.Пользовательский Тогда
		ПоказатьПредупреждение(, ВернутьСтр("ru = 'Пометка на удаление предопределенного варианта отчета запрещена.'"));
		Возврат;
	КонецЕсли;
	
	ТекстВопроса = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(ВернутьСтр("ru = 'Пометить ""%1"" на удаление?'"), Вариант.Наименование);
...
Процедура ВариантыДополнительногоОтчетаОткрыть(Команда)
	...
	Если Вариант = Неопределено Тогда
		ПоказатьПредупреждение(, ВернутьСтр("ru = 'Выберите вариант отчета.'"));
		Возврат;
	КонецЕсли;
...
Процедура НастроитьВидимость(Команда)
	Если Модифицированность Тогда
		...
		ТекстВопроса = ВернутьСтр("ru = 'Для настройки видимости команд печати обработку необходимо записать. Продолжить?'");
		Кнопки = Новый СписокЗначений;
		Кнопки.Добавить("Продолжить", ВернутьСтр("ru = 'Продолжить'"));
...
Процедура ПродолжитьЗаписьНаКлиенте(Результат, ЗакрытьПослеЗаписи)  Экспорт
	...
	Если ПараметрыЗаписи.РегистрацияОбработки Тогда
		ОбновитьПовторноИспользуемыеЗначения();
		ТекстОповещения = ВернутьСтр("ru = 'Для применения изменений в открытых окнах необходимо их закрыть и открыть заново.'");
...
Процедура ОбновлениеИзФайлаЗапуск()
	...
	Если Объект.Ссылка.Пустая() Тогда
		...
		ПараметрыДиалога.Заголовок = ВернутьСтр("ru = 'Выберите файл внешнего отчета или обработки'");
	ИначеЕсли Объект.Вид = ВидДополнительныйОтчет Или Объект.Вид = ВидОтчет Тогда
		ПараметрыДиалога.ИндексФильтра = 1;
		ПараметрыДиалога.Заголовок = ВернутьСтр("ru = 'Выберите файл внешнего отчета'");
	Иначе
		ПараметрыДиалога.ИндексФильтра = 2;
		ПараметрыДиалога.Заголовок = ВернутьСтр("ru = 'Выберите файл внешней обработки'");
	КонецЕсли;
...
Процедура ОбновлениеИзФайлаПослеВыбораФайла(ПомещенныеФайлы, ПараметрыРегистрации) Экспорт
	...
	Иначе
		...
		ТекстПредупреждения = ВернутьСтр("ru = 'Расширение файла не соответствует расширению внешнего отчета (ERF) или обработки (EPF).'");
...
Процедура ОбновлениеИзФайлаМеханикаНаКлиенте(ПараметрыРегистрации)
	...
	Если ПараметрыРегистрации.Успех Тогда
		ОповещениеЗаголовок = ?(ПараметрыРегистрации.ЭтоОтчет, ВернутьСтр("ru = 'Файл внешнего отчета загружен'"), ВернутьСтр("ru = 'Файл внешней обработки загружен'"));
...
Процедура ОбновлениеИзФайлаПоказатьКонфликты(ПараметрыРегистрации)
	Если ПараметрыРегистрации.КоличествоКонфликтующих > 1 Тогда
		Если ПараметрыРегистрации.ЭтоОтчет Тогда
			ЗаголовокВопроса = ВернутьСтр("ru = 'Конфликты при загрузке внешнего отчета'");
			ТекстВопроса = ВернутьСтр("ru = 'Внутреннее имя отчета ""[Name]""
		...
		Иначе
			ЗаголовокВопроса = ВернутьСтр("ru = 'Конфликты при загрузке внешней обработки'");
			ТекстВопроса = ВернутьСтр("ru = 'Внутреннее имя обработки ""[Name]""
		...
		КонецЕсли;
		ПредставлениеКнопкиОтключить = ВернутьСтр("ru = 'Отключить конфликтующие'");
		ПредставлениеКнопкиОткрыть = ВернутьСтр("ru = 'Отменить и показать список'");
	Иначе
		Если ПараметрыРегистрации.ЭтоОтчет Тогда
			ЗаголовокВопроса = ВернутьСтр("ru = 'Конфликт при загрузке внешнего отчета'");
			ТекстВопроса = ВернутьСтр("ru = 'Внутреннее имя отчета ""[Name]""
			...
			ПредставлениеКнопкиОтключить = ВернутьСтр("ru = 'Отключить другой отчет'");
		Иначе
			ЗаголовокВопроса = ВернутьСтр("ru = 'Конфликт при загрузке внешней обработки'");
			ТекстВопроса = ВернутьСтр("ru = 'Внутреннее имя обработки ""[Name]""
			...
			ПредставлениеКнопкиОтключить = ВернутьСтр("ru = 'Отключить другую обработку'");
		КонецЕсли;
		ПредставлениеКнопкиОткрыть = ВернутьСтр("ru = 'Отменить и открыть'");
	КонецЕсли;
	ПредставлениеКнопкиПродолжить = ВернутьСтр("ru = 'В режиме отладки'");
...
Процедура ОбновлениеИзФайлаРазрешениеКонфликтов(Ответ, ПараметрыРегистрации) Экспорт
	...
	ИначеЕсли Ответ = "ОтменитьИОткрыть" Тогда
		...
		Если ПоказатьСписок Тогда // Форма списка с отбором по конфликтующим.
			...
			ФормаЗаголовок = ВернутьСтр("ru = 'Дополнительные отчеты и обработки с внутреннем именем ""%1""'");
...
Процедура ОткрытьВариант()
	...
	Если НЕ ЗначениеЗаполнено(Вариант.Ссылка) Тогда
		ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(ВернутьСтр("ru = 'Вариант отчета ""%1"" не зарегистрирован.'"), Вариант.Наименование);
...
Процедура ИзменитьРегламентноеЗадание(РежимВыбора = Ложь, ИзмененФлажок = Ложь)
	...
	Если ЭлементКоманда.ВариантЗапуска <> ПредопределенноеЗначение("Перечисление.СпособыВызоваДополнительныхОбработок.ВызовСерверногоМетода")
		...
		ТекстОшибки = ВернутьСтр("ru = 'Команда с вариантом запуска ""%1""
...
Процедура ОбновлениеИзФайлаМеханикаНаСервере(ПараметрыРегистрации)
	...
	Иначе
		Если ПараметрыРегистрации.ЭтоОтчет Тогда
			ЗаголовокОшибки = ВернутьСтр("ru = 'Отчет не подключен'");
		Иначе
			ЗаголовокОшибки = ВернутьСтр("ru = 'Обработка не подключена'");
		КонецЕсли;
...
Функция ПодготовитьПараметрыФормыВыборОбъектовМетаданных()
	...
	ПараметрыФормы.Вставить("Заголовок", ВернутьСтр("ru = 'Назначение дополнительной обработки'"));
...
Процедура УстановитьВидимостьДоступность(Регистрация = Ложь)
	...
	Если ЭтоОтчет И Объект.ИспользуетХранилищеВариантов Тогда
		...
		Если Регистрация ИЛИ КоличествоВариантов = 0 Тогда
			...
			Элементы.СтраницаВарианты.Заголовок = ВернутьСтр("ru = 'Варианты отчета'");
		Иначе
			...
			Элементы.СтраницаВарианты.Заголовок = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				ВернутьСтр("ru = 'Варианты отчета (%1)'"),
	...
	Иначе
		...
		ИначеЕсли ТипЗнч(БезопасныйРежим) = Тип("Строка") Тогда
			...
			Элементы.ДекорацияПерсональныйПрофильБезопасностиНадпись.Заголовок = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				ВернутьСтр("ru = 'Дополнительный отчет или обработка будет подключаться к программе с использованием ""персонального""
					|профиля безопасности %1, в котором будут разрешены только следующие операции:'"),
				БезопасныйРежим);
		Иначе
			ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				ВернутьСтр("ru = '%1 не является корректным режимом подключения для дополнительных отчетов и обработок, требующих разрешений на использование профилей безопасности!'"),
				БезопасныйРежим);
		КонецЕсли;
	...
	Если ЭтоГлобальнаяОбработка Тогда
		ПредставлениеНазначения = ВернутьСтр("ru = '<Не задано>'");
	...
	Иначе
		Если Объект.Назначение.Количество() = 0 Тогда
			ПредставлениеНазначения = ВернутьСтр("ru = 'Не определено'");
		Иначе
			ПредставлениеНазначения = "";
	...
	Если ЭтоНовый Тогда
		Заголовок = ?(ЭтоОтчет, ВернутьСтр("ru = 'Дополнительный отчет (создание)'"), ВернутьСтр("ru = 'Дополнительная обработка (создание)'"));
	Иначе
		Заголовок = Объект.Наименование + " " + ?(ЭтоОтчет, ВернутьСтр("ru = '(Дополнительный отчет)'"), ВернутьСтр("ru = '(Дополнительная обработка)'"));
	КонецЕсли;
	...
	Для Каждого ЭлементКоманда Из Объект.Команды Цикл
			...
			Если Не ПустаяСтрока(ЭлементКоманда.ЗаменяемыеКоманды) Тогда
				...
				Если КоличествоЗаменяемых > 0 Тогда
					Если КоличествоЗаменяемых = 1 Тогда
						ШаблонКомментария = ВернутьСтр("ru = 'Заменяет стандартную команду печати %1'");
					Иначе
						ШаблонКомментария = ВернутьСтр("ru = 'Заменяет стандартные команды печати: %1'");
					КонецЕсли;
		...
		Иначе
			ЭлементКоманда.РегламентноеЗаданиеПредставление = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				ВернутьСтр("ru = 'Не применимо для команд с вариантом запуска ""%1""'"),
...
Процедура ВариантыДополнительногоОтчетаЗаполнить()
	...
	Исключение
		ТекстОшибки = ВернутьСтр("ru = 'Не удалось получить список вариантов отчета из-за ошибки, возникшей при подключении этого отчета:'");
...
Функция НазваниеСтраницыКоманд()
	Если Объект.Вид = Перечисления.ВидыДополнительныхОтчетовИОбработок.ПечатнаяФорма Тогда
		Возврат ВернутьСтр("ru = 'Команды печати'");
	Иначе
		Возврат ВернутьСтр("ru = 'Команды'");
	КонецЕсли;
КонецФункции
