...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если ЗапросНаИмпорт Тогда
		Если ФайлыДляИмпорта = Неопределено ИЛИ Не ИмпортироватьФайлыНаСервере(ФайлыДляИмпорта, ВыполнитьРазделениеФайла) Тогда
			Отказ = Истина;
		КонецЕсли;
		Возврат;
	КонецЕсли;
...
Процедура ПриЗаписиНаСервере(Отказ, ТекущийОбъект, ПараметрыЗаписи)
	...
	Если СвойстваФайлов <> Неопределено Тогда
		...
		Если КонтекстЭДОСервер = Неопределено Тогда 
			...
			ТекстСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(ВернутьСтр("ru='Электронное представление не записано:%1.'"),
...
Процедура ФайлыКомплектаПФРПередНачаломДобавления(Элемент, Отказ, Копирование, Родитель, Группа, Параметр)
	...
	ФайлыКомплектаПередНачаломДобавления(Элемент, Отказ, Копирование, Родитель, Группа, Параметр);
...
Процедура ВыгрузитьЗавершение(Результат, ДополнительныеПараметры) Экспорт
	...
	Если ЗначениеЗаполнено(ИмяВременногоФайлаПакета) Тогда
		ОписаниеОповещения = Новый ОписаниеОповещения("ВыгрузитьПослеСохраненияЗавершение", ЭтотОбъект);
...
&НаКлиенте
Процедура ВыгрузитьПослеСохраненияЗавершение(Результат, ДополнительныеПараметры) Экспорт
	
	Если Не Результат Тогда
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(ВернутьСтр("ru = 'Ошибка при выгрузке пакета'"));
	КонецЕсли;
...
Процедура ПривестиПолучателяКТипуНаСервере()
	...
	Если ЗначениеЗаполнено(ТипПолучателя) Тогда
		...
		Элементы.Получатель.КнопкаВыбора  = 
			ТипПолучателя <> ТипФСС И ТипПолучателя <> ТипФСРАР И ТипПолучателя <> ТипРПН;
	Иначе
...
Процедура ОткрытьПриложениеКБухгалтерскойОтчетности(ТипФайлаБухгалтерскойОтчетности)
	...
	КонтекстЭДОКлиент.ОткрытьФайл(АдресДанных, ИмяФайлаПриложения);
...
Процедура ДобавитьФайлПриложенияОтчетности(ТипФайлаОтчетности)
	...
	ОписаниеОповещения = Новый ОписаниеОповещения("ДобавитьФайлПриложенияОтчетностиЗавершение", ЭтотОбъект, ТипФайлаОтчетности);
	КонтекстЭДОКлиент.ПоддерживаетсяИспользованиеРасширенияРаботыСФайлами(ОписаниеОповещения, Истина);
	
КонецПроцедуры

&НаКлиенте
Процедура ДобавитьФайлПриложенияОтчетностиЗавершение(Результат, ТипФайлаОтчетности) Экспорт
	...
	Если ПоддерживаетсяИспользованиеРасширенияРаботыСФайлами Тогда
		...
		Если ДиалогВыбора.Выбрать() Тогда
			...
			Для Каждого ЭлФайл Из ДиалогВыбора.ВыбранныеФайлы Цикл
			...
			СвойстваФайла = МассивФайлов[0];
			ОпределитьФайлВТаблицеСвойствФайлов(СвойстваФайла.Имя, ТипФайлаОтчетности, СвойстваФайла.АдресДанных);
			УправлениеФормой(ЭтаФорма);
...
&НаКлиенте
Процедура ДобавитьФайлПриложенияОтчетностиЗавершениеПослеПомещенияФайла(ФайлыБылиВыбраны, АдресДанных, ВыбранноеИмяФайла, ДополнительныеПараметры) Экспорт
	...
	Если ФайлыБылиВыбраны Тогда
		...
		СвойстваФайла = МассивФайлов[0];
		ОпределитьФайлВТаблицеСвойствФайлов(СвойстваФайла.Имя, ДополнительныеПараметры.ТипФайлаОтчетности, СвойстваФайла.АдресДанных);
		УправлениеФормой(ЭтаФорма);
...
&НаСервере
Функция ИмпортироватьФайлыНаСервере(МассивФайлов, ВыполнитьРазделениеФайла = Ложь)
	
	ВыполнятьПроверкуКомплектаУведомлений = Истина;
	КонтекстЭДОСервер = КонтекстЭДОСервер();
	
	// проверяем файл электронного представления уведомления
	Если ЭтоФайлВыгрузкиУведомленияОКонтролируемыхСделках И ВыполнятьПроверкуКомплектаУведомлений Тогда
		Результат = ПроверитьФайлыВыгрузкиУведомленияОКонтролируемыхСделках(МассивФайлов);
	...
	Если МассивФайлов.Количество() = 1 И ВыполнитьРазделениеФайла Тогда
		
		ОсновнойФайл = МассивФайлов[0];
		ФайлыОтчетов = КонтекстЭДОСервер.РазбитьБольшойФайлУведомленияНаНесколькоМаленьких(ОсновнойФайл, ЭтаФорма.УникальныйИдентификатор);
		МассивФайлов = ФайлыОтчетов;
		ОсновнойФайл = ФайлыОтчетов[0];
		
	Иначе
		
		Для Каждого ВыбранныйФайл Из МассивФайлов Цикл
			РасширениеФайла = нрег(ВыбранныйФайл.Расширение);
			Если РасширениеФайла = ".txt" ИЛИ РасширениеФайла = ".xml" Тогда
	...
	Если ОсновнойФайл = Неопределено Тогда
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю("Среди выбранных файлов не удалось обнаружить ни одного файла с расширением ""xml"" или ""txt""!");
		Возврат Ложь;
	КонецЕсли;
	
	Если ФайлыОтчетов.Количество() > 1 Тогда
		Если КонтекстЭДОСервер.ЭтоФайлВыгрузкиУведомленияОКонтролируемыхСделках(ОсновнойФайл.Имя) Тогда
			// это комплект уведомлений
			Объект.ВидОтчета = Справочники.ВидыОтправляемыхДокументов.УведомлениеОКонтролируемыхСделках;
		...
		Иначе	
			...
			Объект.ВидОтчета = Справочники.ВидыОтправляемыхДокументов.КомплектыОтчетностиПерсучета;
	...
	ЗагрузитьЭлектронныеПредставления(МассивФайлов);
...
Процедура ПривестиПолучателяКТипу(Форма)
	...
	Если ЗначениеЗаполнено(ТипПолучателя) Тогда
		...
		Элементы.Получатель.КнопкаВыбора 	= НЕ ТипПолучателя = ТипФСС И НЕ ТипПолучателя = ТипФСРАР И НЕ ТипПолучателя = ТипРПН;
		Элементы.Получатель.КнопкаОчистки 	= НЕ ТипПолучателя = ТипФСС И НЕ ТипПолучателя = ТипФСРАР И НЕ ТипПолучателя = ТипРПН;
...
Функция ПроверитьФайлыВыгрузкиУведомленияОКонтролируемыхСделках(ФайлыОтчетов)
	...
	Для каждого ФайлОтчета Из ФайлыОтчетов Цикл
		...
		Если РазмерФайла(ВремФайл) > МаксимальныйРазмер Тогда
			...
			ТекстСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(ВернутьСтр("ru='Размер файла %2 превышает максимально допустимый (%1 Мб). Загрузка файлов будет прервана'"), 
			...
			Пока ФайлXML.ПрочитатьАтрибут() Цикл
				...
				Если ФайлXML.Имя = "ВерсФорм" Тогда
					...
					Если НоваяСтрокаТаблицы.ВерсФорм <> "5.02" И ФайлыОтчетов.Количество() > 1 Тогда
						// если файл имеет формат 5.01 и файлов несколько, то это не верно
						ОбщегоНазначенияКлиентСервер.СообщитьПользователю(ВернутьСтр("ru='Файлы с версией, отличной от 5.02, не могут быть загружены комплектом'"));
						Возврат Ложь;
		...
		КонтекстЭДОСервер.УдалитьВременныйФайл(ВремФайл);
	...
	Если ТаблицаПроверкиИдФайлИсх.Количество() > 1 Тогда
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(ВернутьСтр("ru='В файлах различаются значения в полях ИдФайлИсх, ИдФайлПерв. Загрузка файлов будет прервана'"));
		Возврат Ложь;
	КонецЕсли;
	...
	Если КоличествоИзТэгаКолФайл <> КоличествоВыбранныхФайлов Тогда
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(ВернутьСтр("ru='В описании к файлам число файлов комплекта не соответствует числу выбранных файлов. Загрузка файлов будет прервана'"));
		Возврат Ложь;
	КонецЕсли;
	
	ТаблицаПроверкиКолФайл.Свернуть("КолФайл");
	Если ТаблицаПроверкиКолФайл.Количество() > 1 Тогда
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(ВернутьСтр("ru='В файлах различается значение в поле КолФайл. Загрузка файлов будет прервана'"));
		Возврат Ложь;
	КонецЕсли;
	...
	Для сч = 0 По КоличествоФайлов - 1 Цикл
		Если ТаблицаСИнформациейОФайлахОтчетов[сч].НомФайл <> сч + 1 Тогда
			ОбщегоНазначенияКлиентСервер.СообщитьПользователю(ВернутьСтр("ru='Не все файлы комплекта выбраны. Загрузка файлов будет прервана'"));
			Возврат Ложь;
		КонецЕсли;
	КонецЦикла;
...
Функция ПроанализироватьФайлВыгрузкиФСРАР(ОсновнойФайл)
	...
	Если ВидОтправляемогоДокумента = Неопределено Тогда
		РегламентированнаяОтчетностьКлиентСервер.СообщитьОбОшибке(ВернутьСтр("ru='Не удалось определить тип файла выгрузки!'"));
		Возврат Ложь;
	КонецЕсли;
...
Функция ОпределитьОрганизациюПоИННиКПП(ИНН, КПП)
	...
	Запрос.Текст = Запрос.Текст + " Организации.ИНН = &ИНН";
...
Функция ПроанализироватьФайлВыгрузкиРПН(ОсновнойФайл)
	...
	// определяем вид отправляемого документа
	Префикс = нрег(Лев(КороткоеИмяФайла, 10));
	Если СтрНайти(Префикс, "рсч_") > 0 Тогда
		ВидОтправляемогоДокумента = Справочники.ВидыОтправляемыхДокументов.РасчетПлатыОкрСреда;
	Иначе
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(ВернутьСтр("ru = 'Не удалось определить тип файла выгрузки!'"));
		Возврат Ложь;
	КонецЕсли;
	ПараметрыИмпорта.Вставить("ВидОтчета", ВидОтправляемогоДокумента);
	...
	Если Тип(ДеревоXML.ORG_INFO) = Тип("СписокXDTO") Тогда
		...
		ИННВПервомУзел = "";
		Если СписокУзловORG_INFO[0].Свойства().Получить("INN") <> Неопределено Тогда
			ИННВПервомУзел = СписокУзловORG_INFO[0].INN;
		КонецЕсли;
		...
		Для каждого УзелORG_INFO Из СписокУзловORG_INFO Цикл
			...
			Если УзелORG_INFO.Свойства().Получить("INN") <> Неопределено Тогда
				ИННВТекущемУзле = УзелORG_INFO.INN;
			КонецЕсли;

			Если ИННВПервомУзел <> ИННВТекущемУзле Тогда
				// Значит в файле несколько разных организаций
				ОбщегоНазначенияКлиентСервер.СообщитьПользователю(
					ВернутьСтр("ru = 'Отправка файлов, содержащих отчетность по нескольким организациям, не поддерживается'"));
				Возврат Ложь;
			КонецЕсли;
		КонецЦикла;
	КонецЕсли;
...
Функция ОпределитьНомерКорректировкиРПН(ДеревоXML)
	...
	Если ДеревоXML.CALC_TYPE <> "2" Тогда
		Возврат 0;
	ИначеЕсли ДеревоXML.Свойства().Получить("NUMB_COR_RPT") <> Неопределено Тогда
...
Функция ПроанализироватьФайлВыгрузкиФНС(ПолноеИмяФайла)
	...
	Если Расширение = "xml" Тогда
		...
		Если ВидДок <> Неопределено И СтроковыеФункцииКлиентСервер.ТолькоЦифрыВСтроке(ВидДок) Тогда
			ПараметрыИмпорта.Вставить("ВидДокумента", Число(ВидДок));
		ИначеЕсли НомКорр <> Неопределено И СтроковыеФункцииКлиентСервер.ТолькоЦифрыВСтроке(НомКорр) Тогда
	...
	ИначеЕсли Расширение = "txt" Тогда
		...
		Если ЭтоАлко201 Тогда
			Если ИдДок <> Неопределено Тогда
				...
				Если СтроковыеФункцииКлиентСервер.ТолькоЦифрыВСтроке(НомерОтчета) Тогда
		...
		ИначеЕсли ВариантОтчета <> Неопределено Тогда // это формы бух. отчетности 2.01
			ПараметрыИмпорта.Вставить("ВидДокумента", ?(ВариантОтчета <> "999" ИЛИ НЕ СтроковыеФункцииКлиентСервер.ТолькоЦифрыВСтроке(ВариантОтчета), 0, Число(ВариантОтчета) - 1));
		ИначеЕсли _НомерКорректировки <> Неопределено Тогда
			ПараметрыИмпорта.Вставить("ВидДокумента", ?(СтроковыеФункцииКлиентСервер.ТолькоЦифрыВСтроке(_НомерКорректировки), Число(_НомерКорректировки), 0));
		КонецЕсли;
...
Функция ПроанализироватьФайлВыгрузкиОтчет4ФСС(ПолноеИмяФайла)
	...
	Если НомКорр <> Неопределено И СтроковыеФункцииКлиентСервер.ТолькоЦифрыВСтроке(НомКорр) Тогда
		ПараметрыИмпорта.Вставить("ВидДокумента", Число(НомКорр));
	КонецЕсли;
	...
	Если ГодПериодОтч = Неопределено ИЛИ НЕ СтроковыеФункцииКлиентСервер.ТолькоЦифрыВСтроке(ГодПериодОтч) 
	 ИЛИ ПериодОтч = Неопределено ИЛИ НЕ СтроковыеФункцииКлиентСервер.ТолькоЦифрыВСтроке(ПериодОтч) ИЛИ Число(ПериодОтч) = 0 Тогда
		ОшибкаОпределенияПериода = Истина;
	Иначе
		ДатаНачалаПериодаОтчетаВыгрузки = НачалоГода(Дата(Число(ГодПериодОтч), 1, 1));
...
Функция ПроанализироватьФайлВыгрузкиФСГС(ПолноеИмяФайла)
	...
	Если (ГодОтчета = Неопределено ИЛИ НЕ СтроковыеФункцииКлиентСервер.ТолькоЦифрыВСтроке(ГодОтчета))
		ИЛИ (ПериодОтчета = Неопределено ИЛИ НЕ СтроковыеФункцииКлиентСервер.ТолькоЦифрыВСтроке(ПериодОтчета))
...
Функция ПроанализироватьФайлВыгрузкиПФР(ПолноеИмяФайла)
	...
	РезультатОпределения = ОпределитьВидОтчетаПФР(ДеревоXML, ВидКомплектовОтправляемыхДокументов);
	Если ЗначениеЗаполнено(РезультатОпределения) Тогда
...
&НаСервере
Функция ОпределитьВидОтчетаПФР(ДеревоXML, ВидКомплектовОтправляемыхДокументов = Неопределено)
...
Функция ОпределитьПериодПФР(КороткоеИмяФайла, ДеревоXML)
	...
	Если ВхождениеКлюча <> 0 Тогда
		...
		Если СтроковыеФункцииКлиентСервер.ТолькоЦифрыВСтроке(СтрНомерГода) Тогда
	...
	Для Каждого СвойствоУзла Из СвойстваУзла Цикл
		...
		Если СвойстваТекущегоУзла.Получить("ОтчетныйГод") <> Неопределено Тогда
			...
			Если СтроковыеФункцииКлиентСервер.ТолькоЦифрыВСтроке(СтрОтчетныйГод) Тогда
		...
		Если СвойстваТекущегоУзла.Получить("КалендарныйГод") <> Неопределено Тогда
			...
			Если СтроковыеФункцииКлиентСервер.ТолькоЦифрыВСтроке(СтрКалендарныйГод) Тогда
		...
		Если СвойстваТекущегоУзла.Получить("ОтчетныйПериод") <> Неопределено Тогда
			...
			И СвойстваУзлаОтчетныйПериод.Получить("Год") <> Неопределено Тогда
				...
				Если СтроковыеФункцииКлиентСервер.ТолькоЦифрыВСтроке(УзелОтчетныйПериод.Квартал)
				И СтроковыеФункцииКлиентСервер.ТолькоЦифрыВСтроке(УзелОтчетныйПериод.Год) Тогда
		...
		Если СвойстваТекущегоУзла.Получить("КодОтчетногоПериода") <> Неопределено Тогда
			...
			Если СтроковыеФункцииКлиентСервер.ТолькоЦифрыВСтроке(СтрОтчетныйПериод) Тогда
...
Функция ПроанализироватьФайлВыгрузкиОтчет4аФСС(ПолноеИмяФайла)
	...
	Если ГодПериодОтч = Неопределено ИЛИ НЕ СтроковыеФункцииКлиентСервер.ТолькоЦифрыВСтроке(ГодПериодОтч) Тогда
		ОшибкаОпределенияПериода = Истина;
	Иначе
		ДатаНачалаПериодаОтчетаВыгрузки = НачалоГода(Дата(Число(ГодПериодОтч), 1, 1));
	...
	Иначе
		...
		Для Каждого Стр Из СвойстваФайлов Цикл
			Если Стр.ТипФайлаОтчетности <> Перечисления.ТипыСодержимогоТранспортногоКонтейнера.СведенияПриложениеПФР Тогда
				...
				НовСтр.СсылкаПоказать = ВернутьСтр("ru = 'Открыть'");
			Иначе
...
Процедура ОткрытьФайлПослеОтветаНаВопросОБольшомРазмереФайла(Результат, ДополнительныеПараметры) Экспорт
	...
	Если Результат = Истина Тогда
		...
		ОткрытьФорму(КонтекстЭДОКлиент.ПутьКОбъекту + ".Форма.ПросмотрТекст", СтруктураПараметров);	
	КонецЕсли;
...
&НаКлиенте
Процедура ФайлыКомплектаПередНачаломДобавления(Элемент, Отказ, Копирование, Родитель, Группа, Параметр, ЭтоДополнительныеФайлы = Ложь)
	
	Отказ = Истина;
	
	Если Копирование Тогда
		ПоказатьПредупреждение(, ВернутьСтр("ru = 'Копирование запрещено!'"));
		Возврат;
	КонецЕсли;
	
	ДополнительныеПараметры = Новый Структура("ЭтоДополнительныеФайлы", ЭтоДополнительныеФайлы);
...
Процедура ФайлыКомплектаПередНачаломДобавленияЗавершение(Результат, ДополнительныеПараметры) Экспорт
	...
	Если ПоддерживаетсяИспользованиеРасширенияРаботыСФайлами Тогда
		ДиалогВыбора = Новый ДиалогВыбораФайла(РежимДиалогаВыбораФайла.Открытие);
		ДиалогВыбора.Заголовок = ВернутьСтр("ru = 'Выберите файл(ы) для импорта'");
		ДиалогВыбора.МножественныйВыбор = Истина;
		ДиалогВыбора.ПроверятьСуществованиеФайла = Истина;
		ДиалогВыбора.Фильтр = "Все файлы (*.*)|*.*";
		ФайлыБылиВыбраны = ДиалогВыбора.Выбрать();
		
		ДополнительныеПараметры.Вставить("ДиалогВыбора", ДиалогВыбора);
		ФайлыКомплектаПередНачаломДобавленияЗавершениеПослеПомещенияФайла(ФайлыБылиВыбраны, "", "", ДополнительныеПараметры);
	Иначе
		АдресДанных = "";
		
		ОписаниеОповещения = Новый ОписаниеОповещения("ФайлыКомплектаПередНачаломДобавленияЗавершениеПослеПомещенияФайла", ЭтотОбъект, ДополнительныеПараметры);
...
&НаКлиенте
Процедура ФайлыКомплектаПередНачаломДобавленияЗавершениеПослеПомещенияФайла(Результат, АдресДанных, ВыбранноеИмяФайла, ДополнительныеПараметры) Экспорт
	
	ФайлыБылиВыбраны = Результат;
	...
	Если ФайлыБылиВыбраны Тогда
		...
		МассивФайлов = Новый Массив;
		Если ПоддерживаетсяИспользованиеРасширенияРаботыСФайлами Тогда
			ВыбранныеФайлы = ДополнительныеПараметры.ДиалогВыбора.ВыбранныеФайлы;
		Иначе
		...
		Для Каждого ЭлФайл Из ВыбранныеФайлы Цикл
			...
			Если НЕ ДополнительныеПараметры.ЭтоДополнительныеФайлы Тогда
				МаксимальноДопустимыйРазмерФайла = КонтекстЭДОКлиент.МаксимальныйРазмерФайлаУведомленияОКонтролируемыхСделках();
				МаксимальноДопустимыйРазмерФайлаВМб = Цел(МаксимальноДопустимыйРазмерФайла/1024/1024);
				Если РазмерФайла > КонтекстЭДОКлиент.МаксимальныйРазмерФайлаУведомленияОКонтролируемыхСделках() Тогда
					ТекстПредупреждения = ВернутьСтр("ru = 'Размер файла %1 превышает максимальный размер (%2 Мб)'");
					ТекстПредупреждения = СтрЗаменить(ТекстПредупреждения, "%1", Файл.Имя);
					ТекстПредупреждения = СтрЗаменить(ТекстПредупреждения, "%2", МаксимальноДопустимыйРазмерФайлаВМб);
		...
		ДобавитьФайлыКомплектаНаСервере(МассивФайлов, ДополнительныеПараметры.ЭтоДополнительныеФайлы);
...
Процедура УправлениеФормой(Форма)
	...
	ЗагружаетсяНесколькоФайлов = Форма.СвойстваФайлов.Количество() > 1;
	...
	Если Форма.ТипДокумента = ПредопределенноеЗначение("Перечисление.ТипыОтправляемыхДокументов.ЗаявлениеОВвозеТоваров")
		...
		ИЛИ Форма.ТипДокумента = ПредопределенноеЗначение("Перечисление.ТипыОтправляемыхДокументов.ИсходящееУведомлениеФНС") Тогда
	...
	ИначеЕсли Форма.ТипДокумента = ПредопределенноеЗначение("Перечисление.ТипыОтправляемыхДокументов.УведомлениеОКонтролируемыхСделках")
		И ЗагружаетсяНесколькоФайлов Тогда
...
Функция ПолучитьНаименование(ВидОтчета, НачалоПериода, КонецПериода, Организация) Экспорт
	...
	Если ТипДокумента = ПредопределенноеЗначение("Перечисление.ТипыОтправляемыхДокументов.РеестрСведенийФСС")
		ИЛИ ТипДокумента = ПредопределенноеЗначение("Перечисление.ТипыОтправляемыхДокументов.ИсходящееУведомлениеФНС") Тогда
		ШаблонНаименования = ВернутьСтр("ru = '%1 (%2)'");
	Иначе
		ШаблонНаименования = ВернутьСтр("ru = '%1 за %3 (%2)'");
	КонецЕсли;
...
Процедура ДополнительныеФайлыПФРДобавитьНаСервере(Номер, ИмяФайла)
	...
	ДополнительныйФайлПФРУдалить.Заголовок = ВернутьСтр("ru = 'Удалить'");
...
Процедура ДополнительныеФайлыПФРДобавить(Команда)
	...
	Отказ = Ложь;
	ФайлыКомплектаПередНачаломДобавления(Неопределено, Отказ, Ложь, Неопределено, Неопределено, Неопределено, Истина);
