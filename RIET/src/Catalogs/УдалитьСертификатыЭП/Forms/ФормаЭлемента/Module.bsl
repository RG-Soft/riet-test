...
Процедура ЗаполнитьЗаголовкиГиперссылок()
	...
	Если Объект.ЗапомнитьПарольКСертификату Тогда
		ТекстГиперссылкиПароля = ВернутьСтр("ru = 'Изменить пароль.'");
	Иначе
		ТекстГиперссылкиПароля = ВернутьСтр("ru = 'Пароль не сохранен.'");
	КонецЕсли;
...
Процедура УстановитьПароль(ЗапомнитьПароль = Ложь)
	...
	ВидОперации = ВернутьСтр("ru = 'Сохранение пароля в сертификате'");
...
Процедура ТестНастроекСертификата(Команда)
	...
	Если Модифицированность Тогда
		...
		ТекстВопроса = ВернутьСтр("ru = 'Изменены настройки сертификата. Продолжить выполнение теста?'");
		...
		ПоказатьВопрос(ОбработчикОповещения, ТекстВопроса, СписокКнопок, , "Выполнить", ВернутьСтр("ru = 'Тест настроек сертификата'"));
...
Процедура НазначениеОткрытие(Элемент, СтандартнаяОбработка)
	...
	Исключение
		ПоказатьПредупреждение(,ВернутьСтр("ru = 'Невозможно открыть сертификат. Возможно он не установлен на локальный компьютер.'"));
	КонецПопытки;
...
Процедура ПередЗаписью(Отказ, ПараметрыЗаписи)
	...
	Если Объект.ОграничитьДоступКСертификату И НЕ ЗначениеЗаполнено(Объект.Пользователь) Тогда
		Отказ = Истина;
		ПоказатьПредупреждение(,ВернутьСтр("ru = 'Не указан пользователь, которому доступен сертификат!
