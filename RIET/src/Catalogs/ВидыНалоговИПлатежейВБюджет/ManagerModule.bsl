...
//	Если ЗначениеЗаполнено(ВидНалоговогоОбязательства)
//		И (Перечисления.ВидыПлатежейВГосБюджет.ЭтоПени(ВидНалоговогоОбязательства)
//			Или Перечисления.ВидыПлатежейВГосБюджет.ЭтоПроценты(ВидНалоговогоОбязательства)
//			Или Перечисления.ВидыПлатежейВГосБюджет.ЭтоШтраф(ВидНалоговогоОбязательства)) Тогда
//		
...
//			Объект.КодБК = Справочники.ВидыНалоговИПлатежейВБюджет.КБКПоВидуНалоговогоОбязательства(
//				СтрокаДанныхКлассификатора.КодБК, СтрокаДанныхКлассификатора.ВидНалога,, ДатаАктуальностиКлассификатора());
//			ОбновлениеИнформационнойБазы.ЗаписатьОбъект(Объект, Ложь, Истина);
//		КонецЕсли;
//		
//	КонецЦикла;
//	
...
//Процедура УдалитьДублиНалоговПоВидам() Экспорт
//	
//	Если ПланыОбмена.ГлавныйУзел() <> Неопределено Тогда
...
//	|	Справочник.ВидыНалоговИПлатежейВБюджет КАК ВидыНалоговИПлатежейВБюджет
//	|ГДЕ
//	|	ВидыНалоговИПлатежейВБюджет.ВидНалога <> ЗНАЧЕНИЕ(Перечисление.ВидыНалогов.ПустаяСсылка)
//	|	И ВидыНалоговИПлатежейВБюджет.Ссылка <> ЗНАЧЕНИЕ(Справочник.ВидыНалоговИПлатежейВБюджет.ПрочиеНалогиИСборы)
//	|	И НЕ ВидыНалоговИПлатежейВБюджет.ПометкаУдаления
//	|
//	|СГРУППИРОВАТЬ ПО
...
//	|ИЗ
//	|	Справочник.ВидыНалоговИПлатежейВБюджет КАК ВидыНалоговИПлатежейВБюджет
//	|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ ВТ_ВидыНалогов КАК ВТ_ВидыНалогов
//	|		ПО ВидыНалоговИПлатежейВБюджет.ВидНалога = ВТ_ВидыНалогов.ВидНалога
//	|			И (ВТ_ВидыНалогов.Количество > 1)
//	|
//	|УПОРЯДОЧИТЬ ПО
//	|	Ссылка
//	|ИТОГИ ПО
//	|	ВидНалога";
//	
//	ДублиНалогов = Новый Массив;
//	ЗначенияПодстановки = Новый Соответствие;
//	
//	ВыборкаВидНалога = Запрос.Выполнить().Выбрать(ОбходРезультатаЗапроса.ПоГруппировкам, "ВидНалога");
//	Пока ВыборкаВидНалога.Следующий() Цикл
//		
//		ВидНалога = ВыборкаВидНалога.ВидНалога;
//		
//		Оригинал = Справочники.ВидыНалоговИПлатежейВБюджет.ПустаяСсылка();
//		
//		Выборка = ВыборкаВидНалога.Выбрать();
//		Пока Выборка.Следующий() Цикл
//			
//			Если Оригинал.Пустая() Тогда
//				Оригинал = Выборка.Ссылка;
//			Иначе
//				
//				Дубль = Выборка.Ссылка;
//				ДублиНалогов.Добавить(Дубль);
//				ЗначенияПодстановки.Вставить(Дубль, Оригинал);
//				
//				Объект = Дубль.ПолучитьОбъект();
...
//	
//	Запрос = Новый Запрос;
//	Запрос.Параметры.Вставить("ДублиНалогов", ДублиНалогов);
//	Запрос.Текст = 
//	"ВЫБРАТЬ
...
//	|ГДЕ
//	|	ПлатежноеПоручение.Налог В(&ДублиНалогов)
//	|
//	|ОБЪЕДИНИТЬ ВСЕ
//	|
//	|ВЫБРАТЬ
//	|	СписаниеСРасчетногоСчета.Ссылка,
//	|	СписаниеСРасчетногоСчета.Налог
//	|ИЗ
//	|	Документ.СписаниеСРасчетногоСчета КАК СписаниеСРасчетногоСчета
//	|ГДЕ
//	|	СписаниеСРасчетногоСчета.Налог В(&ДублиНалогов)
//	|
//	|ОБЪЕДИНИТЬ ВСЕ
//	|
//	|ВЫБРАТЬ
//	|	РасходныйКассовыйОрдер.Ссылка,
//	|	РасходныйКассовыйОрдер.Налог
//	|ИЗ
//	|	Документ.РасходныйКассовыйОрдер КАК РасходныйКассовыйОрдер
//	|ГДЕ
//	|	РасходныйКассовыйОрдер.Налог В(&ДублиНалогов)";
//	
//	Выборка = Запрос.Выполнить().Выбрать();
//	Пока Выборка.Следующий() Цикл
//		Налог = ЗначенияПодстановки.Получить(Выборка.Налог);
//		Если ЗначениеЗаполнено(Налог) Тогда
...
//// Заменяет некорректные значения реквизита Налог в документах
//// на предопределенный элемент ПрочиеНалогиИСборы
////
//Процедура ИсправитьНекорректныеВидыНалогов() Экспорт
//	
//	ВидыНалоговФизическихЛиц  = РасчетыСБюджетом.ВидыНалоговФизическихЛиц();
//	ВидыНалоговЮридическихЛиц = РасчетыСБюджетом.ВидыНалоговЮридическихЛиц();
//	
//	Запрос = Новый Запрос;
//	Запрос.Параметры.Вставить("ВидыНалоговФизическихЛиц",  ВидыНалоговФизическихЛиц);
//	Запрос.Параметры.Вставить("ВидыНалоговЮридическихЛиц", ВидыНалоговЮридическихЛиц);
//	Запрос.Текст = 
//	"ВЫБРАТЬ
...
//	|ПОМЕСТИТЬ ВТ_НалогиЮридическихЛиц
//	|ИЗ
//	|	Справочник.ВидыНалоговИПлатежейВБюджет КАК ВидыНалоговИПлатежейВБюджет
//	|ГДЕ
//	|	ВидыНалоговИПлатежейВБюджет.ВидНалога В(&ВидыНалоговЮридическихЛиц)
//	|
//	|ИНДЕКСИРОВАТЬ ПО
//	|	Налог
//	|;
...
//	|ВЫБРАТЬ
//	|	ВидыНалоговИПлатежейВБюджет.Ссылка КАК Налог
//	|ПОМЕСТИТЬ ВТ_НалогиФизическихЛиц
//	|ИЗ
//	|	Справочник.ВидыНалоговИПлатежейВБюджет КАК ВидыНалоговИПлатежейВБюджет
//	|ГДЕ
//	|	ВидыНалоговИПлатежейВБюджет.ВидНалога В(&ВидыНалоговФизическихЛиц)
//	|
//	|ИНДЕКСИРОВАТЬ ПО
//	|	Налог
//	|;
...
//	|		ПО Документ.Организация = ВТ_Организации.Организация
//	|			И (ВТ_Организации.ЮридическоеФизическоеЛицо = ЗНАЧЕНИЕ(Перечисление.ЮридическоеФизическоеЛицо.ФизическоеЛицо))
//	|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ ВТ_НалогиЮридическихЛиц КАК ВТ_Налоги
//	|		ПО Документ.Налог = ВТ_Налоги.Налог
//	|ГДЕ
//	|	Документ.ВидОперации = ЗНАЧЕНИЕ(Перечисление.ВидыОперацийСписаниеДенежныхСредств.ПеречислениеНалога)
//	|
//	|ОБЪЕДИНИТЬ ВСЕ
//	|
//	|ВЫБРАТЬ
...
//	|		ПО Документ.Организация = ВТ_Организации.Организация
//	|			И (ВТ_Организации.ЮридическоеФизическоеЛицо <> ЗНАЧЕНИЕ(Перечисление.ЮридическоеФизическоеЛицо.ФизическоеЛицо))
//	|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ ВТ_НалогиФизическихЛиц КАК ВТ_Налоги
//	|		ПО Документ.Налог = ВТ_Налоги.Налог
//	|ГДЕ
//	|	Документ.ВидОперации = ЗНАЧЕНИЕ(Перечисление.ВидыОперацийСписаниеДенежныхСредств.ПеречислениеНалога)
//	|
...
//	|		ПО Документ.Налог = ВТ_Налоги.Налог
//	|ГДЕ
//	|	Документ.ВидОперации = ЗНАЧЕНИЕ(Перечисление.ВидыОперацийРКО.УплатаНалога)
//	|
//	|ОБЪЕДИНИТЬ ВСЕ
//	|
//	|ВЫБРАТЬ
...
//// Обновляет КБК дополнительных страховых взносов
////
//Процедура ОбновитьКБКДополнительныхСтраховыхВзносов() Экспорт
//	
//	МассивВидовНалога = Новый Массив;
//	МассивВидовНалога.Добавить(Перечисления.ВидыНалогов.ДополнительныеВзносы_ПФР_ВредныеУсловия);
//	МассивВидовНалога.Добавить(Перечисления.ВидыНалогов.ДополнительныеВзносы_ПФР_ТяжелыеУсловия);
//	
//	Для Каждого ВидНалога ИЗ МассивВидовНалога Цикл
//		Ссылка = Справочники.ВидыНалоговИПлатежейВБюджет.НайтиПоРеквизиту("ВидНалога", ВидНалога);
//		Если НЕ Ссылка.Пустая() Тогда
//			Объект = Ссылка.ПолучитьОбъект();
//			Объект.КодБК = КБКПоВидуНалоговогоОбязательства(Объект.КодБК, ВидНалога, Перечисления.ВидыПлатежейВГосБюджет.Налог);
//			ОбновлениеИнформационнойБазы.ЗаписатьОбъект(Объект);
//		КонецЕсли;
//		
//	КонецЦикла;
