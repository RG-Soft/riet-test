...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если ЭтоНовый Тогда
		...
		Если Не ЗначениеЗаполнено(Объект.Автор) Тогда
			...
			ПараметрыЖурнала.Вставить("ИмяСобытия", ВернутьСтр("ru = 'Рассылка отчетов. Открытие формы элемента'", ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка()));
			...
			Текст = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				ВернутьСтр("ru = 'Ошибка заполнения автора рассылки:
...
Процедура ПриОткрытии(Отказ)
	...
	УстановитьДоступностьГруппаПараметрыНастроекПериода(Объект.Архивировать);
	// { RGS ASeryakov 13.03.2018 10:00:00 PM SLI-0007499
КонецПроцедуры
...
Процедура ОбработкаПроверкиЗаполненияНаСервере(Отказ, ПроверяемыеРеквизиты)
	...
	Если Не ЗначениеЗаполнено(Объект.Наименование) Тогда
		Отказ = Истина;
		ТекстСообщения = ВернутьСтр("ru = 'Не введено наименование'");
	...
	Если Объект.ИспользоватьЭлектроннуюПочту И Не Объект.Личная Тогда
		Если Не ЗначениеЗаполнено(ТипПолучателейРассылки) Тогда
			Отказ = Истина;
			ТекстСообщения = ВернутьСтр("ru = 'Не выбран тип получателей'");
	...
	Если Объект.Подготовлена Тогда
		Если Объект.Отчеты.Количество() = 0 Тогда
			Отказ = Истина;
			ТекстСообщения = ВернутьСтр("ru = 'Не выбрано ни одного отчета'");
		...
		Если Не ЗначениеЗаполнено(Объект.ПериодичностьРасписания) Тогда
			Отказ = Истина;
			ТекстСообщения = ВернутьСтр("ru = 'Не выбрана периодичность запуска'");
		...
		Если Объект.ИспользоватьFTPРесурс Тогда
			Если Не ЗначениеЗаполнено(Объект.FTPСервер)
				...
				Отказ = Истина;
				ТекстСообщения = ВернутьСтр("ru = 'Не введен FTP адрес'");
		...
		Если Объект.ИспользоватьСетевойКаталог Тогда
			Если Не ЗначениеЗаполнено(Объект.СетевойКаталогWindows) Тогда
				Отказ = Истина;
				ТекстСообщения = ВернутьСтр("ru = 'Не введен сетевой каталог Windows'");
			...
			Если Не ЗначениеЗаполнено(Объект.СетевойКаталогLinux) Тогда
				Отказ = Истина;
				ТекстСообщения = ВернутьСтр("ru = 'Не введен сетевой каталог Linux'");
		...
		Если Объект.ИспользоватьПапку Тогда
			Если Не ЗначениеЗаполнено(Объект.Папка) Тогда
				Отказ = Истина;
				ТекстСообщения = ВернутьСтр("ru = 'Не выбрана папка'");
		...
		Если Объект.ИспользоватьЭлектроннуюПочту Тогда
			Если Объект.Личная Тогда
				Если Не ЗначениеЗаполнено(Объект.ВидПочтовогоАдресаПолучателей) Тогда
					Отказ = Истина;
					ТекстСообщения = ВернутьСтр("ru = 'Не выбран почтовый адрес'");
			...
			Иначе
				...
				Если Не ЗначениеЗаполнено(Объект.ВидПочтовогоАдресаПолучателей) Тогда
					Отказ = Истина;
					ТекстСообщения = ВернутьСтр("ru = 'Не выбран тип почтового адреса получателей'");
			...
			Если Не ЗначениеЗаполнено(Объект.УчетнаяЗапись) Тогда
				Отказ = Истина;
				ТекстСообщения = ВернутьСтр("ru = 'Не выбрана учетная запись для отправки'");
	...
	Для Каждого СтрокаОтчеты Из Объект.Отчеты Цикл
		...
		Если СтрокаОтчеты.СКД Тогда
			...
			Если Не ЗаполненыВсеОбязательныеНастройки Тогда
				...
				ТекстСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					ВернутьСтр("ru = 'Для отчета ''%1'' заполнены не все настройки, обязательные для заполнения. Необходимо заполнить все обязательные настройки или снять флажок ''Подготовлена''.'"),
		...
		Если ПроверитьПерсонализированные Тогда
			...
			Если ОтчетПерсонализирован Тогда
				Отказ = Истина;
				ТекстСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					ВернутьСтр("ru = 'Для отчета ''%1'' задан отбор по получателям рассылки. Необходимо отключить отбор по получателям или изменить вид рассылки на ''Свой отчет для каждого получателя''.'"),
					Строка(СтрокаОтчеты.Отчет));
				Поле = "Отчеты["+ Формат(ТекущийОбъект.Отчеты.Индекс(ОбъектСтрокаОтчеты), "ЧН=0; ЧГ=0") +"].Представление";
				ОбщегоНазначенияКлиентСервер.СообщитьПользователю(ТекстСообщения, ТекущийОбъект, Поле);
			КонецЕсли;
...
Процедура ТипПолучателейРассылкиОбработкаВыбора(Элемент, ВыбранноеЗначение, СтандартнаяОбработка)
	...
	Если Объект.Получатели.Количество() > 0 Тогда
		...
		СтрокаВопроса = ВернутьСтр("ru = 'Для продолжения необходимо очистить список получателей.'");
		
		Кнопки = Новый СписокЗначений;
		Кнопки.Добавить(КодВозвратаДиалога.Да, ВернутьСтр("ru = 'Очистить'"));
...
Процедура FTPСерверИКаталогНачалоВыбора(Элемент, ДанныеВыбора, СтандартнаяОбработка)
	...
	ПараметрыПроизвольнойФормы.Вставить("Заголовок", ВернутьСтр("ru = '<Укажите получателя>'"));
...
Процедура ПапкаОбработкаВыбора(Элемент, ВыбранноеЗначение, СтандартнаяОбработка)
	Если Не ПравоИзмененияПапкиИФайлов(ВыбранноеЗначение) Тогда
		СтандартнаяОбработка = Ложь;
		ТекстПредупреждения = ВернутьСтр("ru = 'Недостаточно прав для изменения файлов папки ""%1"".'");
...
Процедура РодительОбработкаВыбора(Элемент, ВыбранноеЗначение, СтандартнаяОбработка)
	Если ВыбранноеЗначение = Кэш.ГруппаЛичныхРассылок Тогда
		СтандартнаяОбработка = Ложь;
		ПоказатьПредупреждение(, ВернутьСтр("ru = 'Выбранная группа используется только для личных рассылок по электронной почте'"));
	КонецЕсли;
КонецПроцедуры
...
Процедура ОтчетыОбработкаВыбора(Элемент, ВыбранноеЗначение, СтандартнаяОбработка)
	...
	Если СтруктураВыбора.СОшибками.Количество > 0 Тогда
		...
		Если СтруктураВыбора.Выбрано.Количество = 1 Тогда
			ТекстСообщения = ВернутьСтр("ru = 'Не удалось добавить отчет из-за ошибок'");
		ИначеЕсли СтруктураВыбора.Успешно.Количество = 0 Тогда
			ТекстСообщения = ВернутьСтр("ru = 'Не удалось добавить отчеты из-за ошибок'");
		ИначеЕсли СтруктураВыбора.Выбрано.Количество >= 6 Тогда
			ТекстСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				ВернутьСтр("ru = 'Добавлено отчетов: %1 из %2,
		...
		Иначе
			...
			ТекстСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				ВернутьСтр("ru = 'Успешно добавлены отчеты:
					|%1
					|
		...
		ПараметрыПредупреждения.Заголовок = ВернутьСтр("ru = 'Ошибки добавления отчетов в рассылку'");
	...
	Иначе
		...
		Если СтруктураВыбора.Успешно.Количество = 0 Тогда
			...
			ТекстОповещения = ВернутьСтр("ru = 'Все выбранные отчеты уже включены в рассылку'");
		Иначе
			Если СтруктураВыбора.Выбрано.Количество = 1 Тогда
				ЗаголовокОповещения = ВернутьСтр("ru = 'Отчет включен в рассылку'");
			Иначе
				ЗаголовокОповещения = ВернутьСтр("ru = 'Отчеты включены в рассылку'");
			КонецЕсли;
...
Процедура ПолучателиРассылкиНажатие(Элемент, СтандартнаяОбработка)
	...
	Если Не ЗначениеЗаполнено(Объект.ТипПолучателейРассылки) Тогда
		ТекстОшибки = ВернутьСтр("ru = 'Для ввода получателей необходимо выбрать их тип'");
...
Процедура КомандаВыполнитьСейчас(Команда)
	Если Не Объект.Подготовлена Тогда
		ПоказатьПредупреждение(, ВернутьСтр("ru = 'Рассылка не подготовлена'"));
		Возврат;
	КонецЕсли;
	ПараметрыЗаписи = Новый Структура;
...
Процедура ПредварительныйПросмотрОтчета(Команда)
	...
	СтрокаОтчеты = Элементы.Отчеты.ТекущиеДанные;
	Если СтрокаОтчеты = Неопределено Тогда
		ПоказатьПредупреждение(, ВернутьСтр("ru = 'Выберите отчет'"));
		Возврат;
	КонецЕсли;
...
Процедура ИспользоватьПолучателяРассылкиВНастройкеОтчета(Команда)
	...
	Если Не Объект.Персонализирована Тогда
		...
		СообщениеТекст = ВернутьСтр("ru = 'Использовать получателя в параметрах возможно только для вида рассылки ""%1""'");
	...
	Если КоличествоТипов <> 1 И КоличествоТипов <> 2 Тогда
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(ВернутьСтр("ru = 'Поле ""Получатели"" не заполнено'"), , "ТипПолучателейРассылки");
		Возврат;
	КонецЕсли;
	...
	Если НайденныеИОМД.Количество() <> 1 Тогда
		ПоказатьПредупреждение(, ВернутьСтр("ru = 'Тип получателей не найден'"));
		Возврат;
	КонецЕсли;
	...
	Если СКД Тогда
		...
		Если ИдентификаторНастройки = Неопределено Тогда
			ПоказатьПредупреждение(, ВернутьСтр("ru = 'Выберите настройку отчета'"));
			Возврат;
		КонецЕсли;
		...
		Если СтрокаНастроек = Неопределено Тогда
			ПоказатьПредупреждение(, ВернутьСтр("ru = 'Настройка отчета не найдена'"));
			Возврат;
		КонецЕсли;
		...
		Иначе
			ПоказатьПредупреждение(, ВернутьСтр("ru = 'Указывать получателя можно только для параметров и отборов отчетов'"));
			Возврат;
		КонецЕсли;
		...
		Если ЭтоЭлементОтбора Тогда
			...
			Иначе
				...
				КонецЕсли;
				Если ПолеКД = Неопределено Тогда
					ПоказатьПредупреждение(, ВернутьСтр("ru = 'Для настройки отчета не найдено описание доступного поля'"));
					Возврат;
				КонецЕсли;
	...
	Если Не ОписаниеДоступныхТипов.СодержитТип(ОсновнойТипПолучателей) Тогда
		ТекстПредупреждения = ВернутьСтр("ru = 'Тип ""%1"" не подходит по типу к выбранной настройке.
...
Процедура ЗаполнитьРасписаниеПоШаблону(Команда)
	...
	СписокВариантов.ПоказатьВыборЭлемента(Обработчик, ВернутьСтр("ru = 'Выберите шаблон расписания'"));
КонецПроцедуры
...
Процедура ДобавитьШаблонПолучателя(Команда)
	...
	Если НЕ Объект.Персонализирована Тогда
		СообщениеТекст = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			ВернутьСтр("ru = 'Использование получателя в тексте шаблона возможно только для вида рассылки ""%1""'"),
...
Процедура ДобавитьШаблонПоУмолчанию(Команда)
	...
	ИначеЕсли ЗначениеПредмета = ШаблонПоУмолчанию Тогда
		...
		Если ПерезаполняетсяТема Тогда
			ТекстПредупреждения = ВернутьСтр("ru = 'Тема письма уже соответствует шаблону по умолчанию.'");
		Иначе
			ТекстПредупреждения = ВернутьСтр("ru = 'Текст письма уже соответствует шаблону по умолчанию.'");
		КонецЕсли;
		ПоказатьПредупреждение(, ТекстПредупреждения);
	...
	Иначе
		...
		Если ПерезаполняетсяТема Тогда
			ЗаголовокВопроса = ВернутьСтр("ru = 'Добавить в тему письма шаблон по умолчанию'");
			ТекстВопроса = ВернутьСтр("ru = 'Заменить тему письма на шаблон по умолчанию?'");
		Иначе
			ЗаголовокВопроса = ВернутьСтр("ru = 'Добавить в текст письма шаблон по умолчанию'");
			ТекстВопроса = ВернутьСтр("ru = 'Заменить текст письма на шаблон по умолчанию?'");
		КонецЕсли;
		
		Кнопки = Новый СписокЗначений;
		Кнопки.Добавить(1, ВернутьСтр("ru = 'Заменить'"));
		Кнопки.Добавить(2, ВернутьСтр("ru = 'Добавить'"));
		Кнопки.Добавить(КодВозвратаДиалога.Отмена);
...
Процедура ПроверитьРассылкуПослеВыбораПолучателей(РезультатВыбора, ПараметрыДоставки) Экспорт
	...
	Если ПараметрыДоставки.ИспользоватьЭлектроннуюПочту Тогда
		...
		ПараметрыДоставки.Вставить("ШаблонТемы",    ВернутьСтр("ru = 'Тестовое сообщение 1С:Предприятие'"));
		ПараметрыДоставки.Вставить("ШаблонТекста",  ВернутьСтр("ru = 'Это сообщение отправлено системой рассылок 1С:Предприятие.'") + Символы.ПС + Кэш.ЗаголовокСистемы);
...
Функция ВыбратьФормат(СсылкаНаОтчет, ОбработчикРезультата)
	...
	Если ЭтоФорматПоУмолчанию Тогда
		ЗаголовокДиалога = ВернутьСтр("ru = 'Выберите форматы по умолчанию'");
	Иначе
		ЗаголовокДиалога = ВернутьСтр("ru = 'Выберите форматы для отчета ""%1""'");
...
Процедура ПроверитьРассылку(ПараметрыДоставки)
	...
	Если Не Объект.Подготовлена Или Объект.Ссылка.Пустая() Тогда
		ЗаголовокВопроса = ВернутьСтр("ru = 'Проверка способа доставки'");
		Если Не Объект.Подготовлена Тогда
			ТекстВопроса = ВернутьСтр("ru = 'Перед проверкой рассылка должна быть подготовлена.
			|Нажмите ""Продолжить"", чтобы установить флажок ""Подготовлена"" и записать рассылку.'");
		Иначе
			ТекстВопроса = ВернутьСтр("ru = 'Перед проверкой рассылка должна быть записана.
			|Нажмите ""Продолжить"", чтобы записать рассылку.'");
		КонецЕсли;
		
		Кнопки = Новый СписокЗначений;
		Кнопки.Добавить(1, ВернутьСтр("ru = 'Продолжить'"));
		Кнопки.Добавить(КодВозвратаДиалога.Отмена);
...
// { RGS ASeryakov 13.03.2018 10:00:00 PM SLI-0007499
&НаКлиенте
Процедура УстановитьДоступностьГруппаПараметрыНастроекПериода(Архивировать)
	 Элементы.ГруппаПараметрыНастроекПериода.Доступность = НЕ Архивировать;
КонецПроцедуры
// } RGS ASeryakov 13.03.2018 10:00:00 PM SLI-0007499
...
Процедура ВидимостьДоступностьКорректность(Форма, Изменения = "")
	...
	Если Изменения = "" Или Изменения = "ВидРассылки" Тогда
		...
		Если Объект.Личная <> ИспользуетсяГруппаЛичныхРассылок Тогда
			УстановитьМодифицированностьФормы(Форма, "Родитель", , 
				ВернутьСтр("ru = 'Группа установлена в соответствии с видом рассылки'"));
	...
	Если Изменения = "" Или Изменения = "Отчеты" Тогда
		...
		Если КоличествоОтчетов > 0 Тогда
			Элементы.СтраницаОтчеты.Заголовок = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				ВернутьСтр("ru = 'Отчеты (%1)'"), 
				Формат(КоличествоОтчетов, "ЧН=0; ЧГ="));
		Иначе
			Элементы.СтраницаОтчеты.Заголовок = ВернутьСтр("ru = 'Отчеты'") ;
		КонецЕсли;
	КонецЕсли;
	...
	Если Изменения = ""
		...
		СпособыДоставки = "";
		Если Объект.ИспользоватьПапку Тогда
			СпособыДоставки = ВернутьСтр("ru = 'папка'");
		КонецЕсли;
		Если Объект.ИспользоватьСетевойКаталог Тогда
			СпособыДоставки = ВернутьСтр("ru = 'сетевой каталог'");
		КонецЕсли;
		Если Объект.ИспользоватьFTPРесурс Тогда
			СпособыДоставки = ВернутьСтр("ru = 'FTP'");
		КонецЕсли;
		Если Объект.ИспользоватьЭлектроннуюПочту И Не Объект.ТолькоУведомить Тогда
			СпособыДоставки = СпособыДоставки + ?(СпособыДоставки = "", ВернутьСтр("ru = 'эл. почта'"), " "+ ВернутьСтр("ru = 'и эл. почта'"));
		КонецЕсли;
		
		Элементы.СтраницаДоставка.Заголовок = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(ВернутьСтр("ru = 'Доставка (%1)'"), СпособыДоставки);
	КонецЕсли;
	...
	Если Изменения = "" Или Изменения = "ВыполнятьПоРасписанию" Тогда
		Если Объект.ВыполнятьПоРасписанию Тогда
			Элементы.СтраницаРасписание.Заголовок = ВернутьСтр("ru = 'Расписание (активно)'");
		Иначе
			Элементы.СтраницаРасписание.Заголовок = ВернутьСтр("ru = 'Расписание (не активно)'");
		КонецЕсли;
	...
	Если Изменения = "" Или Изменения = "ПериодичностьРасписания" Тогда
		...
		ИначеЕсли ИмяПеречисления = "Ежемесячно" Тогда
			...
			Если Форма.Расписание.ДеньВМесяце >= 0 Тогда
				...
				Элементы.ГиперссылкаНачалоКонецМесяца.Заголовок = ВернутьСтр("ru = 'начала'");
			Иначе
				Форма.ДеньВМесяце = -Форма.Расписание.ДеньВМесяце;
				Элементы.ГиперссылкаНачалоКонецМесяца.Заголовок = ВернутьСтр("ru = 'конца'");
			КонецЕсли;
...
Функция ПредставлениеПолучателей(Форма)
	...
	Если Представление.Всего = 0 Тогда
		Представление.Краткое = ВернутьСтр("ru = '<Укажите получателей>'");
	...
	Если ПредставлениеОтключенных.Всего > 0 Тогда
		ШаблонРазделения = ВернутьСтр("ru = 'Кроме'")+ ": ";
	...
	Если Представление.МаксимумПревышен
		...
		Если ПредставлениеОтключенных.Всего > 0 Тогда
			ШаблонОкончания = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				ВернутьСтр("ru = '(всего %1, исключая %2)'"),
				Представление.Всего,
				ПредставлениеОтключенных.Всего);
		Иначе
			ШаблонОкончания = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				ВернутьСтр("ru = '(всего %1)'"),
				Представление.Всего);
		КонецЕсли;
...
Функция ПредставлениеФорматовПоУмолчанию()
	Возврат ВернутьСтр("ru = 'по умолчанию'");
КонецФункции
...
Функция ПолучателиУказаны(Получатели)
	Для Каждого СтрокаТаблицы Из Получатели Цикл
		...
		КонецЕсли;
	КонецЦикла;
	ТекстСообщения = ВернутьСтр("ru = 'Не выбрано ни одного получателя'");
	ОбщегоНазначенияКлиентСервер.СообщитьПользователю(ТекстСообщения, , "ПолучателиРассылки");
	Возврат Ложь;
КонецФункции
...
Функция ПроверитьСпособДоставки(Рассылка, Знач ПараметрыДоставки)
	...
	ПараметрыЖурнала = Новый Структура;
	ПараметрыЖурнала.Вставить("ИмяСобытия",   ВернутьСтр("ru = 'Рассылка отчетов. Проверка способа доставки'", ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка()));
	...
	Если Выполнена Тогда
		РезультатВыполнения.Текст = ВернутьСтр("ru = 'Тест способа доставки успешно пройден.'");
	Иначе
		РезультатВыполнения.Текст = ВернутьСтр("ru = 'Тест способа доставки не пройден.'");
...
Функция ИнициализироватьОтчет(СтрокаОтчеты, ДобавитьДежурнуюФразу, ПользовательскиеНастройки, Интерактивно = Истина)
	// Параметры журнала
	ПараметрыЖурнала = Новый Структура;
	ПараметрыЖурнала.Вставить("ИмяСобытия",   ВернутьСтр("ru = 'Рассылка отчетов. Инициализация отчета'", ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка()));
	...
	Если Не СтрокаОтчеты.Инициализирован Тогда
		ПараметрыОтчета.Ошибки = (
			СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				ВернутьСтр("ru = 'Отчет %1 был удален из-за ошибки инициализации'"),
...
Процедура ДобавитьНастройкиОтчетов(ПрисоединяемыеОтчеты)
	...
	Для Каждого СтрокаОтчетыПараметры Из ПрисоединяемыеОтчеты Цикл
		...
		Если Не ЭтоНовый Тогда
			Если Найденные.Количество() > 0 Тогда
				ШаблонСтрокиСообщения = ВернутьСтр("ru = 'Для отчета ""%1"" загружены новые пользовательские настройки.'");
			Иначе
				ШаблонСтрокиСообщения = ВернутьСтр("ru = 'Добавлен отчет ""%1"".'");
			КонецЕсли;
...
Процедура ЗаписатьНаКлиенте(Результат, ПараметрыЗаписи) Экспорт
	...
	Если ПараметрыЗаписи.Шаг = 3 И ЖелательноОтключитьАрхивацию() Тогда
		...
		// Вопрос.
		ЗаголовокВопроса = ВернутьСтр("ru = 'Отключить архивацию'");
		ТекстВопроса = ВернутьСтр("ru = 'При публикации отчетов в папку рекомендуется отключать архивацию в ZIP.'");
		
		Кнопки = Новый СписокЗначений;
		Кнопки.Добавить(КодВозвратаДиалога.Да, ВернутьСтр("ru = 'Отключить архивацию в ZIP'"));
		Кнопки.Добавить(КодВозвратаДиалога.Пропустить, ВернутьСтр("ru = 'Продолжить'"));
		Кнопки.Добавить(КодВозвратаДиалога.Отмена);
...
Процедура ВыполнитьСейчасЗавершение(Получатели, ДополнительныеПараметры) Экспорт
	...
	Если ДополнительныеПараметры.ЭтоФормаЭлемента Тогда
		...
		КонецЕсли;
		ТекстСостояния = ВернутьСтр("ru = 'Выполняется рассылка отчетов'");
	Иначе
		ТекстСостояния = ВернутьСтр("ru = 'Выполняются рассылки отчетов'");
	КонецЕсли;
	...
	ИначеЕсли Результат.Статус = "Исключение" Тогда
		ТекстПредупреждения = ВернутьСтр("ru = 'Не удалось выполнить рассылку отчетов.
...
Функция ВыполнитьРассылкиВФоновомЗадании(ПараметрыВызоваСервера, Знач УникальныйИдентификатор)
	...
	Попытка
		РезультатФоновогоЗадания = ДлительныеОперации.ЗапуститьВыполнениеВФоне(
			...
			ВернутьСтр("ru = 'Рассылки отчетов: Выполнение рассылок в фоне'"));
