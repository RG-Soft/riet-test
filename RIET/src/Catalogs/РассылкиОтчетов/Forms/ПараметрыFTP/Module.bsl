...
Процедура Заполнить(Команда)
	Если Сервер = "" Тогда
		ПолныйАдрес = ВернутьСтр("ru = 'ftp://логин:пароль@сервер:порт/каталог/'");
	Иначе
		Если Логин = "" Тогда
	...
	ПоказатьВводСтроки(Обработчик, ПолныйАдрес, ВернутьСтр("ru = 'Введите полный ftp адрес'"))
КонецПроцедуры
...
&НаКлиенте
Функция ПарольСкрыт()
	Возврат ВернутьСтр("ru = '<пароль скрыт>'");
КонецФункции
