...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Заголовок = Параметры.НаименованиеРассылки;
...
Процедура ОткрытьФормаДобавленияПолучателей(ЭтоПодбор)
	...
	Если ЭтоПодбор Тогда
		...
		ПараметрыФормыВыбора.Вставить("ЗаголовокФормыПодбора", ВернутьСтр("ru = 'Подбор получателей рассылки'"));
...
Процедура ДобавитьПеретащитьПолучателя(ПолучательИлиНаборПолучателей)
	...
	Если МассивНовыхСтрок.Количество() > 0 Тогда
		Если МассивНовыхСтрок.Количество() = 1 Тогда
			ЗаголовокОповещения = ВернутьСтр("ru = 'Получатель добавлен в рассылку'");
		Иначе
			ЗаголовокОповещения = ВернутьСтр("ru = 'Получатели добавлены в рассылку'");
		КонецЕсли;
...
Процедура ЗаполнитьПочтовыеАдреса()
	...
	ПараметрыПолучателей.ТипПолучателейРассылки = ИОМД;
...
Процедура УстановитьУсловноеОформление()
	...
	Инструкция.Поля = "ПолучателиПолучатель, ПолучателиИсключен";
	Инструкция.Отборы.Вставить("Получатели.Исключен", Истина);
...
Процедура ПередЗакрытием(Отказ, СтандартнаяОбработка)
	Если Получатели.Количество() > 0 Тогда
		...
		КоличествоПолучателей = Получатели.НайтиСтроки(Фильтр).Количество();
		Если КоличествоПолучателей = 0 Тогда 
			Отказ = Истина;
			ПоказатьПредупреждение(, ВернутьСтр("ru = 'Требуется указать хотя бы одного получателя'"));
		КонецЕсли;
	КонецЕсли;
КонецПроцедуры
