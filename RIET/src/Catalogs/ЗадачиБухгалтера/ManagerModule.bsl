...
// С 1 января 2018 года меняется срок представления квартальной формы П (услуги).
// Так как настройка представляемых форм статистики выполняется на уровне правил,
// то правило для новой формы следует включить всем тем, у кого было включено правило для старой формы.
//
Процедура ВключитьФормыСтатистики2018() Экспорт

	// Правила, относящиеся к новым формам статистики, добавлены в версии 3.0.56.16.
	// Обработчик включения правил выполняется в следующей версии - 3.0.56.17 - чтобы выполнить его точно после того,
	// как новое правило создано в информационной базе.
	
	ЗаменаТребований = НовыйЗаменаТребований();
	
	Изменение = ЗаменаТребований.Добавить();
	Изменение.ИмяСтарогоПравила = "2017_ПУслуги";
	Изменение.ИмяНовогоПравила  = "2018_ПУслугиКвартальная";
	
	ВключитьНовыеНалогиОтчеты("СтатистическаяОтчетность", ЗаменаТребований);
...
Функция НовыйЗаменаТребований()
	
	ЗаменаТребований = Новый ТаблицаЗначений;
	ОписаниеТипаИмяПравила = ОбщегоНазначения.ОписаниеТипаСтрока(Метаданные.Справочники.ПравилаПредставленияОтчетовУплатыНалогов.ДлинаКода);
	ЗаменаТребований.Колонки.Добавить("ИмяСтарогоПравила", ОписаниеТипаИмяПравила);
	ЗаменаТребований.Колонки.Добавить("ИмяНовогоПравила",  ОписаниеТипаИмяПравила);
	
	Возврат ЗаменаТребований;
	
КонецФункции

Процедура ВключитьНовыеНалогиОтчеты(ИмяЗадачи, ЗаменаТребований)

	МенеджерВременныхТаблиц = Новый МенеджерВременныхТаблиц;
	
	ИменаНовыхПравил  = ЗаменаТребований.ВыгрузитьКолонку("ИмяНовогоПравила");
	ИменаСтарыхПравил = ЗаменаТребований.ВыгрузитьКолонку("ИмяСтарогоПравила");
	...
	Запрос.Текст = 
	"ВЫБРАТЬ
	|	ЗадачаБухгалтера.Ссылка КАК Ссылка
	|ПОМЕСТИТЬ ЗадачаБухгалтера
	|ИЗ
	|	Справочник.ЗадачиБухгалтера КАК ЗадачаБухгалтера
	|ГДЕ
	|	ЗадачаБухгалтера.Код = &ИмяЗадачи
	|;
	|
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ
	...
	|	РегистрСведений.НалогиОтчеты КАК НалогиОтчеты
	|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ Справочник.ПравилаПредставленияОтчетовУплатыНалогов КАК Правила
	|			ВНУТРЕННЕЕ СОЕДИНЕНИЕ ЗадачаБухгалтера КАК ЗадачаБухгалтера
	|			ПО Правила.Владелец = ЗадачаБухгалтера.Ссылка
	|		ПО НалогиОтчеты.НалогОтчет = Правила.Ссылка
	|ГДЕ
	|	Правила.Код В(&ИменаСтарыхПравил)
	|;
	...
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ ПЕРВЫЕ 1
	|	ВыполненныеНастройки.Организация КАК Организация
	|ИЗ
	|	НастройкиПрошлыхЛет КАК ВыполненныеНастройки";
	Если Запрос.Выполнить().Пустой() Тогда
		// Никому не нужны
	...
	Запрос = Новый Запрос;
	Запрос.МенеджерВременныхТаблиц = МенеджерВременныхТаблиц;
	Запрос.УстановитьПараметр("ИзмененияТребований",  ЗаменаТребований);
	Запрос.Текст =
	"ВЫБРАТЬ
	|	Таблица.ИмяНовогоПравила КАК ИмяНовогоПравила,
	|	Таблица.ИмяСтарогоПравила КАК ИмяСтарогоПравила
