...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если ЗначениеЗаполнено(ИмяСправочникаБанки) Тогда
		СписокСоглашенийСБанками.ТекстЗапроса = СтрЗаменить(СписокСоглашенийСБанками.ТекстЗапроса,
			"Соглашение.Контрагент", "ВЫРАЗИТЬ(Соглашение.Контрагент КАК Справочник." + ИмяСправочникаБанки + ")");
	КонецЕсли;
	...
	Если ЗначениеЗаполнено(Контрагент) Тогда
		...
		Этаформа.Заголовок = ВернутьСтр("ru = 'Настройки ЭДО с контрагентами'");
	...
	Если ЗначениеЗаполнено(ПрофильНастроекЭДО) Тогда
		ОбщегоНазначенияКлиентСервер.УстановитьЭлементОтбора(СписокСоглашенийСКонтрагентами.Отбор, "ПрофильНастроекЭДО",
			ПрофильНастроекЭДО, ВидСравненияКомпоновкиДанных.Равно);
	...
	Если ЗначениеЗаполнено(Параметры.Банк) Тогда
		...
		Этаформа.АвтоЗаголовок = Ложь;
		Этаформа.Заголовок = ВернутьСтр("ru = 'Настройки ЭДО с банками'");
		Элементы.ГруппаСоглашения.ОтображениеСтраниц = ОтображениеСтраницФормы.Нет;
...
Процедура ЗагрузитьНастройки(Команда)
	...
	Обработчик = Новый ОписаниеОповещения("ОбработкаВыбораФайлаНастроек", ЭтотОбъект);
...
Процедура СписокСоглашенийСБанкамиПередНачаломДобавления(Элемент, Отказ, Копирование, Родитель, Группа)
	...
	ОткрытьФорму(
		"Справочник.СоглашенияОбИспользованииЭД.Форма.ФормаЭлементаБанк", Параметр, Элементы.СписокСоглашенийСБанками);
...
&НаКлиенте
Процедура ОбработкаВыбораФайлаНастроек(Результат, Адрес, ИмяФайла, ДополнительныеПараметры) Экспорт
	
	Если Результат = Истина Тогда
		ПараметрыФормы = Новый Структура("НастройкиЗаполнения", Адрес);
