...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если Параметры.Свойство("НастройкаЭДО", НастройкаЭДО) И ЗначениеЗаполнено(НастройкаЭДО) Тогда
		...
		ШаблонНастройкиЭДО = ВернутьСтр("ru = 'Настройка ЭДО с контрагентом %1'");
...
&НаКлиенте
Процедура ТекстПриглашенияНачалоВыбора(Элемент, ДанныеВыбора, СтандартнаяОбработка)
	
	СтандартнаяОбработка = Ложь;
	
	Оповещение = Новый ОписаниеОповещения("ЗавершитьРедактированиеТекстаПриглашения", ЭтотОбъект);
	ЗаголовокФормы = ВернутьСтр("ru='Текст для приглашения контрагента'");
	ОбщегоНазначенияКлиент.ПоказатьФормуРедактированияМногострочногоТекста(
		Оповещение, Элементы.ТекстПриглашения.ТекстРедактирования, ЗаголовокФормы);
...
Функция ПринятьОтклонитьКонтактЧерезОператораЭДОНаСервере(Идентификатор, ПриглашениеПринято, Маркер)
	...
	Если ИскомаяНастройкаЭДО.НастройкаЭДОУникальна() Тогда
		Результат = ЭлектронныеДокументыВнутренний.ПринятьОтклонитьКонтактЧерезОператораЭДО(Идентификатор, ПриглашениеПринято, Маркер);
	КонецЕсли;
...
Процедура ЗаполнитьМаркер(ОбработчикОповещения)
	...
	ЭлектронныеДокументыСлужебныйКлиент.ПолучитьСоответсвиеСоглашенийИПараметровСертификатов(ОбработчикОповещения, Массив);
...
Процедура ОтправитьПриглашениеОповещение(Результат, ДополнительныеПараметры) Экспорт
	...
	Если ЗначениеЗаполнено(Маркер) Тогда
		...
		КонецЕсли;
	Иначе
		ШаблонОшибки = ВернутьСтр("ru = 'При отправке приглашения возникли ошибки.
...
Процедура ОтклонитьПриглашениеОповещение(Результат, ДополнительныеПараметры) Экспорт
	...
	Если ЗначениеЗаполнено(Маркер) Тогда
		...
		КонецЕсли;
	Иначе
		ШаблонОшибки = ВернутьСтр("ru = 'При отклонении приглашения возникли ошибки.
			|Необходимо выполнить тест настроек ЭДО с контрагентом %1.'");
...
Процедура ПринятьПриглашениеОповещение(Результат, ДополнительныеПараметры) Экспорт
	...
	Если ЗначениеЗаполнено(Маркер) Тогда
		...
		КонецЕсли;
	Иначе
		ШаблонОшибки = ВернутьСтр("ru = 'При принятии приглашения возникли ошибки.
			|Необходимо выполнить тест настроек ЭДО с контрагентом %1.'");
