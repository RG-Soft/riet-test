...
Процедура ПередЗаписью(Отказ)
	...
	Если Не ДополнительныеСвойства.Свойство("ЗаполнениеПредопределенных") Тогда
		ВызватьИсключение ВернутьСтр("ru = 'Справочник ""Предопределенные варианты отчетов"" изменяется только при автоматическом заполнении его данных.'");
	КонецЕсли;
КонецПроцедуры
...
Процедура ПроверитьЗаполнениеПредопределенного(Отказ)
	...
	ИначеЕсли Не ЗначениеЗаполнено(Отчет) Тогда
		ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(ВернутьСтр("ru = 'Не заполнено поле ""%1""'"), "Отчет");
	Иначе
		Возврат;
	КонецЕсли;
КонецПроцедуры
