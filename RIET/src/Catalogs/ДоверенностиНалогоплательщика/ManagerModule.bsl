...
Процедура ОбработкаПолученияФормы(ВидФормы, Параметры, ВыбраннаяФорма, ДополнительнаяИнформация, СтандартнаяОбработка)
	...
	И Метаданные.Справочники.ДоверенностиНалогоплательщика.Формы.Найти(ИмяУпрощеннойФормы) <> Неопределено
	И (РольДоступна("ПолныеПрава") ИЛИ РольДоступна("Пользователь"))Тогда
...
Процедура ДобавитьКомандыПечати(КомандыПечати) Экспорт
	...
	КомандаПечати.Представление = ВернутьСтр("ru = 'Печать'");
...
Процедура Печать(МассивОбъектов, ПараметрыПечати, КоллекцияПечатныхФорм, ОбъектыПечати, ПараметрыВывода) Экспорт
	...
	Для каждого СтрокаМассиваОбъектов Из МассивОбъектов Цикл
		УправлениеПечатью.ВывестиТабличныйДокументВКоллекцию(КоллекцияПечатныхФорм, "ДоверенностьНалогоплательщика", ВернутьСтр("ru='Доверенность налогоплательщика'"), СформироватьДоверенность(СтрокаМассиваОбъектов));
	КонецЦикла; 	
...
Функция СформироватьДоверенность(Знач Доверенность) Экспорт
	...
	Макет = УправлениеПечатью.ПолучитьМакет("Справочник.ДоверенностиНалогоплательщика.ПФ_MXL_Доверенность");
	...
	Шапка.Параметры.ДатаВыдачи = Формат(Доверенность.ДатаВыдачи, "ДФ='«dd» MMMM yyyy'")+ВернутьСтр("ru=' года.'");
	//Обновление рег. Отчетности на бух. Корп 3.0.36.18
	//Шапка.Параметры.НомерДокумента = ПрефиксацияОбъектовКлиентСервер.ПолучитьНомерНаПечать(Доверенность.НомерДовер, Ложь, Истина);
	...
	Если ЗначениеЗаполнено(УдЛичнПредстФЛ) Тогда
		Подвал.Параметры.ДанныеУдостоверения_Представитель = "серия "+УдЛичнПредстФЛ.СерДок+", № " +УдЛичнПредстФЛ.НомДок+", выдан "+ УдЛичнПредстФЛ.ВыдДок + " "+Формат(УдЛичнПредстФЛ.ДатаДок, "ДФ='«dd» MMMM yyyy'")+ВернутьСтр("ru=' года.'");
	КонецЕсли;
