...
Функция ПечатьАктаСверки(ИмяМакета)
		...
		Если ИмяМакета = "АктСверки1" Тогда
			...
			УсловиеЗапроса = ?(ЗначениеЗаполнено(ИнвойсинговыйЦентр), "Сделка.ИнвойсинговыйЦентр = &ИнвойсинговыйЦентр","");
		...
		// { RGS LFedotova 18.12.2018 13:03:58 - вопрос SLI-0007784
		ТабПоДаннымОрганизации.Колонки.Добавить("КодДоговора");
		Для каждого Строка Из ТабПоДаннымОрганизации Цикл
			Строка.КодДоговора = Строка.Договор.Код;
		КонецЦикла; 
		ТабПоДаннымОрганизации.Сортировать("КодДоговора,Дата");
		// } RGS LFedotova 18.12.2018 13:04:12 - вопрос SLI-0007784 
...
Функция ПечатьДополненияАктаСверки()
		...
		Если не ОбщегоНазначения.ЗначениеНеЗаполнено(ИнвойсинговыйЦентр) Тогда
			Запрос.Текст = Запрос.Текст +"   И Сделка.ИнвойсинговыйЦентр = &ИнвойсинговыйЦентр";  
			Запрос.УстановитьПараметр("ИнвойсинговыйЦентр", ИнвойсинговыйЦентр);
		КонецЕсли;
...
Процедура ЗаполнитьПоДаннымБухгалтерскогоУчета(ВыводитьКредитНоты) Экспорт
		...
		?(ЗначениеЗаполнено(ИнвойсинговыйЦентр), "  И ХозрасчетныйОбороты.Регистратор.ИнвойсинговыйЦентр = &ИнвойсинговыйЦентр","") +
