...
Функция ПечатьАкта(МассивПараметров, ОбъектыПечати) Экспорт
	...
	Если СокрЛП(Ссылка.IctDocType) = "IS" Тогда
		...
		|	InventoryСписаниеGoods.Good.Код КАК НомерПартии,
		|	СУММА(InventoryСписаниеGoods.Quantity) КАК Количество,
		|	СУММА(InventoryСписаниеGoods.ManagementPrice) КАК СтоимостьШтВал,
		|	СУММА(InventoryСписаниеGoods.ManagementSum) КАК СтоимостьВсегоВал,
		|	СУММА(ВЫБОР
		|			КОГДА InventoryBatchesОбороты.QuantityРасход <> 0
		|				ТОГДА InventoryBatchesОбороты.SumРасход / InventoryBatchesОбороты.QuantityРасход
		|			ИНАЧЕ InventoryBatchesОбороты.SumРасход
		|		КОНЕЦ) КАК СтоимостьШт,
		|	InventoryСписаниеGoods.Ссылка КАК Ссылка,
		|	InventoryСписаниеGoods.UOM.Код КАК ЕдИзм,
		|	ВЫРАЗИТЬ(InventoryСписаниеGoods.Good.DescriptionRus КАК СТРОКА(250)) КАК НоменклатураРус
		...
		|	СУММА(ВЫРАЗИТЬ(ВТ.СтоимостьШтВал КАК ЧИСЛО(15, 2))) КАК СтоимостьШтВал,
		|	СУММА(ВЫРАЗИТЬ(ВТ.СтоимостьВсегоВал КАК ЧИСЛО(15, 2))) КАК СтоимостьВсегоВал,
		|	СУММА(ВЫРАЗИТЬ(ВТ.СтоимостьШт КАК ЧИСЛО(15, 2))) КАК СтоимостьШт,
		|	ВТ.Ссылка КАК Ссылка,
		|	ВТ.ЕдИзм,
		|	ВТ.НоменклатураРус,
		|	СУММА(ВЫРАЗИТЬ(ВТ.СтоимостьШт * ВТ.Количество КАК ЧИСЛО(15, 2))) КАК СтоимостьВсего
		|ИЗ
		|	ВТ КАК ВТ
		|
		|СГРУППИРОВАТЬ ПО
