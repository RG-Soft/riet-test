...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	//// это пока 
	//КонтекстЭДОСервер.ПометитьСообщениеКакПрочитанное(Объект);
	...
	ПечатьВозможнаДляЦиклаОбмена = ЗначениеЗаполнено(Объект.ЦиклОбмена) И (Объект.ЦиклОбмена.Тип = Перечисления.ТипыЦикловОбмена.НалоговаяИлиБухгалтерскаяОтчетность
		ИЛИ Объект.ЦиклОбмена.Тип = Перечисления.ТипыЦикловОбмена.Заявление ИЛИ Объект.ЦиклОбмена.Тип = Перечисления.ТипыЦикловОбмена.Форма2НДФЛ
		ИЛИ Объект.ЦиклОбмена.Тип = Перечисления.ТипыЦикловОбмена.ОтчетностьПФР ИЛИ Объект.ЦиклОбмена.Тип = Перечисления.ТипыЦикловОбмена.ОтчетностьФСГС);
		...
		Если ФайлыОшибкиОбработкиПакета.Количество() > 0 Тогда
			...
			Если ФайлыОшибкиОбработкиПакета.Количество() > 0 Тогда
				...
				Если ЗначениеЗаполнено(ИмяФайлаОшибки) Тогда
					...
					КонтекстЭДОСервер.УдалитьВременныйФайл(ИмяФайлаОшибки);
...
Процедура ЗаполнитьДеревоСодержимого(ВложенияТекущегоСообщения, Содержимое, ТПСодержимого)
	...
	Если ИменаПодписанныхФайловИзДругихСообщений.Количество() > 0 Тогда
		...
		Если НЕ ЗначениеЗаполнено(СообщениеВладелец) Тогда
			...
			//ИначеЕсли Объект.Тип = Перечисления.ТипыСодержимогоТранспортногоКонтейнера.ИзвещениеОПолученииДокументаФСГС
				//СообщениеВладелец = КонтекстЭДО.ПолучитьСообщениеЦиклаОбмена(Объект.ЦиклОбмена, Перечисления.ТипыТранспортныхСообщений.ПервичноеСообщениеСодержащееОтчетностьФСГС);
...
Процедура ОформитьСтрокуСодержимого(ДанныеСтроки)
		...
		ИначеЕсли ТипДанных = Перечисления.ТипыСодержимогоТранспортногоКонтейнера.ЭлектронноЦифроваяПодпись Тогда
			...
			Если ДанныеСтроки.ЭЦПЭтоПодписьАбонента Тогда
				ПредставлениеЯчейки = "Электронно-цифровая подпись абонента";
				Если Объект.ЦиклОбмена.Вид = Перечисления.ВидыЦикловОбмена.ЦиклОбменаСФСГС Тогда
					ПредставлениеЯчейки = "Электронно-цифровая подпись респондента";
				КонецЕсли;
			ИначеЕсли Объект.ЦиклОбмена.Вид = Перечисления.ВидыЦикловОбмена.ЦиклОбменаСПФР Тогда
				ПредставлениеЯчейки = "Электронно-цифровая подпись представителя ПФР";
			Иначе
				...
				И НЕ Объект.УчетнаяЗапись.ОбменНапрямую Тогда
					ПредставлениеЯчейки = "Электронно-цифровая подпись представителя спецоператора";
				Иначе
					Если Объект.ЦиклОбмена.Вид = Перечисления.ВидыЦикловОбмена.ЦиклОбменаСФСГС
					 ...
					 ИЛИ Объект.Тип = Перечисления.ТипыТранспортныхСообщений.ИзвещениеПисьмоИсходящееФСГС Тогда
						ПредставлениеЯчейки = "Электронно-цифровая подпись представителя ТОГС";
					Иначе
						ПредставлениеЯчейки = "Электронно-цифровая подпись представителя ФНС";
					КонецЕсли;
				КонецЕсли;
			КонецЕсли;
...
&НаКлиенте
Процедура СформироватьПакетСИнициализацией(СсылкаНаОбъект, Оповещать, ВыполняемоеОповещение)
	...
	ДополнительныеПараметры = Новый Структура("СсылкаНаОбъект, Оповещать, ВыполняемоеОповещение", СсылкаНаОбъект, Оповещать, ВыполняемоеОповещение);
	ОписаниеОповещения = Новый ОписаниеОповещения("СформироватьПакетСИнициализациейЗавершение", ЭтотОбъект, ДополнительныеПараметры);
	Если ЭтоЭлектроннаяПодписьВМоделиСервиса Тогда
		КриптосервисВМоделиСервисаКлиент.ИнициализацияСАвторизацией(УчетнаяЗапись, ЭтаФорма, ОписаниеОповещения);
	Иначе
...
&НаКлиенте
Процедура СформироватьПакетСИнициализациейЗавершение(Результат, ДополнительныеПараметры) Экспорт
	
	ВыполняемоеОповещение = ДополнительныеПараметры.ВыполняемоеОповещение;
	СсылкаНаОбъект = ДополнительныеПараметры.СсылкаНаОбъект;
	Оповещать = ДополнительныеПараметры.Оповещать;
	
	Если Результат Тогда
		ПакетСформирован = КонтекстЭДОКлиент.СформироватьПакет(СсылкаНаОбъект, Оповещать);
		ВыполнитьОбработкуОповещения(ВыполняемоеОповещение, ПакетСформирован);
	Иначе
...
Процедура КомандаРасшифроватьТСЗавершение(Ответ, ДополнительныеПараметры) Экспорт
	...
	Если Ответ = КодВозвратаДиалога.Да Тогда
		УчетнаяЗапись = Объект.УчетнаяЗапись;
		ЭтоЭлектроннаяПодписьВМоделиСервиса = ЭлектроннаяПодписьВМоделиСервисаВызовСервера.РеквизитыУчетнойЗаписи(УчетнаяЗапись).ЭлектроннаяПодписьВМоделиСервиса; 
		ОписаниеОповещения = Новый ОписаниеОповещения("КомандаРасшифроватьТСПослеИнициализацииЗавершение", ЭтотОбъект);
		Если ЭтоЭлектроннаяПодписьВМоделиСервиса Тогда
			КриптосервисВМоделиСервисаКлиент.ИнициализацияСАвторизацией(УчетнаяЗапись, ЭтаФорма, ОписаниеОповещения);
		Иначе
			ДокументооборотСКОКлиент.ИнициализацияВК(ОписаниеОповещения);
...
&НаКлиенте
Процедура КомандаРасшифроватьТСПослеИнициализацииЗавершение(Результат, ДополнительныеПараметры) Экспорт
...
// КомандаВыгрузитьВложения()
&НаКлиенте
Процедура КомандаВыгрузитьВложения(Команда)
	
	#Если ВебКлиент Тогда 
		
		ОписаниеОповещения = Новый ОписаниеОповещения("КомандаВыгрузитьВложенияВебЗавершение", ЭтотОбъект);
	...
	#Иначе
		...
		КомандаВыгрузитьВложенияВыгрузка(Диалог, 1, КоличествоСохраненных);
...
&НаКлиенте
Процедура КомандаВыгрузитьВложенияВебЗавершение(Результат, ДополнительныеПараметры) Экспорт
	...
	Если ПоддерживаетсяИспользованиеРасширенияРаботыСФайлами Тогда 
		...
		Для Каждого ЭлементВложения Из ФормаВложения Цикл
			ОписаниеФайла = Новый ОписаниеПередаваемогоФайла(ЭлементВложения.ИмяФайла, ЭлементВложения.Адрес);
		...
		МассивВызовов[0].Добавить(ПолученныеФайлы);
		
		Если ЗапроситьРазрешениеПользователя(МассивВызовов) Тогда
			ПолучитьФайлы(ПолучаемыеФайлы, ПолученныеФайлы, Диалог.Каталог, Ложь);
		КонецЕсли;
		КоличествоВложений = ФормаВложения.Количество();
		КоличествоСохраненных = ПолученныеФайлы.Количество();
		Если КоличествоВложений <> 0 Тогда
			СообщениеТекст = ВернутьСтр("ru='Выгружено файлов: '") + КоличествоСохраненных + ".";
			Если КоличествоСохраненных <> КоличествоВложений Тогда
				СообщениеТекст = СообщениеТекст + Символы.ПС + ВернутьСтр("ru='Не удалось выгрузить: '") + (КоличествоВложений - КоличествоСохраненных) + ".";
			КонецЕсли;
	...
	Иначе 
		...
		Для Каждого ЭлементПакета Из ФормаВложения Цикл
			Попытка
				ПолучитьФайл(ЭлементПакета.АдресФайла, ЭлементПакета.ИмяФайла, Истина);
			Исключение
			КонецПопытки;
		КонецЦикла;
	КонецЕсли;
...
// Процедура, содержащая рекурсивный вызов, заменяет(вместе с процедурой-завершением) цикл "Для каждого" с модальным вопросом внутри
&НаКлиенте
Процедура КомандаВыгрузитьВложенияВыгрузка(Диалог, НомерВложения, КоличествоСохраненных)
	
	Если НомерВложения > ФормаВложения.Количество() Тогда
		// все вложения обработаны, переходим в следующую процедуру
		КомандаВыгрузитьВложенияЗавершение(КоличествоСохраненных);
	Иначе
		// обрабатываем следующее вложение
		Вложение = ФормаВложения[НомерВложения - 1];
		...
		Если ФайлВложения.Существует() Тогда
			ДополнительныеПараметры = Новый Структура("Диалог, НомерВложения, КоличествоСохраненных, Вложение, ИмяФайлаВложения", Диалог, НомерВложения, КоличествоСохраненных, Вложение, ИмяФайлаВложения);
			ОписаниеОповещения = Новый ОписаниеОповещения("КомандаВыгрузитьВложенияВыгрузкаЗавершение", ЭтотОбъект, ДополнительныеПараметры);
			ПоказатьВопрос(ОписаниеОповещения,"Файл """ + Вложение.ИмяФайла + """ существует. Заменить?", РежимДиалогаВопрос.ДаНетОтмена);
			Возврат;
		КонецЕсли;
		Попытка
			//Вложение.Данные.Получить().Записать(ИмяФайлаВложения);
			ПолучитьИзВременногоХранилища(Вложение.Адрес).Записать(ИмяФайлаВложения);
			КоличествоСохраненных = КоличествоСохраненных + 1;
		Исключение
			ТекстСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(ВернутьСтр("ru='Не удалось выгрузить на диск файл %1! %2'"), Символ(34) + Вложение.ИмяФайла + Символ(34), Символы.ПС + ОписаниеОшибки());
			ОбщегоНазначенияКлиентСервер.СообщитьПользователю(ТекстСообщения);
		КонецПопытки;
		// переходим к следующей итерации
		КомандаВыгрузитьВложенияВыгрузка(Диалог, НомерВложения + 1, КоличествоСохраненных);
	КонецЕсли;

КонецПроцедуры

&НаКлиенте
Процедура КомандаВыгрузитьВложенияВыгрузкаЗавершение(Ответ, ДополнительныеПараметры) Экспорт
	
	Диалог = ДополнительныеПараметры.Диалог;
	НомерВложения = ДополнительныеПараметры.НомерВложения;
	КоличествоСохраненных = ДополнительныеПараметры.КоличествоСохраненных;
	Вложение = ДополнительныеПараметры.Вложение;
	ИмяФайлаВложения = ДополнительныеПараметры.ИмяФайлаВложения;
...
&НаКлиенте
Процедура КомандаВыгрузитьВложенияЗавершение(КоличествоСохраненных)
...
Процедура КомандаКонтейнер(Команда)
	...
	Для Каждого Контейнер Из ФормаКонтейнеры Цикл 
		МассивИменФайлов.Добавить(Контейнер.ИмяФайла);
	...
	КонтекстЭДОКлиент.ВыгрузитьВложения(МассивОписанийПолучаемыеФайлы, МассивИменФайлов, 
		ВернутьСтр("ru = 'Выберите каталог для сохранения транспортного контейнера'"));
...
Процедура ОткрытьВложение(ВложениеДляОткрытия)
	...
	#Если ВебКлиент Тогда 
		...
		// Веб клиент без расширения для работы с файлами
		Попытка
			ПолучитьФайл(ВложениеДляОткрытия.Значение, ВложениеДляОткрытия.Представление, Истина);
		Исключение
					ТекстСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(ВернутьСтр("ru='Ошибка сохраненения данных из информационной базы 
					|в локальную файловую систему пользователя: %1'"), Символы.ПС + ИнформацияОбОшибке().Описание);
					ОбщегоНазначенияКлиентСервер.СообщитьПользователю(ТекстСообщения);
		КонецПопытки;
	...
	#Иначе
		...
		Попытка
			ПолучитьИзВременногоХранилища(ВложениеДляОткрытия.Значение).Записать(ИмяВременногоФайла);
		Исключение
			ТекстСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(ВернутьСтр("ru='Ошибка выгрузки вложения во временный каталог: %1'"), Символы.ПС + ИнформацияОбОшибке().Описание);
			ОбщегоНазначенияКлиентСервер.СообщитьПользователю(ТекстСообщения);
			Возврат;
		КонецПопытки;
...
Процедура НадписьСостояниеСообщенияНажатиеЗавершение(Ответ, ДополнительныеПараметры) Экспорт
	...
	Если Ответ = КодВозвратаДиалога.Да Тогда
		Подтверждение = КонтекстЭДОКлиент.ВвестиПодтверждение(Объект.ЦиклОбмена, Объект.Ссылка);
		Если Подтверждение <> Неопределено Тогда 
			ОткрытьФорму("Документ.ТранспортноеСообщение.ФормаОбъекта", Новый Структура("Ключ", Подтверждение));
			ОбновитьДанные();
		КонецЕсли;
	КонецЕсли;
...
Процедура ПоказатьОкноПодписанияДокумента(ВыбраннаяСтрока)
	...
	//ФормаПодписей = ПолучитьФорму("Обработка.ДокументооборотСКонтролирующимиОрганами.Форма.ПодписиПодДокументами", ПараметрыФормы, Новый УникальныйИдентификатор);
	//ФормаПодписей = ПолучитьФорму(КонтекстЭДОКлиент.ПутьКОбъекту + ".Форма.ПодписиПодДокументами", ПараметрыФормы, Новый УникальныйИдентификатор);
...
Процедура КомандаЗашифровать(Команда)
	...
	СформироватьПакетСИнициализацией(Объект.Ссылка, Истина, ОписаниеОповещения);
...
////////////////////////////////////////////////////////////////////////////////
// ПРОЦЕДУРЫ И ФУНКЦИИ ПОКАЗА РАЗЛИЧНЫХ ВЛОЖЕНИЙ (ПФР, ФНС 534 и Росстата)

&НаКлиенте
Процедура ПоказатьПодписьПФР(ВыбраннаяСтрокаИД)
	
	ОписаниеОповещения = Новый ОписаниеОповещения("ПоказатьПодписьПФРПослеКрипто", ЭтотОбъект, ВыбраннаяСтрокаИД);
	
	ЭтоЭлектроннаяПодписьВМоделиСервиса = ЭлектроннаяПодписьВМоделиСервисаВызовСервера.ЭтоЭлектроннаяПодписьВМоделиСервиса(Объект);
	Если ЭтоЭлектроннаяПодписьВМоделиСервиса Тогда
...
&НаКлиенте
Процедура ПоказатьПодписьПФРПослеКрипто(Результат, ВыбраннаяСтрокаИД) Экспорт
	...
	ОписаниеОповещения = Новый ОписаниеОповещения("ПоказатьПодписьПФРЗавершение", ЭтотОбъект);
...
&НаКлиенте
Процедура ПоказатьПодписьПФРЗавершение(Результат, ДополнительныеПараметры) Экспорт
...
Процедура ПоказатьXMLИзФайла(ИмяФайлаСодержимого, ИсходноеИмяФайла, ПолеНастроекПечати = Неопределено) Экспорт
	...
	СтруктураПараметров = Новый Структура("ИмяФайла, Содержимое, ЦиклОбмена, ПолеНастроекПечати", ИсходноеИмяФайла, ИмяФайлаСодержимого, Объект.ЦиклОбмена, ?(ПечатьВозможнаДляЦиклаОбмена, ПолеНастроекПечати, Неопределено));
	...
	ОткрытьФорму(КонтекстЭДОКлиент.ПутьКОбъекту + ".Форма.ПросмотрHTML", СтруктураПараметров,,,,, ОписаниеОповещения, РежимОткрытияОкнаФормы.БлокироватьОкноВладельца);
	
КонецПроцедуры


Процедура ПоказатьXMLИзФайлаЗавершение(Результат, ДополнительныеПараметры);
...
Процедура ПоказатьTXTИзФайла(ИмяФайлаСодержимого, ИсходноеИмяФайла, Кодировка = Неопределено, ПолеНастроекПечати = Неопределено) Экспорт
	...
	СтруктураПараметров = Новый Структура("ИмяФайла, Содержимое, Кодировка, ЦиклОбмена, ПолеНастроекПечати", ИсходноеИмяФайла, ИмяФайлаСодержимого, Кодировка, Объект.ЦиклОбмена, ?(ПечатьВозможнаДляЦиклаОбмена, ПолеНастроекПечати, Неопределено));
...
Функция ПолучитьСтруктуруВложений(СообщениеВладелецФайла, ИмяФайлаДокумента, ЭтоВЕБ = Ложь)
	...
	Если КонтекстЭДОСервер = Неопределено Тогда 
		...
		КонтекстЭДОСервер = ДокументооборотСКОВызовСервера.ПолучитьОбработкуЭДО(ТекстСообщения);
		Если КонтекстЭДОСервер = Неопределено Тогда 
			ТекстПредупреждения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(ВернутьСтр("ru='Вложения транспортного сообщения недоступны.%1'"),
...
Процедура ПоказатьПодтверждениеДатыОтправки(ВыбраннаяСтрока, ПечатьВозможна = Ложь) Экспорт
	...
	СтруктураПараметров = Новый Структура("Сообщение, ИмяФайлаПодтверждения, ЦиклОбмена, ПечатьВозможна", Сообщение, ИмяФайлаПодтверждения, Объект.ЦиклОбмена, ПечатьВозможнаДляЦиклаОбмена И ПечатьВозможна);
	ОткрытьФорму(КонтекстЭДОКлиент.ПутьКОбъекту + ".Форма.ПодтверждениеДатыОтправки", СтруктураПараметров,,,,,, РежимОткрытияОкнаФормы.БлокироватьОкноВладельца);
...
Процедура НадписьСостояниеРезультатПриемаНажатиеВопросЗавершение(Ответ, ДополнительныеПараметры) Экспорт
	...
	Если Ответ = КодВозвратаДиалога.Да Тогда
		РезультатПриема = КонтекстЭДОКлиент.СоздатьРезультатПриемаНаКлиентеСУчетомВопроса(Объект.Ссылка, Истина, Истина);
	ИначеЕсли Ответ = КодВозвратаДиалога.Нет Тогда
		РезультатПриема = КонтекстЭДОКлиент.СоздатьРезультатПриемаНаКлиентеСУчетомВопроса(Объект.Ссылка, Ложь, Истина);
	Иначе
		Возврат;
	КонецЕсли;
