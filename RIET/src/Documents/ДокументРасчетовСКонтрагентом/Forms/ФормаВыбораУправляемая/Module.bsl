...
Функция ПроверитьВедениеУчетаПоДокументамРасчетов(СчетУчета)
	...
	Для Каждого ВидСубконтоСчета из СчетУчета.ВидыСубконто Цикл

		Если НЕ ВидСубконтоСчета.ТолькоОбороты
		   И ВидСубконтоСчета.ВидСубконто = ПланыВидовХарактеристик.ВидыСубконтоХозрасчетные.ДокументыРасчетовСКонтрагентами Тогда
...
Процедура ОграничениеСпискаДокументовПоТипу(Результат, ТипыДокументов)
	...
	Для каждого СтрокаВыборки Из Результат Цикл
		...
		Иначе
			...
			СтрокаВыборки.СостояниеДокумента = ?(СтрокаВыборки.ПометкаУдаления, 1, ?(СтрокаВыборки.Проведен, 0, 2));
...
Функция ПолучитьДанныеСпискаПоРеквизитам(СтруктураОтбора)
	...
	Для Каждого ТипДокумента Из ТипыДокументов.Типы() Цикл
		...
		+", ВЫБОР КОГДА Док.ПометкаУдаления ТОГДА 1 ИНАЧЕ ВЫБОР КОГДА Док.Проведен ТОГДА 0 ИНАЧЕ 2 КОНЕЦ КОНЕЦ КАК СостояниеДокумента"
...
Процедура УправлениеФормой(Форма)
	...
	Если Форма.РежимОтбораДокументов = ПредопределенноеЗначение("Перечисление.РежимОтбораДокументов.ПоОстаткам") Тогда
		Форма.Элементы.ОстаткиОбороты.Заголовок = ВернутьСтр("ru = 'Остатки'");
		Форма.Элементы.СписокОстаток.Заголовок  = ВернутьСтр("ru = 'Остаток'");
		
	ИначеЕсли Форма.РежимОтбораДокументов = ПредопределенноеЗначение("Перечисление.РежимОтбораДокументов.ПоОборотам") Тогда
		Форма.Элементы.ОстаткиОбороты.Заголовок = ВернутьСтр("ru = 'Обороты'");
		Форма.Элементы.СписокОстаток.Заголовок  = ВернутьСтр("ru = 'Оборот'");
	Иначе
		Форма.Элементы.ОстаткиОбороты.Заголовок = ВернутьСтр("ru = 'Ост./об.'");
		Форма.Элементы.СписокОстаток.Заголовок  = ВернутьСтр("ru = 'Остаток (оборот)'");
	КонецЕсли;
...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если Параметры.Свойство("ПараметрыОбъекта") Тогда
		...
		Если Параметры.ПараметрыОбъекта.Свойство("КонПериода") Тогда
			Если Параметры.ПараметрыОбъекта.КонПериода = НачалоДня(Параметры.ПараметрыОбъекта.КонПериода) Тогда
				КонецПериода = КонецДня(Параметры.ПараметрыОбъекта.КонПериода);
			Иначе
				КонецПериода = Параметры.ПараметрыОбъекта.КонПериода - 1;
			КонецЕсли;
		КонецЕсли;
...
Процедура ОбработкаПроверкиЗаполненияНаСервере(Отказ, ПроверяемыеРеквизиты)
	...
	ИначеЕсли РежимОтбораДокументов = Перечисления.РежимОтбораДокументов.ПоОстаткам
		...
		ТекстСообщения = ВернутьСтр("ru = 'На счете не ведется учет по расчетным документам! Выбор по остаткам на счете не может использоваться.'");
