...
Процедура УстановитьДоступностьЭлементовExportReports()
	...
	Если ProcessLevelЕстьВУзле = Неопределено Тогда
		ProcessLevelЕстьВУзле = Ложь;
	КонецЕсли;
	...
	// { RGS AGorlenko 12/27/2018 1:05:26 PM - S-I-0006554 - для обмена также
	Если НЕ DHLProviders.Найти(СокрЛП(Объект.CCA.Код)) = Неопределено Тогда
		Возврат;
	КонецЕсли;
	
	Если (ИспользуетсяExportReports И ProcessLevelЕстьВУзле) ИЛИ НЕ Локи.Найти(СокрЛП(Объект.CCA.Код)) = Неопределено Тогда
	// } RGS AGorlenko 12/27/2018 1:05:49 PM - S-I-0006554 - для обмена также
	...
	// { RGS AGorlenko 12/27/2018 1:07:04 PM - S-I-0006554
	...
	//КонецЕсли;
	// } RGS AGorlenko 12/27/2018 1:07:06 PM - S-I-0006554
