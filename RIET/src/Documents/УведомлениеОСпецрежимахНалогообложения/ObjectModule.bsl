	...
	Если ЗначениеЗаполнено(Титульный) Тогда
		Для Каждого Колонка Из Титульный.Владелец().Колонки Цикл 
			Если СтрНайти(Колонка.Имя, "П_ИНН") = 1 Тогда
				ОсновныеСведения.Вставить("ИННФЛ", Титульный[Колонка.Имя]);
				Прервать;
			КонецЕсли;
		КонецЦикла;
	ИначеЕсли Данные.Свойство("П_ИНН") Тогда
		ОсновныеСведения.Вставить("ИННФЛ", Данные.П_ИНН);
	Иначе
		ОсновныеСведения.Вставить("ИННФЛ", СведИП.ИНН);
	КонецЕсли;
...
Функция ОсновныеСведенияЭлектронногоПредставления_ИзменениеОбъектаУСН()
	...
	ЗаполнитьОбщиеДанные(ОсновныеСведения);
	ОсновныеСведения.Вставить("ВерсФорм", ?(ОсновныеСведения.ЭтоПБОЮЛ,"5.01","5.02"));
	Данные = ДанныеУведомления.Получить();
...
Функция ПечатьСразу(ПФР_ФСС = 0) Экспорт 
	...
	Если НЕ РегламентированнаяОтчетность.ПринтерДоступен() Тогда
		Сообщение = Новый СообщениеПользователю;
		Сообщение.Текст = ВернутьСтр("ru='Перед формированием печатных форм необходимо определить в системе принтер и задать его в качестве используемого по умолчанию!'");
		Сообщение.Сообщить();
		Возврат Неопределено;
	КонецЕсли;
