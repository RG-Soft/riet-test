...
Процедура ЗагрузитьДанные()
	...
	Если ЗначениеЗаполнено(СтруктураПараметров) Тогда 
		Для Каждого КлючИЗначение Из СтруктураПараметров Цикл
			Если КлючИЗначение.Ключ = "ДАТА_ПРЕКРАЩЕНИЯ" Тогда
				ДатаПрекращенияДеятельности = СтруктураПараметров.ДАТА_ПРЕКРАЩЕНИЯ;
				ПредставлениеУведомления.Области["ДАТА_УТРАТЫ_ПРАВА"].Значение = ДатаПрекращенияДеятельности;
...
Процедура СохранитьДанные()
	...
	СтруктураПараметров.Вставить("ДАТА_ПРЕКРАЩЕНИЯ", ДатаПрекращенияДеятельности);
	СтруктураПараметров.Вставить("ПРИЛОЖЕНО_ЛИСТОВ", ПредставлениеУведомления.Области["ПРИЛОЖЕНО_ЛИСТОВ"].Значение);
	СтруктураПараметров.Вставить("ПОДПУНКТ1", ПредставлениеУведомления.Области["ПОДПУНКТ1"].Значение);
	СтруктураПараметров.Вставить("ПУНКТ1", ПредставлениеУведомления.Области["ПУНКТ1"].Значение);
	СтруктураПараметров.Вставить("СТАТЬЯ1", ПредставлениеУведомления.Области["СТАТЬЯ1"].Значение);
	СтруктураПараметров.Вставить("НАЛОГОВЫЙ_ПЕРИОД1", ПредставлениеУведомления.Области["НАЛОГОВЫЙ_ПЕРИОД1"].Значение);
	СтруктураПараметров.Вставить("НАЛОГОВЫЙ_ПЕРИОД2", ПредставлениеУведомления.Области["НАЛОГОВЫЙ_ПЕРИОД2"].Значение);
	СтруктураПараметров.Вставить("ГОД1", ПредставлениеУведомления.Области["ГОД1"].Значение);
	СтруктураПараметров.Вставить("ГОД2", ПредставлениеУведомления.Области["ГОД2"].Значение);
...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Макет = Документы.УведомлениеОСпецрежимахНалогообложения.ПолучитьМакет("УведомлениеОбУтратеПраваНаУСН");
	...
	Документы.УведомлениеОСпецрежимахНалогообложения.ЗагрузитьМакетыПростойФормы(ЭтаФорма, "УведомлениеОбУтратеПраваНаУСНПараметры");
...
Процедура ПредставлениеУведомленияВыбор(Элемент, Область, СтандартнаяОбработка)
	...
	ИначеЕсли ОсобаяОбласть.Значение = "ДАТА_УТРАТЫ_ПРАВА" Тогда 
		
		СтандартнаяОбработка = Ложь;
		НестандартнаяОбработка_ДАТА_УТРАТЫ_ПРАВА();
...
Процедура ПередЗакрытием(Отказ, СтандартнаяОбработка)
	...
	Если Модифицированность Тогда
		...
		ТекстВопроса = ВернутьСтр("ru='Уведомление изменено. Сохранить изменения?'");
...
Процедура НестандартнаяОбработка_ДАТА_ПОДПИСИ()
	НестандартнаяОбработкаДаты("Дата подписи уведомления", Истина, "ДАТА_ПОДПИСИ");
КонецПроцедуры

&НаКлиенте
Процедура НестандартнаяОбработка_ДАТА_УТРАТЫ_ПРАВА()
	НестандартнаяОбработкаДаты("Дата перехода", Ложь, "ДАТА_УТРАТЫ_ПРАВА");
КонецПроцедуры

&НаКлиенте
Процедура НестандартнаяОбработкаДаты(ИмяФормы, ДатаПодписи, ИмяОбласти)
	ДатаДляМодификации = ?(ДатаПодписи, Объект.ДатаПодписи, ДатаПрекращенияДеятельности);
	ПараметрыДаты = Новый Структура("ДатаПодписи", ДатаДляМодификации);
	...
	ДополнительныеПараметры = Новый Структура("ДатаПодписи, ИмяОбласти, ФормаДаты", ДатаПодписи, ИмяОбласти, ФормаДаты);
...
Процедура НестандартнаяОбработкаДатыЗавершение(Результат, ДополнительныеПараметры) Экспорт
	...
	ДатаПодписи = ДополнительныеПараметры.ДатаПодписи;
	...
	Если Результат = КодВозвратаДиалога.Да Тогда
		...
		Если ЗначениеЗаполнено(ДатаРезультат) Тогда 
			ПредставлениеУведомления.Области[ИмяОбласти].Значение = ДатаРезультат;
		Иначе
			ПредставлениеУведомления.Области[ИмяОбласти].Значение = "";
		КонецЕсли;
		
		Если ДатаПодписи Тогда 
			Объект.ДатаПодписи = ДатаРезультат;
		Иначе
			ДатаПрекращенияДеятельности = ДатаРезультат;
		КонецЕсли;
		Модифицированность = Истина;
	КонецЕсли;
