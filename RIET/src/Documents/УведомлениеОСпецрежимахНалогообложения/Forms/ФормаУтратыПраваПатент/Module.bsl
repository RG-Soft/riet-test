...
Процедура СформироватьМакетНаСервере()
	Документы.УведомлениеОСпецрежимахНалогообложения.СформироватьМакетНаСервере(ЭтаФорма, "ЗаявлениеОбУтратеПраваНаПатент", ПолучитьИмяОбласти(ТекущийТипСтраницы), ПолучитьИмяТаблицы(ТекущийТипСтраницы));
КонецПроцедуры
...
Процедура ОбработкаДаты(Инфо)
	...
	Если ИмяПоля = "ДАТА_УТРАТЫ_ПРАВА" Или ИмяПоля = "ДАТА_ПОДПИСИ" Тогда  
		ИмяОбласти = ИмяПоля;
		ТекущиеДанные = ТитульнаяСтраница[0];
	ИначеЕсли СтрНайти(ИмяПоля, "ДАТА_НАЧАЛА_") = 1 Или СтрНайти(ИмяПоля, "ДАТА_ОКОНЧАНИЕ_") = 1 Тогда
		ИмяОбласти = ИмяПоля;
		ИмяТаблицы = ПолучитьИмяТаблицы(ТекущийТипСтраницы);
		ПараметрыОтбора = Новый Структура("UID", ТекущийИдентификаторСтраницы);
		ТекущиеДанные = ЭтаФорма[ИмяТаблицы].НайтиСтроки(ПараметрыОтбора)[0];
	Иначе
		Возврат;
	КонецЕсли;
	
	ДатаИсх = ПредставлениеУведомления.Области[ИмяОбласти].Значение;
	...
	ДополнительныеПараметры = Новый Структура("ИмяОбласти, ФормаДаты, ТекущиеДанные", ИмяОбласти, ФормаДаты, ТекущиеДанные);
...
Процедура ОбработкаДатыЗавершение(Результат, ДополнительныеПараметры) Экспорт
	...
	Если Результат = КодВозвратаДиалога.Да Тогда
		...
		Если ЗначениеЗаполнено(ДатаРезультат) Тогда 
			...
			ТекущиеДанные[ИмяОбласти] = ДатаРезультат;
		Иначе
			ПредставлениеУведомления.Область(ИмяОбласти).Значение = "";
			ТекущиеДанные[ИмяОбласти] = Неопределено;
		КонецЕсли;
...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	ЗагрузитьДанные();
	Документы.УведомлениеОСпецрежимахНалогообложения.ЗагрузитьМакеты(ЭтаФорма, "ЗаявлениеОбУтратеПраваНаПатентПараметры");
...
Функция СформироватьВыгрузкуИПолучитьДанные() Экспорт 
	...
	Возврат Новый Структура("ТестВыгрузки,КодировкаВыгрузки,Данные,ИмяФайла", 
			...
			Документы.УведомлениеОСпецрежимахНалогообложения.ПолучитьМакет("УтратаПраваНаПатент_TIFF"),
