...
Процедура СоздатьУведомление(Команда)
	...
	Если ЗначениеЗаполнено(ИмяФормыУведомления) Тогда 
		ПараметрыОткрытия = Новый Структура("Организация, НалоговыйОрган", Организация, РегистрацияВИФНС);
		Форма = ПолучитьФорму(ИмяФормыУведомления, ПараметрыОткрытия, ЭтаФорма.ВладелецФормы, ИмяФормыУведомления);
		Форма.Открыть();
		Закрыть();
	КонецЕсли;
