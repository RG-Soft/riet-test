...
Процедура ПроверитьЗаполнитьWarehouseTo(Отказ)	
	Если DocВключенВПоставку() Тогда
		Возврат;
	КонецЕсли;
	MOTsSEA = Справочники.MOTs.НайтиПоКоду("SEA");
	...
	СоответствиеMotWHTo.Вставить(MOTsSEA, Справочники.Warehouses.НайтиПоКоду("RUS_SPBM"));
	
	//ОсобыйProject = Справочники.ProjectMobilization.НайтиПоКоду("000000634");
	ЗакрепленныйСкладStockUpProjectSEA = Справочники.Warehouses.НайтиПоКоду("RUS_SPBM");
	...
	Пока ВыборкаЗаписиПарсели.Следующий() Цикл
		...
		Если ЕстьИзСписка Тогда
			...
			Иначе
				Если MOT = MOTsSEA Тогда
					мWHTo = ЗакрепленныйСкладStockUpProjectSEA;
				Иначе
