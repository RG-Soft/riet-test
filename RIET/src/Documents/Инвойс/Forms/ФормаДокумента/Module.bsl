...
Процедура ContractНачалоВыбора(Элемент, ДанныеВыбора, СтандартнаяОбработка)
	...
	// Для НЕ Голдовских инвойсов не проверяем реквизиты, т.к. они будут заполняться по Контракту
	Если Объект.Голд Тогда
	// } RGS ASeryakov 26.06.2018 15:00:00 PM S-I-0004945
...
Процедура ContractОбработкаВыбора(Элемент, ВыбранноеЗначение, СтандартнаяОбработка)
	...
	Если Объект.Ссылка.Пустая() Тогда
		...
		ДанныеПоКонтрактуОбъекта = Новый Структура("Seller,ConsignTo,Incoterms",
		ПредопределенноеЗначение("Справочник.SoldTo.ПустаяСсылка"),
		ПредопределенноеЗначение("Справочник.ConsignTo.ПустаяСсылка"),
		ПредопределенноеЗначение("Справочник.Incoterms.ПустаяСсылка"));
	...
	Если (ЗначениеЗаполнено(ВыбранноеЗначение) ИЛИ Объект.Ссылка.Пустая()) И НЕ ОбщегоНазначения.ДанныеСовпадают(ДанныеПоКонтрактуОбъекта, ДанныеПоКонтрактуВыбора) Тогда
