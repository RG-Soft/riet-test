...
Процедура ПослеЗаписи(ПараметрыЗаписи)
	...
	//Исправила условие Федотова Л., РГ-Софт, 23.05.13, вопрос SLI-0003591
	//Если  НЕ ВладелецФормы = Неопределено И НЕ ВладелецФормы.Имя = "Список" Тогда
	Если  НЕ ВладелецФормы = Неопределено И НЕ Тип(ВладелецФормы) = Тип("ТабличноеПоле") И НЕ ВладелецФормы.Имя = "Список" Тогда
...
Процедура ДатаПриИзменении(Элемент)
	...
	Если НЕ (УчетНДС.ПолучитьВерсиюПостановления(Объект.Дата) = 2) Тогда
		...
		//Если Объект.ВидСчетаФактуры = ПредопределенноеЗначение("Перечисление.ВидСчетаФактурыВыставленного.НаАвансКомитента") Тогда //13.09.2012 Пахоменков. Это условие никогда не выполниться. т.к. таког перечисления нет в конфе
		//	Объект.ВидСчетаФактуры = ПредопределенноеЗначение("Перечисление.ВидСчетаФактурыВыставленного.НаАванс");
		//КонецЕсли;	
	КонецЕсли;
...
Процедура ДокументОснованиеПриИзменении(Элемент)
	...
	Если ЗначениеЗаполнено(Объект.ДокументОснование) Тогда
		ЗаполнитьСчетФактуруНаАвансСервер(Объект.ДокументОснование);
		Объект.КодВидаОперации	= ПолучитьКодВидаОперации();
	КонецЕсли;
...
&НаКлиенте
Процедура ВидСчетаФактурыНачалоВыбора(Элемент, ДанныеВыбора, СтандартнаяОбработка)
	
	СтандартнаяОбработка = Ложь;
	
	СписокВыбора = Новый СписокЗначений;
	СписокВыбора.Добавить(ПредопределенноеЗначение("Перечисление.ВидСчетаФактурыВыставленного.НаАванс"));
	СписокВыбора.Добавить(ПредопределенноеЗначение("Перечисление.ВидСчетаФактурыВыставленного.НаАвансКомитента")); 
	
	ТекущийВид = СписокВыбора.НайтиПоЗначению(Объект.ВидСчетаФактуры);
	ВыбранныйВид = ВыбратьИзСписка(СписокВыбора, Элемент, ТекущийВид);
	Если ВыбранныйВид <> Неопределено Тогда
		Объект.ВидСчетаФактуры = ВыбранныйВид.Значение;
	КонецЕсли;
	
	Если ТекущийВид <> ВыбранныйВид Тогда
		Объект.КодВидаОперации	= ПолучитьКодВидаОперации();
		//Закомментировала Федотова Л., РГ-Софт, 11.12.12, вопрос №SLI-0003079
...
Процедура ПодготовитьФормуНаСервере()
	...
	Элементы.ПредставлениеВалюты.СписокВыбора.Очистить();
...
Процедура УправлениеФормой(Форма)
	...
	ПоОтчетуКомиссионера = ТипЗнч(Объект.ДокументОснование) = Тип("ДокументСсылка.ОтчетПринципалуОПродажах");
	
	Элементы.Организация.Доступность	    = НЕ УказанДокументОснование;
	Элементы.Контрагент.Доступность		    = НЕ УказанДокументОснование;
	Элементы.ДокументОснование.Доступность  = НЕ (УказанДокументОснование И ПоОтчетуКомиссионера);
	
	Элементы.ГруппаИсправление.Доступность				= (УчетНДС.ПолучитьВерсиюПостановления(Объект.Дата) = 2);
	Элементы.ГруппаИсходныйДокумент.Доступность			= (УчетНДС.ПолучитьВерсиюПостановления(Объект.Дата) = 2);
	...
	//Элементы.Комитент.Доступность = (УчетНДС.ПолучитьВерсиюПостановления(Объект.Дата) = 2) 
	//	И Объект.ВидСчетаФактуры = ПредопределенноеЗначение("Перечисление.ВидСчетаФактурыВыставленного.НаАвансКомитента");
		
	Элементы.ВидСчетаФактуры.Доступность = (УчетНДС.ПолучитьВерсиюПостановления(Объект.Дата) = 2);
	
	СчетФактураНаАвансОтКонечногоПокупателя = УказанДокументОснование И ТипЗнч(Объект.ДокументОснование) = Тип("ДокументСсылка.ОтчетПринципалуОПродажах");
	
	Элементы.ДоговорКонтрагента.Доступность = НЕ СчетФактураНаАвансОтКонечногоПокупателя;
	Элементы.ДоговорКонтрагента.ОтметкаНезаполненного = НЕ СчетФактураНаАвансОтКонечногоПокупателя;	
...
&НаСервере
Функция ЗаполнитьСчетФактуруНаАвансСервер(Знач ДокументОснование)
...
Процедура УстановитьЗаголовокФормы(Форма)
	...
	Если ЗначениеЗаполнено(Объект.Ссылка) Тогда
		Форма.Заголовок = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru='Счет-фактура выданный %1 от %2 - аванс'"),
			Объект.Номер,
			Объект.Дата);
	Иначе
		Форма.Заголовок = НСтр("ru='Счет-фактура выданный (создание) - аванс'");
	КонецЕсли;
