...
//	КонецЕсли;
//	
//	СтрокаРеквизиты = "РегистрационныйНомерФСС,ДополнительныйКодФСС,КодПодчиненностиФСС,НаименованиеТерриториальногоОрганаФСС,Руководитель,ГлавныйБухгалтер,ДолжностьРуководителя,ТелефонОрганизации,ОГРН,ИНН,КПП";
//	
...
//	
//	РеквизитыЗаявлений = ОбщегоНазначения.ЗначенияРеквизитовОбъектов(МассивЗаявлений, "ВидПособия,ТипЗанятостиРаботника,ДатаНачалаТрудовогоДоговораПолучателя,ДатаОкончанияТрудовогоДоговораПолучателя");
//	
...
//				И Строка.КодПричиныНетрудоспособности <> "05" Тогда
...
//			КонецЕсли;
//			Если Не ЗначениеЗаполнено(Строка.НомерПутевки) Тогда
//				Текст  = Нстр("ru = 'Если указана дата начала санаторной путевки, то должен быть указан ее номер.'");
//				ДобавитьОшибкуПользователю(Ошибки, "НомерПутевки", Текст, Группа, Строка.НомерСтроки);
//			КонецЕсли;
...
//	ОбщегоНазначенияКлиентСервер.ДобавитьОшибкуПользователю(Ошибки, "Объект.СведенияНеобходимыеДляНазначенияПособий[%1]." + Поле, Текст, ГруппаОшибок, НомерСтроки - 1, Текст);	
//КонецПроцедуры	
