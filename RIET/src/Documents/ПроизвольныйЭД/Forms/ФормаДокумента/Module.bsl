...
Процедура ВложенияПередУдалением(Элемент, Отказ)
	...
	Если НЕ (Объект.СтатусДокумента = ПредопределенноеЗначение("Перечисление.СтатусыЭД.ПодготовленКОтправке")
		...
		Отказ = Истина;
	Иначе
		ТекстВопроса = ВернутьСтр("ru = 'Удалить вложение из документа?'");
		Если ЭП.Количество() > 0 Тогда
			ТекстВопроса = ВернутьСтр("ru = 'При удалении вложения будут удалены установленные подписи.'") + Символы.ПС + ТекстВопроса;
		КонецЕсли;
...
Процедура ПоказатьРезультатПодписи(Подписан)
	...
	ТекстСостояния = ВернутьСтр("ru = 'Подписано: (0)'");
	...
	Если Подписан Тогда
		...
		ТекстСостояния = ВернутьСтр("ru = 'Подписано: (1)'");
	КонецЕсли;
...
Процедура Отклонить(Команда)
	...
	Если НЕ ЭДОтклонен И Вложения.Количество() > 0 Тогда
		ОбработкаПродолжения = Новый ОписаниеОповещения("НачатьОтклонениеДокумента", ЭтотОбъект);
		ПроверитьЗаписьДокумента(ОбработкаПродолжения);
	КонецЕсли;
...
Процедура ПроверитьПодписи(Команда)
	...
	Если Вложения.Количество() > 0 Тогда
		Если ЭлектронныеДокументыСлужебныйВызовСервера.ВыполнятьКриптооперацииНаСервере() Тогда
			ЭлектронныеДокументыСлужебныйВызовСервера.ОпределитьСтатусыПодписей(Вложения[0].Ссылка, Истина);
		Иначе
...
Процедура УстановитьДоступностьЭлементов()
	...
	Элементы.Подписать.Доступность             = (НЕ (СтатусПередан ИЛИ ЭДОтклонен) И ЕстьВложения И (НаправлениеИсходящий ИЛИ ТребуетсяПодтверждение));
	Элементы.ПодписатьИОтправитьЭД.Доступность = (СтатусГотов И ЕстьВложения И (НаправлениеИсходящий ИЛИ ТребуетсяПодтверждение));
	
	КоманднаяПанель.ПодчиненныеЭлементы.ПодписатьИОтправитьЭД.Заголовок = ВернутьСтр(
	...
	Если ЭДОтклонен Тогда
		...
		НовСтрока.ПричинаОтклонения = Вложения[0].Ссылка.ПричинаОтклонения;
	...
	Иначе
		...
		Элементы.ФормаОтклонить.Доступность = ?(СтатусПередан ИЛИ НЕ ЕстьВложения, Ложь, Истина);
...
Процедура ОбновитьЗаголовокФормы()
	...
	Если Объект.Направление = Перечисления.НаправленияЭД.Исходящий Тогда
		ТекстЗаголовка = ВернутьСтр("ru = 'Исходящий документ %1 от %2'");
		Заголовок = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			ТекстЗаголовка, Объект.Номер, Объект.Дата);
	Иначе
		ТекстЗаголовка = ВернутьСтр("ru = 'Входящий документ %1 от %2'");
		Заголовок = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			ТекстЗаголовка, Объект.Номер, Объект.Дата);
	КонецЕсли;
...
Функция ПолучитьМассивЭДДляОтправки(Отказ)
	...
	Если ПрофильНастроекЭДО.СпособОбменаЭД = Перечисления.СпособыОбменаЭД.ЧерезОператораЭДОТакском
		И ЭП.Количество() = 0 Тогда
		ТекстСообщения = ВернутьСтр("ru = 'Операция отменена. Необходимо подписать вложение.'");
...
Процедура ПослеОтправкиПЭД(Результат, ДополнительныеПараметры) Экспорт
	...
	Если ТипЗнч(Результат) = Тип("Структура") Тогда
		Если Результат.Свойство("КолОтправленных", КолОтправленных)
			...
			ТекстСостояния = ВернутьСтр("ru = 'Отправлено: (%1)'");
		...
		ИначеЕсли Результат.Свойство("КолПодготовленных", КолПодготовленных)
			...
			ТекстСостояния = ВернутьСтр("ru = 'Подготовлено к отправке: (%1)'");
...
Функция МожноОтклонитьДокумент()
	...
	Если Объект.Направление = Перечисления.НаправленияЭД.Исходящий
		И НЕ (Объект.СтатусДокумента = Перечисления.СтатусыЭД.Сформирован
...
Функция ЭтоПрямойОбмен()
	...
	Возврат ПрофильНастроекЭДО.СпособОбменаЭД <> Перечисления.СпособыОбменаЭД.ЧерезОператораЭДОТакском;
...
Процедура ПоказатьСертификат(НомерСтроки, Отпечаток)
	...
	Если ВыбранныйСертификат=Неопределено Тогда
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(ВернутьСтр("ru = 'Сертификат не найден.'"));
		Возврат;
	КонецЕсли;
...
Процедура ДобавитьСертификатВДоверенные(ДанныеПодписи)
	...
	Если ДанныеПодписи <> Неопределено И ДанныеПодписи.ОтсутствуетВСписке Тогда
		Текст = ВернутьСтр("ru = 'Добавить сертификат %1 в список ожидаемых сертификатов контрагента?'");
...
&НаКлиенте
Процедура ОбработатьОтклонениеЭД(Результат, ДополнительныеПараметры) Экспорт
	
	Если Результат = КодВозвратаДиалога.Да Тогда
		ОписаниеОповещения = Новый ОписаниеОповещения("ЗавершитьОбработкуОтклоненияЭД", ЭтотОбъект, ДополнительныеПараметры);;
		Подсказка = ВернутьСтр("ru = 'Укажите причины отклонения документа'");
...
&НаКлиенте
Процедура ЗавершитьОбработкуОтклоненияЭД(ТекстУточнения, ДополнительныеПараметры) Экспорт
	...
	Если ЗначениеЗаполнено(ТекстУточнения) Тогда
		...
		Если Объект.Направление = ПредопределенноеЗначение("Перечисление.НаправленияЭД.Исходящий")
			...
			ИЛИ ЭтоПрямойОбмен() Тогда // ограничение на прямой обмен
			ВыполнитьДействияПослеОтклоненияЭД(Истина, Новый Структура("ТекстУточнения", ТекстУточнения))
		Иначе
			ПараметрыОповещения = Новый Структура("ТекстУточнения", ТекстУточнения);
			ОписаниеОповещения = Новый ОписаниеОповещения("ВыполнитьДействияПослеОтклоненияЭД", ЭтотОбъект, ПараметрыОповещения);
			ВидЭД = ПредопределенноеЗначение("Перечисление.ВидыЭД.УведомлениеОбУточнении");
			ЭлектронныеДокументыСлужебныйКлиент.СформироватьПодписатьСлужебныйЭД(Вложения[0].Ссылка,
				ВидЭД, ТекстУточнения, ОписаниеОповещения);
		КонецЕсли;
	Иначе
		ТекстСообщения = ВернутьСтр("ru = 'Для отклонения документа необходимо указать причину отклонения!'");
...
Процедура ПроверитьЗаписьДокумента(ОбработкаПродолжения)
	...
	Если Модифицированность ИЛИ Объект.Ссылка.Пустая() Тогда
		ТекстВопроса = ВернутьСтр("ru = 'Для продолжения операции необходимо записать документ.
			|Записать документ?'");
...
&НаКлиенте
Процедура НачатьОтклонениеДокумента(Неопределен, ДополнительныеПараметры) Экспорт
	
	Если МожноОтклонитьДокумент() Тогда
		// Нельзя снять отклонение документа.
		ТекстВопроса = ВернутьСтр("ru = 'Для отклонения документа необходимо указать причину отклонения.
			|Отклонить документ?'");
		СписокКнопок = Новый СписокЗначений;
		СписокКнопок.Добавить(КодВозвратаДиалога.Да, ВернутьСтр("ru = 'Отклонить'"));
		СписокКнопок.Добавить(КодВозвратаДиалога.Нет, ВернутьСтр("ru = 'Не отклонять'"));
		ОписаниеОповещения = Новый ОписаниеОповещения("ОбработатьОтклонениеЭД", ЭтотОбъект);
		ЗаголовокВопроса = ВернутьСтр("ru = 'Отклонение электронного документа...'");
		ПоказатьВопрос(ОписаниеОповещения, ТекстВопроса, СписокКнопок, 60, КодВозвратаДиалога.Нет, ЗаголовокВопроса);
	КонецЕсли;
...
Процедура ЗакончитьДобавлениеФайла(Неопределен, ДополнительныеПараметры) Экспорт
	...
	Если Вложения.Количество() > 0 Тогда
		ТекстСообщения = ВернутьСтр("ru = 'Операция отменена. Добавить возможно только одно вложение.'");
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(ТекстСообщения);
		Возврат;
	КонецЕсли;
...
Процедура ВыполнитьДействияПослеОтклоненияЭД(Результат, ДополнительныеПараметры) Экспорт
	...
	Если Результат = Истина Тогда
		ТекстУточнения = "";
		ДополнительныеПараметры.Свойство("ТекстУточнения", ТекстУточнения);
