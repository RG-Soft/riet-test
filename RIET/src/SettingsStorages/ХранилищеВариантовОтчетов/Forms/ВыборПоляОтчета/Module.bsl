...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если Не Параметры.Свойство("Режим", Режим) Тогда
		ВызватьИсключение ВернутьСтр("ru = 'Не передан служебный параметр ""Режим"".'");
	КонецЕсли;
	...
	ИначеЕсли Режим = "Отборы" Или Режим = "ВыбранныеПоля" Или Режим = "Сортировка" Или Режим = "ПоляГруппировки" Тогда
		ИмяТаблицы = Режим;
	Иначе
		ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(ВернутьСтр("ru = 'Некорректное значение параметра ""Режим"": ""%1"".'"), Строка(Режим));
	КонецЕсли;
	...
	Если ИмяТаблицы = "ПоляГруппировки" Тогда
		...
		Если Режим = "СтруктураВарианта" Тогда
			...
			СтрокаДерева.Представление  = ВернутьСтр("ru = '<Детальные записи>'");
...
Процедура ВыбратьИЗакрыть()
	...
	Если ТипЗнч(ДоступноеПолеКД) = Тип("ДоступноеПолеКомпоновкиДанных")
		...
		Если ДоступноеПолеКД.Папка Тогда
			ПоказатьПредупреждение(, ВернутьСтр("ru = 'Выберите элемент'"));
			Возврат;
		КонецЕсли;
	КонецЕсли;
