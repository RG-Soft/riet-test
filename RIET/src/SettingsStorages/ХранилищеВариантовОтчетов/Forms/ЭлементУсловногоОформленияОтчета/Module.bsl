...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если Не Параметры.Свойство("КомпоновщикНастроек", КомпоновщикНастроек) Тогда
		ВызватьИсключение ВернутьСтр("ru = 'Не передан служебный параметр ""КомпоновщикНастроек"".'");
	КонецЕсли;
	...
	Если Не Параметры.Свойство("ИдентификаторТекущегоУзлаКД", ИдентификаторТекущегоУзлаКД) Тогда
		ВызватьИсключение ВернутьСтр("ru = 'Не передан служебный параметр ""ИдентификаторТекущегоУзлаКД"".'");
	КонецЕсли;
	Если Не Параметры.Свойство("ИдентификаторКД", ИдентификаторКД) Тогда
		ВызватьИсключение ВернутьСтр("ru = 'Не передан служебный параметр ""ИдентификаторКД"".'");
	КонецЕсли;
	Если Не Параметры.Свойство("Наименование", Наименование) Тогда
		ВызватьИсключение ВернутьСтр("ru = 'Не передан служебный параметр ""Наименование"".'");
	КонецЕсли;
	...
	Иначе
		...
		Если УзелКДИсточник = Неопределено Тогда
			ВызватьИсключение ВернутьСтр("ru = 'Не найден узел отчета.'");
		КонецЕсли;
		ЭлементКДИсточник = УзелКДИсточник.ПолучитьОбъектПоИдентификатору(ИдентификаторКД);
		Если ЭлементКДИсточник = Неопределено Тогда
			ВызватьИсключение ВернутьСтр("ru = 'Не найден элемент условного оформления.'");
		КонецЕсли;
...
Процедура ВыбратьИЗакрыть()
	...
	Если Наименование = "" Тогда
		Если ЭтоНовый Тогда
			ОбщегоНазначенияКлиентСервер.СообщитьПользователю(ВернутьСтр("ru = 'Необходимо заполнить заголовок'"), , "Наименование");
			Возврат;
		КонецЕсли;
